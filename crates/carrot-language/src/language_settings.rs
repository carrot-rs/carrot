...
use inazuma::{App, Modifiers, SharedString};
use inazuma_collections::{FxHashMap, HashMap, HashSet};
use inazuma_settings_framework::{
    DocumentFoldingRanges, DocumentSymbols, IntoInazuma, SemanticTokens,
};
...
use inazuma_settings_framework::{
    RegisterSetting, Settings, SettingsLocation, SettingsStore, merge_from::MergeFrom,
};
...
pub struct LanguageSettings {
    ...
    pub soft_wrap: inazuma_settings_framework::SoftWrap,
    ...
    pub formatter: inazuma_settings_framework::FormatterList,
    /// Carrot's Prettier integration settings.
    ...
    pub show_whitespaces: inazuma_settings_framework::ShowWhitespaceSetting,
...
pub struct IndentGuideSettings {
    ...
    ///
    /// Default: Fixed
    pub coloring: inazuma_settings_framework::IndentGuideColoring,
    /// Determines how indent guide backgrounds are colored.
    ///
    /// Default: Disabled
    pub background_coloring: inazuma_settings_framework::IndentGuideBackgroundColoring,
}
...
impl IndentGuideSettings {
    ...
    pub fn visible_line_width(&self, active: bool) -> Option<u32> {
        if self.coloring == inazuma_settings_framework::IndentGuideColoring::Disabled {
            return None;
        }
        let width = if active {
...
pub struct LanguageTaskSettings {
    ...
    pub enabled: bool,
    /// Use LSP tasks over Carrot language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// Carrot language extension tasks will be used instead.
    ///
    /// Other Carrot tasks will still be shown:
    /// * Carrot task from either of the task config file
    /// * Carrot task from history (e.g. one-off task was spawned before)
    pub prefer_lsp: bool,
}
...
pub struct EditPredictionSettings {
    ...
    pub provider: inazuma_settings_framework::EditPredictionProvider,
    ...
    pub mode: inazuma_settings_framework::EditPredictionsMode,
...
impl inazuma_settings_framework::Settings for AllLanguageSettings {
    fn from_settings(content: &inazuma_settings_framework::SettingsContent) -> Self {
        ...
        for (language_name, settings) in &all_languages.languages.0 {
            ...
            inazuma_settings_framework::merge_from::MergeFrom::merge_from(
