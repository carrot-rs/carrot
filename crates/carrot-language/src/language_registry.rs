...
use inazuma_collections::{FxHashMap, HashMap, HashSet, hash_map};
...
use globset::GlobSet;
use inazuma::{App, BackgroundExecutor};
use parking_lot::{Mutex, RwLock};
use postage::watch;

use carrot_theme::Theme;
use inazuma_sum_tree::Bias;
use inazuma_text::{Point, Rope};
use inazuma_util::{ResultExt, maybe, post_inc};
...
pub struct FakeLanguageServerEntry {
    ...
    pub tx: futures::channel::mpsc::UnboundedSender<carrot_lsp::FakeLanguageServer>,
    pub _server: Option<carrot_lsp::FakeLanguageServer>,
}
...
impl LanguageRegistry {
    ...
    ) -> futures::channel::mpsc::UnboundedReceiver<carrot_lsp::FakeLanguageServer> {
    ...
    pub fn register_fake_lsp_server(
        ...
        initializer: Option<Box<dyn Fn(&mut carrot_lsp::FakeLanguageServer) + Send + Sync>>,
    ) -> futures::channel::mpsc::UnboundedReceiver<carrot_lsp::FakeLanguageServer> {
    ...
    fn get_or_load_grammar(
        ...
        let span = carrot_tracing::debug_span!("get_or_load_grammar", name = &*name.clone());
    ...
    pub fn create_fake_language_server(
        ...
        name: &LanguageServerName,
        binary: carrot_lsp::LanguageServerBinary,
        cx: &mut inazuma::AsyncApp,
    ) -> Option<carrot_lsp::LanguageServer> {
        let mut state = self.state.write();
        let fake_entry = state.fake_server_entries.get_mut(name)?;

        let (server, mut fake_server) = carrot_lsp::FakeLanguageServer::new(
            server_id,
            binary,
            name.0.to_string(),
