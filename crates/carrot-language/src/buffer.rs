...
use carrot_theme::{ActiveTheme as _, SyntaxTheme};
use inazuma_settings_framework::WorktreeId;
use inazuma_sum_tree::TreeMap;
use inazuma_text::operation_queue::OperationQueue;
use inazuma_text::*;
pub use inazuma_text::{
...
use inazuma_util::{RangeExt, debug_panic, maybe, paths::PathStyle, rel_path::RelPath};
...
pub use carrot_lsp::DiagnosticSeverity;
...
pub struct Buffer {
    ...
    /// or saved to disk.
    saved_version: inazuma_clock::Global,
    preview_version: inazuma_clock::Global,
...
impl TreeSitterData {
    fn clear(&mut self, snapshot: &inazuma_text::BufferSnapshot) {
    ...
    fn new(snapshot: &inazuma_text::BufferSnapshot) -> Self {
    ...
    fn version(&self) -> &inazuma_clock::Global {
        self.chunks.version()
    }
}
...
pub struct BufferSnapshot {
    pub text: inazuma_text::BufferSnapshot,
...
impl From<inazuma_settings_framework::CursorShape> for CursorShape {
    fn from(shape: inazuma_settings_framework::CursorShape) -> Self {
        match shape {
            inazuma_settings_framework::CursorShape::Bar => CursorShape::Bar,
            inazuma_settings_framework::CursorShape::Block => CursorShape::Block,
            inazuma_settings_framework::CursorShape::Underline => CursorShape::Underline,
            inazuma_settings_framework::CursorShape::Hollow => CursorShape::Hollow,
        }
    }
}
...
pub trait File: Send + Sync + Any {
    ...
    fn to_proto(&self, cx: &App) -> carrot_rpc::proto::File;

    /// Return whether Carrot considers this to be a private file.
...
pub enum DiskState {
    /// File created in Carrot that has not been saved.
    New,
...
#[derive(Debug, Clone)]
pub struct Diff {
    pub base_version: inazuma_clock::Global,
...
impl HighlightedText {
    ...
    pub fn to_styled_text(&self, default_style: &TextStyle) -> StyledText {
        inazuma::StyledText::new(self.text.clone())
...
impl HighlightedTextBuilder {
    ...
    fn highlighted_chunks<'a>(
        range: Range<usize>,
        snapshot: &'a inazuma_text::BufferSnapshot,
...
pub struct EditPreview {
    old_snapshot: inazuma_text::BufferSnapshot,
    applied_edits_snapshot: inazuma_text::BufferSnapshot,
    syntax_snapshot: SyntaxSnapshot,
}
...
impl EditPreview {
    ...
    pub fn highlight_edits(
        ...
        let insertion_highlight_style = HighlightStyle {
            background_color: Some(cx.theme().status().created.background),
            ..Default::default()
        };
        let deletion_highlight_style = HighlightStyle {
            background_color: Some(cx.theme().status().deleted.background),
            ..Default::default()
        };
...
impl Buffer {
    ...
    pub fn from_proto(
        ...
        this.text.set_line_ending(proto::deserialize_line_ending(
            carrot_rpc::proto::LineEnding::from_i32(message.line_ending)
    ...
    pub fn serialize_ops(
        &self,
        since: Option<inazuma_clock::Global>,
        cx: &App,
    ...
    #[carrot_tracing::instrument(skip_all, fields(lang = language.config.name.0.as_str()))]
    ...
    pub fn build(buffer: TextBuffer, file: Option<Arc<dyn File>>, capability: Capability) -> Self {
        ...
        Self {
            ...
            parse_status: carrot_watch::channel(ParseStatus::Idle),
    ...
    fn on_base_buffer_event(
        ...
        if let Operation::Buffer(inazuma_text::Operation::Edit(operation)) = &operation
    ...
    pub fn as_text_snapshot(&self) -> &inazuma_text::BufferSnapshot {
        &self.text
    }
    ...
    #[carrot_tracing::instrument(skip_all)]
    pub fn text_snapshot(&self) -> inazuma_text::BufferSnapshot {
        // todo lw
    ...
    pub fn saved_version(&self) -> &inazuma_clock::Global {
        &self.saved_version
    }
    ...
    pub fn parse_status(&self) -> carrot_watch::Receiver<ParseStatus> {
    ...
    pub fn set_text<T>(&mut self, text: T, cx: &mut Context<Self>) -> Option<inazuma_clock::Lamport>
    where
        T: Into<Arc<str>>,
    {
        self.autoindent_requests.clear();
    ...
    pub fn append<T>(&mut self, text: T, cx: &mut Context<Self>) -> Option<inazuma_clock::Lamport>
    where
        T: Into<Arc<str>>,
    {
    ...
    fn did_edit(
        &mut self,
        old_version: &inazuma_clock::Global,
        was_dirty: bool,
        is_local: bool,
        cx: &mut Context<Self>,
    ) {
...
impl BufferSnapshot {
    ...
    /// in an arbitrary way due to being stored in a [`Rope`](inazuma_text::Rope). The text is also
    ...
    pub fn syntax_root_ancestor(&self, position: Anchor) -> Option<tree_sitter::Node<'_>> {
        ...
        let row = self
            .summary_for_anchor::<inazuma_text::PointUtf16>(&position)
            .row as usize;
        ...
        O: 'a + FromAnchor,
    {
        let mut iterators: Vec<_> = self
            ...
            .map(|(_, collection)| {
                collection
                    .range::<T, inazuma_text::Anchor>(search_range.clone(), self, true, reversed)
                    .peekable()
            })
            .collect();
...
fn indent_size_for_line(text: &inazuma_text::BufferSnapshot, row: u32) -> IndentSize {
...
impl Deref for BufferSnapshot {
    type Target = inazuma_text::BufferSnapshot;
...
impl File for TestFile {
    ...
    fn full_path(&self, _: &inazuma::App) -> PathBuf {
    ...
    fn file_name<'a>(&'a self, _: &'a inazuma::App) -> &'a str {
