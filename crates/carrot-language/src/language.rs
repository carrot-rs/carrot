//! The `language` crate provides a large chunk of Carrot's language-related
...
use inazuma::{App, AsyncApp, Entity};
use inazuma_collections::{HashMap, HashSet};

pub use carrot_language_core::highlight_map::{HighlightId, HighlightMap};

pub use carrot_language_core::{
...
pub use buffer::*;
pub use carrot_lsp::{LanguageServerId, LanguageServerName};
...
pub use inazuma_text::{AnchorRangeExt, LineEnding};
pub use language_registry::{
...
pub(crate) fn to_settings_soft_wrap(
    value: carrot_language_core::SoftWrap,
) -> inazuma_settings_framework::SoftWrap {
    match value {
        carrot_language_core::SoftWrap::None => inazuma_settings_framework::SoftWrap::None,
        carrot_language_core::SoftWrap::PreferLine => {
            inazuma_settings_framework::SoftWrap::PreferLine
        }
        carrot_language_core::SoftWrap::EditorWidth => {
            inazuma_settings_framework::SoftWrap::EditorWidth
        }
        carrot_language_core::SoftWrap::PreferredLineLength => {
            inazuma_settings_framework::SoftWrap::PreferredLineLength
        }
        carrot_language_core::SoftWrap::Bounded => inazuma_settings_framework::SoftWrap::Bounded,
    }
}
...
impl CachedLspAdapter {
    ...
    pub fn process_diagnostics(
        &self,
        params: &mut carrot_lsp::PublishDiagnosticsParams,
    ...
    pub fn underline_diagnostic(&self, diagnostic: &carrot_lsp::Diagnostic) -> bool {
    ...
    pub async fn process_completions(&self, completion_items: &mut [carrot_lsp::CompletionItem]) {
        self.adapter.process_completions(completion_items).await
    }

    pub async fn labels_for_completions(
        &self,
        completion_items: &[carrot_lsp::CompletionItem],
...
pub trait LspAdapter: 'static + Send + Sync + DynLspInstaller {
    ...
    fn process_diagnostics(
        &self,
        _: &mut carrot_lsp::PublishDiagnosticsParams,
        _: LanguageServerId,
    ) {
    }

    /// When processing new `carrot_lsp::PublishDiagnosticsParams` diagnostics, whether to retain previous one(s) or not.
    ...
    fn underline_diagnostic(&self, _diagnostic: &carrot_lsp::Diagnostic) -> bool {
        true
    }

    /// Post-processes completions provided by the language server.
    async fn process_completions(&self, _: &mut [carrot_lsp::CompletionItem]) {}
    ...
    /// Returns initialization options that are going to be sent to a LSP server as a part of [`carrot_lsp::InitializeParams`]
    ...
    /// provide autocompletion and diagnostics in Carrot setting and keybind
    /// files
    fn is_primary_carrot_json_schema_adapter(&self) -> bool {
        false
    }
    ...
    /// for settings that should be persisted to Carrot's settings file.
    ...
    LI: LspInstaller<BinaryVersion = BinaryVersion> + LspAdapter,
{
    ...
    fn get_language_server_command(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to Carrot installation
            //
...
pub struct FakeLspAdapter {
    ...
    pub capabilities: carrot_lsp::ServerCapabilities,
    pub initializer:
        Option<Box<dyn 'static + Send + Sync + Fn(&mut carrot_lsp::FakeLanguageServer)>>,
    pub label_for_completion: Option<
        Box<
            dyn 'static
                + Send
                + Sync
                + Fn(&carrot_lsp::CompletionItem, &Arc<Language>) -> Option<CodeLabel>,
        >,
    >,
}
...
impl LanguageScope {
    ...
    ///
    /// By default, Carrot treats as paragraph and comment prefixes as boundaries.
    ...
    ///
    /// By default, Carrot treats alphanumeric characters (and '_') as word characters for
...
pub trait CodeLabelExt {
    fn fallback_for_completion(
        item: &carrot_lsp::CompletionItem,
...
impl CodeLabelExt for CodeLabel {
    fn fallback_for_completion(
        ...
        let highlight_id = item.kind.and_then(|kind| {
            ...
            use carrot_lsp::CompletionItemKind as Kind;
            match kind {
...
impl Default for FakeLspAdapter {
    fn default() -> Self {
        Self {
            ...
            capabilities: carrot_lsp::LanguageServer::full_capabilities(),
            initializer: None,
...
pub fn point_to_lsp(point: PointUtf16) -> carrot_lsp::Position {
    carrot_lsp::Position::new(point.row, point.column)
}

pub fn point_from_lsp(point: carrot_lsp::Position) -> Unclipped<PointUtf16> {
    Unclipped(PointUtf16::new(point.line, point.character))
}

pub fn range_to_lsp(range: Range<PointUtf16>) -> Result<carrot_lsp::Range> {
    anyhow::ensure!(
...
pub fn range_from_lsp(range: carrot_lsp::Range) -> Range<Unclipped<PointUtf16>> {
    ...
    if start > end {
        ...
        //
        // See issue #36223.
        carrot_log::debug!("range_from_lsp called with inverted range {start:?}-{end:?}");
...
pub fn rust_lang() -> Arc<Language> {
    ...
    .with_queries(LanguageQueries {
        outline: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/outline.scm"
        ))),
        indents: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/indents.scm"
        ))),
        brackets: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/brackets.scm"
        ))),
        text_objects: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/textobjects.scm"
        ))),
        highlights: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/highlights.scm"
        ))),
        injections: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/injections.scm"
        ))),
        overrides: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/overrides.scm"
        ))),
        redactions: None,
        runnables: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/runnables.scm"
        ))),
        debugger: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/rust/debugger.scm"
        ))),
    })
...
pub fn markdown_lang() -> Arc<Language> {
    ...
    .with_queries(LanguageQueries {
        brackets: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/markdown/brackets.scm"
        ))),
        injections: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/markdown/injections.scm"
        ))),
        highlights: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/markdown/highlights.scm"
        ))),
        indents: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/markdown/indents.scm"
        ))),
        outline: Some(Cow::from(include_str!(
            "../../carrot-grammars/src/markdown/outline.scm"
        ))),
...
mod tests {
    use super::*;
    use inazuma::{TestAppContext, rgba};
    ...
    async fn test_completion_label_omits_duplicate_data() {
        let regular_completion_item_1 = carrot_lsp::CompletionItem {
            label: "regular1".to_string(),
            detail: Some("detail1".to_string()),
            label_details: Some(carrot_lsp::CompletionItemLabelDetails {
                detail: None,
                description: Some("description 1".to_string()),
            }),
            ..carrot_lsp::CompletionItem::default()
        };

        let regular_completion_item_2 = carrot_lsp::CompletionItem {
        ...
        let completion_item_with_duplicate_detail_and_proper_description =
            carrot_lsp::CompletionItem {
                detail: Some(regular_completion_item_1.label.clone()),
                ..regular_completion_item_1.clone()
            };

        let completion_item_with_duplicate_detail = carrot_lsp::CompletionItem {
            detail: Some(regular_completion_item_1.label.clone()),
            label_details: None,
            ..regular_completion_item_1.clone()
        };

        let completion_item_with_duplicate_description = carrot_lsp::CompletionItem {
            label_details: Some(carrot_lsp::CompletionItemLabelDetails {
                detail: None,
