...
use inazuma::{App, AppContext as _, BorrowAppContext, Entity};
use inazuma::{HighlightStyle, TestAppContext};
use inazuma_clock::ReplicaId;
use inazuma_collections::BTreeMap;
use inazuma_settings_framework::SettingsStore;
use inazuma_settings_framework::{AllLanguageSettingsContent, LanguageSettingsContent};
use inazuma_text::network::Network;
use inazuma_text::{BufferId, LineEnding};
use inazuma_text::{Point, ToPoint};
use inazuma_util::rel_path::rel_path;
use inazuma_util::test::marked_text_offsets;
use inazuma_util::{RandomCharIter, assert_set_eq, post_inc, test::marked_text_ranges};
use indoc::indoc;
...
#[inazuma::test]
fn test_line_endings(cx: &mut inazuma::App) {
    init_settings(cx, |_| {});
...
fn test_select_language(cx: &mut App) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("carrot/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("carrot/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("carrot/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
...
fn file(path: &str) -> Arc<dyn File> {
    Arc::new(TestFile {
        ...
        root_name: "carrot".into(),
        local_root: None,
    })
}

#[inazuma::test]
fn test_edit_events(cx: &mut inazuma::App) {
...
#[inazuma::test(iterations = 10)]
async fn test_normalize_whitespace(cx: &mut inazuma::TestAppContext) {
    let text = [
        "zero",     //
...
#[inazuma::test]
async fn test_reparse(cx: &mut inazuma::TestAppContext) {
    let text = "fn a() {}";
...
#[inazuma::test]
async fn test_resetting_language(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_outline(cx: &mut inazuma::TestAppContext) {
    let text = r#"
        struct Person {
            name: String,
            age: usize,
...
#[inazuma::test]
async fn test_outline_nodes_with_newlines(cx: &mut inazuma::TestAppContext) {
    let text = r#"
        impl A for B<
            C
        > {
...
#[inazuma::test]
async fn test_outline_with_extra_context(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_symbols_containing(cx: &mut inazuma::TestAppContext) {
    let text = r#"
        impl Person {
            fn one() {
                1
...
#[inazuma::test]
fn test_serialization(cx: &mut inazuma::App) {
    let mut now = Instant::now();
...
async fn test_preview_edits(cx: &mut TestAppContext) {
    ...
    let insertion_style = HighlightStyle {
        background_color: Some(cx.read(|cx| cx.theme().status().created.background)),
        ..Default::default()
    };
    let deletion_style = HighlightStyle {
        background_color: Some(cx.read(|cx| cx.theme().status().deleted.background)),
        ..Default::default()
    };
...
#[inazuma::test]
fn test_insertion_after_deletion(cx: &mut inazuma::App) {
...
#[inazuma::test]
fn test_words_in_range(cx: &mut inazuma::App) {
    init_settings(cx, |_| {});
...
fn get_tree_sexp(buffer: &Entity<Buffer>, cx: &mut inazuma::TestAppContext) -> String {
    buffer.update(cx, |buffer, _| {
        let snapshot = buffer.snapshot();
