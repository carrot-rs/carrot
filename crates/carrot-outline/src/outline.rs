...
use carrot_editor::{Anchor, AnchorRangeExt, Editor, scroll::Autoscroll};
use carrot_editor::{MultiBufferOffset, RowHighlightOptions, SelectionEffects};
use carrot_language::{Outline, OutlineItem};
use carrot_theme::ActiveTheme;
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{ListItem, ListItemSpacing, prelude::*};
use carrot_workspace::{DismissDecision, ModalView};
use inazuma::{
...
pub fn init(cx: &mut App) {
    ...
    carrot_actions::outline::TOGGLE_OUTLINE
        .set(|view, window, cx| {
...
pub fn toggle(
    ...
    _: &carrot_actions::outline::ToggleOutline,
    window: &mut Window,
    cx: &mut App,
) {
...
pub fn render_item<T>(
    ...
    let highlights = inazuma::combine_highlights(
        custom_highlights,
...
mod tests {
    ...
    use super::*;
    use carrot_language::FakeLspAdapter;
    ...
    async fn test_outline_view_row_highlights(cx: &mut TestAppContext) {
        ...
        project.read_with(cx, |project, _| {
            project.languages().add(carrot_language::rust_lang())
        });
        ...
        cx.dispatch_action(inazuma_menu::Cancel);
        ensure_outline_view_contents(&outline_view, cx);
        assert_eq!(
            highlighted_display_rows(&editor, cx),
            Vec::<u32>::new(),
    ...
    fn open_outline_view(
        ...
        cx.dispatch_action(carrot_actions::outline::ToggleOutline);
    ...
    fn set_single_caret_at_row(
        ...
        editor.update_in(cx, |editor, window, cx| {
            editor.change_selections(SelectionEffects::no_scroll(), window, cx, |s| {
                s.select_ranges([inazuma_rope::Point::new(buffer_row, 0)
                    ..inazuma_rope::Point::new(buffer_row, 0)])
            });
        });
    }
    ...
    fn assert_single_caret_at_row(
        ...
        let selections = editor.update(cx, |editor, cx| {
            editor
                .selections
                .all::<inazuma_rope::Point>(&editor.display_snapshot(cx))
                .into_iter()
                .map(|s| s.start..s.end)
                .collect::<Vec<_>>()
        });
