...
use carrot_db::smol::future::yield_now;
...
pub use dock::Panel;
pub use inazuma_settings_framework::SidebarSide;
...
use carrot_fs::Fs;
use carrot_language::{Buffer, LanguageRegistry, Rope, language_settings::all_language_settings};
...
use inazuma::{
    Action, AnyEntity, AnyView, AnyWeakView, AnyWindowHandle, App, AsyncApp, AsyncWindowContext,
    Axis, Bounds, Context, CursorStyle, Decorations, DragMoveEvent, Entity, EntityId, EventEmitter,
    FocusHandle, Focusable, Global, HitboxBehavior, KeyContext, Keystroke, ManagedView,
    MouseButton, Oklch, PathPromptOptions, Point, PromptLevel, Render, ResizeEdge, Size, Stateful,
    Subscription, SystemWindowTabController, Task, Tiling, WeakEntity, WindowBounds, WindowId,
...
pub use item::{
    FollowableItem, FollowableItemHandle, Item, ItemHandle, ItemSettings, PaneRole,
...
pub use persistence::{
    ...
    model::{
        DockStructure, ItemId, SerializedSession, SerializedWorkspace, SerializedWorkspaceLocation,
        SessionWorkspace,
    },
    read_default_dock_state, read_default_window_bounds, resolve_worktree_workspaces,
};
use postage::stream::Stream;
use schemars::JsonSchema;
use serde::Deserialize;
pub use session::{SessionEvent, SessionId, WorkspaceSession};

use carrot_actions::{Spawn, theme::ToggleMode};
use carrot_sqlez::{
    bindable::{Bind, Column, StaticColumnCount},
    statement::Statement,
};
use carrot_task::{DebugScenario, SharedTaskContext, SpawnInTerminal};
use carrot_theme::{ActiveTheme, GlobalTheme, SystemAppearance};
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{Window, prelude::*};
use inazuma_util::{
...
// ---------------------------------------------------------------------------
// OpenOptions / OpenResult — types used by Workspace::open_paths and callers
// ---------------------------------------------------------------------------

/// Options for opening paths in a workspace.
///
/// Used by [`Workspace::open_paths`] (the method) and by the free `open_paths`
/// function in `carrot_shell`.  The `replace_window` and `env` fields are only
/// relevant for the free function.
...
/// The result of opening a workspace window.
...
// ---------------------------------------------------------------------------
// WorkspaceOpener — global callback interface for window-opening functions
// that live in carrot-shell (to avoid circular dependencies).
// ---------------------------------------------------------------------------

/// Trait that carrot-shell implements to provide window-opening functions
/// to carrot-workspace code.  Registered as a global via [`Global`].
pub trait WorkspaceOpener: 'static + Send + Sync {
    /// Open paths in a new or existing workspace (the free `open_paths`).
    fn open_paths(
        &self,
        abs_paths: &[PathBuf],
    ...
    /// Join a shared project in a call.
    ...
    /// Get the list of local workspace windows (returns AnyWindowHandles).
    fn local_workspace_windows(&self, cx: &App) -> Vec<AnyWindowHandle>;
}

/// Global storage for the [`WorkspaceOpener`] implementation.
struct GlobalWorkspaceOpener(Arc<dyn WorkspaceOpener>);

impl Global for GlobalWorkspaceOpener {}

/// Register the workspace opener (called by carrot-shell during init).
pub fn set_workspace_opener(opener: Arc<dyn WorkspaceOpener>, cx: &mut App) {
    cx.set_global(GlobalWorkspaceOpener(opener));
}

/// Get the workspace opener, if registered.
pub fn workspace_opener(cx: &App) -> Option<Arc<dyn WorkspaceOpener>> {
    cx.try_global::<GlobalWorkspaceOpener>()
        .map(|g| g.0.clone())
}

// ---------------------------------------------------------------------------
// WorkspaceRegistry — maps window handles to workspace entities.
// AppShell (or any wrapper) registers its workspace here so that
// Workspace::for_window can find it even when the root view is not Workspace.
// ---------------------------------------------------------------------------

#[derive(Default)]
struct WorkspaceRegistry {
    workspaces: HashMap<WindowId, WeakEntity<Workspace>>,
}
...
impl WorkspaceRegistry {
    ...
    pub fn register(window_id: WindowId, workspace: WeakEntity<Workspace>, cx: &mut App) {
        Self::ensure(cx);
        cx.global_mut::<Self>()
            .workspaces
    ...
    pub fn unregister(window_id: WindowId, cx: &mut App) {
        if cx.try_global::<Self>().is_some() {
            cx.global_mut::<Self>().workspaces.remove(&window_id);
        }
    }

    pub fn workspace_for_window(window: AnyWindowHandle, cx: &App) -> Option<Entity<Workspace>> {
        cx.try_global::<Self>()
            .and_then(|reg| reg.workspaces.get(&window.window_id()))
            .and_then(|weak| weak.upgrade())
    }
}

/// Register a workspace entity for a window.
/// Called by AppShell (or similar wrappers) after constructing a Workspace.
pub fn register_workspace_for_window(
    window_id: WindowId,
    workspace: WeakEntity<Workspace>,
    cx: &mut App,
) {
    WorkspaceRegistry::register(window_id, workspace, cx);
}

/// Unregister a workspace entity for a window (e.g., on window close).
pub fn unregister_workspace_for_window(window_id: WindowId, cx: &mut App) {
    WorkspaceRegistry::unregister(window_id, cx);
}

/// Look up the workspace entity registered for a given window handle.
pub fn workspace_for_window(window: AnyWindowHandle, cx: &App) -> Option<Entity<Workspace>> {
    WorkspaceRegistry::workspace_for_window(window, cx)
}
...
fn prompt_and_open_paths(app_state: Arc<AppState>, options: PathPromptOptions, cx: &mut App) {
    // Try to find an existing workspace window to host the open-path prompt.
    let existing = if let Some(opener) = workspace_opener(cx) {
        opener.local_workspace_windows(cx).into_iter().next()
    } else {
        None
    };
    if let Some(workspace_window) = existing {
        workspace_window
            .update(cx, |_, window, cx| {
                if let Some(workspace) = Workspace::for_window(window, cx) {
                    workspace.update(cx, |workspace, cx| {
                        prompt_for_open_path_and_open(
                            workspace, app_state, options, true, window, cx,
                        );
                    });
                }
            })
            .ok();
    } else if let Some(opener) = workspace_opener(cx) {
        let task = opener.open_paths(&[], app_state.clone(), None, cx);
        cx.spawn(async move |cx| {
            let workspace = task.await?;
            // Find the window containing this workspace and prompt.
            cx.update(|cx| {
                for window in cx.windows() {
                    let ok = window
                        .update(cx, |_, window, cx| {
                            if let Some(ws) = Workspace::for_window(window, cx) {
                                if ws.entity_id() == workspace.entity_id() {
    ...
    cx.spawn_in(window, async move |this, cx| {
        ...
        if let Some(task) = this
            .update_in(cx, |this, window, cx| {
                this.open_workspace_for_paths(!create_new_window, paths, window, cx)
            })
            .log_err()
        {
            task.await.log_err();
...
impl ProjectItemRegistry {
    fn register<T: ProjectItem>(&mut self) {
        ...
        self.build_project_item_for_path_fns
            .push(|project, project_path, window, cx| {
                ...
                let project_item =
                    <T::Item as carrot_project::ProjectItem>::try_open(project, &project_path, cx)?;
                    ...
                    match project_item.await.with_context(|| {
                        ...
                        Ok(project_item) => {
                            ...
                            let project_entry_id: Option<ProjectEntryId> =
                                project_item.read_with(cx, carrot_project::ProjectItem::entry_id);
    ...
    fn build_item<T: carrot_project::ProjectItem>(
        &self,
        item: Entity<T>,
        project: Entity<Project>,
        pane: Option<&Pane>,
...
impl AppState {
    ...
    pub fn test(cx: &mut App) -> Arc<Self> {
        ...
        let http_client = carrot_http_client::FakeHttpClient::with_404_response();
...
pub struct Workspace {
    ...
    /// Top-level sessions. Each session owns its own `PaneGroup` and is
    /// rendered as a tab in the title bar. This currently lives parallel
    /// to the legacy `center`/`panes`/`active_pane` fields, which mirror
    /// the active session's state. The legacy fields will be removed once
    /// every call site routes through `sessions`.
    sessions: Vec<Entity<WorkspaceSession>>,
    ...
    /// Cached decoded background image for the Glass UI layer. See the
    /// "Design System: Glass UI Pattern" section in CLAUDE.md — the image
    /// is rendered at Workspace root, not inside any pane.
...
impl Workspace {
    pub fn new(
        ...
        cx.subscribe_in(&project, window, move |this, _, event, window, cx| {
            match event {
                carrot_project::Event::RemoteIdChanged(_) => {
                    this.update_window_title(window, cx);
                }

                carrot_project::Event::CollaboratorLeft(peer_id) => {
                    this.collaborator_left(*peer_id, window, cx);
                }

                &carrot_project::Event::WorktreeRemoved(_) => {
                    this.update_window_title(window, cx);
                    this.serialize_workspace(window, cx);
                    this.update_history(cx);
                }

                &carrot_project::Event::WorktreeAdded(id) => {
                    this.update_window_title(window, cx);
                    if this
                        .project()
                        .read(cx)
                ...
                carrot_project::Event::WorktreeUpdatedEntries(..) => {
                    this.update_window_title(window, cx);
                    this.serialize_workspace(window, cx);
                }

                carrot_project::Event::DisconnectedFromHost => {
                ...
                carrot_project::Event::DisconnectedFromRemote {
                    server_not_running: _,
                } => {
                    this.update_window_edited(window, cx);
                }

                carrot_project::Event::Closed => {
                    window.remove_window();
                }

                carrot_project::Event::DeletedEntry(_, entry_id) => {
                ...
                carrot_project::Event::HideToast { notification_id } => {
                    this.dismiss_notification(&NotificationId::named(notification_id.clone()), cx)
                }

                carrot_project::Event::LanguageServerPrompt(request) => {
                ...
                carrot_project::Event::AgentLocationChanged => {
        ...
        let subscriptions = vec![
            ...
            cx.observe_window_appearance(window, |_, window, cx| {
                ...
                *SystemAppearance::global_mut(cx) = SystemAppearance(window_appearance.into());
                ...
                carrot_theme_settings::reload_theme(cx);
                carrot_theme_settings::reload_icon_theme(cx);
            }),
        ...
        // Create a single initial WorkspaceSession that mirrors the legacy
        // center/panes/active_pane state. The duplication will be collapsed
        // once every call site routes through the session list.
        let initial_session = cx.new(|cx| WorkspaceSession::new(center_pane.clone(), cx));
    ...
    pub fn set_bottom_dock_layout(
        ...
        inazuma_settings_framework::update_settings_file(fs.clone(), cx, move |content, _cx| {
        ...
        F: 'static + FnOnce(&mut Workspace, &mut Window, &mut Context<Workspace>) -> T,
    {
        if self.project.read(cx).is_local() {
            ...
            if let Some(opener) = workspace_opener(cx) {
                let task = opener.open_paths(&[], app_state, env, cx);
                cx.spawn_in(window, async move |_vh, cx| {
                    let workspace = task.await?;
                    workspace.update_in(cx, |workspace, window, cx| callback(workspace, window, cx))
                })
            } else {
                Task::ready(Err(anyhow!("No workspace opener registered")))
            }
        }
    }
    ...
    pub fn prepare_to_close(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let workspace_count = cx.update(|_window, cx| cx.windows().len())?;
                    ...
                    .unwrap_or(false)
            {
                ...
                if close_intent == CloseIntent::ReplaceWindow {
                    _ = cx.update(|_window, cx| {
                        // Find any remaining workspace and unshare its project
                        for window in cx.windows() {
    ...
    pub fn open_workspace_for_paths(
        ...
        if let Some(opener) = workspace_opener(cx) {
            let task = opener.open_paths(&paths, app_state, None, cx);
                ...
                'emit_winner: {
                    ...
                    this.update(cx, |workspace, cx| {
                        workspace.project.update(cx, |_, cx| {
                            cx.emit(carrot_project::Event::ActiveEntryChanged(Some(entry_id)));
                        });
                    })
                    .ok();
                }
    ...
    pub fn close_items_with_project_path(
        ...
        for pane in panes {
            pane.update(cx, |pane, cx| {
                pane.close_items_for_project_path(project_path, save_intent, window, cx)
                    .detach_and_log_err(cx);
            });
        }
    }
    ...
    where
        T: ProjectItem,
    {
        use carrot_project::ProjectItem as _;
    ...
    // ---- Session API --------------------------------------------------------
    //
    // The session list is the canonical source of truth for which Panes
    // belong to the workspace. `center`, `panes`, and `active_pane` are
    // kept in sync with the active session as a compatibility bridge so that the
    // 372 legacy call sites continue to work. Phase 1.4 will collapse this
    // duplication.

    /// All sessions in this workspace, in tab order.
    pub fn sessions(&self) -> &[Entity<WorkspaceSession>] {
        &self.sessions
    }
    ...
    pub fn active_session(&self) -> &Entity<WorkspaceSession> {
    ...
    /// Switch the active session to `index`. The legacy pane fields (center,
    /// panes, active_pane, last_active_center_pane) are swapped to reflect
    /// the target session's stored state.
    pub fn activate_session(&mut self, index: usize, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // Save the current legacy state into the outgoing session.
        self.sync_active_session(cx);

        // Pull the target session's stored state into the legacy fields.
        let target = self.sessions[index].clone();
        let (pane_group, panes, active_pane, last_active) = target.read_with(cx, |s, _| {
            (
                s.pane_group().clone(),
                s.panes().to_vec(),
                s.active_pane().clone(),
                s.last_active_center_pane().cloned(),
            )
        });
        self.center = pane_group;
        self.panes = panes;
        self.active_pane = active_pane.clone();
        self.last_active_center_pane = last_active;
        self.active_session_index = index;

        // Rebuild panes_by_item from the new active session's items so that
        // workspace-wide item lookups (search, navigation, follow) resolve
        // against the visible panes.
        self.panes_by_item.clear();
        for pane in &self.panes {
            let pane_weak = pane.downgrade();
            for item in pane.read(cx).items() {
                self.panes_by_item.insert(item.item_id(), pane_weak.clone());
            }
        }
    ...
    /// Close the session at `index`. If it was the active session, focus
    /// shifts to the neighbouring session. If it was the last remaining
    /// session, the window is closed.
    pub fn close_session(&mut self, index: usize, window: &mut Window, cx: &mut Context<Self>) {
        if index >= self.sessions.len() {
            return;
        }

        // Last session → close the window.
        if self.sessions.len() == 1 {
            window.remove_window();
            return;
        }

        // If we're closing the currently active session, switch focus to a
        // neighbour first so the workspace's legacy fields point to a valid
        // pane after removal.
        if index == self.active_session_index {
            let fallback = if index + 1 < self.sessions.len() {
                index + 1
            } else {
                index - 1
            };
            self.activate_session(fallback, window, cx);
            // After activation, active_session_index points at `fallback`.
            // Removing `index` will shift indices ≥ index down by one, so
            // we need to fix up active_session_index accordingly.
        }
    ...
    /// Move the session at `index` one slot to the left. No-op if it is
    /// already the leftmost session.
    pub fn move_session_left(&mut self, index: usize, cx: &mut Context<Self>) {
        if index == 0 || index >= self.sessions.len() {
            return;
        }
        self.sessions.swap(index, index - 1);
        if self.active_session_index == index {
            self.active_session_index -= 1;
        } else if self.active_session_index == index - 1 {
            self.active_session_index += 1;
        }
        cx.notify();
    }

    /// Move the session at `index` one slot to the right. No-op if it is
    /// already the rightmost session.
    pub fn move_session_right(&mut self, index: usize, cx: &mut Context<Self>) {
    ...
    /// Move session from one index to another (for drag&drop reordering).
    pub fn move_session(&mut self, from: usize, to: usize, cx: &mut Context<Self>) {
        ...
        // Update active index to follow the moved session if needed
        if self.active_session_index == from {
            self.active_session_index = to;
        } else if from < self.active_session_index && to >= self.active_session_index {
            self.active_session_index -= 1;
        } else if from > self.active_session_index && to <= self.active_session_index {
            self.active_session_index += 1;
        }
    ...
    /// Close every session except the one at `keep_index`. The kept session
    /// stays active.
    pub fn close_other_sessions(
        ...
        if keep_index >= self.sessions.len() {
            return;
        }
        // Activate the kept session first so legacy fields are correct.
        if self.active_session_index != keep_index {
            self.activate_session(keep_index, window, cx);
        }
        let kept = self.sessions[self.active_session_index].clone();
        self.sessions.clear();
    ...
    /// Close every session strictly to the right of `index`.
    pub fn close_sessions_to_right(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        // If the active session is to the right of `index`, switch back to
        // `index` first so the legacy pane state is restored from a session
        // that will still exist after the truncation.
        if self.active_session_index > index {
            self.activate_session(index, window, cx);
        }
        self.sessions.truncate(index + 1);
    ...
    /// Mirror the workspace's legacy pane state (center, panes, active_pane,
    /// last_active_center_pane) into the active session. Called after every
    /// mutation of the legacy fields during Phase 1 so that
    /// `sessions[active_session_index]` always reflects reality. Phase 3's
    /// `activate_session()` uses the mirrored state to swap between tabs.
    ///
    /// Note: `zoomed` and `zoomed_position` are NOT mirrored yet because
    /// `AnyWeakView` is not Clone. Phase 3 will store them via move-semantics
    /// when implementing real session switching.
    pub(crate) fn sync_active_session(&self, cx: &mut App) {
        let session = self.sessions[self.active_session_index].clone();
        let pane_group = self.center.clone();
    ...
    pub fn follow(
        ...
        if let CollaboratorId::PeerId(peer_id) = leader_id {
            ...
            if let Some(project_id) = other_project_id {
                ...
                if let Some(opener) = workspace_opener(cx) {
                    opener
                        .join_in_room_project(project_id, remote_participant.user.id, app_state, cx)
                        .detach_and_log_err(cx);
                }
            }
        }
    ...
    async fn add_view_from_leader(
        ...
        let item = if let Some(existing_item) = existing_item {
            ...
            let Some(task) = task else {
                anyhow::bail!(
                    "failed to construct view from leader (maybe from a different version of carrot?)"
                );
            };
                ...
                let session_data: Vec<_> = self
                    ...
                    .map(|session| {
                        ...
                        (
                            ...
                            s.color().map(|c| format!("{},{},{}", c.l, c.c, c.h)),
                            s.pane_group().root.clone(),
                        )
                    })
                    .collect();
                let sessions: Vec<SerializedSession> = session_data
                    .into_iter()
                    .map(|(name, color, root)| SerializedSession {
                        name,
                        color,
                        pane_group: build_serialized_pane_group(&root, window, cx),
                    })
                    .collect();
    ...
    pub fn actions(&self, div: Div, window: &mut Window, cx: &mut Context<Self>) -> Div {
        self.add_workspace_actions_listeners(div, window, cx)
            .on_action(cx.listener(
                |_workspace,
                 action_sequence: &inazuma_settings_framework::ActionSequence,
                 window,
                 cx| {
            ...
            // Session tab actions
            .on_action(cx.listener(
                |workspace, _: &carrot_actions::session::NewSession, window, cx| {
                    workspace.new_session(window, cx);
                },
            ))
            .on_action(cx.listener(
                |workspace, _: &carrot_actions::session::CloseActiveSession, window, cx| {
                    let session = workspace.active_session().clone();
                    let panes_count = session.read(cx).panes().len();
                    if panes_count > 1 {
                        let active_pane = session.read(cx).active_pane().clone();
                        workspace.remove_pane(active_pane, None, window, cx);
                    } else {
                        let index = workspace.active_session_index();
                        workspace.close_session(index, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |workspace, _: &carrot_actions::session::ActivatePreviousSession, window, cx| {
                    let current = workspace.active_session_index();
                    if current > 0 {
                        workspace.activate_session(current - 1, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |workspace, _: &carrot_actions::session::ActivateNextSession, window, cx| {
                    let current = workspace.active_session_index();
            ...
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession1, window, cx| {
                    if ws.sessions().len() > 0 {
                        ws.activate_session(0, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession2, window, cx| {
                    if ws.sessions().len() > 1 {
                        ws.activate_session(1, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession3, window, cx| {
                    if ws.sessions().len() > 2 {
                        ws.activate_session(2, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession4, window, cx| {
                    if ws.sessions().len() > 3 {
                        ws.activate_session(3, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession5, window, cx| {
                    if ws.sessions().len() > 4 {
                        ws.activate_session(4, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession6, window, cx| {
                    if ws.sessions().len() > 5 {
                        ws.activate_session(5, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession7, window, cx| {
                    if ws.sessions().len() > 6 {
                        ws.activate_session(6, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession8, window, cx| {
                    if ws.sessions().len() > 7 {
                        ws.activate_session(7, window, cx);
                    }
                },
            ))
            .on_action(cx.listener(
                |ws, _: &carrot_actions::session::ActivateSession9, window, cx| {
                    if ws.sessions().len() > 8 {
                        ws.activate_session(8, window, cx);
                    }
                },
            ))
    ...
    pub fn for_window(window: &Window, cx: &App) -> Option<Entity<Workspace>> {
        // Try to get a Workspace directly (if it's the root view)
        if let Some(workspace) = window.root::<Workspace>().flatten() {
            return Some(workspace);
        }
        // The root might be an AppShell or similar wrapper.
        // Look up the workspace via the global registry.
        WorkspaceRegistry::workspace_for_window(window.window_handle(), cx)
    }
    ...
    fn toggle_theme_mode(&mut self, _: &ToggleMode, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        let next_mode = match current_mode {
            Some(carrot_theme_settings::ThemeAppearanceMode::Light) => {
                carrot_theme_settings::ThemeAppearanceMode::Dark
            }
            Some(carrot_theme_settings::ThemeAppearanceMode::Dark) => {
                carrot_theme_settings::ThemeAppearanceMode::Light
            }
            Some(carrot_theme_settings::ThemeAppearanceMode::System) | None => {
                match cx.theme().appearance() {
                    carrot_theme::Appearance::Light => {
                        carrot_theme_settings::ThemeAppearanceMode::Dark
                    }
                    carrot_theme::Appearance::Dark => {
                        carrot_theme_settings::ThemeAppearanceMode::Light
                    }
                }
            }
        ...
        let fs = self.project().read(cx).fs().clone();
        inazuma_settings_framework::update_settings_file(fs, cx, move |settings, _cx| {
            carrot_theme_settings::set_mode(settings, next_mode);
        });
    }
...
impl Render for Workspace {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Resolve the Glass UI background image layer. Rendered at the
        // workspace root so every pane, panel and bar sees it underneath.
        // Keep the decoded image in `cached_bg_image` to avoid reloading.
        let theme_snapshot = GlobalTheme::theme(cx).clone();
        let bg_image_layer =
            theme_snapshot
                .styles
                .background_image
                ...
                .and_then(|bg_config| {
                    ...
                    let render_image = if let Some((cached_path, cached_img)) =
                        &self.cached_bg_image
                        && cached_path == std::path::Path::new(&bg_config.path)
                    {
                        Some(Arc::clone(cached_img))
                    } else if let Some(base_dir) = &theme_snapshot.base_dir {
                        let image_path = base_dir.join(&bg_config.path);
                        let img = inazuma::preload_image(&image_path)?;
                        self.cached_bg_image = Some((image_path, Arc::clone(&img)));
                        Some(img)
                    } else {
                        let asset_path = format!("themes/{}/{}", theme_snapshot.id, bg_config.path);
                        let bytes = cx.asset_source().load(&asset_path).ok().flatten()?;
                        let img = inazuma::preload_image_from_bytes(&bytes)?;
                        self.cached_bg_image =
                            Some((std::path::PathBuf::from(&bg_config.path), Arc::clone(&img)));
                        Some(img)
                    };
                    render_image.map(|img| (img, opacity))
                });
        ...
        self.actions(div(), window, cx)
            ...
            .gap_0()
                ...
                // Glass UI: theme background image as an overlay ABOVE every
                // pane, panel and bar. Rendered as the last child so painter's
                // order puts it on top. `opacity` controls how strongly the
                // image tints the UI (low = subtle texture, high = heavy
                // overlay that obscures content).
                // See CLAUDE.md "Design System: Glass UI Pattern".
                .when_some(bg_image_layer, |el, (img, opacity)| {
                    el.child(
                        inazuma::img(inazuma::ImageSource::Render(img))
                            .absolute()
                            .top_0()
                            .left_0()
                            .size_full()
                ...
                // Modal layer overlays everything (panels, image, status
                // bar) so command-search, command-palette, branch picker
                // etc. paint on top. Upstream mounts this in a separate
                // multi-workspace wrapper which we don't have, so we
                // attach it directly to the workspace render root.
...
actions!(
    ...
    [
        ...
        ///
        /// If you want to open a specific channel, use `carrot::OpenCarrotUrl` with a channel notes URL -
        /// can be copied via "Copy link to section" in the context menu of the channel notes
        /// buffer. These URLs look like `https://carrot.dev/channel/channel-name-CHANNEL_ID/notes`.
        OpenChannelNotes,
...
actions!(
    carrot,
    [
        /// Opens the Carrot log file.
        OpenLog,
        /// Reveals the Carrot log file in the system file manager.
...
async fn join_channel_internal(
    ...
    app_state: &Arc<AppState>,
    requesting_window: Option<AnyWindowHandle>,
    ...
    if already_in_channel {
        let task = cx.update(|cx| {
            if let Some((project, host)) = active_call.most_active_project(cx) {
                if let Some(opener) = workspace_opener(cx) {
                    Some(opener.join_in_room_project(project, host, app_state.clone(), cx))
                } else {
                    None
                }
            } else {
    ...
    // this loop will terminate within carrot_client::CONNECTION_TIMEOUT seconds.
    'outer: loop {
    ...
    let task = cx.update(|cx| {
        if let Some((project, host)) = active_call.most_active_project(cx) {
            if let Some(opener) = workspace_opener(cx) {
                return Some(opener.join_in_room_project(project, host, app_state.clone(), cx));
            }
        }
...
pub fn join_channel(
    ...
    cx.spawn(async move |cx| {
        ...
        let mut active_window = requesting_window.or_else(|| cx.update(|cx| cx.active_window()));
        if active_window.is_none() {
            // no open workspaces, make one to show the error in
            let opener_task = cx.update(|cx| {
                if let Some(opener) = workspace_opener(cx) {
                    Some(opener.open_paths(&[], app_state.clone(), None, cx))
                } else {
                    None
                }
            });
            ...
            if let Some(task) = opener_task {
                if let Ok(workspace) = task.await {
                    let entity_id = workspace.entity_id();
                    // Find the window containing this workspace
                    let wh = cx.update(|cx| {
                        cx.windows().into_iter().find(|w| {
                            w.update(cx, |_, window, cx| {
                                Workspace::for_window(window, cx)
                                    .map(|ws| ws.entity_id() == entity_id)
                                    .unwrap_or(false)
                            })
                            .unwrap_or(false)
                        })
                    ...
                    if let Some(wh) = wh {
                        wh.update(cx, |_, window, _cx| {
                            window.activate_window();
                        })
                        .ok();
                        active_window = Some(wh);
                    }
                }
            }
                    ...
                    .update(cx, |_, window, cx| {
                        let detail: SharedString = match err.error_code() {
                            ...
                            ErrorCode::UpgradeRequired => concat!(
                                "Your are running an unsupported version of Carrot.",
...
pub fn open_remote_project_with_new_connection(
    ...
    cx.spawn(async move |cx| {
        ...
        let session = match cx
            .update(|cx| {
                carrot_remote::RemoteClient::new(
...
async fn open_remote_project_inner(
    ...
    let workspace = window.update(cx, |_, window, cx| {
        carrot_telemetry::event!("SSH Project Opened");
...
pub fn client_side_decorations(
    ...
    match decorations {
        Decorations::Client { .. } => {
            window.set_client_inset(carrot_theme::CLIENT_SIDE_DECORATION_SHADOW)
        }
        ...
        .map(|div| match decorations {
            ...
            Decorations::Client { .. } => div
                .when(
                    ...
                    |div| div.rounded_tr(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.top
                        || tiling.left
                    ...
                    |div| div.rounded_tl(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.bottom
                        || tiling.right
                        ...
                        || border_radius_tiling.right),
                    |div| div.rounded_br(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(
                    !(tiling.bottom
                        || tiling.left
                        || border_radius_tiling.bottom
                        || border_radius_tiling.left),
                    |div| div.rounded_bl(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING),
                )
                .when(!tiling.top, |div| {
                    div.pt(carrot_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.bottom, |div| {
                    div.pb(carrot_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.left, |div| {
                    div.pl(carrot_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .when(!tiling.right, |div| {
                    div.pr(carrot_theme::CLIENT_SIDE_DECORATION_SHADOW)
                })
                .on_mouse_move(move |e, window, cx| {
                    ...
                    let new_edge = resize_edge(
                        pos,
                        carrot_theme::CLIENT_SIDE_DECORATION_SHADOW,
                        size,
                        tiling,
                    );
                    ...
                    Decorations::Client { .. } => div
                        ...
                        .when(!tiling.is_tiled(), |div| {
                            div.shadow(vec![inazuma::BoxShadow {
                                ...
                                blur_radius: carrot_theme::CLIENT_SIDE_DECORATION_SHADOW / 2.,
                                spread_radius: px(0.),
...
mod tests {
    ...
    use carrot_fs::FakeFs;
    use carrot_project::{Project, ProjectEntryId};
    use inazuma::{
    ...
    #[inazuma::test]
    async fn test_autosave(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_autosave_on_focus_change_in_multibuffer(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_activate_last_pane(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        let project = Project::test(fs, [], cx).await;
        let (workspace, cx) =
    ...
    #[inazuma::test]
    async fn test_toggle_docks_and_panels(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_close_panel_on_toggle(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_toggle_all_docks(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_toggle_all_with_manual_close(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_toggle_all_docks_after_dock_move(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        let project = Project::test(fs, [], cx).await;
        let (workspace, cx) =
    ...
    #[inazuma::test]
    async fn test_flexible_dock_sizing(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_panel_size_state_persistence(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_flexible_panel_left_dock_sizing(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_panels(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_move_focused_panel_to_next_position(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        let project = Project::test(fs, [], cx).await;
        let (workspace, cx) =
    ...
    mod register_project_item_tests {
        ...
        impl carrot_project::ProjectItem for TestPngItem {
            fn try_open(
        ...
        impl Item for TestPngItemView {
            ...
            fn pane_role(&self, _cx: &App) -> crate::item::PaneRole {
                crate::item::PaneRole::Editor
            }
        }
        ...
        impl carrot_project::ProjectItem for TestIpynbItem {
            fn try_open(
                _project: &Entity<Project>,
                path: &ProjectPath,
                cx: &mut App,
    ...
    pub fn init_test(cx: &mut TestAppContext) {
        cx.update(|cx| {
            let settings_store = SettingsStore::test(cx);
            cx.set_global(settings_store);
            cx.set_global(carrot_db::AppDatabase::test_new());
            carrot_theme_settings::init(carrot_theme::LoadThemes::JustBase, cx);
        });
    }
