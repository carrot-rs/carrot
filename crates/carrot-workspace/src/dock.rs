...
pub trait Panel: Focusable + EventEmitter<PanelEvent> + Render + Sized {
    ...
    /// Minimum size enforced when the user resizes this panel via its drag
    /// handle. Default is the resize-handle width itself, which lets the
    /// panel collapse to nothing — most panels override this with a sane
    /// minimum (e.g. enough room for their header controls to stay usable).
    fn min_size(&self, _window: &Window, _cx: &App) -> Pixels {
        RESIZE_HANDLE_SIZE
    }
    ...
    fn icon(&self, window: &Window, cx: &App) -> Option<carrot_ui::IconName>;
...
pub trait PanelHandle: Send + Sync {
    ...
    fn default_size(&self, window: &Window, cx: &App) -> Pixels;
    fn min_size(&self, window: &Window, cx: &App) -> Pixels;
...
pub struct Dock {
    ...
    pub serialized_dock: Option<DockData>,
...
impl From<inazuma_settings_framework::DockPosition> for DockPosition {
    fn from(value: inazuma_settings_framework::DockPosition) -> Self {
        match value {
            inazuma_settings_framework::DockPosition::Left => Self::Left,
            inazuma_settings_framework::DockPosition::Bottom => Self::Bottom,
            inazuma_settings_framework::DockPosition::Right => Self::Right,
        }
    }
}

impl Into<inazuma_settings_framework::DockPosition> for DockPosition {
    fn into(self) -> inazuma_settings_framework::DockPosition {
        match self {
            Self::Left => inazuma_settings_framework::DockPosition::Left,
            Self::Bottom => inazuma_settings_framework::DockPosition::Bottom,
            Self::Right => inazuma_settings_framework::DockPosition::Right,
        }
    }
}
...
impl Dock {
    ...
    pub fn resize_active_panel(
        ...
        {
            let min = entry.panel.min_size(window, cx);
            let size = size.map(|size| size.max(min).round());
...
pub mod test {
    use super::*;
    use inazuma::{App, Context, Window, actions, div};
    ...
    impl Panel for TestPanel {
        ...
        fn icon(&self, _window: &Window, _: &App) -> Option<carrot_ui::IconName> {
            None
        }
