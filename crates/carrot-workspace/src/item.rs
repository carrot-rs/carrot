...
use anyhow::Result;
use carrot_client::{Client, proto};
use carrot_language::Capability;
pub use carrot_language::HighlightedText;
use carrot_project::{Project, ProjectEntryId, ProjectPath};
use carrot_ui::{Color, Icon, IntoElement, Label, LabelCommon};
use inazuma::{
...
/// Defines how an item interacts with smart file-open routing.
///
/// Every Item must declare its role explicitly — there is no default.
/// The compiler enforces a conscious decision per item type.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PaneRole {
    /// Will never be replaced by file-open (Cmd+P).
    /// Examples: Terminal, Settings, AI Chat.
    Terminal,
    /// Can be replaced by file-open. Cmd+P lands here.
    /// Examples: Code editor, Markdown viewer, Image viewer.
    Editor,
}
...
pub trait Item: Focusable + EventEmitter<Self::Event> + Render + Sized {
    ...
    /// Declares how this item interacts with smart file-open routing.
    /// Required — no default — every Item must declare its role explicitly.
    ...
    /// Live shell state for the item. Used by panels that surface CWD /
    /// git branch / diff stats per tab (see `carrot-vertical-tabs`).
    /// Default returns `None` for items without a shell — editors,
    /// settings pages, pickers, etc. Terminal items override this to
    /// expose their live `ShellContext`.
    fn shell_context(&self, _cx: &App) -> Option<carrot_shell_integration::ShellContext> {
        None
    }

    /// Backing `TerminalHandle` for items that wrap a PTY-backed
    /// terminal, used by panels that want to push commands into the
    /// user's shell (e.g. the vertical-tabs "install gh" flow mirrors
    /// the shell-install modal: on confirm, it writes `brew install
    /// gh\r` to the active terminal so the user sees the install
    /// happen in their own shell). Default `None` for non-terminal items.
    ...
    fn for_each_project_item(
        &self,
        _: &App,
        _: &mut dyn FnMut(EntityId, &dyn carrot_project::ProjectItem),
    ) {
    }
...
pub trait ItemHandle: 'static + Send {
    ...
    fn tab_tooltip_content(&self, cx: &App) -> Option<TabTooltipContent>;
    fn shell_context(&self, cx: &App) -> Option<carrot_shell_integration::ShellContext>;
    fn terminal_handle(&self, cx: &App) -> Option<carrot_terminal::TerminalHandle>;
    ...
    fn pane_role(&self, cx: &App) -> PaneRole;
    ...
    fn breadcrumb_prefix(&self, window: &mut Window, cx: &mut App) -> Option<inazuma::AnyElement>;
...
impl<T: Item> ItemHandle for Entity<T> {
    ...
    fn shell_context(&self, cx: &App) -> Option<carrot_shell_integration::ShellContext> {
        self.read(cx).shell_context(cx)
    }

    fn terminal_handle(&self, cx: &App) -> Option<carrot_terminal::TerminalHandle> {
        self.read(cx).terminal_handle(cx)
    }
    ...
    fn for_each_project_item(
        &self,
        cx: &App,
        f: &mut dyn FnMut(EntityId, &dyn carrot_project::ProjectItem),
    ) {
            ...
            cx.on_focus_out(
                ...
                move |workspace, _event, window, cx| {
                        ...
                        && item.workspace_settings(cx).autosave == AutosaveSetting::OnFocusChange
                    {
                        ...
                        let vim_mode = carrot_vim_mode_setting::VimModeSetting::is_enabled(cx);
                        let helix_mode = carrot_vim_mode_setting::HelixModeSetting::is_enabled(cx);
...
pub trait ProjectItem: Item {
    type Item: carrot_project::ProjectItem;
    ...
    /// A fallback handler, which will be called after [`carrot_project::ProjectItem::try_open`] fails,
...
pub trait FollowableItem: Item {
    ...
    fn update_agent_location(
        &mut self,
        _location: carrot_language::Anchor,
        _window: &mut Window,
        _cx: &mut Context<Self>,
    ) {
    }
...
pub trait FollowableItemHandle: ItemHandle {
    ...
    fn update_agent_location(
        &self,
        location: carrot_language::Anchor,
        window: &mut Window,
        cx: &mut App,
    );
}
...
pub mod test {
    use super::{Item, ItemEvent, PaneRole, SerializableItem, TabContentParams};
    use crate::{
        ItemId, ItemNavHistory, Workspace, WorkspaceId,
        item::{ItemBufferKind, SaveOptions},
    };
    use carrot_project::{Project, ProjectEntryId, ProjectPath, WorktreeId};
    use inazuma::{
    ...
    impl carrot_project::ProjectItem for TestProjectItem {
        fn try_open(
            _project: &Entity<Project>,
            _path: &ProjectPath,
            _cx: &mut App,
    ...
    impl Render for TestItem {
        fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
            let parent = inazuma::div().track_focus(&self.focus_handle(cx));
            self.child_focus_handles
                .iter()
                .fold(parent, |parent, child_handle| {
                    parent.child(inazuma::div().track_focus(child_handle))
                })
        }
    }
