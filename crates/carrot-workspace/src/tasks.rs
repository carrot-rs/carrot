...
use carrot_project::{TaskSourceKind, WorktreeId};
use carrot_remote::ConnectionState;
use carrot_task::{
    DebugScenario, ResolvedTask, SaveStrategy, SharedTaskContext, SpawnInTerminal, TaskContext,
    TaskTemplate,
};
use carrot_ui::Window;
use inazuma::{AppContext, Context, Entity, Task};
...
mod tests {
    ...
    use carrot_project::{FakeFs, Project, TaskSourceKind};
    ...
    async fn create_fixture(
    ...
    ) -> (Fixture, &mut inazuma::VisualTestContext) {
        cx.update(|cx| {
        ...
        let task = template
            .resolve_task("test", &carrot_task::TaskContext::default())
            .unwrap();
    ...
    impl TerminalProvider for TestTerminalProvider {
        fn spawn(
            &self,
            _task: carrot_task::SpawnInTerminal,
