...
use inazuma::{
    Along, AnyView, AnyWeakView, Axis, Bounds, Entity, IntoElement, MouseButton, Oklch, Pixels,
...
pub struct PaneRenderResult {
    pub element: inazuma::AnyElement,
            ...
            CollaboratorId::PeerId(peer_id) => {
                ...
                let leader_status_box = match leader.location {
                    ...
                    ParticipantLocation::UnsharedProject => Some(Label::new(format!(
                        "{} is viewing an unshared Carrot project",
                        leader.user.github_login
                    ))),
                    ParticipantLocation::External => Some(Label::new(format!(
                        "{} is viewing a window outside of Carrot",
                        leader.user.github_login
                    ))),
                };
...
mod element {
    ...
    use inazuma::{CursorStyle, Hitbox};
    use inazuma_settings_framework::Settings;
    ...
    impl Element for PaneAxisElement {
        ...
        fn paint(
            ...
            _inspector_id: Option<&inazuma::InspectorElementId>,
            bounds: inazuma::Bounds<carrot_ui::prelude::Pixels>,
            ...
            for (ix, child) in &mut layout.children.iter_mut().enumerate() {
                if overlay_opacity.is_some() || overlay_border.is_some() {
                    ...
                    {
                        window.paint_quad(inazuma::fill(overlay_bounds, overlay_background));
                    }
                    ...
                    if let Some(border) = overlay_border
                        ...
                        && child.is_leaf_pane
                    {
                        window.paint_quad(inazuma::quad(
                            overlay_bounds,
                            0.,
