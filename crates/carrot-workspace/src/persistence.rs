...
use inazuma::{Axis, Bounds, Task, WindowBounds, WindowId, point, size};
use inazuma_collections::{HashMap, HashSet, IndexSet};

use carrot_language::{LanguageName, Toolchain, ToolchainScope};
use carrot_remote::{
...
use carrot_ui::{App, SharedString, px};
use inazuma_util::{ResultExt, maybe, rel_path::RelPath};
use uuid::Uuid;
...
use model::{
    ...
    SerializedPaneGroup, SerializedSession, SerializedWorkspace,
};
...
impl carrot_sqlez::bindable::StaticColumnCount for SerializedAxis {}
impl carrot_sqlez::bindable::Bind for SerializedAxis {
    fn bind(
        &self,
        statement: &carrot_sqlez::statement::Statement,
        start_index: i32,
    ) -> anyhow::Result<i32> {
        match self.0 {
            inazuma::Axis::Horizontal => "Horizontal",
            inazuma::Axis::Vertical => "Vertical",
        }
        .bind(statement, start_index)
    }
}

impl carrot_sqlez::bindable::Column for SerializedAxis {
    fn column(
        statement: &mut carrot_sqlez::statement::Statement,
        start_index: i32,
...
impl carrot_sqlez::bindable::StaticColumnCount for Breakpoint {
    fn column_count() -> usize {
...
impl carrot_sqlez::bindable::Bind for Breakpoint {
    fn bind(
        &self,
        statement: &carrot_sqlez::statement::Statement,
        start_index: i32,
...
impl carrot_sqlez::bindable::StaticColumnCount for SerializedPixels {}

impl carrot_sqlez::bindable::Bind for SerializedPixels {
    fn bind(
        &self,
        statement: &carrot_sqlez::statement::Statement,
        start_index: i32,
    ...
    const MIGRATIONS: &[&str] = &[
        sql!(
            CREATE TABLE workspaces(
                ...
                dock_visible INTEGER, // Deprecated. Preserving so users can downgrade Carrot.
                dock_anchor TEXT, // Deprecated. Preserving so users can downgrade Carrot.
                dock_pane INTEGER, // Deprecated.  Preserving so users can downgrade Carrot.
        ...
        // Preserving so users can downgrade Carrot.
        sql!(
...
carrot_db::static_connection!(WorkspaceDb, []);
...
impl WorkspaceDb {
    ...
    pub fn workspace_for_id(&self, workspace_id: WorkspaceId) -> Option<SerializedWorkspace> {
        let (
            paths,
            paths_order,
            window_bounds,
        ...
        for (id, paths, remote_connection_id, timestamp) in self.recent_workspaces()? {
            ...
            let has_wsl_path = if cfg!(windows) {
                paths
                    .paths()
                    .iter()
                    .any(|path| inazuma_util::paths::WslPath::from_path(path).is_some())
            } else {
                false
            };
    ...
    // session was closed (i.e. when Carrot was quit).
    ...
    pub(crate) async fn save_trusted_worktrees(
        ...
        use carrot_db::sqlez::statement::Statement;
...
pub async fn resolve_worktree_workspaces(
    ...
    let resolved = futures::future::join_all(workspaces.into_iter().map(|entry| async move {
        ...
        let resolved_paths =
            futures::future::join_all(paths.iter().map(|path| {
                carrot_project::git_store::resolve_git_worktree_to_main_repo(fs, path)
            }))
            .await;
    ...
    let mut seen: inazuma_collections::HashMap<Vec<PathBuf>, usize> =
...
mod tests {
    ...
    async fn test_breakpoints() {
        ...
        let workspace = SerializedWorkspace {
            ...
            breakpoints: {
                let mut map = inazuma_collections::BTreeMap::default();
                map.insert(
                    Arc::from(path),
                    vec![
                        SourceBreakpoint {
    ...
    async fn test_remove_last_breakpoint() {
        ...
        let workspace_without_breakpoint = SerializedWorkspace {
            ...
            docks: Default::default(),
            centered_layout: false,
            breakpoints: inazuma_collections::BTreeMap::default(),
            session_id: None,
            window_id: None,
            user_toolchains: Default::default(),
        };
    ...
    #[inazuma::test]
    async fn test_last_session_workspace_locations(cx: &mut inazuma::TestAppContext) {
        ...
        let fs = carrot_fs::FakeFs::new(cx.executor());
    ...
    #[inazuma::test]
    async fn test_last_session_workspace_locations_remote(cx: &mut inazuma::TestAppContext) {
        let fs = carrot_fs::FakeFs::new(cx.executor());
        let db =
    ...
    async fn test_flush_serialization_completes_before_quit(cx: &mut inazuma::TestAppContext) {
        ...
        let (workspace, cx) = cx
            .add_window_view(|window, cx| crate::Workspace::test_new(project.clone(), window, cx));
        ...
        // alone is sufficient).
        let task = workspace.update_in(cx, |ws, window, cx| ws.flush_serialization(window, cx));
        task.await;
    ...
    #[inazuma::test]
    async fn test_resolve_worktree_workspaces(cx: &mut inazuma::TestAppContext) {
        let fs = carrot_fs::FakeFs::new(cx.executor());
