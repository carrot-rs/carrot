...
use carrot_editor::{CompletionProvider, SelectionEffects};
use carrot_editor::{
    CurrentLineHighlight, Editor, EditorElement, EditorEvent, EditorStyle, actions::Tab,
};
use carrot_language::{Buffer, LanguageRegistry, language_settings::SoftWrap};
...
use carrot_ui::{Divider, ListItem, ListItemSpacing, ListSubHeader, Tooltip, prelude::*};
use carrot_ui_input::ErasedEditor;
use carrot_workspace::{Workspace, WorkspaceSettings, client_side_decorations};
use inazuma::{
...
use inazuma_settings_framework::{ActionSequence, Settings};
        ...
        cx.update(|cx| {
            ...
            let window_decorations = match std::env::var("CARROT_WINDOW_DECORATIONS") {
                ...
                _ => match WorkspaceSettings::get_global(cx).window_decorations {
                    inazuma_settings_content::WindowDecorations::Server => {
                        inazuma::WindowDecorations::Server
                    }
                    inazuma_settings_content::WindowDecorations::Client => {
            ...
            cx.open_window(
                WindowOptions {
                    ...
                    kind: inazuma::WindowKind::Floating,
                    ..Default::default()
                },
                |window, cx| {
                    cx.new(|cx| {
        ...
        match self.filtered_entries.get(ix)? {
            RulePickerEntry::Header(title) => {
                let tooltip_text = if title.as_ref() == "Built-in Rules" {
                    "Built-in rules are those included out of the box with Carrot."
                } else {
...
impl RulesLibrary {
    ...
    fn move_down_from_title(
        &mut self,
        _: &carrot_actions::editor::MoveDown,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn move_up_from_body(
        &mut self,
        _: &carrot_actions::editor::MoveUp,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if let Some(rule_id) = self.active_rule_id
        ...
        div()
            ...
            .child(EditorElement::new(
                ...
                EditorStyle {
                    ...
                    edit_prediction_styles: carrot_editor::make_suggestion_styles(cx),
                    ..EditorStyle::default()
                },
            ))
    }
    ...
    fn render_active_rule(&mut self, cx: &mut Context<RulesLibrary>) -> inazuma::Stateful<Div> {
        div()
            .id("rule-editor")
            .h_full()
            .flex_grow()
