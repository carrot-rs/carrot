...
use carrot_client::TelemetrySettings;
use carrot_fs::Fs;
use carrot_project::project_settings::ProjectSettings;
use carrot_theme::{Appearance, SystemAppearance, ThemeRegistry};
use carrot_theme_settings::{ThemeAppearanceMode, ThemeName, ThemeSelection, ThemeSettings};
use carrot_ui::{
    Divider, StatefulInteractiveElement, SwitchField, ToggleButtonGroup, ToggleButtonGroupSize,
...
fn render_theme_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let system_appearance = carrot_theme::SystemAppearance::global(cx);
    ...
    fn write_mode_change(mode: ThemeAppearanceMode, cx: &mut App) {
        ...
        update_settings_file(fs, cx, move |settings, _cx| {
            carrot_theme_settings::set_mode(settings, mode);
        });
    }
    ...
    fn write_theme_change(
        ...
        update_settings_file(fs, cx, move |settings, cx| match theme_mode {
            ThemeAppearanceMode::System => {
                ...
                settings.theme.theme = Some(inazuma_settings_framework::ThemeSelection::Dynamic {
                    mode: ThemeAppearanceMode::System,
                    light: ThemeName(light_theme.into()),
                    dark: ThemeName(dark_theme.into()),
                });
            }
            ThemeAppearanceMode::Light => carrot_theme_settings::set_theme(
                settings,
                theme,
                Appearance::Light,
                *SystemAppearance::global(cx),
            ),
            ThemeAppearanceMode::Dark => carrot_theme_settings::set_theme(
                settings,
                theme,
                Appearance::Dark,
                *SystemAppearance::global(cx),
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Carrot by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Carrot by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new("Base Keymap")).child(
        ...
        .full_width()
        .tab_index(tab_index)
        .size(carrot_ui::ToggleButtonGroupSize::Medium)
        .style(carrot_ui::ToggleButtonGroupStyle::Outlined),
    );
    ...
    fn write_keymap_base(keymap_base: BaseKeymap, cx: &App) {
        ...
        carrot_telemetry::event!("Welcome Keymap Changed", keymap = keymap_base);
    }
}
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    let tooltip_description = "Carrot can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some(
            "Automatically mark all new projects as trusted to unlock all Carrot's features".into(),
        ),
...
pub(crate) fn render_basics_page(cx: &mut App) -> impl IntoElement {
    ...
    v_flex()
        ...
        .child(Divider::horizontal().color(carrot_ui::DividerColor::BorderVariant))
