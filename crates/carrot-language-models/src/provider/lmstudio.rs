...
use carrot_lmstudio::{LMSTUDIO_API_URL, ModelType, get_models};
use futures::Stream;
use futures::{FutureExt, StreamExt, future::BoxFuture, stream::BoxStream};
use inazuma::{AnyView, App, AsyncApp, Context, CursorStyle, Entity, Subscription, Task};
...
pub use inazuma_settings_framework::LmStudioAvailableModel as AvailableModel;
...
pub struct State {
    ...
    http_client: Arc<dyn HttpClient>,
    available_models: Vec<carrot_lmstudio::Model>,
...
impl State {
    ...
    fn fetch_models(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let mut models: Vec<carrot_lmstudio::Model> = models
                .into_iter()
                .filter(|model| model.r#type != ModelType::Embeddings)
                .map(|model| {
                    carrot_lmstudio::Model::new(
                        &model.id,
                        None,
                        model
...
impl LanguageModelProvider for LmStudioLanguageModelProvider {
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models: BTreeMap<String, carrot_lmstudio::Model> = BTreeMap::default();
...
impl LmStudioLanguageModel {
    fn to_lmstudio_request(
    ...
    ) -> carrot_lmstudio::ChatCompletionRequest {
            ...
            for content in message.content {
                match content {
                    MessageContent::Text(text) => add_message_content_part(
                        carrot_lmstudio::MessagePart::Text { text },
                        message.role,
                        &mut messages,
                    ),
                    ...
                    MessageContent::Image(image) => {
                        add_message_content_part(
                            carrot_lmstudio::MessagePart::Image {
                                image_url: carrot_lmstudio::ImageUrl {
                    ...
                    MessageContent::ToolUse(tool_use) => {
                        let tool_call = carrot_lmstudio::ToolCall {
                            id: tool_use.id.to_string(),
                            content: carrot_lmstudio::ToolCallContent::Function {
                                function: carrot_lmstudio::FunctionContent {
                        ...
                        if let Some(carrot_lmstudio::ChatMessage::Assistant {
                            tool_calls, ..
                        }) = messages.last_mut()
                        {
                            tool_calls.push(tool_call);
                        } else {
                            messages.push(carrot_lmstudio::ChatMessage::Assistant {
                                content: None,
                    ...
                    MessageContent::ToolResult(tool_result) => {
                        let content = match &tool_result.content {
                            LanguageModelToolResultContent::Text(text) => {
                                vec![carrot_lmstudio::MessagePart::Text {
                                    text: text.to_string(),
                                }]
                            }
                            LanguageModelToolResultContent::Image(image) => {
                                vec![carrot_lmstudio::MessagePart::Image {
                                    image_url: carrot_lmstudio::ImageUrl {
                                        url: image.to_base64_url(),
                                        detail: None,
                                    },
                        ...
                        messages.push(carrot_lmstudio::ChatMessage::Tool {
        ...
        carrot_lmstudio::ChatCompletionRequest {
            ...
            // It would be a bad UX to silently override these settings from Carrot, so we pass no temperature as a default.
            temperature: request.temperature.or(None),
            tools: request
                .tools
                .into_iter()
                .map(|tool| carrot_lmstudio::ToolDefinition::Function {
                    function: carrot_lmstudio::FunctionDefinition {
                        name: tool.name,
            ...
            tool_choice: request.tool_choice.map(|choice| match choice {
                LanguageModelToolChoice::Auto => carrot_lmstudio::ToolChoice::Auto,
                LanguageModelToolChoice::Any => carrot_lmstudio::ToolChoice::Required,
                LanguageModelToolChoice::None => carrot_lmstudio::ToolChoice::None,
            }),
        }
    }
    ...
    fn stream_completion(
        &self,
        request: carrot_lmstudio::ChatCompletionRequest,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<futures::stream::BoxStream<'static, Result<carrot_lmstudio::ResponseStreamEvent>>>,
    > {
        let http_client = self.http_client.clone();
        ...
        let future = self.request_limiter.stream(async move {
            let stream = carrot_lmstudio::stream_chat_completion(
                http_client.as_ref(),
                &api_url,
                api_key.as_deref(),
                request,
...
impl LmStudioEventMapper {
    ...
    pub fn map_stream(
        mut self,
        events: Pin<Box<dyn Send + Stream<Item = Result<carrot_lmstudio::ResponseStreamEvent>>>>,
    ...
    pub fn map_event(
        &mut self,
        event: carrot_lmstudio::ResponseStreamEvent,
...
fn add_message_content_part(
    new_part: carrot_lmstudio::MessagePart,
    role: Role,
    messages: &mut Vec<carrot_lmstudio::ChatMessage>,
) {
    match (role, messages.last_mut()) {
        (Role::User, Some(carrot_lmstudio::ChatMessage::User { content }))
        | (
            Role::Assistant,
            Some(carrot_lmstudio::ChatMessage::Assistant {
                content: Some(content),
                ..
            }),
        )
        | (Role::System, Some(carrot_lmstudio::ChatMessage::System { content })) => {
            content.push_part(new_part);
        }
        _ => {
            messages.push(match role {
                Role::User => carrot_lmstudio::ChatMessage::User {
                    content: carrot_lmstudio::MessageContent::from(vec![new_part]),
                },
                Role::Assistant => carrot_lmstudio::ChatMessage::Assistant {
                    content: Some(carrot_lmstudio::MessageContent::from(vec![new_part])),
                    tool_calls: Vec::new(),
                },
                Role::System => carrot_lmstudio::ChatMessage::System {
                    content: carrot_lmstudio::MessageContent::from(vec![new_part]),
                },
            });
        }
