...
use carrot_ai_onboarding::YoungAccountBanner;
use carrot_anthropic::AnthropicModelMode;
use carrot_client::{Client, UserStore, carrot_urls};
use carrot_cloud_api_types::{OrganizationId, Plan};
use carrot_cloud_llm_client::{
    CARROT_VERSION_HEADER_NAME, CLIENT_SUPPORTS_STATUS_MESSAGES_HEADER_NAME,
...
use carrot_google_ai::GoogleModelMode;
use carrot_http_client::http::{HeaderMap, HeaderValue};
use carrot_http_client::{AsyncBody, HttpClient, HttpRequestExt, Method, Response, StatusCode};
use carrot_language_model::{
...
use carrot_release_channel::AppVersion;
use carrot_ui::{TintColor, prelude::*};
use futures::{
    AsyncBufReadExt, FutureExt, Stream, StreamExt,
    future::BoxFuture,
    stream::{self, BoxStream},
};
use inazuma::{AnyElement, AnyView, App, AsyncApp, Context, Entity, Subscription, Task};
pub use inazuma_settings_framework::CarrotDotDevAvailableModel as AvailableModel;
pub use inazuma_settings_framework::CarrotDotDevAvailableProvider as AvailableProvider;
...
const PROVIDER_ID: LanguageModelProviderId = carrot_language_model::ZED_CLOUD_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = carrot_language_model::ZED_CLOUD_PROVIDER_NAME;

#[derive(Default, Clone, Debug, PartialEq)]
pub struct CarrotDotDevSettings {
...
pub struct State {
    ...
    models: Vec<Arc<carrot_cloud_llm_client::LanguageModel>>,
    default_model: Option<Arc<carrot_cloud_llm_client::LanguageModel>>,
    default_fast_model: Option<Arc<carrot_cloud_llm_client::LanguageModel>>,
    recommended_models: Vec<Arc<carrot_cloud_llm_client::LanguageModel>>,
...
impl State {
        ...
        Self {
            ...
            _user_store_subscription: cx.subscribe(
                ...
                move |this, _user_store, event, cx| match event {
                    carrot_client::user::Event::PrivateUserInfoUpdated => {
    ...
    async fn fetch_models(
        ...
        let request = carrot_http_client::Request::builder()
            .method(Method::GET)
            .header(CLIENT_SUPPORTS_X_AI_HEADER_NAME, "true")
            .uri(http_client.build_carrot_llm_url("/models", &[])?.as_ref())
...
impl CloudLanguageModelProvider {
    ...
    fn create_language_model(
        &self,
        model: Arc<carrot_cloud_llm_client::LanguageModel>,
        llm_api_token: LlmApiToken,
        user_store: Entity<UserStore>,
...
impl LanguageModelProvider for CloudLanguageModelProvider {
    ...
    fn icon(&self) -> IconOrSvg {
        IconOrSvg::Icon(IconName::AiCarrot)
    }
    ...
    fn configuration_view(
        &self,
        _target_agent: carrot_language_model::ConfigurationViewTargetAgent,
        _: &mut Window,
        cx: &mut App,
    ) -> AnyView {
    ...
    async fn perform_llm_completion(
        ...
        loop {
            let request = carrot_http_client::Request::builder()
                ...
                .uri(
                    http_client
                        .build_carrot_llm_url("/completions", &[])?
                        .as_ref(),
                )
                .when_some(app_version.as_ref(), |builder, app_version| {
                    builder.header(CARROT_VERSION_HEADER_NAME, app_version.to_string())
                })
...
/// Represents error responses from Carrot's cloud API.
///
...
impl LanguageModel for CloudLanguageModel {
    ...
    fn upstream_provider_id(&self) -> LanguageModelProviderId {
        use carrot_cloud_llm_client::LanguageModelProvider::*;
        match self.model.provider {
            Anthropic => carrot_language_model::ANTHROPIC_PROVIDER_ID,
            OpenAi => carrot_language_model::OPEN_AI_PROVIDER_ID,
            Google => carrot_language_model::GOOGLE_PROVIDER_ID,
            XAi => carrot_language_model::X_AI_PROVIDER_ID,
        }
    }
    ...
    fn upstream_provider_name(&self) -> LanguageModelProviderName {
        use carrot_cloud_llm_client::LanguageModelProvider::*;
        match self.model.provider {
            Anthropic => carrot_language_model::ANTHROPIC_PROVIDER_NAME,
            OpenAi => carrot_language_model::OPEN_AI_PROVIDER_NAME,
            Google => carrot_language_model::GOOGLE_PROVIDER_NAME,
            XAi => carrot_language_model::X_AI_PROVIDER_NAME,
        }
    }
    ...
    fn telemetry_id(&self) -> String {
        format!("carrot.dev/{}", self.model.id)
    }

    fn tool_input_format(&self) -> LanguageModelToolSchemaFormat {
        match self.model.provider {
            carrot_cloud_llm_client::LanguageModelProvider::Anthropic
            | carrot_cloud_llm_client::LanguageModelProvider::OpenAi => {
                LanguageModelToolSchemaFormat::JsonSchema
            }
            carrot_cloud_llm_client::LanguageModelProvider::Google
            | carrot_cloud_llm_client::LanguageModelProvider::XAi => {
    ...
    fn cache_configuration(&self) -> Option<LanguageModelCacheConfiguration> {
        match &self.model.provider {
            ...
            carrot_cloud_llm_client::LanguageModelProvider::OpenAi
            | carrot_cloud_llm_client::LanguageModelProvider::XAi
            | carrot_cloud_llm_client::LanguageModelProvider::Google => None,
        }
    }
    ...
    fn count_tokens(
        ...
        match self.model.provider {
            carrot_cloud_llm_client::LanguageModelProvider::Anthropic => cx
                .background_spawn(async move { count_anthropic_tokens_with_tiktoken(request) })
                .boxed(),
            carrot_cloud_llm_client::LanguageModelProvider::OpenAi => {
                let model = match carrot_open_ai::Model::from_id(&self.model.id.0) {
                    Ok(model) => model,
            ...
            carrot_cloud_llm_client::LanguageModelProvider::XAi => {
                let model = match carrot_x_ai::Model::from_id(&self.model.id.0) {
                    Ok(model) => model,
                    Err(err) => return async move { Err(anyhow!(err)) }.boxed(),
                };
            ...
            carrot_cloud_llm_client::LanguageModelProvider::Google => {
                ...
                async move {
                    ...
                    let request_body = CountTokensBody {
                        provider: carrot_cloud_llm_client::LanguageModelProvider::Google,
                        model: model_id,
                        provider_request: serde_json::to_value(
                            &carrot_google_ai::CountTokensRequest {
                    ...
                    let request = carrot_http_client::Request::builder()
                        ...
                        .uri(
                            http_client
                                .build_carrot_llm_url("/count_tokens", &[])?
                                .as_ref(),
                        )
                        .header("Content-Type", "application/json")
                        .header("Authorization", format!("Bearer {token}"))
        ...
        match self.model.provider {
            carrot_cloud_llm_client::LanguageModelProvider::Anthropic => {
                let effort = request
                    .thinking_effort
                    .as_ref()
                    .and_then(|effort| carrot_anthropic::Effort::from_str(effort).ok());
                ...
                if enable_thinking && effort.is_some() {
                    request.thinking = Some(carrot_anthropic::Thinking::Adaptive);
                    request.output_config = Some(carrot_anthropic::OutputConfig { effort });
                }
                ...
                let future = self.request_limiter.stream(async move {
                    let PerformLlmCompletionResponse {
                        ...
                        CompletionBody {
                            thread_id,
                            prompt_id,
                            provider: carrot_cloud_llm_client::LanguageModelProvider::Anthropic,
            ...
            carrot_cloud_llm_client::LanguageModelProvider::OpenAi => {
                ...
                let effort = request
                    .thinking_effort
                    .as_ref()
                    .and_then(|effort| carrot_open_ai::ReasoningEffort::from_str(effort).ok());
                ...
                if enable_thinking && let Some(effort) = effort {
                    request.reasoning = Some(carrot_open_ai::responses::ReasoningConfig {
                        effort,
                        summary: Some(carrot_open_ai::responses::ReasoningSummaryMode::Auto),
                    });
                }
                ...
                let future = self.request_limiter.stream(async move {
                    let PerformLlmCompletionResponse {
                        ...
                        CompletionBody {
                            thread_id,
                            prompt_id,
                            provider: carrot_cloud_llm_client::LanguageModelProvider::OpenAi,
                            model: request.model.clone(),
                            provider_request: serde_json::to_value(&request)
                                .map_err(|e| anyhow!(e))?,
                        },
                ...
                let future = self.request_limiter.stream(async move {
                    let PerformLlmCompletionResponse {
                        ...
                        CompletionBody {
                            thread_id,
                            prompt_id,
                            provider: carrot_cloud_llm_client::LanguageModelProvider::XAi,
                            model: request.model.clone(),
                            provider_request: serde_json::to_value(&request)
                                .map_err(|e| anyhow!(e))?,
                        },
...
fn provider_name(
    provider: &carrot_cloud_llm_client::LanguageModelProvider,
) -> LanguageModelProviderName {
    match provider {
        carrot_cloud_llm_client::LanguageModelProvider::Anthropic => {
            carrot_language_model::ANTHROPIC_PROVIDER_NAME
        }
        carrot_cloud_llm_client::LanguageModelProvider::OpenAi => {
            carrot_language_model::OPEN_AI_PROVIDER_NAME
        }
        carrot_cloud_llm_client::LanguageModelProvider::Google => {
            carrot_language_model::GOOGLE_PROVIDER_NAME
        }
        carrot_cloud_llm_client::LanguageModelProvider::XAi => {
            carrot_language_model::X_AI_PROVIDER_NAME
        }
    }
}
...
impl RenderOnce for CarrotAiConfiguration {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let (subscription_text, has_paid_plan) = match self.plan {
            Some(Plan::CarrotPro) => (
                "You have access to Carrot's hosted models through your Pro subscription.",
                true,
            ),
            Some(Plan::CarrotProTrial) => (
                "You have access to Carrot's hosted models through your Pro trial.",
                false,
            ),
            Some(Plan::CarrotStudent) => (
                "You have access to Carrot's hosted models through your Student subscription.",
                true,
            ),
            Some(Plan::CarrotBusiness) => (
                "You have access to Carrot's hosted models through your Organization.",
                true,
            ),
            Some(Plan::CarrotFree) | None => (
                if self.eligible_for_trial {
                    "Subscribe for access to Carrot's hosted models. Start with a 14 day free trial."
                } else {
                    "Subscribe for access to Carrot's hosted models."
                },
                false,
            ),
        };
        ...
        let manage_subscription_buttons = if has_paid_plan {
            Button::new("manage_settings", "Manage Subscription")
                ...
                .on_click(|_, _, cx| cx.open_url(&carrot_urls::account_url(cx)))
                .into_any_element()
        } else if self.plan.is_none() || self.eligible_for_trial {
            Button::new("start_trial", "Start 14-day Free Pro Trial")
                .full_width()
                .style(carrot_ui::ButtonStyle::tinted(carrot_ui::TintColor::Accent))
                .on_click(|_, _, cx| cx.open_url(&carrot_urls::start_trial_url(cx)))
                .into_any_element()
        } else {
            Button::new("upgrade", "Upgrade to Pro")
                .full_width()
                .style(carrot_ui::ButtonStyle::tinted(carrot_ui::TintColor::Accent))
                .on_click(|_, _, cx| cx.open_url(&carrot_urls::upgrade_to_carrot_pro_url(cx)))
                .into_any_element()
        };
        ...
        if !self.is_connected {
            return v_flex()
                .gap_2()
                .child(Label::new("Sign in to have access to Carrot's complete agentic experience with hosted models."))
                .child(
                    Button::new("sign_in", "Sign In to use Carrot AI")
            ...
            if self.account_too_young {
                this.child(YoungAccountBanner).child(
                    Button::new("upgrade", "Upgrade to Pro")
                        ...
                        .full_width()
                        .on_click(|_, _, cx| {
                            cx.open_url(&carrot_urls::upgrade_to_carrot_pro_url(cx))
                        }),
                )
            } else {
                this.text_sm()
...
impl Component for CarrotAiConfiguration {
    fn name() -> &'static str {
        ...
        Some(
            v_flex()
                ...
                .children(vec![
                    ...
                    single_example(
                        "Free Plan",
                        configuration(true, Some(Plan::CarrotFree), true, false),
                    ),
                    single_example(
                        "Carrot Pro Trial Plan",
                        configuration(true, Some(Plan::CarrotProTrial), true, false),
                    ),
                    single_example(
                        "Carrot Pro Plan",
                        configuration(true, Some(Plan::CarrotPro), true, false),
                    ),
                ])
                .into_any_element(),
        )
...
mod tests {
    use super::*;
    use carrot_http_client::http::{HeaderMap, StatusCode};
    use carrot_language_model::LanguageModelCompletionError;
    ...
    fn test_api_error_conversion_with_upstream_http_error() {
        ...
        // Regular 500 error without upstream_http_error should remain ApiInternalServerError for Carrot
