...
use carrot_x_ai::{Model, XAI_API_URL};
use futures::{FutureExt, StreamExt, future::BoxFuture};
use inazuma::{AnyView, App, AsyncApp, Context, Entity, Task, Window};
use inazuma_collections::BTreeMap;
pub use inazuma_settings_framework::XaiAvailableModel as AvailableModel;
...
impl XAiLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_x_ai::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for XAiLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_x_ai::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_x_ai::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        for model in carrot_x_ai::Model::iter() {
            if !matches!(model, carrot_x_ai::Model::Custom { .. }) {
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("To use Carrot's agent with xAI, you need to add an API key. Follow these steps:"))
                .child(
                    List::new()
                        .child(
                            ListBulletItem::new("")
