...
use carrot_ui::{ButtonLink, ConfiguredApiCard, List, ListBulletItem, prelude::*};
use carrot_ui_input::InputField;
use futures::{FutureExt, Stream, StreamExt, future::BoxFuture, stream::BoxStream};
use inazuma::{AnyView, App, AsyncApp, Context, Entity, Task};
...
const PROVIDER_ID: LanguageModelProviderId = carrot_language_model::ANTHROPIC_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = carrot_language_model::ANTHROPIC_PROVIDER_NAME;

#[derive(Default, Clone, Debug, PartialEq)]
pub struct AnthropicSettings {
    pub api_url: String,
    /// Extend Carrot's list of Anthropic models.
...
impl AnthropicLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_anthropic::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for AnthropicLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_anthropic::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_anthropic::Model::default_fast()))
    }

    fn recommended_models(&self, _cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        [carrot_anthropic::Model::ClaudeSonnet4_6]
            .into_iter()
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        // Add base models from carrot_anthropic::Model::iter()
        for model in carrot_anthropic::Model::iter() {
            if !matches!(model, carrot_anthropic::Model::Custom { .. }) {
        ...
        for model in &AnthropicLanguageModelProvider::settings(cx).available_models {
            models.insert(
                model.name.clone(),
                carrot_anthropic::Model::Custom {
                    name: model.name.clone(),
                    display_name: model.display_name.clone(),
                    max_tokens: model.max_tokens,
                    tool_override: model.tool_override.clone(),
                    cache_configuration: model.cache_configuration.as_ref().map(|config| {
                        carrot_anthropic::AnthropicModelCacheConfiguration {
                    ...
                    mode: match model.mode.unwrap_or_default() {
                        inazuma_settings_framework::ModelMode::Default => {
...
pub struct AnthropicModel {
    id: LanguageModelId,
    model: carrot_anthropic::Model,
    state: Entity<State>,
    http_client: Arc<dyn HttpClient>,
    request_limiter: RateLimiter,
}

fn to_anthropic_content(content: MessageContent) -> Option<carrot_anthropic::RequestContent> {
    match content {
        MessageContent::Text(text) => {
            ...
            if !text.is_empty() {
                Some(carrot_anthropic::RequestContent::Text {
                    text,
                    cache_control: None,
                })
            } else {
        ...
        MessageContent::Thinking {
            ...
            if let Some(signature) = signature
                && !thinking.is_empty()
            {
                Some(carrot_anthropic::RequestContent::Thinking {
                    thinking,
                    signature,
                    cache_control: None,
                })
        ...
        MessageContent::RedactedThinking(data) => {
            if !data.is_empty() {
                Some(carrot_anthropic::RequestContent::RedactedThinking { data })
            } else {
                None
            }
        }
        MessageContent::Image(image) => Some(carrot_anthropic::RequestContent::Image {
            source: carrot_anthropic::ImageSource {
        ...
        MessageContent::ToolUse(tool_use) => Some(carrot_anthropic::RequestContent::ToolUse {
        ...
        MessageContent::ToolResult(tool_result) => {
            Some(carrot_anthropic::RequestContent::ToolResult {
...
pub fn into_anthropic_count_tokens_request(
    ...
    let mut new_messages: Vec<carrot_anthropic::Message> = Vec::new();
    ...
    for message in request.messages {
        ...
        match message.role {
            Role::User | Role::Assistant => {
                let anthropic_message_content: Vec<carrot_anthropic::RequestContent> = message
                    .content
                    .into_iter()
                    .filter_map(to_anthropic_content)
                    .collect();
                let anthropic_role = match message.role {
                    Role::User => carrot_anthropic::Role::User,
                    Role::Assistant => carrot_anthropic::Role::Assistant,
                ...
                new_messages.push(carrot_anthropic::Message {
                    role: anthropic_role,
    ...
    CountTokensRequest {
        ...
        system: if system_message.is_empty() {
            ...
            Some(carrot_anthropic::StringOrContents::String(system_message))
        },
        thinking: if request.thinking_allowed {
            match mode {
                AnthropicModelMode::Thinking { budget_tokens } => {
                    Some(carrot_anthropic::Thinking::Enabled { budget_tokens })
                }
                AnthropicModelMode::AdaptiveThinking => Some(carrot_anthropic::Thinking::Adaptive),
        ...
        tools: request
            .tools
            .into_iter()
            .map(|tool| carrot_anthropic::Tool {
                name: tool.name,
        ...
        tool_choice: request.tool_choice.map(|choice| match choice {
            LanguageModelToolChoice::Auto => carrot_anthropic::ToolChoice::Auto,
            LanguageModelToolChoice::Any => carrot_anthropic::ToolChoice::Any,
            LanguageModelToolChoice::None => carrot_anthropic::ToolChoice::None,
        }),
    }
}

/// Estimate tokens using tiktoken. Used as a fallback when the API is unavailable,
/// or by providers (like Carrot Cloud) that don't have direct Anthropic API access.
...
impl AnthropicModel {
    fn stream_completion(
        &self,
        request: carrot_anthropic::Request,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<
            BoxStream<'static, Result<carrot_anthropic::Event, AnthropicError>>,
    ...
    > {
        ...
        async move {
            ...
            let request = carrot_anthropic::stream_completion(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
...
impl LanguageModel for AnthropicModel {
    ...
    fn supported_effort_levels(&self) -> Vec<carrot_language_model::LanguageModelEffortLevel> {
        if self.model.supports_adaptive_thinking() {
            vec![
                carrot_language_model::LanguageModelEffortLevel {
                    name: "Low".into(),
                    value: "low".into(),
                    is_default: false,
                },
    ...
    fn count_tokens(
        ...
        async move {
            ...
            match carrot_anthropic::count_tokens(
                http_client.as_ref(),
                &api_url,
                &api_key,
                count_request,
...
pub fn into_anthropic(
    ...
    for message in request.messages {
        ...
        match message.role {
            Role::User | Role::Assistant => {
                let mut anthropic_message_content: Vec<carrot_anthropic::RequestContent> = message
                    .content
                    .into_iter()
                    .filter_map(to_anthropic_content)
                    .collect();
                ...
                if message.cache {
                    let cache_control_value = Some(carrot_anthropic::CacheControl {
                        cache_type: carrot_anthropic::CacheControlType::Ephemeral,
                    });
                    for message_content in anthropic_message_content.iter_mut().rev() {
                        match message_content {
                            carrot_anthropic::RequestContent::RedactedThinking { .. } => {
                                // Caching is not possible, fallback to next message
                            }
                            carrot_anthropic::RequestContent::Text { cache_control, .. }
                            | carrot_anthropic::RequestContent::Thinking {
                                cache_control, ..
                            }
                            | carrot_anthropic::RequestContent::Image { cache_control, .. }
                            | carrot_anthropic::RequestContent::ToolUse { cache_control, .. }
                            | carrot_anthropic::RequestContent::ToolResult {
                                cache_control, ..
                            } => {
            ...
            && matches!(mode, AnthropicModelMode::AdaptiveThinking)
        {
            request.thinking_effort.as_deref().and_then(|effort| {
                let effort = match effort {
                    "low" => Some(carrot_anthropic::Effort::Low),
                    "medium" => Some(carrot_anthropic::Effort::Medium),
                    "high" => Some(carrot_anthropic::Effort::High),
                    "max" => Some(carrot_anthropic::Effort::Max),
                    _ => None,
                };
                effort.map(|effort| carrot_anthropic::OutputConfig {
                    effort: Some(effort),
                })
            })
        } else {
...
fn convert_usage(usage: &Usage) -> carrot_language_model::TokenUsage {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new(format!("To use {}, you need to add an API key. Follow these steps:", match &self.target_agent {
                    ConfigurationViewTargetAgent::CarrotAgent => "Carrot's agent with Anthropic".into(),
                ...
                .child(
                    Label::new(
                        format!("You can also set the {API_KEY_ENV_VAR_NAME} environment variable and restart Carrot."),
                    )
...
mod tests {
    ...
    use carrot_language_model::{LanguageModelRequestMessage, MessageContent};
    ...
    fn test_cache_control_only_on_last_segment() {
        let request = LanguageModelRequest {
            messages: vec![LanguageModelRequestMessage {
                ...
                content: vec![
                    ...
                    MessageContent::Image(carrot_language_model::LanguageModelImage::empty()),
                    MessageContent::Image(carrot_language_model::LanguageModelImage::empty()),
                    MessageContent::Image(carrot_language_model::LanguageModelImage::empty()),
                    MessageContent::Image(carrot_language_model::LanguageModelImage::empty()),
                ],
        ...
        assert!(matches!(
            message.content[0],
            carrot_anthropic::RequestContent::Text {
                cache_control: None,
                ..
            }
        ));
        for i in 1..3 {
            assert!(matches!(
                message.content[i],
                carrot_anthropic::RequestContent::Image {
                    cache_control: None,
                    ..
                }
            ));
        ...
        assert!(matches!(
            message.content[4],
            carrot_anthropic::RequestContent::Image {
                cache_control: Some(carrot_anthropic::CacheControl {
                    cache_type: carrot_anthropic::CacheControlType::Ephemeral,
                }),
                ..
            }
    ...
    fn test_unsigned_thinking_blocks_stripped() {
        ...
        let assistant_message = result
            .messages
            .iter()
            .find(|m| m.role == carrot_anthropic::Role::Assistant)
        ...
        assert!(matches!(
            ...
            carrot_anthropic::RequestContent::Text { text, .. } if text == "Some response text"
        ));
    }
    ...
    fn test_signed_thinking_blocks_preserved() {
        ...
        assert!(matches!(
            &assistant_message.content[0],
            carrot_anthropic::RequestContent::Thinking { thinking, signature, .. }
    ...
    fn test_only_unsigned_thinking_block_omits_entire_message() {
        ...
        let assistant_messages: Vec<_> = result
            .messages
            .iter()
            .filter(|m| m.role == carrot_anthropic::Role::Assistant)
            .collect();
