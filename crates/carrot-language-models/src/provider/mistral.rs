...
use inazuma_collections::BTreeMap;
...
pub use carrot_mistral::{MISTRAL_API_URL, StreamResponse};
use carrot_ui::{ButtonLink, ConfiguredApiCard, List, ListBulletItem, prelude::*};
use carrot_ui_input::InputField;
use futures::{FutureExt, Stream, StreamExt, future::BoxFuture, stream::BoxStream};
use inazuma::{AnyView, App, AsyncApp, Context, Entity, Global, SharedString, Task, Window};
pub use inazuma_settings_framework::MistralAvailableModel as AvailableModel;
...
impl MistralLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_mistral::Model) -> Arc<dyn LanguageModel> {
    ...
    pub fn api_url(cx: &App) -> SharedString {
        ...
        if api_url.is_empty() {
            carrot_mistral::MISTRAL_API_URL.into()
        } else {
...
impl LanguageModelProvider for MistralLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_mistral::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_mistral::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        // Add base models from carrot_mistral::Model::iter()
        for model in carrot_mistral::Model::iter() {
            if !matches!(model, carrot_mistral::Model::Custom { .. }) {
...
impl MistralLanguageModel {
    fn stream_completion(
        &self,
        request: carrot_mistral::Request,
        affinity: Option<String>,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<futures::stream::BoxStream<'static, Result<carrot_mistral::StreamResponse>>>,
    > {
        ...
        let future = self.request_limiter.stream(async move {
            ...
            let request = carrot_mistral::stream_completion(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
...
pub fn into_mistral(
...
) -> (carrot_mistral::Request, Option<String>) {
    let stream = true;
    ...
    for message in &request.messages {
        match message.role {
            Role::User => {
                let mut message_content = carrot_mistral::MessageContent::empty();
                for content in &message.content {
                    match content {
                        MessageContent::Text(text) => {
                            message_content.push_part(carrot_mistral::MessagePart::Text {
                                text: text.clone(),
                            });
                        }
                        MessageContent::Image(image_content) => {
                            if model.supports_images() {
                                message_content.push_part(carrot_mistral::MessagePart::ImageUrl {
                        ...
                        MessageContent::Thinking { text, .. } => {
                            if model.supports_thinking() {
                                message_content.push_part(carrot_mistral::MessagePart::Thinking {
                                    thinking: vec![carrot_mistral::ThinkingPart::Text {
                                        text: text.clone(),
                                    }],
                                });
                            }
                        ...
                        MessageContent::ToolResult(tool_result) => {
                            let tool_content = match &tool_result.content {
                                ...
                                LanguageModelToolResultContent::Image(_) => {
                                    "[Tool responded with an image, but Carrot doesn't support these in Mistral models yet]".to_string()
                                }
                            };
                            messages.push(carrot_mistral::RequestMessage::Tool {
                ...
                if !matches!(message_content, carrot_mistral::MessageContent::Plain { ref content } if content.is_empty())
                {
                    messages.push(carrot_mistral::RequestMessage::User {
            ...
            Role::Assistant => {
                for content in &message.content {
                    match content {
                        ...
                        MessageContent::Text(text) => {
                            messages.push(carrot_mistral::RequestMessage::Assistant {
                                content: Some(carrot_mistral::MessageContent::Plain {
                                    content: text.clone(),
                                }),
                                tool_calls: Vec::new(),
                            });
                        ...
                        MessageContent::Thinking { text, .. } => {
                            if model.supports_thinking() {
                                messages.push(carrot_mistral::RequestMessage::Assistant {
                                    content: Some(carrot_mistral::MessageContent::Multipart {
                                        content: vec![carrot_mistral::MessagePart::Thinking {
                                            thinking: vec![carrot_mistral::ThinkingPart::Text {
                                                text: text.clone(),
                                            }],
                                        }],
                        ...
                        MessageContent::ToolUse(tool_use) => {
                            let tool_call = carrot_mistral::ToolCall {
                                id: tool_use.id.to_string(),
                                content: carrot_mistral::ToolCallContent::Function {
                                    function: carrot_mistral::FunctionContent {
                            ...
                            if let Some(carrot_mistral::RequestMessage::Assistant {
                                tool_calls,
                                ..
                            }) = messages.last_mut()
                            {
            ...
            Role::System => {
                for content in &message.content {
                    match content {
                        MessageContent::Text(text) => {
                            messages.push(carrot_mistral::RequestMessage::System {
                                content: carrot_mistral::MessageContent::Plain {
                                    content: text.clone(),
                                },
                            });
                        }
                        MessageContent::Thinking { text, .. } => {
                            if model.supports_thinking() {
                                messages.push(carrot_mistral::RequestMessage::System {
                                    content: carrot_mistral::MessageContent::Multipart {
                                        content: vec![carrot_mistral::MessagePart::Thinking {
                                            thinking: vec![carrot_mistral::ThinkingPart::Text {
                                                text: text.clone(),
                                            }],
    ...
    (
        carrot_mistral::Request {
            model: model.id().to_string(),
            messages,
            stream,
            stream_options: if stream {
                Some(carrot_mistral::StreamOptions {
            ...
            tool_choice: match request.tool_choice {
                Some(LanguageModelToolChoice::Auto) if !request.tools.is_empty() => {
                    Some(carrot_mistral::ToolChoice::Auto)
                }
                Some(LanguageModelToolChoice::Any) if !request.tools.is_empty() => {
                    Some(carrot_mistral::ToolChoice::Any)
                }
                Some(LanguageModelToolChoice::None) => Some(carrot_mistral::ToolChoice::None),
                _ if !request.tools.is_empty() => Some(carrot_mistral::ToolChoice::Auto),
                _ => None,
            },
            ...
            tools: request
                .tools
                .into_iter()
                .map(|tool| carrot_mistral::ToolDefinition::Function {
                    function: carrot_mistral::FunctionDefinition {
                        name: tool.name,
...
impl MistralEventMapper {
    ...
    pub fn map_event(
        &mut self,
        event: carrot_mistral::StreamResponse,
        ...
        if let Some(content) = choice.delta.content.as_ref() {
            match content {
                carrot_mistral::MessageContentDelta::Text(text) => {
                    events.push(Ok(LanguageModelCompletionEvent::Text(text.clone())));
                }
                carrot_mistral::MessageContentDelta::Parts(parts) => {
                    for part in parts {
                        match part {
                            carrot_mistral::MessagePart::Text { text } => {
                                events.push(Ok(LanguageModelCompletionEvent::Text(text.clone())));
                            }
                            carrot_mistral::MessagePart::Thinking { thinking } => {
                                for tp in thinking.iter().cloned() {
                                    match tp {
                                        carrot_mistral::ThinkingPart::Text { text } => {
                                            events.push(Ok(
                            ...
                            carrot_mistral::MessagePart::ImageUrl { .. } => {
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new("To use Carrot's agent with Mistral, you need to add an API key. Follow these steps:"))
                .child(
                    List::new()
                        .child(
                            ListBulletItem::new("")
...
mod tests {
    use super::*;
    use carrot_language_model::{LanguageModelImage, LanguageModelRequestMessage, MessageContent};
    ...
    fn test_into_mistral_basic_conversion() {
        ...
        let (mistral_request, affinity) =
            into_mistral(request, carrot_mistral::Model::MistralSmallLatest, None);
    ...
    fn test_into_mistral_with_image() {
        ...
        let (mistral_request, _) =
            into_mistral(request, carrot_mistral::Model::Pixtral12BLatest, None);

        assert_eq!(mistral_request.messages.len(), 1);
        assert!(matches!(
            &mistral_request.messages[0],
            carrot_mistral::RequestMessage::User {
                content: carrot_mistral::MessageContent::Multipart { .. }
            }
        ));

        if let carrot_mistral::RequestMessage::User {
            content: carrot_mistral::MessageContent::Multipart { content },
        } = &mistral_request.messages[0]
        {
            assert_eq!(content.len(), 2);
            assert!(matches!(
                &content[0],
                carrot_mistral::MessagePart::Text { text } if text == "What's in this image?"
            ));
            assert!(matches!(
                &content[1],
                carrot_mistral::MessagePart::ImageUrl { image_url } if image_url.starts_with("data:image/png;base64,")
            ));
        }
    }
}
