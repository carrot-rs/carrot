use anyhow::Result;
use carrot_http_client::{AsyncBody, HttpClient, Method, Request as HttpRequest, http};
...
use carrot_open_ai::ResponseStreamEvent;
...
pub use inazuma_settings_framework::OpenAiCompatibleModelCapabilities as ModelCapabilities;
pub use inazuma_settings_framework::VercelAiGatewayAvailableModel as AvailableModel;
...
fn map_open_ai_error(error: carrot_open_ai::RequestError) -> LanguageModelCompletionError {
    match error {
        ...
        carrot_open_ai::RequestError::Other(error) => LanguageModelCompletionError::Other(error),
    }
}
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new(
                    "To use Carrot's agent with Vercel AI Gateway, you need to add an API key. Follow these steps:",
                ))
                .child(
                    List::new()
                        .child(
