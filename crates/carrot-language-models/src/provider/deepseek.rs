...
use carrot_deepseek::DEEPSEEK_API_URL;
...
pub use inazuma_settings_framework::DeepseekAvailableModel as AvailableModel;
...
impl DeepSeekLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_deepseek::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for DeepSeekLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_deepseek::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_deepseek::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        models.insert("deepseek-chat", carrot_deepseek::Model::Chat);
        models.insert("deepseek-reasoner", carrot_deepseek::Model::Reasoner);

        for available_model in &Self::settings(cx).available_models {
            models.insert(
                &available_model.name,
                carrot_deepseek::Model::Custom {
...
impl DeepSeekLanguageModel {
    fn stream_completion(
        &self,
        request: carrot_deepseek::Request,
        cx: &AsyncApp,
    ) -> BoxFuture<'static, Result<BoxStream<'static, Result<carrot_deepseek::StreamResponse>>>>
    {
        ...
        let future = self.request_limiter.stream(async move {
            ...
            let request = carrot_deepseek::stream_completion(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
...
pub fn into_deepseek(
...
) -> carrot_deepseek::Request {
    let is_reasoner = model == &carrot_deepseek::Model::Reasoner;
        ...
        for content in message.content {
            match content {
                MessageContent::Text(text) => {
                    ...
                    if should_add {
                        messages.push(match message.role {
                            Role::User => carrot_deepseek::RequestMessage::User { content: text },
                            Role::Assistant => carrot_deepseek::RequestMessage::Assistant {
                                content: Some(text),
                                tool_calls: Vec::new(),
                                reasoning_content: current_reasoning.take(),
                            },
                            Role::System => {
                                carrot_deepseek::RequestMessage::System { content: text }
                            }
                        });
                    }
                }
                ...
                MessageContent::ToolUse(tool_use) => {
                    let tool_call = carrot_deepseek::ToolCall {
                        id: tool_use.id.to_string(),
                        content: carrot_deepseek::ToolCallContent::Function {
                            function: carrot_deepseek::FunctionContent {
                    ...
                    if let Some(carrot_deepseek::RequestMessage::Assistant { tool_calls, .. }) =
                        messages.last_mut()
                    {
                        tool_calls.push(tool_call);
                    } else {
                        messages.push(carrot_deepseek::RequestMessage::Assistant {
                            content: None,
                ...
                MessageContent::ToolResult(tool_result) => {
                    match &tool_result.content {
                        LanguageModelToolResultContent::Text(text) => {
                            messages.push(carrot_deepseek::RequestMessage::Tool {
    ...
    carrot_deepseek::Request {
        ...
        tools: request
            .tools
            .into_iter()
            .map(|tool| carrot_deepseek::ToolDefinition::Function {
                function: carrot_deepseek::FunctionDefinition {
                    name: tool.name,
...
impl DeepSeekEventMapper {
    ...
    pub fn map_stream(
        mut self,
        events: Pin<Box<dyn Send + Stream<Item = Result<carrot_deepseek::StreamResponse>>>>,
    ...
    pub fn map_event(
        &mut self,
        event: carrot_deepseek::StreamResponse,
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new("To use DeepSeek in Carrot, you need an API key:"))
                .child(
                    List::new()
                        .child(
                            ListBulletItem::new("")
