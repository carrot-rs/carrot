...
use carrot_aws_http_client::AwsHttpClient;
use carrot_bedrock::bedrock_client::Client as BedrockClient;
use carrot_bedrock::bedrock_client::config::timeout::TimeoutConfig;
use carrot_bedrock::bedrock_client::types::{
...
use carrot_ui::{ButtonLink, ConfiguredApiCard, Divider, List, ListBulletItem, prelude::*};
...
use inazuma_settings_framework::{
    BedrockAvailableModel as AvailableModel, Settings, SettingsStore,
};
...
actions!(carrot_bedrock, [Tab, TabPrev]);
...
impl From<inazuma_settings_framework::BedrockAuthMethodContent> for BedrockAuthMethod {
    fn from(value: inazuma_settings_framework::BedrockAuthMethodContent) -> Self {
        match value {
            inazuma_settings_framework::BedrockAuthMethodContent::SingleSignOn => {
                BedrockAuthMethod::SingleSignOn
            }
            inazuma_settings_framework::BedrockAuthMethodContent::Automatic => {
                BedrockAuthMethod::Automatic
            }
            inazuma_settings_framework::BedrockAuthMethodContent::NamedProfile => {
                BedrockAuthMethod::NamedProfile
            }
            inazuma_settings_framework::BedrockAuthMethodContent::ApiKey => {
...
pub enum ModelMode {
    ...
    AdaptiveThinking {
        effort: carrot_bedrock::BedrockAdaptiveThinkingEffort,
    },
}
...
impl BedrockLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_bedrock::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for BedrockLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_bedrock::Model::default()))
    }

    fn default_fast_model(&self, cx: &App) -> Option<Arc<dyn LanguageModel>> {
        let region = self.state.read(cx).get_region();
        Some(self.create_language_model(carrot_bedrock::Model::default_fast(region.as_str())))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        for model in carrot_bedrock::Model::iter() {
            if !matches!(model, carrot_bedrock::Model::Custom { .. }) {
            ...
            models.insert(
                ...
                carrot_bedrock::Model::Custom {
                    ...
                    cache_configuration: model.cache_configuration.as_ref().map(|config| {
                        carrot_bedrock::BedrockModelCacheConfiguration {
...
impl BedrockModel {
    ...
    fn stream_completion(
        &self,
        request: carrot_bedrock::Request,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
    ...
    > {
        ...
        let task = Tokio::spawn(
            cx,
            carrot_bedrock::stream_completion(runtime_client, request),
        );
        ...
        match message.role {
            Role::User | Role::Assistant => {
                ...
                let bedrock_role = match message.role {
                    Role::User => carrot_bedrock::BedrockRole::User,
                    Role::Assistant => carrot_bedrock::BedrockRole::Assistant,
        ...
        thinking: if request.thinking_allowed {
            match thinking_mode {
                BedrockModelMode::Thinking { budget_tokens } => {
                    Some(carrot_bedrock::Thinking::Enabled { budget_tokens })
                }
                BedrockModelMode::AdaptiveThinking {
                    ...
                    let effort = request
                        ...
                        .as_deref()
                        .and_then(|e| match e {
                            "low" => Some(carrot_bedrock::BedrockAdaptiveThinkingEffort::Low),
                            "medium" => Some(carrot_bedrock::BedrockAdaptiveThinkingEffort::Medium),
                            "high" => Some(carrot_bedrock::BedrockAdaptiveThinkingEffort::High),
                            "max" => Some(carrot_bedrock::BedrockAdaptiveThinkingEffort::Max),
                            _ => None,
                        })
                        .unwrap_or(default_effort);
                    Some(carrot_bedrock::Thinking::Adaptive { effort })
                }
    ...
    cx.background_executor()
        .spawn(async move {
            ...
            for message in messages {
                use carrot_language_model::MessageContent;
                    ...
                    let result = match event {
                        ...
                        ConverseStreamOutput::MessageStop(message_stop) => {
                            let stop_reason = if state.emitted_tool_use {
                                ...
                                // the stop reason.
                                carrot_language_model::StopReason::ToolUse
                            } else {
                                match message_stop.stop_reason {
                                    StopReason::ToolUse => {
                                        carrot_language_model::StopReason::ToolUse
                                    }
                                    _ => carrot_language_model::StopReason::EndTurn,
                                }
                            };
...
impl Render for ConfigurationView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("To use Carrot's agent with Bedrock, you can set a custom authentication strategy through your settings file or use static credentials."))
...
impl ConfigurationView {
    fn render_static_credentials_ui(&self) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "You can also set the {}, {} and {} environment variables (or {} for Bedrock API Key authentication) and restart Carrot.",
                    ZED_BEDROCK_ACCESS_KEY_ID_VAR.name,
                    ZED_BEDROCK_SECRET_ACCESS_KEY_VAR.name,
