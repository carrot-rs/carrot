...
use inazuma::{AnyView, App, AsyncApp, Context, Entity, SharedString, Task, Window};
use inazuma_collections::BTreeMap;
pub use inazuma_settings_framework::GoogleAvailableModel as AvailableModel;
...
use carrot_language_model::ApiKeyState;

const PROVIDER_ID: LanguageModelProviderId = carrot_language_model::GOOGLE_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = carrot_language_model::GOOGLE_PROVIDER_NAME;
...
impl GoogleLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_google_ai::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for GoogleLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_google_ai::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_google_ai::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        // Add base models from carrot_google_ai::Model::iter()
        for model in carrot_google_ai::Model::iter() {
            if !matches!(model, carrot_google_ai::Model::Custom { .. }) {
    ...
    fn configuration_view(
        &self,
        target_agent: carrot_language_model::ConfigurationViewTargetAgent,
        window: &mut Window,
        cx: &mut App,
    ) -> AnyView {
...
impl GoogleLanguageModel {
    fn stream_completion(
        &self,
        request: carrot_google_ai::GenerateContentRequest,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
    ...
    > {
        ...
        async move {
            ...
            let request = carrot_google_ai::stream_generate_content(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
...
impl LanguageModel for GoogleLanguageModel {
    ...
    fn count_tokens(
        ...
        async move {
            ...
            let response = carrot_google_ai::count_tokens(
                http_client.as_ref(),
                &api_url,
                &api_key,
...
pub fn into_google(
...
) -> carrot_google_ai::GenerateContentRequest {
    fn map_content(content: Vec<MessageContent>) -> Vec<Part> {
        content
            .into_iter()
            .flat_map(|content| match content {
                carrot_language_model::MessageContent::Text(text) => {
                    if !text.is_empty() {
                        vec![Part::TextPart(carrot_google_ai::TextPart { text })]
                    } else {
                        vec![]
                    }
                }
                carrot_language_model::MessageContent::Thinking {
                    ...
                    if !signature.is_empty() {
                        vec![Part::ThoughtPart(carrot_google_ai::ThoughtPart {
                            thought: true,
                ...
                carrot_language_model::MessageContent::RedactedThinking(_) => vec![],
                carrot_language_model::MessageContent::Image(image) => {
                    vec![Part::InlineDataPart(carrot_google_ai::InlineDataPart {
                        inline_data: carrot_google_ai::GenerativeContentBlob {
                            mime_type: "image/png".to_string(),
                            data: image.source.to_string(),
                        },
                    })]
                }
                carrot_language_model::MessageContent::ToolUse(tool_use) => {
                    // Normalize empty string signatures to None
                    let thought_signature = tool_use.thought_signature.filter(|s| !s.is_empty());

                    vec![Part::FunctionCallPart(carrot_google_ai::FunctionCallPart {
                        function_call: carrot_google_ai::FunctionCall {
                ...
                carrot_language_model::MessageContent::ToolResult(tool_result) => {
                    match tool_result.content {
                        carrot_language_model::LanguageModelToolResultContent::Text(text) => {
                            vec![Part::FunctionResponsePart(
                                carrot_google_ai::FunctionResponsePart {
                                    function_response: carrot_google_ai::FunctionResponse {
                        ...
                        carrot_language_model::LanguageModelToolResultContent::Image(image) => {
                            vec![
                                ...
                                Part::InlineDataPart(carrot_google_ai::InlineDataPart {
                                    inline_data: carrot_google_ai::GenerativeContentBlob {
                                        mime_type: "image/png".to_string(),
                                        data: image.source.to_string(),
                                    },
    ...
    carrot_google_ai::GenerateContentRequest {
        model: carrot_google_ai::ModelName { model_id },
        ...
        contents: request
            ...
            .filter_map(|message| {
                ...
                if parts.is_empty() {
                    ...
                    Some(carrot_google_ai::Content {
                        parts,
                        role: match message.role {
                            Role::User => carrot_google_ai::Role::User,
                            Role::Assistant => carrot_google_ai::Role::Model,
                            Role::System => carrot_google_ai::Role::User, // Google AI doesn't have a system role
                        },
                    })
                }
            })
            .collect(),
        generation_config: Some(carrot_google_ai::GenerationConfig {
        ...
        tool_config: request
            .tool_choice
            .map(|choice| carrot_google_ai::ToolConfig {
                function_calling_config: carrot_google_ai::FunctionCallingConfig {
                    mode: match choice {
                        LanguageModelToolChoice::Auto => {
                            carrot_google_ai::FunctionCallingMode::Auto
                        }
                        LanguageModelToolChoice::Any => carrot_google_ai::FunctionCallingMode::Any,
                        LanguageModelToolChoice::None => {
                            carrot_google_ai::FunctionCallingMode::None
                        }
                    },
...
fn convert_usage(usage: &UsageMetadata) -> carrot_language_model::TokenUsage {
...
impl Render for ConfigurationView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let configured_card_label = if env_var_set {
            ...
            let api_url = GoogleLanguageModelProvider::api_url(cx);
            if api_url == carrot_google_ai::API_URL {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new(format!("To use {}, you need to add an API key. Follow these steps:", match &self.target_agent {
                    ConfigurationViewTargetAgent::CarrotAgent => "Carrot's agent with Google AI".into(),
                ...
                .child(
                    Label::new(
                        format!("You can also set the {GEMINI_API_KEY_VAR_NAME} environment variable and restart Carrot."),
                    )
...
mod tests {
    ...
    use carrot_language_model::{LanguageModelToolUseId, MessageContent, Role};
    use serde_json::json;
    ...
    fn test_tool_use_with_signature_converts_to_function_call_part() {
        let tool_use = carrot_language_model::LanguageModelToolUse {
        ...
        let request = super::into_google(
            LanguageModelRequest {
                messages: vec![carrot_language_model::LanguageModelRequestMessage {
                    role: Role::Assistant,
