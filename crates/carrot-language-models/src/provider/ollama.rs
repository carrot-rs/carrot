...
use inazuma::{AnyView, App, AsyncApp, Context, CursorStyle, Entity, Task};
use inazuma_menu;
pub use inazuma_settings_framework::OllamaAvailableModel as AvailableModel;
...
pub struct State {
    ...
    http_client: Arc<dyn HttpClient>,
    fetched_models: Vec<carrot_ollama::Model>,
                    ...
                    async move {
                        ...
                        let model =
                            show_model(http_client.as_ref(), &api_url, api_key.as_deref(), name)
                                .await?;
                        let ollama_model = carrot_ollama::Model::new(
                            name,
                            None,
...
impl LanguageModelProvider for OllamaLanguageModelProvider {
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models: HashMap<String, carrot_ollama::Model> = HashMap::new();
            ...
            .child(
                List::new()
                    ...
                    .child(ListBulletItem::new(
                        "Click 'Connect' below to start using Ollama in Carrot",
                    )),
            )
            .child(Label::new(
...
fn merge_settings_into_models(
    models: &mut HashMap<String, carrot_ollama::Model>,
...
fn tool_into_ollama(tool: LanguageModelRequestTool) -> carrot_ollama::OllamaTool {
    carrot_ollama::OllamaTool::Function {
...
mod tests {
    ...
    fn test_merge_settings_preserves_display_names_for_similar_models() {
        // Regression test for https://github.com/carrot-rs/carrot/issues/43646
