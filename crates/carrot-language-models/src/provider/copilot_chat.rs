...
use carrot_copilot::{GlobalCopilotAuth, Status};
use carrot_copilot_chat::responses as copilot_responses;
use carrot_copilot_chat::{
...
use inazuma::{AnyView, App, AsyncApp, Entity, Subscription, Task};
...
impl LanguageModelProvider for CopilotChatLanguageModelProvider {
    ...
    fn configuration_view(
        ...
        cx.new(|cx| {
            carrot_copilot_ui::ConfigurationView::new(
                |cx| {
                    CopilotChat::global(cx)
                        .map(|m| m.read(cx).is_authenticated())
                        .unwrap_or(false)
                },
                carrot_copilot_ui::ConfigurationMode::Chat,
                cx,
            )
        })
        .into()
        ...
        if self.model.supports_messages() {
            ...
            let future = cx.spawn(async move |cx| {
                let effort = request
                    .thinking_effort
                    .as_ref()
                    .and_then(|e| carrot_anthropic::Effort::from_str(e).ok());
                ...
                if model.supports_adaptive_thinking() {
                    if anthropic_request.thinking.is_some() {
                        anthropic_request.thinking = Some(carrot_anthropic::Thinking::Adaptive);
                        anthropic_request.output_config =
                            Some(carrot_anthropic::OutputConfig { effort });
                    }
                }
                ...
                let body = serde_json::to_string(&carrot_anthropic::StreamingRequest {
...
mod tests {
    use super::*;
    use carrot_copilot_chat::responses;
    ...
    fn responses_stream_failed_maps_http_response_error() {
        ...
        match &mapped_results[0] {
            Err(LanguageModelCompletionError::HttpResponseError {
                ...
                assert_eq!(
                    *status_code,
                    carrot_http_client::StatusCode::TOO_MANY_REQUESTS
                );
