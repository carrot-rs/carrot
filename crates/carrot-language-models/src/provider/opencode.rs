...
use carrot_opencode::{ApiProtocol, OPENCODE_API_URL};
...
pub use inazuma_settings_framework::OpenCodeAvailableModel as AvailableModel;
...
impl OpenCodeLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_opencode::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for OpenCodeLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_opencode::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_opencode::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        for model in carrot_opencode::Model::iter() {
            if !matches!(model, carrot_opencode::Model::Custom { .. }) {
...
impl OpenCodeLanguageModel {
    ...
    fn stream_anthropic(
        ...
        Result<
            futures::stream::BoxStream<
                'static,
                Result<carrot_anthropic::Event, carrot_anthropic::AnthropicError>,
            >,
    ...
    fn stream_openai_chat(
        ...
        'static,
        Result<futures::stream::BoxStream<'static, Result<carrot_open_ai::ResponseStreamEvent>>>,
    > {
        let http_client = self.http_client.clone();
        ...
        let future = self.request_limiter.stream(async move {
            ...
            let request = carrot_open_ai::stream_completion(
                http_client.as_ref(),
                &provider_name,
                &api_url,
                &api_key,
    ...
    fn stream_openai_response(
        &self,
        request: carrot_open_ai::responses::Request,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<futures::stream::BoxStream<'static, Result<carrot_open_ai::responses::StreamEvent>>>,
    > {
        let http_client = self.http_client.clone();
        ...
        let future = self.request_limiter.stream(async move {
            ...
            let request = carrot_open_ai::responses::stream_response(
                http_client.as_ref(),
                &provider_name,
                &api_url,
                &api_key,
    ...
    fn stream_google_zen(
        ...
        'static,
        Result<
            futures::stream::BoxStream<'static, Result<carrot_google_ai::GenerateContentResponse>>,
        >,
    > {
        let http_client = self.http_client.clone();
        let api_url = self.base_api_url(cx);
        ...
        let future = self.request_limiter.stream(async move {
            ...
            let request = carrot_opencode::stream_generate_content_zen(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
        ...
        match self.model.protocol() {
            ApiProtocol::Anthropic => {
                let anthropic_request = into_anthropic(
                    ...
                    carrot_anthropic::AnthropicModelMode::Default,
                );
            ...
            ApiProtocol::Google => {
                let google_request = into_google(
                    request,
                    self.model.id().to_string(),
                    carrot_google_ai::GoogleModelMode::Default,
                );
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new(
                    "To use OpenCode Zen models in Carrot, you need an API key:",
                ))
                .child(
                    List::new()
                        .child(
