...
use inazuma::{AnyView, App, AsyncApp, Context, Entity, SharedString, Task};
use inazuma_collections::HashMap;
use inazuma_settings_framework::{
    OpenRouterAvailableModel as AvailableModel, Settings, SettingsStore,
};
...
pub struct State {
    ...
    http_client: Arc<dyn HttpClient>,
    available_models: Vec<carrot_open_router::Model>,
...
impl OpenRouterLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_open_router::Model) -> Arc<dyn LanguageModel> {
...
impl LanguageModelProvider for OpenRouterLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_open_router::Model::default()))
    }
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        for model in &Self::settings(cx).available_models {
            settings_models.push(carrot_open_router::Model {
...
pub struct OpenRouterLanguageModel {
    id: LanguageModelId,
    model: carrot_open_router::Model,
    state: Entity<State>,
    http_client: Arc<dyn HttpClient>,
...
impl OpenRouterLanguageModel {
    fn stream_completion(
        &self,
        request: carrot_open_router::Request,
        cx: &AsyncApp,
    ) -> BoxFuture<
        'static,
        Result<
            futures::stream::BoxStream<
                'static,
                Result<ResponseStreamEvent, carrot_open_router::OpenRouterError>,
            >,
    ...
    > {
        ...
        async move {
            ...
            let request = carrot_open_router::stream_completion(
                http_client.as_ref(),
                &api_url,
                &api_key,
                request,
...
pub fn into_open_router(
        ...
        for content in message.content {
            match content {
                MessageContent::Text(text) => add_message_content_part(
                    carrot_open_router::MessagePart::Text { text },
                    message.role,
                    &mut messages,
                ...
                MessageContent::Image(image) => {
                    add_message_content_part(
                        carrot_open_router::MessagePart::Image {
                ...
                MessageContent::ToolUse(tool_use) => {
                    let tool_call = carrot_open_router::ToolCall {
                        id: tool_use.id.to_string(),
                        content: carrot_open_router::ToolCallContent::Function {
                            function: carrot_open_router::FunctionContent {
                    ...
                    if let Some(carrot_open_router::RequestMessage::Assistant {
                        tool_calls, ..
                    }) = messages.last_mut()
                    {
                        tool_calls.push(tool_call);
                    } else {
                        messages.push(carrot_open_router::RequestMessage::Assistant {
                            content: None,
                ...
                MessageContent::ToolResult(tool_result) => {
                    let content = match &tool_result.content {
                        LanguageModelToolResultContent::Text(text) => {
                            vec![carrot_open_router::MessagePart::Text {
                                text: text.to_string(),
                            }]
                        }
                        LanguageModelToolResultContent::Image(image) => {
                            vec![carrot_open_router::MessagePart::Image {
                                image_url: image.to_base64_url(),
                            }]
                        }
                    };

                    messages.push(carrot_open_router::RequestMessage::Tool {
    ...
    carrot_open_router::Request {
        ...
        tools: request
            .tools
            .into_iter()
            .map(|tool| carrot_open_router::ToolDefinition::Function {
                function: carrot_open_router::FunctionDefinition {
                    name: tool.name,
        ...
        tool_choice: request.tool_choice.map(|choice| match choice {
            LanguageModelToolChoice::Auto => carrot_open_router::ToolChoice::Auto,
            LanguageModelToolChoice::Any => carrot_open_router::ToolChoice::Required,
            LanguageModelToolChoice::None => carrot_open_router::ToolChoice::None,
        }),
        provider: model.provider.clone(),
    }
}
...
fn add_message_content_part(
    new_part: carrot_open_router::MessagePart,
    role: Role,
    messages: &mut Vec<carrot_open_router::RequestMessage>,
    reasoning_details: Option<serde_json::Value>,
) {
    match (role, messages.last_mut()) {
        (Role::User, Some(carrot_open_router::RequestMessage::User { content }))
        | (Role::System, Some(carrot_open_router::RequestMessage::System { content })) => {
            content.push_part(new_part);
        }
        (
            Role::Assistant,
            Some(carrot_open_router::RequestMessage::Assistant {
        ...
        _ => {
            messages.push(match role {
                Role::User => carrot_open_router::RequestMessage::User {
                    content: carrot_open_router::MessageContent::from(vec![new_part]),
                },
                Role::Assistant => carrot_open_router::RequestMessage::Assistant {
                    content: Some(carrot_open_router::MessageContent::from(vec![new_part])),
                    tool_calls: Vec::new(),
                    reasoning_details,
                },
                Role::System => carrot_open_router::RequestMessage::System {
                    content: carrot_open_router::MessageContent::from(vec![new_part]),
                },
            });
        }
...
impl OpenRouterEventMapper {
    ...
    pub fn map_stream(
        ...
        events: Pin<
            Box<
                dyn Send
                    + Stream<Item = Result<ResponseStreamEvent, carrot_open_router::OpenRouterError>>,
            >,
        >,
...
pub fn count_open_router_tokens(
    request: LanguageModelRequest,
    _model: carrot_open_router::Model,
    cx: &App,
) -> BoxFuture<'static, Result<u64>> {
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new("To use Carrot's agent with OpenRouter, you need to add an API key. Follow these steps:"))
                .child(
                    List::new()
                        .child(
                            ListBulletItem::new("")
...
mod tests {
    ...
    use carrot_open_router::{ChoiceDelta, FunctionChunk, ResponseMessageDelta, ToolCallChunk};
    ...
    async fn test_usage_only_chunk_with_empty_choices_does_not_error() {
        ...
        let events = mapper.map_event(ResponseStreamEvent {
            ...
            choices: Vec::new(),
            usage: Some(carrot_open_router::Usage {
                prompt_tokens: 12,
                completion_tokens: 7,
                total_tokens: 19,
            }),
