...
use inazuma_menu;
use inazuma_settings_framework::{OpenAiAvailableModel as AvailableModel, Settings, SettingsStore};
...
const PROVIDER_ID: LanguageModelProviderId = carrot_language_model::OPEN_AI_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = carrot_language_model::OPEN_AI_PROVIDER_NAME;
...
impl OpenAiLanguageModelProvider {
    ...
    fn create_language_model(&self, model: carrot_open_ai::Model) -> Arc<dyn LanguageModel> {
    ...
    fn api_url(cx: &App) -> SharedString {
        ...
        if api_url.is_empty() {
            carrot_open_ai::OPEN_AI_API_URL.into()
        } else {
...
impl LanguageModelProvider for OpenAiLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_open_ai::Model::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(carrot_open_ai::Model::default_fast()))
    }

    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        let mut models = BTreeMap::default();

        // Add base models from carrot_open_ai::Model::iter()
        for model in carrot_open_ai::Model::iter() {
            if !matches!(model, carrot_open_ai::Model::Custom { .. }) {
...
impl OpenAiLanguageModel {
    fn stream_completion(
        &self,
        request: carrot_open_ai::Request,
        cx: &AsyncApp,
...
pub fn into_open_ai(
            ...
            match content {
                MessageContent::Text(text) | MessageContent::Thinking { text, .. } => {
                    let should_add = if message.role == Role::User {
                        ...
                        // See https://github.com/carrot-rs/carrot/issues/40097
                        !text.trim().is_empty()
                    } else {
                        !text.is_empty()
                    };
                    if should_add {
                        add_message_content_part(
                            carrot_open_ai::MessagePart::Text { text },
                            message.role,
                            &mut messages,
                        );
                    }
                ...
                MessageContent::ToolUse(tool_use) => {
                    let tool_call = carrot_open_ai::ToolCall {
                        id: tool_use.id.to_string(),
                        content: carrot_open_ai::ToolCallContent::Function {
                            function: carrot_open_ai::FunctionContent {
                    ...
                    if let Some(carrot_open_ai::RequestMessage::Assistant { tool_calls, .. }) =
                        messages.last_mut()
                    {
                        tool_calls.push(tool_call);
                    } else {
                        messages.push(carrot_open_ai::RequestMessage::Assistant {
                            content: None,
                ...
                MessageContent::ToolResult(tool_result) => {
                    let content = match &tool_result.content {
                        LanguageModelToolResultContent::Text(text) => {
                            vec![carrot_open_ai::MessagePart::Text {
                                text: text.to_string(),
                            }]
                        }
                        LanguageModelToolResultContent::Image(image) => {
                            vec![carrot_open_ai::MessagePart::Image {
                                image_url: ImageUrl {
                                    url: image.to_base64_url(),
                                    detail: None,
                                },
                    ...
                    messages.push(carrot_open_ai::RequestMessage::Tool {
    ...
    carrot_open_ai::Request {
        ...
        stream_options: if stream {
            Some(carrot_open_ai::StreamOptions::default())
        } else {
            None
        },
        stop: request.stop,
        ...
        tools: request
            .tools
            .into_iter()
            .map(|tool| carrot_open_ai::ToolDefinition::Function {
                function: carrot_open_ai::FunctionDefinition {
                    name: tool.name,
        ...
        tool_choice: request.tool_choice.map(|choice| match choice {
            LanguageModelToolChoice::Auto => carrot_open_ai::ToolChoice::Auto,
            LanguageModelToolChoice::Any => carrot_open_ai::ToolChoice::Required,
            LanguageModelToolChoice::None => carrot_open_ai::ToolChoice::None,
        }),
        reasoning_effort,
    }
}
...
pub fn into_open_ai_response(
    ...
    let tools: Vec<_> = tools
        .into_iter()
        .map(|tool| carrot_open_ai::responses::ToolDefinition::Function {
            name: tool.name,
            description: Some(tool.description),
            parameters: Some(tool.input_schema),
            strict: None,
    ...
    ResponseRequest {
        ...
        reasoning: reasoning_effort.map(|effort| carrot_open_ai::responses::ReasoningConfig {
            effort,
...
fn flush_response_parts(
    ...
    let item = ResponseInputItem::Message(ResponseMessageItem {
        role: match role {
            Role::User => carrot_open_ai::Role::User,
            Role::Assistant => carrot_open_ai::Role::Assistant,
            Role::System => carrot_open_ai::Role::System,
        },
        content: parts.clone(),
    });
...
fn add_message_content_part(
    new_part: carrot_open_ai::MessagePart,
    role: Role,
    messages: &mut Vec<carrot_open_ai::RequestMessage>,
) {
    match (role, messages.last_mut()) {
        (Role::User, Some(carrot_open_ai::RequestMessage::User { content }))
        | (
            Role::Assistant,
            Some(carrot_open_ai::RequestMessage::Assistant {
                content: Some(content),
                ..
            }),
        )
        | (Role::System, Some(carrot_open_ai::RequestMessage::System { content, .. })) => {
            content.push_part(new_part);
        }
        _ => {
            messages.push(match role {
                Role::User => carrot_open_ai::RequestMessage::User {
                    content: carrot_open_ai::MessageContent::from(vec![new_part]),
                },
                Role::Assistant => carrot_open_ai::RequestMessage::Assistant {
                    content: Some(carrot_open_ai::MessageContent::from(vec![new_part])),
                    tool_calls: Vec::new(),
                },
                Role::System => carrot_open_ai::RequestMessage::System {
                    content: carrot_open_ai::MessageContent::from(vec![new_part]),
                },
            });
        }
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("To use Carrot's agent with OpenAI, you need to add an API key. Follow these steps:"))
                .child(
                    List::new()
                        .child(
                            ListBulletItem::new("")
                ...
                .child(
                    Label::new(format!(
                        "You can also set the {API_KEY_ENV_VAR_NAME} environment variable and restart Carrot."
                    ))
        ...
        let compatible_api_section = h_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(Label::new("Carrot also supports OpenAI-compatible models.")),
            )
            .child(
                Button::new("docs", "Learn More")
                    ...
                    .on_click(move |_, _window, cx| {
                        cx.open_url(
                            "https://carrot.dev/docs/ai/llm-providers#openai-api-compatible",
                        )
                    }),
            );
