...
use carrot_editor::display_map::{BlockPlacement, BlockProperties, BlockStyle};
use carrot_editor::{
    Addon, Editor, EditorEvent, ExcerptRange, MultiBuffer, multibuffer_context_lines,
};
use carrot_feature_flags::{FeatureFlagAppExt as _, GitGraphFeatureFlag};
use carrot_git::repository::{CommitDetails, CommitDiff, RepoPath, is_binary_content};
use carrot_git::status::{FileStatus, StatusCode, TrackedStatus};
use carrot_git::{
...
use carrot_project::{Project, WorktreeId, git_store::Repository};
use carrot_theme::ActiveTheme;
use carrot_ui::{DiffStat, Divider, Tooltip, prelude::*};
use carrot_workspace::item::TabTooltipContent;
use carrot_workspace::{
    Item, ItemHandle, ItemNavHistory, PaneRole, ToolbarItemEvent, ToolbarItemLocation,
...
use inazuma::{
    AnyElement, App, AsyncApp, AsyncWindowContext, ClipboardItem, Context, Entity, EventEmitter,
...
use inazuma_util::{ResultExt, paths::PathStyle, rel_path::RelPath, truncate_and_trailoff};
use std::{
    any::{Any, TypeId},
    collections::HashSet,
    path::PathBuf,
...
struct CommitDiffAddon {
    file_statuses: HashMap<carrot_language::BufferId, FileStatus>,
}
...
impl CommitView {
    ...
    fn new(
        ...
        let editor = cx.new(|cx| {
            ...
            editor.insert_blocks(
                [BlockProperties {
                    placement: BlockPlacement::Above(carrot_editor::Anchor::min()),
                    height: Some(1),
        ...
        cx.spawn(async move |this, cx| {
            let mut binary_buffer_ids: HashSet<carrot_language::BufferId> = HashSet::default();
            let mut file_statuses: HashMap<carrot_language::BufferId, FileStatus> =
                HashMap::default();
            ...
            for file in commit_diff.files {
                ...
                let file = Arc::new(GitBlob {
                    ...
                    worktree_id,
                    display_name,
                }) as Arc<dyn carrot_language::File>;
                ...
                this.update(cx, |this, cx| {
                    this.multibuffer.update(cx, |multibuffer, cx| {
                        ...
                        let excerpt_ranges = if is_binary {
                            vec![carrot_language::Point::zero()..snapshot.max_point()]
    ...
    fn render_commit_avatar(
        &self,
        sha: &SharedString,
        size: impl Into<inazuma::AbsoluteLength>,
        window: &mut Window,
        cx: &mut App,
    ) -> AnyElement {
        CommitAvatar::new(
    ...
    fn render_header(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let date_string = carrot_time_format::format_localized_timestamp(
            commit_date,
            time::OffsetDateTime::now_utc(),
            local_offset,
            carrot_time_format::TimestampFormat::MediumAbsolute,
        );
...
impl carrot_language::File for GitBlob {
    ...
    fn to_proto(&self, _cx: &App) -> carrot_language::proto::File {
        unimplemented!()
    }
...
async fn build_buffer(
    mut text: String,
    blob: Arc<dyn File>,
    language_registry: &Arc<carrot_language::LanguageRegistry>,
    cx: &mut AsyncApp,
            ...
            .child(
                IconButton::new("buffer-search", IconName::MagnifyingGlass)
                    ...
                    .tooltip(move |_, cx| {
                        Tooltip::for_action(
                            "Buffer Search",
                            &carrot_actions::buffer_search::Deploy::find(),
                            cx,
                        )
                    })
                    .on_click(|_, window, cx| {
                        window.dispatch_action(
                            Box::new(carrot_actions::buffer_search::Deploy::find()),
                            cx,
                        );
                    }),
            )
