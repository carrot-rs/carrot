...
use carrot_askpass::AskPassDelegate;
use carrot_db::kvp::KeyValueStore;
use carrot_editor::{
    Direction, Editor, EditorElement, EditorMode, MultiBuffer, MultiBufferOffset,
    actions::ExpandAllDiffHunks,
};
use carrot_editor::{EditorStyle, RewrapOptions};
use carrot_feature_flags::{FeatureFlagAppExt as _, GitGraphFeatureFlag};
use carrot_file_icons::FileIcons;
use carrot_git::commit::ParsedCommitMessage;
use carrot_git::repository::{
...
use carrot_git::stash::GitStash;
use carrot_git::status::{DiffStat, StageStatus};
use carrot_git::{Amend, Signoff, ToggleStaged, repository::RepoPath, status::FileStatus};
use carrot_git::{
...
use carrot_language::{Buffer, File};
use carrot_language_model::{
    CompletionIntent, ConfiguredModel, LanguageModelRegistry, LanguageModelRequest,
    LanguageModelRequestMessage, Role,
};
use carrot_multi_buffer::ExcerptInfo;
use carrot_notifications::status_toast::{StatusToast, ToastIcon};
use carrot_panel::{PanelHeader, panel_button, panel_filled_button, panel_icon_button};
use carrot_project::{
    Fs, Project, ProjectPath,
    git_store::{GitStoreEvent, Repository, RepositoryEvent, RepositoryId, pending_op},
    project_settings::{GitPathStyle, ProjectSettings},
};
use carrot_prompt_store::{BuiltInPrompt, PromptId, PromptStore, RULES_FILE_NAMES};
use carrot_proto::RpcError;
...
use carrot_workspace::SERIALIZATION_THROTTLE_TIME;
...
use inazuma_collections::{BTreeMap, HashMap, HashSet};
use inazuma_menu;
use inazuma_settings_framework::{Settings, SettingsStore, StatusStyle};
use inazuma_util::paths::PathStyle;
use inazuma_util::{ResultExt, TryFutureExt, maybe, rel_path::RelPath};
...
#[derive(Clone, PartialEq, serde::Deserialize, schemars::JsonSchema, inazuma::Action)]
...
fn git_panel_context_menu(
    ...
    ContextMenu::build(window, cx, move |context_menu, _, _| {
        context_menu
            ...
            .action("View Stash", carrot_actions::git::ViewStash.boxed_clone())
            .separator()
...
pub fn register(workspace: &mut Workspace) {
    ...
    workspace.register_action(|workspace, _: &carrot_git::Init, window, cx| {
...
impl GitPanel {
    ...
    fn stage_status_for_directory(
        ...
        let GitPanelViewMode::Tree(tree_state) = &self.view_mode else {
            inazuma_util::debug_panic!(
                "We should never render a directory entry while in flat view mode"
            );
    ...
    fn on_commit(&mut self, _: &carrot_git::Commit, window: &mut Window, cx: &mut Context<Self>) {
        if self.commit(&self.commit_editor.focus_handle(cx), window, cx) {
            carrot_telemetry::event!("Git Committed", source = "Git Panel");
        }
    }
    ...
    fn on_amend(&mut self, _: &carrot_git::Amend, window: &mut Window, cx: &mut Context<Self>) {
        if self.amend(&self.commit_editor.focus_handle(cx), window, cx) {
            carrot_telemetry::event!("Git Amended", source = "Git Panel");
        }
    }
    ...
    pub(crate) fn uncommit(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        carrot_telemetry::event!("Git Uncommitted");
    ...
    fn generate_commit_message_action(
        &mut self,
        _: &carrot_git::GenerateCommitMessage,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub fn generate_commit_message(&mut self, cx: &mut Context<Self>) {
        ...
        carrot_telemetry::event!("Git Commit Message Generated");
    ...
    pub(crate) fn fetch(
        ...
        carrot_telemetry::event!("Git Fetched");
    ...
    pub(crate) fn git_clone(&mut self, repo: String, window: &mut Window, cx: &mut Context<Self>) {
        ...
        crate::clone::clone_and_open(
            ...
            window,
            cx,
            Arc::new(|_workspace: &mut carrot_workspace::Workspace, _window, _cx| {}),
        );
    }
        ...
        let worktree = if worktrees.len() == 1 {
            ...
            let worktree_directories = worktrees
                ...
                .map(|worktree_abs_path| {
                    if let Ok(path) =
                        worktree_abs_path.strip_prefix(inazuma_util::paths::home_dir())
                    {
                        Path::new("~")
                            .join(path)
                            .to_string_lossy()
    ...
    pub(crate) fn pull(&mut self, rebase: bool, window: &mut Window, cx: &mut Context<Self>) {
        ...
        carrot_telemetry::event!("Git Pulled");
    ...
    pub(crate) fn push(
        ...
        carrot_telemetry::event!("Git Pushed");
        let branch = branch.clone();
    ...
    pub fn create_pull_request(&self, window: &mut Window, cx: &mut Context<Self>) {
        let result = (|| -> anyhow::Result<()> {
            ...
            let Some((provider, parsed_remote)) =
                carrot_git::parse_git_remote_url(provider_registry, &remote_url)
            else {
    ...
    fn potential_co_authors(&self, cx: &App) -> Vec<(String, String)> {
        ...
        let Some(room) =
            carrot_call::ActiveCall::try_global(cx).and_then(|call| call.read(cx).room().cloned())
        else {
            return Vec::default();
        };
    ...
    fn local_committer(&self, room: &carrot_call::Room, cx: &App) -> Option<(String, String)> {
    ...
    pub(crate) fn render_generate_commit_message_button(
        ...
        if !carrot_agent_settings::AgentSettings::get_global(cx).enabled(cx) {
            return None;
        }
        ...
        Some(
            IconButton::new("generate-commit-message", IconName::AiEdit)
                .shape(carrot_ui::IconButtonShape::Square)
                ...
                .tooltip(move |_window, cx| {
                    if !can_commit {
                        ...
                        Tooltip::for_action_in(
                            ...
                            &carrot_git::GenerateCommitMessage,
                            &editor_focus_handle,
                            cx,
                        )
                    }
    ...
    fn render_git_commit_menu(
        ...
        PopoverMenu::new(id.into())
            .trigger(
                carrot_ui::ButtonLike::new_rounded_right("commit-split-button-right")
                    .layer(carrot_ui::ElevationIndex::ModalSurface)
    ...
    fn expand_commit_editor(
        &mut self,
        _: &carrot_git::ExpandCommitEditor,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let workspace = self.workspace.clone();
            ...
            .child(
                panel_editor_container(window, cx)
                    ...
                    .child(
                        div()
                            .pr_2p5()
                            .on_action(|&carrot_actions::editor::MoveUp, _, cx| {
                                cx.stop_propagation();
                            })
                            .on_action(|&carrot_actions::editor::MoveDown, _, cx| {
                                cx.stop_propagation();
                            })
                        ...
                        h_flex()
                            ...
                            .child(
                                panel_icon_button("expand-commit-editor", IconName::Maximize)
                                    .icon_size(IconSize::Small)
                                    .size(carrot_ui::ButtonSize::Default)
                                    .tooltip(move |_window, cx| {
                                        Tooltip::for_action_in(
                                            "Open Commit Modal",
                                            &carrot_git::ExpandCommitEditor,
                                    ...
                                    .on_click(cx.listener({
                                        move |_, _, window, cx| {
                                            window.dispatch_action(
                                                carrot_git::ExpandCommitEditor.boxed_clone(),
                                                cx,
                                            )
                                        }
                                    })),
                ...
                this.child(
                    panel_filled_button("Initialize Repository")
                        ...
                        .on_click(move |_, _, cx| {
                            cx.defer(move |cx| {
                                cx.dispatch_action(&carrot_git::Init);
                            })
                        }),
                )
            })
    ...
    fn render_buffer_header_controls(
        ...
        let checkbox = Checkbox::new("stage-file")
            .toggle_state(is_staging_or_staged.into())
                    ...
                    .child(
                        ...
                        .when(is_tree_view, |list| {
                            ...
                            list.with_decoration(
                                carrot_ui::indent_guides(indent_size, IndentGuideColors::panel(cx))
    ...
    fn deploy_entry_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, |context_menu, _, _| {
            ...
            context_menu
                ...
                .action(
                    restore_title,
                    carrot_git::RestoreFile::default().boxed_clone(),
                )
                .action_disabled_when(
                    !is_created,
                    "Add to .gitignore",
                    carrot_git::AddToGitignore.boxed_clone(),
                )
                .separator()
                ...
                .action_disabled_when(
                    is_created,
                    "View File History",
                    Box::new(carrot_git::FileHistory),
                )
        });
        self.selected_entry = Some(ix);
        ...
        h_flex()
            ...
            .when(GitPanelSettings::get_global(cx).diff_stats, |el| {
                el.when_some(entry.diff_stat, move |this, stat| {
                    ...
                    this.child(carrot_ui::DiffStat::new(
                        id,
                        stat.added as usize,
...
impl carrot_editor::Addon for GitPanelAddon {
...
impl Panel for GitPanel {
    ...
    fn icon(&self, _: &Window, cx: &App) -> Option<carrot_ui::IconName> {
        Some(carrot_ui::IconName::GitBranchAlt).filter(|_| GitPanelSettings::get_global(cx).button)
    }
...
impl RenderOnce for PanelRepoFooter {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let repo_selector = PopoverMenu::new("repository-switcher")
            ...
            .trigger_with_tooltip(
                repo_selector_trigger
                    .when(single_repo, |this: Button| {
                        this.disabled(true).color(Color::Muted)
                    })
                    .truncate(true),
                move |_, cx| {
                    if single_repo {
        ...
        .on_click(|_, window: &mut Window, cx| {
            window.dispatch_action(carrot_actions::git::Switch.boxed_clone(), cx);
        });
        ...
        let branch_selector = PopoverMenu::new("popover-button")
            ...
            .trigger_with_tooltip(
                ...
                Tooltip::for_action_title("Switch Branch", &carrot_actions::git::Switch),
            )
            .anchor(Corner::BottomLeft)
            .offset(inazuma::Point {
                x: px(0.0),
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> Option<AnyElement> {
        ...
        fn custom(branch_name: &str, upstream: Option<UpstreamTracking>) -> Branch {
            Branch {
                ...
                upstream: upstream.map(|tracking| Upstream {
                    ref_name: format!("carrot/{}", branch_name).into(),
                    tracking,
                }),
                most_recent_commit: Some(CommitSummary {
                    sha: "abc123".into(),
                        ...
                        vec![
                            ...
                            single_example(
                                ...
                                div()
                                    ...
                                    .overflow_hidden()
                                    .child(PanelRepoFooter::new_preview(
                                        SharedString::from("carrot-rs-community-examples"),
...
fn open_output(
    ...
    buffer.update(cx, |buffer, cx| {
        buffer.set_capability(carrot_language::Capability::ReadOnly, cx);
    });
...
pub(crate) fn show_error_toast(
    ...
    if message
        .matches(carrot_git::repository::REMOTE_CANCELLED_BY_USER)
        .next()
        .is_some()
...
mod tests {
    ...
    use inazuma::{TestAppContext, UpdateGlobal, VisualTestContext, px};
    ...
    fn init_test(cx: &mut inazuma::TestAppContext) {
            ...
            &carrot_proto::Error {
                message:
                    "Your local changes to the following files would be overwritten by merge\n"
                        .to_string(),
                code: carrot_proto::ErrorCode::Internal as i32,
    ...
    async fn test_entry_worktree_paths(cx: &mut TestAppContext) {
        ...
        let project =
            Project::test(fs.clone(), [path!("/root/carrot/crates/gpui").as_ref()], cx).await;
        let window_handle =
    ...
    async fn test_bulk_staging(cx: &mut TestAppContext) {
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            panel.stage_range(&carrot_git::StageRange, window, cx);
        });
    ...
    async fn test_dispatch_context_with_focus_states(cx: &mut TestAppContext) {
        ...
        cx.simulate_resize(inazuma::size(px(800.), px(600.)));
