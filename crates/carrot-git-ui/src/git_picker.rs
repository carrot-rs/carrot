...
use carrot_workspace::{ModalView, Workspace, pane};
use inazuma::{
...
pub fn open_stash(
    workspace: &mut Workspace,
    _: &carrot_actions::git::ViewStash,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
...
pub fn register(workspace: &mut Workspace) {
    workspace.register_action(|workspace, _: &carrot_actions::git::Branch, window, cx| {
        open_with_tab(workspace, GitPickerTab::Branches, window, cx);
    });
    workspace.register_action(|workspace, _: &carrot_actions::git::Switch, window, cx| {
        open_with_tab(workspace, GitPickerTab::Branches, window, cx);
    });
    workspace.register_action(
        |workspace, _: &carrot_actions::git::CheckoutBranch, window, cx| {
            open_with_tab(workspace, GitPickerTab::Branches, window, cx);
        },
    );
    workspace.register_action(|workspace, _: &carrot_actions::git::Worktree, window, cx| {
        open_with_tab(workspace, GitPickerTab::Worktrees, window, cx);
    });
    workspace.register_action(
        |workspace, _: &carrot_actions::git::ViewStash, window, cx| {
            open_with_tab(workspace, GitPickerTab::Stash, window, cx);
        },
    );
}
