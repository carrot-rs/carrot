...
use carrot_language::{self, Buffer, Point};
use carrot_project::Project;
use carrot_ui::{Color, Icon, IconName, Label, LabelCommon as _, SharedString};
...
use inazuma_util::paths::PathExt;
use std::{
    any::{Any, TypeId},
    cmp,
    ops::Range,
...
use carrot_workspace::{
    Item, ItemNavHistory, PaneRole, Workspace,
...
pub struct TextDiffView {
    ...
    buffer_changes_tx: carrot_watch::Sender<()>,
...
impl TextDiffView {
    pub fn open(
        ...
        let Some((source_buffer, expanded_selection_range)) = selection_data else {
            log::warn!("There should always be at least one selection in Carrot. This is a bug.");
            return None;
        };
    ...
    pub fn new(
        ...
        let (buffer_changes_tx, mut buffer_changes_rx) = carrot_watch::channel(());
        ...
        cx.subscribe(&source_buffer, move |this, _, event, _| match event {
            ...
            | carrot_language::BufferEvent::Reparsed => {
...
fn build_clipboard_buffer(
    ...
    let source_buffer_snapshot = source_buffer.read(cx).snapshot();
    cx.new(|cx| {
        let mut buffer = carrot_language::Buffer::local(source_buffer_snapshot.text(), cx);
...
mod tests {
    ...
    use inazuma::{BorrowAppContext, TestAppContext, VisualContext};
    ...
    async fn test_diffing_clipboard_from_multibuffer_with_selection(cx: &mut TestAppContext) {
        ...
        let editor = cx.new_window_entity(|window, cx| {
            let multibuffer = cx.new(|cx| {
                let mut mb = MultiBuffer::new(carrot_language::Capability::ReadWrite);
