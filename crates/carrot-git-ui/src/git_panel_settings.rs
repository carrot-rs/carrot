...
use carrot_ui::{ScrollbarVisibility, ShowScrollbar, px};
...
/// Convert from the editor's `ShowScrollbar` (which is `pub(crate)` in `carrot_editor`)
/// to `carrot_ui::ShowScrollbar` by matching on the `Debug` representation.
fn editor_show_scrollbar_to_ui(cx: &carrot_ui::App) -> ShowScrollbar {
...
impl ScrollbarVisibility for GitPanelSettings {
    fn visibility(&self, cx: &carrot_ui::App) -> ShowScrollbar {
        ...
        //
        // We should fix this. PR: https://github.com/carrot-rs/carrot/pull/19495
        self.scrollbar
            .show
