...
use carrot_git::repository::Branch;
use carrot_project::git_store::Repository;
use carrot_project::project_settings::ProjectSettings;
use carrot_ui::{
    Divider, HighlightedLabel, KeyBinding, ListItem, ListItemSpacing, Tooltip, prelude::*,
};
use carrot_ui_input::ErasedEditor;
use carrot_workspace::notifications::DetachAndPromptErr;
...
pub fn checkout_branch(
    ...
    _: &carrot_actions::git::CheckoutBranch,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open(workspace, &carrot_actions::git::Branch, window, cx);
}

pub fn switch(
    workspace: &mut Workspace,
    _: &carrot_actions::git::Switch,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open(workspace, &carrot_actions::git::Branch, window, cx);
        ...
        let (commit_time, author_name, subject) = entry
            ...
            .and_then(|branch| {
                branch.most_recent_commit.as_ref().map(|commit| {
                    ...
                    let formatted_time = carrot_time_format::format_localized_timestamp(
                        commit_time,
                        OffsetDateTime::now_utc(),
                        local_offset,
                        carrot_time_format::TimestampFormat::Relative,
                    );
...
mod tests {
    ...
    use super::*;
    use carrot_git::repository::{CommitSummary, Remote};
    use carrot_project::{FakeFs, Project};
    use inazuma::{AppContext, TestAppContext, VisualTestContext};
