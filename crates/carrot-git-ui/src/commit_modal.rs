...
use carrot_git::repository::CommitOptions;
use carrot_git::{Amend, Commit, GenerateCommitMessage, Signoff};
use carrot_panel::panel_button;
...
use carrot_editor::{Editor, EditorElement};
...
impl CommitModal {
    ...
    pub fn render_footer(&self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let branch_picker_button = panel_button(branch)
            ...
            .on_click(cx.listener(|_, _, window, cx| {
                window.dispatch_action(carrot_actions::git::Branch.boxed_clone(), cx);
            }))
        ...
        let branch_picker = PopoverMenu::new("popover-button")
            ...
            .trigger_with_tooltip(
                ...
                Tooltip::for_action_title("Switch Branch", &carrot_actions::git::Branch),
            )
        ...
        let close_kb_hint =
            carrot_ui::KeyBinding::for_action(&inazuma_menu::Cancel, cx).map(|close_kb| {
                KeybindingHint::new(close_kb, cx.theme().colors().editor.background)
                    .suffix("Cancel")
            });
            ...
            .child(
                h_flex()
                    ...
                    .child(SplitButton::new(
                        carrot_ui::ButtonLike::new_rounded_left(ElementId::Name(
                        ...
                        .on_click(cx.listener(move |this, _: &ClickEvent, window, cx| {
                            carrot_telemetry::event!("Git Committed", source = "Git Modal");
    ...
    fn on_amend(&mut self, _: &carrot_git::Amend, window: &mut Window, cx: &mut Context<Self>) {
        if self.git_panel.update(cx, |git_panel, cx| {
            ...
            carrot_telemetry::event!("Git Amended", source = "Git Modal");
            cx.emit(DismissEvent);
        }
    }
...
impl Render for CommitModal {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_action(
                cx.listener(|this, _: &carrot_actions::git::Branch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                }),
            )
            .on_action(cx.listener(
                |this, _: &carrot_actions::git::CheckoutBranch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                },
            ))
            .on_action(
                cx.listener(|this, _: &carrot_actions::git::Switch, window, cx| {
                    this.toggle_branch_selector(window, cx);
                }),
            )
            .elevation_3(cx)
