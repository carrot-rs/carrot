use anyhow::anyhow;
use carrot_editor::{Editor, actions::DiffClipboardWithSelectionData};
...
use carrot_project::git_store::Repository;
use carrot_ui::prelude::*;
use carrot_workspace::{ModalView, Workspace, notifications::DetachAndPromptErr};
use inazuma::{
    App, Context, DismissEvent, Entity, EventEmitter, FocusHandle, Focusable, SharedString, Window,
};
use inazuma_menu::{Cancel, Confirm};
...
pub fn init(cx: &mut App) {
    carrot_editor::set_blame_renderer(blame_ui::GitBlameRenderer, cx);
    commit_view::init(cx);
    ...
    cx.observe_new(|workspace: &mut Workspace, _, cx| {
        ...
        if !project.is_via_collab() {
            workspace.register_action(
                |workspace, _: &carrot_actions::git::CreatePullRequest, window, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::Fetch, window, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::FetchFrom, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::Push, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::PushTo, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::ForcePush, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::Pull, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
            ...
            workspace.register_action(|workspace, _: &carrot_git::PullRebase, window, cx| {
                let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                    return;
                };
                panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, action: &carrot_git::StashAll, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, action: &carrot_git::StashPop, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, action: &carrot_git::StashApply, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, action: &carrot_git::StageAll, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, action: &carrot_git::UnstageAll, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, _: &carrot_git::Uncommit, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, _action: &carrot_git::Init, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
            panel.update(cx, |panel, cx| {
        ...
        workspace.register_action(|workspace, _action: &carrot_git::Clone, window, cx| {
            let Some(panel) = workspace.panel::<git_panel::GitPanel>(cx) else {
                return;
            };
        ...
        workspace.register_action(|workspace, _: &carrot_git::OpenModifiedFiles, window, cx| {
            open_modified_files(workspace, window, cx);
        });
        workspace.register_action(|workspace, _: &carrot_git::RenameBranch, window, cx| {
...
mod remote_button {
    ...
    use inazuma::{Action, AnyView, ClickEvent, Corner, FocusHandle};
    ...
    pub fn render_fetch_button(
        ...
        split_button(
            ...
            move |_, window, cx| {
                window.dispatch_action(Box::new(carrot_git::Fetch), cx);
            },
            move |_window, cx| {
                git_action_tooltip(
    ...
    pub fn render_push_button(
        ...
        split_button(
            ...
            keybinding_target.clone(),
            move |_, window, cx| {
                window.dispatch_action(Box::new(carrot_git::Push), cx);
            },
            move |_window, cx| {
                git_action_tooltip(
    ...
    pub fn render_pull_button(
        ...
        split_button(
            ...
            keybinding_target.clone(),
            move |_, window, cx| {
                window.dispatch_action(Box::new(carrot_git::Pull), cx);
            },
            move |_window, cx| {
                git_action_tooltip(
                    "Pull",
    ...
    fn render_git_action_menu(
        ...
        PopoverMenu::new(id.into())
            .trigger(
                carrot_ui::ButtonLike::new_rounded_right("split-button-right")
            ...
            .menu(move |window, cx| {
                Some(ContextMenu::build(window, cx, |context_menu, _, _| {
                    context_menu
                        ...
                        .action("Fetch", carrot_git::Fetch.boxed_clone())
                        .action("Fetch From", carrot_git::FetchFrom.boxed_clone())
                        .action("Pull", carrot_git::Pull.boxed_clone())
                        .action("Pull (Rebase)", carrot_git::PullRebase.boxed_clone())
                        .separator()
                        .action("Push", carrot_git::Push.boxed_clone())
                        .action("Push To", carrot_git::PushTo.boxed_clone())
                        .action("Force Push", carrot_git::ForcePush.boxed_clone())
                }))
            })
    ...
    fn split_button(
        ...
        let left = carrot_ui::ButtonLike::new_rounded_left(ElementId::Name(
            format!("split-button-left-{}", id).into(),
        ))
        .layer(carrot_ui::ElevationIndex::ModalSurface)
        .size(carrot_ui::ButtonSize::Compact)
...
impl RenderOnce for GitStatusIcon {
    fn render(self, _window: &mut carrot_ui::Window, cx: &mut App) -> impl IntoElement {
...
impl Component for GitStatusIcon {
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> Option<AnyElement> {
        fn tracked_file_status(code: StatusCode) -> FileStatus {
            FileStatus::Tracked(carrot_git::status::TrackedStatus {
                index_status: code,
