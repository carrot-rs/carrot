...
use carrot_editor::{Editor, EditorEvent, MultiBuffer};
use carrot_language::{Buffer, HighlightedText, LanguageRegistry};
use carrot_project::Project;
use carrot_ui::{Color, Icon, IconName, Label, LabelCommon as _, SharedString};
use carrot_workspace::{
    Item, ItemHandle as _, ItemNavHistory, PaneRole, ToolbarItemLocation, Workspace,
...
use inazuma_util::paths::PathExt as _;
use std::{
    any::{Any, TypeId},
    path::PathBuf,
    pin::pin,
...
mod tests {
    use super::*;
    use carrot_editor::test::editor_test_context::assert_state_with_diff;
    use carrot_project::{FakeFs, Fs, Project};
    ...
    async fn test_save_changes_in_diff_view(cx: &mut TestAppContext) {
        ...
        let save_task = diff_view.update_in(cx, |diff_view, window, cx| {
            carrot_workspace::Item::save(
                diff_view,
                carrot_workspace::item::SaveOptions::default(),
                project.clone(),
                window,
                cx,
            )
