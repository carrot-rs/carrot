...
use carrot_buffer_diff::{BufferDiff, DiffHunkSecondaryStatus};
use carrot_editor::{
...
use carrot_git::repository::DiffType;
use inazuma_collections::{HashMap, HashSet};

use carrot_actions::agent::ReviewBranchDiff;
use carrot_git::{
    Commit, StageAll, StageAndNext, ToggleStaged, UnstageAll, UnstageAndNext, repository::RepoPath,
    status::FileStatus,
};
use carrot_language::{Anchor, Buffer, BufferId, Capability, OffsetRangeExt};
...
use carrot_ui::{DiffStat, Divider, KeyBinding, Tooltip, prelude::*, vertical_divider};
use carrot_workspace::{
    CloseActiveItem, ItemNavHistory, PaneRole, SerializableItem, ToolbarItemEvent,
...
use inazuma::{
    Action, AnyElement, App, AsyncWindowContext, Entity, EventEmitter, FocusHandle, Focusable,
    Render, Subscription, Task, WeakEntity, actions,
};
use inazuma_settings_framework::{Settings, SettingsStore};
use inazuma_util::{ResultExt as _, rel_path::RelPath};
...
impl ProjectDiff {
    pub(crate) fn register(workspace: &mut Workspace, cx: &mut Context<Workspace>) {
        ...
        carrot_workspace::register_serializable_item::<ProjectDiff>(cx);
    }
    ...
    fn deploy_branch_diff(
        ...
        carrot_telemetry::event!("Git Branch Diff Opened");
    ...
    pub fn deploy_at_project_path(
        ...
        carrot_telemetry::event!("Git Diff Opened", source = "Agent Panel");
        let existing = workspace
            .items_of_type::<Self>(cx)
            .find(|item| matches!(item.read(cx).diff_base(cx), DiffBase::Head));
        let project_diff = if let Some(existing) = existing {
        ...
        let editor = cx.new(|cx| {
            ...
            match branch_diff.read(cx).diff_base() {
                ...
                DiffBase::Merge { .. } => diff_display_editor.set_render_diff_hunk_controls(
                    Arc::new(|_, _, _, _, _, _, _, _| inazuma::Empty.into_any_element()),
                    cx,
                ),
            }
        ...
        self.editor.update(cx, |editor, cx| {
            editor.rhs_editor().update(cx, |editor, cx| {
                editor.change_selections(Default::default(), window, cx, |s| {
                    s.select_ranges(vec![
                        carrot_multi_buffer::Anchor::min()..carrot_multi_buffer::Anchor::min(),
                    ]);
                });
            });
        });
    ...
    fn button_states(&self, cx: &App) -> ButtonStates {
        ...
        if !ranges.iter().any(|range| range.start != range.end) {
            ...
            if let Some((excerpt_id, _, range)) = self
                ...
                .active_excerpt(cx)
            {
                ranges = vec![carrot_multi_buffer::Anchor::range_in_buffer(
                    excerpt_id, range,
                )];
            } else {
                ranges = Vec::default();
            ...
            editor.rhs_editor().update(cx, |editor, cx| {
                if was_empty {
                    editor.change_selections(
                        ...
                        cx,
                        |selections| {
                            selections.select_ranges([carrot_multi_buffer::Anchor::min()
                                ..carrot_multi_buffer::Anchor::min()])
                        },
                    );
                }
...
impl Item for ProjectDiff {
    ...
    fn clone_on_split(
        &self,
        _workspace_id: Option<carrot_workspace::WorkspaceId>,
        window: &mut Window,
        cx: &mut Context<Self>,
...
impl SerializableItem for ProjectDiff {
    ...
    fn cleanup(
        _: carrot_workspace::WorkspaceId,
        _: Vec<carrot_workspace::ItemId>,
        _: &mut Window,
        _: &mut App,
    ) -> Task<Result<()>> {
        Task::ready(Ok(()))
...
mod persistence {
    ...
    use carrot_project::git_store::branch_diff::DiffBase;
    use carrot_workspace::{ItemId, WorkspaceDb, WorkspaceId};
    ...
    carrot_db::static_connection!(ProjectDiffDb, [WorkspaceDb]);
...
fn render_send_review_to_agent_button(review_count: usize, focus_handle: &FocusHandle) -> Button {
    ...
    .start_icon(
        Icon::new(IconName::CarrotAssistant)
            .size(IconSize::Small)
            .color(Color::Muted),
    )
    .tooltip(Tooltip::for_action_title_in(
...
impl Addon for BranchDiffAddon {
    ...
    fn override_status_for_buffer_id(
        &self,
        buffer_id: carrot_language::BufferId,
        cx: &App,
    ) -> Option<FileStatus> {
        self.branch_diff
            .read(cx)
...
mod tests {
    use carrot_db::indoc;
    use carrot_editor::test::editor_test_context::{EditorTestContext, assert_state_with_diff};
    use carrot_git::status::{TrackedStatus, UnmergedStatus, UnmergedStatusCode};
            ...
            cx.update_window_entity(&buffer_editor, |buffer_editor, window, cx| {
                ...
                let prev_buffer_hunks = buffer_editor
                    .diff_hunks_in_ranges(
                        &[carrot_editor::Anchor::min()..carrot_editor::Anchor::max()],
                        snapshot,
                    )
                    .collect::<Vec<_>>();
    ...
    async fn test_go_to_prev_hunk_multibuffer(cx: &mut TestAppContext) {
        ...
        cx.dispatch_action(carrot_editor::actions::GoToPreviousHunk);
    ...
    async fn test_excerpts_splitting_after_restoring_the_middle_excerpt(cx: &mut TestAppContext) {
        ...
        cx.dispatch_action(carrot_editor::actions::GoToHunk);
        cx.dispatch_action(carrot_editor::actions::GoToHunk);
        cx.dispatch_action(carrot_git::Restore);
        cx.dispatch_action(carrot_editor::actions::MoveToBeginning);
                        ...
                        &[
                            (
                                ...
                                FileStatus::Tracked(TrackedStatus {
                                    index_status: carrot_git::status::StatusCode::Modified,
                                    worktree_status: carrot_git::status::StatusCode::Modified,
                                }),
                            ),
                            (
                                "helper.txt",
        ...
        assert_eq!(
            ...
            HashMap::from_iter([
                ...
                (
                    ...
                    Some(FileStatus::Tracked(TrackedStatus {
                        index_status: carrot_git::status::StatusCode::Added,
                        worktree_status: carrot_git::status::StatusCode::Added
                    }))
                )
            ])
        );
