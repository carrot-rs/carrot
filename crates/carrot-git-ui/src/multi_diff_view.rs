...
use carrot_editor::{Editor, EditorEvent, MultiBuffer, multibuffer_context_lines};
use carrot_language::{Buffer, Capability, HighlightedText, OffsetRangeExt};
use carrot_multi_buffer::PathKey;
use carrot_project::Project;
use carrot_ui::{Color, Icon, IconName, Label, LabelCommon as _};
...
fn register_entry(
    ...
    let ranges: Vec<std::ops::Range<carrot_language::Point>> = diff_snapshot
        .hunks(&snapshot)
...
impl Item for MultiDiffView {
    ...
    fn tab_tooltip_text(&self, _cx: &App) -> Option<carrot_ui::SharedString> {
        Some(self.title())
    }
