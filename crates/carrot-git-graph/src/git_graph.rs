...
use carrot_git_ui::{commit_tooltip::CommitAvatar, commit_view::CommitView, git_status_icon};
use carrot_language::line_diff;
use carrot_project::{
    Project,
    git_store::{
...
use carrot_theme::AccentColors;
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{
    ButtonLike, Chip, CommonAnimationExt as _, ContextMenu, DataTable, DiffStat, Divider,
    HighlightedLabel, IconButtonShape, ScrollableHandle, TableColumnWidths, TableInteractionState,
    TableResizeBehavior, Tooltip, WithScrollbar, prelude::*,
};
use carrot_workspace::{
    ItemId, PaneRole, Workspace, WorkspaceId, delete_unloaded_items,
...
use inazuma::{
    ...
    UniformListScrollHandle, WeakEntity, Window, actions, anchored, deferred, point, px,
    uniform_list,
};
use inazuma_collections::{BTreeMap, HashMap, IndexSet};
            ...
            div.when(
                ...
                |div| {
                    ...
                    div.on_action({
                        ...
                        move |_: &carrot_git_ui::git_panel::Open, window, cx| {
                            workspace
                    ...
                    .on_action(
                        move |action: &carrot_git_ui::git_panel::OpenAtCommit, window, cx| {
...
impl GitGraph {
    ...
    pub fn new(
        ...
        cx.observe_global_in::<inazuma_settings_framework::SettingsStore>(
            window,
                ...
                window.paint_layer(bounds, |window| {
                    ...
                    for visible_row_idx in 0..rows.len() {
                        ...
                        if is_hovered || is_selected {
                            ...
                            window.paint_quad(inazuma::fill(row_bounds, bg_color));
                        }
                    }
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .on_drag(DraggedSplitHandle, |_, _, _, cx| cx.new(|_| inazuma::Empty)),
            )
            .into_any_element()
    }
}
...
mod persistence {
    ...
    carrot_db::static_connection!(GitGraphsDb, [WorkspaceDb]);
...
mod tests {
    ...
    use carrot_git::Oid;
    use carrot_git::repository::InitialGraphCommitData;
    use carrot_project::Project;
    use carrot_project::git_store::{GitStoreEvent, RepositoryEvent};
