...
use carrot_client::proto::ViewId;
use inazuma_collections::HashMap;
use carrot_editor::DisplayPoint;
use carrot_feature_flags::{FeatureFlagAppExt as _, NotebookFeatureFlag};
...
use carrot_ui::{CommonAnimationExt, Tooltip, prelude::*};
use carrot_workspace::item::{ItemEvent, SaveOptions, TabContentParams};
use carrot_workspace::searchable::SearchableItemHandle;
use carrot_workspace::{Item, ItemHandle, Pane, ProjectItem, ToolbarItemLocation};
...
use carrot_ui::PopoverMenuHandle;
use carrot_actions::editor::{MoveDown, MoveUp};
use carrot_actions::notebook::{NotebookMoveDown, NotebookMoveUp};
...
pub fn init(cx: &mut App) {
    if cx.has_flag::<NotebookFeatureFlag>() || std::env::var("LOCAL_NOTEBOOK_DEV").is_ok() {
        carrot_workspace::register_project_item::<NotebookEditor>(cx);
    }
    ...
    cx.observe_flag::<NotebookFeatureFlag, _>({
        move |is_enabled, cx| {
            if is_enabled {
                ...
                // gets turned off they need to restart Carrot.
            }
        }
    })
    .detach();
...
pub struct NotebookEditor {
    ...
    worktree_id: carrot_project::WorktreeId,
...
impl NotebookEditor {
    pub fn new(
            ...
            match &cell_entity {
                Cell::Code(code_cell) => {
                    ...
                    cx.subscribe(&editor, move |this, _editor, event, cx| {
                        if let carrot_editor::EditorEvent::Focused = event {
                            if let Some(index) = this
                                .cell_order
                                .iter()
        ...
        let this = cx.entity();
        let cell_list = ListState::new(cell_count, inazuma::ListAlignment::Top, px(1000.));
    ...
    fn button_group(window: &mut Window, cx: &mut Context<Self>) -> Div {
        v_flex()
            ...
            .overflow_hidden()
            .rounded(px(5.))
            .bg(cx.theme().colors().title_bar.background)
            .p_px()
            .border_1()
...
impl carrot_project::ProjectItem for NotebookItem {
    fn try_open(
        project: &Entity<Project>,
        path: &ProjectPath,
        cx: &mut App,
...
impl Item for NotebookEditor {
    ...
    fn buffer_kind(&self, _: &App) -> carrot_workspace::item::ItemBufferKind {
    ...
    fn set_nav_history(
        &mut self,
        _: carrot_workspace::ItemNavHistory,
        _window: &mut Window,
        _: &mut Context<Self>,
    ) {
        // TODO
    ...
    fn reload(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| {
                ...
                this.cell_list =
                    ListState::new(this.cell_order.len(), inazuma::ListAlignment::Top, px(1000.));
                cx.notify();
            })?;
