...
use carrot_editor::{Editor, EditorMode, MultiBuffer, SizingBehavior};
...
use carrot_language::{Buffer, Language, LanguageRegistry};
...
use carrot_ui::{CommonAnimationExt, IconButtonShape, prelude::*};
...
impl Clickable for CellControl {
    fn on_click(
        self,
        handler: impl Fn(&inazuma::ClickEvent, &mut Window, &mut App) + 'static,
    ) -> Self {
        let button = self.button.on_click(handler);
        Self { button }
    }

    fn cursor_style(self, _cursor_style: inazuma::CursorStyle) -> Self {
        self
    }
}
...
pub struct MarkdownCell {
    ...
    _editor_subscription: inazuma::Subscription,
}
...
pub struct CodeCell {
    ...
    source: String,
    editor: Entity<carrot_editor::Editor>,
    outputs: Vec<Output>,
    selected: bool,
    cell_position: Option<CellPosition>,
    _language_task: Task<()>,
...
impl CodeCell {
    ...
    pub fn editor(&self) -> &Entity<carrot_editor::Editor> {
        &self.editor
    }
