...
use inazuma::{App, Entity, WeakEntity, Window, prelude::*};
use carrot_language::{BufferSnapshot, Language, LanguageName, Point};
use carrot_project::{ProjectItem as _, WorktreeId};
use carrot_workspace::{Workspace, notifications::NotificationId};
...
mod tests {
    ...
    use inazuma::App;
    use indoc::indoc;
    use carrot_language::{Buffer, Language, LanguageConfig, LanguageRegistry};
    ...
    fn test_markdown_code_blocks(cx: &mut App) {
        ...
        let fs = Arc::new(carrot_project::RealFs::new(None, cx.background_executor().clone()));
