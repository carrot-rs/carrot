...
use carrot_ui::{CommonAnimationExt, CopyButton, IconButton, Tooltip, prelude::*};

mod image;
use image::ImageView;

mod markdown;
use carrot_markdown::MarkdownView;
...
fn rank_mime_type(mimetype: &MimeType) -> usize {
    match mimetype {
        ...
        MimeType::Plain(_) => 1,
        // All other media types are not supported in Carrot at this time
        _ => 0,
    }
}
                                    ...
                                    if let Some(workspace) = workspace.upgrade() {
                                        ...
                                        let buffer = cx.new(|cx| {
                                            let mut buffer = Buffer::local(full_error, cx)
                                                .with_language(carrot_language::PLAIN_TEXT.clone(), cx);
                                            buffer
...
mod tests {
    ...
    ) -> (inazuma::VisualTestContext, WeakEntity<carrot_workspace::Workspace>) {
        cx.update(|cx| {
            let settings_store = SettingsStore::test(cx);
            cx.set_global(settings_store);
            carrot_theme_settings::init(carrot_theme::LoadThemes::JustBase, cx);
        });
        let fs = carrot_project::FakeFs::new(cx.background_executor.clone());
        let project = carrot_project::Project::test(fs, [] as [&Path; 0], cx).await;
        let window =
            cx.add_window(|window, cx| carrot_workspace::Workspace::test_new(project, window, cx));
    ...
    fn create_execution_view(
        ...
        weak_workspace: WeakEntity<carrot_workspace::Workspace>,
