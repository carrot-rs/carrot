...
use inazuma::{App, PlatformDispatcher, Priority, RunnableMeta};
...
pub const KERNEL_DOCS_URL: &str = "https://carrot.dev/docs/repl#changing-kernels";

pub fn init(fs: Arc<dyn Fs>, cx: &mut App) {
    set_dispatcher(carrot_dispatcher(cx));
    repl_sessions_ui::init(cx);
    ReplStore::init(fs, cx);
}

fn carrot_dispatcher(cx: &mut App) -> impl Dispatcher {
    struct CarrotDispatcher {
    ...
    // other crates in Carrot.
    impl Dispatcher for CarrotDispatcher {
        #[track_caller]
