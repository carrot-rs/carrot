...
pub struct SshRunningKernel {
    ...
    _ssh_tunnel_process: inazuma_util::command::Child,
...
impl SshRunningKernel {
    pub fn new<S: KernelSession + 'static>(
        ...
        window.spawn(cx, async move |cx| {
            ...
            let mut command = inazuma_util::command::new_command(&command_template.program);
            ...
            let local_connection_file =
                std::env::temp_dir().join(format!("carrot_ssh_kernel_{}.json", kernel_id));
            std::fs::write(
