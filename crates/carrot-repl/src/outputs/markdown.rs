use inazuma::{App, AppContext, ClipboardItem, Context, Entity, Window, div, prelude::*};
...
impl OutputContent for MarkdownView {
    ...
    fn buffer_content(&mut self, _: &mut Window, cx: &mut App) -> Option<Entity<Buffer>> {
        ...
        let buffer = cx.new(|cx| {
            let mut buffer =
                Buffer::local(source.clone(), cx).with_language(carrot_language::PLAIN_TEXT.clone(), cx);
