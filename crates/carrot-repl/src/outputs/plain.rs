...
//! It uses `carrot-term` to process and display text, supporting ANSI escape sequences for
//! formatting, colors, and other terminal features.
//!
...
use inazuma::{Bounds, ClipboardItem, Entity, FontStyle, Pixels, TextStyle, WhiteSpace, canvas, size};
...
use carrot_terminal_view::terminal_element::TerminalElement;
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{IntoElement, prelude::*};
...
/// * error tracebacks
///
/// It uses `carrot-term` to process and render text, supporting ANSI escape
/// sequences for text formatting and colors.
///
pub struct TerminalOutput {
    full_buffer: Option<Entity<Buffer>>,
    /// ANSI escape sequence processor for parsing input text.
    parser: Processor,
    /// `carrot-term` instance that manages the terminal state and content.
    handler: carrot_term::Term<VoidListener>,
}
...
pub fn terminal_size(window: &mut Window, cx: &mut App) -> carrot_terminal::TerminalBounds {
    ...
    carrot_terminal::TerminalBounds {
        cell_width,
        line_height,
        bounds: Bounds {
            origin: inazuma::Point::default(),
...
impl Render for TerminalOutput {
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let grid = self
            ...
            .renderable_content()
            .display_iter
            .map(|ic| carrot_terminal::IndexedCell {
                point: ic.point,
                cell: ic.cell.clone(),
            });
...
impl OutputContent for TerminalOutput {
    ...
    fn buffer_content(&mut self, _: &mut Window, cx: &mut App) -> Option<Entity<Buffer>> {
        ...
        let buffer = cx.new(|cx| {
            let mut buffer =
                Buffer::local(self.full_text(), cx).with_language(carrot_language::PLAIN_TEXT.clone(), cx);
