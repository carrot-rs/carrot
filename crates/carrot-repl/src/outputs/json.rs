...
use inazuma::{App, ClipboardItem, Context, Entity, Window, div, prelude::*};
use carrot_language::Buffer;
use serde_json::Value;
use carrot_ui::{Disclosure, prelude::*};
...
impl OutputContent for JsonView {
    ...
    fn buffer_content(&mut self, _window: &mut Window, cx: &mut App) -> Option<Entity<Buffer>> {
        ...
        let buffer = cx.new(|cx| {
            let mut buffer =
                Buffer::local(json_text, cx).with_language(carrot_language::PLAIN_TEXT.clone(), cx);
...
mod tests {
    ...
    #[inazuma::test]
    fn test_json_view_toggle_path(cx: &mut inazuma::App) {
        let view =
