...
use carrot_actions::project_panel::{Toggle, ToggleFocus};
use carrot_actions::workspace::OpenWithSystem;
use carrot_client::{ErrorCode, ErrorExt};
...
use carrot_feature_flags::{FeatureFlagAppExt, ProjectPanelUndoRedoFeatureFlag};
use carrot_file_icons::FileIcons;
use carrot_git::status::GitSummary;
use carrot_git_ui::file_diff_view::FileDiffView;
...
use carrot_worktree::CreatedEntry;
use inazuma::{
    ...
    ExternalPaths, FocusHandle, Focusable, FontWeight, InteractiveElement, KeyContext,
    ListHorizontalSizingBehavior, ListSizingBehavior, Modifiers, ModifiersChangedEvent,
    MouseButton, MouseDownEvent, Oklch, ParentElement, PathPromptOptions, Pixels, Point,
    PromptLevel, Render, ScrollStrategy, Stateful, Styled, Subscription, Task,
    UniformListScrollHandle, WeakEntity, Window, actions, anchored, deferred, div,
    linear_color_stop, linear_gradient, oklcha, point, px, size, transparent_white, uniform_list,
};
use inazuma_collections::{BTreeSet, HashMap, hash_map};
use inazuma_menu::{Confirm, SelectFirst, SelectLast, SelectNext, SelectPrevious};
...
pub fn init(cx: &mut App) {
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        ...
        workspace.register_action(|workspace, _: &carrot_git::FileHistory, window, cx| {
            ...
            if let Some(panel) = workspace.panel::<ProjectPanel>(cx) {
                ...
                if let Some(project_path) = maybe_project_path {
                    ...
                    {
                        carrot_git_ui::file_history_view::FileHistoryView::open(
                            repo_path,
                            git_store.downgrade(),
                            repo.downgrade(),
...
impl ProjectPanel {
        ...
        let project_panel = cx.new(|cx| {
            ...
            cx.subscribe_in(
                ...
                |this, project, event, window, cx| match event {
                    carrot_project::Event::ActiveEntryChanged(Some(entry_id)) => {
                        if ProjectPanelSettings::get_global(cx).auto_reveal_entries {
                            this.reveal_entry(project.clone(), *entry_id, true, window, cx)
                                .ok();
                        }
                    }
                    carrot_project::Event::ActiveEntryChanged(None) => {
                    ...
                    carrot_project::Event::RevealInProjectPanel(entry_id) => {
                        if let Some(()) = this
                    ...
                    carrot_project::Event::ActivateProjectPanel => {
                        cx.emit(PanelEvent::Activate);
                    }
                    ...
                    carrot_project::Event::WorktreeRemoved(id) => {
                    ...
                    | carrot_project::Event::WorktreeOrderChanged => {
                        this.update_visible_entries(None, false, false, window, cx);
                        cx.notify();
                    }
                    carrot_project::Event::ExpandedAllForEntry(worktree_id, entry_id) => {
                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        ...
                        menu.action("New File", Box::new(NewFile))
                            ...
                            .when(is_local, |menu| {
                                menu.action(
                                    carrot_ui::utils::reveal_in_file_manager_label(is_remote),
                            ...
                            .when(!is_dir && self.has_git_changes(entry_id), |menu| {
                                menu.separator().action(
                                    "Restore File",
                                    Box::new(carrot_git::RestoreFile { skip_prompt: false }),
                                )
                            })
                            .when(has_git_repo, |menu| {
                                menu.separator()
                                    .action("View File History", Box::new(carrot_git::FileHistory))
                            })
                            ...
                            .when(!is_collab && is_root, |menu| {
                                menu.separator()
                                    .action(
                                        ...
                                        Box::new(carrot_workspace::AddFolderToProject),
                                    )
            ...
            && edit_state.processing_filename.is_none()
        {
            self.filename_editor.update(cx, |editor, cx| {
                editor.move_to_beginning_of_line(
                    &carrot_editor::actions::MoveToBeginningOfLine {
                        ...
                        .update_in(cx, |project_panel, window, cx| {
                            ...
                            if is_dir {
                                project_panel.project.update(cx, |_, cx| {
                                    cx.emit(carrot_project::Event::Toast {
            ...
            && edit_state.processing_filename.is_none()
        {
            self.filename_editor.update(cx, |editor, cx| {
                editor.move_to_end_of_line(
                    &carrot_editor::actions::MoveToEndOfLine {
                        stop_at_soft_wraps: false,
                    },
                    window,
                    cx,
    ...
    fn download_from_remote(
        ...
        let notification_id =
            carrot_workspace::notifications::NotificationId::Named("download-progress".into());
            ...
            let dir_path = if entry.is_dir() {
                ...
                match entry.path.parent() {
                    ...
                    None => {
                        ...
                        self.workspace
                            .update(cx, |workspace, cx| {
                                carrot_search::ProjectSearchView::new_search_in_directory(
                                    workspace,
                                    RelPath::empty(),
                                    window,
                                    cx,
    ...
    pub fn selected_entry<'a>(
        &self,
        cx: &'a App,
    ) -> Option<(&'a Worktree, &'a carrot_project::Entry)> {
        let (worktree, entry) = self.selected_entry_handle(cx)?;
    ...
    fn selected_sub_entry<'a>(
        &self,
        cx: &'a App,
    ) -> Option<(Entity<Worktree>, &'a carrot_project::Entry)> {
    ...
    fn render_entry(
        ...
        div()
            ...
            .on_click(cx.listener(
                move |project_panel, event: &inazuma::ClickEvent, window, cx| {
                    ...
                    v_flex()
                        .child(
                            ...
                            .when(show_sticky_entries, |list| {
                                let sticky_items = carrot_ui::sticky_items(
                                    cx.entity(),
                                    |this, range, window, cx| {
                                        let mut items =
                                            SmallVec::with_capacity(range.end - range.start);
                        ...
                        .child(
                            div()
                                ...
                                .on_click(cx.listener(|this, event, window, cx| {
                                    if matches!(event, inazuma::ClickEvent::Keyboard(_)) {
                                        return;
                                    }
                                    cx.stop_propagation();
                                    this.selection = None;
                                ...
                                .when(!project.is_read_only(cx), |el| {
                                    el.on_click(cx.listener(
                                        |this, event: &inazuma::ClickEvent, window, cx| {
                                            if event.click_count() > 1
                                                && let Some(entry_id) =
                    ...
                    Button::new("open_project", "Open Project")
                        ...
                        .on_click(cx.listener(|this, _, window, cx| {
                            this.workspace
                                .update(cx, |_, cx| {
                                    window.dispatch_action(
                                        carrot_workspace::Open::default().boxed_clone(),
                                        cx,
                                    );
                                })
                                .log_err();
...
fn cmp_files_first(a: &Entry, b: &Entry) -> cmp::Ordering {
    inazuma_util::paths::compare_rel_paths_files_first(
        (&a.path, a.is_file()),
        (&b.path, b.is_file()),
    )
}
...
fn cmp_with_mode(
    ...
    match mode {
        inazuma_settings_framework::ProjectPanelSortMode::DirectoriesFirst => {
            cmp_directories_first(a, b)
        }
