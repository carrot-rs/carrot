...
use carrot_remote::{ConnectionIdentifier, RemoteClientDelegate, SshConnectionOptions};
use clap::Parser;
...
impl RemoteClientDelegate for BenchmarkRemoteClient {
    ...
    fn get_download_url(
        &self,
        _platform: carrot_remote::RemotePlatform,
        _release_channel: ReleaseChannel,
        _version: Option<Version>,
        _cx: &mut inazuma::AsyncApp,
    ) -> inazuma::Task<inazuma::Result<Option<String>>> {
        unimplemented!()
    }
    ...
    fn download_server_binary_locally(
        ...
        _version: Option<Version>,
        _cx: &mut inazuma::AsyncApp,
    ) -> inazuma::Task<inazuma::Result<std::path::PathBuf>> {
        unimplemented!()
    }

    fn set_status(&self, status: Option<&str>, _: &mut inazuma::AsyncApp) {
...
fn main() -> Result<(), anyhow::Error> {
    ...
    inazuma_platform::headless().run(|cx| {
        carrot_release_channel::init_test(semver::Version::new(0, 0, 0), ReleaseChannel::Dev, cx);
        inazuma_settings_framework::init(cx);
        let client = Client::production(cx);
        let http_client = FakeHttpClient::with_200_response();
        let (_, rx) = carrot_watch::channel(None);
        ...
        let fs = Arc::new(RealFs::new(None, cx.background_executor().clone()));
            ...
            cx.spawn(async move |cx| {
                let project = if let Some(ssh_target) = args.ssh {
                    ...
                    let connection_options = carrot_remote::RemoteConnectionOptions::from(ssh_connection_options);
                    let delegate = Arc::new(BenchmarkRemoteClient);
                    let remote_connection = carrot_remote::connect(connection_options.clone(), delegate.clone(), cx).await.unwrap();

                    let (_tx, rx) = oneshot::channel();
                    let remote_client =  cx.update(|cx| carrot_remote::RemoteClient::new(ConnectionIdentifier::setup(), remote_connection, rx, delegate.clone(), cx )).await?.ok_or_else(|| anyhow!("ssh initialization returned None"))?;
