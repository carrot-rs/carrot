...
use carrot_http_client::github::latest_github_release;
pub use carrot_language::*;
use carrot_language::{
    LanguageName, LanguageToolchainStore, LspAdapterDelegate, LspInstaller,
    language_settings::LanguageSettings,
};
use carrot_lsp::{LanguageServerBinary, LanguageServerName};
use futures::StreamExt;
use inazuma::{App, AsyncApp, Entity, Task};
...
use inazuma_settings_framework::SemanticTokenRules;
...
pub(crate) fn semantic_token_rules() -> SemanticTokenRules {
    let content = carrot_grammars::get_file("go/semantic_token_rules.json")
...
impl LspInstaller for GoLspAdapter {
    ...
    async fn fetch_server_binary(
        ...
        let go_version_output = inazuma_util::command::new_command(&go)
            .args(["version"])
            .output()
            .await
        ...
        let version_output = inazuma_util::command::new_command(&installed_binary_path)
            .arg("version")
            .output()
            .await
...
impl LspAdapter for GoLspAdapter {
    ...
    async fn label_for_completion(
        ...
        match completion.kind.zip(completion.detail.as_ref()) {
            Some((carrot_lsp::CompletionItemKind::MODULE, detail)) => {
            ...
            Some((
                carrot_lsp::CompletionItemKind::CONSTANT | carrot_lsp::CompletionItemKind::VARIABLE,
                detail,
            )) => {
                let text = format!("{label} {detail}");
                let source =
            ...
            Some((carrot_lsp::CompletionItemKind::STRUCT, _)) => {
            ...
            Some((carrot_lsp::CompletionItemKind::INTERFACE, _)) => {
            ...
            Some((carrot_lsp::CompletionItemKind::FIELD, detail)) => {
                let text = format!("{label} {detail}");
                let source =
            ...
            Some((
                carrot_lsp::CompletionItemKind::FUNCTION | carrot_lsp::CompletionItemKind::METHOD,
                detail,
            )) => {
    ...
    async fn label_for_symbol(
        ...
        let (text, filter_range, display_range) = match symbol.kind {
            ...
            carrot_lsp::SymbolKind::VARIABLE => {
                let text = format!("var {} = nil", name);
                let filter_range = 4..4 + name.len();
                let display_range = 0..filter_range.end;
                (text, filter_range, display_range)
            }
            carrot_lsp::SymbolKind::MODULE => {
...
mod tests {
    ...
    use inazuma::{AppContext, Oklch, TestAppContext};
