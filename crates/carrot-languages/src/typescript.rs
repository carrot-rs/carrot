...
use carrot_lsp::{CodeActionKind, LanguageServerBinary, LanguageServerName, Uri};
use carrot_node_runtime::{NodeRuntime, VersionStrategy};
use carrot_project::{Fs, lsp_store::language_server_settings};
use carrot_task::{TaskTemplate, TaskTemplates, VariableName};
use chrono::{DateTime, Local};
use futures::future::join_all;
use inazuma::{App, AppContext, AsyncApp, Entity, Task};
...
impl ContextProvider for TypeScriptContextProvider {
    fn associated_tasks(
        ...
        let Some(file) = carrot_project::File::from_dyn(file).cloned() else {
    ...
    fn build_context(
        &self,
        current_vars: &carrot_task::TaskVariables,
        ...
        let mut vars = carrot_task::TaskVariables::default();
...
mod tests {
    ...
    async fn test_outline(cx: &mut TestAppContext) {
        ...
        "#
            ...
            let buffer =
                cx.new(|cx| carrot_language::Buffer::local(text, cx).with_language(language, cx));
