...
use carrot_lsp::{InitializeParams, LanguageServerBinary, LanguageServerName};
use carrot_project::lsp_store::clangd_ext;
use futures::StreamExt;
use inazuma::{App, AsyncApp};
...
impl super::LspAdapter for CLspAdapter {
    ...
    async fn label_for_completion(
        ...
        match completion.kind {
            Some(carrot_lsp::CompletionItemKind::FIELD) if completion.detail.is_some() => {
            ...
            Some(kind) => {
                let highlight_name = match kind {
                    ...
                    | carrot_lsp::CompletionItemKind::CLASS
    ...
    async fn label_for_symbol(
        ...
        let (text, filter_range, display_range) = match symbol.kind {
            ...
            carrot_lsp::SymbolKind::ENUM => {
                let text = format!("enum {} {{}}", name);
                let filter_range = 5..5 + name.len();
                let display_range = 0..filter_range.end;
                (text, filter_range, display_range)
            }
            carrot_lsp::SymbolKind::INTERFACE | carrot_lsp::SymbolKind::CLASS => {
            ...
            carrot_lsp::SymbolKind::TYPE_PARAMETER => {
    ...
    fn underline_diagnostic(&self, diagnostic: &carrot_lsp::Diagnostic) -> bool {
...
mod tests {
    ...
    use inazuma::{AppContext as _, BorrowAppContext, TestAppContext};
