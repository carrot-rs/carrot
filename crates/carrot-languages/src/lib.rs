...
use inazuma::{App, SharedString, UpdateGlobal};
use inazuma_settings_framework::{SemanticTokenRules, SettingsStore};
    ...
    std::sync::LazyLock::new(|| {
        Arc::new(Language::new(
            LanguageConfig {
                ...
                soft_wrap: Some(carrot_language::SoftWrap::EditorWidth),
...
pub fn init(languages: Arc<LanguageRegistry>, fs: Arc<dyn Fs>, node: NodeRuntime, cx: &mut App) {
    ...
    languages.register_native_grammars(carrot_grammars::native_grammars());
                    ...
                    SettingsStore::update_global(cx, |settings, cx| {
                        settings
                            .set_extension_settings(
                                inazuma_settings_framework::ExtensionsSettingsContent {
    ...
    for provider in manifest_providers {
        carrot_project::ManifestProvidersStore::global(cx).register(provider);
    }
}
...
fn register_language(
    ...
    languages.register_language(
        ...
        Arc::new(move || {
            Ok(LoadedLanguage {
                config: config.clone(),
                queries: carrot_grammars::load_queries(name),
...
pub fn language(name: &str, grammar: tree_sitter::Language) -> Arc<Language> {
    Arc::new(
        Language::new(carrot_grammars::load_config(name), Some(grammar))
            .with_queries(carrot_grammars::load_queries(name))
            .unwrap(),
    )
}
...
fn load_config(name: &str) -> LanguageConfig {
    ...
    carrot_grammars::load_config_for_feature(name, grammars_loaded)
}
