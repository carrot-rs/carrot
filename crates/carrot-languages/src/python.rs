...
use carrot_http_client::github::{AssetKind, GitHubLspBinaryVersion, latest_github_release};
use carrot_language::language_settings::LanguageSettings;
use carrot_language::{
    Buffer, ContextLocation, DynLspInstaller, LanguageToolchainStore, LspInstaller, Symbol,
};
use carrot_language::{ContextProvider, LspAdapter, LspAdapterDelegate};
use carrot_language::{LanguageName, ManifestName, ManifestProvider, ManifestQuery};
use carrot_language::{Toolchain, ToolchainList, ToolchainLister, ToolchainMetadata};
use carrot_lsp::{LanguageServerBinary, Uri};
use carrot_lsp::{LanguageServerBinaryOptions, LanguageServerName};
use carrot_node_runtime::{NodeRuntime, VersionStrategy};
use carrot_project::Fs;
use carrot_project::lsp_store::language_server_settings;
use carrot_terminal::terminal_settings::TerminalSettings;
use futures::future::BoxFuture;
use futures::lock::OwnedMutexGuard;
use futures::{AsyncBufReadExt, StreamExt as _};
use inazuma::{App, AsyncApp, Entity, SharedString, Task};
use inazuma_collections::HashMap;
use inazuma_settings_framework::{SemanticTokenRules, Settings};
...
use carrot_task::{ShellKind, TaskTemplate, TaskTemplates, VariableName};
...
pub(crate) fn semantic_token_rules() -> SemanticTokenRules {
    let content = carrot_grammars::get_file("python/semantic_token_rules.json")
...
fn process_pyright_completions(items: &mut [carrot_lsp::CompletionItem]) {
    for item in items {
        ...
        let kind_priority = match item.kind {
            Some(carrot_lsp::CompletionItemKind::KEYWORD) => '0',
            Some(carrot_lsp::CompletionItemKind::ENUM_MEMBER) => '1',
            Some(carrot_lsp::CompletionItemKind::FIELD) => '2',
            Some(carrot_lsp::CompletionItemKind::PROPERTY) => '3',
            Some(carrot_lsp::CompletionItemKind::VARIABLE) => '4',
            Some(carrot_lsp::CompletionItemKind::CONSTANT) => '5',
            Some(carrot_lsp::CompletionItemKind::METHOD) => '6',
            Some(carrot_lsp::CompletionItemKind::FUNCTION) => '6',
            Some(carrot_lsp::CompletionItemKind::CLASS) => '7',
            Some(carrot_lsp::CompletionItemKind::MODULE) => '8',
...
fn label_for_pyright_completion(
    ...
    language: &Arc<carrot_language::Language>,
) -> Option<carrot_language::CodeLabel> {
    let label = &item.label;
    let label_len = label.len();
    let grammar = language.grammar()?;
    let highlight_id = match item.kind? {
        carrot_lsp::CompletionItemKind::METHOD => grammar.highlight_id_for_name("function.method"),
        carrot_lsp::CompletionItemKind::FUNCTION => grammar.highlight_id_for_name("function"),
        carrot_lsp::CompletionItemKind::CLASS => grammar.highlight_id_for_name("type"),
        carrot_lsp::CompletionItemKind::CONSTANT => grammar.highlight_id_for_name("constant"),
        carrot_lsp::CompletionItemKind::VARIABLE => grammar.highlight_id_for_name("variable"),
        _ => {
            return None;
        }
    };
    ...
    Some(carrot_language::CodeLabel::filtered(
        text,
        label_len,
...
fn label_for_python_symbol(
    ...
    let (text, filter_range, display_range) = match symbol.kind {
        carrot_lsp::SymbolKind::METHOD | carrot_lsp::SymbolKind::FUNCTION => {
            let text = format!("def {}():\n", name);
            let filter_range = 4..4 + name.len();
            let display_range = 0..filter_range.end;
            (text, filter_range, display_range)
        }
        carrot_lsp::SymbolKind::CLASS => {
            let text = format!("class {}:", name);
            let filter_range = 6..6 + name.len();
            let display_range = 0..filter_range.end;
            (text, filter_range, display_range)
        }
        carrot_lsp::SymbolKind::CONSTANT => {
    ...
    Some(carrot_language::CodeLabel::new(
    ...
    async fn label_for_completion(
        ...
        let highlight_id = match item.kind? {
            carrot_lsp::CompletionItemKind::METHOD => {
                grammar.highlight_id_for_name("function.method")
            }
            carrot_lsp::CompletionItemKind::FUNCTION => grammar.highlight_id_for_name("function"),
            carrot_lsp::CompletionItemKind::CLASS => grammar.highlight_id_for_name("type"),
            carrot_lsp::CompletionItemKind::CONSTANT => grammar.highlight_id_for_name("constant"),
...
impl LspAdapter for PyrightLspAdapter {
    ...
    async fn process_completions(&self, items: &mut [carrot_lsp::CompletionItem]) {
...
impl ContextProvider for PythonContextProvider {
    fn build_context(
        &self,
        variables: &carrot_task::TaskVariables,
        location: ContextLocation<'_>,
        _: Option<HashMap<String, String>>,
        toolchains: Arc<dyn LanguageToolchainStore>,
        cx: &mut inazuma::App,
    ) -> Task<Result<carrot_task::TaskVariables>> {
        ...
        cx.spawn(async move |cx| {
            ...
            Ok(carrot_task::TaskVariables::from_iter(
                test_target
                    .into_iter()
...
impl ToolchainLister for PythonToolchainProvider {
    ...
    fn activation_script(
        ...
        let conda_manager = settings
            ...
            .unwrap_or(inazuma_settings_framework::CondaManager::Auto);
            ...
            match toolchain.environment.kind {
                Some(PythonEnvironmentKind::Conda) => {
                    ...
                    let manager = match conda_manager {
                        inazuma_settings_framework::CondaManager::Conda => "conda",
                        inazuma_settings_framework::CondaManager::Mamba => "mamba",
                        inazuma_settings_framework::CondaManager::Micromamba => "micromamba",
                        inazuma_settings_framework::CondaManager::Auto => toolchain
                            .environment
                            .manager
                            .as_ref()
                ...
                Some(PythonEnvironmentKind::Pyenv) => {
                    ...
                    activation_script.extend(match shell {
                        ...
                        ShellKind::Bash => Some(format!("\"{pyenv}\" shell - bash {version}")),
                        ShellKind::Zsh => Some(format!("\"{pyenv}\" shell - zsh {version}")),
...
impl PyLspAdapter {
    ...
    async fn ensure_venv(delegate: &dyn LspAdapterDelegate) -> Result<Arc<Path>> {
        ...
        if !path.exists() {
            inazuma_util::command::new_command(python_path)
                .arg("-m")
                .arg("venv")
                .arg("pylsp-venv")
    ...
    async fn label_for_completion(
        ...
        let highlight_id = match item.kind? {
            ...
            carrot_lsp::CompletionItemKind::FUNCTION => {
                grammar.highlight_id_for_name("function")?
            }
            carrot_lsp::CompletionItemKind::CLASS => grammar.highlight_id_for_name("type")?,
            carrot_lsp::CompletionItemKind::CONSTANT => {
                grammar.highlight_id_for_name("constant")?
            }
            _ => return None,
        };
        Some(carrot_language::CodeLabel::filtered(
...
impl LspInstaller for PyLspAdapter {
    ...
    async fn fetch_server_binary(
        ...
        let pip_path = venv.join(BINARY_DIR).join("pip3");
            ...
            inazuma_util::command::new_command(pip_path.as_path())
                .arg("install")
            ...
            inazuma_util::command::new_command(pip_path)
                .arg("install")
                .arg("pylsp-mypy")
                .arg("--upgrade")
                .output()
...
#[cfg(test)]
mod tests {
    use carrot_language::{AutoindentMode, Buffer};
    use inazuma::{AppContext as _, BorrowAppContext, Context, TestAppContext};
    ...
    async fn test_conda_activation_script_injection(cx: &mut TestAppContext) {
        use carrot_language::{LanguageName, Toolchain, ToolchainLister};
        use carrot_task::ShellKind;
        use inazuma_settings_framework::{CondaManager, VenvSettings};
        ...
        let fs = carrot_project::FakeFs::new(cx.executor());
