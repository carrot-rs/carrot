...
use carrot_lsp::{LanguageServerBinary, LanguageServerName, Uri};
...
impl ContextProvider for JsonTaskProvider {
    fn associated_tasks(
        ...
        cx: &App,
    ) -> inazuma::Task<Option<TaskTemplates>> {
...
impl LspAdapter for JsonLspAdapter {
    ...
    async fn workspace_configuration(
        ...
        let mut config = cx.update(|cx| {
            let schemas = carrot_json_schema_store::all_schema_file_associations(
                &self.languages,
...
impl LspInstaller for NodeVersionAdapter {
    ...
    async fn fetch_latest_server_version(
        ...
        let release = latest_github_release(
            "carrot-rs/package-version-server",
            true,
            false,
