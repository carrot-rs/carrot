...
use anyhow::Result;
use carrot_ui::input::{InputState, Rope};
use inazuma::{Context, Task, Window};
use lsp_types::{
    CompletionContext, CompletionItem, CompletionItemKind, CompletionResponse,
    InlineCompletionContext, InlineCompletionItem, InlineCompletionResponse, InsertTextFormat,
};
...
impl ShellCompletionProvider {
    ...
    fn complete_command(&self, prefix: &str) -> Vec<CompletionItem> {
        ...
        let builtins = match self.shell.as_str() {
            "zsh" | "bash" | "sh" => vec![
                "cd", "echo", "export", "source", "alias", "unalias", "type", "which", "eval",
                "exec", "set", "unset", "readonly", "shift", "test", "true", "false", "pwd",
                "pushd", "popd", "dirs", "bg", "fg", "jobs", "kill", "wait", "trap", "umask",
            ],
            "fish" => vec![
                "cd", "echo", "set", "function", "end", "if", "else", "for", "while", "switch",
                "case", "begin", "return", "source", "alias", "type", "test", "true", "false",
            ],
            "nu" => vec![
                "cd", "ls", "mv", "cp", "rm", "mkdir", "open", "save", "where", "select", "get",
                "each", "par-each", "sort-by", "group-by", "flatten", "merge", "wrap", "unwrap",
    ...
    /// Try to load an external spec from ~/.config/carrot/specs/ or app bundle.
    fn load_external_spec(&self, command: &str) -> Option<crate::CliSpec> {
        let spec_dirs = [
            dirs::config_dir().map(|c| c.join("carrot/specs")),
    ...
    fn build_completions(&self, text: &str, offset: usize) -> Vec<CompletionItem> {
        ...
        match &ctx.token_position {
            crate::TokenPosition::Command => self.complete_command(&ctx.current_token),
            ...
            crate::TokenPosition::Argument(_) => {
                ...
                if ctx.command == "git" {
                    let git_branch_cmds =
                        ["checkout", "switch", "merge", "rebase", "branch", "diff"];
                    if ctx
                        .subcommands
                        .first()
                        .map_or(false, |s| git_branch_cmds.contains(&s.as_str()))
                    {
...
impl carrot_ui::input::CompletionProvider for ShellCompletionProvider {
    fn completions(
        &self,
        text: &Rope,
        offset: usize,
