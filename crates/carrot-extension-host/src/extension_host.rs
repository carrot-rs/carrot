...
use carrot_client::{Client, proto, telemetry::Telemetry};
use carrot_cloud_api_types::{ExtensionMetadata, ExtensionProvides, GetExtensionsResponse};
pub use carrot_extension::ExtensionManifest;
use carrot_extension::extension_builder::{CompileExtensionOptions, ExtensionBuilder};
...
use carrot_http_client::{AsyncBody, HttpClient, HttpClientWithUrl};
use carrot_language::{
    LanguageConfig, LanguageMatcher, LanguageName, LanguageQueries, LoadedLanguage,
    QUERY_FILENAME_PREFIXES, Rope,
};
use carrot_node_runtime::NodeRuntime;
use carrot_project::ContextProviderWithTasks;
use carrot_remote::RemoteClient;
use carrot_task::TaskTemplates;
...
use inazuma_settings_framework::{SemanticTokenRules, Settings, SettingsStore};
...
/// The current extension [`SchemaVersion`] supported by Carrot.
...
/// Returns the [`SchemaVersion`] range that is compatible with this version of Carrot.
pub fn schema_version_range() -> RangeInclusive<SchemaVersion> {
    SchemaVersion::ZERO..=CURRENT_SCHEMA_VERSION
}

/// Returns whether the given extension version is compatible with this version of Carrot.
...
pub fn init(
    ...
    let store = cx.new(move |cx| {
        ExtensionStore::new(
            carrot_paths::extensions_dir().clone(),
            None,
...
impl ExtensionStore {
    ...
    /// Installs any extensions that should be included with Carrot by default.
    ///
    ...
    fn fetch_extensions_from_api(
        ...
        let url = self.http_client.build_carrot_api_url(path, query);
    ...
    fn install_or_upgrade_extension_at_endpoint(
        ...
        cx.spawn(async move |this, cx| {
            ...
            let content_length = response
                .headers()
                .get(carrot_http_client::http::header::CONTENT_LENGTH)
                ...
                this.update(cx, |this, cx| {
                        ...
                        && let Some(manifest) = this.extension_manifest_for_id(&extension_id)
                    {
                        events.update(cx, |this, cx| {
                            this.emit(
                                carrot_extension::Event::ExtensionInstalled(manifest.clone()),
                                cx,
                            )
                        });
                    }
        ...
        cx.spawn(async move |extension_store, cx| {
            ...
            extension_store.update(cx, |_, cx| {
                    ...
                    && let Some(manifest) = extension_manifest
                {
                    events.update(cx, |this, cx| {
                        this.emit(
                            carrot_extension::Event::ExtensionUninstalled(manifest.clone()),
                            cx,
                        )
                    });
                }
    ...
    fn extensions_updated(
        ...
        carrot_telemetry::event!("Extensions Loaded", id_and_versions = extension_ids);
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                ...
                if let Some(events) = ExtensionEvents::try_global(cx) {
                    events.update(cx, |this, cx| {
                        this.emit(carrot_extension::Event::ExtensionsInstalledChanged, cx)
                    });
                }
            })
            .ok();
    ...
    fn prepare_remote_extension(
        ...
        cx.background_spawn(async move {
            ...
            for (adapter_name, meta) in loaded_extension.manifest.debug_adapters.iter() {
                let schema_path =
                    &carrot_extension::build_debug_adapter_schema_path(adapter_name, meta);
