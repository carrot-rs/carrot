...
use carrot_fs::{FakeFs, Fs, RealFs};
use carrot_http_client::{FakeHttpClient, Response};
use carrot_language::{BinaryStatus, LanguageMatcher, LanguageName, LanguageRegistry};
use carrot_language_extension::LspAccess;
use carrot_lsp::LanguageServerName;
use carrot_node_runtime::NodeRuntime;
use carrot_project::{DEFAULT_COMPLETION_CONTEXT, Project};
use carrot_release_channel::AppVersion;
use carrot_reqwest_client::ReqwestClient;
use carrot_theme::ThemeRegistry;
use futures::{AsyncReadExt, FutureExt, StreamExt, io::BufReader};
use inazuma::{AppContext as _, BackgroundExecutor, TestAppContext};
use inazuma_collections::{BTreeMap, HashSet};
...
async fn test_extension_store(cx: &mut TestAppContext) {
    ...
    let mut expected_index = ExtensionIndex {
        extensions: [
            ...
            (
                ...
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "carrot-monokai".into(),
                        name: "Carrot Monokai".into(),
                        version: "2.0.0".into(),
                        schema_version: SchemaVersion::ZERO,
                        description: None,
                        authors: vec![],
    ...
    // Theme extension proxy is registered via ExtensionStore, no separate init needed
    let language_registry = Arc::new(LanguageRegistry::test(cx.executor()));
    carrot_language_extension::init(LspAccess::Noop, proxy.clone(), language_registry.clone());
    ...
    fs.insert_tree(
        "/the-extension-dir/installed/carrot-gruvbox",
        json!({
            "extension.json": r#"{
                "id": "carrot-gruvbox",
                "name": "Carrot Gruvbox",
    ...
    expected_index.extensions.insert(
        ...
        ExtensionIndexEntry {
            manifest: Arc::new(ExtensionManifest {
                id: "carrot-gruvbox".into(),
                name: "Carrot Gruvbox".into(),
                version: "1.0.0".into(),
                schema_version: SchemaVersion::ZERO,
                description: None,
                authors: vec![],
    ...
    // Create new extension store, as if Carrot were restarting.
    drop(store);
    let store = cx.new(|cx| {
        ExtensionStore::new(
            PathBuf::from("/the-extension-dir"),
    ...
    store.update(cx, |store, cx| {
        store
            .uninstall_extension("carrot-ruby".into(), cx)
            .detach_and_log_err(cx);
    });

    cx.executor().advance_clock(RELOAD_DEBOUNCE_DURATION);
    expected_index.extensions.remove("carrot-ruby");
...
async fn test_extension_store_with_test_extension(cx: &mut TestAppContext) {
    ...
    let proxy = Arc::new(ExtensionHostProxy::new());
    let _theme_registry = Arc::new(ThemeRegistry::new(Box::new(())));
    // Theme extension proxy is registered via ExtensionStore, no separate init needed
    let language_registry = project.read_with(cx, |project, _cx| project.languages().clone());
    carrot_language_extension::init(
    ...
    fake_server.set_request_handler::<carrot_lsp::request::Completion, _, _>(|_, _| async move {
        Ok(Some(carrot_lsp::CompletionResponse::Array(vec![
            ...
            carrot_lsp::CompletionItem {
                label: "Quux".into(),
                kind: Some(carrot_lsp::CompletionItemKind::CONSTRUCTOR),
                detail: Some("fn(String) -> T".into()),
                ..Default::default()
            },
            carrot_lsp::CompletionItem {
                label: "my_string".into(),
                kind: Some(carrot_lsp::CompletionItemKind::CONSTANT),
    ...
    await_or_timeout(
        ...
        async {
            fake_server
                .clone()
                .try_receive_notification::<carrot_lsp::notification::Initialized>()
                .await;
        },
    )
    .await;
