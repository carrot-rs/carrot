...
use carrot_workspace::{
    AppState, CloseIntent, ItemHandle, OpenVisible, SerializedWorkspaceLocation, Workspace,
    WorkspaceDb, WorkspaceId, WorkspaceLocation, WorkspaceSettings, open_items,
...
fn notify_if_database_failed(workspace: Entity<Workspace>, cx: &mut inazuma::AsyncApp) {
    let _ = workspace.update(cx, |workspace, cx| {
        if (*carrot_db::ALL_FILE_DB_FAILED).load(std::sync::atomic::Ordering::Acquire) {
                ...
                |cx| {
                    cx.new(|cx| {
                        MessageNotification::new("Failed to load the database file.", cx)
                            ...
                            .primary_icon(carrot_ui::IconName::Plus)
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    Box::new(carrot_actions::feedback::FileBugReport),
                                    cx,
                                )
                            })
                    })
...
pub async fn get_any_active_workspace(
    ...
    activate_any_workspace_window(&mut cx).context("could not open carrot")
}
...
fn workspace_from_window(window: WindowHandle<AppShell>, cx: &App) -> Option<Entity<Workspace>> {
    window
        .read(cx)
        .ok()
        .and_then(|shell| shell.view().clone().downcast::<Workspace>().ok())
}
...
pub fn new_local(
    ...
    init: Option<
        Box<dyn FnOnce(&mut Workspace, &mut inazuma::Window, &mut Context<Workspace>) + Send>,
    >,
    _activate: bool,
    cx: &mut App,
        ...
        let (window, workspace): (WindowHandle<AppShell>, Entity<Workspace>) =
            if let Some(window) = requesting_window {
                ...
                let workspace = window.update(cx, |_shell, window, cx| {
                    ...
                    // Single workspace per window in Carrot — just return it
                    workspace
                })?;
                (window, workspace)
            } else {
                ...
                let workspace = window.update(cx, |shell: &mut AppShell, _, _cx| {
                    shell
                        ...
                        .clone()
                        .downcast::<Workspace>()
                        .expect("AppShell view should be a Workspace")
                })?;
                (window, workspace)
            };
                    ...
                    .update(cx, |_, window, cx| {
                        workspace.update(cx, |workspace, cx| {
                            for (dock, serialized_dock) in [
                                ...
                                dock.update(cx, |dock: &mut carrot_workspace::dock::Dock, cx| {
            ...
            result.window
                .update(cx, move |_shell, _window, cx| {
                    if let Some(workspace) = AppShell::workspace(_window, cx) {
                        workspace.update(cx, |workspace, cx| {
                            workspace.show_notification(carrot_workspace::notifications::NotificationId::unique::<OpenInWsl>(), cx, move |cx| {
                                ...
                                cx.new(move |cx| {
                                    carrot_workspace::notifications::simple_message_notification::MessageNotification::new(msg, cx)
                                        .primary_message("Open in WSL")
                                        .primary_icon(carrot_ui::IconName::FolderOpen)
                                        .primary_on_click(move |window, cx| {
                                            window.dispatch_action(Box::new(carrot_remote::OpenWslPath {
                                                    distro: carrot_remote::WslConnectionOptions {
...
pub fn join_in_room_project(
    ...
    cx.spawn(async move |cx| {
        let existing_window_and_workspace: Option<(WindowHandle<AppShell>, Entity<Workspace>)> =
            ...
            existing_window_and_workspace
        {
            ...
            let active_call =
                cx.update(|cx| carrot_workspace::GlobalAnyActiveCall::global(cx).clone());
            let project = cx
                .update(|cx| {
        ...
        shell_window.update(cx, |_shell, window, cx| {
            ...
            if let Some(workspace) = AppShell::workspace(window, cx) {
                workspace.update(cx, |workspace, cx| {
                    let follow_peer_id = carrot_workspace::GlobalAnyActiveCall::try_global(cx)
