...
use carrot_ui::NotificationList;
use carrot_ui::StyledExt as _;
use carrot_ui::input::InputState;
use carrot_ui::utils::window_border::{self, window_border};
use carrot_ui::{
    ClearNotifications, CloseAllDialogs, CloseDialog, CloseSheet, DeferCloseDialog, PendingDialogs,
};
...
pub fn init(cx: &mut App) {
    ...
    // Register the WorkspaceOpener so carrot-workspace can open windows via carrot-shell
    carrot_workspace::set_workspace_opener(std::sync::Arc::new(ShellWorkspaceOpener), cx);
}

/// WorkspaceOpener implementation that delegates to carrot-shell's open functions.
struct ShellWorkspaceOpener;

impl carrot_workspace::WorkspaceOpener for ShellWorkspaceOpener {
    fn open_paths(
        &self,
        abs_paths: &[std::path::PathBuf],
        app_state: std::sync::Arc<carrot_workspace::AppState>,
        env: Option<inazuma_collections::HashMap<String, String>>,
        cx: &mut App,
    ) -> inazuma::Task<anyhow::Result<Entity<carrot_workspace::Workspace>>> {
...
impl AppShell {
    pub fn new(view: impl Into<AnyView>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        subscriptions.push(cx.observe_global_in::<PendingDialogs>(
            window,
        ...
        if let Ok(workspace) = view.clone().downcast::<carrot_workspace::Workspace>() {
            let window_id = window.window_handle().window_id();
            carrot_workspace::register_workspace_for_window(window_id, workspace.downgrade(), cx);
        }
        ...
        subscriptions.push(cx.on_release(move |_this: &mut Self, cx| {
            carrot_workspace::unregister_workspace_for_window(release_window_id, cx);
        }));
    ...
    pub fn workspace(window: &Window, cx: &App) -> Option<Entity<carrot_workspace::Workspace>> {
        window.root::<AppShell>().flatten().and_then(|shell| {
            shell
                .read(cx)
                .view
                .clone()
                .downcast::<carrot_workspace::Workspace>()
                .ok()
        })
    }
}
...
pub fn with_active_workspace(
    cx: &mut App,
    f: impl FnOnce(
        &mut carrot_workspace::Workspace,
        &mut Window,
        &mut Context<carrot_workspace::Workspace>,
    ) + Send
    + 'static,
) {
...
impl Render for AppShell {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        window.set_rem_size(carrot_theme::theme_settings(cx).ui_font_size(cx));

        let notification = self.notification.clone();
        let notification_placement = Anchor::TopRight;

        let sheet_el = Self::build_sheet_element(self, window, cx).map(|el| el.into_any_element());
        let dialog_el =
            Self::build_dialog_element(self, window, cx).map(|el| el.into_any_element());

        let workspace_view = self.view.clone();
        ...
        window_border().shadow_size(self.window_shadow_size).child(
            div()
                ...
                // Action handlers for window_shell actions dispatched from carrot-ui
                ...
                .relative()
                .size_full()
                .font_family(carrot_theme::theme_settings(cx).ui_font(cx).family.clone())
