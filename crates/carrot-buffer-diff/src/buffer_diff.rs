...
use inazuma::{App, AppContext as _, Context, Entity, EventEmitter, Task};
...
pub struct BufferDiff {
    ...
    inner: BufferDiffInner<Entity<carrot_language::Buffer>>,
    secondary_diff: Option<Entity<BufferDiff>>,
}

#[derive(Clone)]
pub struct BufferDiffSnapshot {
    inner: BufferDiffInner<carrot_language::BufferSnapshot>,
...
impl<BaseText> BufferDiffInner<BaseText> {
    fn buffer_version(&self) -> &inazuma_clock::Global {
...
impl inazuma_sum_tree::Item for InternalDiffHunk {
    type Summary = DiffHunkSummary;

    fn summary(&self, _cx: &inazuma_text::BufferSnapshot) -> Self::Summary {
        DiffHunkSummary {
...
impl inazuma_sum_tree::Item for PendingHunk {
    type Summary = DiffHunkSummary;
...
impl inazuma_sum_tree::Summary for DiffHunkSummary {
    type Context<'a> = &'a inazuma_text::BufferSnapshot;
...
impl inazuma_sum_tree::SeekTarget<'_, DiffHunkSummary, DiffHunkSummary> for Anchor {
    fn cmp(
        &self,
...
impl inazuma_sum_tree::SeekTarget<'_, DiffHunkSummary, DiffHunkSummary> for usize {
    fn cmp(
        &self,
        cursor_location: &DiffHunkSummary,
        _cx: &inazuma_text::BufferSnapshot,
    ) -> Ordering {
...
impl std::fmt::Debug for BufferDiffInner<carrot_language::BufferSnapshot> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BufferDiffSnapshot")
...
impl BufferDiffSnapshot {
    ...
    pub fn buffer_version(&self) -> &inazuma_clock::Global {
        self.inner.buffer_version()
    }

    fn original_buffer_snapshot(&self) -> &inazuma_text::BufferSnapshot {
    ...
    pub fn hunks_intersecting_range<'a>(
        &'a self,
        range: Range<Anchor>,
        buffer: &'a inazuma_text::BufferSnapshot,
    ...
    pub fn hunks_intersecting_base_text_range<'a>(
        &'a self,
        range: Range<usize>,
        main_buffer: &'a inazuma_text::BufferSnapshot,
    ) -> impl 'a + Iterator<Item = DiffHunk> {
        let unstaged_counterpart = self.secondary_diff.as_ref().map(|diff| &diff.inner);
        let filter = move |summary: &DiffHunkSummary| {
    ...
    pub fn hunks<'a>(
        &'a self,
        buffer_snapshot: &'a inazuma_text::BufferSnapshot,
    ) -> impl 'a + Iterator<Item = DiffHunk> {
    ...
    pub fn base_text(&self) -> &carrot_language::BufferSnapshot {
    ...
    fn hunk_before_base_text_offset<'a>(
        &self,
        target: usize,
        cursor: &mut inazuma_sum_tree::Cursor<'a, '_, InternalDiffHunk, DiffHunkSummary>,
...
impl BufferDiffInner<Entity<carrot_language::Buffer>> {
...
impl BufferDiffInner<carrot_language::BufferSnapshot> {
...
fn build_diff_options(
    ...
    language_scope: Option<carrot_language::LanguageScope>,
    cx: &App,
) -> Option<DiffOptions> {
    #[cfg(any(test, feature = "test-support"))]
    {
        if !cx.has_global::<inazuma_settings_framework::SettingsStore>() {
...
fn compute_hunks(
    ...
    if let Some((diff_base, diff_base_rope)) = diff_base {
        ...
        // A common case in Carrot is that the empty buffer is represented as just a newline,
...
fn compare_hunks(
    ...
    new_snapshot: &inazuma_text::BufferSnapshot,
    old_base_text: &inazuma_text::BufferSnapshot,
    new_base_text: &inazuma_text::BufferSnapshot,
) -> DiffChanged {
    ...
    let mut last_unchanged_new_hunk_end: Option<inazuma_text::Anchor> = None;
    let mut has_changes = false;
    let mut extended_end_candidate: Option<inazuma_text::Anchor> = None;
    ...
    let extended_range = if has_changes && let Some(changed_range) = changed_range.clone() {
        let extended_start = *last_unchanged_new_hunk_end
            .unwrap_or(inazuma_text::Anchor::min_for_buffer(
        ...
        let extended_end = *extended_end_candidate
            .unwrap_or(inazuma_text::Anchor::max_for_buffer(
                new_snapshot.remote_id(),
            ))
...
pub struct DiffChanged {
    pub changed_range: Option<Range<inazuma_text::Anchor>>,
    pub base_text_changed_range: Option<Range<usize>>,
    pub extended_range: Option<Range<inazuma_text::Anchor>>,
}
...
impl BufferDiff {
    pub fn new(buffer: &inazuma_text::BufferSnapshot, cx: &mut App) -> Self {
        let base_text = cx.new(|cx| {
            let mut buffer = carrot_language::Buffer::local("", cx);
    ...
    pub fn new_unchanged(buffer: &inazuma_text::BufferSnapshot, cx: &mut Context<Self>) -> Self {
        let base_text = buffer.text();
        let base_text = cx.new(|cx| {
            let mut buffer = carrot_language::Buffer::local(base_text, cx);
            buffer.set_capability(Capability::ReadOnly, cx);
            buffer
        });
    ...
    pub fn new_with_base_text(
        ...
        inazuma_text::LineEnding::normalize(&mut base_text);
    ...
    fn set_snapshot_with_secondary_inner(
        ...
        let DiffChanged {
            ...
            _ => {
                let full_range = inazuma_text::Anchor::min_max_range_for_buffer(self.buffer_id);
    ...
    pub fn base_text(&self, cx: &App) -> carrot_language::BufferSnapshot {
    ...
    pub fn set_base_text(
        ...
        let complete_on_drop = inazuma_util::defer(|| {
            tx.send(()).ok();
        });
        cx.spawn(async move |this, cx| {
            let Some(state) = this
    ...
    pub fn base_text_buffer(&self) -> &Entity<carrot_language::Buffer> {
        &self.inner.base_text
    }
}
...
mod tests {
    ...
    use inazuma_text::{Buffer, BufferId, ReplicaId, Rope};
    ...
    #[inazuma::test]
    async fn test_buffer_diff_simple(cx: &mut inazuma::TestAppContext) {
        let diff_base = "
            one
            two
            three
    ...
    #[inazuma::test]
    async fn test_buffer_diff_with_secondary(cx: &mut inazuma::TestAppContext) {
        let head_text = "
            zero
            one
            two
    ...
    async fn test_stage_all_with_stale_buffer(cx: &mut TestAppContext) {
        // Regression test for CARROT-5R2: when the buffer is edited after the diff is
    ...
    async fn test_staging_and_unstaging_hunks(cx: &mut TestAppContext, mut rng: StdRng) {
        ...
        fn uncommitted_diff(
            working_copy: &carrot_language::BufferSnapshot,
            index_text: &Rope,
            head_text: String,
        ...
        let working_copy = cx.new(|cx| {
            carrot_language::Buffer::local_normalized(
                Rope::from(working_copy.as_str()),
                inazuma_text::LineEnding::default(),
                cx,
            )
        });
    ...
    #[inazuma::test]
    async fn test_changed_ranges(cx: &mut inazuma::TestAppContext) {
        let base_text = "
            one
            two
            three
        ...
        "
        .unindent();
        let buffer = cx.new(|cx| carrot_language::Buffer::local(buffer_text, cx));
        let diff = cx.new(|cx| {
