...
pub struct GoToLine {
    ...
    prev_scroll_position: Option<inazuma::Point<ScrollOffset>>,
    current_line: u32,
...
impl GoToLine {
    fn register(editor: &mut Editor, _window: Option<&mut Window>, cx: &mut Context<Editor>) {
        ...
        editor
            .register_action(
                move |_: &carrot_editor::actions::ToggleGoToLine, window, cx| {
    ...
    pub fn new(
        ...
        let (user_caret, last_line, scroll_position) = active_editor.update(cx, |editor, cx| {
            ...
            let last_line = editor
                ...
                .into_iter()
                .map(move |(_, _, range)| {
                    inazuma_text::ToPoint::to_point(&range.context.end, &snapshot).row
                })
                .max()
                .unwrap_or(0);
    ...
    fn on_line_editor_event(
        ...
        match event {
            carrot_editor::EditorEvent::Blurred => {
                self.prev_scroll_position.take();
                cx.emit(DismissEvent)
            }
            carrot_editor::EditorEvent::BufferEdited => self.highlight_current_line(cx),
            _ => {}
        }
    }
...
mod tests {
    use super::*;
    use carrot_editor::actions::{MoveRight, MoveToBeginning, SelectAll};
    ...
    fn open_go_to_line_view(
        ...
        cx.dispatch_action(carrot_editor::actions::ToggleGoToLine);
        workspace.update(cx, |workspace, cx| {
