...
use carrot_http_client::{self, AsyncBody, HttpClient};
use carrot_language::LanguageConfig;
...
impl ExtensionBuilder {
    ...
    async fn compile_rust_extension(
        ...
        let output = inazuma_util::command::new_command("cargo")
        ...
        let wasm_extension_api_version =
            parse_wasm_extension_version(&manifest.id, &component_bytes)
                .context("compiled wasm did not contain a valid carrot extension api version")?;
    ...
    async fn compile_grammar(
        ...
        if file_newer_than_deps(&grammar_wasm_path, &[&parser_path, &scanner_path]).unwrap_or(false)
        {
            ...
            let clang_output = inazuma_util::command::new_command(&clang_path)
                .args(["-fPIC", "-shared", "-Os"])
    ...
    async fn checkout_repo(&self, directory: &Path, url: &str, rev: &str) -> Result<()> {
        ...
        if directory.exists() {
            let remotes_output = inazuma_util::command::new_command("git")
                .arg("--git-dir")
                .arg(&git_dir)
                .args(["remote", "-v"])
                .output()
            ...
            let remote_add_output = inazuma_util::command::new_command("git")
                .arg("--git-dir")
                .arg(&git_dir)
        ...
        let fetch_output = inazuma_util::command::new_command("git")
            .arg("--git-dir")
            .arg(&git_dir)
        ...
        let checkout_output = inazuma_util::command::new_command("git")
            .arg("--git-dir")
            .arg(&git_dir)
            .args(["checkout", rev])
            .current_dir(directory)
    ...
    async fn install_rust_wasm_target_if_needed(&self) -> Result<()> {
        let rustc_output = inazuma_util::command::new_command("rustc")
            .arg("--print")
            .arg("sysroot")
            .output()
            .await
        ...
        let output = inazuma_util::command::new_command("rustup")
    ...
    async fn install_wasi_sdk_if_needed(&self) -> Result<PathBuf> {
        ...
        let tar_output = inazuma_util::command::new_command("tar")
            .arg("-xzf")
            .arg(&tar_gz_path)
            .arg("-C")
            .arg(&tar_out_dir)
