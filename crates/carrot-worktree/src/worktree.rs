...
use inazuma_collections::{HashMap, HashSet, VecDeque};
use inazuma_fuzzy::CharBag;

use carrot_paths::local_settings_folder_name;
use carrot_rpc::{
    AnyProtoClient,
    proto::{self, split_worktree_update},
};
pub use inazuma_settings_framework::WorktreeId;
use inazuma_settings_framework::{Settings, SettingsLocation, SettingsStore};
use inazuma_sum_tree::{
    Bias, Dimensions, Edit, KeyedItem, SeekTarget, SumTree, Summary, TreeMap, TreeSet,
};
use inazuma_text::{LineEnding, Rope};
use inazuma_util::{
...
///
/// Has an absolute path, and may be set to be visible in Carrot UI or not.
...
/// * a directory opened in Carrot — may be added as a visible entry to the current worktree
///
...
pub struct LocalSnapshot {
    ...
    root_file_handle: Option<Arc<dyn carrot_fs::FileHandle>>,
}
...
impl inazuma_sum_tree::Item for LocalRepositoryEntry {
    type Summary = PathSummary<inazuma_sum_tree::NoSummary>;

    fn summary(&self, _: <Self::Summary as Summary>::Context<'_>) -> Self::Summary {
        PathSummary {
            max_path: self.work_directory.path_key().0,
            item_summary: inazuma_sum_tree::NoSummary,
        }
    }
}
    ...
    pub fn check_invariants(&self, git_state: bool) {
        ...
        assert_eq!(
            ...
            self.entries_by_id
                .cursor::<()>(())
                .map(|e| (&e.path, e.id))
                .collect::<inazuma_collections::BTreeSet<_>>()
                .into_iter()
                .collect::<Vec<_>>(),
...
impl File {
    ...
    pub fn from_dyn(file: Option<&Arc<dyn carrot_language::File>>) -> Option<&Self> {
        file.and_then(|f| {
            let f: &dyn carrot_language::File = f.borrow();
            let f: &dyn Any = f;
            f.downcast_ref()
        })
    }
...
impl<'a, S: Summary> inazuma_sum_tree::Dimension<'a, PathSummary<S>> for PathProgress<'a> {
...
impl<'a> inazuma_sum_tree::Dimension<'a, PathSummary<GitSummary>> for GitSummary {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a>
    inazuma_sum_tree::SeekTarget<
        'a,
        PathSummary<GitSummary>,
        Dimensions<TraversalProgress<'a>, GitSummary>,
    > for PathTarget<'_>
{
    fn cmp(
        &self,
...
impl<'a, S: Summary> inazuma_sum_tree::Dimension<'a, PathSummary<S>> for PathKey {
...
impl<'a, S: Summary> inazuma_sum_tree::Dimension<'a, PathSummary<S>> for TraversalProgress<'a> {
...
impl inazuma_sum_tree::Item for Entry {
...
impl inazuma_sum_tree::KeyedItem for Entry {
    type Key = PathKey;
...
impl inazuma_sum_tree::ContextLessSummary for EntrySummary {
    fn zero() -> Self {
        Default::default()
    }
...
impl inazuma_sum_tree::Item for PathEntry {
...
impl inazuma_sum_tree::KeyedItem for PathEntry {
...
impl inazuma_sum_tree::ContextLessSummary for PathEntrySummary {
    fn zero() -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, PathEntrySummary> for ProjectEntryId {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, EntrySummary> for PathKey {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl BackgroundScanner {
    async fn run(&mut self, mut fs_events_rx: Pin<Box<dyn Send + Stream<Item = Vec<PathEvent>>>>) {
        ...
        let global_gitignore_file = carrot_paths::global_gitignore_path();
        ...
        if let Some(abs_path) = containing_git_repository {
            self.process_events(vec![PathEvent {
                path: abs_path,
                kind: Some(carrot_fs::PathEventKind::Changed),
            }])
            .await;
        }
    ...
    async fn process_events(&self, mut events: Vec<PathEvent>) {
        ...
        // Certain directories may have FS changes, but do not lead to git data changes that Carrot cares about.
        // Ignore these, to avoid Carrot unnecessarily rescanning git metadata.
        ...
        events.dedup_by(|left, right| {
            if left.path == right.path {
                if matches!(left.kind, Some(carrot_fs::PathEventKind::Rescan)) {
        ...
        });
        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                relative_paths.push(EventRoot {
                    ...
                    was_rescanned: matches!(event.kind, Some(carrot_fs::PathEventKind::Rescan)),
                });
            }
...
async fn discover_ancestor_git_repo(
    ...
    for (index, ancestor) in root_abs_path.as_path().ancestors().enumerate() {
        if index != 0 {
            if ancestor == carrot_paths::home_dir() {
...
pub trait WorktreeModelHandle {
    ...
    fn flush_fs_events<'a>(
        &self,
        cx: &'a mut inazuma::TestAppContext,
...
impl<'a> inazuma_sum_tree::Dimension<'a, EntrySummary> for TraversalProgress<'a> {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
pub struct Traversal<'a> {
    snapshot: &'a Snapshot,
    cursor: inazuma_sum_tree::Cursor<'a, 'static, Entry, TraversalProgress<'a>>,
    include_ignored: bool,
    include_files: bool,
    include_dirs: bool,
}
...
impl<'a> SeekTarget<'a, PathSummary<inazuma_sum_tree::NoSummary>, TraversalProgress<'a>>
...
impl carrot_fs::Watcher for NullWatcher {
