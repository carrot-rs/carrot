...
use anyhow::Result;
use carrot_fs::{FakeFs, Fs, RealFs, RemoveOptions};
use carrot_git::{DOT_GIT, GITIGNORE, REPO_EXCLUDE};
use carrot_worktree::{Entry, EntryKind, Event, PathChange, Worktree, WorktreeModelHandle};
use encoding_rs;
use inazuma::{
    AppContext as _, BackgroundExecutor, BorrowAppContext, Context, Task, TestAppContext,
};
...
use inazuma_settings_framework::{SettingsStore, WorktreeId};
use inazuma_util::{
    ResultExt, path,
    paths::PathStyle,
...
async fn test_root_rescan_reconciles_stale_state(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root", Some(carrot_fs::PathEventKind::Rescan));
...
async fn test_subtree_rescan_reports_unchanged_descendants_as_updated(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root/dir", Some(carrot_fs::PathEventKind::Rescan));
    tree.flush_fs_events(cx).await;
...
async fn test_write_file(cx: &mut TestAppContext) {
    ...
    carrot_fs::fs_watcher::global(|_| {}).unwrap();
...
async fn test_fs_event_for_gitignored_dir_does_not_lose_contents(cx: &mut TestAppContext) {
    ...
    fs.emit_fs_event("/root/ignored_dir", Some(carrot_fs::PathEventKind::Changed));
    tree.flush_fs_events(cx).await;
...
#[inazuma::test]
async fn test_write_file_encoding(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    for (i, case) in cases.into_iter().enumerate() {
        ...
        let text = inazuma_text::Rope::from(case.text);
