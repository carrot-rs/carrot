...
use carrot_language::{
    BufferSnapshot, CodeLabelBuilder, HighlightId, LineEnding, LspAdapterDelegate,
};
use carrot_project::{PathMatchCandidateSet, Project};
use carrot_ui::prelude::*;
use carrot_workspace::Workspace;
use carrot_worktree::ChildEntriesOptions;
use futures::Stream;
use futures::channel::mpsc;
use inazuma::{App, Entity, Task, WeakEntity};
use inazuma_fuzzy::PathMatch;
use inazuma_util::{ResultExt, rel_path::RelPath};
            ...
            let candidate_sets = worktrees
                ...
                .map(|worktree| {
                    ...
                    PathMatchCandidateSet {
                        ...
                        candidates: carrot_project::Candidates::Entries,
                    }
                })
                .collect::<Vec<_>>();
...
fn collect_files(
    ...
    let Ok(matchers) = glob_inputs
        .iter()
        .map(|glob_input| {
            inazuma_util::paths::PathMatcher::new(
                &[glob_input.to_owned()],
                project.read(cx).path_style(cx),
            )
            .with_context(|| format!("invalid path {glob_input}"))
        })
        .collect::<anyhow::Result<Vec<inazuma_util::paths::PathMatcher>>>()
    else {
...
#[cfg(test)]
mod test {
    use carrot_assistant_slash_command::SlashCommandOutput;
    ...
    async fn test_file_sub_directory_rendering(cx: &mut TestAppContext) {
        ...
        let project = Project::test(fs, [path!("/carrot").as_ref()], cx).await;

        let result = cx
            .update(|cx| collect_files(project.clone(), &["carrot/assets/themes".to_string()], cx));
        let result = SlashCommandOutput::from_event_stream(result.boxed())
            .await
            .unwrap();

        // Sanity check
        assert!(result.text.starts_with(path!("carrot/assets/themes\n")));
        assert_eq!(result.sections.len(), 7);
        ...
        assert!(
            result
                .text
                .contains(path!("carrot/assets/themes/andromeda/LICENSE"))
        );
        assert!(
            result
                .text
                .contains(path!("carrot/assets/themes/ayu/LICENSE"))
        );
        assert!(
            result
                .text
                .contains(path!("carrot/assets/themes/summercamp/LICENSE"))
        );
    ...
    async fn test_file_deep_sub_directory_rendering(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            result.sections[0].label,
            path!("carrot/assets/themes/LICENSE")
        );
        assert_eq!(
        ...
        assert_eq!(
            result.sections[2].label,
            path!("carrot/assets/themes/summercamp/subdir/LICENSE")
        );
        assert_eq!(
            result.sections[3].label,
            path!("carrot/assets/themes/summercamp/subdir/subsubdir/LICENSE")
        );
        assert_eq!(result.sections[4].label, "subsubdir");
        assert_eq!(result.sections[5].label, "subdir");
        assert_eq!(result.sections[6].label, "summercamp");
        assert_eq!(result.sections[7].label, path!("carrot/assets/themes"));

        assert_eq!(
            result.text,
            path!(
                "carrot/assets/themes\n```carrot/assets/themes/LICENSE\n1\n```\n\nsummercamp\n```carrot/assets/themes/summercamp/LICENSE\n1\n```\n\nsubdir\n```carrot/assets/themes/summercamp/subdir/LICENSE\n1\n```\n\nsubsubdir\n```carrot/assets/themes/summercamp/subdir/subsubdir/LICENSE\n3\n```\n\n"
            )
        );
