...
use carrot_language::{
    BufferSnapshot, CodeLabel, CodeLabelBuilder, HighlightId, LspAdapterDelegate,
};
use carrot_ui::{ActiveTheme, App, Window, prelude::*};
            ...
            let matched_items = if strict_match {
                ...
                let match_candidates = open_buffers
                    ...
                    .filter_map(|(id, (full_path, ..))| {
                        Some(inazuma_fuzzy::StringMatchCandidate::new(
                            id,
                            full_path.as_ref()?,
                        ))
                    })
