...
pub use carrot_paths::database_dir;
pub use carrot_sqlez as sqlez;
pub use carrot_sqlez_macros as sqlez_macros;
pub use inazuma;
use inazuma::{App, AppContext, Global};
pub use indoc::indoc;
pub use inventory;
pub use smol;
pub use uuid;
...
pub use carrot_release_channel::RELEASE_CHANNEL;
use carrot_sqlez::domain::Migrator;
use carrot_sqlez::thread_safe_connection::ThreadSafeConnection;
...
macro_rules! static_connection {
    ($t:ident, [ $($d:ty),* ]) => {
        ...
        impl $t {
            ...
            pub fn global(cx: &$crate::inazuma::App) -> Self {
...
mod tests {
    ...
    use carrot_sqlez::domain::Domain;
    use carrot_sqlez_macros::sql;
    ...
    async fn test_bad_migration_panics() {
        ...
        let _bad_db = open_db::<BadDB>(
            tempdir.path(),
            carrot_release_channel::ReleaseChannel::Dev.dev_name(),
        )
        .await;
    }
    ...
    #[inazuma::test]
    async fn test_db_corruption(cx: &mut inazuma::TestAppContext) {
    ...
    async fn test_simultaneous_db_corruption(cx: &mut inazuma::TestAppContext) {
        cx.executor().allow_parking();
