...
use carrot_client::ProxySettings;
use carrot_extension::ExtensionHostProxy;
use carrot_fs::{Fs, RealFs};
use carrot_git::GitHostingProviderRegistry;
use carrot_http_client::{Url, read_proxy_from_env};
use carrot_language::LanguageRegistry;
use carrot_net::async_net::{UnixListener, UnixStream};
use carrot_node_runtime::{NodeBinaryOptions, NodeRuntime};
use carrot_paths::logs_dir;
use carrot_project::{project_settings::ProjectSettings, trusted_worktrees};
use carrot_proto::CrashReport;
use carrot_release_channel::{AppCommitSha, AppVersion, RELEASE_CHANNEL, ReleaseChannel};
use carrot_remote::{
    RemoteClient,
    json_log::LogRecord,
...
use carrot_rpc::proto::{self, Envelope, REMOTE_SERVER_PROJECT_ID};
use carrot_rpc::{AnyProtoClient, TypedEnvelope};
use clap::Subcommand;
use futures::{
...
use inazuma::{App, AppContext as _, Context, Entity, UpdateGlobal as _};
use inazuma_collections::HashMap;
use inazuma_settings_framework::{Settings, SettingsStore, watch_config_file};
use inazuma_tokio::Tokio;
use inazuma_util::{ResultExt, command::new_command};
use smol::{
...
pub fn run(command: Commands) -> anyhow::Result<()> {
    use anyhow::Context;
    use carrot_release_channel::{RELEASE_CHANNEL, ReleaseChannel};
        ...
        Commands::Version => {
            ...
            match release_channel {
                ReleaseChannel::Stable | ReleaseChannel::Preview => {
                    println!("{}", env!("CARROT_PKG_VERSION"))
                }
                ReleaseChannel::Nightly | ReleaseChannel::Dev => {
                    let commit_sha =
                        option_env!("CARROT_COMMIT_SHA").unwrap_or(release_channel.dev_name());
                    let build_id = option_env!("CARROT_BUILD_ID");
...
pub static VERSION: LazyLock<String> = LazyLock::new(|| match *RELEASE_CHANNEL {
    ReleaseChannel::Stable | ReleaseChannel::Preview => env!("CARROT_PKG_VERSION").to_owned(),
    ReleaseChannel::Nightly | ReleaseChannel::Dev => {
        let commit_sha = option_env!("CARROT_COMMIT_SHA").unwrap_or("missing-carrot-commit-sha");
        let build_identifier = option_env!("CARROT_BUILD_ID");
...
fn handle_crash_files_requests(project: &Entity<HeadlessProject>, client: &AnyProtoClient) {
    client.add_request_handler(
        ...
        |_, _: TypedEnvelope<proto::GetCrashFiles>, _cx| async move {
            ...
            let mut children = smol::fs::read_dir(carrot_paths::logs_dir()).await?;
            while let Some(child) = children.next().await {
                ...
                if extension == Some(OsStr::new("panic")) {
                    ...
                    if !filename.starts_with("carrot") {
                        continue;
                    }
...
fn init_paths() -> anyhow::Result<()> {
    for path in [
        carrot_paths::config_dir(),
        carrot_paths::extensions_dir(),
        carrot_paths::languages_dir(),
        carrot_paths::logs_dir(),
        carrot_paths::temp_dir(),
        carrot_paths::hang_traces_dir(),
        carrot_paths::remote_extensions_dir(),
        carrot_paths::remote_extensions_uploads_dir(),
    ]
    .iter()
    {
...
pub fn execute_run(
    ...
    carrot_crashes::init(
        carrot_crashes::InitCrashHandler {
            session_id: id,
            carrot_version: VERSION.to_owned(),
            binary: "carrot-remote-server".to_string(),
            release_channel: carrot_release_channel::RELEASE_CHANNEL_NAME.clone(),
            commit_sha: option_env!("CARROT_COMMIT_SHA")
                .unwrap_or("no_sha")
                .to_owned(),
        },
        |task| {
    ...
    let run = move |cx: &mut _| {
        ...
        let app_commit_sha =
            option_env!("CARROT_COMMIT_SHA").map(|s| AppCommitSha::new(s.to_owned()));
        let app_version = AppVersion::load(
            env!("CARROT_PKG_VERSION"),
            option_env!("CARROT_BUILD_ID"),
            app_commit_sha,
        );
        ...
        carrot_extension::init(cx);
        let extension_host_proxy = ExtensionHostProxy::global(cx);

        carrot_json_schema_store::init(cx);
        ...
        let project = cx.new(|cx| {
            ...
            languages.set_language_server_download_dir(carrot_paths::languages_dir().clone());
...
impl ServerPaths {
    fn new(identifier: &str) -> Result<Self, ServerPathError> {
        let server_dir = carrot_paths::remote_server_state_dir().join(identifier);
...
where
    ...
    W: AsyncWrite + Unpin,
{
    use carrot_remote::protocol::{read_message_raw, write_size_prefixed_buffer};
...
fn initialize_settings(
    ...
    fs: Arc<dyn Fs>,
    cx: &mut App,
) -> carrot_watch::Receiver<Option<NodeBinaryOptions>> {
    let (user_settings_file_rx, watcher_task) = watch_config_file(
    ...
    let (mut tx, rx) = carrot_watch::channel(None);
...
fn cleanup_old_binaries() -> Result<()> {
    let server_dir = carrot_paths::remote_server_dir_relative();
    let release_channel = carrot_release_channel::RELEASE_CHANNEL.dev_name();
    let prefix = format!("carrot-remote-server-{}-", release_channel);
...
fn cleanup_old_binaries_wsl() {
    let server_dir = carrot_paths::remote_wsl_server_dir_relative();
...
fn is_new_version(version: &str) -> bool {
    semver::Version::from_str(version)
        .ok()
        .zip(semver::Version::from_str(env!("CARROT_PKG_VERSION")).ok())
