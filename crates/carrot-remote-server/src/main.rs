use carrot_remote_server::Commands;
use clap::Parser;
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(socket_path) = &cli.askpass {
        carrot_askpass::main(socket_path);
        return Ok(());
    }

    if let Some(socket) = &cli.crash_handler {
        carrot_crashes::crash_server(socket.as_path());
        return Ok(());
    }
    ...
    if let Some(command) = cli.command {
        use carrot_remote_server::ExecuteProxyError;

        let res = carrot_remote_server::run(command);
        if let Err(e) = &res
