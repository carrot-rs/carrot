...
use carrot_extension_host::headless_host::HeadlessExtensionStore;
use carrot_fs::Fs;
use carrot_http_client::HttpClient;
use carrot_language::{Buffer, BufferEvent, LanguageRegistry, proto::serialize_operation};
...
use inazuma::{App, AppContext as _, AsyncApp, Context, Entity, PromptLevel};
use smol::process::Child;

use carrot_worktree::Worktree;
use inazuma_settings_framework::initial_server_settings_content;
...
pub struct HeadlessProject {
    ...
    pub profiling_collector: inazuma::ProfilingCollector,
...
impl HeadlessProject {
    ...
    pub fn new(
        ...
        carrot_debug_adapter_extension::init(proxy.clone(), cx);
        ...
        if init_worktree_trust {
            carrot_project::trusted_worktrees::track_worktree_trust(
        ...
        carrot_language_extension::init(
            carrot_language_extension::LspAccess::ViaLspStore(lsp_store.clone()),
            proxy.clone(),
            languages.clone(),
        );
        ...
        let extensions = HeadlessExtensionStore::new(
            fs.clone(),
            http_client.clone(),
            carrot_paths::remote_extensions_dir().to_path_buf(),
            proxy,
            node_runtime,
            cx,
        );
        ...
        HeadlessProject {
            ...
            git_store,
            environment,
            profiling_collector: inazuma::ProfilingCollector::new(startup_time),
    ...
    pub async fn handle_add_worktree(
        ...
        use carrot_client::ErrorCodeExt;
    ...
    pub async fn handle_open_image_by_path(
        ...
        use carrot_proto::create_image_for_peer::Variant;
    ...
    pub async fn handle_download_file_by_path(
        ...
        use carrot_proto::create_file_for_peer::Variant;
    ...
    async fn handle_open_server_settings(
        ...
        let settings_path = carrot_paths::settings_file();
    ...
    async fn handle_find_search_candidates(
        ...
        let task = cx.spawn(async move |cx| {
            ...
            let (batcher, batches) =
                carrot_project::project_search::AdaptiveBatcher::new(cx.background_executor());
    ...
    async fn handle_get_directory_environment(
        ...
        let shell =
            carrot_task::shell_from_proto(envelope.payload.shell.context("missing shell")?)?;
...
fn prompt_to_proto(
    prompt: &carrot_project::LanguageServerPromptRequest,
