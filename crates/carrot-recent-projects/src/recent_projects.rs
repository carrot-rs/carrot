...
pub use carrot_remote_connection::{RemoteConnectionModal, connect};
...
use carrot_project::{Worktree, git_store::Repository};
use carrot_ui_input::ErasedEditor;
use inazuma_picker::{
...
use carrot_actions::{OpenDevContainer, OpenRecent, OpenRemote};
use carrot_dev_container::{DevContainerContext, find_devcontainer_configs};
use carrot_ui::{
...
use carrot_workspace::{
    HistoryManager, ModalView, OpenOptions, OpenVisible, PathList, SerializedWorkspaceLocation,
...
pub fn init(cx: &mut App) {
    #[cfg(target_os = "windows")]
    cx.on_action(|open_wsl: &carrot_actions::wsl_actions::OpenFolderInWsl, cx| {
        let create_new_window = open_wsl.create_new_window;
        carrot_shell::with_active_workspace(cx, move |workspace, window, cx| {
            ...
            let window_handle = window.window_handle().downcast::<carrot_shell::AppShell>();
            ...
            cx.spawn_in(window, async move |workspace, cx| {
                ...
                if let Some(inazuma_util::paths::WslPath { distro, path }) = wsl_path {
                    use carrot_remote::WslConnectionOptions;
                    ...
                    let open_options = carrot_shell::OpenOptions {
                        replace_window,
                        ..Default::default()
                    };
                ...
                if paths.is_empty() {
                    let message = indoc::indoc! { r#"
                        ...
                        Please note that Carrot currently does not support opening network share folders inside wsl.
                    "#};
    ...
    #[cfg(target_os = "windows")]
    cx.on_action(|open_wsl: &carrot_actions::wsl_actions::OpenWsl, cx| {
        let create_new_window = open_wsl.create_new_window;
        carrot_shell::with_active_workspace(cx, move |workspace, window, cx| {
    ...
    #[cfg(target_os = "windows")]
    cx.on_action(|open_wsl: &carrot_remote::OpenWslPath, cx| {
        let open_wsl = open_wsl.clone();
        carrot_shell::with_active_workspace(cx, move |workspace, window, cx| {
            let fs = workspace.project().read(cx).fs().clone();
            add_wsl_distro(fs, &open_wsl.distro, cx);
            let open_options = carrot_shell::OpenOptions {
                replace_window: window.window_handle().downcast::<carrot_shell::AppShell>(),
                ..Default::default()
            };
        ...
        |workspace: &mut Workspace, window: Option<&mut Window>, cx: &mut Context<Workspace>| {
            ...
            cx.subscribe_in(
                ...
                move |_, project, event, window, cx| {
                    if let carrot_project::Event::WorktreeUpdatedEntries(
                        worktree_id,
                        updated_entries,
                    ) = event
                    {
...
pub fn add_wsl_distro(
    fs: Arc<dyn carrot_project::Fs>,
    connection_options: &carrot_remote::WslConnectionOptions,
    cx: &App,
) {
    ...
    fn new(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let workspaces = db
                ...
                .unwrap_or_default();
            let workspaces =
                carrot_workspace::resolve_worktree_workspaces(workspaces, fs.as_ref()).await;
                    ...
                    match candidate_workspace_location {
                        ...
                        SerializedWorkspaceLocation::Remote(mut connection) => {
                            ...
                            let replace_window = if replace_current_window {
                                window.window_handle().downcast::<carrot_shell::AppShell>()
                            } else {
                                None
                            };
                            let open_options = carrot_shell::OpenOptions {
        ...
        if popover_style {
            return Some(
                v_flex()
                    ...
                    .border_color(cx.theme().colors().border_variant)
                    .child({
                        let open_action = carrot_workspace::Open {
                    ...
                    if is_already_open_entry {
                        ...
                        this.child(
                            Button::new("open_new_window", "New Window")
                                ...
                                .on_click(|_, window, cx| {
                                    window.dispatch_action(
                                        inazuma_menu::SecondaryConfirm.boxed_clone(),
                                        cx,
                                    )
                                }),
                        )
                                ...
                                Some(ContextMenu::build(window, cx, {
                                    ...
                                    move |menu, _, _| {
                                        menu.context(focus_handle)
                                            .action(
                                                "Open Local Project",
                                                carrot_workspace::Open { create_new_window }
                                                    .boxed_clone(),
                                            )
                                            .action(
                                                "Open Remote Project",
            ...
            self.filtered_entries.get(ix)
        {
            ...
            cx.spawn_in(window, async move |this, cx| {
                ...
                this.update_in(cx, move |picker, window, cx| {
                    ...
                    // But we do not emit a update event when user opens a project, because it's handled in `carrot_workspace::load_workspace`.
...
mod tests {
    ...
    async fn test_open_dev_container_action_with_single_config(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            carrot_shell::open_paths(
                &[PathBuf::from(path!("/project"))],
                app_state,
                carrot_shell::OpenOptions::default(),
                cx,
            )
        })
        .await
        ...
        shell
            .update(cx, |_shell, window, cx| {
                ...
                let modal = workspace.read(cx).active_modal::<RemoteServerProjects>(cx);
                assert!(
                    modal.is_some(),
