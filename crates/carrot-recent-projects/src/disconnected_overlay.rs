...
use carrot_ui::{ElevationIndex, Modal, ModalFooter, ModalHeader, Section, prelude::*};
use carrot_workspace::{ModalView, Workspace, notifications::DetachAndPromptErr};
use inazuma::{ClickEvent, DismissEvent, EventEmitter, FocusHandle, Focusable, Render, WeakEntity};
...
impl Focusable for DisconnectedOverlay {
    fn focus_handle(&self, _cx: &inazuma::App) -> inazuma::FocusHandle {
...
impl ModalView for DisconnectedOverlay {
    fn on_before_dismiss(
        ...
        carrot_workspace::DismissDecision::Dismiss(self.finished)
    }
...
impl DisconnectedOverlay {
    ...
    fn reconnect_to_remote_project(
        ...
        let Some(window_handle) = window.window_handle().downcast::<carrot_shell::AppShell>()
        else {
            return;
        };
