...
use carrot_dev_container::{
    DevContainerConfig, DevContainerContext, find_devcontainer_configs,
    start_dev_container_with_config,
};
use carrot_editor::Editor;

use carrot_open_path_prompt::OpenPathDelegate;
use carrot_paths::{global_ssh_config_file, user_ssh_config_file};
    ...
    fn render_match(
        ...
        Some(
            ListItem::new(SharedString::from(format!("li-devcontainer-config-{}", ix)))
                .inset(true)
                .spacing(carrot_ui::ListItemSpacing::Sparse)
...
impl ProjectPicker {
    fn new(
        ...
        let lister = carrot_project::DirectoryLister::Project(project.clone());
        let delegate =
            carrot_open_path_prompt::OpenPathDelegate::new(tx, lister, false, cx).show_hidden();
                    ...
                    let window = cx
                        .open_window(options, |window, cx| {
                            let workspace = cx.new(|cx| {
                                carrot_telemetry::event!("SSH Project Created");
...
impl inazuma::Render for ProjectPicker {
...
impl RemoteServerProjects {
    ...
    fn project_picker(
        create_new_window: bool,
        index: ServerIndex,
        connection_options: carrot_remote::RemoteConnectionOptions,
        project: Entity<Project>,
        home_dir: RemotePathBuf,
        window: &mut Window,
        cx: &mut Context<Self>,
            ...
            match connection.await {
                Some(Some(client)) => this
                    .update_in(cx, |this, window, cx| {
                        ...
                        carrot_telemetry::event!("SSH Server Created");
        ...
        let creating = cx.spawn_in(window, async move |this, cx| {
            match connection.await {
                Some(Some(client)) => this.update_in(cx, |this, window, cx| {
                    carrot_telemetry::event!("WSL Distro Added");
                    this.retained_connections.push(client);
                    let Some(fs) = this
                        .workspace
                        .read_with(cx, |workspace, cx| {
    ...
    fn render_remote_project(
        ...
        let create_new_window = self.create_new_window;
        let is_from_carrot = server.is_from_carrot();
        ...
        let callback = Rc::new({
            ...
            move |remote_server_projects: &mut Self,
                ...
                let replace_window = match (create_new_window, secondary_confirm) {
                    ...
                    (true, true) | (false, false) => window.window_handle().downcast::<AppShell>(),
                };
        ...
        div()
            ...
            .child(
                ListItem::new((element_id_base, ix))
                    ...
                    .when(is_from_carrot, |server_list_item| {
    ...
    fn add_ssh_server(
        &mut self,
        connection_options: carrot_remote::SshConnectionOptions,
        cx: &mut Context<Self>,
    ) {
        self.update_settings_file(cx, move |setting, _| {
            setting
                    ...
                    div()
                        ...
                        .child(
                            div()
                                ...
                                .on_action(cx.listener(
                                    |_, _: &inazuma_menu::Confirm, window, cx| {
                                ...
                                .child(
                                    ListItem::new("li-devcontainer-see-log")
                                        ...
                                        .child(Label::new("Open Carrot Log"))
                        ...
                        .child(
                            div()
                                ...
                                .on_action(cx.listener(
                                    |this, _: &inazuma_menu::Confirm, window, cx| {
                                        this.cancel(&inazuma_menu::Cancel, window, cx);
                                        cx.notify();
                                    },
                                ))
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("learn-more", "Learn More")
                                            ...
                                            .on_click(|_, _, cx| {
                                                cx.open_url(
                                                    "https://carrot.dev/docs/remote-development",
                                                );
                                            }),
                                    ),
                            )
