...
use carrot_askpass::EncryptedPassword;
use carrot_editor::Editor;
use carrot_extension_host::ExtensionStore;
use futures::{FutureExt as _, channel::oneshot, select};
use inazuma::{AnyWindowHandle, AppContext, AsyncApp, PromptLevel};

use carrot_project::trusted_worktrees;
use carrot_remote::{
    DockerConnectionOptions, Interactive, RemoteConnection, RemoteConnectionOptions,
    SshConnectionOptions,
};
use carrot_shell::{AppShell, OpenOptions, find_existing_workspace};
use carrot_workspace::{AppState, SerializedWorkspaceLocation, Workspace};
pub use inazuma_settings_framework::SshConnection;
use inazuma_settings_framework::{
    DevContainerConnection, ExtendingVec, RegisterSetting, Settings, WslConnection,
};
use inazuma_util::paths::PathWithPosition;

pub use carrot_remote_connection::{
...
pub async fn open_remote_project(
    ...
    if let Some((existing_window, existing_workspace)) = existing {
        ...
        if let Some(remote_connection) = remote_connection {
            ...
            _ = existing_window.update(cx, |_shell, _, cx| {
                existing_workspace.update(cx, |workspace, cx| {
            ...
            let items = open_results
                ...
                .collect::<Vec<_>>();
            navigate_to_positions(&existing_window.into(), items, &paths_with_positions, cx);
    ...
    let (window, initial_workspace) = if let Some(window) = open_options.replace_window {
        let workspace = window.update(cx, |_shell: &mut AppShell, window, cx| {
            Workspace::for_window(window, cx)
                .expect("Workspace should be registered for replace_window")
        })?;
        (window, workspace)
    } else {
        let workspace_position = cx
            .update(|cx| {
                carrot_workspace::remote_workspace_position_from_db(
        ...
        let workspace = window.update(cx, |_shell: &mut AppShell, window, cx| {
            Workspace::for_window(window, cx)
                .expect("Workspace should be registered after AppShell creation")
        })?;
        (window, workspace)
    };
    ...
    loop {
        ...
        let delegate = window.update(cx, {
            ...
            move |_shell: &mut AppShell, window, cx| {
                window.activate_window();
        ...
        let connection = carrot_remote::connect(connection_options.clone(), delegate.clone(), cx);
        ...
        let opened_items = cx
            .update(|cx| {
                carrot_workspace::open_remote_project_with_new_connection(
                    window.into(),
                    remote_connection,
                    cancel_rx,
                    delegate.clone(),
        ...
        match opened_items {
            ...
            Ok(items) => {
                navigate_to_positions(&window.into(), items, &paths_with_positions, cx);
            }
        }
    ...
    // Register the remote client with extensions.
    // The active workspace is now the one with the remote project.
    window
        .update(cx, |_shell: &mut AppShell, window, cx| {
...
pub fn navigate_to_positions(
    ...
    items: impl IntoIterator<Item = Option<Box<dyn carrot_workspace::item::ItemHandle>>>,
...
mod tests {
    ...
    use carrot_http_client::BlockedHttpClient;
    use carrot_node_runtime::NodeRuntime;
    use carrot_remote::RemoteClient;
    use carrot_remote_server::{HeadlessAppState, HeadlessProject};
    use carrot_shell::find_existing_workspace;
    use inazuma::{AppContext, TestAppContext};
    ...
    async fn test_open_remote_project_with_mock_connection(
        ...
        let languages = Arc::new(carrot_language::LanguageRegistry::new(server_cx.executor()));
        ...
        let shell_handle = cx.update(|cx| cx.windows()[0].downcast::<AppShell>().unwrap());
    ...
    async fn test_reuse_existing_remote_workspace_window(
        ...
        let first_window = cx.update(|cx| cx.windows()[0].downcast::<AppShell>().unwrap());
        ...
        let still_first_window = cx.update(|cx| cx.windows()[0].downcast::<AppShell>().unwrap());
        assert_eq!(
    ...
    async fn test_reconnect_when_server_not_running(
        ...
        let window = cx.update(|cx| cx.windows()[0].downcast::<AppShell>().unwrap());
