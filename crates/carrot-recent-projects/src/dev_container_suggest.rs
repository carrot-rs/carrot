...
use carrot_dev_container::find_configs_in_snapshot;
...
use carrot_worktree::UpdatedEntriesSet;
use inazuma::{SharedString, Window};
...
pub fn suggest_on_worktree_updated(
    ...
    project: &inazuma::Entity<Project>,
    window: &mut Window,
    ...
    cx.on_next_frame(window, move |workspace, _window, cx| {
        ...
        workspace.show_notification(notification_id, cx, |cx| {
            cx.new(move |cx| {
                ...
                .primary_on_click({
                    move |window, cx| {
                        window.dispatch_action(Box::new(carrot_actions::OpenDevContainer), cx);
                    }
                })
