...
use carrot_editor::{EditorSettings, items::entry_git_aware_label_color};
use carrot_file_icons::FileIcons;
use carrot_language::File as _;
use carrot_project::{ImageItem, Project, ProjectPath, image_store::ImageItemEvent};
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{Tooltip, prelude::*};
use carrot_workspace::{
    ...
    item::{
        HighlightedText, Item, ItemHandle, PaneRole, ProjectItem, SerializableItem,
        TabContentParams,
    },
};
    ...
    fn to_item_events(event: &Self::Event, f: &mut dyn FnMut(carrot_workspace::item::ItemEvent)) {
        match event {
            ImageViewEvent::TitleChanged => {
                f(carrot_workspace::item::ItemEvent::UpdateTab);
                f(carrot_workspace::item::ItemEvent::UpdateBreadcrumbs);
            }
        }
    }
            ...
            .child({
                ...
                let container = div()
                    ...
                    .cursor(if self.is_dragging() {
                        inazuma::CursorStyle::ClosedHand
                    } else {
                        inazuma::CursorStyle::OpenHand
                    })
...
pub struct ImageViewToolbarControls {
    ...
    _subscription: Option<inazuma::Subscription>,
}
...
pub fn init(cx: &mut App) {
    carrot_workspace::register_project_item::<ImageView>(cx);
    carrot_workspace::register_serializable_item::<ImageView>(cx);
}
...
mod persistence {
    ...
    carrot_db::static_connection!(ImageViewerDb, [WorkspaceDb]);
