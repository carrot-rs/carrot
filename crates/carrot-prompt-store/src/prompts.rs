...
use carrot_fs::Fs;
use carrot_language::{BufferSnapshot, LanguageName, Point};
use futures::StreamExt;
use handlebars::{Handlebars, RenderError};
use inazuma::{App, AppContext as _, AssetSource};
...
impl PromptBuilder {
    ...
    fn watch_fs_for_template_overrides(
        ...
        let templates_dir = carrot_paths::prompt_overrides_dir(params.repo_path.as_deref());
    ...
    pub fn generate_inline_transformation_prompt_tools(
        ...
        let truncated_before = if before_range.len() > MAX_CTX {
            is_truncated = true;
            let start = buffer.clip_offset(range.start - MAX_CTX, inazuma_text::Bias::Right);
            start..range.start
        } else {
            before_range
        };
        ...
        let truncated_after = if after_range.len() > MAX_CTX {
            is_truncated = true;
            let end = buffer.clip_offset(range.end + MAX_CTX, inazuma_text::Bias::Left);
            range.end..end
        } else {
            after_range
        };
