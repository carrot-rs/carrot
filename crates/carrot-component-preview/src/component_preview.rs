...
use carrot_ui::{
    ButtonLike, Divider, HighlightedLabel, ListItem, ListSubHeader, Tooltip, prelude::*,
};
use carrot_ui_input::InputField;
use carrot_workspace::AppState;
use carrot_workspace::{
    Item, ItemId, PaneRole, SerializableItem, Workspace, WorkspaceId, delete_unloaded_items,
    item::ItemEvent,
};
use inazuma::{App, Entity, EventEmitter, FocusHandle, Focusable, Task, WeakEntity, Window, list};
use inazuma::{ListState, ScrollHandle, ScrollStrategy, UniformListScrollHandle};
...
pub fn init(app_state: Arc<AppState>, cx: &mut App) {
    carrot_workspace::register_serializable_item::<ComponentPreview>(cx);
    ...
    cx.observe_new(move |workspace: &mut Workspace, _window, cx| {
        ...
        workspace.register_action(
            move |workspace, _: &carrot_workspace::OpenComponentPreview, window, cx| {
                let app_state = app_state.clone();
...
impl ComponentPreview {
    ...
    fn update_component_list(&mut self, cx: &mut Context<Self>) {
        ...
        self.component_list = ListState::new(new_len, inazuma::ListAlignment::Top, px(1500.0));
        ...
        if let Some(workspace) = self.workspace.upgrade() {
            workspace.update(cx, |workspace, cx| {
                let status_toast = StatusToast::new(
                    "`carrot/new-notification-system` created!",
                    cx,
                    |this, _cx| {
...
impl Item for ComponentPreview {
    ...
    fn clone_on_split(
        ...
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Option<inazuma::Entity<Self>>>
    where
        Self: Sized,
    {
