...
use carrot_http_client::{AsyncBody, HttpClient, Request};
...
/// The CIMD URL where Carrot's OAuth client metadata document is hosted.
pub const CIMD_URL: &str = "https://carrot.dev/oauth/client-metadata.json";
...
/// an attacker-controlled MCP server from directing Carrot to fetch internal
...
fn parse_auth_params(input: &str) -> inazuma_collections::HashMap<String, String> {
    let mut params = inazuma_collections::HashMap::default();
...
pub async fn perform_dcr(
    ...
    let request = Request::builder()
        .method(carrot_http_client::http::Method::POST)
            ...
            let (status_code, body) = match &result {
                Ok(_) => (
                    ...
                    "<html><body><h1>Authorization successful</h1>\
                     <p>You can close this tab and return to Carrot.</p></body></html>",
                ),
                Err(err) => {
                    ...
                    (
                        ...
                        "<html><body><h1>Authorization failed</h1>\
                         <p>Something went wrong. Please try again from Carrot.</p></body></html>",
                    )
                }
            };
...
async fn fetch_json<T: serde::de::DeserializeOwned>(
    ...
    let request = Request::builder()
        .method(carrot_http_client::http::Method::GET)
        .uri(url.as_str())
        .header("Accept", "application/json")
...
mod tests {
    use super::*;
    use carrot_http_client::Response;
    ...
    fn test_build_authorization_url() {
        ...
        let url = build_authorization_url(
            &metadata,
            "https://carrot.dev/oauth/client-metadata.json",
    ...
    fn test_dcr_registration_body_shape() {
        ...
        assert_eq!(body["client_name"], "Carrot");
    ...
    fn make_fake_http_client(
        handler: impl Fn(
            carrot_http_client::Request<AsyncBody>,
        ) -> std::pin::Pin<
        ...
        carrot_http_client::FakeHttpClient::create(handler) as Arc<dyn HttpClient>
    }
