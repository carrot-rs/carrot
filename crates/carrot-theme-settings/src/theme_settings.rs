...
///
/// Follows Carrot's pattern: `theme::init` + settings wiring.
...
pub fn init(themes_to_load: LoadThemes, cx: &mut App) {
    ...
    // 1. Initialize theme subsystems (Carrot: theme::init)
    ...
    // 2. Register ThemeSettingsProvider — without this, carrot-ui crates crash
    ...
    // 4. Load user themes from ~/.carrot/themes/
    if load_user_themes {
        let registry = ThemeRegistry::global(cx);
        let themes_dir = carrot_paths::themes_dir().clone();
            ...
            if let Ok(entries) = std::fs::read_dir(&themes_dir) {
                for entry in entries.flatten() {
                    ...
                    match std::fs::read_to_string(&theme_toml) {
                        Ok(content) => {
                            match carrot_theme::load_theme_from_toml_with_base_dir(
                                &id,
                                &content,
                                Some(path),
                            ) {
...
fn configured_theme(cx: &mut App) -> Arc<carrot_theme::Theme> {
    let registry = ThemeRegistry::global(cx);
...
/// Loads a user theme from TOML bytes into the registry.
///
/// The `path` is used to derive the theme id (file stem).
pub fn load_user_theme(
    ...
    let content = std::str::from_utf8(bytes).context("theme file is not valid UTF-8")?;
    let id = path
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("user-theme");
    let base_dir = path.parent().map(|p| p.to_path_buf());
    let theme = carrot_theme::load_theme_from_toml_with_base_dir(id, content, base_dir)
