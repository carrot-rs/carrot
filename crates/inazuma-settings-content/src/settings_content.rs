...
pub use agent::*;
pub use appearance::*;
use carrot_json_compat::parse_json_with_comments;
pub use chip::*;
pub use editor::*;
pub use extension::*;
...
use inazuma_collections::{HashMap, IndexMap};
...
pub use inazuma_util::serde::default_true;
...
pub struct SettingsContent {
    ...
    /// In Carrot, the terminal IS the app, so these are top-level settings.
    pub general: Option<GeneralSettingsContent>,

    /// Appearance settings — font, cursor, contrast, colorspace, symbol maps.
    /// In Carrot, the terminal font is THE font, not a secondary override.
    ...
    /// Configuration of audio in Carrot.
    ...
    /// This base keymap settings adjusts the default keybindings in Carrot to be similar
    /// to other common code editors. By default, Carrot's keymap closely follows VSCode's
    ...
    pub vertical_tabs: Option<VerticalTabsSettingsContent>,
    ...
    /// The URL of the Carrot server to connect to.
    pub server_url: Option<String>,

    /// Configuration for session-related features
    pub session: Option<SessionSettingsContent>,
    /// Control what info is collected by Carrot.
    pub telemetry: Option<TelemetrySettingsContent>,

    /// Configuration of the terminal in Carrot.
    ...
    // Settings related to calls in Carrot
    pub calls: Option<CallSettingsContent>,

    /// Settings for the which-key popup.
    pub which_key: Option<WhichKeySettingsContent>,

    /// Settings related to Vim mode in Carrot.
...
pub trait RootUserSettings: Sized + DeserializeOwned {
    ...
    fn parse_toml(toml_str: &str) -> (Option<Self>, ParseStatus);
}
...
impl RootUserSettings for SettingsContent {
    ...
    fn parse_toml(toml_str: &str) -> (Option<Self>, ParseStatus) {
        fallible_options::parse_toml(toml_str)
    }
}
...
pub struct AudioSettingsContent {
    ...
    /// Microphones are too quiet in Carrot, until everyone is on experimental
...
pub struct TelemetrySettingsContent {
    ...
    /// Send anonymized usage data like what languages you're using Carrot with.
    ///
    /// Default: true
...
pub struct DebuggerSettingsContent {
    ...
    /// Whether the breakpoints should be reused across Carrot sessions.
    ///
    /// Default: true
    ...
    /// Whether to log messages between active debug adapters and Carrot
    ///
    /// Default: true
...
/// Configuration of voice calls in Carrot.
#[with_fallible_options]
#[derive(Clone, PartialEq, Default, Serialize, Deserialize, JsonSchema, MergeFrom, Debug)]
...
pub struct FileFinderSettingsContent {
    ...
    /// Only the file Carrot had indexed will be used, not necessary all the gitignored files.
    ///
    /// Default: Smart
...
pub enum IncludeIgnoredContent {
    ...
    All,
    /// Use only the files Carrot had indexed
    Indexed,
...
pub struct SshConnection {
    ...
    // By default Carrot will download the binary to the host directly.
    // If this is set to true, Carrot will download the binary to your local machine,
...
/// Display density for vertical tab rows.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum VerticalTabsDensity {
    /// Single-line rows with optional subtitle.
    #[default]
    Compact,
    /// Multi-line rows with full metadata.
    Expanded,
}

/// View mode for the vertical tabs panel.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum VerticalTabsViewMode {
    /// Every split pane gets its own row under the tab header.
    #[default]
    Panes,
    /// Only the focused pane per session is shown.
    Tabs,
}

/// What to display as the main title of a pane row.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum PaneTitleSource {
    /// Last command, conversation title, or CLI agent session name.
    #[default]
    Command,
...
/// What to display as the subtitle in compact mode.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum AdditionalMetadata {
    ...
    Directory,
    /// Last command or conversation title.
    Command,
}

#[with_fallible_options]
#[derive(Clone, Default, Serialize, Deserialize, JsonSchema, MergeFrom, Debug, PartialEq)]
pub struct VerticalTabsSettingsContent {
    /// Whether to show the vertical tabs panel button in the status bar.
    ///
    /// Default: true
    pub button: Option<bool>,
    ...
    ///
    /// Default: compact
    pub density: Option<VerticalTabsDensity>,
    /// View mode: show all panes or only the focused pane per session.
    ///
    /// Default: panes
    pub view_mode: Option<VerticalTabsViewMode>,
    /// What to show as the main pane title.
    ///
    /// Default: command
    pub pane_title: Option<PaneTitleSource>,
    ...
    ///
    /// Default: true
    pub show_details_on_hover: Option<bool>,
}
