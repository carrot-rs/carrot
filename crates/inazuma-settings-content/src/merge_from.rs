...
///
/// When Carrot starts it loads settings from `default.json` to initialize
...
impl<K, V> MergeFrom for inazuma_collections::HashMap<K, V>
where
...
impl<K, V> MergeFrom for inazuma_collections::BTreeMap<K, V>
where
...
impl<K, V> MergeFrom for inazuma_collections::IndexMap<K, V>
where
...
impl<T> MergeFrom for inazuma_collections::BTreeSet<T>
where
    T: Clone + Ord,
{
    fn merge_from(&mut self, other: &Self) {
...
impl<T> MergeFrom for inazuma_collections::HashSet<T>
where
