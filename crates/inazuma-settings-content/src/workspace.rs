...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Carrot instance.
    ...
    /// When set to false, Carrot will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Carrot will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Carrot will always
    ...
    /// What draws window decorations/titlebar, the client application (Carrot) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// Carrot draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Carrot.
    #[default]
    LastSession,
