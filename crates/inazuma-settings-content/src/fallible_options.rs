...
pub fn parse_toml<T>(toml_str: &str) -> (Option<T>, ParseStatus)
where
    T: serde::de::DeserializeOwned,
{
    ...
    let value = match toml::from_str::<T>(toml_str) {
        Ok(value) => value,
        Err(error) => {
            return (
                None,
...
#[cfg(test)]
mod tests {
    use inazuma_settings_macros::with_fallible_options;
    use serde::Deserialize;
    ...
    fn test_fallible() {
        ...
        assert!(carrot_json_compat::parse_json_with_comments::<Foo>(&input).is_err());
