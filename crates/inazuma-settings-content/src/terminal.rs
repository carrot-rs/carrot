...
pub struct TerminalSettingsContent {
    ...
    /// TUI-awareness policy. Controls how aggressively the block system
    /// detects and protects in-place TUI redraws (Ink-style log-update
    /// patterns, DEC 2026 synchronized updates, shell-emitted TUI hints).
    ///
    /// - `full` (default): DEC 2026, shell hints, and cursor-up heuristic
    ///   all active.
    /// - `strict_protocol`: only DEC 2026 and shell hints; heuristic off.
    /// - `off`: legacy behaviour, all three signals disabled (escape
    ///   hatch for diagnosis).
    ///
    /// Default: full
    ...
    /// Controls how many lines of scrollback history Carrot keeps per terminal.
    ///
    /// Default: 10_000
...
/// TUI-awareness policy — how aggressively the block system detects
/// and protects in-place TUI redraws.
#[derive(
    Clone,
    Copy,
    Debug,
...
pub enum TuiAwareness {
    /// All three activation signals active: DEC 2026, shell hints, and
    /// the cursor-up heuristic (default).
    #[default]
    Full,
    /// Only DEC 2026 and shell hints; heuristic disabled. Useful when
    /// workflows produce cursor-up activity that should not be treated
    /// as TUI redraws.
    StrictProtocol,
    /// Completely off — legacy behaviour. Escape hatch for diagnosis.
    Off,
}
