...
use carrot_ui_input::{ErasedEditor, ErasedEditorEvent};
use carrot_workspace::{ModalView, item::Settings};
use head::Head;
use inazuma::{
    ...
    ScrollStrategy, Task, UniformListScrollHandle, Window, actions, canvas, div, list,
    uniform_list,
};
...
pub trait PickerDelegate: Sized + 'static {
    ...
    // This avoids a flash of an empty command-palette on cmd-shift-p, and lets carrot_workspace::SendKeystrokes
...
impl<D: PickerDelegate> Picker<D> {
    /// A picker, which displays its matches using `inazuma::uniform_list`, all matches should have the same height.
    ...
    /// A picker, which displays its matches using `inazuma::list`, matches can have different heights.
    /// The picker allows the user to perform search items by text.
    ...
    fn create_element_container(container: ContainerKind) -> ElementContainer {
        match container {
            ...
            ContainerKind::List => {
                ElementContainer::List(ListState::new(0, inazuma::ListAlignment::Top, px(1000.)))
            }
        }
    }

    pub fn width(mut self, width: impl Into<inazuma::Length>) -> Self {
    ...
    pub fn max_height(mut self, max_height: Option<inazuma::Length>) -> Self {
        ...
        match event {
            ...
            ErasedEditorEvent::Blurred => {
                if self.is_modal && window.is_window_active() {
                    self.cancel(&inazuma_menu::Cancel, window, cx);
                }
            }
        }
    }
...
mod tests {
    ...
    impl PickerDelegate for TestDelegate {
        ...
        fn render_match(
            ...
            Some(
                carrot_ui::ListItem::new(ix)
                    .inset(true)
                    .toggle_state(selected)
                    .child(carrot_ui::Label::new(format!("Item {ix}"))),
            )
        }
    }
    ...
    async fn test_keyboard_navigation_skips_non_selectable_items(cx: &mut TestAppContext) {
        ...
        picker.update_in(cx, |picker, window, cx| {
            picker.select_next(&inazuma_menu::SelectNext, window, cx);
        });
        picker.update(cx, |picker, _cx| {
            assert_eq!(
        ...
        picker.update_in(cx, |picker, window, cx| {
            picker.select_previous(&inazuma_menu::SelectPrevious, window, cx);
        });
        picker.update(cx, |picker, _cx| {
            assert_eq!(
                picker.delegate.selected_index(),
