...
pub enum Msg {
    ...
    /// Indicates that the `EventLoop` should shut down, as Carrot is shutting down.
    Shutdown,
    ...
    U: EventListener + Send + 'static,
{
        ...
        X: Write,
    {
        ...
        loop {
            ...
            // Single-path v2 VT pipeline. Legacy parser + parity
            // probe are gone; `Term::advance` drives the
            // VtWriter directly.
            terminal.advance(&buf[..unprocessed]);
        ...
        // Queue terminal redraw when we actually processed bytes.
        // The v2 writer performs sync-update buffering inside its own
        // state (DEC 2026 BSU/ESU boundary tracking), so there is no
        // external `sync_bytes_count` comparison left to make.
        let _ = state; // explicit — state is not used here post-collapse
        if processed > 0 {
    ...
    pub fn spawn(mut self) -> JoinHandle<(Self, State)> {
        thread::spawn_named("PTY reader", move || {
            ...
            let mut pipe = if self.ref_test {
                Some(File::create("./carrot.recording").expect("create carrot recording"))
            } else {
                None
            };

            'event_loop: loop {
                // Post-legacy: sync-update timing lives inside the v2
                // VT writer. Poll with no custom timeout — the writer
                // self-commits on ESU, so we just wait for the next
                // PTY byte or channel event.
                events.clear();
