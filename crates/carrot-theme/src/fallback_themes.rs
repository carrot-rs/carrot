...
use inazuma::{
    FontStyle, FontWeight, HighlightStyle, WindowBackgroundAppearance, oklch, oklcha, px,
};

use crate::{
    AccentColors, Appearance, BlockColors, ChartColors, ChipColors, DEFAULT_DARK_THEME,
    DEFAULT_LIGHT_THEME, EditorColors, MinimapColors, PaneColors, PanelColors, PlayerColor,
    PlayerColors, ScrollbarColors, SearchColors, StatusBarColors, StatusColors,
    StatusColorsRefinement, StatusStyle, StatusStyleRefinement, SyntaxTheme, SystemColors,
    TabColors, TerminalAnsiColors, TerminalColors, Theme, ThemeBackgroundImage, ThemeColors,
    ThemeColorsRefinement, ThemeFamily, ThemeStyles, TitleBarColors, ToolbarColors,
    VersionControlColors, VimColors, default_color_scales,
};

/// The default theme family for Carrot.
///
/// Contains Carrot Dark and Carrot Light as compile-time fallback themes.
pub fn carrot_default_themes() -> ThemeFamily {
    ThemeFamily {
        id: "carrot-default".to_string(),
        name: "Carrot Default".into(),
        author: "nyxb".into(),
        themes: vec![carrot_default_dark(), carrot_default_light()],
...
// ---------------------------------------------------------------------------
// Carrot Dark — from assets/themes/carrot-dark/theme.toml
// Fields not in our TOML are derived from the original One Dark, adapted to our palette.
// ---------------------------------------------------------------------------

pub(crate) fn carrot_default_dark() -> Theme {
    let transparent = oklcha(0.0, 0.0, 0.0, 0.0);

    // === Core palette (from carrot-dark/theme.toml) ===
    // Terminal-first hierarchy: terminal/editor sit at the darkest level (bg),
    // UI chrome (panel, title bar, status bar) is clearly lighter so it reads
    // as a distinct frame around the terminal. The surface/element steps above
    // stay on top of chrome_bg for cards, inputs and hover states.
    let bg = oklch(0.1822, 0.0, 0.0);
    let chrome_bg = oklch(0.22, 0.0, 0.0);
    let chrome_inactive = oklch(0.20, 0.0, 0.0);
    let surface = oklch(0.26, 0.0, 0.0);
    let elevated = oklch(0.35, 0.0, 0.0);
    let element_bg = oklch(0.28, 0.0, 0.0);
    let hover = oklch(0.33, 0.0, 0.0);
    let active = oklch(0.38, 0.0, 0.0);
    let subtle_border = oklch(0.55, 0.0, 0.0);

    // Accent: Orange #E8630B
    let accent = oklch(0.6572, 0.1838, 45.27);
    ...
    let player = PlayerColors(vec![
        PlayerColor {
            cursor: accent,
            background: accent,
            selection: oklcha(0.6572, 0.1838, 45.27, 0.239),
        },
        PlayerColor {
            cursor: blue,
            background: blue,
    ...
    Theme {
        ...
        styles: ThemeStyles {
            ...
            colors: ThemeColors {
                ...
                drop_target_background: oklcha(0.6572, 0.1838, 45.27, 0.251),
                ...
                border_selected: oklcha(0.6572, 0.1838, 45.27, 0.4),
                ...
                editor: EditorColors {
                    ...
                    active_line_background: oklcha(0.26, 0.0, 0.0, 0.749),
                    ...
                    invisible: active,
                    wrap_guide: oklcha(0.33, 0.0, 0.0, 0.051),
                    active_wrap_guide: oklcha(0.33, 0.0, 0.0, 0.1),
                    indent_guide: subtle_border,
                    indent_guide_active: hover,
                ...
                terminal: TerminalColors {
                    ...
                    ansi: TerminalAnsiColors {
                        ...
                        dim_black: oklch(0.2178, 0.0, 0.0),
                ...
                panel: PanelColors {
                    ...
                    indent_guide: subtle_border,
                    indent_guide_hover: hover,
                    indent_guide_active: hover,
                    overlay_background: chrome_bg,
                    overlay_hover: hover,
                },
                pane: PaneColors {
                    focused_border: accent,
                ...
                tab: TabColors {
                    // bar_background matches title_bar so the bar blends in.
                    bar_background: chrome_bg,
                    // Inactive tabs match the title bar (invisible boxes).
                    inactive_background: chrome_bg,
                    // Active tab pops with a clearly lighter background.
                    active_background: hover,
                ...
                title_bar: TitleBarColors {
                    background: chrome_bg,
                    inactive_background: chrome_inactive,
                },
                toolbar: ToolbarColors {
                    background: surface,
                },
                search: SearchColors {
                    match_background: oklcha(0.6572, 0.1838, 45.27, 0.251),
                    active_match_background: oklcha(0.6572, 0.1838, 45.27, 0.4),
                },
                vim: VimColors {
                ...
                chip: ChipColors {
                    background: oklcha(1.0, 0.0, 0.0, 0.08),
                    border: oklcha(1.0, 0.0, 0.0, 0.15),
                    text: fg,
                    hover: oklcha(1.0, 0.0, 0.0, 0.12),
                    username: cyan,
                    hostname: fg,
                    directory: green,
                    time: red,
                    shell: magenta,
                    git_branch_icon: green,
                    git_branch_text: cyan,
                    git_stats_neutral: fg,
                    git_stats_insert: green,
                    git_stats_delete: red,
                },
            },
            status: StatusColors {
                error: StatusStyle {
            ...
            syntax: Arc::new(SyntaxTheme::new(vec![
                ...
                (
                    ...
                    oklch(0.7276, 0.0609, 273.09).into(),
                ),
...
// ---------------------------------------------------------------------------
// Carrot Light — inverted version of Carrot Dark
// Uses light-adapted values, fills from the original One Light where needed.
// ---------------------------------------------------------------------------

pub(crate) fn carrot_default_light() -> Theme {
    let transparent = oklcha(0.0, 0.0, 0.0, 0.0);
        ...
        styles: ThemeStyles {
            ...
            colors: ThemeColors {
                ...
                tab: TabColors {
                    ...
                    // Active tab pops with a clearly darker background.
                    active_background: hover,
                    inactive_foreground: text_muted,
                    active_foreground: fg,
                },
                ...
                status_bar: StatusBarColors { background: bg },
                title_bar: TitleBarColors {
                    background: bg,
                    inactive_background: surface,
                },
                toolbar: ToolbarColors { background: bg },
                search: SearchColors {
