...
use inazuma::{
    App, Global, Oklch, Pixels, SharedString, WindowAppearance, WindowBackgroundAppearance, px,
};
...
impl ThemeBackgroundImage {
    ...
    pub fn resolve(&self, base_dir: Option<&Path>) -> Option<(PathBuf, f32)> {
        ...
        let resolved = if path.is_absolute() {
            ...
            log::warn!(
                "Cannot resolve relative background image path without base_dir: {}",
                self.path
            );
            return None;
        };
...
pub struct Theme {
    ...
    ///
    /// For user themes loaded from `~/.carrot/themes/my-theme/theme.toml`, this is
...
/// A family of related themes (e.g. "Carrot" with Dark and Light variants).
#[derive(Clone, Debug)]
