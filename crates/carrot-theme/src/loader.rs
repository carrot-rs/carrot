...
use crate::colors::{
    ...
    TabColorsRefinement, TerminalAnsiColorsRefinement, TerminalColorsRefinement, ThemeColors,
...
fn parse_oklch_functional(s: &str) -> Result<Oklch> {
    ...
    if parts.len() != 3 {
        bail!(
            "oklch() requires 3 components (l c h), got {}: '{s}'",
            parts.len()
        );
    }
...
fn parse_theme_colors(
    ...
    for (key, value) in flat {
        ...
        let color =
            parse_color(value).with_context(|| format!("invalid color for '{key}': '{value}'"))?;
        ...
        if let Some((prefix, rest)) = key.split_once('.') {
            match prefix {
                ...
                "version_control" => {
                    set_version_control_field(&mut refinement.version_control, rest, color)
                }
                ...
                "ghost_element" => set_ghost_element_field(&mut refinement, rest, color),
                "element" => set_element_field(&mut refinement, rest, color),
                _ => {
...
fn set_terminal_ansi_field(
    refinement: &mut TerminalAnsiColorsRefinement,
    name: &str,
    color: Oklch,
) {
    match_fields!(
...
fn set_pane_field(refinement: &mut PaneColorsRefinement, name: &str, color: Oklch) {
    match_fields!(refinement, name, color, focused_border, group_border,);
}
...
fn set_ghost_element_field(refinement: &mut ThemeColorsRefinement, name: &str, color: Oklch) {
    match name {
        "background" => refinement.ghost_element_background = Some(color),
        "hover" => refinement.ghost_element_hover = Some(color),
        "active" => refinement.ghost_element_active = Some(color),
        "selected" => refinement.ghost_element_selected = Some(color),
        "disabled" => refinement.ghost_element_disabled = Some(color),
        other => log::trace!("unknown ghost_element field: '{other}'"),
    }
}

fn set_element_field(refinement: &mut ThemeColorsRefinement, name: &str, color: Oklch) {
    match name {
        "background" => refinement.element_background = Some(color),
        "hover" => refinement.element_hover = Some(color),
        "active" => refinement.element_active = Some(color),
        "selected" => refinement.element_selected = Some(color),
        "disabled" => refinement.element_disabled = Some(color),
        "selection" => refinement.element_selection = Some(color),
        other => log::trace!("unknown element field: '{other}'"),
    }
}

fn set_status_bar_field(refinement: &mut StatusBarColorsRefinement, name: &str, color: Oklch) {
    match_fields!(refinement, name, color, background,);
}

fn set_title_bar_field(refinement: &mut TitleBarColorsRefinement, name: &str, color: Oklch) {
    match_fields!(refinement, name, color, background, inactive_background,);
}
...
fn set_version_control_field(
    refinement: &mut VersionControlColorsRefinement,
    name: &str,
    color: Oklch,
) {
    match_fields!(
...
fn set_chart_field(refinement: &mut ChartColorsRefinement, name: &str, color: Oklch) {
    match_fields!(
        refinement, name, color, chart_1, chart_2, chart_3, chart_4, chart_5,
    );
}
