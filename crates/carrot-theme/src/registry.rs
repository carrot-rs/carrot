...
use inazuma::{App, AssetSource, Global, SharedString};
use parking_lot::RwLock;
use std::collections::HashMap;
use thiserror::Error;

use crate::{
    Appearance, AppearanceContent, IconThemeFamilyContent, Theme, ThemeFamily,
    icon_theme::{
        ChevronIcons, DEFAULT_ICON_THEME_NAME, DirectoryIcons, IconDefinition, IconTheme,
        default_icon_theme,
    },
};
...
pub struct GlobalThemeRegistry(pub Arc<ThemeRegistry>);
...
impl ThemeRegistry {
    ...
    pub fn set_global(assets: Box<dyn AssetSource>, cx: &mut App) {
    ...
    pub fn new(assets: Box<dyn AssetSource>) -> Self {
        ...
        // Load Carrot default themes so there's always a theme available.
        registry.insert_theme_families([crate::fallback_themes::carrot_default_themes()]);
    ...
    /// Inserts a single theme into the registry.
    pub fn insert_theme(&self, theme: Theme) {
        let mut state = self.state.write();
        state.themes.insert(theme.name.clone(), Arc::new(theme));
    }
    ...
    pub fn load_bundled_themes(&self, assets: &dyn AssetSource) {
        ...
        for path in all_paths {
            ...
            match crate::loader::load_theme_from_toml(id, content) {
                Ok(theme) => {
