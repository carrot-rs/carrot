...
/// Deserializes an icon theme from TOML bytes.
pub fn deserialize_icon_theme(bytes: &[u8]) -> Result<IconThemeFamilyContent> {
    let content = std::str::from_utf8(bytes).context("icon theme is not valid UTF-8")?;
    let family: IconThemeFamilyContent =
        toml::from_str(content).context("failed to parse icon theme TOML")?;
    Ok(family)
}
