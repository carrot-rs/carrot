...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("Carrot.exe"), p("old\\Carrot.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\Carrot.exe"), p("old\\bin\\Carrot.exe")),
        Job::move_file(p("bin\\carrot"), p("old\\bin\\carrot")),
        //
        ...
        // Copy new files
        Job::move_file(p("install\\Carrot.exe"), p("Carrot.exe")),
        Job::move_file(p("install\\bin\\Carrot.exe"), p("bin\\Carrot.exe")),
        Job::move_file(p("install\\bin\\carrot"), p("bin\\carrot")),
        //
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("Carrot.exe"),
        app_dir.join("bin\\Carrot.exe"),
        app_dir.join("bin\\carrot"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("Carrot.exe")).spawn();
    }
