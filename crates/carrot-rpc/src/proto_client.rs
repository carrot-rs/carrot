...
use inazuma::{AnyEntity, AnyWeakEntity, AsyncApp, BackgroundExecutor, Entity, FutureExt as _};
        ...
        HashMap<
            ...
            oneshot::Sender<
                Result<
                    Option<
                        TypedEnvelope<
                            Vec<carrot_proto::ProtoLspResponse<Box<dyn AnyTypedEnvelope>>>,
                        >,
                    >,
                >,
            >,
...
impl AnyProtoClient {
    ...
    pub fn request_lsp<T>(
        ...
        Output = Result<Option<TypedEnvelope<Vec<carrot_proto::ProtoLspResponse<T::Response>>>>>,
    > + use<T>
    where
        T: LspRequestMessage,
    {
        ...
        let query = carrot_proto::LspQuery {
            project_id,
            server_id,
    ...
    pub fn send_lsp_response<T: LspRequestMessage>(
        ...
        self.send(carrot_proto::LspQueryResponse {
            project_id,
            lsp_request_id: lsp_request_id.0,
            responses: server_responses
                .into_iter()
                .map(|(server_id, response)| carrot_proto::LspResponse {
                    server_id,
    ...
    pub fn handle_lsp_response(&self, mut envelope: TypedEnvelope<carrot_proto::LspQueryResponse>) {
        ...
        if let Some(tx) = response_senders.remove(&request_id) {
            ...
            tx.send(Ok(Some(carrot_proto::TypedEnvelope {
                ...
                payload: responses
                    .into_iter()
                    .filter_map(|response| {
                        use carrot_proto::lsp_response::Response;
                        ...
                        Some(carrot_proto::ProtoLspResponse {
                            server_id,
                            response,
                        })
                    })
    ...
    pub fn add_request_handler<M, E, H, F>(&self, entity: inazuma::WeakEntity<E>, handler: H)
    where
        M: RequestMessage,
        E: 'static,
    ...
    where
        ...
        E: 'static,
        H: 'static + Sync + Send + Fn(inazuma::Entity<E>, TypedEnvelope<M>, AsyncApp) -> F,
        F: 'static + Future<Output = Result<M::Response>>,
    {
...
fn to_any_envelope<T: EnvelopedMessage>(
    envelope: &TypedEnvelope<carrot_proto::LspQueryResponse>,
    response: T,
) -> Box<dyn AnyTypedEnvelope> {
    Box::new(carrot_proto::TypedEnvelope {
        sender_id: envelope.sender_id,
        original_sender_id: envelope.original_sender_id,
        message_id: envelope.message_id,
        received_at: envelope.received_at,
