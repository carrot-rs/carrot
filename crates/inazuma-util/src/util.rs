...
pub use inazuma_gpui_util::*;

#[cfg(any(test, feature = "test-support"))]
pub use inazuma_util_macros::{line_endings, path, uri};
pub use take_until::*;
...
/// `CARROT_ALLOW_ROOT` environment variable.
#[cfg(unix)]
pub fn prevent_root_execution() {
    let is_root = nix::unistd::geteuid().is_root();
    let allow_root = std::env::var("CARROT_ALLOW_ROOT").is_ok_and(|val| val == "true");

    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running Carrot as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of Carrot.
       It is untested and not recommended, don't complain when things break.
       If you wish to proceed anyways, set `CARROT_ALLOW_ROOT=true` in your environment."
        );
        std::process::exit(1);
    }
}
...
pub fn get_shell_safe_app_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    ...
    if cfg!(target_os = "linux")
        && !carrot_path.is_file()
        && let Some(truncated) = carrot_path
            .clone()
            .file_name()
            .and_then(|s| s.to_str())
            .and_then(|n| n.strip_suffix(" (deleted)"))
    {
        ...
        carrot_path.set_file_name(truncated);
    }
...
/// Returns a path for the Carrot CLI executable.
pub fn get_carrot_cli_path() -> Result<PathBuf> {
    use anyhow::Context as _;
    let carrot_path =
        std::env::current_exe().context("Failed to determine current Carrot executable path.")?;
    let parent = carrot_path
        .parent()
        .context("Failed to determine parent directory of Carrot executable path.")?;

    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        &["./cli"]
    } else if cfg!(target_os = "windows") {
        &["bin/carrot.exe", "./cli.exe"]
    } else if cfg!(target_os = "linux") || cfg!(target_os = "freebsd") {
        &["../bin/carrot", "./cli"]
    } else {
        anyhow::bail!("unsupported platform for determining Carrot CLI path");
    };
    ...
    possible_locations
        ...
        .find_map(|p| {
            parent
                ...
                .canonicalize()
                .ok()
                .filter(|p| p != &carrot_path)
        })
        .with_context(|| {
            format!(
                "could not find Carrot CLI from any of: {}",
...
pub async fn load_login_shell_environment() -> Result<()> {
    ...
    {
        // Skip SHLVL to prevent it from polluting Carrot's process environment.
        // The login shell used for env capture increments SHLVL, and if we propagate it,
        // terminals spawned by Carrot will inherit it and increment again, causing SHLVL
