...
pub enum ShellKind {
    /// Generic POSIX-compatible shell — sh, dash, ksh, and anything
    /// falling back here. Behaves like Bash/Zsh for shell-builder
    /// purposes but is used as the "unknown POSIX" catch-all.
    #[default]
    Posix,
    /// GNU Bash. Separated from Posix so parsers / language-aware
    /// consumers (tree-sitter-bash, highlight, completion specs) can
    /// dispatch on the concrete shell.
    Bash,
    /// Z shell. Separated from Posix for the same reason as Bash.
    Zsh,
    Csh,
    Tcsh,
    Rc,
...
impl ShellKind {
    ...
    pub fn new(program: impl AsRef<Path>, is_windows: bool) -> Self {
        ...
        match &*program {
            ...
            "sh" | "dash" | "ksh" | "ash" => ShellKind::Posix,
    ...
    pub fn to_shell_variable(self, input: &str) -> String {
        match self {
            ...
            Self::Posix | Self::Bash | Self::Zsh => input.to_owned(),
...
mod tests {
    ...
    // wsl.exe --distribution NixOS --cd ~ env RUST_LOG=info,remote=debug .carrot_wsl_server/carrot-remote-server-dev-build proxy --identifier dev-workspace-53
    ...
    #[test]
    fn test_shell_kind_new_bash_and_zsh_are_distinct() {
        assert_eq!(ShellKind::new("bash", false), ShellKind::Bash);
        assert_eq!(ShellKind::new("zsh", false), ShellKind::Zsh);
        assert_eq!(ShellKind::new("/usr/local/bin/zsh", false), ShellKind::Zsh);
        assert_eq!(ShellKind::new("/bin/bash", false), ShellKind::Bash);
        // Generic POSIX shells still fall back to Posix.
        assert_eq!(ShellKind::new("sh", false), ShellKind::Posix);
        assert_eq!(ShellKind::new("dash", false), ShellKind::Posix);
        assert_eq!(ShellKind::new("ksh", false), ShellKind::Posix);
    }

    #[test]
    fn test_bash_and_zsh_inherit_posix_behaviour() {
        for kind in [ShellKind::Bash, ShellKind::Zsh] {
            assert!(kind.supports_posix_chaining());
            assert_eq!(kind.command_prefix(), None);
            assert_eq!(kind.sequential_commands_separator(), ';');
            assert_eq!(kind.sequential_and_commands_separator(), "&&");
            assert_eq!(kind.activate_keyword(), "source");
            assert_eq!(kind.clear_screen_command(), "clear");
        }
    }

    #[test]
    fn test_shell_kind_display_names() {
        assert_eq!(ShellKind::Bash.to_string(), "bash");
        assert_eq!(ShellKind::Zsh.to_string(), "zsh");
        assert_eq!(ShellKind::Posix.to_string(), "sh");
    }
