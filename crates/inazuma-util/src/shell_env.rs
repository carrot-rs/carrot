...
async fn capture_unix(
    ...
    let carrot_path = super::get_shell_safe_app_path(shell_kind)?;
    ...
    // See: https://github.com/carrot-rs/carrot/pull/32136#issuecomment-2999645482
    ...
    command_string.push_str(&format!("{} --printenv {}", carrot_path, redir));
    ...
    // Parse the JSON output from carrot --printenv
...
async fn capture_windows(
    ...
    let carrot_path_string = carrot_path.display().to_string();
    ...
    let cmd = match shell_kind {
        ...
        | ShellKind::Posix => {
            ...
            let quoted_carrot_path = quote_for_shell(&carrot_path_string);
            cmd.args([
                "-l",
                "-i",
                "-c",
                &format!("cd {}; {} --printenv", quoted_directory, quoted_carrot_path),
            ])
        }
        ShellKind::PowerShell | ShellKind::Pwsh => {
            let quoted_directory = ShellKind::quote_pwsh(&directory_string);
            let quoted_carrot_path = ShellKind::quote_pwsh(&carrot_path_string);
            cmd.args([
                "-NonInteractive",
                "-NoProfile",
                "-Command",
                &format!(
                    ...
                    quoted_directory, quoted_carrot_path
                ),
            ])
        }
        ShellKind::Elvish => {
        ...
        ShellKind::Nushell => {
            let quoted_directory = quote_for_shell(&directory_string);
            let quoted_carrot_path = quote_for_shell(&carrot_path_string);
            let carrot_command = shell_kind
                .prepend_command_prefix(&quoted_carrot_path)
                .into_owned();
            cmd.args([
                "-c",
                &format!("cd {}; {} --printenv", quoted_directory, carrot_command),
            ])
        }
