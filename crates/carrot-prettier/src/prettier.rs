...
use carrot_fs::Fs;
use carrot_language::language_settings::{LanguageSettings, PrettierSettings};
use carrot_language::{Buffer, Diff, Language};
use carrot_lsp::{LanguageServer, LanguageServerId};
use carrot_node_runtime::NodeRuntime;
use carrot_paths::default_prettier_dir;
use inazuma::{AsyncApp, Entity};
...
impl carrot_lsp::request::Request for Format {
...
impl carrot_lsp::request::Request for ClearCache {
    type Params = ();
    type Result = ();
...
mod tests {
    ...
    #[inazuma::test]
    async fn test_prettier_lookup_finds_nothing(cx: &mut inazuma::TestAppContext) {
        ...
        assert_eq!(
            Prettier::locate_prettier_installation(
                fs.as_ref(),
                &HashSet::default(),
                Path::new("/root/.config/carrot/settings.json"),
            )
            .await
            .unwrap(),
    ...
    #[inazuma::test]
    async fn test_prettier_lookup_in_simple_npm_projects(cx: &mut inazuma::TestAppContext) {
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            "/root",
            json!({
    ...
    #[inazuma::test]
    async fn test_prettier_lookup_for_not_installed(cx: &mut inazuma::TestAppContext) {
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            "/root",
            json!({
    ...
    #[inazuma::test]
    async fn test_prettier_lookup_in_npm_workspaces(cx: &mut inazuma::TestAppContext) {
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            "/root",
            json!({
    ...
    #[inazuma::test]
    async fn test_prettier_ignore_with_editor_prettier(cx: &mut inazuma::TestAppContext) {
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            "/root",
            json!({
