...
use carrot_fs::Fs;
use carrot_http_client::HttpRequestExt;
use carrot_http_client::{AsyncBody, HttpClient, Method, Request as HttpRequest};
use carrot_paths::home_dir;
use futures::{AsyncBufReadExt, AsyncReadExt, StreamExt, io::BufReader, stream::BoxStream};
use inazuma::WeakEntity;
use inazuma::{App, AsyncApp, Global, prelude::*};
use inazuma_collections::HashSet;
use serde::{Deserialize, Serialize};

use inazuma_settings_framework::watch_config_dir;
...
pub struct Model {
    ...
    // reached. Carrot does not currently implement this behaviour
...
struct GlobalCopilotChat(inazuma::Entity<CopilotChat>);
...
impl CopilotChat {
    pub fn global(cx: &App) -> Option<inazuma::Entity<Self>> {
    ...
    pub async fn stream_messages(
        ...
        mut cx: AsyncApp,
    ) -> Result<BoxStream<'static, Result<carrot_anthropic::Event, carrot_anthropic::AnthropicError>>>
    {
        let (client, oauth_token, api_endpoint, configuration) =
            Self::get_auth_details(&mut cx).await?;
...
pub(crate) fn copilot_request_headers(
    builder: carrot_http_client::Builder,
    oauth_token: &str,
    is_user_initiated: Option<bool>,
    location: Option<ChatLocation>,
) -> carrot_http_client::Builder {
    builder
        .header("Authorization", format!("Bearer {}", oauth_token))
        .header("Content-Type", "application/json")
        .header(
                ...
                Ok(line) => {
                    ...
                    match serde_json::from_str(line) {
                        ...
                        Err(error) => {
                            ...
                            Some(Err(carrot_anthropic::AnthropicError::DeserializeResponse(error)))
                        }
                    }
                }
                Err(error) => Some(Err(carrot_anthropic::AnthropicError::ReadResponse(error))),
            }
        })
        .boxed())
}
