...
use carrot_language::{Anchor, Buffer, BufferSnapshot, EditPreview, OffsetRangeExt, ToPointUtf16};
use inazuma::{App, Context, Entity, Task};
...
mod tests {
    use super::*;
    use carrot_edit_prediction_types::EditPredictionGranularity;
    use carrot_editor::{
    ...
    use inazuma::{AppContext as _, BackgroundExecutor, TestAppContext, UpdateGlobal};
    use inazuma_settings_framework::{AllLanguageSettingsContent, SettingsStore};
    use inazuma_util::{
        path,
    ...
    async fn test_copilot_multibuffer(executor: BackgroundExecutor, cx: &mut TestAppContext) {
        ...
        let multibuffer = cx.new(|cx| {
            let mut multibuffer = MultiBuffer::new(carrot_language::Capability::ReadWrite);
    ...
    async fn test_copilot_does_not_prevent_completion_triggers(
        ...
        let mut cx = EditorLspTestContext::new_rust(
            carrot_lsp::ServerCapabilities {
                completion_provider: Some(carrot_lsp::CompletionOptions {
                    trigger_characters: Some(vec![".".to_string(), ":".to_string()]),
                    ..carrot_lsp::CompletionOptions::default()
                }),
    ...
    fn handle_copilot_completion_request(
        lsp: &carrot_lsp::FakeLanguageServer,
    ...
    fn handle_completion_request(
        ...
        let mut request = cx.set_request_handler::<carrot_lsp::request::Completion, _, _>(
            move |url, params, _| {
                let completions = completions.clone();
                async move {
                    assert_eq!(params.text_document_position.text_document.uri, url.clone());
                    Ok(Some(carrot_lsp::CompletionResponse::Array(
                        completions
                            .iter()
                            .map(|completion_text| carrot_lsp::CompletionItem {
