...
use carrot_language::language_settings::{AllLanguageSettings, CopilotSettings};
use carrot_language::{
...
use carrot_lsp::{LanguageServer, LanguageServerBinary, LanguageServerId, LanguageServerName};
use carrot_node_runtime::{NodeRuntime, VersionStrategy};
use carrot_project::project_settings::ProjectSettings;
use carrot_project::{DisableAiSettings, Project};
...
use inazuma_util::{ResultExt, fs::remove_matching};
...
pub(crate) struct CopilotEditPrediction {
    ...
    pub(crate) command: Option<carrot_lsp::Command>,
...
impl Copilot {
    pub fn new(
        ...
        let send_focus_notification = project.map(|project| {
            cx.subscribe(&project, |this, project, e: &carrot_project::Event, cx| {
                if let carrot_project::Event::ActiveEntryChanged(new_entry) = e
                    && let Ok(running) = this.server.as_authenticated()
                {
                    let uri = new_entry
                        ...
                        .and_then(|abs_path| carrot_lsp::Uri::from_file_path(abs_path).ok());
    ...
    fn build_env(&self, copilot_settings: &CopilotSettings) -> Option<HashMap<String, String>> {
        ...
        if let Ok(oauth_token) = env::var(carrot_copilot_chat::COPILOT_OAUTH_ENV_VAR) {
            env.insert(
                carrot_copilot_chat::COPILOT_OAUTH_ENV_VAR.to_string(),
                oauth_token,
            );
        }
    ...
    ) -> (Entity<Self>, carrot_lsp::FakeLanguageServer) {
        use carrot_fs::FakeFs;
        use carrot_lsp::FakeLanguageServer;
    ...
    async fn start_language_server(
        ...
        let start_language_server = async {
            ...
            server
                .on_request::<carrot_lsp::request::ShowDocument, _, _>(move |params, cx| {
                    if params.external.unwrap_or(false) {
                        let url = params.uri.to_string();
                        cx.update(|cx| cx.open_url(&url));
                    }
                    async move { Ok(carrot_lsp::ShowDocumentResult { success: true }) }
                })
                .detach();

            let configuration = carrot_lsp::DidChangeConfigurationParams {
            ...
            let server = cx
                .update(|cx| {
                    ...
                    params
                        ...
                        .show_document =
                        Some(carrot_lsp::ShowDocumentClientCapabilities { support: true });
    ...
    pub fn register_buffer(&mut self, buffer: &Entity<Buffer>, cx: &mut Context<Self>) {
        ...
        }) = &mut self.server
        {
            ...
            if let Entry::Vacant(e) = entry {
                ...
                server
                    .notify::<carrot_lsp::notification::DidOpenTextDocument>(
                        carrot_lsp::DidOpenTextDocumentParams {
                            text_document: carrot_lsp::TextDocumentItem {
                                uri: uri.clone(),
    ...
    fn handle_buffer_event(
            ...
            && let Some(registered_buffer) = server.registered_buffers.get_mut(&buffer.entity_id())
        {
            match event {
                ...
                | carrot_language::BufferEvent::LanguageChanged(_) => {
                        ...
                        || new_language_id != registered_buffer.language_id
                    {
                        ...
                        server
                            .lsp
                            .notify::<carrot_lsp::notification::DidCloseTextDocument>(
                                carrot_lsp::DidCloseTextDocumentParams {
                                    text_document: carrot_lsp::TextDocumentIdentifier::new(old_uri),
                                },
                            )
                            .ok();
                        server
                            ...
                            .notify::<carrot_lsp::notification::DidOpenTextDocument>(
                                carrot_lsp::DidOpenTextDocumentParams {
                                    text_document: carrot_lsp::TextDocumentItem::new(
                                        registered_buffer.uri.clone(),
    ...
    fn unregister_buffer(&mut self, buffer: &WeakEntity<Buffer>) {
            ...
            && let Some(buffer) = server.registered_buffers.remove(&buffer.entity_id())
        {
            server
                ...
                .notify::<carrot_lsp::notification::DidCloseTextDocument>(
                    carrot_lsp::DidCloseTextDocumentParams {
                        text_document: carrot_lsp::TextDocumentIdentifier::new(buffer.uri),
                    },
                )
                .ok();
        }
            ...
            let nes_fut = if nes_enabled {
                lsp.request::<NextEditSuggestions>(
                    request::NextEditSuggestionsParams {
                        text_document: carrot_lsp::VersionedTextDocumentIdentifier {
                            uri: uri.clone(),
                            version,
                        },
    ...
    pub(crate) fn accept_completion(
        ...
        if let Some(command) = &completion.command {
            ...
            let request = server.lsp.request::<carrot_lsp::ExecuteCommand>(
...
fn uri_for_buffer(buffer: &Entity<Buffer>, cx: &App) -> Result<carrot_lsp::Uri, ()> {
    if let Some(file) = buffer.read(cx).file().and_then(|file| file.as_local()) {
        carrot_lsp::Uri::from_file_path(file.abs_path(cx))
    } else {
...
fn notify_did_change_config_to_server(
    ...
    if let Some(copilot_chat) = carrot_copilot_chat::CopilotChat::global(cx) {
        copilot_chat.update(cx, |chat, cx| {
            chat.set_configuration(
                carrot_copilot_chat::CopilotChatConfiguration {
...
async fn clear_copilot_dir() {
    remove_matching(carrot_paths::copilot_dir(), |_| true).await
}

async fn clear_copilot_config_dir() {
    remove_matching(carrot_copilot_chat::copilot_chat_config_dir(), |_| true).await
}
...
async fn get_copilot_lsp(fs: Arc<dyn Fs>, node_runtime: NodeRuntime) -> anyhow::Result<PathBuf> {
    ...
    let latest_version = node_runtime
        ...
        .await?;
    let server_path = carrot_paths::copilot_dir().join(SERVER_PATH);

    fs.create_dir(carrot_paths::copilot_dir()).await?;

    let should_install = node_runtime
        .should_install_npm_package(
            PACKAGE_NAME,
            &server_path,
            carrot_paths::copilot_dir(),
...
mod tests {
    use super::*;
    use carrot_fs::FakeFs;
    use carrot_language::language_settings::AllLanguageSettings;
    use carrot_node_runtime::NodeRuntime;
    ...
    async fn test_buffer_management(cx: &mut TestAppContext) {
        ...
        let buffer_1_uri: carrot_lsp::Uri = format!("buffer://{}", buffer_1.entity_id().as_u64())
            .parse()
            .unwrap();
        copilot.update(cx, |copilot, cx| copilot.register_buffer(&buffer_1, cx));
        assert_eq!(
            lsp.receive_notification::<carrot_lsp::notification::DidOpenTextDocument>()
                .await,
            carrot_lsp::DidOpenTextDocumentParams {
                text_document: carrot_lsp::TextDocumentItem::new(
                    buffer_1_uri.clone(),
        ...
        let buffer_2_uri: carrot_lsp::Uri = format!("buffer://{}", buffer_2.entity_id().as_u64())
            .parse()
            .unwrap();
        ...
        assert_eq!(
            lsp.receive_notification::<carrot_lsp::notification::DidChangeTextDocument>()
                .await,
            carrot_lsp::DidChangeTextDocumentParams {
                ...
                content_changes: vec![carrot_lsp::TextDocumentContentChangeEvent {
                    range: Some(carrot_lsp::Range::new(
        ...
        assert_eq!(
            lsp.receive_notification::<carrot_lsp::notification::DidCloseTextDocument>()
                .await,
            carrot_lsp::DidCloseTextDocumentParams {
                text_document: carrot_lsp::TextDocumentIdentifier::new(buffer_1_uri),
            }
        );
        let buffer_1_uri = carrot_lsp::Uri::from_file_path(path!("/root/child/buffer-1")).unwrap();
        assert_eq!(
            lsp.receive_notification::<carrot_lsp::notification::DidOpenTextDocument>()
                .await,
            carrot_lsp::DidOpenTextDocumentParams {
        ...
        assert_eq!(
            ...
            vec![
                carrot_lsp::DidCloseTextDocumentParams {
                    text_document: carrot_lsp::TextDocumentIdentifier::new(buffer_2_uri.clone()),
                },
                carrot_lsp::DidCloseTextDocumentParams {
                    text_document: carrot_lsp::TextDocumentIdentifier::new(buffer_1_uri.clone()),
                },
            ],
        );
        ...
        assert_eq!(
            lsp.receive_notification::<carrot_lsp::notification::DidCloseTextDocument>()
                .await,
            carrot_lsp::DidCloseTextDocumentParams {
                text_document: carrot_lsp::TextDocumentIdentifier::new(buffer_2_uri),
            }
        );
    }
    ...
    impl carrot_language::File for File {
        fn as_local(&self) -> Option<&dyn carrot_language::LocalFile> {
            Some(self)
        }

        fn disk_state(&self) -> carrot_language::DiskState {
            carrot_language::DiskState::Present {
                mtime: ::carrot_fs::MTime::from_seconds_and_nanos(100, 42),
                size: 0,
            }
        }
        ...
        fn to_proto(&self, _: &App) -> carrot_rpc::proto::File {
            unimplemented!()
        }
    ...
    impl carrot_language::LocalFile for File {
