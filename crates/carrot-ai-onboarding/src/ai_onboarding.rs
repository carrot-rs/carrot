...
use carrot_ui::{Divider, RegisterComponent, Tooltip, prelude::*};
...
impl From<carrot_client::Status> for SignInStatus {
    fn from(status: carrot_client::Status) -> Self {
...
pub struct CarrotAiOnboarding {
    pub sign_in_status: SignInStatus,
    pub plan: Option<Plan>,
    pub account_too_young: bool,
    pub continue_with_carrot_ai: Arc<dyn Fn(&mut Window, &mut App)>,
    pub sign_in: Arc<dyn Fn(&mut Window, &mut App)>,
    pub dismiss_onboarding: Option<Arc<dyn Fn(&mut Window, &mut App)>>,
}

impl CarrotAiOnboarding {
    pub fn new(
        client: Arc<Client>,
        user_store: &Entity<UserStore>,
        continue_with_carrot_ai: Arc<dyn Fn(&mut Window, &mut App)>,
        cx: &mut App,
    ) -> Self {
                ...
                .child(
                    IconButton::new("dismiss_onboarding", IconName::Close)
                        ...
                        .on_click(move |_, window, cx| {
                            carrot_telemetry::event!("Banner Dismissed", source = "AI Onboarding",);
                            callback(window, cx)
                        }),
                )
                .into_any_element()
    ...
    fn render_sign_in_disclaimer(&self, _cx: &mut App) -> AnyElement {
        ...
        v_flex()
            .relative()
            .gap_1()
            .child(Headline::new("Welcome to Carrot AI"))
            .child(
                Label::new("Sign in to try Carrot Pro for 14 days, no credit card required.")
                    .color(Color::Muted)
                    .mb_2(),
            )
            .child(PlanDefinitions.pro_plan())
            .child(
                Button::new("sign_in", "Try Carrot Pro for Free")
                    .disabled(signing_in)
                    .full_width()
                    ...
                    .on_click({
                        ...
                        move |_, window, cx| {
                            carrot_telemetry::event!("Start Trial Clicked", state = "pre-sign-in");
                            callback(window, cx)
                        }
                    }),
            )
                        ...
                        .child(
                            Button::new("pro", "Start Free Trial")
                                ...
                                .on_click(move |_, _window, cx| {
                                    ...
                                    cx.open_url(&carrot_urls::start_trial_url(cx))
                                }),
                        ),
                )
                .into_any_element()
    ...
    fn render_trial_state(&self, _cx: &mut App) -> AnyElement {
        v_flex()
            .relative()
            .gap_1()
            .child(Headline::new("Welcome to the Carrot Pro Trial"))
            .child(
    ...
    fn render_pro_plan_state(&self, _cx: &mut App) -> AnyElement {
        v_flex()
            .gap_1()
            .child(Headline::new("Welcome to Carrot Pro"))
            .child(
    ...
    fn render_business_plan_state(&self, _cx: &mut App) -> AnyElement {
        v_flex()
            .gap_1()
            .child(Headline::new("Welcome to Carrot Business"))
            .child(
                Label::new("Here's what you get:")
                    .color(Color::Muted)
                    .mb_2(),
    ...
    fn render_student_plan_state(&self, _cx: &mut App) -> AnyElement {
        v_flex()
            .gap_1()
            .child(Headline::new("Welcome to Carrot Student"))
            .child(
                Label::new("Here's what you get:")
                    .color(Color::Muted)
                    .mb_2(),
...
impl RenderOnce for CarrotAiOnboarding {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        if matches!(self.sign_in_status, SignInStatus::SignedIn) {
            match self.plan {
                None => self.render_free_plan_state(cx),
                Some(Plan::CarrotFree) => self.render_free_plan_state(cx),
                Some(Plan::CarrotProTrial) => self.render_trial_state(cx),
                Some(Plan::CarrotPro) => self.render_pro_plan_state(cx),
                Some(Plan::CarrotBusiness) => self.render_business_plan_state(cx),
                Some(Plan::CarrotStudent) => self.render_student_plan_state(cx),
            }
        } else {
...
impl Component for CarrotAiOnboarding {
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> Option<AnyElement> {
        fn onboarding(
            ...
            CarrotAiOnboarding {
                ...
                plan,
                account_too_young,
                continue_with_carrot_ai: Arc::new(|_, _| {}),
                sign_in: Arc::new(|_, _| {}),
                dismiss_onboarding: None,
            }
            .into_any_element()
        ...
        Some(
            v_flex()
                ...
                .children(vec![
                    ...
                    single_example(
                        "Free Plan",
                        onboarding(SignInStatus::SignedIn, Some(Plan::CarrotFree), false),
                    ),
                    single_example(
                        "Pro Trial",
                        onboarding(SignInStatus::SignedIn, Some(Plan::CarrotProTrial), false),
                    ),
                    single_example(
                        "Pro Plan",
                        onboarding(SignInStatus::SignedIn, Some(Plan::CarrotPro), false),
                    ),
                    single_example(
                        "Business Plan",
                        onboarding(SignInStatus::SignedIn, Some(Plan::CarrotBusiness), false),
                    ),
                    single_example(
                        "Student Plan",
                        onboarding(SignInStatus::SignedIn, Some(Plan::CarrotStudent), false),
                    ),
                ])
                .into_any_element(),
        )
