...
use carrot_channel::{Channel, ChannelBuffer, ChannelBufferEvent, ChannelStore};
use carrot_client::{
...
use carrot_rpc::proto::ChannelVisibility;
use carrot_ui::prelude::*;
use carrot_workspace::{CollaboratorId, item::TabContentParams};
use carrot_workspace::{
    ItemNavHistory, Pane, SaveIntent, Toast, ViewId, Workspace, WorkspaceId,
    item::{FollowableItem, Item, ItemEvent, PaneRole},
    searchable::SearchableItemHandle,
};
use carrot_workspace::{item::Dedup, notifications::NotificationId};
use inazuma::{
...
pub fn init(cx: &mut App) {
    carrot_workspace::FollowableViewRegistry::register::<ChannelView>(cx)
}
    ...
    pub fn new(
        ...
        let editor = cx.new(|cx| {
            ...
            editor.set_custom_context_menu(move |_, position, window, cx| {
                let this = this.clone();
                Some(carrot_ui::ContextMenu::build(
                    window,
                    cx,
                    move |menu, _, _| {
        ...
        match event {
            ...
            ChannelBufferEvent::ChannelChanged => {
                self.editor.update(cx, |_, cx| {
                    cx.emit(carrot_editor::EditorEvent::TitleChanged);
                    cx.notify()
                });
            }
...
impl Item for ChannelView {
    ...
    fn tab_content(&self, params: TabContentParams, _: &Window, cx: &App) -> inazuma::AnyElement {
        let (name, status) = self.get_channel(cx);
        h_flex()
            .gap_2()
            .child(
...
impl FollowableItem for ChannelView {
    ...
    fn from_state_proto(
        workspace: Entity<carrot_workspace::Workspace>,
        remote_id: carrot_workspace::ViewId,
    ...
    fn to_follow_event(event: &Self::Event) -> Option<carrot_workspace::item::FollowEvent> {
