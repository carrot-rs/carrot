...
use carrot_channel::{Channel, ChannelEvent, ChannelStore};
use carrot_client::{ChannelId, Client, Contact, User, UserStore};
use carrot_db::kvp::KeyValueStore;
use carrot_editor::{Editor, EditorElement, EditorStyle};
use carrot_project::{Fs, Project};
use carrot_rpc::{
    ErrorCode, ErrorExt,
...
use carrot_workspace::{
    CopyRoomId, Deafen, LeaveCall, Mute, OpenChannelNotes, OpenChannelNotesById, ScreenShare,
...
use inazuma::{
    ...
    anchored, canvas, deferred, div, fill, list, point, px,
};
use inazuma_collections::{HashMap, HashSet};
use inazuma_fuzzy::{StringMatch, StringMatchCandidate, match_strings};
use inazuma_menu::{Cancel, Confirm, SecondaryConfirm, SelectNext, SelectPrevious};
...
pub fn init(cx: &mut App) {
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        ...
        workspace.register_action(|_, action: &OpenChannelNotesById, window, cx| {
            let channel_id = carrot_client::ChannelId(action.channel_id);
            let workspace = cx.entity();
            window.defer(cx, move |window, cx| {
        ...
        workspace.register_action(|_, _: &Mute, _, cx| carrot_title_bar::collab::toggle_mute(cx));
        workspace
            .register_action(|_, _: &Deafen, _, cx| carrot_title_bar::collab::toggle_deafen(cx));
        ...
        workspace.register_action(|workspace, _: &CopyRoomId, window, cx| {
            use carrot_workspace::notifications::{NotificationId, NotifyTaskExt as _};
                ...
                window.defer(cx, move |_window, cx| {
                    room.update(cx, |room, cx| {
                        if room.is_sharing_screen() {
                            ...
                            #[cfg(target_os = "linux")]
                            let is_wayland = inazuma::guess_compositor() == "Wayland";
        ...
        cx.new(|cx| {
            ...
            cx.subscribe_in(
                ...
                |this: &mut Self, _, event, window, cx| {
                    if let carrot_editor::EditorEvent::Blurred = event {
            ...
            .on_click(cx.listener(move |this, _, window, cx| {
                this.workspace
                    .update(cx, |workspace, cx| {
                        ...
                        carrot_shell::join_in_room_project(project_id, host_user_id, app_state, cx)
                    ...
                    window.handler_for(&this, move |_, window, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("Failed to grant write access", window, cx, |e, _, _| {
                                match e.error_code() {
                                    ErrorCode::NeedsCla => Some("This user has not yet signed the CLA at https://carrot.dev/cla.".into()),
                                    _ => None,
                                }
                            })
                    }),
            ...
            match entry {
                ...
                ListEntry::ParticipantProject {
                    ...
                    if let Some(workspace) = self.workspace.upgrade() {
                        ...
                        carrot_shell::join_in_room_project(
                            *project_id,
                            *host_user_id,
                            app_state,
                            cx,
                ...
                Avatar::new(contact.user.avatar_uri.clone())
                    .indicator::<AvatarAvailabilityIndicator>(if online {
                        Some(AvatarAvailabilityIndicator::new(match busy {
                            true => carrot_ui::CollaboratorAvailability::Busy,
                            false => carrot_ui::CollaboratorAvailability::Free,
                        }))
                    } else {
                        None
                    }),
...
impl Panel for CollabPanel {
    ...
    fn icon(&self, _window: &Window, cx: &App) -> Option<carrot_ui::IconName> {
        CollaborationPanelSettings::get_global(cx)
            .button
            .then_some(carrot_ui::IconName::UserGroup)
    }
