...
use carrot_extension::{Extension, WorktreeDelegate};
...
impl ExtensionDapAdapter {
    pub(crate) fn new(
        extension: Arc<dyn carrot_extension::Extension>,
        debug_adapter_name: Arc<str>,
        schema_path: &Path,
    ) -> Result<Self> {
...
/// An adapter that allows an [`carrot_dap::adapters::DapDelegate`] to be used as a [`WorktreeDelegate`].
...
impl DebugAdapter for ExtensionDapAdapter {
    ...
    async fn config_from_carrot_format(
        ...
        self.extension.dap_config_to_scenario(carrot_scenario).await
    }
