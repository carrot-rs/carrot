...
use carrot_dap::{DebugRequest, StartDebuggingRequestArguments, adapters::DebugTaskDefinition};
use carrot_fs::RemoveOptions;
use carrot_language::{LanguageName, Toolchain};
use carrot_paths::debug_adapters_dir;
use futures::{StreamExt, TryStreamExt};
use inazuma::http_client::AsyncBody;
use inazuma::{AsyncApp, SharedString};
...
impl PythonDebugAdapter {
    ...
    async fn generate_debugpy_arguments<'a>(
        ...
        let mut args = if let Some(user_installed_path) = user_installed_path {
            ...
            let adapter_path =
                carrot_paths::debug_adapters_dir().join(Self::DEBUG_ADAPTER_NAME.as_ref());
    ...
    async fn request_args(
        ...
        if let Ok(console) = configuration.dot_get_mut("console") {
            // Use built-in Carrot terminal if user did not explicitly provide a setting for console.
            if console.is_null() {
    ...
    async fn fetch_wheel(
        ...
        let installation_succeeded = inazuma_util::command::new_command(venv_python.as_ref())
            .args([
                "-m",
                "pip",
                "download",
        ...
        inazuma_util::archive::extract_zip(
    ...
    async fn base_venv_path(
        ...
        let result = self.base_venv_path
            .get_or_init(|| async {
                ...
                let debug_adapter_path = carrot_paths::debug_adapters_dir().join(Self::DEBUG_ADAPTER_NAME.as_ref());
                let output = inazuma_util::command::new_command(&base_python)
                    .args(["-m", "venv", "carrot_base_venv"])
                    .current_dir(
                        &debug_adapter_path,
                    )
                    .spawn()
                ...
                Ok(Arc::from(
                    carrot_paths::debug_adapters_dir()
                        .join(Self::DEBUG_ADAPTER_NAME.as_ref())
                        .join("carrot_base_venv")
                        .join(PYTHON_PATH)
                        .as_ref(),
                ))
            })
...
impl DebugAdapter for PythonDebugAdapter {
    ...
    async fn config_from_carrot_format(
        &self,
        carrot_scenario: CarrotDebugConfig,
    ) -> Result<DebugScenario> {
        let mut args = json!({
            "request": match carrot_scenario.request {
        ...
        match &carrot_scenario.request {
            ...
            DebugRequest::Launch(launch) => {
                ...
                if let Some(stop_on_entry) = carrot_scenario.stop_on_entry {
        ...
        Ok(DebugScenario {
            adapter: carrot_scenario.adapter,
            label: carrot_scenario.label,
            config: args,
            build: None,
            tcp_connection: None,
        })
                ...
                },
                {
                    ...
                    "then": {
                        "properties": {
                            ...
                            "envFile": {
                                "default": "${CARROT_WORKTREE_ROOT}/.env",
    ...
    async fn get_binary(
        ...
        let base_paths = ["cwd", "program", "module"]
            ...
            .chain(
                ...
                // (Which should always be the case because Carrot defaults to the cwd worktree root)
        ...
        for base_path in base_paths {
            if let Some(found_toolchain) = delegate
                ...
                .active_toolchain(
                    ...
                    base_path.into_arc(),
                    carrot_language::LanguageName::new_static(Self::LANGUAGE_NAME),
                    cx,
                )
                .await
            {
...
mod tests {
    ...
    use super::*;
    use carrot_task::TcpArgumentsTemplate;
