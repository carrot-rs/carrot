...
use carrot_dap::adapters::{DebugTaskDefinition, latest_github_release};
use carrot_task::{CarrotDebugConfig, DebugRequest, DebugScenario};
use futures::StreamExt;
use inazuma::AsyncApp;
use inazuma_collections::HashMap;
use inazuma_util::fs::remove_matching;
...
impl CodeLldbDebugAdapter {
    ...
    async fn request_args(
        ...
        label: &str,
    ) -> Result<carrot_dap::StartDebuggingRequestArguments> {
        ...
        Ok(carrot_dap::StartDebuggingRequestArguments {
            request,
            configuration,
        })
    }
...
impl DebugAdapter for CodeLldbDebugAdapter {
    ...
    async fn config_from_carrot_format(
        ...
        map.insert(
            "name".into(),
            Value::String(String::from(carrot_scenario.label.as_ref())),
        );
    ...
    async fn get_binary(
        ...
        if command.is_none() {
            ...
            let adapter_path = carrot_paths::debug_adapters_dir().join(&Self::ADAPTER_NAME);
