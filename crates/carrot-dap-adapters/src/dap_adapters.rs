...
pub fn init(cx: &mut App) {
    cx.update_default_global(|registry: &mut DapRegistry, _cx| {
        ...
        {
            registry.add_adapter(Arc::from(carrot_dap::FakeAdapter {}));
        }
    })
}
...
mod test_mocks {
    ...
    impl adapters::DapDelegate for MockDelegate {
        ...
        fn http_client(&self) -> Arc<dyn carrot_http_client::HttpClient> {
            unimplemented!("Not needed for tests")
        }

        fn node_runtime(&self) -> carrot_node_runtime::NodeRuntime {
            unimplemented!("Not needed for tests")
        }

        fn toolchain_store(&self) -> Arc<dyn carrot_language::LanguageToolchainStore> {
            unimplemented!("Not needed for tests")
        }

        fn fs(&self) -> Arc<dyn carrot_fs::Fs> {
            unimplemented!("Not needed for tests")
        }
