...
impl GoDebugAdapter {
    ...
    async fn install_shim(&self, delegate: &Arc<dyn DapDelegate>) -> anyhow::Result<PathBuf> {
        ...
        let adapter_dir = carrot_paths::debug_adapters_dir().join("delve-shim-dap");
...
impl DebugAdapter for GoDebugAdapter {
    ...
    async fn config_from_carrot_format(
        ...
        let mut args = match &carrot_scenario.request {
            carrot_dap::DebugRequest::Attach(attach_config) => {
                json!({
                    "request": "attach",
                    "mode": "local",
            ...
            carrot_dap::DebugRequest::Launch(launch_config) => {
