...
use carrot_editor::code_context_menus::{CodeContextMenu, ContextMenuOrigin};
...
use carrot_project::{DisableAiSettings, project_settings::DiagnosticSeverity};
...
use carrot_vim_mode_setting::{HelixModeSetting, VimModeSetting};
use carrot_workspace::item::ItemBufferKind;
use carrot_workspace::{
    ToolbarItemEvent, ToolbarItemLocation, ToolbarItemView, Workspace, item::ItemHandle,
};
use carrot_actions::{agent::AddSelectionToThread, assistant::InlineAssist, outline::ToggleOutline};
                        ...
                        |mut menu, _, _| {
                            ...
                            if supports_inlay_hints {
                                menu = menu.toggleable_entry(
                                    ...
                                    inlay_hints_enabled,
                                    IconPosition::Start,
                                    Some(carrot_editor::actions::ToggleInlayHints.boxed_clone()),
                                    {
                                            ...
                                            editor
                                                .update(cx, |editor, cx| {
                                                    editor.toggle_inlay_hints(
                                                        &carrot_editor::actions::ToggleInlayHints,
                                                        window,
                                                        cx,
                                                    );
                                                })
                                ...
                                menu = menu.toggleable_entry(
                                    ...
                                    IconPosition::Start,
                                    Some(carrot_editor::actions::ToggleInlineValues.boxed_clone()),
                                    {
                                        let editor = editor.clone();
                                        move |window, cx| {
                                            editor
                                                .update(cx, |editor, cx| {
                                                    editor.toggle_inline_values(
                                                        &carrot_editor::actions::ToggleInlineValues,
                                                        window,
                                                        cx,
                                                    );
                                                })
                            ...
                            if supports_semantic_tokens {
                                menu = menu.toggleable_entry(
                                    ...
                                    IconPosition::Start,
                                    Some(carrot_editor::actions::ToggleSemanticHighlights.boxed_clone()),
                                    {
                                        let editor = editor.clone();
                                        move |window, cx| {
                                            editor
                                                .update(cx, |editor, cx| {
                                                    editor.toggle_semantic_highlights(
                                                        &carrot_editor::actions::ToggleSemanticHighlights,
                                                        window,
                                                        cx,
                                                    );
                                                })
                            ...
                            if supports_minimap {
                                menu = menu.toggleable_entry("Minimap", minimap_enabled, IconPosition::Start, Some(carrot_editor::actions::ToggleMinimap.boxed_clone()), {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                                                editor.toggle_minimap(
                                                    &carrot_editor::actions::ToggleMinimap,
                                                    window,
                                                    cx,
                                                );
                                            })
                            ...
                            if has_edit_prediction_provider {
                                let mut edit_prediction_entry = ContextMenuEntry::new("Edit Predictions")
                                    ...
                                    .action(
                                        carrot_editor::actions::ToggleEditPrediction.boxed_clone(),
                                    ).handler({
                                        let editor = editor.clone();
                                        move |window, cx| {
                                            editor
                            ...
                            menu = menu.toggleable_entry(
                                ...
                                show_line_numbers,
                                IconPosition::Start,
                                Some(carrot_editor::actions::ToggleLineNumbers.boxed_clone()),
                                {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                                                editor.toggle_line_numbers(
                                                    &carrot_editor::actions::ToggleLineNumbers,
                                                    window,
                                                    cx,
                                                );
                                            })
                            ...
                            menu = menu.toggleable_entry(
                                ...
                                IconPosition::Start,
                                Some(carrot_editor::actions::ToggleSelectionMenu.boxed_clone()),
                                {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                                                editor.toggle_selection_menu(
                                                    &carrot_editor::actions::ToggleSelectionMenu,
                                                    window,
                                                    cx,
                                                )
                                            })
                            ...
                            menu = menu.toggleable_entry(
                                ...
                                IconPosition::Start,
                                Some(carrot_editor::actions::ToggleAutoSignatureHelp.boxed_clone()),
                                {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                                                editor.toggle_auto_signature_help_menu(
                                                    &carrot_editor::actions::ToggleAutoSignatureHelp,
                                                    window,
                                                    cx,
                                                );
                                            })
                            ...
                            menu = menu.toggleable_entry(
                                ...
                                IconPosition::Start,
                                Some(carrot_editor::actions::ToggleGitBlameInline.boxed_clone()),
                                {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
                                            .update(cx, |editor, cx| {
                                                editor.toggle_git_blame_inline(
                                                    &carrot_editor::actions::ToggleGitBlameInline,
                                                    window,
                                                    cx,
                                                )
                                            })
                            ...
                            menu = menu.toggleable_entry(
                                ...
                                IconPosition::Start,
                                Some(carrot_git::Blame.boxed_clone()),
                                {
                                    let editor = editor.clone();
                                    move |window, cx| {
                                        editor
