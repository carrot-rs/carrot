...
use carrot_codestral::{CodestralEditPredictionDelegate, load_codestral_api_key};
use inazuma_collections::HashMap;
use carrot_copilot::CopilotEditPredictionDelegate;
use carrot_edit_prediction::{EditPredictionModel, CarrotEditPredictionDelegate};
use carrot_editor::Editor;
use inazuma::{AnyWindowHandle, App, AppContext as _, Context, Entity, WeakEntity};
use carrot_language::language_settings::{EditPredictionProvider, all_language_settings};

use inazuma_settings_framework::{EditPredictionPromptFormat, SettingsStore};
use std::{cell::RefCell, rc::Rc, sync::Arc};
use carrot_ui::Window;

pub fn init(client: Arc<Client>, user_store: Entity<UserStore>, cx: &mut App) {
    carrot_edit_prediction::EditPredictionStore::global(&client, &user_store, cx);
    ...
    cx.subscribe(&user_store, {
        ...
        move |user_store, event, cx| {
            if let carrot_client::user::Event::PrivateUserInfoUpdated = event {
                let provider_config = edit_prediction_provider_config_for_settings(cx);
...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        EditPredictionProvider::Carrot => {
            Some(EditPredictionProviderConfig::Carrot(EditPredictionModel::Zeta))
        }
        ...
        EditPredictionProvider::Ollama | EditPredictionProvider::OpenAiCompatibleApi => {
            ...
            if matches!(
                ...
                Some(EditPredictionProviderConfig::Carrot(
                    EditPredictionModel::Fim { format },
                ))
            }
        }

        EditPredictionProvider::Mercury => Some(EditPredictionProviderConfig::Carrot(
...
impl EditPredictionProviderConfig {
    fn name(&self) -> &'static str {
        match self {
            ...
            EditPredictionProviderConfig::Carrot(model) => match model {
...
fn clear_edit_prediction_store_edit_history(_: &carrot_edit_prediction::ClearHistory, cx: &mut App) {
    if let Some(ep_store) = carrot_edit_prediction::EditPredictionStore::try_global(cx) {
...
fn register_backward_compatible_actions(editor: &mut Editor, cx: &mut Context<Editor>) {
    ...
    editor
        .register_action(cx.listener(
            |editor, _: &carrot_copilot::Suggest, window: &mut Window, cx: &mut Context<Editor>| {
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<CarrotEditPredictionDelegate>(None, window, cx);
        }
        Some(EditPredictionProviderConfig::Copilot) => {
            let ep_store = carrot_edit_prediction::EditPredictionStore::global(client, &user_store, cx);
        ...
        Some(EditPredictionProviderConfig::Carrot(model)) => {
            let ep_store = carrot_edit_prediction::EditPredictionStore::global(client, &user_store, cx);
            ...
            if let Some(project) = editor.project() {
                ...
                let provider = cx.new(|cx| {
                    CarrotEditPredictionDelegate::new(
                        project.clone(),
                        singleton_buffer,
                        &client,
                        &user_store,
...
mod tests {
    use super::*;
    use carrot_editor::MultiBuffer;
    use inazuma::{BorrowAppContext, TestAppContext};
    use inazuma_settings_framework::{EditPredictionProvider, SettingsStore};
    ...
    async fn test_subscribe_uses_stale_provider_config_after_settings_change(
        ...
        let app_state = cx.update(|cx| {
            ...
            carrot_language_model::init(app_state.user_store.clone(), app_state.client.clone(), cx);
            carrot_editor::init(cx);
            app_state
        });

        // Override the default provider to None so the subscribe closure
        // captures None at init time. (The test default is Carrot/Zeta1, which
        ...
        let editor = cx.add_window(|window, cx| {
            let buffer = cx.new(|_cx| MultiBuffer::new(carrot_language::Capability::ReadWrite));
            Editor::new(carrot_editor::EditorMode::full(), buffer, None, window, cx)
        });
        ...
        cx.update(|cx| {
            app_state.user_store.update(cx, |_, cx| {
                cx.emit(carrot_client::user::Event::PrivateUserInfoUpdated);
            });
        });
        cx.run_until_parked();
