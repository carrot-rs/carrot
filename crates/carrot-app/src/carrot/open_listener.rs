...
use carrot_agent_ui::ExternalSourcePrompt;
use anyhow::{Context as _, Result, anyhow};
use carrot_cli::{CliRequest, CliResponse, ipc::IpcSender};
use carrot_cli::{IpcHandshake, ipc};
use carrot_client::{CarrotLink, parse_carrot_link};
...
use carrot_git_ui::{file_diff_view::FileDiffView, multi_diff_view::MultiDiffView};
use inazuma::{App, AsyncApp, Global, WindowHandle};
use carrot_onboarding::FIRST_OPEN;
use carrot_onboarding::show_onboarding_view;
use carrot_recent_projects::{RemoteSettings, navigate_to_positions, open_remote_project};
use carrot_remote::{RemoteConnectionOptions, WslConnectionOptions};
use inazuma_settings_framework::Settings;
use std::path::{Path, PathBuf};
use std::sync::Arc;
use std::thread;
use std::time::Duration;
use carrot_ui::SharedString;
use inazuma_util::ResultExt;
use inazuma_util::paths::PathWithPosition;
use carrot_workspace::PathList;
use carrot_workspace::item::ItemHandle;
use carrot_workspace::{AppState, MultiWorkspace, OpenOptions, OpenResult, SerializedWorkspaceLocation};
...
impl OpenRequest {
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("carrot-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("carrot-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("carrot://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("carrot://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("carrot://extension/") {
                this.kind = Some(OpenRequestKind::Extension {
                    extension_id: extension_id.to_string(),
                });
            } else if let Some(session_id_str) = url.strip_prefix("carrot://agent/shared/") {
            ...
            } else if let Some(agent_path) = url.strip_prefix("carrot://agent") {
                this.parse_agent_url(agent_path)
            } else if let Some(schema_path) = url.strip_prefix("carrot://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "carrot://settings" || url == "carrot://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("carrot://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("carrot://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("carrot://git/commit/") {
                this.parse_git_commit_url(commit_path)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(carrot_link) = parse_carrot_link(&url, cx) {
                match carrot_link {
                    CarrotLink::Channel { channel_id } => {
                        this.join_channel = Some(channel_id);
                    }
                    CarrotLink::ChannelNotes {
                        channel_id,
                        heading,
                    } => {
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    use carrot_release_channel::RELEASE_CHANNEL_NAME;
    use std::os::unix::net::UnixDatagram;

    let sock_path = carrot_paths::data_dir().join(format!("carrot-{}.sock", *RELEASE_CHANNEL_NAME));
...
fn connect_to_cli(
    ...
    let handshake_tx = carrot_cli::ipc::IpcSender::<IpcHandshake>::connect(server_name.to_string())
...
pub async fn open_paths_with_positions(
    ...
    open_options: carrot_workspace::OpenOptions,
    cx: &mut AsyncApp,
) -> Result<(
    ...
    let OpenResult {
        ...
        .update(|cx| carrot_workspace::open_paths(&paths, app_state, open_options, cx))
        .await?;
    ...
    if grouped_locations.is_empty() {
        ...
        // If not the first launch, show an empty window with empty editor
            ...
            cx.update(|cx| {
                ...
                carrot_workspace::open_new(open_options, app_state, cx, |workspace, window, cx| {
    ...
    for (location, workspace_paths) in grouped_locations {
        ...
        let (open_new_workspace, replace_window) = if reuse {
            (
                Some(true),
                cx.update(|cx| {
                    carrot_workspace::workspace_windows_for_location(&location, cx)
                        .into_iter()
                        .next()
                }),
            )
        ...
        let open_options = carrot_workspace::OpenOptions {
            open_new_workspace,
            replace_window,
            wait,
            env: env.clone(),
...
mod tests {
    use super::*;
    use crate::carrot::{open_listener::open_local_workspace, tests::init_test};
    use carrot_cli::{
        CliResponse,
        ipc::{self},
    };
    use carrot_editor::Editor;
    use futures::poll;
    use inazuma::{AppContext as _, TestAppContext};
    use carrot_language::LineEnding;
    use carrot_remote::SshConnectionOptions;
    use inazuma_rope::Rope;
    use serde_json::json;
    use std::{sync::Arc, task::Poll};
    use inazuma_util::path;
    use carrot_workspace::{AppState, MultiWorkspace};
    ...
    fn agent_url_with_prompt(prompt: &str) -> String {
        let mut serializer = url::form_urlencoded::Serializer::new("carrot://agent?".to_string());
    ...
    fn test_parse_shared_agent_thread_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![format!("carrot://agent/shared/{session_id}")],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_shared_agent_thread_url_with_invalid_uuid(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["carrot://agent/shared/not-a-uuid".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["carrot://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["carrot://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["carrot://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["carrot://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell Carrot to open a new window
        open_workspace_file(
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Carrot to add it to the current workspace
        ...
        // but tell Carrot to NOT add it to the current workspace
    ...
    async fn test_reuse_flag_functionality(cx: &mut TestAppContext) {
        ...
        let window_to_replace = carrot_workspace::find_existing_workspace(
            &paths,
            &carrot_workspace::OpenOptions::default(),
            &carrot_workspace::SerializedWorkspaceLocation::Local,
            &mut cx.to_async(),
        )
        .await
        .0
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "carrot://git/clone/?repo=https://github.com/example-user/example-repo.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
        ...
        match request.kind {
            Some(OpenRequestKind::GitClone { repo_url }) => {
                assert_eq!(repo_url, "https://github.com/example-user/example-repo.git");
            }
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "carrot://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fexample-user%2Fexample-repo.git"
                            .into(),
                    ],
                    ..Default::default()
                },
