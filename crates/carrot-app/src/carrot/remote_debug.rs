use carrot_actions::remote_debug::{SimulateDisconnect, SimulateTimeout, SimulateTimeoutExhausted};
...
pub fn init(cx: &mut inazuma::App) {
    cx.observe_new(|workspace: &mut Workspace, _, cx| {
        ...
        workspace.register_action(move |_, _: &SimulateTimeoutExhausted, _window, cx| {
            ...
            log::info!("SimulateTimeout: forcing heartbeat timeout on remote connection");
            remote_client.update(cx, |client, cx| {
                client.force_heartbeat_timeout(
                    carrot_remote::remote_client::MAX_RECONNECT_ATTEMPTS,
                    cx,
                );
            });
        });
