//! Visual testing infrastructure for Carrot.
//!
//! This module provides utilities for visual regression testing of Carrot's UI.
//! It allows capturing screenshots of the real Carrot application window and comparing
...
//! cargo test -p carrot-app visual_tests -- --ignored --test-threads=1
//!
//! # Update baselines when UI intentionally changes
//! UPDATE_BASELINES=1 cargo test -p carrot-app visual_tests -- --ignored --test-threads=1
//! ```
//!
//! ## Screenshot Output
//!
...
/// Initialize a visual test context with all necessary subsystems.
...
pub async fn open_test_workspace(
...
) -> Result<WindowHandle<carrot_workspace::Workspace>> {
    let window_size = default_window_size();

    let project = cx.update(|cx| {
        carrot_project::Project::local(
    ...
    let window = cx.open_offscreen_window(window_size, |window, cx| {
        cx.new(|cx| {
            carrot_workspace::Workspace::new(None, project.clone(), app_state.clone(), window, cx)
        })
    })?;
...
pub fn default_window_size() -> Size<inazuma::Pixels> {
...
mod tests {
    ...
    fn test_visual_test_smoke() {
        let mut cx = VisualTestAppContext::new(inazuma::current_platform(false));
    ...
    /// This test captures a screenshot of an empty workspace.
    ///
    ...
    /// Full baseline regression test: captures the workspace window and either
    /// compares it against a saved baseline or updates it when `UPDATE_BASELINES=1`
    /// is set. Exercises the complete visual-testing pipeline
    /// (`capture_and_save_screenshot` → `assert_or_update_baseline` →
    /// `save_baseline` / `assert_screenshot_matches` → `load_image`).
    ///
    /// Ignored by default for the same reasons as the other visual tests
    /// (macOS + Screen Recording permission + main-thread requirement).
    #[test]
    #[ignore]
    fn test_workspace_matches_baseline() {
        let mut cx = VisualTestAppContext::new(inazuma::current_platform(false));
        let app_state = init_visual_test(&mut cx);
        ...
        let actual_path = output_dir.join("workspace_matches_baseline.actual.png");
        let baseline_path = output_dir.join("workspace_matches_baseline.png");

        smol::block_on(async {
            let screenshot_result =
                capture_and_save_screenshot(&mut cx, workspace.into(), Some(&actual_path)).await;
            ...
            // On the very first run (no baseline yet), seed the baseline and
            // skip comparison — the next run will validate against it.
            if !baseline_path.exists() && !should_update_baselines() {
                save_baseline(&screenshot, &baseline_path)
                    .expect("Failed to seed initial baseline");
                eprintln!(
                    "Seeded initial baseline at {}. Re-run to validate.",
                    baseline_path.display()
                );
                return;
            }

            // Tolerance: accept if >= 99% of pixels match, with per-channel
            // slack of 5/255 to absorb anti-aliasing noise.
            assert_or_update_baseline(&screenshot, &baseline_path, 0.01, 5)
                .expect("Workspace screenshot did not match baseline");
        });
