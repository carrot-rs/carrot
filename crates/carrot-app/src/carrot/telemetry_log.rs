...
use carrot_client::telemetry::Telemetry;
use carrot_fs::Fs;
use carrot_language::LanguageRegistry;
use carrot_markdown::{CodeBlockRenderer, Markdown, MarkdownElement, MarkdownStyle};
use carrot_project::Project;
use carrot_telemetry_events::{Event, EventWrapper};
...
use carrot_workspace::{
    Item, ItemHandle, PaneRole, Toast, ToolbarItemEvent, ToolbarItemLocation, ToolbarItemView,
    Workspace, notifications::NotificationId,
};
use futures::StreamExt;
use inazuma::{
    App, Empty, Entity, EventEmitter, FocusHandle, Focusable, ListAlignment, ListState,
    StyleRefinement, Task, TextStyleRefinement, Window, list,
};
    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace.register_action(
                |workspace, _: &carrot_actions::OpenTelemetryLog, window, cx| {
                    let telemetry_log =
...
impl TelemetryLogView {
    pub fn new(project: Entity<Project>, _window: &mut Window, cx: &mut Context<Self>) -> Self {
        let telemetry = carrot_client::Client::global(cx).telemetry().clone();
    ...
    fn render_entry(
        ...
        let selection_background_color = colors.element_selection;
        let syntax = theme.syntax().clone();
        let expanded = self.expanded.contains(&event_index);

        let local_timezone =
            time::UtcOffset::current_local_offset().unwrap_or(time::UtcOffset::UTC);
        let timestamp_str = carrot_time_format::format_localized_timestamp(
            entry.received_at,
            OffsetDateTime::now_utc(),
            local_timezone,
            carrot_time_format::TimestampFormat::EnhancedAbsolute,
        );
            ...
            .child(
                h_flex()
                    ...
                    .when(signed_in, |this| {
                        this.child(
                            div()
                                .child(carrot_ui::Chip::new("signed in"))
...
pub struct TelemetryLogToolbarItemView {
    ...
    search_editor: Entity<carrot_editor::Editor>,
}

impl TelemetryLogToolbarItemView {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        let search_editor = cx.new(|cx| {
            let mut editor = carrot_editor::Editor::single_line(window, cx);
        ...
        cx.subscribe(
            &search_editor,
            |this, editor, event: &carrot_editor::EditorEvent, cx| {
