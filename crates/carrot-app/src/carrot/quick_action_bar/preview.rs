...
use inazuma::{AnyElement, Modifiers, WeakEntity};
use carrot_markdown_preview::{
        ...
        let (button_id, tooltip_text, open_action, open_to_side_action, open_action_for_tooltip) =
            match preview_type {
                PreviewType::Markdown => (
                    ...
                    "Preview Markdown",
                    Box::new(MarkdownOpenPreview) as Box<dyn inazuma::Action>,
                    Box::new(MarkdownOpenPreviewToTheSide) as Box<dyn inazuma::Action>,
                    &carrot_markdown_preview::OpenPreview as &dyn inazuma::Action,
                ),
                PreviewType::Svg => (
                    "toggle-svg-preview",
                    "Preview SVG",
                    Box::new(SvgOpenPreview) as Box<dyn inazuma::Action>,
                    Box::new(SvgOpenPreviewToTheSide) as Box<dyn inazuma::Action>,
                    &carrot_svg_preview::OpenPreview as &dyn inazuma::Action,
                ),
                PreviewType::Csv => (
                    "toggle-csv-preview",
                    "Preview CSV",
                    Box::new(CsvOpenPreview) as Box<dyn inazuma::Action>,
                    Box::new(CsvOpenPreviewToTheSide) as Box<dyn inazuma::Action>,
                    &carrot_csv_preview::OpenPreview as &dyn inazuma::Action,
                ),
            };

        let alt_click = inazuma::Keystroke {
            key: "click".into(),
