...
const CARROT_REPL_DOCUMENTATION: &str = "https://carrot.dev/docs/repl";
...
impl QuickActionBar {
    pub fn render_repl_menu(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        let session = carrot_repl::session(editor.downgrade(), cx);
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    .custom_entry(
                        ...
                        },
                        {
                            let editor = editor.clone();
                            move |window, cx| {
                                carrot_repl::run(editor.clone(), true, window, cx).log_err();
                            }
                        },
                    )
                    .custom_entry(
                        ...
                        },
                        {
                            let editor = editor.clone();
                            move |_, cx| {
                                carrot_repl::interrupt(editor.clone(), cx);
                            }
                        },
                    )
                    .custom_entry(
                        ...
                        },
                        {
                            let editor = editor.clone();
                            move |_, cx| {
                                carrot_repl::clear_outputs(editor.clone(), cx);
                            }
                        },
                    )
                    .separator()
                    .custom_entry(
                        ...
                        },
                        {
                            let editor = editor.clone();
                            move |window, cx| {
                                carrot_repl::shutdown(editor.clone(), window, cx);
                            }
                        },
                    )
                    .custom_entry(
                        ...
                        {
                            move |window, cx| {
                                carrot_repl::restart(editor.clone(), window, cx);
                            }
                        },
                    )
                    .separator()
                    .action("View Sessions", Box::new(carrot_repl::Sessions))
                    // TODO: Add shut down all kernels action
                    // .action("Shut Down all Kernels", Box::new(inazuma::NoAction))
                })
                .into()
            })
        ...
        let button = ButtonLike::new_rounded_left("toggle_repl_icon")
            ...
            .on_click(|_, window, cx| window.dispatch_action(Box::new(carrot_repl::Run {}), cx))
            .into_any_element();
                ...
                .child(
                    IconButton::new("toggle_repl_icon", IconName::ReplNeutral)
                        ...
                        .on_click(|_, window, cx| {
                            window.dispatch_action(Box::new(carrot_repl::Run {}), cx)
                        }),
                )
                .into_any_element(),
        )
    ...
    pub fn render_kernel_selector(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let store = carrot_repl::ReplStore::global(cx);
        ...
        KernelSelector::new(
            {
                Box::new(move |kernelspec, window, cx| {
                    if kernelspec.has_ipykernel() {
                        carrot_repl::assign_kernelspec(kernelspec, editor.downgrade(), window, cx).ok();
                    } else {
                        carrot_repl::install_ipykernel_and_assign(
                            kernelspec,
                            editor.downgrade(),
                            window,
                            cx,
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip: SharedString = SharedString::from(format!("Setup Carrot REPL for {}", language));
        Some(
            h_flex()
                ...
                .child(
                    IconButton::new("toggle_repl_icon", IconName::ReplNeutral)
                        ...
                        .on_click(|_, _window, cx| {
                            cx.open_url(&format!("{}#installation", CARROT_REPL_DOCUMENTATION))
                        }),
                )
                .into_any_element(),
        )
