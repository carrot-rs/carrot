...
use carrot_client::{Client, telemetry::MINIDUMP_ENDPOINT};
use carrot_feature_flags::FeatureFlagAppExt;
use futures::{AsyncReadExt, TryStreamExt};
use inazuma::{App, AppContext as _, SerializedThreadTaskTimings};
use carrot_http_client::{self, AsyncBody, HttpClient, Request};
use log::info;
use carrot_project::Project;
use carrot_proto::{CrashReport, GetCrashFilesResponse};
use reqwest::{
    Method,
    multipart::{Form, Part},
};
    ...
    cx.observe_new(move |project: &mut Project, _, cx| {
        ...
        remote_client.update(cx, |remote_client, cx| {
            ...
            let request = remote_client
                .proto_client()
                .request(carrot_proto::GetCrashFiles {});
            cx.background_spawn(async move {
...
fn cleanup_old_hang_traces() {
    if let Ok(entries) = std::fs::read_dir(carrot_paths::hang_traces_dir()) {
...
fn save_hang_trace(
    ...
    background_executor: &inazuma::BackgroundExecutor,
    ...
    let trace_path = carrot_paths::hang_traces_dir().join(&format!(
        "hang-{}.miniprof.json",
...
async fn upload_minidump(
    ...
    endpoint: &str,
    minidump: Vec<u8>,
    metadata: &carrot_crashes::CrashInfo,
) -> Result<()> {
    let mut form = Form::new()
        ...
        .text("sentry[tags][version]", metadata.init.carrot_version.clone())
    ...
    ::carrot_telemetry::event!(
        ...
        crashed_version = metadata.init.carrot_version.clone(),
    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        let carrot_system_specs::GpuInfo {
            device_name,
            device_pci_id,
            vendor_name,
            vendor_pci_id,
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Carrot is running on",
            )
...
// users - except - we know internal users use Carrot.
...
async fn upload_build_timings(_client: Arc<Client>) -> Result<()> {
    let build_timings_dir = carrot_paths::data_dir().join("build_timings");
