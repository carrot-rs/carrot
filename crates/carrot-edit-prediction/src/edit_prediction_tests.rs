...
use carrot_client::{UserStore, test::FakeServer};
use carrot_cloud_api_types::{CreateLlmTokenResponse, LlmToken};
...
use carrot_language_model::RefreshLlmTokenListener;
...
macro_rules! assert_matches {
    ...
    ($expression:expr, $pattern:pat if $guard:expr $(,)?) => {
        assert!(
            ...
            "assertion failed: `{:?}` does not match `{} if {}`",
            &$expression,
            stringify!($pattern),
            stringify!($guard),
        )
...
use carrot_workspace::{AppState, CollaboratorId, Workspace};
use carrot_zeta_prompt::ZetaPromptInput;
...
async fn test_current_state(cx: &mut TestAppContext) {
    ...
    let diagnostic = carrot_lsp::Diagnostic {
        range: carrot_lsp::Range::new(
            carrot_lsp::Position::new(1, 1),
            carrot_lsp::Position::new(1, 5),
        ),
            ...
            lsp_store
                .update_diagnostics(
                    ...
                    carrot_lsp::PublishDiagnosticsParams {
                        uri: carrot_lsp::Uri::from_file_path(path!("/root/2.txt")).unwrap(),
                        diagnostics: vec![diagnostic],
                        version: None,
                    },
                    None,
                    carrot_language::DiagnosticSourceKind::Pushed,
                    &[],
                    cx,
                )
                .unwrap();
...
async fn test_edit_history_getter_pause_splits_last_event(cx: &mut TestAppContext) {
    ...
    let carrot_zeta_prompt::Event::BufferChange { diff, .. } = events[0].event.as_ref();
    assert_eq!(
        diff.as_str(),
        indoc! {"
            @@ -1,3 +1,3 @@
    ...
    let carrot_zeta_prompt::Event::BufferChange { diff, .. } = events[1].event.as_ref();
    assert_eq!(
        diff.as_str(),
        indoc! {"
            @@ -1,3 +1,3 @@
...
fn render_events(events: &[StoredEvent]) -> String {
    events
        .iter()
        .map(|e| {
            let carrot_zeta_prompt::Event::BufferChange { diff, .. } = e.event.as_ref();
            diff.as_str()
        })
        .collect::<Vec<_>>()
        .join("\n---\n")
...
fn render_events_with_predicted(events: &[StoredEvent]) -> Vec<String> {
    events
        .iter()
        .map(|e| {
            let carrot_zeta_prompt::Event::BufferChange {
                diff, predicted, ..
            } = e.event.as_ref();
...
async fn apply_collaborator_edit(
    ...
    *since_version = collaborator.read_with(cx, |collaborator, _cx| collaborator.version());
    ...
    buffer.update(cx, |buffer, cx| {
        buffer.apply_ops(
            ops.into_iter()
                .map(|op| carrot_language::proto::deserialize_operation(op).unwrap()),
            cx,
        );
    });
}
        ...
        project.lsp_store().update(cx, |lsp_store, cx| {
            lsp_store
                .update_diagnostics(
                    LanguageServerId(0),
                    carrot_lsp::PublishDiagnosticsParams {
                        uri: carrot_lsp::Uri::from_file_path(path!("/root/bar.md")).unwrap(),
                        diagnostics: vec![diagnostic],
                        version: None,
                    },
                    None,
...
fn test_active_buffer_diagnostics_fetching(cx: &mut TestAppContext) {
            ...
            diagnostic_ranges
                ...
                .map(|(index, range)| DiagnosticEntry {
                    ...
                    diagnostic: Diagnostic {
                        ...
                        group_id: index + 1,
                        is_primary: true,
                        source_kind: carrot_language::DiagnosticSourceKind::Pushed,
                        ..Diagnostic::default()
                    },
                }),
            &snapshot,
    ...
    assert_eq!(
        ...
        vec![carrot_zeta_prompt::ActiveBufferDiagnostic {
            severity: Some(1),
...
fn model_response(request: &PredictEditsV3Request, diff_to_apply: &str) -> PredictEditsV3Response {
    let editable_range = carrot_zeta_prompt::excerpt_range_for_format(
        Default::default(),
...
fn prompt_from_request(request: &PredictEditsV3Request) -> String {
    carrot_zeta_prompt::format_zeta_prompt(
        &request.input,
        carrot_zeta_prompt::ZetaFormat::default(),
    )
...
fn init_test_with_fake_client(
    ...
    cx.update(move |cx| {
        ...
        let client = carrot_client::Client::new(Arc::new(FakeSystemClock::new()), http_client, cx);
...
async fn test_unauthenticated_without_custom_url_blocks_prediction_impl(cx: &mut TestAppContext) {
    ...
    let http_client = FakeHttpClient::create(|_req| async move {
        Ok(inazuma::http_client::Response::builder()
            .status(401)
    ...
    let client = cx
        .update(|cx| carrot_client::Client::new(Arc::new(FakeSystemClock::new()), http_client, cx));
    let user_store = cx.update(|cx| cx.new(|cx| UserStore::new(client.clone(), cx)));
    cx.update(|cx| {
        RefreshLlmTokenListener::register(client.clone(), user_store.clone(), cx);
    });
...
async fn test_diagnostic_jump_excludes_collaborator_regions(cx: &mut TestAppContext) {
    ...
    fn publish_diagnostics(
        ...
        let diagnostics: Vec<_> = rows
            .iter()
            .map(|&row| carrot_lsp::Diagnostic {
                range: carrot_lsp::Range::new(
                    carrot_lsp::Position::new(row, 0),
                    carrot_lsp::Position::new(row, 5),
                ),
                severity: Some(carrot_lsp::DiagnosticSeverity::ERROR),
            ...
            project.lsp_store().update(cx, |lsp_store, cx| {
                lsp_store
                    .update_diagnostics(
                        LanguageServerId(0),
                        carrot_lsp::PublishDiagnosticsParams {
                            uri: carrot_lsp::Uri::from_file_path(uri_path).expect("invalid uri"),
                            diagnostics,
                            version: None,
                        },
                        None,
