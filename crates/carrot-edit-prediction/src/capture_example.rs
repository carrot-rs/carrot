...
pub fn capture_example(
    ...
    Some(cx.spawn(async move |mut cx| {
        ...
        events.retain(|stored_event| {
            let carrot_zeta_prompt::Event::BufferChange { path, .. } = stored_event.event.as_ref();
...
fn write_event_with_relative_paths(
    output: &mut String,
    event: &carrot_zeta_prompt::Event,
    root_name: &str,
) {
...
fn compute_cursor_excerpt(
    ...
    let (_, context_range) = carrot_zeta_prompt::compute_editable_and_context_ranges(
        &excerpt_text,
        cursor_offset_in_excerpt,
        &syntax_ranges,
        100,
...
async fn collect_snapshots(
    project: &Entity<Project>,
    git_store: &Entity<carrot_project::git_store::GitStore>,
...
fn compute_uncommitted_diff(
    ...
    for (relative_path, (before_text, diff_snapshot)) in snapshots_by_path {
        if let Some(head_text) = &diff_snapshot.base_text_string() {
            let file_diff = carrot_language::unified_diff(head_text, &before_text.text());
...
mod tests {
    ...
    use carrot_language::{Anchor, Point};
    use carrot_project::{FakeFs, Project};
    use inazuma::{AppContext as _, TestAppContext, http_client::FakeHttpClient};
        ...
        assert!(
            matches!(
                events
                    ...
                    .event
                    .as_ref(),
                carrot_zeta_prompt::Event::BufferChange { path, .. } if path.as_ref() == "/external/external.rs"
            ),
