...
use carrot_language::{Anchor, Buffer, OffsetRangeExt as _, TextBufferSnapshot, text_diff};
use carrot_project::Project;
use carrot_worktree::Worktree;
use inazuma::{AsyncApp, Entity};
use inazuma_collections::{HashMap, hash_map::Entry};
...
fn resolve_hunk_edits_in_buffer(
    ...
    let iter = hunk.edits.into_iter().flat_map(move |edit| {
        let old_text = buffer
            ...
            .collect::<String>();
        let edits_within_hunk = carrot_language::text_diff(&old_text, &edit.text);
        edits_within_hunk
            .into_iter()
