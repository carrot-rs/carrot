...
use carrot_cloud_llm_client::EditPredictionRejectReason;
use carrot_language::{ToOffset, ToPoint as _};
use carrot_language_model::{ApiKeyState, EnvVar, env_var};
...
impl Mercury {
    ...
    pub(crate) fn request_prediction(
        ...
        let result = cx.background_spawn(async move {
            ...
            let related_files = carrot_zeta_prompt::filter_redundant_excerpts(
                related_files,
                full_path.as_ref(),
            ...
            let inputs = carrot_zeta_prompt::ZetaPromptInput {
                events,
            ...
            let request_body = carrot_open_ai::Request {
                model: "mercury-coder".into(),
                messages: vec![carrot_open_ai::RequestMessage::User {
                    content: carrot_open_ai::MessageContent::Plain(prompt),
                }],
                stream: false,
                stream_options: None,
            ...
            let mut response: carrot_open_ai::Response =
    ...
    push_delimited(
        ...
        |prompt| {
            for event in inputs.events.iter() {
                carrot_zeta_prompt::write_event(prompt, &event);
            }
        },
    );
...
pub fn load_mercury_api_token(
    cx: &mut App,
) -> Task<Result<(), carrot_language_model::AuthenticateError>> {
