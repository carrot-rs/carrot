use anyhow::Result;
use carrot_client::{Client, EditPredictionUsage, UserStore};
use carrot_cloud_api_types::{OrganizationId, SubmitEditPredictionFeedbackBody};
use carrot_cloud_llm_client::predict_edits_v3::{
    PredictEditsV3Request, PredictEditsV3Response, RawCompletionRequest, RawCompletionResponse,
};
use carrot_cloud_llm_client::{
    CARROT_VERSION_HEADER_NAME, EditPredictionRejectReason, EditPredictionRejection,
    MAX_EDIT_PREDICTION_REJECTIONS_PER_REQUEST, MINIMUM_REQUIRED_VERSION_HEADER_NAME,
    PredictEditsRequestTrigger, RejectEditPredictionsBodyRef,
};
use carrot_copilot::{Copilot, Reinstall, SignIn, SignOut};
use carrot_db::kvp::{Dismissable, KeyValueStore};
use carrot_edit_prediction_context::{RelatedExcerptStore, RelatedExcerptStoreEvent, RelatedFile};
use carrot_feature_flags::{FeatureFlag, FeatureFlagAppExt as _};
use carrot_language::language_settings::all_language_settings;
use carrot_language::{Anchor, Buffer, File, Point, TextBufferSnapshot, ToOffset, ToPoint};
use carrot_language::{BufferSnapshot, OffsetRangeExt};
use carrot_language_model::{LlmApiToken, NeedsLlmTokenRefresh};
use carrot_project::{DisableAiSettings, Project, ProjectPath, WorktreeId};
use carrot_release_channel::AppVersion;
use carrot_workspace::Workspace;
use carrot_zeta_prompt::{ZetaFormat, ZetaPromptInput};
use futures::{
...
use inazuma_util::{RangeExt as _, ResultExt as _};
use thiserror::Error;
...
mod capture_example;
mod carrot_edit_prediction_delegate;
...
use crate::onboarding_modal::CarrotPredictModal;
pub use crate::prediction::EditPrediction;
pub use crate::prediction::EditPredictionId;
use crate::prediction::EditPredictionResult;
pub use capture_example::capture_example;
pub use carrot_edit_prediction_delegate::CarrotEditPredictionDelegate;
pub use carrot_language_model::ApiKeyState;
pub use carrot_telemetry_events::EditPredictionRating;
...
const CARROT_PREDICT_DATA_COLLECTION_CHOICE: &str = "carrot_predict_data_collection_choice";
...
pub struct EditPredictionModelInput {
    ...
    position: Anchor,
    events: Vec<Arc<carrot_zeta_prompt::Event>>,
...
pub struct StoredEvent {
    pub event: Arc<carrot_zeta_prompt::Event>,
...
impl StoredEvent {
    fn can_merge(
        ...
        let a_is_predicted = matches!(
            self.event.as_ref(),
            carrot_zeta_prompt::Event::BufferChange {
                predicted: true,
                ..
            }
        );
...
struct ProjectState {
    ...
    registered_buffers: HashMap<inazuma::EntityId, RegisteredBuffer>,
...
struct CurrentEditPrediction {
    ...
    pub was_shown: bool,
    pub shown_with: Option<carrot_edit_prediction_types::SuggestionDisplayType>,
...
impl LastEvent {
    pub fn finalize(
        ...
        if path == old_path && diff.is_empty() {
            ...
            Some(StoredEvent {
                event: Arc::new(carrot_zeta_prompt::Event::BufferChange {
                    old_path,
                    path,
                    diff,
                    in_open_source_repo,
...
fn compute_diff_between_snapshots_in_range(
    ...
    let diff = carrot_language::unified_diff_with_offsets(
        &old_region_text,
        &new_region_text,
...
impl EditPredictionStore {
    ...
    fn zeta2_raw_config_from_env() -> Option<Zeta2RawConfig> {
        let version_str = env::var("CARROT_ZETA_FORMAT").ok()?;
        let format = ZetaFormat::parse(&version_str).ok()?;
        let model_id = env::var("CARROT_ZETA_MODEL").ok();
        let environment = env::var("CARROT_ZETA_ENVIRONMENT").ok();
        Some(Zeta2RawConfig {
            model_id,
            environment,
            format,
        ...
        cx.spawn(async move |this, cx| {
            let experiments = cx
                .background_spawn(async move {
                    ...
                    let url =
                        http_client.build_carrot_llm_url("/edit_prediction_experiments", &[])?;
                    let request = http_client::Request::builder()
                        .method(Method::GET)
                        .uri(url.as_ref())
                        .header("Authorization", format!("Bearer {}", token))
                        .header(CARROT_VERSION_HEADER_NAME, app_version.to_string())
    ...
    pub fn icons(&self, cx: &App) -> carrot_edit_prediction_types::EditPredictionIconSet {
        use carrot_ui::IconName;
        match self.edit_prediction_model {
            EditPredictionModel::Mercury => {
                carrot_edit_prediction_types::EditPredictionIconSet::new(IconName::Inception)
            }
            EditPredictionModel::Zeta => {
                carrot_edit_prediction_types::EditPredictionIconSet::new(IconName::CarrotPredict)
                    .with_disabled(IconName::CarrotPredictDisabled)
                    .with_up(IconName::CarrotPredictUp)
                    .with_down(IconName::CarrotPredictDown)
                    .with_error(IconName::CarrotPredictError)
            }
            EditPredictionModel::Fim { .. } => {
                let settings = &all_language_settings(None, cx).edit_predictions;
                match settings.provider {
                    EditPredictionProvider::Ollama => {
                        carrot_edit_prediction_types::EditPredictionIconSet::new(IconName::AiOllama)
                    }
                    _ => carrot_edit_prediction_types::EditPredictionIconSet::new(
    ...
    fn handle_project_event(
        ...
        match event {
            carrot_project::Event::ActiveEntryChanged(Some(active_entry_id)) => {
                    ...
                    _subscriptions: [
                        cx.subscribe(buffer, {
                            ...
                            move |this, buffer, event, cx| {
                                if let carrot_language::BufferEvent::Edited { is_local } = event
    ...
    async fn handle_rejected_predictions(
        ...
        }) = rx.next().await
        {
            ...
            let url = client
                .http_client()
                .build_carrot_llm_url("/predict_edits/reject", &[])
                .unwrap();
    ...
    fn did_show_current_prediction(
        &mut self,
        project: &Entity<Project>,
        display_type: carrot_edit_prediction_types::SuggestionDisplayType,
        ...
        let is_jump = display_type == carrot_edit_prediction_types::SuggestionDisplayType::Jump;
...
fn currently_following(_project: &Entity<Project>, _cx: &App) -> bool {
    // Carrot has a single window and no collaboration — never following anyone.
    false
}

fn is_ep_store_provider(provider: EditPredictionProvider) -> bool {
    match provider {
        EditPredictionProvider::Carrot
...
impl EditPredictionStore {
    ...
    fn request_prediction_internal(
        ...
        let events: Vec<Arc<carrot_zeta_prompt::Event>> =
    ...
    async fn send_raw_llm_request(
        ...
        let url = if let Some(custom_url) = custom_url {
            ...
            client
                .http_client()
                .build_carrot_llm_url("/predict_edits/raw", &[])?
        };
    ...
    pub(crate) async fn send_v3_request(
        ...
        let url = client
            .http_client()
            .build_carrot_llm_url("/predict_edits/v3", &[])?;
    ...
    fn load_data_collection_choice(cx: &App) -> DataCollectionChoice {
        let choice = KeyValueStore::global(cx)
            .read_kvp(CARROT_PREDICT_DATA_COLLECTION_CHOICE)
            .log_err()
            .flatten();
        ...
        match choice.as_deref() {
            ...
            Some(_) => {
                log::error!("unknown value in '{CARROT_PREDICT_DATA_COLLECTION_CHOICE}'");
    ...
    fn toggle_data_collection_choice(&mut self, cx: &mut Context<Self>) {
        ...
        carrot_db::write_and_log(cx, move || async move {
            kvp.write_kvp(
                CARROT_PREDICT_DATA_COLLECTION_CHOICE.into(),
...
#[derive(Error, Debug)]
#[error(
    "You must update to Carrot version {minimum_version} or higher to continue using edit predictions."
)]
pub struct CarrotUpdateRequiredError {
...
struct CarrotPredictUpsell;

impl Dismissable for CarrotPredictUpsell {
    const KEY: &'static str = "dismissed-edit-predict-upsell";

    fn dismissed(cx: &App) -> bool {
        // To make this backwards compatible with older versions of Carrot, we
...
pub fn should_show_upsell_modal(cx: &App) -> bool {
    !CarrotPredictUpsell::dismissed(cx)
}

pub fn init(cx: &mut App) {
    cx.observe_new(move |workspace: &mut Workspace, _, _cx| {
        workspace.register_action(
            move |workspace, _: &carrot_actions::OpenCarrotPredictOnboarding, window, cx| {
                CarrotPredictModal::toggle(
                    workspace,
        ...
        workspace.register_action(|workspace, _: &Reinstall, window, cx| {
            if let Some(copilot) = copilot_for_project(workspace.project(), cx) {
                carrot_copilot_ui::reinstall_and_sign_in(copilot, window, cx);
            }
        });
        workspace.register_action(|workspace, _: &SignOut, window, cx| {
            if let Some(copilot) = copilot_for_project(workspace.project(), cx) {
                carrot_copilot_ui::initiate_sign_out(copilot, window, cx);
            }
        });
    })
    .detach();
