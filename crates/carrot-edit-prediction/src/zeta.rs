use crate::{
    CarrotUpdateRequiredError, CurrentEditPrediction, DebugEvent, EditPredictionFinishedDebugEvent,
...
use carrot_edit_prediction_types::PredictedCursorPosition;
use carrot_language::{
...
use carrot_workspace::notifications::{ErrorMessagePrompt, NotificationId, show_app_notification};
use carrot_zeta_prompt::{ParsedOutput, ZetaPromptInput};
...
fn handle_api_response<T>(
    ...
    response: Result<(T, Option<carrot_client::EditPredictionUsage>)>,
    ...
    match response {
        ...
        Err(err) => {
            if err.is::<CarrotUpdateRequiredError>() {
                cx.update(|cx| {
                    ...
                    show_app_notification(
                        NotificationId::unique::<CarrotUpdateRequiredError>(),
                        cx,
                        move |cx| {
                            cx.new(|cx| {
                                ErrorMessagePrompt::new(error_message.clone(), cx).with_link_button(
                                    "Update Carrot",
                                    "https://carrot.dev/releases",
                                )
                            })
                        },
                    );
...
pub(crate) fn active_buffer_diagnostics(
    ...
    snapshot
        ...
        .map(|entry| {
            ...
            carrot_zeta_prompt::ActiveBufferDiagnostic {
                severity,
...
pub fn zeta2_prompt_input(
    snapshot: &BufferSnapshot,
    related_files: Vec<carrot_zeta_prompt::RelatedFile>,
...
) -> (Range<usize>, carrot_zeta_prompt::ZetaPromptInput) {
    ...
    let excerpt_ranges = carrot_zeta_prompt::compute_legacy_excerpt_ranges(
        &cursor_excerpt,
    ...
    let prompt_input = carrot_zeta_prompt::ZetaPromptInput {
...
pub(crate) fn edit_prediction_accepted(
    ...
    cx.background_spawn(async move {
        let url = if let Some(accept_edits_url) = custom_accept_url {
            inazuma::http_client::Url::parse(&accept_edits_url)?
        } else {
            client
                .http_client()
                .build_carrot_llm_url("/predict_edits/accept", &[])?
        };
