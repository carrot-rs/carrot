...
/// Like Carrot's `SettingsAssets`, each crate embeds only the assets it needs.
...
struct KeymapSection {
    /// Context predicate string (e.g. "Terminal", "CarrotInput", "" for global).
    #[serde(default)]
    context: String,
...
/// User keymap is loaded from `~/.carrot/keymap.toml` if it exists.
...
pub fn load_default_and_user_keymap(cx: &mut App) {
    ...
    // Load default keymap from bundled assets (compile-time, like Carrot's SettingsAssets)
    ...
    // Load user keymap from ~/.carrot/keymap.toml (if exists)
    let user_keymap_path = carrot_paths::keymap_file().clone();
