...
use carrot_fs::Fs;
use carrot_http_client::read_no_proxy_from_env;
use carrot_project::{AgentId, Project, agent_server_store::AgentServerStore};
pub use custom::*;
use inazuma_collections::{HashMap, HashSet};

use anyhow::Result;
use carrot_acp_thread::AgentConnection;
use inazuma::{App, AppContext, Entity, Task};
...
pub struct AgentServerDelegate {
    ...
    new_version_available: Option<carrot_watch::Sender<Option<String>>>,
}

impl AgentServerDelegate {
    pub fn new(
        store: Entity<AgentServerStore>,
        new_version_tx: Option<carrot_watch::Sender<Option<String>>>,
    ) -> Self {
        Self {
            store,
