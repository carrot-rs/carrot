...
use carrot_acp_tools::AcpConnectionRegistry;
use carrot_action_log::ActionLog;
use carrot_feature_flags::{AcpBetaFeatureFlag, FeatureFlagAppExt as _};
use carrot_project::agent_server_store::AgentServerCommand;
use carrot_project::{AgentId, Project};
use carrot_task::{ShellBuilder, SpawnInTerminal};
...
use inazuma_util::process::Child;
...
use inazuma::{App, AppContext as _, AsyncApp, Entity, SharedString, Task, WeakEntity};

use carrot_acp_thread::{AcpThread, AuthRequired, LoadError, TerminalProviderEvent};
use carrot_terminal::TerminalBuilder;
use carrot_terminal::terminal_settings::{AlternateScroll, CursorShape, TerminalSettings};
...
struct ConfigOptions {
    ...
    tx: Rc<RefCell<carrot_watch::Sender<()>>>,
    rx: carrot_watch::Receiver<()>,
}

impl ConfigOptions {
    fn new(config_options: Rc<RefCell<Vec<acp::SessionConfigOption>>>) -> Self {
        let (tx, rx) = carrot_watch::channel(());
        Self {
            config_options,
...
impl AcpSessionList {
    ...
    fn send_info_update(&self, session_id: acp::SessionId, update: acp::SessionInfoUpdate) {
        self.updates_tx
            .try_send(carrot_acp_thread::SessionListUpdate::SessionInfo { session_id, update })
            .log_err();
    }
}
...
impl AcpConnection {
    pub async fn stdio(
        ...
        let (release_channel, version): (Option<&str>, String) = cx.update(|cx| {
            (
                carrot_release_channel::ReleaseChannel::try_global(cx)
                    .map(|release_channel| release_channel.display_name()),
                carrot_release_channel::AppVersion::global(cx).to_string(),
            )
        });
        ...
        let response = connection
            .initialize(
                acp::InitializeRequest::new(acp::ProtocolVersion::V1)
                    ...
                    .client_info(
                        acp::Implementation::new("carrot", version)
...
fn terminal_auth_task(
    ...
    carrot_acp_thread::build_terminal_auth_task(
...
fn meta_terminal_auth_task(
    ...
    Some(carrot_acp_thread::build_terminal_auth_task(
...
impl AgentConnection for AcpConnection {
        ...
        cx.spawn(async move |cx| {
            ...
            let thread: Entity<AcpThread> = cx.new(|cx| {
                AcpThread::new(
                    ...
                    carrot_watch::Receiver::constant(self.agent_capabilities.prompt_capabilities.clone()),
                    cx,
                )
            });
    ...
    fn session_modes(
        ...
        session_id: &acp::SessionId,
        _cx: &App,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentSessionModes>> {
        let sessions = self.sessions.clone();
    ...
    fn model_selector(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentModelSelector>> {
        let sessions = self.sessions.clone();
        let sessions_ref = sessions.borrow();
        let Some(session) = sessions_ref.get(session_id) else {
            return None;
    ...
    fn session_config_options(
        ...
        session_id: &acp::SessionId,
        _cx: &App,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentSessionConfigOptions>> {
...
fn mcp_servers_for_project(project: &Entity<Project>, cx: &App) -> Vec<acp::McpServer> {
    ...
    context_server_store
        ...
        .filter_map(|id| {
            ...
            match &*configuration {
                carrot_project::context_server_store::ContextServerConfiguration::Custom {
                    command,
                    remote,
                    ..
                }
                | carrot_project::context_server_store::ContextServerConfiguration::Extension {
                    command,
                    remote,
                    ..
                ...
                carrot_project::context_server_store::ContextServerConfiguration::Http {
                    url,
                    headers,
                    timeout: _,
...
impl carrot_acp_thread::AgentSessionModes for AcpSessionModes {
...
impl carrot_acp_thread::AgentModelSelector for AcpModelSelector {
    fn list_models(&self, _cx: &mut App) -> Task<Result<carrot_acp_thread::AgentModelList>> {
        Task::ready(Ok(carrot_acp_thread::AgentModelList::Flat(
            self.state
                .borrow()
                .available_models
                .clone()
                .into_iter()
                .map(carrot_acp_thread::AgentModelInfo::from)
                .collect(),
        )))
    }
    ...
    fn selected_model(&self, _cx: &mut App) -> Task<Result<carrot_acp_thread::AgentModelInfo>> {
...
struct AcpSessionConfigOptions {
    ...
    watch_tx: Rc<RefCell<carrot_watch::Sender<()>>>,
    watch_rx: carrot_watch::Receiver<()>,
}

impl carrot_acp_thread::AgentSessionConfigOptions for AcpSessionConfigOptions {
    ...
    fn watch(&self, _cx: &mut App) -> Option<carrot_watch::Receiver<()>> {
...
impl acp::Client for ClientDelegate {
    async fn request_permission(
        ...
        let task = thread.update(cx, |thread, cx| {
            thread.request_tool_call_authorization(
                arguments.tool_call,
                carrot_acp_thread::PermissionOptions::Flat(arguments.options),
                cx,
            )
        })??;
    ...
    async fn create_terminal(
        ...
        let terminal_entity = carrot_acp_thread::create_terminal_entity(
            args.command.clone(),
            &args.args,
            args.env
                .into_iter()
