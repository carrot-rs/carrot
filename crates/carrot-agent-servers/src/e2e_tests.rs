...
use carrot_acp_thread::{AcpThread, AgentThreadEntry, ToolCall, ToolCallStatus};
...
where
    ...
    F: AsyncFn(&Arc<dyn carrot_fs::Fs>, &mut TestAppContext) -> T,
{
    let fs = init_test(cx).await as Arc<dyn carrot_fs::Fs>;
    ...
    F: AsyncFn(&Arc<dyn carrot_fs::Fs>, &mut TestAppContext) -> T,
{
    ...
    thread.update(cx, |thread, cx| {
        thread.authorize_tool_call(
            tool_call_id,
            carrot_acp_thread::SelectedPermissionOutcome::new(
                allow_option_id,
    ...
    F: AsyncFn(&Arc<dyn carrot_fs::Fs>, &mut TestAppContext) -> T,
{
    ...
    let project = Project::test(fs.clone(), [path!("/private/tmp").as_ref()], cx).await;
    let thread = new_test_thread(server(&fs, cx).await, project.clone(), "/private/tmp", cx).await;
    // Fire-and-forget: we observe the turn via run_until_first_tool_call
    // below, so the returned future is intentionally dropped here.
    let _turn = thread.update(cx, |thread, cx| {
        thread.send_raw(
            r#"Run exactly `touch hello.txt && echo "Hello, world!" | tee hello.txt` in the terminal."#,
            cx,
        )
...
macro_rules! common_e2e_tests {
    ($server:expr, allow_option_id = $allow_option_id:expr) => {
        mod common_e2e {
            ...
            async fn basic(cx: &mut ::inazuma::TestAppContext) {
                $crate::e2e_tests::test_basic($server, cx).await;
            }

            #[::inazuma::test]
            #[cfg_attr(not(feature = "e2e"), ignore)]
            async fn path_mentions(cx: &mut ::inazuma::TestAppContext) {
                $crate::e2e_tests::test_path_mentions($server, cx).await;
            }

            #[::inazuma::test]
            #[cfg_attr(not(feature = "e2e"), ignore)]
            async fn tool_call(cx: &mut ::inazuma::TestAppContext) {
                $crate::e2e_tests::test_tool_call($server, cx).await;
            }

            #[::inazuma::test]
            #[cfg_attr(not(feature = "e2e"), ignore)]
            async fn tool_call_with_permission(cx: &mut ::inazuma::TestAppContext) {
            ...
            #[::inazuma::test]
            #[cfg_attr(not(feature = "e2e"), ignore)]
            async fn cancel(cx: &mut ::inazuma::TestAppContext) {
                $crate::e2e_tests::test_cancel($server, cx).await;
            }

            #[::inazuma::test]
            #[cfg_attr(not(feature = "e2e"), ignore)]
            async fn thread_drop(cx: &mut ::inazuma::TestAppContext) {
...
pub async fn init_test(cx: &mut TestAppContext) -> Arc<FakeFs> {
    ...
    cx.update(|cx| {
        ...
        let http_client = carrot_reqwest_client::ReqwestClient::user_agent("agent tests").unwrap();
        cx.set_http_client(Arc::new(http_client));
        let client = carrot_client::Client::production(cx);
        let user_store = cx.new(|cx| carrot_client::UserStore::new(client.clone(), cx));
        carrot_language_model::init(user_store, client, cx);

        #[cfg(test)]
        carrot_project::agent_server_store::AllAgentServersSettings::override_global(
            carrot_project::agent_server_store::AllAgentServersSettings(
...
pub fn get_carrot_path() -> PathBuf {
    let mut carrot_path = std::env::current_exe().unwrap();
