...
use carrot_acp_thread::AgentConnection;
use carrot_credentials_provider::CredentialsProvider;
use carrot_fs::Fs;
use carrot_language_model::{ApiKey, EnvVar};
use carrot_project::{
    Project,
...
use inazuma_settings_framework::{SettingsStore, update_settings_file};
    ...
    fn connect(
        ...
        let default_config_options = cx.read_global(|settings: &SettingsStore, _| {
            settings
                ...
                .get(self.agent_id().as_ref())
                .map(|s| match s {
                    carrot_project::agent_server_store::CustomAgentServerSettings::Custom {
                        default_config_options,
                        ..
                    }
                    | carrot_project::agent_server_store::CustomAgentServerSettings::Extension {
                        default_config_options,
                        ..
                    }
                    | carrot_project::agent_server_store::CustomAgentServerSettings::Registry {
                        default_config_options,
                        ..
        ...
        if is_registry_agent {
            if let Some(registry_store) = carrot_project::AgentRegistryStore::try_global(cx) {
        ...
        if is_registry_agent {
            match agent_id.as_ref() {
                ...
                GEMINI_ID => {
                    extra_env.insert("SURFACE".to_owned(), "carrot".to_owned());
                }
                _ => {}
            }
        }
...
fn api_key_for_gemini_cli(cx: &mut App) -> Task<Result<String>> {
    ...
    let api_url = carrot_google_ai::API_URL.to_string();
    cx.spawn(async move |cx| {
        Ok(
...
fn is_registry_agent(agent_id: impl Into<AgentId>, cx: &App) -> bool {
    ...
    let is_in_registry = carrot_project::AgentRegistryStore::try_global(cx)
...
mod tests {
    use super::*;
    use carrot_project::agent_registry_store::{
