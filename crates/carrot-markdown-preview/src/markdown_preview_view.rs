...
use carrot_editor::{Editor, EditorEvent, MultiBufferOffset, SelectionEffects};
...
use carrot_workspace::item::{Item, ItemHandle};
use carrot_workspace::{OpenOptions, OpenVisible, Pane, PaneRole, Workspace};
use inazuma::{
    ...
    pub fn register(workspace: &mut Workspace, _window: &mut Window, _cx: &mut Context<Workspace>) {
        ...
        workspace.register_action(move |workspace, _: &OpenPreviewToTheSide, window, cx| {
            if let Some(editor) = Self::resolve_active_item_as_markdown_editor(workspace, cx) {
                let view = Self::create_markdown_view(workspace, editor.clone(), window, cx);
                let pane = workspace
                    .find_pane_in_direction(carrot_workspace::SplitDirection::Right, cx)
                    .unwrap_or_else(|| {
...
impl Item for MarkdownPreviewView {
    ...
    fn to_item_events(_event: &Self::Event, _f: &mut dyn FnMut(carrot_workspace::item::ItemEvent)) {
    }
}
...
mod tests {
    ...
    fn does_not_treat_web_links_as_preview_paths() {
        assert_eq!(resolve_preview_path("https://carrot.dev", None), None);
