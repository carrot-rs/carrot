...
pub trait PanelHeader: carrot_workspace::Panel {
...
pub fn panel_button(label: impl Into<SharedString>) -> carrot_ui::Button {
    let label = label.into();
    let id = ElementId::Name(label.to_lowercase().replace(' ', "_").into());
    carrot_ui::ButtonCommon::size(
        carrot_ui::Button::new(id, label)
            .label_size(carrot_ui::LabelSize::Small)
            // TODO: Change this once we use on_surface_bg in button_like
            .layer(carrot_ui::ElevationIndex::ModalSurface),
        carrot_ui::ButtonSize::Compact,
    )
}

pub fn panel_filled_button(label: impl Into<SharedString>) -> carrot_ui::Button {
    panel_button(label).style(carrot_ui::ButtonStyle::Primary)
}

pub fn panel_icon_button(id: impl Into<SharedString>, icon: IconName) -> carrot_ui::IconButton {
...
pub fn panel_filled_icon_button(
    ...
    icon: IconName,
) -> carrot_ui::IconButton {
    panel_icon_button(id, icon).style(carrot_ui::ButtonStyle::Primary)
}
