...
use carrot_cloud_api_types::OrganizationId;
use carrot_cloud_llm_client::{WebSearchBody, WebSearchResponse};
use carrot_http_client::{HttpClient, Method};
use carrot_language_model::{LlmApiToken, NeedsLlmTokenRefresh};
use carrot_web_search::{WebSearchProvider, WebSearchProviderId};
use futures::AsyncReadExt as _;
use inazuma::{App, AppContext, Context, Entity, Task};
...
pub const ZED_WEB_SEARCH_PROVIDER_ID: &str = "carrot.dev";
...
async fn perform_web_search(
    ...
    loop {
        ...
        let request = carrot_http_client::Request::builder()
            ...
            .uri(
                http_client
                    .build_carrot_llm_url("/web_search", &[])?
                    .as_ref(),
            )
