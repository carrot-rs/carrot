...
use carrot_ui::{Label, Tooltip, prelude::*, utils::platform_title_bar_height};
use carrot_workspace::AppState;
use inazuma::{App, DivInspectorState, Inspector, InspectorElementId, IntoElement, Window};
use inazuma_util::{ResultExt as _, command::new_command};
...
fn render_inspector_id(inspector_id: &InspectorElementId, cx: &App) -> Div {
    ...
    let source_location_string = source_location_string
        .strip_prefix(env!("CARROT_REPO_DIR"))
    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running Carrot CLI"))
                .on_click(move |_, _window, cx| {
                    cx.background_spawn(open_carrot_source_location(source_location))
...
async fn open_carrot_source_location(
    location: &'static std::panic::Location<'static>,
) -> anyhow::Result<()> {
    let mut path = Path::new(env!("CARROT_REPO_DIR")).to_path_buf();
    ...
    let output = new_command("carrot")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running carrot to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running carrot to open {path_arg} failed with stderr: {}",
