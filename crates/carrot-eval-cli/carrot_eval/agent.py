"""Harbor agent wrapper for Carrot's eval-cli binary.
...
Usage:
    ...
    harbor run -d "dataset@version" \
        --agent-import-path carrot_eval.agent:CarrotAgent \
...
class CarrotAgent(BaseInstalledAgent):
    """Runs Carrot's headless AI agent (eval-cli) to solve tasks.

    The eval-cli binary boots a headless GPUI application and uses the same
    NativeAgent + AcpThread pipeline as the production Carrot terminal, driving
