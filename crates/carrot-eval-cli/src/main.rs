//! Headless CLI binary for running Carrot's agent in evaluation/benchmark environments.
//!
...
use carrot_agent::{NativeAgent, NativeAgentConnection, Templates, ThreadStore};
...
use inazuma::{AppContext as _, AsyncApp, Entity, UpdateGlobal};
...
#[command(
    name = "eval-cli",
    about = "Run Carrot's agent headlessly in evaluation/benchmark environments"
)]
struct Args {
    /// Output current environment variables as JSON to stdout.
    /// Used internally by Carrot's shell environment capture.
    #[arg(long, hide = true)]
    printenv: bool,
...
async fn run_agent(
    ...
    cx: &mut AsyncApp,
) -> (Result<AgentOutcome>, Option<carrot_language_model::TokenUsage>) {
    let setup_result: Result<()> = cx.update(|cx| {
        ...
        registry.update(cx, |registry, cx| {
            registry.set_default_model(
                Some(carrot_language_model::ConfiguredModel {
                    provider: registry
    ...
    let acp_usage = cx.update(|cx| {
        acp_thread
            .read(cx)
            .token_usage()
            .map(|usage| carrot_language_model::TokenUsage {
