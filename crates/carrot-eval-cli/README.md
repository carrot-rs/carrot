...
Headless CLI binary for running Carrot's agent in evaluation/benchmark
...
Uses the same `NativeAgent` + `AcpThread` pipeline as the production Carrot
...
The `carrot_eval/` directory contains a Python package that
