...
use carrot_denoise::{Denoiser, DenoiserError};
...
impl<S: Source<Item = f32>> Iterator for ToMono<S> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.connected_channels = if active_channels > 0 {
            active_channels
        } else {
            1
        };
...
impl<S: Source<Item = f32>> Source for TakeSamples<S> {
    ...
    fn total_duration(&self) -> Option<Duration> {
        Some(Duration::from_secs_f64(
            self.left_to_take as f64 / self.sample_rate() as f64 / self.channels() as f64,
        ))
    }
}
