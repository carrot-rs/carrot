//! Paths to locations used by Carrot.
...
/// On macOS, this is `~/Library/Application Support/Carrot`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/carrot`.
/// On Windows, this is `%LOCALAPPDATA%\Carrot`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/carrot`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/carrot`.
/// On Windows, this is `%APPDATA%\Carrot`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the carrot_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".carrot_server").unwrap());
    *CACHED
}

/// Returns the relative path to the carrot_wsl_server directory on the WSL host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".carrot_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by Carrot.
///
/// All platforms use `~/.carrot/` as the config directory.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.join("config")
        } else {
            home_dir().join(".carrot")
        }
    })
}

/// Returns the path to the data directory used by Carrot.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
        } else if cfg!(target_os = "macos") {
            home_dir().join("Library/Application Support/Carrot")
...
pub fn state_dir() -> &'static PathBuf {
    ...
    STATE_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            return home_dir().join(".local").join("state").join("Carrot");
        }
...
/// Returns the path to the temp directory used by Carrot.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("carrot")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Carrot")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the Carrot server directory on this SSH host.
...
/// Returns the path to the `Carrot.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Carrot.log"))
}

/// Returns the path to the `Carrot.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Carrot.log.old"))
}
...
///
/// This is where language servers are downloaded to for languages built-in to Carrot.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Carrot.
...
/// Returns the relative path to a `.carrot` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".carrot/settings.toml").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".carrot/tasks.toml").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".carrot/debug.json").unwrap());
    *CACHED
}
