...
use inazuma::{App, AppContext as _, AsyncApp, BackgroundExecutor, SharedString, Task};
...
use inazuma_util::{ConnectionResult, ResultExt, TryFutureExt, redact};
use std::path::Path;
use std::{
    any::TypeId,
...
impl LanguageServer {
        ...
        Stdout: AsyncRead + Unpin + Send + 'static,
    {
        ...
        let _clear_response_handlers = inazuma_util::defer({
            let response_handlers = response_handlers.clone();
            move || {
    ...
    pub fn default_initialize_params(
        ...
        InitializeParams {
                        ...
                        completion_item: Some(CompletionItemCapability {
                            ...
                            resolve_support: Some(CompletionItemCapabilityResolveSupport {
                                properties: vec![
                                    ...
                                    // NB: Do not have this resolved, otherwise Carrot becomes slow to complete things
                                    // "textEdit".to_string(),
                                ],
                            }),
            ...
            client_info: carrot_release_channel::ReleaseChannel::try_global(cx).map(
                |release_channel| ClientInfo {
                    name: release_channel.display_name().to_string(),
                    version: Some(carrot_release_channel::AppVersion::global(cx).to_string()),
                },
            ),
            locale: None,
        ...
        U: Future<Output = String>,
    {
        ...
        LspRequest::new(id, async move {
            ...
            let cancel_on_drop = inazuma_util::defer(move || {
...
impl FakeLanguageServer {
    ...
    where
        ...
        T::Params: 'static + Send,
        F: 'static + Send + FnMut(T::Params, inazuma::AsyncApp) -> Fut,
        Fut: 'static + Future<Output = Result<T::Result>>,
    {
        ...
        self.server
            .on_request::<T, _, _>(move |params, cx| {
                ...
                async move {
                    let _guard = inazuma_gpui_util::defer({
                        let responded_tx = responded_tx.clone();
                        move || {
    ...
    where
        ...
        T::Params: 'static + Send,
        F: 'static + Send + FnMut(T::Params, inazuma::AsyncApp),
    {
