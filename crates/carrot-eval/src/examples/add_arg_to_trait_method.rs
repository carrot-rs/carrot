...
impl Example for AddArgToTraitMethod {
    fn meta(&self) -> ExampleMetadata {
        ExampleMetadata {
            ...
            url: "https://github.com/carrot-rs/carrot.git".to_string(),
    ...
    async fn conversation(&self, cx: &mut ExampleContext) -> Result<()> {
        ...
        let _ = cx.prompt(format!(
            r#"
            Add a `window: Option<inazuma::AnyWindowHandle>` argument to the `Tool::run` trait method in {FILENAME},
        ...
        for tool_name in add_ignored_window_paths {
            ...
            let ignored = edits.is_some_and(|edits| {
                edits.has_added_line("        _window: Option<inazuma::AnyWindowHandle>,\n")
            });
            let uningored = edits.is_some_and(|edits| {
                edits.has_added_line("        window: Option<inazuma::AnyWindowHandle>,\n")
            });
