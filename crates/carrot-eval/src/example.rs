...
use carrot_agent::{Thread, ThreadEvent, UserMessageContent};
...
use inazuma::{App, AppContext, AsyncApp, Entity};
use carrot_language_model::Role;
...
impl AppContext for ExampleContext {
    fn new<T: 'static>(
        &mut self,
        build_entity: impl FnOnce(&mut inazuma::Context<T>) -> T,
    ) -> Entity<T> {
    ...
    fn update_entity<T, R>(
        &mut self,
        handle: &Entity<T>,
        update: impl FnOnce(&mut T, &mut inazuma::Context<T>) -> R,
    ) -> R
    where
        T: 'static,
    {
    ...
    fn as_mut<'a, T>(&'a mut self, handle: &Entity<T>) -> inazuma::GpuiBorrow<'a, T>
    where
        T: 'static,
    {
    ...
    fn update_window<T, F>(&mut self, window: inazuma::AnyWindowHandle, f: F) -> Result<T>
    where
        F: FnOnce(inazuma::AnyView, &mut inazuma::Window, &mut App) -> T,
    {
