...
use clap::Parser;
use carrot_client::{Client, ProxySettings, UserStore};
...
use inazuma::{App, AppContext, AsyncApp, Entity, UpdateGlobal};
use inazuma_tokio::Tokio;
use carrot_language::LanguageRegistry;
use carrot_language_model::{ConfiguredModel, LanguageModel, LanguageModelRegistry, SelectedModel};
...
use carrot_release_channel::{AppCommitSha, AppVersion};
...
fn main() {
    ...
    std::fs::create_dir_all(&carrot_paths::config_dir()).unwrap();

    let carrot_commit_sha = commit_sha_for_path(&root_dir);
    let carrot_branch_name = git_branch_for_path(&root_dir);
    ...
    app.run(move |cx| {
        ...
        let enable_telemetry = env::var("CARROT_EVAL_TELEMETRY").is_ok_and(|value| value == "1")
            && telemetry.has_checksum_seed();
        if enable_telemetry {
            println!("Telemetry enabled");
            carrot_telemetry::event!(
                "Agent Eval Started",
                carrot_commit_sha = carrot_commit_sha,
                carrot_branch_name = carrot_branch_name,
                run_id = run_id,
            );
        }
        ...
        cx.spawn(async move |cx| {
            ...
            future::join_all((0..args.concurrency).map(|_| {
                ...
                let carrot_commit_sha = carrot_commit_sha.clone();
                let carrot_branch_name = carrot_branch_name.clone();
...
pub fn init(cx: &mut App) -> Arc<AgentAppState> {
    let app_commit_sha = option_env!("CARROT_COMMIT_SHA").map(|s| AppCommitSha::new(s.to_owned()));
    ...
    carrot_release_channel::init(app_version.clone(), cx);
    ...
    debug_adapter_carrot_extension::init(extension_host_proxy.clone(), cx);
    language_carrot_extension::init(LspAccess::Noop, extension_host_proxy, languages.clone());
    carrot_language_model::init(user_store.clone(), client.clone(), cx);
    language_models::init(user_store.clone(), client.clone(), cx);
    carrot_languages::init(languages.clone(), fs.clone(), node_runtime.clone(), cx);
...
async fn judge_example(
    ...
    carrot_commit_sha: &str,
    carrot_branch_name: &str,
    run_id: &str,
