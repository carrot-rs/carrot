...
use carrot_language::{Buffer, Language};
...
use carrot_workspace::{
    DeploySearch, ItemNavHistory, NewSearch, PaneRole, ToolbarItemEvent, ToolbarItemLocation,
...
use inazuma::{
    ...
    Global, InteractiveElement, IntoElement, KeyContext, Oklch, ParentElement, Point, Render,
...
use inazuma_util::{ResultExt as _, paths::PathMatcher, rel_path::RelPath};
...
pub fn init(cx: &mut App) {
    ...
    cx.observe_new(|workspace: &mut Workspace, _window, _cx| {
        ...
        register_workspace_action_for_present_search(
            workspace,
            |workspace, _: &inazuma_menu::Cancel, window, cx| {
...
impl ProjectSearch {
    ...
    fn subscribe_to_excerpts(
        ...
        cx.subscribe(excerpts, |this, _, event, cx| {
            if matches!(event, carrot_multi_buffer::Event::FileHandleChanged) {
    ...
    fn remove_deleted_buffers(&mut self, cx: &mut Context<Self>) {
        let (deleted_paths, removed_excerpt_ids) = {
            ...
            let removed_excerpt_ids: inazuma_collections::HashSet<ExcerptId> = deleted_paths
                .iter()
                ...
                let (buffers_with_ranges, has_reached_limit) = cx
                    ...
                    .spawn(async move {
                        ...
                        for result in results {
                            match result {
                                carrot_project::search::SearchResult::Buffer { buffer, ranges } => {
                                    buffers_with_ranges.push((buffer, ranges));
                                }
                                carrot_project::search::SearchResult::LimitReached => {
                                    limit_reached = true;
                                }
                            }
                        }
...
pub enum ViewEvent {
    UpdateTab,
    Activate,
    EditorEvent(carrot_editor::EditorEvent),
    Dismiss,
}
    ...
    fn tab_content_text(&self, _detail: usize, cx: &App) -> SharedString {
        let last_query: Option<SharedString> = self
            ...
            .map(|query| {
                ...
                let query_text = inazuma_util::truncate_and_trailoff(&query, MAX_TAB_TITLE_LEN);
                query_text.into()
            });
...
impl ProjectSearchView {
    ...
    pub fn new(
        ...
        subscriptions.push(
            cx.subscribe(&results_editor, |this, _, event: &EditorEvent, cx| {
                if matches!(event, carrot_editor::EditorEvent::SelectionsChanged { .. }) {
    ...
    pub fn deploy_search(
        workspace: &mut Workspace,
        action: &carrot_workspace::DeploySearch,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
            ...
            let skip_save_on_close = this
                .read_with(cx, |this, cx| {
                    this.workspace.read_with(cx, |workspace, cx| {
                        carrot_workspace::Pane::skip_save_on_close(
                            &this.results_editor,
                            workspace,
                            cx,
                        )
...
pub mod tests {
    ...
    use super::*;
    use carrot_editor::{DisplayPoint, display_map::DisplayRow};
    use carrot_language::{FakeLspAdapter, rust_lang};
    use carrot_project::{FakeFs, Fs};
    use carrot_workspace::{DeploySearch, Workspace};
    use inazuma::{Action, TestAppContext, VisualTestContext, WindowHandle};
    ...
    async fn test_deploy_project_search_focus(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, move |workspace, window, cx| {
            ...
            ProjectSearchView::deploy_search(
                workspace,
                &carrot_workspace::DeploySearch::find(),
                window,
                cx,
            )
        });
    ...
    async fn test_new_project_search_focus(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, move |workspace, window, cx| {
            ...
            ProjectSearchView::new_search(workspace, &carrot_workspace::NewSearch, window, cx)
        });
        ...
        assert!(
            ...
            "New search view should be open after `carrot_workspace::NewSearch` event"
        );
        ...
        let second_pane = workspace
            .update_in(cx, |workspace, window, cx| {
                workspace.split_and_clone(
                    first_pane.clone(),
                    carrot_workspace::SplitDirection::Right,
                    window,
                    cx,
                )
            })
    ...
    async fn test_deploy_search_with_multiple_panes(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            ...
            workspace.activate_pane_in_direction(
                carrot_workspace::SplitDirection::Left,
                window,
                cx,
            );
        });
    ...
    async fn test_search_dismisses_modal(cx: &mut TestAppContext) {
        ...
        impl EventEmitter<inazuma::DismissEvent> for EmptyModalView {}
        ...
        impl carrot_workspace::ModalView for EmptyModalView {}
    ...
    async fn test_search_with_inlays(cx: &mut TestAppContext) {
                            ...
                            move |_, _| {
                                ...
                                async move {
                                    ...
                                    Ok(Some(vec![carrot_lsp::InlayHint {
                                        position: carrot_lsp::Position::new(0, 17),
        ...
        project.update(cx, |_, cx| {
            cx.emit(carrot_project::Event::RefreshInlayHints {
    ...
    async fn test_deleted_file_removed_from_search_results(cx: &mut TestAppContext) {
        ...
        fs.remove_file(
            ...
            carrot_fs::RemoveOptions::default(),
        )
        .await
        .unwrap();
        cx.run_until_parked();
    ...
    fn perform_search(
        ...
        cx.executor().advance_clock(
            carrot_editor::SELECTION_HIGHLIGHT_DEBOUNCE_TIMEOUT + Duration::from_millis(100),
        );
        cx.background_executor.run_until_parked();
    }
}
