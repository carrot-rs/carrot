...
use carrot_language::{Language, LanguageRegistry};
use carrot_project::{
    search::SearchQuery,
...
use carrot_actions::{outline::ToggleOutline, workspace::CopyPath, workspace::CopyRelativePath};
use carrot_fs::Fs;
use inazuma_settings_framework::{DiffViewStyle, Settings, update_settings_file};
...
use inazuma_util::{ResultExt, paths::PathMatcher};
...
pub use carrot_actions::buffer_search::{Deploy, DeployReplace, Dismiss, FocusEditor};
...
impl EventEmitter<carrot_workspace::ToolbarItemEvent> for BufferSearchBar {}
impl Render for BufferSearchBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
                                        ...
                                        .child(
                                            h_flex()
                                                ...
                                                .children(render_modifiers(
                                                    &inazuma::Modifiers::secondary_key(),
        ...
        let carrot_workspace::searchable::SearchOptions {
            case,
            word,
            regex,
            replacement,
...
impl BufferSearchBar {
    ...
    pub fn register(registrar: &mut impl SearchActionsRegistrar) {
        ...
        registrar.register_handler(ForDeployed(
            |this, _: &carrot_editor::actions::Cancel, window, cx| {
    ...
    pub fn new(
        ...
        Self {
            ...
            search_history: SearchHistory::new(
                ...
                carrot_project::search_history::QueryInsertionBehavior::ReplacePreviousIfContains,
            ),
    ...
    fn supported_options(
        &self,
        cx: &mut Context<Self>,
    ) -> carrot_workspace::searchable::SearchOptions {
    ...
    pub fn update_find_pasteboard(&mut self, cx: &mut App) {
        cx.write_to_find_pasteboard(inazuma::ClipboardItem::new_string_with_metadata(
            self.query(cx),
    ...
    fn on_query_editor_event(
        ...
        match event {
            carrot_editor::EditorEvent::Focused => self.query_editor_focused = true,
            carrot_editor::EditorEvent::Blurred => self.query_editor_focused = false,
            carrot_editor::EditorEvent::Edited { .. } => {
                self.smartcase(window, cx);
                self.clear_matches(window, cx);
    ...
    fn on_replacement_editor_event(
        ...
        match event {
            carrot_editor::EditorEvent::Focused => self.replacement_editor_focused = true,
            carrot_editor::EditorEvent::Blurred => self.replacement_editor_focused = false,
            _ => {}
        }
    }
    ...
    fn focus(&self, handle: &inazuma::FocusHandle, window: &mut Window, cx: &mut App) {
...
mod tests {
    ...
    use carrot_language::{Buffer, Point};
    use inazuma::{Oklch, TestAppContext, UpdateGlobal, VisualTestContext};
    use inazuma_settings_framework::{SearchSettingsContent, SettingsStore};
    ...
    fn init_multibuffer_test(
        ...
        let multibuffer = cx.new(|cx| {
            let mut buffer = MultiBuffer::new(carrot_language::Capability::ReadWrite);
    ...
    async fn test_search_simple(cx: &mut TestAppContext) {
        ...
        let display_points_of = |background_highlights: Vec<(Range<DisplayPoint>, Oklch)>| {
    ...
    fn display_points_of(
        background_highlights: Vec<(Range<DisplayPoint>, Oklch)>,
    ...
    async fn test_replace_focus(cx: &mut TestAppContext) {
        ...
        cx.simulate_click(button_bounds.center(), inazuma::Modifiers::none());
    ...
    async fn test_hides_and_uses_secondary_when_in_singleton_buffer(cx: &mut TestAppContext) {
        ...
        let mut events = cx.events::<ToolbarItemEvent, _>(&search_bar);
