...
use inazuma_settings_framework::{KeymapFile, Settings as _};
use log::LevelFilter;
...
fn main() {
    ...
    inazuma_platform::application()
        ...
        .run(move |cx| {
            ...
            let http_client = ReqwestClient::user_agent("carrot_storybook").unwrap();
            cx.set_http_client(Arc::new(http_client));

            inazuma_settings_framework::init(cx);
            carrot_theme_settings::init(carrot_theme::LoadThemes::All(Box::new(Assets)), cx);

            let selector = story_selector;

            let mut theme_settings = ThemeSettings::get_global(cx).clone();
            theme_settings.theme = carrot_theme_settings::ThemeSelection::Static(
                inazuma_settings_framework::ThemeName(theme_name.into()),
            );
