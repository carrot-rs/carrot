...
use anyhow::Result;
use carrot_client::{
    CARROT_ALWAYS_ACTIVE, ChannelId, Client, Collaborator, Subscription as ClientSubscription,
    UserStore,
};
use carrot_language::proto::serialize_version;
use carrot_rpc::{
...
impl ChannelBuffer {
    ...
    fn on_buffer_update(
        &mut self,
        _: Entity<carrot_language::Buffer>,
        ...
        match event {
            carrot_language::BufferEvent::Operation {
                ...
                if *CARROT_ALWAYS_ACTIVE
                    && let carrot_language::Operation::UpdateSelections { selections, .. } =
                        operation
                    && selections.is_empty()
                {
                    return;
    ...
    pub fn buffer(&self) -> Entity<carrot_language::Buffer> {
        self.buffer.clone()
    }
