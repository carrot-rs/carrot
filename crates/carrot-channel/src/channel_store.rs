...
use carrot_client::{ChannelId, Client, ClientSettings, Subscription, User, UserId, UserStore};
        ...
        let watch_connection_status = cx.spawn(async move |this, cx| {
            while let Some(status) = connection_status.next().await {
                ...
                match status {
                    ...
                    carrot_client::Status::SignedOut | carrot_client::Status::UpgradeRequired => {
    ...
    async fn handle_update_user_channels(
        ...
        this.update(&mut cx, |this, cx| {
            for buffer_version in message.payload.observed_channel_buffer_version {
                let version = carrot_language::proto::deserialize_version(&buffer_version.version);
                ...
                && let Some(buffer) = buffer.upgrade()
            {
                buffer.update(cx, |channel_buffer, cx| {
                    ...
                    buffer_versions.push(proto::ChannelBufferVersion {
                        ...
                        version: carrot_language::proto::serialize_version(&inner_buffer.version()),
                    });
                });
            }
        }
                ...
                this.opened_buffers.retain(|_, buffer| match buffer {
                    OpenEntityHandle::Open(channel_buffer) => {
                        ...
                        channel_buffer.update(cx, |channel_buffer, cx| {
                            ...
                            {
                                let channel_id = channel_buffer.channel_id;
                                let remote_version = carrot_language::proto::deserialize_version(
                                    &remote_buffer.version,
                                );
                                ...
                                if let Some(operations) = operations {
                                    ...
                                    cx.background_spawn(async move {
                                        ...
                                        for chunk in
                                            carrot_language::proto::split_operations(operations)
                                        {
                                            client
    ...
    pub(crate) fn update_channels(
        ...
        if channels_changed {
            ...
            for latest_buffer_version in payload.latest_channel_buffer_versions {
                let version =
                    carrot_language::proto::deserialize_version(&latest_buffer_version.version);
                self.channel_states
