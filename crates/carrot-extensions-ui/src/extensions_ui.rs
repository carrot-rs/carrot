...
use carrot_actions::ExtensionCategoryFilter;
use carrot_client::carrot_urls;
use carrot_cloud_api_types::{ExtensionMetadata, ExtensionProvides};
use carrot_editor::{Editor, EditorElement, EditorStyle};
use carrot_extension_host::{ExtensionManifest, ExtensionOperation, ExtensionStore};
use carrot_project::DirectoryLister;
...
pub fn init(cx: &mut App) {
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        workspace
            .register_action(
                move |workspace, action: &carrot_actions::Extensions, window, cx| {
        ...
        cx.subscribe_in(workspace.project(), window, |_, _, event, window, cx| {
            if let carrot_project::Event::LanguageNotFound(buffer) = event {
...
impl ExtensionsPage {
            ...
            let subscriptions = [
                ...
                cx.subscribe_in(
                    ...
                    move |this, _, event, window, cx| match event {
                        carrot_extension_host::Event::ExtensionsUpdated => {
                            this.fetch_extensions_debounced(None, cx)
                        }
                        carrot_extension_host::Event::ExtensionInstalled(extension_id) => this
    ...
    fn on_extension_installed(
            ...
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        carrot_actions::theme_selector::Toggle {
        ...
        if !icon_themes.is_empty() {
            workspace
                .update(cx, |_workspace, cx| {
                    window.dispatch_action(
                        carrot_actions::icon_theme_selector::Toggle {
                            ...
                            .when(can_configure, |this| {
                                this.child(
                                    ...
                                    .on_click({
                                        ...
                                        move |_, _, cx| {
                                            if let Some(events) =
                                                carrot_extension::ExtensionEvents::try_global(cx)
                                            {
                                                events.update(cx, |this, cx| {
                                                    this.emit(
                                                        carrot_extension::Event::ConfigureExtensionRequested(
                                                            manifest.clone(),
                                                        ),
                                                        cx,
                                                    )
    ...
    fn buttons_for_entry(
        ...
        let is_compatible = carrot_extension_host::is_version_compatible(extension);
        ...
        match status.clone() {
            ExtensionStatus::NotInstalled => ExtensionCardButtons {
                ...
                .style(ButtonStyle::tinted(carrot_ui::TintColor::Accent))
                .start_icon(
                ...
                .on_click({
                    let extension_id = extension.id.clone();
                    move |_, _, cx| {
                        carrot_telemetry::event!("Extension Installed");
                        ExtensionStore::global(cx).update(cx, |store, cx| {
            ...
            ExtensionStatus::Installed(installed_version) => ExtensionCardButtons {
                ...
                .on_click({
                    let extension_id = extension.id.clone();
                    move |_, _, cx| {
                        carrot_telemetry::event!("Extension Uninstalled", extension_id);
                        ExtensionStore::global(cx).update(cx, |store, cx| {
                            store
                        ...
                        Button::new(extension_button_id(&extension.id, ExtensionOperation::Upgrade), "Upgrade")
                          .style(ButtonStyle::tinted(carrot_ui::TintColor::Accent))
                            .when(!is_compatible, |upgrade_button| {
                                upgrade_button.disabled(true).tooltip({
                                    ...
                                    move |_, cx| {
                                        Tooltip::simple(
                                            format!(
                                                "v{version} is not compatible with this version of Carrot.",
                                            ),
                                             cx,
                                        )
                                    }
                            ...
                            .on_click({
                                ...
                                let version = extension.manifest.version.clone();
                                move |_, _, cx| {
                                    carrot_telemetry::event!("Extension Installed", extension_id, version);
                                    ExtensionStore::global(cx).update(cx, |store, cx| {
                                        store
                                            .upgrade_extension(
                                                extension_id.clone(),
    ...
    fn on_query_change(
        &mut self,
        _: Entity<Editor>,
        event: &carrot_editor::EditorEvent,
        cx: &mut Context<Self>,
    ) {
        if let carrot_editor::EditorEvent::Edited { .. } = event {
    ...
    fn render_acp_registry_upsell(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let registry_url = carrot_urls::acp_registry_blog(cx);
        ...
        let view_registry = Button::new("view_registry", "View Registry")
            ...
            .on_click({
                ...
                move |_, window, cx| {
                    ...
                    window.dispatch_action(Box::new(carrot_actions::AcpRegistry), cx)
                }
            });
                        ...
                        if vim {
                            this.action_slot(
                                h_flex()
                                    ...
                                    .child(
                                        Divider::vertical().color(carrot_ui::DividerColor::Border),
                                    )
                                    .child(
                                        h_flex()
                                            .pl_1()
        ...
        for feature in &self.upsells {
            let banner = match feature {
                Feature::AgentClaude => self.render_feature_upsell_banner(
                    "Claude Agent support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/ai/external-agents#claude-agent".into(),
                    false,
                    cx,
                ),
                Feature::AgentCodex => self.render_feature_upsell_banner(
                    "Codex CLI support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/ai/external-agents#codex-cli".into(),
                    false,
                    cx,
                ),
                Feature::AgentGemini => self.render_feature_upsell_banner(
                    "Gemini CLI support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/ai/external-agents#gemini-cli".into(),
                    false,
                    cx,
                ),
                Feature::ExtensionBasedpyright => self.render_feature_upsell_banner(
                    "Basedpyright (Python language server) support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/python#basedpyright".into(),
                    false,
                    cx,
                ),
                Feature::ExtensionRuff => self.render_feature_upsell_banner(
                    "Ruff (linter for Python) support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/python#code-formatting--linting".into(),
                    false,
                    cx,
                ),
                Feature::ExtensionTailwind => self.render_feature_upsell_banner(
                    "Tailwind CSS support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/tailwindcss".into(),
                    false,
                    cx,
                ),
                Feature::ExtensionTy => self.render_feature_upsell_banner(
                    "Ty (Python language server) support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/python".into(),
                    false,
                    cx,
                ),
                Feature::Git => self.render_feature_upsell_banner(
                    "Carrot comes with basic Git support—more features are coming in the future."
                        .into(),
                    "https://carrot.dev/docs/git".into(),
                    false,
                    cx,
                ),
                Feature::LanguageBash => self.render_feature_upsell_banner(
                    "Shell support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/bash".into(),
                    false,
                    cx,
                ),
                Feature::LanguageC => self.render_feature_upsell_banner(
                    "C support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/c".into(),
                    false,
                    cx,
                ),
                Feature::LanguageCpp => self.render_feature_upsell_banner(
                    "C++ support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/cpp".into(),
                    false,
                    cx,
                ),
                Feature::LanguageGo => self.render_feature_upsell_banner(
                    "Go support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/go".into(),
                    false,
                    cx,
                ),
                Feature::LanguagePython => self.render_feature_upsell_banner(
                    "Python support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/python".into(),
                    false,
                    cx,
                ),
                Feature::LanguageReact => self.render_feature_upsell_banner(
                    "React support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/typescript".into(),
                    false,
                    cx,
                ),
                Feature::LanguageRust => self.render_feature_upsell_banner(
                    "Rust support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/rust".into(),
                    false,
                    cx,
                ),
                Feature::LanguageTypescript => self.render_feature_upsell_banner(
                    "Typescript support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/languages/typescript".into(),
                    false,
                    cx,
                ),
                Feature::OpenIn => self.render_feature_upsell_banner(
                    "Carrot supports linking to a source line on GitHub and others.".into(),
                    "https://carrot.dev/docs/git#git-integrations".into(),
                    false,
                    cx,
                ),
                Feature::Vim => self.render_feature_upsell_banner(
                    "Vim support is built-in to Carrot!".into(),
                    "https://carrot.dev/docs/vim".into(),
                    true,
                    cx,
                ),
            };
