...
use carrot_cloud_llm_client::UsageLimit;
use carrot_codestral::{self, CodestralEditPredictionDelegate};
use carrot_copilot::Status;
use carrot_edit_prediction::EditPredictionStore;
use carrot_edit_prediction_types::EditPredictionDelegateHandle;
use carrot_editor::{
...
use inazuma_settings_framework::{Settings, SettingsStore, update_settings_file};
...
use carrot_actions::{OpenBrowser, OpenSettingsAt};
...
const PRIVACY_DOCS: &str = "https://carrot.dev/docs/ai/privacy-and-security";
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match language_settings.edit_predictions.provider {
                                    ...
                                    workspace.update(cx, |workspace, cx| {
                                        ...
                                        workspace.show_toast(
                                            ...
                                            .on_click(
                                                "Reinstall Copilot",
                                                move |window, cx| {
                                                    carrot_copilot_ui::reinstall_and_sign_in(
                                                        copilot.clone(),
                                                        window,
                                                        cx,
                                                    )
            ...
            EditPredictionProvider::Codestral => {
                ...
                let has_api_key = carrot_codestral::codestral_api_key(cx).is_some();
                let this = cx.weak_entity();
                let file = self.file.clone();
                let language = self.language.clone();
                let project = self.project.clone();
            ...
            | EditPredictionProvider::Mercury) => {
                ...
                let provider_name: &'static str = match provider {
                    ...
                    EditPredictionProvider::Carrot => "carrot",
                    _ => "unknown",
                };
                let icons = self
                    ...
                    .map(|p| p.icons(cx))
                    .unwrap_or_else(|| {
                        carrot_edit_prediction_types::EditPredictionIconSet::new(
                            IconName::CarrotPredict,
                        )
                    });
                ...
                match provider {
                    EditPredictionProvider::Mercury => {
                        ...
                        let mercury_has_error =
                            carrot_edit_prediction::EditPredictionStore::try_global(cx)
                                .is_some_and(|ep_store| {
                                    ep_store.read(cx).mercury_has_payment_required_error()
                                });
                        missing_token = carrot_edit_prediction::EditPredictionStore::try_global(cx)
                ...
                if carrot_edit_prediction::should_show_upsell_modal(cx) {
                    ...
                    return div().child(
                        IconButton::new("carrot-predict-pending-button", ep_icon)
                            .shape(IconButtonShape::Square)
                            ...
                            .on_click(cx.listener(move |_, _, window, cx| {
                                ...
                                window.dispatch_action(
                                    carrot_actions::OpenCarrotPredictOnboarding.boxed_clone(),
                                    cx,
                                );
                            })),
                    );
                ...
                let mercury_has_error = matches!(provider, EditPredictionProvider::Mercury)
                    && carrot_edit_prediction::EditPredictionStore::try_global(cx).is_some_and(
                ...
                let icon_button = IconButton::new("carrot-predict-pending-button", ep_icon)
                    .shape(IconButtonShape::Square)
...
impl EditPredictionButton {
    pub fn new(
        ...
        carrot_edit_prediction::ollama::ensure_authenticated(cx);
        let mercury_api_token_task = carrot_edit_prediction::mercury::load_mercury_api_token(cx);
        let open_ai_compatible_api_token_task =
            carrot_edit_prediction::open_ai_compatible::load_open_ai_compatible_api_token(cx);
    ...
    pub fn build_copilot_start_menu(
        ...
        ContextMenu::build(window, cx, |menu, _, _| {
            ...
            .separator()
            .entry("Use Carrot AI", None, {
                let fs = fs.clone();
                move |_window, cx| {
                    set_completion_provider(fs.clone(), cx, EditPredictionProvider::Carrot)
                }
            })
        })
    }
    ...
    pub fn build_language_settings_menu(
        ...
        if let Some(editor_focus_handle) = self.editor_focus_handle.clone() {
            let entry = ContextMenuEntry::new("This Buffer")
                ...
                .action(Box::new(carrot_editor::actions::ToggleEditPrediction))
                .handler(move |window, cx| {
                    editor_focus_handle.dispatch_action(
                        &carrot_editor::actions::ToggleEditPrediction,
                        window,
                        cx,
                    );
                });
        ...
        let entry = ContextMenuEntry::new("All Files")
            ...
            .action(carrot_workspace::ToggleEditPrediction.boxed_clone())
            .handler(|window, cx| {
                window.dispatch_action(carrot_workspace::ToggleEditPrediction.boxed_clone(), cx)
            });
        ...
        if matches!(provider, EditPredictionProvider::Carrot) {
                                ...
                                v_flex()
                                    ...
                                    .child(
                                        Label::new(indoc!{
                                            ...
                                            Carrot must detect a license file in your repo for this setting to take effect. \
        ...
        menu = menu.item(
            ContextMenuEntry::new("Configure Excluded Files")
                ...
                .documentation_aside(DocumentationSide::Left, |_| {
                    Label::new(indoc!{"
                        Open your settings to add sensitive paths for which Carrot will never predict edits."}).into_any_element()
                })
                .handler(move |window, cx| {
                    carrot_telemetry::event!(
                        "Edit Prediction Menu Action",
    ...
    fn build_copilot_context_menu(
        ...
        let next_edit_suggestions = all_language_settings
            ...
            .unwrap_or(true);
        let copilot_config = carrot_copilot_chat::CopilotChatConfiguration {
        ...
        ContextMenu::build(window, cx, |menu, window, cx| {
            ...
            menu.separator()
                ...
                .action("Sign Out", carrot_copilot::SignOut.boxed_clone())
        })
    }
    ...
    fn build_edit_prediction_context_menu(
        ...
        ContextMenu::build(window, cx, |mut menu, window, cx| {
            ...
            let needs_sign_in = user.is_none()
                && matches!(
                    provider,
                    EditPredictionProvider::None | EditPredictionProvider::Carrot
                );
            ...
            if needs_sign_in {
                menu = menu
                    ...
                    .link_with_handler(
                        "Learn More",
                        OpenBrowser {
                            url: carrot_urls::edit_prediction_docs(cx),
                        }
                        .boxed_clone(),
                        |_window, _cx| {
                            carrot_telemetry::event!(
                    ...
                    .and_then(|provider| provider.usage(cx))
                {
                    ...
                    menu = menu
                        .custom_entry(
                            ...
                            move |_, cx| cx.open_url(&carrot_urls::account_url(cx)),
                        )
                    ...
                    menu = menu
                        .custom_entry(
                            ...
                            |_window, cx| cx.open_url(&carrot_urls::account_url(cx)),
                        )
                        .entry(
                            "Upgrade to Carrot Pro or contact us.",
                            None,
                            |_window, cx| {
                                carrot_telemetry::event!(
                                    "Edit Prediction Menu Action",
                    ...
                    menu = menu
                        ...
                        .entry(
                            "Check your payment status or contact us at billing-support@carrot.dev to continue using this feature.",
                            None,
                            |_window, cx| {
                                cx.open_url(&carrot_urls::account_url(cx))
                            },
...
async fn open_disabled_globs_setting_in_editor(
    ...
    let settings_editor = workspace
        .update_in(cx, |_, window, cx| {
            create_and_open_local_file(carrot_paths::settings_file(), window, cx, || {
...
pub fn get_available_providers(cx: &mut App) -> Vec<EditPredictionProvider> {
    ...
    providers.push(EditPredictionProvider::Carrot);

    if let Some(app_state) = carrot_workspace::AppState::global(cx).upgrade()
        && carrot_copilot::GlobalCopilotAuth::try_get_or_init(app_state, cx)
            .is_some_and(|copilot| copilot.0.read(cx).is_authenticated())
    {
        providers.push(EditPredictionProvider::Copilot);
    };

    if carrot_codestral::codestral_api_key(cx).is_some() {
        providers.push(EditPredictionProvider::Codestral);
    }

    if carrot_edit_prediction::ollama::is_available(cx) {
    ...
    if carrot_edit_prediction::mercury::mercury_api_token(cx)
        .read(cx)
        .has_key()
    {
