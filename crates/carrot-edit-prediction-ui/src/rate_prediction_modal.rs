...
use carrot_edit_prediction::{EditPrediction, EditPredictionRating, EditPredictionStore};
use carrot_editor::{Editor, Inlay, MultiBuffer};
use carrot_feature_flags::FeatureFlag;
use carrot_language::{Buffer, CodeLabel, LanguageRegistry, Point, ToOffset, language_settings};
use carrot_markdown::{Markdown, MarkdownStyle};
use carrot_project::{
...
use inazuma::{
    ...
    Focusable, Length, StyleRefinement, TextStyleRefinement, Window, actions,
};
...
impl RatePredictionsModal {
    pub fn toggle(workspace: &mut Workspace, window: &mut Window, cx: &mut Context<Workspace>) {
        if let Some(ep_store) = EditPredictionStore::try_global(cx) {
            ...
            carrot_telemetry::event!("Rate Prediction Modal Open", source = "Edit Prediction");
        }
    }
    ...
    pub fn new(
        ...
        Self {
            ...
            diff_editor: cx.new(|cx| {
                let multibuffer =
                    cx.new(|_| MultiBuffer::new(carrot_language::Capability::ReadOnly));
    ...
    pub fn select_completion(
        ...
        if let Some(prediction) = prediction {
            ...
            for event in &prediction.inputs.events {
                ...
                carrot_zeta_prompt::write_event(&mut formatted_inputs, event.as_ref());
                ...
                .child(
                    div()
                        ...
                        .child(if let Some(active_prediction) = &self.active_prediction {
                            carrot_markdown::MarkdownElement::new(
                                ...
                                MarkdownStyle {
                                    ...
                                    code_block: StyleRefinement {
                                        ...
                                        border_colors: Some(Edges::all(Some(
...
impl carrot_editor::CompletionProvider for FeedbackCompletionProvider {
    fn completions(
        &self,
        ...
        if !query.starts_with('@') {
            return inazuma::Task::ready(Ok(vec![CompletionResponse {
                completions: vec![],
        ...
        inazuma::Task::ready(Ok(vec![CompletionResponse {
            completions,
            display_options: CompletionDisplayOptions {
                dynamic_width: true,
            },
