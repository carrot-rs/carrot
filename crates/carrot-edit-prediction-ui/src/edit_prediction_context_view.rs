...
use carrot_multi_buffer::{Anchor, MultiBuffer};
use carrot_project::Project;
use carrot_ui::{
    Clickable as _, Disableable, FluentBuilder as _, IconButton, IconName, StyledTypography as _,
    h_flex, v_flex,
};
...
impl EditPredictionContextView {
    ...
    fn handle_context_retrieval_started(
        ...
        let multibuffer = cx.new(|_| MultiBuffer::new(carrot_language::Capability::ReadOnly));
        let editor = cx
    ...
    fn render_informational_footer(
        ...
        h_flex()
            ...
            .child(v_flex().h_full().flex_1().child({
                ...
                let mut rows: Vec<carrot_ui::TableRow> = Vec::new();
                for (key, value) in &run.metadata {
                    rows.push(
                        carrot_ui::TableRow::new()
                            .child(carrot_ui::TableCell::new().child((*key).to_string()))
                            .child(carrot_ui::TableCell::new().child(value.clone())),
                    );
                }
                rows.push(
                    carrot_ui::TableRow::new()
                        .child(carrot_ui::TableCell::new().child("Total Time"))
                        .child(carrot_ui::TableCell::new().child(format!(
                            "{} ms",
                ...
                carrot_ui::Table::new()
                    .w(carrot_ui::px(300.))
                    .child(carrot_ui::TableBody::new().children(rows))
            }))
            .child(
                    ...
                    h_flex()
                        ...
                        .child(
                            IconButton::new("go-back", IconName::ChevronLeft)
                                ...
                                .tooltip(carrot_ui::Tooltip::for_action_title(
                                    "Go to previous run",
                                    &EditPredictionContextGoBack,
                                ))
...
impl Item for EditPredictionContextView {
    ...
    fn buffer_kind(&self, _cx: &App) -> carrot_workspace::item::ItemBufferKind {
        carrot_workspace::item::ItemBufferKind::Multibuffer
    }
