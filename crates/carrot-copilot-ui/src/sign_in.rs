...
use carrot_workspace::{AppState, Toast, Workspace, notifications::NotificationId};
use inazuma::{
...
fn open_copilot_code_verification_window(copilot: &Entity<Copilot>, window: &Window, cx: &mut App) {
    ...
    let height = px(450.);
    let width = px(350.);
    let window_bounds = WindowBounds::Windowed(inazuma::bounds(
        current_window_center - point(height / 2.0, width / 2.0),
        inazuma::size(height, width),
    ));
    cx.open_window(
        WindowOptions {
            kind: inazuma::WindowKind::PopUp,
            window_bounds: Some(window_bounds),
            is_resizable: false,
            is_movable: true,
            titlebar: Some(inazuma::TitlebarOptions {
...
impl CopilotCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .items_center()
            .text_center()
            .child(Headline::new("Use GitHub Copilot in Carrot").size(HeadlineSize::Large))
            .child(
            ...
            .child(
                v_flex()
                    ...
                    .child(
                        ButtonCommon::size(
                            Button::new("copilot-enable-cancel-button", "Cancel").full_width(),
                            ButtonSize::Medium,
                        )
    ...
    fn render_unauthorized_modal(&self, cx: &mut Context<Self>) -> impl Element {
        ...
        v_flex()
            ...
            .child(
                ButtonCommon::size(
                    Button::new("copilot-subscribe-cancel-button", "Cancel").full_width(),
                    ButtonSize::Medium,
                )
                .on_click(cx.listener(|_, _, _, cx| cx.emit(DismissEvent))),
            )
...
impl Render for CopilotCodeVerification {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Vector::new(VectorName::CarrotXCopilot, rems(8.), rems(4.))
...
impl ConfigurationView {
    ...
    fn render_for_chat(&self) -> impl IntoElement {
        let start_label = "To use Carrot's agent with GitHub Copilot, you need to be logged in to GitHub. Note that your GitHub account must have an active Copilot Chat subscription.";
