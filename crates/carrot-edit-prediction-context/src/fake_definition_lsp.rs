    ...
    language_registry.register_fake_lsp(
        ...
        carrot_language::FakeLspAdapter {
            ...
            capabilities: carrot_lsp::ServerCapabilities {
                ...
                type_definition_provider: Some(carrot_lsp::TypeDefinitionProviderCapability::Simple(true)),
            ...
            initializer: Some(Box::new({
                move |server| {
                    server.handle_notification::<carrot_lsp::notification::DidOpenTextDocument, _>({
                    ...
                    server.handle_notification::<carrot_lsp::notification::DidCloseTextDocument, _>({
                        let index = index.clone();
                        let fs = fs.clone();
                        move |params, cx| {
                    ...
                    server.handle_notification::<carrot_lsp::notification::DidChangeWatchedFiles, _>({
                        let index = index.clone();
                        let fs = fs.clone();
                        move |params, cx| {
                            let index = index.clone();
                            ...
                            cx.spawn(async move |_cx| {
                                for event in params.changes {
                                    ...
                                    match event.typ {
                                        carrot_lsp::FileChangeType::DELETED => {
                                            index.lock().remove_definitions_for_file(&event.uri);
                                        }
                                        carrot_lsp::FileChangeType::CREATED
                    ...
                    server.handle_notification::<carrot_lsp::notification::DidChangeTextDocument, _>({
                        let index = index.clone();
                        move |params, _cx| {
                    ...
                    server.handle_notification::<carrot_lsp::notification::DidChangeWorkspaceFolders, _>(
                        {
                            let index = index.clone();
                            let fs = fs.clone();
                            move |params, cx| {
                    ...
                    server.set_request_handler::<carrot_lsp::request::GotoDefinition, _, _>({
                        let index = index.clone();
                        move |params, _cx| {
                    ...
                    server.set_request_handler::<carrot_lsp::request::GotoTypeDefinition, _, _>({
                        let index = index.clone();
                        move |params, _cx| {
...
struct DefinitionIndex {
    language: Arc<Language>,
    definitions: HashMap<String, Vec<carrot_lsp::Location>>,
...
impl DefinitionIndex {
    ...
    fn index_file_inner(&mut self, uri: Uri, content: &str, is_open_in_buffer: bool) -> Option<()> {
        ...
        for (name, byte_range) in declarations {
            ...
            let location = carrot_lsp::Location {
                uri: uri.clone(),
                range,
            };
            self.definitions
    ...
    fn get_definitions(
        &mut self,
        uri: Uri,
        position: carrot_lsp::Position,
    ) -> Option<carrot_lsp::GotoDefinitionResponse> {
        ...
        Some(carrot_lsp::GotoDefinitionResponse::Array(locations))
    }
    ...
    fn get_type_definitions(
            ...
            .and_then(|annotations| annotations.get(name))
        {
            if let Some(locations) = self.definitions.get(type_name) {
                return Some(carrot_lsp::GotoDefinitionResponse::Array(locations.clone()));
            }
        }
...
fn extract_declarations_from_tree(
    tree: &Tree,
    content: &str,
    outline_config: &carrot_language::OutlineConfig,
...
fn byte_range_to_lsp_range(content: &str, byte_range: Range<usize>) -> carrot_lsp::Range {
    let start = byte_offset_to_position(content, byte_range.start);
    let end = byte_offset_to_position(content, byte_range.end);
    carrot_lsp::Range { start, end }
}

fn byte_offset_to_position(content: &str, offset: usize) -> carrot_lsp::Position {
    let mut line = 0;
    let mut character = 0;
    ...
    carrot_lsp::Position { line, character }
}

fn word_at_position(content: &str, position: carrot_lsp::Position) -> Option<&str> {
