...
use inazuma::{
    Action, AnyElement, App, Context, Decorations, Entity, InteractiveElement, IntoElement,
    MouseButton, Oklch, ParentElement, StatefulInteractiveElement, Styled, Window,
...
impl Render for PlatformTitleBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let close_action = Box::new(carrot_workspace::CloseWindow);
        ...
        let title_bar = h_flex()
            ...
            .map(|el| match decorations {
                ...
                Decorations::Client { tiling, .. } => el
                    .when(!(tiling.top || tiling.right), |el| {
                        el.rounded_tr(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING)
                    })
                    .when(!(tiling.top || tiling.left), |el| {
                        el.rounded_tl(carrot_theme::CLIENT_SIDE_DECORATION_ROUNDING)
                    })
            ...
            .content_stretch()
            .child(
                // No `items_center` here on purpose: children (session
                // tabs especially) need to stretch to the full title bar
                // height. Icon/button children that want their content
                // centered already do so via their own inner flex layout.
                div()
                    .id(self.id.clone())
                    .flex()
                    .flex_row()
