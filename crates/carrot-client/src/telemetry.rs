...
use carrot_fs::Fs;
use carrot_http_client::{self, AsyncBody, HttpClient, HttpClientWithUrl, Method, Request};
use carrot_release_channel::ReleaseChannel;
use carrot_telemetry_events::{
    AssistantEventData, AssistantPhase, Event, EventRequestBody, EventWrapper,
};
use futures::channel::mpsc;
use futures::{Future, StreamExt};
use inazuma::{App, AppContext as _, BackgroundExecutor, Task};
...
use carrot_worktree::{UpdatedEntriesSet, WorktreeId};
...
static CARROT_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("CARROT_CLIENT_CHECKSUM_SEED")
        .map(|s| s.as_bytes().into())
        .or_else(|| {
            env::var("CARROT_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
pub static MINIDUMP_ENDPOINT: LazyLock<Option<String>> = LazyLock::new(|| {
    option_env!("CARROT_MINIDUMP_ENDPOINT")
        .map(str::to_string)
        .or_else(|| env::var("CARROT_MINIDUMP_ENDPOINT").ok())
});
...
pub fn os_name() -> String {
    ...
    #[cfg(target_os = "linux")]
    {
        format!("Linux {}", inazuma::guess_compositor())
    }
    #[cfg(target_os = "freebsd")]
    {
        format!("FreeBSD {}", inazuma::guess_compositor())
    }
...
impl Telemetry {
    pub fn new(
        ...
        let state = Arc::new(Mutex::new(TelemetryState {
            ...
            os_version: None,
            os_name: os_name(),
            app_version: carrot_release_channel::AppVersion::global(cx).to_string(),
            subscribers: Vec::new(),
        }));
    ...
    fn shutdown_telemetry(self: &Arc<Self>) -> impl Future<Output = ()> + use<> {
        carrot_telemetry::event!("App Closed");
        // TODO: close final edit period and make sure it's sent
        Task::ready(())
    }

    pub fn log_file_path() -> PathBuf {
        carrot_paths::logs_dir().join("telemetry.log")
    }
    ...
    pub fn has_checksum_seed(&self) -> bool {
        CARROT_CLIENT_CHECKSUM_SEED.is_some()
    }
    ...
    pub fn start(
        ...
        state.app_version = carrot_release_channel::AppVersion::global(cx).to_string();
    ...
    pub fn report_discovered_project_type_events(
        ...
        for project_type in project_types {
            carrot_telemetry::event!("Project Opened", project_type = project_type);
        }
    }
    ...
    fn report_event(self: &Arc<Self>, mut event: Event) {
        ...
        match &mut event {
            Event::Flexible(event) => event
                .event_properties
                .insert("event_source".into(), "carrot".into()),
        };
    ...
    fn build_request(
        ...
        Ok(Request::builder()
            ...
            .uri(
                self.http_client
                    .build_carrot_api_url("/telemetry/events", &[])?
                    .as_ref(),
            )
            .header("Content-Type", "application/json")
            .header("x-carrot-checksum", checksum)
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let checksum_seed = CARROT_CLIENT_CHECKSUM_SEED.as_ref()?;
...
mod tests {
    ...
    use carrot_telemetry_events::FlexibleEvent;
    use carrot_worktree::{PathChange, ProjectEntryId, WorktreeId};
    ...
    #[inazuma::test]
    fn test_project_discovery_does_not_double_report(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_pnpm_project_discovery(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_yarn_project_discovery(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_dotnet_project_discovery(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
