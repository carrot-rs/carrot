...
use carrot_rpc::proto::{RequestMessage, UsersResponse};
...
use inazuma_collections::{HashMap, HashSet, hash_map::Entry};
use inazuma_text::ReplicaId;
use inazuma_util::{ResultExt, TryFutureExt as _};
...
impl UserStore {
    ...
    pub fn plan(&self) -> Option<Plan> {
        ...
        if let Ok(plan) = std::env::var("ZED_SIMULATE_PLAN").as_ref() {
            use carrot_cloud_api_client::Plan;

            return match plan.as_str() {
                "free" => Some(Plan::CarrotFree),
                "trial" => Some(Plan::CarrotProTrial),
                "pro" => Some(Plan::CarrotPro),
                _ => {
    ...
    fn update_authenticated_user(
        ...
        let staff = response.user.is_staff && !*carrot_feature_flags::CARROT_DISABLE_STAFF;
