...
use carrot_cloud_api_client::CloudApiClient;
use carrot_cloud_api_client::websocket_protocol::MessageToClient;
use carrot_credentials_provider::CredentialsProvider;
use carrot_feature_flags::FeatureFlagAppExt as _;
use carrot_http_client::{HttpClient, HttpClientWithUrl, http, read_proxy_from_env};
use carrot_release_channel::{AppVersion, ReleaseChannel};
use carrot_rpc::proto::{AnyTypedEnvelope, EnvelopedMessage, PeerId, RequestMessage};
use futures::{
    AsyncReadExt, FutureExt, SinkExt, Stream, StreamExt, TryFutureExt as _, TryStreamExt,
    channel::{mpsc, oneshot},
    future::BoxFuture,
};
use inazuma::{App, AsyncApp, Entity, Global, Task, WeakEntity, actions};
use inazuma_clock::SystemClock;
use inazuma_settings_framework::{RegisterSetting, Settings, SettingsContent};
use inazuma_util::{ConnectionResult, ResultExt};
...
pub use carrot_rpc::*;
pub use carrot_telemetry_events::Event;
pub use user::*;

static CARROT_SERVER_URL: LazyLock<Option<String>> =
    LazyLock::new(|| std::env::var("CARROT_SERVER_URL").ok());
static CARROT_RPC_URL: LazyLock<Option<String>> =
    LazyLock::new(|| std::env::var("CARROT_RPC_URL").ok());

pub static IMPERSONATE_LOGIN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("CARROT_IMPERSONATE")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static USE_WEB_LOGIN: LazyLock<bool> =
    LazyLock::new(|| std::env::var("CARROT_WEB_LOGIN").is_ok());

pub static ADMIN_API_TOKEN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("CARROT_ADMIN_API_TOKEN")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static CARROT_APP_PATH: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| std::env::var("CARROT_APP_PATH").ok().map(PathBuf::from));

pub static CARROT_ALWAYS_ACTIVE: LazyLock<bool> =
    LazyLock::new(|| std::env::var("CARROT_ALWAYS_ACTIVE").is_ok_and(|e| !e.is_empty()));
...
actions!(
    client,
    [
        /// Signs in to Carrot account.
        SignIn,
        /// Signs out of Carrot account.
        SignOut,
...
impl Settings for ClientSettings {
    fn from_settings(content: &inazuma_settings_framework::SettingsContent) -> Self {
        if let Some(server_url) = &*CARROT_SERVER_URL {
            return Self {
...
impl inazuma_settings_framework::Settings for TelemetrySettings {
...
impl Client {
    ...
    ///
    /// Only Carrot staff automatically connect to Collab.
    ...
    fn rpc_url(
        ...
        async move {
            ...
            if let Some(url) = &*CARROT_RPC_URL {
    ...
    fn establish_websocket_connection(
        ...
        cx.spawn(async move |cx| {
            ...
            let stream = inazuma_tokio::Tokio::spawn_result(cx, {
            ...
            request_headers.insert(
                "x-carrot-protocol-version",
                HeaderValue::from_str(&carrot_rpc::PROTOCOL_VERSION.to_string())?,
            );
            request_headers.insert("x-carrot-app-version", HeaderValue::from_str(&app_version)?);
            request_headers.insert(
                "x-carrot-release-channel",
            ...
            if let Some(system_id) = system_id {
                request_headers.insert("x-carrot-system-id", HeaderValue::from_str(&system_id)?);
            }
            if let Some(metrics_id) = metrics_id {
                request_headers.insert("x-carrot-metrics-id", HeaderValue::from_str(&metrics_id)?);
            }

            let (stream, _) = async_tungstenite::tokio::client_async_tls_with_connector_and_config(
                request,
                stream,
                Some(Arc::new(carrot_http_client_tls::tls_config()).into()),
                None,
            )
            .await?;
        ...
        cx.spawn(async move |cx| {
            ...
            let credentials = background
                ...
                .spawn(async move {
                    ...
                    // Carrot server to encrypt the user's access token, so that it can't be intercepted by
                    // any other app running on the user's device.
                    let (public_key, private_key) = carrot_rpc::auth::keypair()
                        .context("failed to generate keypair for auth")?;
                    ...
                    // Start an HTTP server to receive the redirect from Carrot's sign-in page.
                    ...
                    // Open the Carrot sign-in page in the user's browser, with query parameters that indicate
                    // that the user is signing in from a Carrot app running on the same device.
    ...
    async fn authenticate_as_admin(
        ...
        let url = self
            .http
            .build_carrot_cloud_url("/internal/users/impersonate")?;
...
/// prefix for the carrot:// url scheme
pub const CARROT_URL_SCHEME: &str = "carrot";

/// A parsed Carrot link that can be handled internally by the application.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CarrotLink {
    /// Join a channel: `carrot.dev/channel/channel-name-123` or `carrot://channel/channel-name-123`
    Channel { channel_id: u64 },
    /// Open channel notes: `carrot.dev/channel/channel-name-123/notes` or with heading `notes#heading`
    ChannelNotes {
        channel_id: u64,
...
/// Parses the given link into a Carrot link.
///
/// Returns a [`Some`] containing the parsed link if the link is a recognized Carrot link
/// that should be handled internally by the application.
/// Returns [`None`] for links that should be opened in the browser.
pub fn parse_carrot_link(link: &str, cx: &App) -> Option<CarrotLink> {
    let server_url = &ClientSettings::get_global(cx).server_url;
    let path = link
        .strip_prefix(server_url)
        .and_then(|result| result.strip_prefix('/'))
        .or_else(|| {
            link.strip_prefix(CARROT_URL_SCHEME)
                .and_then(|result| result.strip_prefix("://"))
        })?;
    ...
    let Some(next) = parts.next() else {
        return Some(CarrotLink::Channel { channel_id });
    };

    if let Some(heading) = next.strip_prefix("notes#") {
        return Some(CarrotLink::ChannelNotes {
            channel_id,
    ...
    async fn test_auth_failure_during_reconnection(cx: &mut TestAppContext) {
        ...
        http_client
            ...
            .replace_handler(|_, _request| async move {
                Ok(carrot_http_client::Response::builder()
                    .status(503)
                    .body("".into())
                    .unwrap())
            });
    ...
    async fn test_authenticating_more_than_once(
        ...
        client.override_authenticate({
            ...
            move |cx| {
                ...
                cx.background_spawn(async move {
                    *auth_count.lock() += 1;
                    let _drop = inazuma_util::defer(move || *dropped_auth_count.lock() += 1);
                    future::pending::<()>().await;
                    unreachable!()
                })
            }
