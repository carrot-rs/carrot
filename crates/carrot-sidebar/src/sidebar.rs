use carrot_acp_thread::ThreadStatus;
use carrot_action_log::DiffStats;
use carrot_agent_client_protocol::{self as acp};
use carrot_agent_settings::AgentSettings;
use carrot_agent_ui::thread_metadata_store::{SidebarThreadMetadataStore, ThreadMetadata};
use carrot_agent_ui::threads_archive_view::{
...
use carrot_feature_flags::{AgentV2FeatureFlag, FeatureFlagViewExt as _};
use inazuma::{
    Action as _, AnyElement, AnyWindowHandle, App, Context, Entity, FocusHandle, Focusable,
    KeyContext, ListState, Pixels, Render, SharedString, WeakEntity, Window, list, prelude::*, px,
};
use inazuma_menu::{
    Cancel, Confirm, SelectChild, SelectFirst, SelectLast, SelectNext, SelectParent, SelectPrevious,
};
use carrot_project::{Event as ProjectEvent, linked_worktree_short_name};
use carrot_recent_projects::sidebar_recent_projects::SidebarRecentProjects;
use carrot_ui::utils::platform_title_bar_height;
...
use carrot_workspace::{
    AddFolderToProject, Open, SidebarSide, Workspace, WorkspaceId, workspace_for_window,
};

use carrot_actions::OpenRecent;
use carrot_actions::editor::{MoveDown, MoveUp};

use carrot_actions::agents_sidebar::FocusSidebarFilter;
...
impl From<&ActiveThreadInfo> for carrot_acp_thread::AgentSessionInfo {
...
struct ThreadEntry {
    agent: Agent,
    session_info: carrot_acp_thread::AgentSessionInfo,
    icon: IconName,
    icon_from_external_svg: Option<SharedString>,
    status: AgentThreadStatus,
...
fn root_repository_snapshots(
    workspace: &Entity<Workspace>,
    cx: &App,
) -> impl Iterator<Item = carrot_project::git_store::RepositorySnapshot> {
...
/// Trait for sidebar width/side/notification management.
pub trait WorkspaceSidebar: Focusable + Render {
...
pub struct Sidebar {
    ...
    _draft_observation: Option<inazuma::Subscription>,
}
...
impl Sidebar {
    pub fn new(
        ...
        cx.subscribe(&filter_editor, |this: &mut Self, _, event, cx| {
            if let carrot_editor::EditorEvent::BufferEdited = event {
        ...
        cx.defer_in(window, move |this, window, cx| {
            this.subscribe_to_workspace(&ws, window, cx);
            this.update_entries(cx);
        });
        ...
        Self {
            ...
            focus_handle,
            filter_editor,
            list_state: ListState::new(0, inazuma::ListAlignment::Top, px(1000.)),
    ...
    fn is_active_workspace(&self, workspace: &Entity<Workspace>, _cx: &App) -> bool {
        self.workspace
            .upgrade()
            .map_or(false, |ws| &ws == workspace)
    }
    ...
    fn subscribe_to_workspace(
        ...
        cx.subscribe_in(
            &git_store,
            window,
            |this, _, event: &carrot_project::git_store::GitStoreEvent, window, cx| {
                if matches!(
                    event,
                    carrot_project::git_store::GitStoreEvent::RepositoryUpdated(
                        _,
                        carrot_project::git_store::RepositoryEvent::GitWorktreeListChanged,
                        _,
                    )
                ) {
        ...
        cx.subscribe_in(
            workspace,
            window,
            |this, _workspace, event: &carrot_workspace::Event, window, cx| {
                if let carrot_workspace::Event::PanelAdded(view) = event {
    ...
    fn rebuild_contents(&mut self, cx: &App) {
        ...
        let agent_server_store =
            Some(workspace.read(cx).project().read(cx).agent_server_store().clone());

        let query = self.filter_editor.read(cx).text(cx);

        // Re-derive agent_panel_visible from the workspace so it stays correct.
        self.agent_panel_visible = AgentPanel::is_visible(&workspace, cx);

        // Derive active_thread_is_draft BEFORE focused_thread so we can
        // use it as a guard below.
        self.active_thread_is_draft = workspace
            .read(cx)
            .panel::<AgentPanel>(cx)
            .map_or(false, |panel| panel.read(cx).active_thread_is_draft(cx));

        // Derive focused_thread from the workspace's agent panel.
        ...
        let project_groups = ProjectGroupBuilder::from_workspaces(&workspaces, cx);
        ...
        let resolve_agent = |row: &ThreadMetadata| -> (Agent, IconName, Option<SharedString>) {
            match &row.agent_id {
                None => (Agent::NativeAgent, IconName::CarrotAgent, None),
                Some(id) => {
        ...
        h_flex()
            ...
            .map(|this| {
                if is_selected {
                    ...
                    this.border_color(inazuma::transparent_black())
                }
            })
            .justify_between()
            ...
            .child({
                ...
                h_flex()
                    ...
                    .on_mouse_down(inazuma::MouseButton::Left, |_, _, cx| {
                        cx.stop_propagation();
                    })
    ...
    fn render_project_header_menu(
        ...
        let workspace_for_menu = workspace.clone();
        let _workspace_for_remove = workspace_for_remove.clone();
            ...
            .menu(move |window, cx| {
                ...
                let menu = ContextMenu::build_persistent(window, cx, move |menu, _window, cx| {
                    ...
                    let mut menu = menu
                        ...
                        .end_slot_action(Box::new(inazuma_menu::EndSlot));

                    for (worktree_id, name) in &worktrees {
                        let worktree_id = *worktree_id;
                        let workspace_for_worktree = workspace.clone();

                        let remove_handler = move |_window: &mut Window, cx: &mut App| {
                ...
                let this = this.clone();
                window
                    .subscribe(&menu, cx, move |_, _: &inazuma::DismissEvent, _window, cx| {
                        this.update(cx, |sidebar, cx| {
    ...
    fn remove_workspace(
        ...
        // With a single workspace per window, removing a workspace is a no-op.
        // The worktree-level removal is handled by `project.remove_worktree()`.
        let _ = workspace;
    }
    ...
    fn focus_sidebar_filter(
        ...
        // immediately.
        if carrot_vim_mode_setting::VimModeSetting::get_global(cx).0 {
    ...
    fn find_workspace_across_windows(
        ...
        cx.windows().into_iter().find_map(|window| {
            let workspace = workspace_for_window(window, cx)?;
            if predicate(&workspace, cx) {
    ...
    fn find_workspace_in_current_window(
        ...
        self.workspace.upgrade().filter(|ws| predicate(ws, cx))
    }
    ...
    fn activate_thread_in_other_window(
        ...
        let _target_session_id = session_info.session_id.clone();
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| {
                if let Some(ws) = this.workspace.upgrade() {
                    this.activate_thread_locally(agent, session_info, &ws, window, cx);
                }
            })?;
            anyhow::Ok(())
        })
    ...
    fn toggle_selected_fold(
        &mut self,
        _: &carrot_editor::actions::ToggleFold,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let Some(ix) = self.selection else { return };
    ...
    fn fold_all(
        &mut self,
        _: &carrot_editor::actions::FoldAll,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn unfold_all(
        &mut self,
        _: &carrot_editor::actions::UnfoldAll,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn render_filter_input(&self, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            ...
            .flex_1()
            .capture_action(
                cx.listener(|this, _: &carrot_editor::actions::Newline, window, cx| {
    ...
    fn render_recent_projects_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let workspace = ws.as_ref().map(|w| w.downgrade());
        ...
        let sibling_workspace_ids: HashSet<WorkspaceId> = ws
            .as_ref()
        ...
        PopoverMenu::new("sidebar-recent-projects-menu")
            ...
            .anchor(inazuma::Corner::BottomRight)
    }
    ...
    fn create_new_thread(
        ...
        // highlighted while the new blank thread is being shown.
        self.focused_thread = None;
        ...
        v_flex()
            ...
            .child(
                Button::new("open_project", "Open Project")
                    .full_width()
                    .key_binding(KeyBinding::for_action(&carrot_workspace::Open::default(), cx))
            ...
            .child(
                Button::new("clone_repo", "Clone Repository")
                    .full_width()
                    .on_click(|_, window, cx| {
                        window.dispatch_action(carrot_git::Clone.boxed_clone(), cx);
                    }),
            )
    }
    ...
    fn show_archive(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let Some(active_workspace) = self.workspace.upgrade() else {
            return;
        };
...
mod tests {
    use super::*;
    use carrot_acp_thread::StubAgentConnection;
    use carrot_agent::ThreadStore;
    use carrot_agent_ui::test_support::{active_session_id, open_thread_with_connection, send_message};
    use carrot_assistant_text_thread::TextThreadStore;
    use chrono::DateTime;
    use carrot_feature_flags::FeatureFlagAppExt as _;
    ...
    ) -> Entity<carrot_project::Project> {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(worktree_path, serde_json::json!({ "src": {} }))
            .await;
        cx.update(|cx| <dyn carrot_fs::Fs>::set_global(fs.clone(), cx));
        carrot_project::Project::test(fs, [worktree_path.as_ref()], cx).await
    }
    ...
    fn setup_sidebar(
        ...
        let workspace = workspace.clone();
        let sidebar =
            cx.update(|window, cx| cx.new(|cx| Sidebar::new(workspace.clone(), window, cx)));
        cx.run_until_parked();
        sidebar
    }
    ...
    fn open_and_focus_sidebar(sidebar: &Entity<Sidebar>, cx: &mut inazuma::VisualTestContext) {
        cx.run_until_parked();
    ...
    fn setup_sidebar_with_agent_panel(
        ...
        let sidebar = setup_sidebar(workspace, cx);
    ...
    fn type_in_search(sidebar: &Entity<Sidebar>, query: &str, cx: &mut inazuma::VisualTestContext) {
    ...
    async fn test_search_only_shows_workspace_headers_with_matches(cx: &mut TestAppContext) {
        ...
        // "sidebar" matches a thread title — header stays visible.
        ...
        // "typo" matches nothing in this workspace — empty results.
        ...
        // "project-a" matches the workspace name — the header appears
    ...
    async fn test_cmd_n_shows_new_thread_entry_in_absorbed_worktree(cx: &mut TestAppContext) {
        ...
        let main_project = carrot_project::Project::test(fs.clone(), ["/project".as_ref()], cx).await;
        let worktree_project =
            carrot_project::Project::test(fs.clone(), ["/wt-feature-a".as_ref()], cx).await;
    ...
    async fn init_test_project_with_git(
        ...
        .await;
        cx.update(|cx| <dyn carrot_fs::Fs>::set_global(fs.clone(), cx));
        let project = carrot_project::Project::test(fs.clone(), [worktree_path.as_ref()], cx).await;
        (project, fs)
    }
    ...
    async fn test_two_worktree_workspaces_absorbed_when_main_added(cx: &mut TestAppContext) {
        ...
        let project_a = carrot_project::Project::test(fs.clone(), ["/wt-feature-a".as_ref()], cx).await;
        let project_b = carrot_project::Project::test(fs.clone(), ["/wt-feature-b".as_ref()], cx).await;
        ...
        // Both worktree threads should be grouped under the main
    ...
    async fn test_activate_archived_thread_reuses_workspace_in_another_window(
        ...
        sidebar.update_in(cx_a, |sidebar, window, cx| {
            sidebar.activate_archived_thread(
                Agent::NativeAgent,
                carrot_acp_thread::AgentSessionInfo {
    ...
    async fn test_linked_worktree_threads_not_duplicated_across_groups(cx: &mut TestAppContext) {
        ...
        let project_only = carrot_project::Project::test(fs.clone(), ["/project".as_ref()], cx).await;
        project_only
            .update(cx, |p, cx| p.git_scans_complete(cx))
            .await;

        let multi_root =
            carrot_project::Project::test(fs.clone(), ["/other".as_ref(), "/project".as_ref()], cx).await;
        multi_root
            .update(cx, |p, cx| p.git_scans_complete(cx))
            .await;
