...
use inazuma_collections::{HashMap, HashSet, vecmap::VecMap};
use std::{
    path::{Path, PathBuf},
    sync::Arc,
};
...
mod tests {
    ...
    async fn create_fs_with_main_and_worktree(cx: &mut TestAppContext) -> Arc<FakeFs> {
        ...
        fs.with_git_state(std::path::Path::new("/project/.git"), false, |state| {
            state.worktrees.push(carrot_git::repository::Worktree {
    ...
    async fn test_main_repo_maps_to_itself(cx: &mut TestAppContext) {
        ...
        let project = carrot_project::Project::test(fs.clone(), ["/project".as_ref()], cx).await;
        project
            .update(cx, |project, cx| project.git_scans_complete(cx))
            .await;

        let (_workspace_view, cx) = cx.add_window_view(|window, cx| {
            carrot_workspace::Workspace::test_new(project.clone(), window, cx)
        });
    ...
    async fn test_worktree_checkout_canonicalizes_to_main_repo(cx: &mut TestAppContext) {
        ...
        let project = carrot_project::Project::test(fs.clone(), ["/wt/feature-a".as_ref()], cx).await;
        project
            .update(cx, |project, cx| project.git_scans_complete(cx))
            .await;
