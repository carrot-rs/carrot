use anyhow::Result;
use carrot_edit_prediction::cursor_excerpt;
...
use carrot_language_model::{env_var, ApiKeyState, AuthenticateError, EnvVar};
use futures::AsyncReadExt;
use inazuma::{App, AppContext as _, Context, Entity, Global, SharedString, Task};
...
impl CurrentCompletion {
    ...
    fn interpolate(&self, new_snapshot: &BufferSnapshot) -> Option<Vec<(Range<Anchor>, Arc<str>)>> {
        carrot_edit_prediction_types::interpolate_edits(&self.snapshot, new_snapshot, &self.edits)
    }
}
...
impl CodestralEditPredictionDelegate {
    ...
    async fn fetch_completion(
        ...
        let http_request = carrot_http_client::Request::builder()
            .method(carrot_http_client::Method::POST)
            .uri(format!("{}/v1/fim/completions", api_url))
            .header("Content-Type", "application/json")
            .header("Authorization", format!("Bearer {}", api_key))
            .body(carrot_http_client::AsyncBody::from(request_body))?;
