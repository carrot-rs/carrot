...
use carrot_editor::{
    Editor, EditorEvent, MultiBufferOffset, actions::MoveToEnd, scroll::Autoscroll,
};
use carrot_language::{LanguageServerId, language_settings::SoftWrap};
use carrot_lsp::{
...
use carrot_proto::toggle_lsp_logs::LogType;
use carrot_ui::{Checkbox, ContextMenu, PopoverMenu, ToggleState, prelude::*};
use carrot_workspace::{
    PaneRole, SplitDirection, ToolbarItemEvent, ToolbarItemLocation, ToolbarItemView, Workspace,
    WorkspaceId,
...
pub(crate) struct LogMenuItem {
    ...
    pub trace_level: carrot_lsp::TraceValue,
    ...
    pub(crate) fn try_ensure_copilot_for_project(&self, cx: &mut App) {
        self.log_store.update(cx, |this, cx| {
            ...
            if let Some(subscription_slot @ None) = log_subscription {
                ...
                *subscription_slot = Some(
                    server.on_notification::<carrot_lsp::notification::LogMessage, _>(
                        move |params, cx| {
                            weak_lsp_store
            ...
            .map(|(server_id, state)| match &state.kind {
                ...
                | LanguageServerKind::LocalSsh { .. } => {
                    ...
                    LogMenuItem {
                        ...
                        trace_level: carrot_lsp::TraceValue::Off,
                    }
                }
        ...
        project
            .update(cx, |project, cx| {
                if let Some((client, project_id)) = project.lsp_store().read(cx).upstream_client() {
                    client
                        .send(carrot_proto::ToggleLspLogs {
                            project_id,
...
impl SearchableItem for LspLogView {
    ...
    fn supported_options(&self) -> carrot_workspace::searchable::SearchOptions {
...
impl ToolbarItemView for LspLogToolbarItemView {
    fn set_active_pane_item(
        ...
        _: &mut Window,
        cx: &mut Context<Self>,
    ) -> carrot_workspace::ToolbarItemLocation {
                                        ...
                                        .child(
                                            div().child(
                                                Checkbox::new("LspLogEnableRpcTrace")
                                                    .toggle_state(if rpc_trace_enabled {
