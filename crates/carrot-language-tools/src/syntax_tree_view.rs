...
use carrot_language::{Buffer, OwnedSyntaxLayer};
...
use inazuma::{
    App, Context, Div, Entity, EntityId, EventEmitter, FocusHandle, Focusable, InteractiveElement,
    IntoElement, MouseButton, MouseDownEvent, MouseMoveEvent, Oklch, ParentElement, Render,
    ScrollStrategy, SharedString, Styled, Task, UniformListScrollHandle, WeakEntity, Window,
...
pub struct SyntaxTreeToolbarItemView {
    ...
    subscription: Option<inazuma::Subscription>,
}
    ...
    fn set_editor(&mut self, editor: Entity<Editor>, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let subscription = cx.subscribe_in(&editor, window, |this, _, event, window, cx| {
            let did_reparse = match event {
                carrot_editor::EditorEvent::Reparsed(_) => true,
                carrot_editor::EditorEvent::SelectionsChanged { .. } => false,
                _ => return,
            };
