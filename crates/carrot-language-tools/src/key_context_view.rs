use carrot_actions;
use carrot_ui::{KeyBinding, prelude::*};
use carrot_workspace::{Item, PaneRole, SplitDirection, Workspace};
use inazuma::{
    Action, App, Entity, EventEmitter, FocusHandle, Focusable, KeyBindingContextPredicate,
    KeyContext, Keystroke, MouseButton, Render, Subscription, Task, actions,
};
...
impl Item for KeyContextView {
    ...
    fn to_item_events(_: &Self::Event, _: &mut dyn FnMut(carrot_workspace::item::ItemEvent)) {}
...
impl Render for KeyContextView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("This view lets you determine the current context stack for creating custom key bindings in Carrot. When a keyboard shortcut is triggered, it also shows all the possible contexts it could have triggered in, and which one matched."))
            .child(
                h_flex()
                    .mt_4()
                    .gap_4()
                    .child(
                        Button::new("open_documentation", "Open Documentation")
                            ...
                            .on_click(|_, _, cx| cx.open_url("https://carrot.dev/docs/key-bindings")),
                    )
                    .child(
                        Button::new("view_default_keymap", "View Default Keymap")
                            ...
                            .on_click(|_, window, cx| {
                                window.dispatch_action(carrot_actions::OpenDefaultKeymap.boxed_clone(), cx);
                            }),
                    )
                    .child(
                        Button::new("edit_your_keymap", "Edit Keymap File")
                            .style(ButtonStyle::FILLED)
                            .key_binding(KeyBinding::for_action(&carrot_actions::OpenKeymapFile, cx))
                            .on_click(|_, window, cx| {
