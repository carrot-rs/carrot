...
use super::*;
use carrot_language::{
    FakeLspAdapter, Language, LanguageConfig, LanguageMatcher, tree_sitter_rust,
};
...
async fn test_lsp_log_view(cx: &mut TestAppContext) {
    ...
    language_server.notify::<carrot_lsp::notification::LogMessage>(carrot_lsp::LogMessageParams {
        message: "hello from the server".into(),
        typ: carrot_lsp::MessageType::INFO,
    });
