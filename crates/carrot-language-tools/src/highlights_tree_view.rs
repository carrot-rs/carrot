...
use carrot_language::ToOffset;
use inazuma::{
    Action, App, AppContext as _, Context, Corner, Div, Entity, EntityId, EventEmitter,
    FocusHandle, Focusable, HighlightStyle, InteractiveElement, IntoElement, MouseButton,
    MouseDownEvent, MouseMoveEvent, Oklch, ParentElement, Render, ScrollStrategy, SharedString,
    Styled, Task, UniformListScrollHandle, WeakEntity, Window, actions, div, rems, uniform_list,
};
...
use carrot_workspace::{
    Event as WorkspaceEvent, PaneRole, SplitDirection, ToolbarItemEvent, ToolbarItemLocation,
    ToolbarItemView, Workspace,
    item::{Item, ItemHandle},
};
use inazuma_menu::{SelectNext, SelectPrevious};
    ...
    fn set_editor(&mut self, editor: Entity<Editor>, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let subscription =
            cx.subscribe_in(&editor, window, |this, _, event, window, cx| match event {
                carrot_editor::EditorEvent::Reparsed(_)
                | carrot_editor::EditorEvent::SelectionsChanged { .. } => {
...
impl Item for HighlightsTreeView {
    ...
    fn clone_on_split(
        &self,
        _: Option<carrot_workspace::WorkspaceId>,
        window: &mut Window,
        cx: &mut Context<Self>,
...
fn format_anchor_range(
    ...
    if is_singleton {
        ...
        if let Some(buffer) = buffer {
            let start = carrot_language::ToPoint::to_point(&range.start.text_anchor, buffer);
            let end = carrot_language::ToPoint::to_point(&range.end.text_anchor, buffer);
            let display = SharedString::from(format!(
                "[{}:{} - {}:{}]",
                start.row + 1,
                start.column + 1,
