...
use inazuma::{App, AppContext, AsyncApp, Context, Entity, Task};
use inazuma_util::get_default_system_shell_preferring_bash;
use std::{
    path::PathBuf,
    process::ExitStatus,
    sync::{
...
impl Terminal {
    ...
    pub fn inner(&self) -> &Entity<carrot_terminal::Terminal> {
        &self.terminal
    }
...
pub async fn create_terminal_entity(
    ...
    cx: &mut AsyncApp,
) -> Result<Entity<carrot_terminal::Terminal>> {
    ...
    let (task_command, task_args) = carrot_task::ShellBuilder::new(&shell, is_windows)
