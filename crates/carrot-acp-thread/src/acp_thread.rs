...
use carrot_action_log::{ActionLog, ActionLogTelemetry};
use carrot_language::language_settings::FormatOnSave;
use carrot_language::{
    Anchor, Buffer, BufferSnapshot, LanguageRegistry, Point, ToPoint, text_diff,
};
use carrot_markdown::Markdown;
use carrot_project::lsp_store::{FormatTrigger, LspFormatTarget};
use carrot_project::{AgentLocation, Project, git_store::GitStoreCheckpoint};
use carrot_task::{Shell, ShellBuilder};
use carrot_ui::App;
pub use connection::*;
pub use diff::*;
...
use inazuma_util::{ResultExt, get_default_system_shell_preferring_bash, paths::PathStyle};
...
impl ContentBlock {
    ...
    fn decode_image(image_content: &acp::ImageContent) -> Option<Arc<inazuma::Image>> {
        use base64::Engine as _;

        let bytes = base64::engine::general_purpose::STANDARD
            .decode(image_content.data.as_bytes())
            .ok()?;
        let format = inazuma::ImageFormat::from_mime_type(&image_content.mime_type)?;
        Some(Arc::new(inazuma::Image::from_bytes(format, bytes)))
    }
    ...
    pub fn image(&self) -> Option<&Arc<inazuma::Image>> {
        match self {
...
pub enum TerminalProviderEvent {
    Created {
        ...
        terminal: Entity<::carrot_terminal::Terminal>,
    },
    Output {
        terminal_id: acp::TerminalId,
        data: Vec<u8>,
...
impl AcpThread {
    pub fn new(
        ...
        action_log: Entity<ActionLog>,
        session_id: acp::SessionId,
        mut prompt_capabilities_rx: carrot_watch::Receiver<acp::PromptCapabilities>,
        cx: &mut Context<Self>,
    ) -> Self {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let format_on_save = cx.update(|cx| {
                ...
                let format_on_save = buffer.update(cx, |buffer, cx| {
                    ...
                    let settings = carrot_language::language_settings::LanguageSettings::for_buffer(
                        buffer, cx,
                    );
...
mod tests {
    use super::*;
    use anyhow::anyhow;
    use carrot_project::{AgentId, FakeFs, Fs};
    use futures::{channel::mpsc, future::LocalBoxFuture, select};
    use inazuma::{App, AsyncApp, TestAppContext, WeakEntity};
    use inazuma_settings_framework::SettingsStore;
    use inazuma_util::{path, path_list::PathList};
    use indoc::indoc;
    ...
    #[inazuma::test]
    async fn test_terminal_output_buffered_before_created_renders(
        ...
        let lower = cx.new(|cx| {
            let builder = ::carrot_terminal::TerminalBuilder::new_display_only(
                ::carrot_terminal::terminal_settings::CursorShape::default(),
                ::carrot_terminal::terminal_settings::AlternateScroll::On,
                None,
                0,
                cx.background_executor(),
                PathStyle::local(),
    ...
    #[inazuma::test]
    async fn test_terminal_output_and_exit_buffered_before_created(
        cx: &mut inazuma::TestAppContext,
    ) {
        init_test(cx);
    ...
    #[cfg(unix)]
    #[inazuma::test]
    async fn test_terminal_kill_allows_wait_for_exit_to_complete(cx: &mut inazuma::TestAppContext) {
        use carrot_task::Shell;
        use inazuma_util::shell_builder::ShellBuilder;
        use std::collections::HashMap;
        ...
        let builder = cx
            .update(|cx| {
                ::carrot_terminal::TerminalBuilder::new(
                    None,
                    None,
                    Shell::WithArguments {
                        program,
    ...
    #[inazuma::test]
    async fn test_push_user_content_block(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_thinking_concatenation(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        ...
        thread
            .update(cx, |thread, cx| thread.send_raw("Hello from Carrot!", cx))
            .await
            .unwrap();
