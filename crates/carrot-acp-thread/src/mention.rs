...
use carrot_prompt_store::{PromptId, UserPromptId};
use carrot_ui::{App, IconName, SharedString};
use inazuma_util::{ResultExt, paths::PathStyle};
...
impl MentionUri {
    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::PastedImage => Url::parse("carrot:///agent/pasted-image").unwrap(),
            ...
            MentionUri::Selection {
                ...
                let mut url = if let Some(path) = abs_path {
                    ...
                    let mut url = Url::parse("carrot:///").unwrap();
            ...
            MentionUri::TerminalSelection { line_count } => {
                let mut url = Url::parse("carrot:///agent/terminal-selection").unwrap();
                url.query_pairs_mut()
                    .append_pair("lines", &line_count.to_string());
                url
            }
            MentionUri::GitDiff { base_ref } => {
                let mut url = Url::parse("carrot:///agent/git-diff").unwrap();
                url.query_pairs_mut().append_pair("base", base_ref);
                url
            }
            MentionUri::MergeConflict { file_path } => {
                let mut url = Url::parse("carrot:///agent/merge-conflict").unwrap();
...
mod tests {
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("carrot:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "carrot:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_rule_uri() {
        let rule_uri = "carrot:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule";
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "carrot:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "carrot:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    #[test]
    fn test_invalid_carrot_path() {
        assert!(MentionUri::parse("carrot:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("carrot:///agent/unknown/test", PathStyle::local()).is_err());
    }

    #[test]
    fn test_single_line_number() {
        // https://github.com/carrot-rs/carrot/issues/46114
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "carrot:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "carrot:///agent/terminal-selection?lines=1";
