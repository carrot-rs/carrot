...
use carrot_actions::toast;
use carrot_ui::{Tooltip, prelude::*};
use carrot_workspace::{ToastAction, ToastView};
use inazuma::{DismissEvent, Entity, EventEmitter, FocusHandle, Focusable, IntoElement};
...
impl Component for StatusToast {
    ...
    fn preview(_window: &mut Window, cx: &mut App) -> Option<AnyElement> {
        ...
        let success_example =
            StatusToast::new("Pushed 4 changes to `carrot/main`", cx, |this, _| {
                this.icon(ToastIcon::new(IconName::Check).color(Color::Success))
            });

        let error_example = StatusToast::new(
            "git push: Couldn't find remote origin `iamnbutler/carrot`",
            cx,
            |this, _cx| {
