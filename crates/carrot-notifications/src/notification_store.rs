...
use carrot_client::{ChannelId, Client, UserStore};
use carrot_db::smol::stream::StreamExt;
use carrot_rpc::{Notification, TypedEnvelope, proto};
use inazuma::{App, AppContext as _, AsyncApp, Context, Entity, EventEmitter, Global, Task};
...
impl inazuma_sum_tree::Item for NotificationEntry {
...
impl inazuma_sum_tree::ContextLessSummary for NotificationSummary {
    fn zero() -> Self {
        Default::default()
    }
...
impl inazuma_sum_tree::Dimension<'_, NotificationSummary> for NotificationId {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl inazuma_sum_tree::Dimension<'_, NotificationSummary> for Count {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
