...
use carrot_assistant_slash_commands::FileSlashCommand;
use carrot_fs::FakeFs;
use carrot_language::{Buffer, BufferSnapshot, LanguageRegistry, LspAdapterDelegate};
...
use carrot_ui::{IconName, Window};
use carrot_workspace::Workspace;
use futures::{
    channel::mpsc,
    stream::{self, StreamExt},
};
use inazuma::{App, Entity, SharedString, Task, TestAppContext, WeakEntity, prelude::*};
use inazuma_collections::{HashMap, HashSet};
use inazuma_settings_framework::SettingsStore;
use inazuma_text::{ReplicaId, ToOffset, network::Network};
...
async fn test_slash_commands(cx: &mut TestAppContext) {
    ...
    struct ContextRanges {
        parsed_commands: HashSet<Range<carrot_language::Anchor>>,
        command_outputs: HashMap<InvokedSlashCommandId, Range<carrot_language::Anchor>>,
        output_sections: HashSet<Range<carrot_language::Anchor>>,
    }
