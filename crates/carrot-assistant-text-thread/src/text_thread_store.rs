...
use carrot_assistant_slash_command::{SlashCommandId, SlashCommandWorkingSet};
use carrot_client::{Client, TypedEnvelope, proto};
use carrot_context_server::ContextServerId;
use carrot_env_vars::CARROT_STATELESS;
use carrot_fs::{Fs, RemoveOptions};
...
use carrot_rpc::AnyProtoClient;
use futures::StreamExt;
use inazuma::{App, AppContext as _, AsyncApp, Context, Entity, Task, WeakEntity};
...
pub struct TextThreadStore {
    ...
    client_subscription: Option<carrot_client::Subscription>,
...
impl TextThreadStore {
    ...
    fn handle_project_event(
        ...
        match event {
            ...
            carrot_project::Event::Reshared => {
                self.advertise_contexts(cx);
            }
            carrot_project::Event::HostReshared | carrot_project::Event::Rejoined => {
                        ...
                        *text_thread = TextThreadHandle::Weak(text_thread.downgrade());
                        strong_context.update(cx, |text_thread, cx| {
                            if text_thread.replica_id() != ReplicaId::default() {
                                text_thread
                                    .set_capability(carrot_language::Capability::ReadOnly, cx);
                            }
                        });
                        true
                    } else {
        ...
        cx.spawn(async move |this, cx| {
            ...
            while let Some(path) = paths.next().await {
                ...
                static ASSISTANT_CONTEXT_REGEX: LazyLock<Regex> =
                    LazyLock::new(|| Regex::new(r" - \d+.carrot.json$").unwrap());
    ...
    fn handle_context_server_event(
        ...
        event: &carrot_project::context_server_store::ServerStatusChangedEvent,
        cx: &mut Context<Self>,
    ) {
        let carrot_project::context_server_store::ServerStatusChangedEvent { server_id, status } =
            event;
    ...
    fn load_context_server_slash_commands(
        ...
        cx.spawn(async move |this, cx| {
            ...
            if protocol.capable(carrot_context_server::protocol::ServerCapability::Prompts)
                && let Some(response) = protocol
                    .request::<carrot_context_server::types::requests::PromptsList>(())
                    .await
                    .log_err()
            {
...
mod tests {
    ...
    #[inazuma::test]
    async fn ordered_text_threads_sort_by_mtime(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        ...
        let older = SavedTextThreadMetadata {
            title: "older".into(),
            path: Arc::from(PathBuf::from("/root/older.carrot.json")),
            mtime: now - chrono::TimeDelta::days(1),
        };
        let middle = SavedTextThreadMetadata {
            title: "middle".into(),
            path: Arc::from(PathBuf::from("/root/middle.carrot.json")),
            mtime: now - chrono::TimeDelta::hours(1),
        };
        let newer = SavedTextThreadMetadata {
            title: "newer".into(),
            path: Arc::from(PathBuf::from("/root/newer.carrot.json")),
            mtime: now,
        };
    ...
    #[inazuma::test]
    async fn has_saved_text_threads_reflects_metadata(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        ...
        store.update(cx, |store, _| {
            store.text_threads_metadata = vec![SavedTextThreadMetadata {
                title: "thread".into(),
                path: Arc::from(PathBuf::from("/root/thread.carrot.json")),
    ...
    #[inazuma::test]
    async fn delete_all_local_clears_metadata_and_files(cx: &mut inazuma::TestAppContext) {
        init_test(cx);

        let fs = FakeFs::new(cx.background_executor.clone());
        fs.insert_tree("/root", json!({})).await;

        let thread_a = PathBuf::from("/root/thread-a.carrot.json");
        let thread_b = PathBuf::from("/root/thread-b.carrot.json");
