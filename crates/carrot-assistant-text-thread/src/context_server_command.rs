...
use carrot_context_server::{ContextServerId, types::Prompt};
use carrot_language::{BufferSnapshot, CodeLabel, LspAdapterDelegate};
use carrot_project::context_server_store::ContextServerStore;
use carrot_ui::{IconName, SharedString};
use carrot_workspace::Workspace;
use inazuma::{App, Entity, Task, WeakEntity, Window};
use inazuma_collections::HashMap;
use inazuma_text::LineEnding;
use std::sync::Arc;
use std::sync::atomic::AtomicBool;

use carrot_assistant_slash_command::create_label_for_command;
...
impl SlashCommand for ContextServerSlashCommand {
    ...
    fn label(&self, cx: &App) -> carrot_language::CodeLabel {
        ...
        if let Some(server) = self.store.read(cx).get_running_server(&server_id) {
            cx.foreground_executor().spawn(async move {
                ...
                let response = protocol
                    .request::<carrot_context_server::types::requests::CompletionComplete>(
                        carrot_context_server::types::CompletionCompleteParams {
                            reference: carrot_context_server::types::CompletionReference::Prompt(
                                carrot_context_server::types::PromptReference {
                                    ty: carrot_context_server::types::PromptReferenceType::Prompt,
                                    name: prompt_name,
                                },
                            ),
                            argument: carrot_context_server::types::CompletionArgument {
                                name: arg_name,
                                value: arg_value,
                            },
                            meta: None,
            ...
            cx.foreground_executor().spawn(async move {
                ...
                anyhow::ensure!(
                    response
                        .messages
                        .iter()
                        .all(|msg| matches!(msg.role, carrot_context_server::types::Role::User)),
                ...
                let mut prompt = response
                    ...
                    .filter_map(|msg| match msg.content {
                        carrot_context_server::types::MessageContent::Text { text, .. } => {
                            Some(text)
                        }
                        _ => None,
                    })
