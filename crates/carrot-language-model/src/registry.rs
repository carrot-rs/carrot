...
use inazuma::{App, Context, Entity, EventEmitter, Global, prelude::*};
...
impl LanguageModelRegistry {
    ...
    pub fn providers(&self) -> Vec<Arc<dyn LanguageModelProvider>> {
        let carrot_provider_id = LanguageModelProviderId("carrot.dev".into());
        let mut providers = Vec::with_capacity(self.providers.len());
        if let Some(provider) = self.providers.get(&carrot_provider_id) {
            providers.push(provider.clone());
        }
        providers.extend(self.providers.values().filter_map(|p| {
            if p.id() != carrot_provider_id {
                Some(p.clone())
            } else {
                None
            }
