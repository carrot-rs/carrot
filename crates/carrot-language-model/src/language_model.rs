...
use carrot_anthropic::{AnthropicError, parse_prompt_too_long};
use carrot_client::Client;
use carrot_client::UserStore;
use carrot_cloud_llm_client::CompletionRequestStatus;
use carrot_http_client::{StatusCode, http};
use carrot_open_router::OpenRouterError;
use futures::FutureExt;
use futures::{StreamExt, future::BoxFuture, stream::BoxStream};
use inazuma::{AnyView, App, AsyncApp, Entity, SharedString, Task, Window};
use inazuma_icons::IconName;
pub use inazuma_settings_framework::LanguageModelCacheConfiguration;
use inazuma_util::serde::is_default;
...
pub use carrot_env_vars::{EnvVar, env_var};
...
pub const ZED_CLOUD_PROVIDER_ID: LanguageModelProviderId =
    LanguageModelProviderId::new("carrot.dev");
pub const ZED_CLOUD_PROVIDER_NAME: LanguageModelProviderName =
    LanguageModelProviderName::new("Carrot");
...
impl From<carrot_anthropic::ApiError> for LanguageModelCompletionError {
    fn from(error: carrot_anthropic::ApiError) -> Self {
        use carrot_anthropic::ApiErrorCode::*;
        let provider = ANTHROPIC_PROVIDER_NAME;
        match error.code() {
...
impl From<carrot_open_ai::RequestError> for LanguageModelCompletionError {
    fn from(error: carrot_open_ai::RequestError) -> Self {
        match error {
            carrot_open_ai::RequestError::HttpResponseError {
                provider,
                status_code,
                body,
                headers,
            ...
            carrot_open_ai::RequestError::Other(e) => Self::Other(e),
        }
    }
}
...
impl From<carrot_open_router::ApiError> for LanguageModelCompletionError {
    fn from(error: carrot_open_router::ApiError) -> Self {
        use carrot_open_router::ApiErrorCode::*;
        let provider = LanguageModelProviderName::new("OpenRouter");
        match error.code {
            InvalidRequestError => Self::BadRequestFormat {
                provider,
...
pub enum IconOrSvg {
    /// A built-in icon from Carrot's icon set.
    Icon(IconName),
...
impl Default for IconOrSvg {
    fn default() -> Self {
        Self::Icon(IconName::CarrotAssistant)
    }
}
...
pub trait LanguageModelProviderState: 'static {
    ...
    fn observable_entity(&self) -> Option<inazuma::Entity<Self::ObservableEntity>>;

    fn subscribe<T: 'static>(
        &self,
        cx: &mut inazuma::Context<T>,
        callback: impl Fn(&mut T, &mut inazuma::Context<T>) + 'static,
    ) -> Option<inazuma::Subscription> {
