...
///
/// The namespace argument `editor` can also be omitted, though it is required for Carrot actions.
#[macro_export]
macro_rules! actions {
    ($namespace:path, [ $( $(#[$attr:meta])* $name:ident),* $(,)? ]) => {
        $(
            #[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug, inazuma::Action)]
...
/// specified. In Carrot these trait impls are used to load keymaps from JSON.
///
/// Multiple arguments separated by commas may be specified in `#[action(...)]`:
///
/// - `namespace = some_namespace` sets the namespace. In Carrot this is required.
///
...
///   can then still be used to refer to invoke this action. In Carrot, the keymap JSON schema will
///   accept these old names and provide warnings.
///
/// - `deprecated = "Message about why this action is deprecation"` specifies a deprecation message.
///   In Carrot, the keymap JSON schema will cause this to be displayed as a warning.
///
...
///
/// ```
/// use inazuma::{SharedString, register_action};
/// #[derive(Clone, PartialEq, Eq, serde::Deserialize, schemars::JsonSchema)]
/// pub struct Paste {
///     pub content: SharedString,
/// }
///
/// impl inazuma::Action for Paste {
///     # fn boxed_clone(&self) -> Box<dyn inazuma::Action> { unimplemented!()}
///     # fn partial_eq(&self, other: &dyn inazuma::Action) -> bool { unimplemented!() }
///     # fn name(&self) -> &'static str { "Paste" }
///     # fn name_for_type() -> &'static str { "Paste" }
///     # fn build(value: serde_json::Value) -> anyhow::Result<Box<dyn inazuma::Action>> {
///     #     unimplemented!()
///     # }
/// }
///
...
pub trait Action: Any + Send {
    ...
    /// invoke the action. In Carrot, the keymap JSON schema will accept these old names and provide
    /// warnings.
    ...
    /// Returns the deprecation message for this action, if any. In Carrot, the keymap JSON schema will
...
/// Error type for `Keystroke::parse`. This is used instead of `anyhow::Error` so that Carrot can use
...
mod no_action {
    ...
    ///
    /// `["carrot::Unbind", "editor::NewLine"]`
    #[derive(Clone, Debug, PartialEq, Deserialize, JsonSchema, inazuma::Action)]
    #[action(namespace = carrot)]
    pub struct Unbind(pub inazuma::SharedString);

    /// Returns whether or not this action represents a removed key binding.
    pub fn is_no_action(action: &dyn inazuma::Action) -> bool {
        action.as_any().is::<NoAction>()
    }

    /// Returns whether or not this action represents an unbind marker.
    pub fn is_unbind(action: &dyn inazuma::Action) -> bool {
