...
use crate::{
    App, ClipboardItem, DisplayId, EntityId, PathPromptOptions, PlatformDisplay,
    PlatformKeyboardLayout, PlatformKeyboardMapper, RenderImage, ScreenCaptureSource, Subscription,
    Task, ThermalState, Window, WindowAppearance, WindowButtonLayout,
};
use anyhow::Result;
...
impl App {
    ...
    /// Registers the given URL scheme (e.g. `carrot` for `carrot://` urls) to be
    ...
    pub fn http_client(&self) -> Arc<dyn carrot_http_client::HttpClient> {
        self.http_client.clone()
    }

    /// Sets the HTTP client for the application.
    pub fn set_http_client(&mut self, new_client: Arc<dyn carrot_http_client::HttpClient>) {
