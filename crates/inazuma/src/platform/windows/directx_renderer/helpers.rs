...
fn apply_swap_chain_colorspace(swap_chain: &IDXGISwapChain1, colorspace: crate::WindowColorspace) {
...
#[inline]
pub(super) fn set_rasterizer_state(
    device: &ID3D11Device,
    device_context: &ID3D11DeviceContext,
) -> Result<()> {
...
#[inline]
pub(super) fn create_blend_state_for_subpixel_rendering(
    device: &ID3D11Device,
...
#[inline]
pub(super) fn create_blend_state_for_path_rasterization(
    device: &ID3D11Device,
) -> Result<ID3D11BlendState> {
...
#[inline]
pub(super) fn create_blend_state_for_path_sprite(
    device: &ID3D11Device,
) -> Result<ID3D11BlendState> {
    // If the feature level is set to greater than D3D_FEATURE_LEVEL_9_3, the display
    // device performs the blend in linear space, which is ideal.
...
#[inline]
pub(super) fn create_fragment_shader(
    device: &ID3D11Device,
    bytes: &[u8],
