...
impl ScreenCaptureSource for MacScreenCaptureSource {
    ...
    fn stream(
        ...
        unsafe {
            let excluded_windows = objc2_foundation::NSArray::<SCWindow>::new();
            ...
            let configuration = SCStreamConfiguration::init(SCStreamConfiguration::alloc());
...
impl Drop for MacScreenCaptureStream {
    fn drop(&mut self) {
        unsafe {
            if let Err(err) = self
                .sc_stream
                .removeStreamOutput_type_error(&self.sc_stream_output, SCStreamOutputType::Screen)
            {
                let message = err.localizedDescription();
...
fn screen_id_to_human_label() -> HashMap<CGDirectDisplayID, ScreenMeta> {
    ...
    unsafe {
        ...
        for (i, screen) in screens.iter().enumerate() {
            ...
            let Some(screen_number) = NSDictionary::objectForKey(&device_desc, &screen_number_key)
            else {
                continue;
            };
...
impl InazumaStreamOutput {
    fn create(callback: Box<dyn Fn(ScreenCaptureFrame) + Send>) -> Retained<Self> {
...
impl Drop for InazumaStreamOutput {
    fn drop(&mut self) {
        ...
        if !callback_ptr.is_null() {
            unsafe {
                let _ = Box::from_raw(callback_ptr as *mut Box<dyn Fn(ScreenCaptureFrame) + Send>);
            }
        }
    }
}
