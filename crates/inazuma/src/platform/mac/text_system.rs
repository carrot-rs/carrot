    ...
    fn all_font_names(&self) -> Vec<String> {
        ...
        if let Some(descriptors) = descriptors {
            ...
            for i in 0..count {
                ...
                let desc_ref: &CTFontDescriptor = unsafe { &*(ptr as *const CTFontDescriptor) };
                if let Some(family_name) = unsafe { desc_ref.attribute(kCTFontFamilyNameAttribute) }
                {
        ...
        if let Some(font_id) = lock.font_selections.get(font) {
            ...
            for (ix, font_id) in candidates.iter().enumerate() {
                ...
                let italic_diff = if target_italic {
                    if attrs.style() != swash::Style::Normal {
                        0.0
                    } else {
                        100.0
                    }
...
impl MacTextSystemState {
    ...
    fn load_family(
        ...
        if !found_via_fontique {
            ...
            let ct_font = unsafe { CTFont::with_name(&cf_name, 12.0, std::ptr::null()) };
            ...
            if font_ids.is_empty() {
                ...
                if !postscript_name.is_empty() {
                    if let Some(&existing) = self.font_ids_by_postscript_name.get(&postscript_name)
                    {
                        ...
                        if let Some(url_attr) =
                            unsafe { desc.attribute(objc2_core_text::kCTFontURLAttribute) }
                        {
                                                ...
                                                if let Some(swash_ref) = font_data.get(i) {
                                                        ...
                                                        || font_data.len() == 1
                                                    {
                                                        match self.register_font_with_features(
                                                            ...
                                                            Err(e) => {
                                                                log::warn!(
                                                                    "Failed to register font from file: {}",
                                                                    e
                                                                );
                                                            }
                                                        }
    ...
    fn register_font_with_features(
        ...
        let ct_font = unsafe { CTFont::with_graphics_font(&cg_font, 12.0, std::ptr::null(), None) };
        ...
        if !has_m_glyph && !is_segoe_fluent_icons {
            ...
            return Err(anyhow!("font '{}' has no 'm' character", postscript_name));
        }
    ...
    fn id_for_native_font(&mut self, requested_font: &CTFont) -> FontId {
        ...
        if let Some(font_id) = self.font_ids_by_postscript_name.get(&postscript_name) {
            *font_id
                        ...
                        for i in 0..font_data.len() {
                            if let Some(swash_ref) = font_data.get(i) {
                                let face_name = swash_ref
                                    ...
                                    .map(|s| s.to_string());
                                if face_name.as_deref() == Some(&postscript_name)
                                    || font_data.len() == 1
                                {
            ...
            let cg_font = unsafe { requested_font.graphics_font(std::ptr::null_mut()) };
            let native_ct_font =
                unsafe { requested_font.copy_with_attributes(0.0, std::ptr::null(), None) };
            ...
            let data = self
                .extract_font_data_from_ct_font(requested_font)
                .unwrap_or_default();
            let arc_data = Arc::new(data);
    ...
    fn raster_bounds(&mut self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
        ...
        let mut scaler = self.scale_context.builder(swash_ref).size(ppem).build();
    ...
    fn emoji_raster_bounds(&mut self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
        ...
        let height =
            ((bounding_rect.origin.y + bounding_rect.size.height) * scale).ceil() as i32 - y;
    ...
    fn rasterize_glyph(
        ...
        // Emoji must be checked BEFORE data.is_empty(): AppleColorEmoji is a
        // protected system font whose data extraction often fails, leaving
        // data empty. Without this order, emoji would fall into the grayscale
        // path and render as white rectangles instead of color bitmaps.
        if params.is_emoji {
            return self.rasterize_emoji(params, glyph_bounds, bitmap_size);
        }

        if self.fonts[params.font_id.0].data.is_empty() {
            // System fonts without raw data (e.g. .SFNS): use CoreText grayscale
        ...
        if let Some(image) = image {
            ...
            log::debug!(
                "swash rasterized glyph {}: {}x{}, {} bytes",
                params.glyph_id.0,
                w,
                h,
                image.data.len()
            ...
            log::warn!(
                "swash returned None for glyph {} (font_id={}, ppem={}, font_data_len={})",
                params.glyph_id.0,
                params.font_id.0,
                ppem,
    ...
    fn rasterize_coretext_grayscale(
        ...
        unsafe {
            ct_font.draw_glyphs(NonNull::from(&mut glyph), NonNull::from(&position), 1, &cx);
        }
    ...
    fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> LineLayout {
        ...
        for i in 0..run_count {
            ...
            let font_value =
                unsafe { attributes.value(kCTFontAttributeName as *const _ as *const _) };
            ...
            if !positions_ptr.is_null() {
                positions_slice =
                    unsafe { std::slice::from_raw_parts(positions_ptr, glyph_count_usize) };
            } else {
...
mod tests {
    ...
    fn test_layout_line_zwnj_insertion() {
        ...
        let font_runs = &[FontRun { font_id, len: 5 }, FontRun { font_id, len: 6 }];
