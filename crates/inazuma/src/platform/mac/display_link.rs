    ...
    pub fn new(
        ...
        unsafe {
            ...
            let display_link = {
                ...
                let code = CVDisplayLink::create_with_active_cg_displays(NonNull::new_unchecked(
                    &mut dl_ptr,
                ));
                anyhow::ensure!(
                    code == kCVReturnSuccess,
                    "could not create display link, code: {}",
                    code
                );
                let dl =
                    objc2_core_foundation::CFRetained::from_raw(NonNull::new_unchecked(dl_ptr));
                ...
                anyhow::ensure!(
                    code == kCVReturnSuccess,
                    "could not set output callback, code: {}",
                    code
                );
