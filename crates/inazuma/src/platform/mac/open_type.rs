...
pub fn apply_features_and_fallbacks(
    ...
    unsafe {
        ...
        let mut values: Vec<*const c_void> = vec![&*feature_array as *const _ as *const c_void];
        ...
        let new_font = font.copy_with_attributes(0.0, ptr::null(), Some(&new_descriptor));
...
fn generate_feature_array(features: &FontFeatures) -> CFRetained<CFMutableArray> {
    unsafe {
        ...
        for (tag, value) in features.tag_value_list() {
            ...
            CFMutableArray::append_value(Some(&feature_array), &*dict as *const _ as *const c_void);
        }
        feature_array
    }
}

fn generate_fallback_array(fallbacks: &FontFallbacks, font: &CTFont) -> CFRetained<CFMutableArray> {
    unsafe {
        let fallback_array =
            CFMutableArray::new(None, 0, &kCFTypeArrayCallBacks).expect("failed to create array");
    ...
    unsafe {
        ...
        if let Some(default_fallbacks) = default_fallbacks {
            ...
            for i in 0..count {
                ...
                let desc: &CTFontDescriptor = &*(ptr as *const CTFontDescriptor);
