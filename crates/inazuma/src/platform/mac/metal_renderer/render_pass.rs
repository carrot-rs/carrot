    ...
    fn draw_monochrome_sprites(
        ...
        unsafe {
            ...
            let texture_size = size(
                DevicePixels(texture_ref.width() as i32),
                DevicePixels(texture_ref.height() as i32),
            );
            ...
            for surface in surfaces {
                ...
                command_encoder.setVertexBytes_length_atIndex(
                    NonNull::new_unchecked(
                        &texture_size as *const Size<DevicePixels> as *mut c_void,
                    ),
                    mem::size_of_val(&texture_size),
                ...
                let y_tex =
                    CVMetalTextureGetTexture(&*y_texture).expect("CVMetalTexture has no texture");
                command_encoder
                    .setFragmentTexture_atIndex(Some(&y_tex), SurfaceInputIndex::YTexture as usize);

                let cbcr_tex = CVMetalTextureGetTexture(&*cbcr_texture)
...
impl MetalHeadlessRenderer {
    // `InstanceBufferPool` stores Metal `Retained<ProtocolObject<dyn MTLBuffer>>`
    // handles that are !Send/!Sync by design — the whole Metal renderer runs
    // single-threaded on the main thread. The `Arc<Mutex<…>>` shape matches
    // the production `MetalRenderer` context type for API parity, even though
    // it is never actually shared across threads.
