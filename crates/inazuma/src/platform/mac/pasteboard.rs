...
impl Pasteboard {
    ...
    fn new(inner: Retained<NSPasteboard>) -> Self {
        Self {
            inner,
            text_hash_type: NSString::from_str("carrot-text-hash"),
            metadata_type: NSString::from_str("carrot-metadata"),
        }
    }
...
mod tests {
    ...
    unsafe fn simulate_external_file_copy(pasteboard: &Pasteboard, paths: &[&str]) {
        ...
        let types_array = NSArray::from_slice(&[filenames_type, string_type]);

        unsafe {
            pasteboard.inner.declareTypes_owner(&types_array, None);
        ...
        pasteboard.inner.setData_forType(Some(&data), string_type);
    }
