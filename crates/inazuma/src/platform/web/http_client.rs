use anyhow::anyhow;
use carrot_http_client::{AsyncBody, HttpClient, RedirectPolicy};
...
pub struct FetchHttpClient {
    user_agent: Option<carrot_http_client::http::header::HeaderValue>,
}
...
impl FetchHttpClient {
    ...
    pub unsafe fn with_user_agent(user_agent: &str) -> anyhow::Result<Self> {
        Ok(Self {
            user_agent: Some(carrot_http_client::http::header::HeaderValue::from_str(
                user_agent,
            )?),
        })
    }
...
impl HttpClient for FetchHttpClient {
    fn user_agent(&self) -> Option<&carrot_http_client::http::header::HeaderValue> {
        self.user_agent.as_ref()
    }

    fn proxy(&self) -> Option<&carrot_http_client::Url> {
        None
    }

    fn send(
        &self,
        req: carrot_http_client::http::Request<AsyncBody>,
    ) -> futures::future::BoxFuture<
        'static,
        anyhow::Result<carrot_http_client::http::Response<AsyncBody>>,
    > {
        ...
        Box::pin(AssertSend(async move {
            ...
            let mut builder = carrot_http_client::http::Response::builder().status(status);
