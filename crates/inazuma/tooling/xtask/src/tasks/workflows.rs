...
impl WorkflowFile {
    fn carrot(f: fn() -> Workflow) -> WorkflowFile {
        WorkflowFile {
            source: WorkflowSource::Contextless(f),
            r#type: WorkflowType::Carrot,
        }
    }
...
pub enum WorkflowType {
    /// Workflows living in the Carrot repository
    Carrot,
    /// Workflows living in the `carrot-extensions/workflows` repository that are
...
impl WorkflowType {
    fn disclaimer(&self, workflow_name: &str) -> String {
        format!(
            ...
            workflow_name,
            (*self != WorkflowType::Carrot)
                .then_some(" within the Carrot repository.")
                .unwrap_or_default(),
        )
    }
    ...
    pub fn folder_path(&self) -> PathBuf {
        match self {
            WorkflowType::Carrot => PathBuf::from(".github/workflows"),
...
pub fn run_workflows(args: GenerateWorkflowArgs) -> Result<()> {
    if !Path::new("crates/carrot-app/").is_dir() {
        anyhow::bail!("xtask workflows must be ran from the project root");
    }

    let workflows = [
        WorkflowFile::carrot(after_release::after_release),
        WorkflowFile::carrot(autofix_pr::autofix_pr),
        WorkflowFile::carrot(bump_patch_version::bump_patch_version),
        WorkflowFile::carrot(cherry_pick::cherry_pick),
        WorkflowFile::carrot(compare_perf::compare_perf),
        WorkflowFile::carrot(danger::danger),
        WorkflowFile::carrot(deploy_collab::deploy_collab),
        WorkflowFile::carrot(extension_bump::extension_bump),
        WorkflowFile::carrot(extension_auto_bump::extension_auto_bump),
        WorkflowFile::carrot(extension_tests::extension_tests),
        WorkflowFile::carrot(extension_workflow_rollout::extension_workflow_rollout),
        WorkflowFile::carrot(publish_extension_cli::publish_extension_cli),
        WorkflowFile::carrot(release::release),
        WorkflowFile::carrot(release_nightly::release_nightly),
        WorkflowFile::carrot(run_agent_evals::run_agent_evals),
        WorkflowFile::carrot(run_agent_evals::run_cron_unit_evals),
        WorkflowFile::carrot(run_agent_evals::run_unit_evals),
        WorkflowFile::carrot(run_bundling::run_bundling),
        WorkflowFile::carrot(run_tests::run_tests),
