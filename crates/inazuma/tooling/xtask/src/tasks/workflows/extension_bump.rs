...
// This is used by various extensions repos in the carrot-extensions org to bump extension versions.
    ...
    let step = named::bash(formatdoc! {r#"
        ...
        if [[ "$WORKING_DIR" == "." || -z "$WORKING_DIR" ]]; then
            {{
                ...
                echo "branch_name=carrot-zippy-autobump";
            }} >> "$GITHUB_OUTPUT"
        else
            {{
                ...
                echo "- N/A";
                echo "EOF";
                echo "branch_name=carrot-zippy-${{EXTENSION_ID}}-autobump";
            }} >> "$GITHUB_OUTPUT"
        fi
...
fn trigger_release(
    ...
    let extension_registry = RepositoryTarget::new("carrot-rs", &["extensions"]);
    ...
    named::uses("actions", "github-script", "v7")
        ...
        .add_with((
            ...
            indoc! {r#"
                ...
                console.log(`Assigned ${author} to PR #${prNumber} in carrot-rs/extensions`);
                ...
                console.log(`Automerge enabled for PR #${prNumber} in carrot-rs/extensions`);
            "#},
        ))
