    ...
    fn build_docker_image() -> Step<Run> {
        named::bash(indoc! {r#"
            docker build -f Dockerfile-collab \
              ...
              --tag "registry.digitalocean.com/carrot/collab:$GITHUB_SHA" \
              .
        "#})
    }

    fn publish_docker_image() -> Step<Run> {
        named::bash(r#"docker push "registry.digitalocean.com/carrot/collab:${GITHUB_SHA}""#)
    }
    ...
    fn start_rollout() -> Step<Run> {
        named::bash(indoc! {r#"
            ...
            if [[ $GITHUB_REF_NAME = "collab-production" ]]; then
              export CARROT_KUBE_NAMESPACE=production
              export CARROT_COLLAB_LOAD_BALANCER_SIZE_UNIT=10
              export CARROT_API_LOAD_BALANCER_SIZE_UNIT=2
            elif [[ $GITHUB_REF_NAME = "collab-staging" ]]; then
              export CARROT_KUBE_NAMESPACE=staging
              export CARROT_COLLAB_LOAD_BALANCER_SIZE_UNIT=1
              export CARROT_API_LOAD_BALANCER_SIZE_UNIT=1
            else
              echo "cowardly refusing to deploy from an unknown branch"
              exit 1
            fi

            echo "Deploying collab:$GITHUB_SHA to $CARROT_KUBE_NAMESPACE"

            source script/lib/deploy-helpers.sh
            export_vars_for_environment "$CARROT_KUBE_NAMESPACE"

            CARROT_DO_CERTIFICATE_ID="$(doctl compute certificate list --format ID --no-header)"
            export CARROT_DO_CERTIFICATE_ID
            export CARROT_IMAGE_ID="registry.digitalocean.com/carrot/collab:${GITHUB_SHA}"

            export CARROT_SERVICE_NAME=collab
            export CARROT_LOAD_BALANCER_SIZE_UNIT=$CARROT_COLLAB_LOAD_BALANCER_SIZE_UNIT
            export DATABASE_MAX_CONNECTIONS=850
            envsubst < crates/collab/k8s/collab.template.yml | kubectl apply -f -
            kubectl -n "$CARROT_KUBE_NAMESPACE" rollout status "deployment/$CARROT_SERVICE_NAME" --watch
            echo "deployed ${CARROT_SERVICE_NAME} to ${CARROT_KUBE_NAMESPACE}"

            export CARROT_SERVICE_NAME=api
            export CARROT_LOAD_BALANCER_SIZE_UNIT=$CARROT_API_LOAD_BALANCER_SIZE_UNIT
