...
fn run_bump_patch_version(branch: &WorkflowInput) -> steps::NamedJob {
    ...
    fn bump_patch_version(token: &StepOutput) -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            channel="$(cat crates/carrot-app/RELEASE_CHANNEL)"
            ...
            output="$(cargo set-version -p carrot-app --bump patch 2>&1 | sed 's/.* //')"
            git commit -am "Bump to $output for @$GITHUB_ACTOR"
            git tag "v${output}${tag_suffix}"
            git push origin HEAD "v${output}${tag_suffix}"
        "#})
        .add_env(("GIT_COMMITTER_NAME", "Carrot Zippy"))
        .add_env((
            "GIT_COMMITTER_EMAIL",
            "234243425+carrot-zippy[bot]@users.noreply.github.com",
        ))
        .add_env(("GIT_AUTHOR_NAME", "Carrot Zippy"))
        .add_env((
            "GIT_AUTHOR_EMAIL",
            "234243425+carrot-zippy[bot]@users.noreply.github.com",
        ))
    ...
    named::job(
        Job::default()
            .cond(Expression::new(
                "github.repository_owner == 'carrot-rs'",
            ))
