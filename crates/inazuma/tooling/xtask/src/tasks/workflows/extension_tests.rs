...
pub(crate) const CARROT_EXTENSION_CLI_SHA: &str = "03d8e9aee95ea6117d75a48bcac2e19241f6e667";
...
// This is used by various extensions repos in the carrot-extensions org to run automated tests.
pub(crate) fn extension_tests() -> Workflow {
    ...
    named::workflow()
        ...
        .add_env(("CARROT_EXTENSION_CLI_SHA", CARROT_EXTENSION_CLI_SHA))
...
pub(crate) fn check_extension() -> NamedJob {
    let (cache_download, cache_hit) = cache_carrot_extension_cli();
    ...
    let job = Job::default()
        ...
        .add_step(download_carrot_extension_cli(cache_hit))
...
pub fn cache_carrot_extension_cli() -> (Step<Use>, StepOutput) {
    let step = named::uses(
        "actions",
        "cache",
        "0057852bfaa89a56745cba8c7296529d2fc39830",
    )
    .id("cache-carrot-extension-cli")
    .with(
        Input::default()
            .add("path", "carrot-extension")
            .add("key", "carrot-extension-${{ env.CARROT_EXTENSION_CLI_SHA }}"),
    );
    let output = StepOutput::new(&step, "cache-hit");
    (step, output)
}

pub fn download_carrot_extension_cli(cache_hit: StepOutput) -> Step<Run> {
    named::bash(
    indoc! {
        r#"
        wget --quiet "https://carrot-extension-cli.nyc3.digitaloceanspaces.com/$CARROT_EXTENSION_CLI_SHA/x86_64-unknown-linux-gnu/carrot-extension" -O "$GITHUB_WORKSPACE/carrot-extension"
        chmod +x "$GITHUB_WORKSPACE/carrot-extension"
        "#,
    }
...
pub fn check() -> Step<Run> {
    named::bash(indoc! {
        ...
        "$GITHUB_WORKSPACE/carrot-extension" --source-dir . --scratch-dir /tmp/ext-scratch --output-dir /tmp/ext-output
        "#
    })
}
...
fn verify_version_did_not_change(version_changed: StepOutput) -> Step<Run> {
    named::bash(indoc! {r#"
        if [[ "$VERSION_CHANGED" == "true" && "$GITHUB_EVENT_NAME" == "pull_request" && "$PR_USER_LOGIN" != "carrot-zippy[bot]" ]] ; then
            echo "Version change detected in your change!"
            echo "Version changes happen in separate PRs and will be performed by the carrot-zippy bot"
            exit 42
        fi
        "#
    })
