...
fn fetch_extension_repos(filter_repos_input: &WorkflowInput) -> (NamedJob, JobOutput, JobOutput) {
    ...
    fn checkout_carrot_repo() -> CheckoutStep {
        steps::checkout_repo()
            .with_full_history()
            .with_custom_name("checkout_carrot_repo")
    }
...
fn rollout_workflows_to_extension(
    ...
    fn checkout_extension_repo(token: &StepOutput) -> CheckoutStep {
        steps::checkout_repo()
            ...
            .with_token(token)
            .with_repository("carrot-extensions/${{ matrix.repo }}")
            .with_path("extension")
    }
    ...
    fn create_pull_request(
        ...
        let body = formatdoc! {r#"
            This PR updates the CI workflow files from the main Carrot repository
            based on the commit carrot-rs/carrot@${{{{ github.sha }}}}
        ...
        named::uses("peter-evans", "create-pull-request", "v7")
            ...
            .add_with((
                "committer",
                "carrot-zippy[bot] <234243425+carrot-zippy[bot]@users.noreply.github.com>",
            ))
            .add_with((
                "author",
                "carrot-zippy[bot] <234243425+carrot-zippy[bot]@users.noreply.github.com>",
    ...
    let (authenticate, token) = generate_token(
        ...
        Some(
            RepositoryTarget::new("carrot-extensions", &["${{ matrix.repo }}"]).permissions([
...
fn create_rollout_tag(rollout_job: &NamedJob, filter_repos_input: &WorkflowInput) -> NamedJob {
    fn checkout_carrot_repo(token: &StepOutput) -> CheckoutStep {
    ...
    fn configure_git() -> Step<Run> {
        named::bash(indoc! {r#"
            git config user.name "carrot-zippy[bot]"
            git config user.email "234243425+carrot-zippy[bot]@users.noreply.github.com"
        "#})
    }
    ...
    let job = Job::default()
        ...
        .add_step(authenticate)
        .add_step(checkout_carrot_repo(&token))
