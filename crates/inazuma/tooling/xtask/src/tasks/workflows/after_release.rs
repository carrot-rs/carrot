...
pub fn after_release() -> Workflow {
    ...
    let refresh_carrot_dev = rebuild_releases_page();
    let post_to_discord = post_to_discord(&[&refresh_carrot_dev]);
    ...
    named::workflow()
        ...
        .add_job(refresh_carrot_dev.name, refresh_carrot_dev.job)
...
fn rebuild_releases_page() -> NamedJob {
    fn refresh_cloud_releases() -> Step<Run> {
        named::bash(format!(
            "curl -fX POST https://cloud.carrot.dev/releases/refresh?expect_tag={TAG_NAME}"
        ))
    }

    fn redeploy_carrot_dev() -> Step<Run> {
...
fn post_to_discord(deps: &[&NamedJob]) -> NamedJob {
    ...
    URL="https://carrot.dev/releases/preview"
else
    URL="https://carrot.dev/releases/stable"
fi
...
"#
    ...
    fn get_content() -> Step<Use> {
        ...
        .add_with((
            "stringToTruncate",
            format!(
                "📣 Carrot [{TAG_NAME}](<${{{{ steps.get-release-url.outputs.URL }}}}>)  was just released!\n\n{RELEASE_BODY}\n"
            ),
        ))
...
fn publish_winget() -> NamedJob {
    ...
    $PACKAGE_NAME = "CarrotIndustries.Carrot.Preview"
}} else {{
    $PACKAGE_NAME = "CarrotIndustries.Carrot"
}}
