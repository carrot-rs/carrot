//! Provides constructs for the Carrot app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("CARROT_RELEASE_CHANNEL").unwrap_or_else(|_| {
            include_str!("../../carrot-app/RELEASE_CHANNEL")
                .trim()
                .to_string()
        })
    } else {
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Carrot-Dev",
        ReleaseChannel::Nightly => "Carrot-Nightly",
        ReleaseChannel::Preview => "Carrot-Preview",
        ReleaseChannel::Stable => "Carrot-Stable",
    }
}

/// The Git commit SHA that Carrot was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("CARROT_APP_VERSION") {
            from_env.parse().expect("invalid CARROT_APP_VERSION")
        } else {
...
/// A Carrot release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Carrot.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Carrot on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.nyxb.Carrot-Dev",
            ReleaseChannel::Nightly => "dev.nyxb.Carrot-Nightly",
            ReleaseChannel::Preview => "dev.nyxb.Carrot-Preview",
            ReleaseChannel::Stable => "dev.nyxb.Carrot",
        }
    }
