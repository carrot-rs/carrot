...
use inazuma_collections::{BTreeMap, HashMap, IndexMap};
...
pub struct KeymapSection {
    ...
    /// documentation](https://carrot.dev/docs/key-bindings#contexts) for more details.
    #[serde(default)]
    pub context: String,
...
/// TOML wrapper — TOML requires a root table, so `[[section]]` becomes a Vec field.
#[derive(Deserialize)]
struct TomlKeymapFile {
    #[serde(default)]
    section: Vec<toml::Value>,
}
...
impl KeymapFile {
    pub fn parse(content: &str) -> anyhow::Result<Self> {
        ...
        // Parse TOML → toml::Value → serde_json::Value → KeymapFile sections
        let wrapper: TomlKeymapFile =
            toml::from_str(content).context("failed to parse keymap TOML")?;
        let toml_sections = toml::Value::Array(wrapper.section);
        let json_sections =
            serde_json::to_value(&toml_sections).context("failed to convert TOML to JSON value")?;
        serde_json::from_value(json_sections).context("failed to deserialize keymap sections")
    }
    ...
    pub fn load_asset(
        ...
        match Self::load(asset_str::<SettingsAssets>(asset_path).as_ref(), cx) {
            ...
            KeymapFileLoadResult::ParseFailure { error } => {
                anyhow::bail!("Parse error in built-in keymap \"{asset_path}\": {error}")
            }
        }
    }
    ...
    pub fn load_panic_on_failure(content: &str, cx: &App) -> Vec<KeyBinding> {
        match Self::load(content, cx) {
            ...
            KeymapFileLoadResult::ParseFailure { error } => {
                panic!("Keymap parse error: {error}");
            }
        }
    }
    ...
    pub fn load(content: &str, cx: &App) -> KeymapFileLoadResult {
        let keymap_file = match Self::parse(content) {
            ...
            Err(error) => {
                return KeymapFileLoadResult::ParseFailure { error };
            }
        };
    ...
    fn parse_action_value(action: &Value) -> Result<Option<(&String, Option<&Value>)>, String> {
        let name_and_input = match action {
            // TOML table format: { action = "name", data = {...} }
            Value::Object(map) if map.contains_key("action") => {
                let Some(Value::String(name)) = map.get("action") else {
                    return Err(format!(
                        "expected `action` field to be a string in {}.",
                        MarkdownInlineCode(&action.to_string())
                    ));
                };
                Some((name, map.get("data")))
            }
            // JSON array format (legacy): ["name", {...}]
            Value::Array(items) => {
                if items.len() != 2 {
                    return Err(format!(
            ...
            _ => {
                return Err(format!(
                    "expected action string or {{ action = \"...\", data = {{...}} }}. \
                    Instead found {}.",
                    MarkdownInlineCode(&action.to_string())
                ));
            }
    ...
    pub async fn load_keymap_file(fs: &Arc<dyn Fs>) -> Result<String> {
        match fs.load(carrot_paths::keymap_file()).await {
            result @ Ok(_) => result,
            Err(err) => {
    ...
    pub fn update_keybinding<'a>(
        ...
        tab_size: usize,
        keyboard_mapper: &dyn inazuma::PlatformKeyboardMapper,
    ) -> Result<String> {
...
impl<'a> KeybindUpdateTarget<'a> {
    fn action_value(&self) -> Result<Value> {
        if self.action_name == inazuma::NoAction.name() {
...
#[cfg(test)]
mod tests {
    use inazuma::{Action, App, DummyKeyboardMapper, KeybindingKeystroke, Keystroke, Unbind};
    ...
    inazuma::actions!(test_keymap_file, [StringAction, InputAction]);

    #[test]
    fn can_deserialize_toml_keymap() {
    ...
    fn keymap_section_unbinds_are_loaded_before_bindings(cx: &mut App) {
        let key_bindings = match KeymapFile::load(
            indoc::indoc! {r#"
                ...
                "ctrl-b" = { action = "test_keymap_file::InputAction", data = {} }
            ...
            crate::keymap_file::KeymapFileLoadResult::ParseFailure { error } => {
                panic!("Keymap parse error: {error}");
            }
        };
    ...
    fn keymap_unbind_loads_valid_target_action_with_input(cx: &mut App) {
        let key_bindings = match KeymapFile::load(
            indoc::indoc! {r#"
                ...
                [section.unbind]
                "ctrl-a" = { action = "test_keymap_file::InputAction", data = {} }
            "#},
            cx,
        ) {
            crate::keymap_file::KeymapFileLoadResult::Success { key_bindings } => key_bindings,
    ...
    fn keymap_unbind_rejects_null(cx: &mut App) {
        // In TOML there is no native null. We test with an empty string which
        // deserializes to Value::String("") and should fail action lookup, or
        // we can test the parse-level behavior with a NoAction table.
        // The closest TOML equivalent is omitting the value (parse error) or
        // using an explicit NoAction. We test that NoAction is rejected.
        match KeymapFile::load(
            indoc::indoc! {r#"
                [[section]]

                [section.unbind]
                "ctrl-a" = { action = "vim::NoAction" }
            "#},
            cx,
        ) {
            crate::keymap_file::KeymapFileLoadResult::SomeFailedToLoad {
                ...
                assert!(
                    error_message.0.contains("expected action name string")
                        || error_message.0.contains("didn't find an action named"),
                    "unexpected error: {}",
                    error_message.0,
                );
            }
    ...
    fn keymap_unbind_rejects_unbind_action(cx: &mut App) {
        match KeymapFile::load(
            indoc::indoc! {r#"
                ...
                [section.unbind]
                "ctrl-a" = { action = "carrot::Unbind", data = "test_keymap_file::StringAction" }
            "#},
            cx,
        ) {
            crate::keymap_file::KeymapFileLoadResult::SomeFailedToLoad {
                ...
                assert!(
                    error_message
                        .0
                        .contains("can't use `\"carrot::Unbind\"` as an unbind target.")
                );
            }
            other => panic!("expected SomeFailedToLoad, got {other:?}"),
        }
    ...
    fn keymap_update() {
        ...
        check_keymap_update(
            ...
            KeybindUpdateOperation::add(KeybindUpdateTarget {
                ...
                action_name: "carrot::SomeOtherAction",
                context: None,
                action_arguments: None,
            }),
            r#"[
        ...
        check_keymap_update(
            ...
            KeybindUpdateOperation::add(KeybindUpdateTarget {
                keystrokes: &parse_keystrokes("ctrl-b"),
                action_name: "carrot::SomeOtherAction",
                context: Some("Carrot > Editor && some_condition = true"),
                action_arguments: Some(r#"{"foo": "bar"}"#),
            }),
            r#"[
                {
                ...
                {
                    "context": "Carrot > Editor && some_condition = true",
                    "bindings": {
                        "ctrl-b": [
                            "carrot::SomeOtherAction",
                            {
        ...
        check_keymap_update(
            ...
            KeybindUpdateOperation::Replace {
                target: KeybindUpdateTarget {
                    keystrokes: &parse_keystrokes("ctrl-a"),
                    action_name: "carrot::SomeNonexistentAction",
                    context: None,
                    action_arguments: None,
                },
                source: KeybindUpdateTarget {
