...
use inazuma_collections::{BTreeMap, HashMap, btree_map, hash_map};

use carrot_paths::{local_settings_file_relative_path, task_file_name};
use inazuma_settings_content::ParseStatus;
use inazuma_util::{
    ResultExt as _,
    rel_path::RelPath,
...
pub const LSP_SETTINGS_SCHEMA_URL_PREFIX: &str = "carrot://schemas/settings/lsp/";
...
impl inazuma::Global for DefaultSemanticTokenRules {}
...
impl SettingsStore {
    ...
    pub fn new_with_semantic_tokens(cx: &mut App, default_settings: &str) -> Self {
        ...
        let default_settings: SettingsContent =
            toml::from_str(default_settings).expect("failed to parse default settings TOML");
    ...
    pub fn observe_active_settings_profile_name(cx: &mut App) -> inazuma::Subscription {
    ...
    pub fn update_user_settings(
        ...
        fn trail(this: &mut SettingsStore, content: Box<SettingsContent>, cx: &mut App) {
            let new_text = toml::to_string(&UserSettingsContent {
                content,
                ..Default::default()
            })
            .unwrap();
    ...
    pub async fn load_settings(fs: &Arc<dyn Fs>) -> Result<String> {
        match fs.load(carrot_paths::settings_file()).await {
            result @ Ok(_) => result,
            Err(err) => {
            ...
            .unbounded_send(Box::new(move |cx: AsyncApp| {
                async move {
                    let res = async move {
                        ...
                        let settings_path = carrot_paths::settings_file().as_path();
    ...
    #[inline(always)]
    fn parse_settings<SettingsContentType: RootUserSettings>(
        &mut self,
        user_settings_content: &str,
        file: SettingsFile,
    ) -> (Option<SettingsContentType>, SettingsParseResult) {
        let (settings, parse_status) = if user_settings_content.is_empty() {
            SettingsContentType::parse_toml("")
        } else {
            SettingsContentType::parse_toml(user_settings_content)
        };
...
impl SettingsStore {
    /// Updates the value of a setting in a TOML file, returning the new text.
    pub fn new_text_for_update(
        &self,
        old_text: String,
        update: impl FnOnce(&mut SettingsContent),
    ) -> String {
        let mut content: UserSettingsContent = toml::from_str(&old_text).unwrap_or_default();
        update(&mut content.content);
        toml::to_string_pretty(&content).unwrap_or(old_text)
    }

    /// Updates the value of a setting in a TOML file, returning a list
    /// of edits to apply. For TOML, this returns a single full-text replacement.
    pub fn edits_for_update(
        &self,
        text: &str,
        update: impl FnOnce(&mut SettingsContent),
    ) -> Vec<(Range<usize>, String)> {
        let mut content: UserSettingsContent = toml::from_str(text).unwrap_or_default();
        update(&mut content.content);
        let new_text = toml::to_string_pretty(&content).unwrap_or_default();
        vec![(0..text.len(), new_text)]
    }
    ...
    ///
    /// Sets default settings from a TOML string.
    pub fn set_default_settings(
        &mut self,
        default_settings_content: &str,
        cx: &mut App,
    ) -> Result<()> {
        self.default_settings = toml::from_str(default_settings_content)
            .context("failed to parse default settings TOML")?;
        self.recompute_values(None, cx);
        Ok(())
    }
    ...
    pub fn set_user_settings(
        ...
        let (settings, parse_result) =
            self.parse_settings::<UserSettingsContent>(user_settings_content, SettingsFile::User);
    ...
    pub fn set_global_settings(
        ...
        let (settings, parse_result) =
            self.parse_settings::<SettingsContent>(global_settings_content, SettingsFile::Global);
    ...
    pub fn set_server_settings(
        ...
        let settings = if server_settings_content.is_empty() {
            ...
            Some(
                toml::from_str::<SettingsContent>(server_settings_content)
                    .context("failed to parse server settings TOML")?,
            )
        };
    ...
    pub fn set_local_settings(
        ...
        let mut carrot_settings_changed = false;
        match (path.clone(), kind, content) {
            ...
            (
                ...
                let (new_settings, parse_result) = self.parse_settings::<ProjectSettingsContent>(
                    settings_contents,
...
mod tests {
    ...
    fn test_setting_store_assign_json_before_register(cx: &mut App) {
        ...
        store.set_user_settings("auto_update = false", cx).unwrap();
        store.register_setting::<AutoUpdateSetting>();
    ...
    fn check_settings_update(
        ...
        expected_new_toml: String,
        cx: &mut App,
    ) {
        store.set_user_settings(&old_toml, cx).ok();
        let edits = store.edits_for_update(&old_toml, update);
        let mut new_toml = old_toml;
        for (range, replacement) in edits.into_iter() {
            new_toml.replace_range(range, &replacement);
        }
        pretty_assertions::assert_eq!(new_toml, expected_new_toml);
    }
    ...
    fn test_update_git_settings(cx: &mut App) {
        ...
        let actual = store.new_text_for_update(String::new(), |current| {
            current
                .git
        ...
        // Verify the TOML output contains the git inline_blame setting
        let parsed: toml::Value = toml::from_str(&actual).expect("output should be valid TOML");
        assert_eq!(
            parsed["git"]["inline_blame"]["enabled"].as_bool(),
            Some(true)
        );
    }
    ...
    fn test_get_value_for_field_basic(cx: &mut App) {
        ...
        store
            .set_user_settings("preferred_line_length = 0", cx)
            .unwrap();
    ...
    fn test_get_value_for_field_local_worktrees_dont_interfere(cx: &mut App) {
        ...
        let local_1_child = (
            ...
            RelPath::new(
                ...
                inazuma_util::paths::PathStyle::Posix,
            )
            .unwrap()
            .into_arc(),
        );
    ...
    fn test_get_overrides_for_field(cx: &mut App) {
        ...
        store
            .set_user_settings("preferred_line_length = 100", cx)
            .unwrap();
        ...
        store
            .set_local_settings(
                ...
                LocalSettingsKind::Settings,
                Some("preferred_line_length = 120"),
                cx,
            )
            .unwrap();
        store
        ...
        store
            .set_local_settings(
                ...
                LocalSettingsKind::Settings,
                Some("preferred_line_length = 140"),
                cx,
            )
            .unwrap();
    ...
    fn test_lsp_settings_schema_generation(cx: &mut App) {
        ...
        let schema = SettingsStore::json_schema(&SettingsJsonSchemaParams {
            ...
            font_names: &["Carrot Mono".to_string()],
            theme_names: &["One Dark".into()],
            icon_theme_names: &["Carrot Icons".into()],
            lsp_adapter_names: &[
        ...
        assert_eq!(
            init_options_ref,
            "carrot://schemas/settings/lsp/rust-analyzer/initialization_options"
        );
        ...
        assert_eq!(
            settings_ref,
            "carrot://schemas/settings/lsp/rust-analyzer/settings"
        );
    }
