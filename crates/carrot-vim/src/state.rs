...
use anyhow::Result;
use carrot_command_palette_hooks::{CommandPaletteFilter, GlobalCommandPaletteInterceptor};
use carrot_db::{
    sqlez::{domain::Domain, thread_safe_connection::ThreadSafeConnection},
    sqlez_macros::sql,
};
use carrot_editor::display_map::{is_invisible, replacement};
use carrot_editor::{Anchor, ClipboardSelection, Editor, MultiBuffer, ToPoint as EditorToPoint};
use carrot_language::{Buffer, BufferEvent, BufferId, Chunk, Point};
use inazuma::{
    ...
    pub fn new(workspace: &Workspace, cx: &mut App) -> Entity<MarksState> {
        cx.new(|cx| {
            ...
            let subscription = cx.subscribe(&buffer_store, move |this: &mut Self, _, event, cx| {
                if let carrot_project::buffer_store::BufferStoreEvent::BufferAdded(buffer) = event {
...
pub struct SearchState {
    ...
    pub _dismiss_subscription: Option<inazuma::Subscription>,
}
...
carrot_db::static_connection!(VimDb, [WorkspaceDb]);
