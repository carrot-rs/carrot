use carrot_editor::{Bias, Direction, Editor, display_map::ToDisplayPoint, movement};
...
mod test {
    ...
    #[inazuma::test]
    async fn test_change_list_insert(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_change_list_delete(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_gi(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {
        "one two
        three fˇr"})
    ...
    #[inazuma::test]
    async fn test_dot_mark(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {
        "one two
        three fˇr"})
