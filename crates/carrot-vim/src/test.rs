...
use carrot_lsp as lsp;
use std::{sync::Arc, time::Duration};

use carrot_command_palette::CommandPalette;
use carrot_editor::{
...
use carrot_language::{CursorShape, Language, LanguageConfig, Point};
use carrot_ui::Pixels;
use futures::StreamExt;
use inazuma::{KeyBinding, Modifiers, MouseButton, TestAppContext, px};
...
use carrot_search::BufferSearchBar;
use carrot_search::{ProjectSearchView, project_search};
use carrot_workspace::{DeploySearch, Workspace};
use inazuma::VisualTestContext;
use indoc::indoc;
use serde_json::json;
...
#[perf]
#[inazuma::test]
async fn test_initially_disabled(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_neovim(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_toggle_through_settings(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_vim_linked_edits_delete_x(app_cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_vim_linked_edits_change_iw(app_cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new_html(app_cx).await;
...
#[perf]
#[inazuma::test]
async fn test_vim_linked_edits_substitute_s(app_cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new_html(app_cx).await;
...
#[perf]
#[inazuma::test]
async fn test_vim_linked_edits_visual_change(app_cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new_html(app_cx).await;
...
#[perf]
#[inazuma::test]
async fn test_vim_linked_edits_visual_substitute_s(app_cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new_html(app_cx).await;
...
#[perf]
#[inazuma::test]
async fn test_cancel_selection(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_buffer_search(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_count_down(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_end_of_document_710(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
    ...
    // can go to line 1 (https://github.com/carrot-rs/carrot/issues/5812)
...
#[perf]
#[inazuma::test]
async fn test_end_of_line_with_times(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_indent_outdent(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_escape_command_palette(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_escape_cancels(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_selection_on_search(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_word_characters(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_kebab_case(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_join_lines(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_wrapped_lines(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_folds(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_folds_panic(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_neovim_option("foldmethod=manual").await;
...
#[perf]
#[inazuma::test]
async fn test_clear_counts(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_zero(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_selection_goal(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_wrapped_motions(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_wrapped_delete_end_document(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_paragraphs_dont_wrap(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_select_all_issue_2170(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_jk(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
fn assert_pending_input(cx: &mut VimTestContext, expected: &str) {
    cx.update_editor(|editor, window, cx| {
        ...
        let highlights = editor
            .text_highlights(carrot_editor::HighlightKey::PendingInput, cx)
            .unwrap()
            .1;
...
#[perf]
#[inazuma::test]
async fn test_jk_multi(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_jk_delay(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_jk_max_count(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_comma_w(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[inazuma::test]
async fn test_go_to_definition(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new_typescript(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_remap(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g z",
            carrot_workspace::SendKeystrokes("l l l l".to_string()),
            None,
        )])
    });
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g y",
            carrot_workspace::SendKeystrokes("i f o o escape l".to_string()),
            None,
        )])
    });
    cx.set_state("ˇ123456789", Mode::Normal);
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g x",
            carrot_workspace::SendKeystrokes("g z g y".to_string()),
            None,
        )])
    });
    cx.set_state("ˇ123456789", Mode::Normal);
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g w",
            carrot_workspace::SendKeystrokes(": j enter".to_string()),
            None,
        )])
    });
    cx.set_state("ˇ1234\n56789", Mode::Normal);
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g u",
            carrot_workspace::SendKeystrokes("g w g z".to_string()),
            None,
        )])
    });
    cx.set_state("ˇ1234\n56789", Mode::Normal);
    ...
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "g t",
            carrot_workspace::SendKeystrokes("i space escape".to_string()),
            None,
        )])
    });
...
#[perf]
#[inazuma::test]
async fn test_undo(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_dw_eol(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
async fn test_toggle_comments(cx: &mut inazuma::TestAppContext) {
    ...
    let language = std::sync::Arc::new(carrot_language::Language::new(
        carrot_language::LanguageConfig {
            line_comments: vec!["// ".into(), "//! ".into(), "/// ".into()],
            ..Default::default()
        },
        Some(carrot_language::tree_sitter_rust::LANGUAGE.into()),
    ));
...
#[perf]
#[inazuma::test]
async fn test_find_multibyte(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_sneak(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_plus_minus(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_command_alias(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_remap_adjacent_dog_cat(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.update(|_, cx| {
        cx.bind_keys([
            KeyBinding::new(
                "d o g",
                carrot_workspace::SendKeystrokes("🐶".to_string()),
                Some("vim_mode == insert"),
            ),
            KeyBinding::new(
                "c a t",
...
#[perf]
#[inazuma::test]
async fn test_remap_nested_pineapple(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.update(|_, cx| {
        cx.bind_keys([
            KeyBinding::new(
...
#[perf]
#[inazuma::test]
async fn test_remap_recursion(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.update(|_, cx| {
        cx.bind_keys([KeyBinding::new(
            "x",
            carrot_workspace::SendKeystrokes("\" _ x".to_string()),
            Some("VimControl"),
        )]);
        cx.bind_keys([KeyBinding::new(
            "y",
            carrot_workspace::SendKeystrokes("2 x".to_string()),
            Some("VimControl"),
        )])
    });
...
#[perf]
#[inazuma::test]
async fn test_escape_while_waiting(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_ctrl_w_override(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.update(|_, cx| {
...
#[perf]
#[inazuma::test]
async fn test_visual_indent_count(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[perf]
#[inazuma::test]
async fn test_record_replay_recursion(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_blackhole_register(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_sentence_backwards(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_sentence_forwards(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_ctrl_o_visual(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_ctrl_o_position(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf]
#[inazuma::test]
async fn test_ctrl_o_dot(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[perf(iterations = 1)]
#[inazuma::test]
async fn test_folded_multibuffer_excerpts(cx: &mut inazuma::TestAppContext) {
...
#[perf]
#[inazuma::test]
async fn test_delete_paragraph_motion(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_shared_state(indoc! {
        "ˇhello world.
...
#[perf]
#[inazuma::test]
async fn test_delete_unmatched_brace(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_shared_state(indoc! {
        "fn o(wow: i32) {
          othˇ(wow)
...
#[perf]
#[inazuma::test]
async fn test_paragraph_multi_delete(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_shared_state(indoc! {
        "
        Emacs is
...
#[perf]
#[inazuma::test]
async fn test_yank_paragraph_with_paste(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_shared_state(indoc! {
        "
        first paragraph
...
#[perf]
#[inazuma::test]
async fn test_change_paragraph(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
    cx.set_shared_state(indoc! {
        "
        first paragraph
...
#[perf]
#[inazuma::test]
async fn test_multi_cursor_replay(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
    cx.set_state(
        indoc! {
            "
...
#[inazuma::test]
async fn test_clipping_on_mode_change(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[inazuma::test]
async fn test_wrap_selections_in_tag_line_mode(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;

    let js_language = Arc::new(Language::new(
        LanguageConfig {
            name: "JavaScript".into(),
            wrap_characters: Some(carrot_language::WrapCharactersConfig {
                start_prefix: "<".into(),
                start_suffix: ">".into(),
                end_prefix: "</".into(),
                end_suffix: ">".into(),
...
#[inazuma::test]
async fn test_repeat_grouping_41735(cx: &mut inazuma::TestAppContext) {
    let mut cx = NeovimBackedTestContext::new(cx).await;
...
#[inazuma::test]
async fn test_deactivate(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
// carrot_workspace::SendKeystrokes should pass literal keystrokes without triggering vim motions.
// When sending `" _ x`, the `_` should select the blackhole register, not trigger
// vim::StartOfLineDownward.
#[inazuma::test]
async fn test_send_keystrokes_underscore_is_literal_46509(cx: &mut inazuma::TestAppContext) {
    let mut cx = VimTestContext::new(cx, true).await;
...
#[inazuma::test]
async fn test_send_keystrokes_no_key_equivalent_mapping_46509(cx: &mut inazuma::TestAppContext) {
    use inazuma::{KeybindingKeystroke, Keystroke, PlatformKeyboardMapper};
    use inazuma_collections::HashMap;
...
#[inazuma::test]
async fn test_project_search_opens_in_normal_mode(cx: &mut inazuma::TestAppContext) {
    VimTestContext::init(cx);
    ...
    let project = carrot_project::Project::test(fs.clone(), [path!("/dir").as_ref()], cx).await;
    let window_handle =
