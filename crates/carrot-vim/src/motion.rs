...
use carrot_language::{CharKind, Point, Selection, SelectionGoal};
use carrot_multi_buffer::MultiBufferRow;
use carrot_workspace::searchable::Direction;
use inazuma::{Action, Context, Window, actions, px};
...
impl Vim {
    pub(crate) fn search_motion(&mut self, m: Motion, window: &mut Window, cx: &mut Context<Self>) {
        if let Motion::CarrotSearchResult {
            prior_selections, ..
        } = &m
        {
            match self.mode {
...
impl Motion {
    fn default_kind(&self) -> MotionKind {
        ...
        match self {
            ...
            | SneakBackward { .. }
            | Jump { .. }
            | CarrotSearchResult { .. } => MotionKind::Exclusive,
    ...
    pub fn move_point(
        ...
        let (new_point, goal) = match self {
            ...
            CarrotSearchResult { new_selections, .. } => {
...
fn up_down_buffer_rows(
    ...
    //
    // See: https://github.com/carrot-rs/carrot/issues/29134
...
fn go_to_line(map: &DisplaySnapshot, display_point: DisplayPoint, line: usize) -> DisplayPoint {
    ...
    for (excerpt, buffer, range) in map.buffer_snapshot().excerpts() {
        let excerpt_range = carrot_language::ToOffset::to_offset(&range.context.start, buffer)
            ..carrot_language::ToOffset::to_offset(&range.context.end, buffer);
...
fn matching(
    ...
    let make_range_filter = |require_on_bracket: bool| {
        move |buffer: &carrot_language::BufferSnapshot,
...
fn method_motion(
    ...
    for _ in 0..times {
        ...
        let possibilities = snapshot
            .text_object_ranges(
                range,
                carrot_language::TreeSitterOptions::max_start_depth(4),
            )
            .filter_map(|(range, object)| {
                if !matches!(object, carrot_language::TextObject::AroundFunction) {
                    return None;
                }
...
fn comment_motion(
    ...
    for _ in 0..times {
        ...
        let possibilities = snapshot
            .text_object_ranges(
                range,
                carrot_language::TreeSitterOptions::max_start_depth(6),
            )
            .filter_map(|(range, object)| {
                if !matches!(object, carrot_language::TextObject::AroundComment) {
                    return None;
                }
    ...
    if map.buffer_snapshot().as_singleton().is_some() {
        for _ in 0..times {
            ...
            let mut possibilities = map
                ...
                .text_object_ranges(
                    range,
                    carrot_language::TreeSitterOptions::max_start_depth(3),
                )
                .filter(|(_, object)| {
                    matches!(
                        object,
                        carrot_language::TextObject::AroundClass
                            | carrot_language::TextObject::AroundFunction
                    )
                })
                .collect::<Vec<_>>();
            ...
            let possibilities = possibilities.into_iter().filter_map(|(range, t)| {
                if t == carrot_language::TextObject::AroundFunction
...
mod test {
    ...
    #[inazuma::test]
    async fn test_start_end_of_paragraph(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_paragraph_motion_with_whitespace_lines(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_matching(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_matching_quotes_disabled(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_matching_quotes_enabled(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_unmatched_forward(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_unmatched_backward(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_unmatched_forward_markdown(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_unmatched_backward_markdown(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new_markdown_with_rust(cx).await;
    ...
    #[inazuma::test]
    async fn test_matching_tags(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_matching_tag_with_quotes(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new_html(cx).await;
        cx.update(|_, cx| {
            cx.bind_keys([KeyBinding::new(
                "%",
    ...
    #[inazuma::test]
    async fn test_matching_braces_in_tag(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_matching_nested_brackets(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_comma_semicolon(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_next_word_end_newline_last_char(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_next_line_start(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_end_of_line_downward(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_end_of_line_with_vertical_motion(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_window_top(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_window_middle(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_window_bottom(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        let initial_state = indoc! {r"abc
          deˇf
          paragraph
    ...
    #[inazuma::test]
    async fn test_previous_word_end(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {r"
        456 5ˇ67 678
        "})
    ...
    #[inazuma::test]
    async fn test_visual_match_eol(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_clipping_with_inlay_hints(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_clipping_with_inlay_hints_end_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_visual_mode_with_inlay_hints_on_empty_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_go_to_percentage(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        // Normal mode
        cx.set_shared_state(indoc! {"
            The ˇquick brown
    ...
    #[inazuma::test]
    async fn test_space_non_ascii(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_space_non_ascii_eol(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_backspace_non_ascii_bol(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_go_to_indent(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.set_state(
            indoc! {
    ...
    #[inazuma::test]
    async fn test_delete_key_can_remove_last_character(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_forced_motion_delete_to_start_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_forced_motion_delete_to_middle_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_forced_motion_delete_to_end_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_forced_motion_yank(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_inclusive_to_exclusive_delete(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_next_subword_start(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_next_subword_end(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_previous_subword_start(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_previous_subword_end(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_method_motion_with_expanded_diff_hunks(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        ...
        cx.update_editor(|editor, window, cx| {
            editor.expand_all_diff_hunks(&carrot_editor::actions::ExpandAllDiffHunks, window, cx);
        });
    ...
    #[inazuma::test]
    async fn test_comment_motion_with_expanded_diff_hunks(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
