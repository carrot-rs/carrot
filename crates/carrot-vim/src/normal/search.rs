...
use carrot_search::{BufferSearchBar, SearchOptions, buffer_search};
use carrot_workspace::{notifications::NotifyResultExt, searchable::Direction};
use inazuma::{Action, Context, Window, actions};
    ...
    fn replace_command(
        ...
        cx.spawn_in(window, async move |vim, cx| {
            ...
            search_bar.update_in(cx, |search_bar, window, cx| {
                ...
                let _receiver = search_bar.search(&search_bar.query(cx), None, false, window, cx);
                vim.update(cx, |vim, cx| {
                    vim.move_cursor(
                        Motion::StartOfLine {
                            display_lines: false,
...
mod test {
    ...
    use carrot_search as search;
    use carrot_search::BufferSearchBar;
    use inazuma_settings_framework::SettingsStore;
    use indoc::indoc;

    #[inazuma::test]
    async fn test_move_to_next(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_move_to_next_with_no_search_wrap(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_non_vim_search(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_visual_star_hash(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_d_search(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_backwards_n(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_v_search(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_v_search_aa(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_visual_block_search(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_replace_with_range_at_start(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_search_skipping(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {
            "ˇaa aa aa"
        })
    ...
    #[inazuma::test]
    async fn test_replace_n(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {
            "ˇaa
            bb
    ...
    #[inazuma::test]
    async fn test_replace_g(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {
            "ˇaa aa aa aa
            aa
    ...
    async fn test_replace_gdefault(cx: &mut inazuma::TestAppContext) {
        ...
        // Set the `gdefault` option in both Carrot and Neovim.
    ...
    #[inazuma::test]
    async fn test_replace_c(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.set_state(
            indoc! {
                "ˇaa
    ...
    #[inazuma::test]
    async fn test_replace_with_range(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_search_dismiss_restores_cursor(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_search_dismiss_restores_cursor_no_matches(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.set_state("ˇapple\nbanana\ncherry\n", Mode::Normal);
