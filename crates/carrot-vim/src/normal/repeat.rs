...
use inazuma::{Action, App, Context, Window, actions};
...
fn should_replay(action: &dyn Action) -> bool {
    ...
    if carrot_editor::actions::ShowCharacterPalette.partial_eq(action) {
        return false;
    }
    true
}
...
#[cfg(test)]
mod test {
    use carrot_editor::test::editor_lsp_test_context::EditorLspTestContext;
    use carrot_lsp as lsp;
    use futures::StreamExt;
    use indoc::indoc;

    use inazuma::EntityInputHandler;
    ...
    #[inazuma::test]
    async fn test_dot_repeat(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_dot_repeat_registers_paste(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    // by default.
    #[inazuma::test]
    async fn test_dot_repeat_registers_replace(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_repeat_ime(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_repeat_completion(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_repeat_completion_unicode_bug(cx: &mut inazuma::TestAppContext) {
        VimTestContext::init(cx);
        let cx = EditorLspTestContext::new_rust(
            lsp::ServerCapabilities {
                completion_provider: Some(lsp::CompletionOptions {
    ...
    #[inazuma::test]
    async fn test_repeat_visual(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_repeat_motion_counts(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_interrupted(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_repeat_over_blur(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_repeat_after_blur_resets_dot_replaying(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        ...
        cx.update(|_, cx| {
            cx.bind_keys([inazuma::KeyBinding::new(
                "ctrl-f",
                carrot_search::buffer_search::Deploy::find(),
                None,
            )])
        });
    ...
    #[inazuma::test]
    async fn test_undo_repeated_insert(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_replay(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_replay_count(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_replay_dot(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_replay_of_dot(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_record_replay_interleaved(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_repeat_clear(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_repeat_clear_repeat(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_repeat_clear_count(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
