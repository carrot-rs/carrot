use carrot_editor::{Editor, MultiBufferSnapshot, ToOffset, ToPoint};
use carrot_language::{Bias, Point};
use inazuma::{Action, Context, Window};
            ...
            for selection in editor.selections.all_adjusted(&editor.display_snapshot(cx)) {
                ...
                for row in selection.start.row..=selection.end.row {
                    ...
                    let end = if row == selection.end.row {
                        ...
                        Point::new(
                            row,
                            snapshot.line_len(carrot_multi_buffer::MultiBufferRow(row)),
                        )
                    };
...
mod test {
    ...
    #[inazuma::test]
    async fn test_increment(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_increment_with_dot(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_with_leading_zeros(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_with_leading_zeros_and_zero(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_with_changing_leading_zeros(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_with_two_dots(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_sign_change(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
                ˇ0
                "})
    ...
    #[inazuma::test]
    async fn test_increment_sign_change_with_leading_zeros(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
                00ˇ1
                "})
    ...
    #[inazuma::test]
    async fn test_increment_bin_wrapping_and_padding(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
    ...
    #[inazuma::test]
    async fn test_increment_hex_wrapping_and_padding(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
    ...
    #[inazuma::test]
    async fn test_increment_wrapping(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
    ...
    #[inazuma::test]
    async fn test_increment_inline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
                    inline0x3ˇ9u32
                    "})
    ...
    #[inazuma::test]
    async fn test_increment_hex_casing(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state(indoc! {"
                        0xFˇa
                    "})
    ...
    #[inazuma::test]
    async fn test_increment_radix(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_steps(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_toggle(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_increment_order(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_increment_visual_partial_number(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_markdown_list_markers_multiline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_increment_with_multibyte_characters(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
