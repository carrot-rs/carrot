...
use carrot_search::{BufferSearchBar, project_search::ProjectSearchBar};
use inazuma::{Context, Entity, UpdateGlobal};
use semver::Version;
...
impl VimTestContext {
    pub fn init(cx: &mut inazuma::TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            carrot_release_channel::init(Version::new(0, 0, 0), cx);
    ...
    pub async fn new(cx: &mut inazuma::TestAppContext, enabled: bool) -> VimTestContext {
        Self::init(cx);
        let lsp = EditorLspTestContext::new_rust(Default::default(), cx).await;
        Self::new_with_lsp(lsp, enabled)
    }

    pub async fn new_html(cx: &mut inazuma::TestAppContext) -> VimTestContext {
        Self::init(cx);
        Self::new_with_lsp(EditorLspTestContext::new_html(cx).await, true)
    }

    pub async fn new_markdown_with_rust(cx: &mut inazuma::TestAppContext) -> VimTestContext {
        Self::init(cx);
        Self::new_with_lsp(EditorLspTestContext::new_markdown_with_rust(cx).await, true)
    }

    pub async fn new_typescript(cx: &mut inazuma::TestAppContext) -> VimTestContext {
        Self::init(cx);
        Self::new_with_lsp(
    ...
    pub async fn new_tsx(cx: &mut inazuma::TestAppContext) -> VimTestContext {
        Self::init(cx);
        Self::new_with_lsp(
