use inazuma::{AppContext as _, UpdateGlobal, px, size};
...
impl SharedState {
    /// Assert that both Carrot and NeoVim have the same content and mode.
    #[track_caller]
    pub fn assert_matches(&self) {
        if self.neovim != self.editor || self.neovim_mode != self.editor_mode {
            panic!(
                indoc! {"Test failed (carrot does not match nvim behavior)
                    # initial state:
                    {}
                    # keystrokes:
                    {}
...
impl NeovimBackedTestContext {
    pub async fn new(cx: &mut inazuma::TestAppContext) -> NeovimBackedTestContext {
        #[cfg(feature = "neovim")]
    ...
    pub async fn new_html(cx: &mut inazuma::TestAppContext) -> NeovimBackedTestContext {
        #[cfg(feature = "neovim")]
        cx.executor().allow_parking();
        // rust stores the name of the test on the current thread.
        // We use this to automatically name a file that will store
    ...
    pub async fn new_markdown_with_rust(
    ...
    pub async fn new_typescript(cx: &mut inazuma::TestAppContext) -> NeovimBackedTestContext {
        #[cfg(feature = "neovim")]
        cx.executor().allow_parking();
        // rust stores the name of the test on the current thread.
        // We use this to automatically name a file that will store
    ...
    pub async fn new_tsx(cx: &mut inazuma::TestAppContext) -> NeovimBackedTestContext {
        #[cfg(feature = "neovim")]
        cx.executor().allow_parking();
        let thread = thread::current();
        let test_name = thread
    ...
    pub async fn set_scroll_height(&mut self, rows: u32) {
        // match Carrot's scrolling behavior
