    ...
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        ...
        workspace.register_action(|_, _: &ToggleProjectPanelFocus, window, cx| {
            if Vim::take_count(cx).is_none() {
                window
                    .dispatch_action(carrot_actions::project_panel::ToggleFocus.boxed_clone(), cx);
            }
        });
        ...
        workspace.register_action(|_, _: &carrot_actions::vim::OpenDefaultKeymap, _, cx| {
            cx.emit(carrot_workspace::Event::OpenBundledFile {
        ...
        workspace.register_action(|_, _: &GoToTab, window, cx| {
            ...
            if let Some(tab_index) = count {
                ...
                window.dispatch_action(
                    carrot_workspace::pane::ActivateItem(zero_based_index).boxed_clone(),
                    cx,
                );
            } else {
                // If no count is provided, go to the next tab.
                window.dispatch_action(carrot_workspace::pane::ActivateNextItem.boxed_clone(), cx);
            }
        });
            ...
            if let Some(count) = count {
                ...
                if item_count > 0 {
                    ...
                    let target_index = (current_index as isize - count as isize)
                        ...
                        as usize;
                    window.dispatch_action(
                        carrot_workspace::pane::ActivateItem(target_index).boxed_clone(),
                        cx,
                    );
                }
            } else {
                ...
                window.dispatch_action(
                    carrot_workspace::pane::ActivatePreviousItem.boxed_clone(),
                    cx,
                );
            }
        });
...
impl carrot_editor::Addon for VimAddon {
...
pub(crate) struct Vim {
    ...
    pub(crate) replacements: Vec<(Range<carrot_editor::Anchor>, String)>,
    ...
    fn activate(editor: &mut Editor, window: &mut Window, cx: &mut Context<Editor>) {
        ...
        vim.update(cx, |_, cx| {
            ...
            Vim::action(
                editor,
                cx,
                |vim, action: &carrot_editor::actions::AcceptEditPrediction, window, cx| {
            ...
            Vim::action(
                editor,
                cx,
                |vim, _: &carrot_editor::actions::Paste, window, cx| match vim.mode {
                ...
                if self.mode == Mode::Normal {
                    self.update_editor(cx, |_, editor, cx| {
                        editor.accept_edit_prediction(
                            &carrot_editor::actions::AcceptEditPrediction {},
                            window,
                            cx,
                        );
                    });
...
impl From<inazuma_settings_framework::VimInsertModeCursorShape> for InsertModeCursorShape {
    fn from(shape: inazuma_settings_framework::VimInsertModeCursorShape) -> Self {
        match shape {
            inazuma_settings_framework::VimInsertModeCursorShape::Inherit => {
                InsertModeCursorShape::Inherit
            }
