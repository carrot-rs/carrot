...
use carrot_editor::{Anchor, SelectionEffects};
use carrot_editor::{Bias, ToPoint};
use carrot_editor::{display_map::ToDisplayPoint, movement};
use carrot_language::{Point, SelectionGoal};
use carrot_multi_buffer::MultiBufferRow;
use convert::ConvertTarget;
use inazuma::{Context, Window, actions};
...
pub(crate) fn register(editor: &mut Editor, cx: &mut Context<Vim>) {
    ...
    Vim::action(editor, cx, |vim, _: &Undo, window, cx| {
        ...
        vim.update_editor(cx, |_, editor, cx| {
            for _ in 0..times.unwrap_or(1) {
                editor.undo(&carrot_editor::actions::Undo, window, cx);
            }
        });
    });
    Vim::action(editor, cx, |vim, _: &Redo, window, cx| {
        ...
        vim.update_editor(cx, |_, editor, cx| {
            for _ in 0..times.unwrap_or(1) {
                editor.redo(&carrot_editor::actions::Redo, window, cx);
            }
        });
    });
    ...
    pub(crate) fn normal_replace(
        ...
        self.update_editor(cx, |_, editor, cx| {
            editor.transact(window, cx, |editor, window, cx| {
                ...
                editor.edit(edits, cx);
                if is_return_char {
                    editor.newline(&carrot_editor::actions::Newline, window, cx);
                }
...
#[cfg(test)]
mod test {
    use inazuma::{KeyBinding, TestAppContext, UpdateGlobal};
    ...
    #[inazuma::test]
    async fn test_h(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_backspace(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "backspace",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_j(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_enter(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "enter",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_k(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "k",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_l(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "l",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_jump_to_line_boundaries(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "$",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_jump_to_end(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_w(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "w",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_end_of_word(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "e",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_b(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "b",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_gg(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "g g",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_end_of_document(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "shift-g",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_a(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_insert_end_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate_at_each_offset(
            "shift-a",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_jump_to_first_non_whitespace(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_insert_first_non_whitespace(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_delete_to_end_of_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.simulate(
            "shift-d",
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_x(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_delete_left(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_o(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_insert_line_above(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_insert_empty_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_dd(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_cc(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_repeated_word(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_h_through_unicode(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_f_and_t(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_capital_f_and_capital_t(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        let test_case = indoc! {"
            ˇaaaˇbˇ ˇbˇ   ˇbˇbˇ aˇaaˇbaaa
            ˇ    ˇbˇaaˇa ˇbˇbˇb
    ...
    #[inazuma::test]
    async fn test_f_and_t_smartcase(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_end_of_line_with_neovim(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_subword_motions(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.update(|_, cx| {
            cx.bind_keys(vec![
                KeyBinding::new(
    ...
    #[inazuma::test]
    async fn test_r(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_gq(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_o_comment(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_yank_line_with_trailing_newline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_yank_line_without_trailing_newline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_yank_multiline_without_trailing_newline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state("heˇllo\nhello").await;
    ...
    #[inazuma::test]
    async fn test_dd_then_paste_without_trailing_newline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
        cx.set_shared_state("heˇllo").await;
    ...
    #[inazuma::test]
    async fn test_visual_mode_insert_before_after(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_jump_list(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_undo_last_line(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_undo_last_line_newline(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_undo_last_line_newline_many_changes(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
    ...
    #[inazuma::test]
    async fn test_undo_last_line_multicursor(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_go_to_tab_with_count(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_go_to_previous_tab_with_count(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
    ...
    #[inazuma::test]
    async fn test_temporary_mode(cx: &mut inazuma::TestAppContext) {
        let mut cx = NeovimBackedTestContext::new(cx).await;
