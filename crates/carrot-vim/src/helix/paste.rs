use carrot_editor::{ToOffset, movement};
...
mod test {
    ...
    #[inazuma::test]
    async fn test_system_clipboard_paste(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_point_selection_paste(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.enable_helix();
        cx.set_state(
            indoc! {"
    ...
    #[inazuma::test]
    async fn test_multi_cursor_paste(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.enable_helix();
    ...
    #[inazuma::test]
    async fn test_line_mode_paste(cx: &mut inazuma::TestAppContext) {
        let mut cx = VimTestContext::new(cx, true).await;
        cx.enable_helix();
        cx.set_state(
            indoc! {"
