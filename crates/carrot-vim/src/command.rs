...
use carrot_actions::{OpenDocs, RevealTarget};
use carrot_command_palette_hooks::{CommandInterceptItem, CommandInterceptResult};
use carrot_editor::{
...
use carrot_search::{BufferSearchBar, SearchOptions};
use carrot_task::{HideStrategy, RevealStrategy, SaveStrategy, SpawnInTerminal, TaskId};
use carrot_ui::ActiveTheme;
use carrot_workspace::{Item, SaveIntent, Workspace, notifications::NotifyResultExt};
use carrot_workspace::{SplitDirection, notifications::DetachAndPromptErr};
            ...
            vim.update_editor(cx, |_, editor, cx| match option {
                VimOption::Wrap(true) => {
                    editor.set_soft_wrap_mode(
                        carrot_language::language_settings::SoftWrap::EditorWidth,
                        cx,
                    );
                }
                VimOption::Wrap(false) => {
                    editor
                        .set_soft_wrap_mode(carrot_language::language_settings::SoftWrap::None, cx);
                }
...
fn generate_commands(_: &App) -> Vec<VimCommand> {
    vec![
        ...
        VimCommand::new(("e", "dit"), carrot_editor::actions::ReloadFile)
            .bang(carrot_editor::actions::ReloadFile)
        ...
        VimCommand::new(("sp", "lit"), carrot_workspace::SplitHorizontal::default()).filename(
            |_, filename| {
                Some(
                    VimSplit {
                        vertical: false,
        ...
        VimCommand::new(("vs", "plit"), carrot_workspace::SplitVertical::default()).filename(
            |_, filename| {
                Some(
                    VimSplit {
                        vertical: true,
        ...
        VimCommand::new(("tabe", "dit"), carrot_workspace::NewFile)
            .filename(|_action, filename| Some(VimEdit { filename }.boxed_clone())),
        VimCommand::new(("tabnew", ""), carrot_workspace::NewFile)
            .filename(|_action, filename| Some(VimEdit { filename }.boxed_clone())),
        VimCommand::new(
            ("q", "uit"),
            carrot_workspace::CloseActiveItem {
                save_intent: Some(SaveIntent::Close),
            },
        )
        .bang(carrot_workspace::CloseActiveItem {
        ...
        VimCommand::new(
            ("up", "date"),
            carrot_workspace::Save {
                save_intent: Some(SaveIntent::SaveAll),
            },
        ),
        VimCommand::new(
            ("wa", "ll"),
            carrot_workspace::SaveAll {
                save_intent: Some(SaveIntent::SaveAll),
            },
        )
        .bang(carrot_workspace::SaveAll {
            save_intent: Some(SaveIntent::Overwrite),
        }),
        VimCommand::new(
            ("qa", "ll"),
            carrot_workspace::CloseAllItemsAndPanes {
                save_intent: Some(SaveIntent::Close),
            },
        )
        .bang(carrot_workspace::CloseAllItemsAndPanes {
            save_intent: Some(SaveIntent::Skip),
        }),
        VimCommand::new(
            ("quita", "ll"),
        ...
        VimCommand::new(("cq", "uit"), carrot_actions::Quit),
        VimCommand::new(
            ("bd", "elete"),
            carrot_workspace::CloseItemInAllPanes {
                save_intent: Some(SaveIntent::Close),
            },
        )
        .bang(carrot_workspace::CloseItemInAllPanes {
            save_intent: Some(SaveIntent::Skip),
        }),
        VimCommand::new(
            ("norm", "al"),
        ...
        VimCommand::new(("bn", "ext"), carrot_workspace::ActivateNextItem).count(),
        VimCommand::new(("bN", "ext"), carrot_workspace::ActivatePreviousItem).count(),
        VimCommand::new(("bp", "revious"), carrot_workspace::ActivatePreviousItem).count(),
        VimCommand::new(("bf", "irst"), carrot_workspace::ActivateItem(0)),
        VimCommand::new(("br", "ewind"), carrot_workspace::ActivateItem(0)),
        VimCommand::new(("bl", "ast"), carrot_workspace::ActivateLastItem),
        VimCommand::str(("buffers", ""), "tab_switcher::ToggleAll"),
        VimCommand::str(("ls", ""), "tab_switcher::ToggleAll"),
        VimCommand::new(("new", ""), carrot_workspace::NewFileSplitHorizontal),
        VimCommand::new(("vne", "w"), carrot_workspace::NewFileSplitVertical),
        VimCommand::new(("tabn", "ext"), carrot_workspace::ActivateNextItem).count(),
        VimCommand::new(("tabp", "revious"), carrot_workspace::ActivatePreviousItem).count(),
        VimCommand::new(("tabN", "ext"), carrot_workspace::ActivatePreviousItem).count(),
        VimCommand::new(
            ("tabc", "lose"),
            carrot_workspace::CloseActiveItem {
                save_intent: Some(SaveIntent::Close),
        ...
        VimCommand::new(
            ("tabo", "nly"),
            carrot_workspace::CloseOtherItems {
                save_intent: Some(SaveIntent::Close),
            },
        )
        .bang(carrot_workspace::CloseOtherItems {
            save_intent: Some(SaveIntent::Skip),
        }),
        VimCommand::new(
            ("on", "ly"),
            carrot_workspace::CloseInactiveTabsAndPanes {
                save_intent: Some(SaveIntent::Close),
            },
        )
        .bang(carrot_workspace::CloseInactiveTabsAndPanes {
            save_intent: Some(SaveIntent::Skip),
        }),
        VimCommand::str(("cl", "ist"), "diagnostics::Deploy"),
        VimCommand::new(("cc", ""), carrot_editor::actions::Hover),
        VimCommand::new(("ll", ""), carrot_editor::actions::Hover),
        VimCommand::new(
            ("cn", "ext"),
        ...
        VimCommand::new(("fo", "ld"), carrot_editor::actions::FoldSelectedRanges)
            .range(act_on_range),
        VimCommand::new(("foldo", "pen"), carrot_editor::actions::UnfoldLines)
            .bang(carrot_editor::actions::UnfoldRecursive)
            .range(act_on_range),
        VimCommand::new(("foldc", "lose"), carrot_editor::actions::Fold)
            .bang(carrot_editor::actions::FoldRecursive)
            .range(act_on_range),
        VimCommand::new(
            ("dif", "fupdate"),
            carrot_editor::actions::ToggleSelectedDiffHunks,
        )
        .range(act_on_range),
        VimCommand::str(("rev", "ert"), "git::Restore").range(act_on_range),
        VimCommand::new(("d", "elete"), VisualDeleteLine).range(select_range),
        VimCommand::new(("y", "ank"), inazuma::NoAction).range(|_, range| {
            Some(
                YankCommand {
                    range: range.clone(),
                }
        ...
        VimCommand::new(("noh", "lsearch"), carrot_search::buffer_search::Dismiss),
        VimCommand::new(("$", ""), EndOfDocument),
        VimCommand::new(("%", ""), EndOfDocument),
        VimCommand::new(("0", ""), StartOfDocument),
        VimCommand::new(("ex", ""), carrot_editor::actions::ReloadFile)
            .bang(carrot_editor::actions::ReloadFile),
        VimCommand::new(("cpp", "link"), carrot_editor::actions::CopyPermalinkToLine)
            .range(act_on_range),
        VimCommand::str(("opt", "ions"), "carrot::OpenDefaultSettings"),
...
impl OnMatchingLines {
    // convert a vim query into something more usable by carrot.
    ...
    pub fn run(&self, vim: &mut Vim, window: &mut Window, cx: &mut Context<Vim>) {
        ...
        vim.update_editor(cx, |vim, editor, cx| {
            ...
            editor.highlight_rows::<ShellExec>(
                ...
                cx.theme().status().unreachable.background,
                Default::default(),
                cx,
            );
...
mod test {
    ...
    use carrot_editor::{Editor, EditorSettings};
    use carrot_workspace::{OpenOptions, Workspace};
    use inazuma::{Context, TestAppContext};
