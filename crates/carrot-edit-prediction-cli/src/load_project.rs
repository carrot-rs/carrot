...
use carrot_language::{Anchor, Buffer, LanguageNotFound, ToOffset};
use carrot_project::{Project, ProjectPath, buffer_store::BufferStoreEvent};
...
pub async fn run_load_project(
    ...
    let events: Vec<Arc<carrot_zeta_prompt::Event>> = ep_store.update(&mut cx, |store, cx| {
        store
...
async fn cursor_position(
    ...
    // paths like "carrot/crates/foo.rs" instead of "crates/foo.rs".
...
async fn apply_edit_history(
    ...
    carrot_edit_prediction::udiff::apply_diff(&example.spec.edit_history, project, cx).await
}
