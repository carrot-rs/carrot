...
use carrot_sqlez::bindable::Bind;
use carrot_sqlez::bindable::StaticColumnCount;
use carrot_sqlez_macros::sql;
...
impl PlainLlmClient {
    pub fn new() -> Result<Self> {
        let http_client: Arc<dyn carrot_http_client::HttpClient> = Arc::new(ReqwestClient::new());
        ...
        while let Some(event_result) = stream.next().await {
            ...
            match event {
                ...
                Event::ContentBlockDelta { delta, .. } => {
                    if let carrot_anthropic::ContentDelta::TextDelta { text } = delta {
...
pub struct BatchingLlmClient {
    connection: Mutex<carrot_sqlez::connection::Connection>,
...
impl BatchingLlmClient {
    fn new(cache_path: &Path) -> Result<Self> {
        ...
        let connection =
            carrot_sqlez::connection::Connection::open_file(&cache_path.to_str().unwrap());
        let mut statement = carrot_sqlez::statement::Statement::prepare(
            &connection,
            indoc! {"
    ...
    pub async fn import_batches(&self, batch_ids: &[String]) -> Result<()> {
        for batch_id in batch_ids {
            ...
            let batch_status = carrot_anthropic::batches::retrieve_batch(
                self.http_client.as_ref(),
                ANTHROPIC_API_URL,
                &self.api_key,
                batch_id,
            ...
            let results = carrot_anthropic::batches::retrieve_batch_results(
                self.http_client.as_ref(),
                ANTHROPIC_API_URL,
                &self.api_key,
                batch_id,
            ...
            for result in results {
                ...
                match result.result {
                    carrot_anthropic::batches::BatchResult::Succeeded { message } => {
                        let response_json = serde_json::to_string(&message)?;
                        updates.push((request_hash, response_json, batch_id.clone()));
                        success_count += 1;
                    }
                    carrot_anthropic::batches::BatchResult::Errored { error } => {
                        log::error!(
                    ...
                    carrot_anthropic::batches::BatchResult::Canceled => {
                        log::warn!("Batch request {} was canceled", request_hash);
                        error_count += 1;
                    }
                    carrot_anthropic::batches::BatchResult::Expired => {
    ...
    async fn upload_pending_requests(&self) -> Result<Vec<String>> {
        ...
        loop {
            ...
            for row in rows {
                ...
                let batch_request = carrot_anthropic::batches::BatchRequest { custom_id, params };
            ...
            for (batch_rows, batch_size) in batches_to_upload {
                ...
                let batch_requests: Vec<carrot_anthropic::batches::BatchRequest> =
                ...
                let batch = carrot_anthropic::batches::create_batch(
                    self.http_client.as_ref(),
                    ANTHROPIC_API_URL,
                    &self.api_key,
                    carrot_anthropic::batches::CreateBatchRequest {
