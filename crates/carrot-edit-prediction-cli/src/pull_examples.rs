...
use carrot_http_client::{AsyncBody, HttpClient, Method, Request};
use carrot_telemetry_events::EditPredictionRating;
...
use carrot_zeta_prompt::{ZetaFormat, ZetaPromptInput, excerpt_range_for_format};

use crate::PredictionProvider;
use crate::example::{Example, ExamplePrompt};
use crate::progress::{InfoStyle, Progress, Step};
use carrot_edit_prediction::example_spec::{ExampleSpec, TelemetrySource};
...
/// Minimum Carrot version for filtering captured examples.
/// For example, `MinCaptureVersion { minor: 224, patch: 1 }` means only pull examples
/// where `carrot_version >= 0.224.1`.
    ...
    for after_date in after_timestamps.iter() {
        ...
        let statement = indoc! {r#"
            SELECT
                ...
                carrot_version AS carrot_version
            ...
            WHERE ep_outcome LIKE 'Rejected%'
                ...
                AND (? IS NULL OR (
                    TRY_CAST(SPLIT_PART(carrot_version, '.', 2) AS INTEGER) > ?
                    OR (
                        TRY_CAST(SPLIT_PART(carrot_version, '.', 2) AS INTEGER) = ?
                        AND TRY_CAST(SPLIT_PART(SPLIT_PART(carrot_version, '.', 3), '+', 1) AS INTEGER) >= ?
                    )
                ))
...
pub async fn fetch_requested_examples_after(
    ...
    for after_date in after_timestamps.iter() {
        ...
        let examples = fetch_examples_with_query(
            ...
            &["request_id", "device_id", "time", "input", "carrot_version"],
...
fn rated_examples_from_response<'a>(
    ...
    let iter = response
        ...
        .filter_map(move |(row_index, data_row)| {
            ...
            let carrot_version = get_string("carrot_version");
...
fn build_rated_example(
    ...
    if !feedback.is_empty() {
        example
            .spec
            .human_feedback
            .push(carrot_edit_prediction::example_spec::HumanFeedback { message: feedback });
    }
...
fn build_example_from_snowflake(
    ...
    for event in &input.events {
        carrot_zeta_prompt::write_event(&mut edit_history, event);
        edit_history.push('\n');
    }
...
fn build_output_patch(
    ...
    let diff_body = carrot_language::unified_diff_with_offsets(
        old_text,
        model_output,
        editable_start_row,
        editable_start_row,
