...
use carrot_edit_prediction::{DebugEvent, EditPredictionStore};
...
async fn wait_for_language_servers_to_start(
    ...
    let subscriptions = [cx.subscribe(&lsp_store, {
        ...
        move |lsp_store, event, cx| match event {
            carrot_project::LspStoreEvent::LanguageServerAdded(id, name, _) => {
                step_progress.set_substatus(format!("LSP started: {}", name));
                started_tx.try_send(*id).ok();
            }
            carrot_project::LspStoreEvent::DiskBasedDiagnosticsFinished { language_server_id } => {
                let name = lsp_store
                    .read(cx)
            ...
            carrot_project::LspStoreEvent::LanguageServerUpdate {
                message:
                    carrot_client::proto::update_language_server::Variant::WorkProgress(
                        carrot_client::proto::LspWorkProgress {
