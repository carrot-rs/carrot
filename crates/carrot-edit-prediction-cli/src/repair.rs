    ...
    let response = match args.backend {
        BatchProvider::Anthropic => {
            ...
            let messages = vec![
                ...
                carrot_anthropic::Message {
                    role: carrot_anthropic::Role::Assistant,
                    content: vec![carrot_anthropic::RequestContent::Text {
            ...
            response
                ...
                .filter_map(|c| match c {
                    carrot_anthropic::ResponseContent::Text { text } => Some(text.as_str()),
                    _ => None,
                })
                .collect::<Vec<_>>()
                .join("")
        ...
        BatchProvider::Openai => {
            ...
            let messages = vec![
                // Turn 1: Original teacher prompt
                carrot_open_ai::RequestMessage::User {
                    content: carrot_open_ai::MessageContent::Plain(teacher_prompt.input.clone()),
                },
                // Turn 2: Original teacher response
                carrot_open_ai::RequestMessage::Assistant {
                    content: Some(carrot_open_ai::MessageContent::Plain(
                        teacher_response.clone(),
                    )),
                    tool_calls: vec![],
                },
                // Turn 3: Repair critique and instructions
                carrot_open_ai::RequestMessage::User {
                    content: carrot_open_ai::MessageContent::Plain(repair_message),
                },
            ];
...
mod tests {
    ...
    use carrot_edit_prediction::example_spec::ExampleSpec;
    ...
    fn example_with_previous_prediction() -> Example {
        Example {
            spec: ExampleSpec {
                ...
                repository_url: "https://github.com/carrot-rs/carrot.git".to_string(),
