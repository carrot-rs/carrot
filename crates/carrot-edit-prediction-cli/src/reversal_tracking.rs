...
use carrot_edit_prediction::udiff::apply_diff_to_string;
use carrot_language::{char_diff, text_diff};
...
fn filter_edit_history_by_path<'a>(
    edit_history: &'a [Arc<carrot_zeta_prompt::Event>],
    cursor_path: &std::path::Path,
) -> Vec<&'a carrot_zeta_prompt::Event> {
    edit_history
        .iter()
        .filter(|event| match event.as_ref() {
            carrot_zeta_prompt::Event::BufferChange { path, .. } => {
...
fn extract_diff_from_event(event: &carrot_zeta_prompt::Event) -> &str {
    match event {
        carrot_zeta_prompt::Event::BufferChange { diff, .. } => diff.as_str(),
    }
}

fn is_predicted_event(event: &carrot_zeta_prompt::Event) -> bool {
    match event {
        carrot_zeta_prompt::Event::BufferChange { predicted, .. } => *predicted,
    }
}
...
pub fn compute_prediction_reversal_ratio(
    ...
    let edit_history: &[Arc<carrot_zeta_prompt::Event>] = &prompt_inputs.events;
...
mod tests {
    use super::*;
    use carrot_edit_prediction::udiff::apply_diff_to_string;
    use carrot_zeta_prompt::ExcerptRanges;
    use indoc::indoc;
    ...
    fn test_reverse_diff_roundtrip() {
        ...
        let diff_body = carrot_language::unified_diff(original, modified);
    ...
    fn test_filter_edit_history_by_path() {
        ...
        let events = vec![
            Arc::new(carrot_zeta_prompt::Event::BufferChange {
    ...
    fn test_compute_prediction_reversal_ratio_full_file() {
        let prompt_inputs = make_test_prompt_inputs(
            indoc! {"
                ...
                line2
            "},
            vec![Arc::new(carrot_zeta_prompt::Event::BufferChange {
