...
use carrot_cloud_llm_client::predict_edits_v3::{RawCompletionRequest, RawCompletionResponse};
use carrot_edit_prediction::{DebugEvent, EditPredictionStore, Zeta2RawConfig};
use carrot_http_client::{AsyncBody, HttpClient, Method};
use carrot_reqwest_client::ReqwestClient;
use carrot_zeta_prompt::ZetaFormat;
...
pub async fn run_prediction(
    ...
    ep_store.update(&mut cx, |store, _cx| {
        let model = match provider {
            PredictionProvider::Zeta1 => carrot_edit_prediction::EditPredictionModel::Zeta,
            PredictionProvider::Zeta2(_) => carrot_edit_prediction::EditPredictionModel::Zeta,
            PredictionProvider::Mercury => carrot_edit_prediction::EditPredictionModel::Mercury,
        ...
        let prediction = ep_store
            .update(&mut cx, |store, cx| {
                store.request_prediction(
                    ...
                    carrot_cloud_llm_client::PredictEditsRequestTrigger::Cli,
                    cx,
                )
            })
            .await?;
...
async fn predict_anthropic(
    ...
    for ix in 0..repetition_count {
        ...
        let messages = vec![carrot_anthropic::Message {
            role: carrot_anthropic::Role::User,
            content: vec![carrot_anthropic::RequestContent::Text {
        ...
        let actual_output = response
            ...
            .filter_map(|content| match content {
                carrot_anthropic::ResponseContent::Text { text } => Some(text),
                _ => None,
            })
            .collect::<Vec<String>>()
            .join("\n");
...
async fn predict_openai(
    ...
    for ix in 0..repetition_count {
        ...
        let messages = vec![carrot_open_ai::RequestMessage::User {
            content: carrot_open_ai::MessageContent::Plain(prompt.input.clone()),
        }];
        ...
        let actual_output = response
            ...
            .filter_map(|choice| match choice.message {
                carrot_open_ai::RequestMessage::Assistant { content, .. } => {
                    content.map(|c| match c {
                        carrot_open_ai::MessageContent::Plain(text) => text,
                        carrot_open_ai::MessageContent::Multipart(parts) => parts
                            .into_iter()
                            .filter_map(|p| match p {
                                carrot_open_ai::MessagePart::Text { text } => Some(text),
                                _ => None,
                            })
                            .collect::<Vec<_>>()
                            .join(""),
