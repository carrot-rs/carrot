/// History Panel — visual overlay between terminal output and input area.
///
...
use inazuma::{IntoElement, Oklch, ParentElement, Styled, div, oklcha, prelude::*, px, rgb};

use carrot_session::command_history::{self, CommandHistory, HistoryEntry};
    ...
    pub fn render(&self) -> impl IntoElement {
        ...
        for (i, entry) in visible_entries.iter().enumerate() {
            ...
            let row = div()
                ...
                .gap_2()
                .text_xs()
                .when(is_selected, |this| this.bg(oklcha(0.70, 0.15, 160.0, 0.10)))
                // >_ icon
                .child(div().text_color(rgb(0x666666)).flex_shrink_0().child(">_"))
                // Command text
                .child(
                    div()
                        .flex_1()
