...
//! via Inazuma's `paint_quad` API. This is the industry-standard approach for
...
use inazuma::{Bounds, Oklch, Pixels, Window, fill, point, px, size};
...
pub fn builtin_char(c: char) -> Option<BuiltinChar> {
    ...
    match code {
        ...
        0x2500..=0x257F => {
            ...
            match code {
                0x2504 => {
                    return Some(BuiltinChar::DashedH {
                        heavy: false,
                        dashes: 3,
                    });
                }
                ...
                0x2506 => {
                    return Some(BuiltinChar::DashedV {
                        heavy: false,
                        dashes: 3,
                    });
                }
            ...
            match code {
                0x2571 => {
                    return Some(BuiltinChar::Diagonal {
                        rising: true,
                        falling: false,
                    });
                }
...
fn draw_quadrant(bits: u8, x: f32, y: f32, w: f32, h: f32, color: Oklch, window: &mut Window) {
    ...
    if bits & 0b1000 != 0 {
        paint_rect(x, y, hw, hh, color, window);
    }
    if bits & 0b0100 != 0 {
        paint_rect(x + hw, y, hw, hh, color, window);
    }
    if bits & 0b0010 != 0 {
        paint_rect(x, y + hh, hw, hh, color, window);
    }
    if bits & 0b0001 != 0 {
        paint_rect(x + hw, y + hh, hw, hh, color, window);
    }
}
