...
//! colors are read from `carrot_theme::Theme` via `GlobalTheme`.
...
/// Convert an ANSI color to theme-aware Oklch with full context.
///
/// Takes the terminal's color palette and dim flag for accurate resolution.
/// Used by raw snapshot → resolved snapshot conversion.
pub fn ansi_color_to_oklch(
    color: &AnsiColor,
    colors: &carrot_term::term::color::Colors,
    dim: bool,
    theme: &Theme,
) -> Oklch {
    match color {
