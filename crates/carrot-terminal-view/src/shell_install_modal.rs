use carrot_shell_integration::shell_install::{ShellInstallInfo, detect_available_installer};
use carrot_ui::{
...
///
/// Uses carrot_shell_integration::shell_install for platform detection and install commands.
/// Uses carrot-ui Modal components and theme tokens — no hardcoded colors.
use inazuma::{
...
impl ShellInstallModal {
    ...
    fn install(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        if let Some(ref cmd) = self.install_command {
            cx.global_mut::<PendingShellInstallName>().0 = Some(self.shell.binary.to_string());
            {
...
impl Render for ShellInstallModal {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let description = if let Some(pm) = self.pm_name {
            format!(
                "Carrot supports {} natively with intelligent completions. \
                    ...
                    .footer(
                        ModalFooter::new().end_slot(
                            carrot_ui::h_flex()
                                ...
                                .child(Button::new("install", install_label).primary().on_click(
