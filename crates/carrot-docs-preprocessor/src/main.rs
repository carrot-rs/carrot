...
use inazuma_settings_framework::{KeymapFile, SettingsStore};
use mdbook::BookItem;
...
static KEYMAP_MACOS: LazyLock<KeymapFile> = LazyLock::new(|| {
    load_keymap("keymaps/default-macos.toml").expect("Failed to load MacOS keymap")
});

static KEYMAP_LINUX: LazyLock<KeymapFile> = LazyLock::new(|| {
    load_keymap("keymaps/default-linux.toml").expect("Failed to load Linux keymap")
});

static KEYMAP_WINDOWS: LazyLock<KeymapFile> = LazyLock::new(|| {
    load_keymap("keymaps/default-windows.toml").expect("Failed to load Windows keymap")
});
...
const FRONT_MATTER_COMMENT: &str = "<!-- CARROT_META {} -->";

fn main() -> Result<()> {
    carrot_log::init();
    carrot_log::init_output_stderr();
    ...
    for_each_labeled_code_block_mut(book, errors, &|label, snippet_json| {
        ...
        match label {
            "settings" => {
                ...
                let value = inazuma_settings_framework::parse_json_with_comments::<
                    serde_json::Value,
                >(&snippet_json_fixed)?;
            ...
            "debug" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<carrot_task::DebugTaskFile>(
                    &snippet_json_fixed,
                )?;
            }
            "tasks" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<carrot_task::TaskTemplates>(
                    &snippet_json_fixed,
                )?;
            }
            "icon-theme" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<
                    carrot_theme::IconThemeFamilyContent,
                >(&snippet_json_fixed)?;
            }
            "semantic_token_rules" => {
                ...
                inazuma_settings_framework::parse_json_with_comments::<
                    inazuma_settings_framework::SemanticTokenRules,
                >(&snippet_json_fixed)?;
            }
...
fn load_keymap(asset_path: &str) -> Result<KeymapFile> {
    let content = inazuma_util::asset_str::<inazuma_settings_framework::SettingsAssets>(asset_path);
...
fn handle_postprocessing() -> Result<()> {
    let logger = carrot_log::scoped!("render");
    ...
    let carrot_html = output
        .remove("carrot-html")
        .expect("carrot-html output defined");
    let default_description = carrot_html
        .get("default-description")
        .expect("Default description not found")
        .as_str()
        .expect("Default description not a string")
        .to_string();
    let default_title = carrot_html
        .get("default-title")
    ...
    output.insert("html".to_string(), carrot_html);
        ...
        for entry in std::fs::read_dir(&dir).context("failed to read docs dir")? {
                ...
                )
            {
                if ignore_list.contains(&&*entry.file_name().to_string_lossy()) {
                    carrot_log::info!(logger => "Ignoring {}", entry.path().to_string_lossy());
                } else {
    ...
    carrot_log::info!(logger => "Processing {} `.html` files", files.len());
    ...
    for file in files {
        ...
        let contents = meta_regex.replace(&contents, |caps: &regex::Captures| {
            ...
            for (kind, content) in metadata {
                match kind.as_str() {
                    ...
                    _ => {
                        carrot_log::warn!(logger => "Unrecognized frontmatter key: {} in {:?}", kind, pretty_path(&file, &root_dir));
                    }
                }
            }
            String::new()
        ...
        let meta_description = meta_description.as_ref().unwrap_or_else(|| {
            carrot_log::warn!(logger => "No meta description found for {:?}", pretty_path(&file, &root_dir));
            &default_description
        });
        let page_title = extract_title_from_page(&contents, pretty_path(&file, &root_dir));
        let meta_title = meta_title.as_ref().unwrap_or_else(|| {
            carrot_log::debug!(logger => "No meta title found for {:?}", pretty_path(&file, &root_dir));
            &default_title
        });
        let meta_title = format!("{} | {}", page_title, meta_title);
        carrot_log::trace!(logger => "Updating {:?}", pretty_path(&file, &root_dir));
...
fn keymap_schema_for_actions(
    ...
    let mut documentation = inazuma_collections::HashMap::<&str, &str>::default();
    let mut deprecations = inazuma_collections::HashMap::<&str, &str>::default();
    let mut deprecation_messages = inazuma_collections::HashMap::<&str, &str>::default();
