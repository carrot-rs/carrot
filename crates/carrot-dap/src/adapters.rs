...
use carrot_fs::Fs;
pub use carrot_http_client::{HttpClient, github::latest_github_release};
use carrot_language::{LanguageName, LanguageToolchainStore};
use carrot_node_runtime::NodeRuntime;
use carrot_task::{CarrotDebugConfig, DebugScenario, TcpArgumentsTemplate};
...
use inazuma_util::{archive::extract_zip, rel_path::RelPath};
...
impl TcpArguments {
    pub fn from_proto(proto: carrot_proto::TcpHost) -> anyhow::Result<Self> {
    ...
    pub fn to_proto(&self) -> carrot_proto::TcpHost {
...
impl DebugTaskDefinition {
    ...
    pub fn to_proto(&self) -> carrot_proto::DebugTaskDefinition {
        carrot_proto::DebugTaskDefinition {
    ...
    pub fn from_proto(proto: carrot_proto::DebugTaskDefinition) -> Result<Self> {
        Ok(Self {
...
impl DebugAdapterBinary {
    pub fn from_proto(binary: carrot_proto::DebugAdapterBinary) -> anyhow::Result<Self> {
        let request = match binary.launch_type() {
            carrot_proto::debug_adapter_binary::LaunchType::Launch => {
                StartDebuggingRequestArgumentsRequest::Launch
            }
            carrot_proto::debug_adapter_binary::LaunchType::Attach => {
    ...
    pub fn to_proto(&self) -> carrot_proto::DebugAdapterBinary {
        carrot_proto::DebugAdapterBinary {
            ...
            launch_type: match self.request_args.request {
                StartDebuggingRequestArgumentsRequest::Launch => {
                    carrot_proto::debug_adapter_binary::LaunchType::Launch.into()
                }
                StartDebuggingRequestArgumentsRequest::Attach => {
                    carrot_proto::debug_adapter_binary::LaunchType::Attach.into()
                }
            },
...
pub async fn download_adapter_from_github(
    ...
    let adapter_path = carrot_paths::debug_adapters_dir().join(&adapter_name.as_ref());
    ...
    match file_type {
        ...
        DownloadedFileType::Zip | DownloadedFileType::Vsix => {
            ...
            inazuma_util::fs::remove_matching(&adapter_path, |entry| {
                entry
                    .file_name()
...
impl DebugAdapter for FakeAdapter {
    ...
    async fn config_from_carrot_format(
        ...
        let config = serde_json::to_value(carrot_scenario.request).unwrap();
