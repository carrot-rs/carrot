...
use inazuma::{AppContext as _, AsyncApp, BackgroundExecutor, Task};
use inazuma_settings_framework::Settings as _;
use inazuma_util::{ConnectionResult, ResultExt, process::Child};
...
impl TcpTransport {
    ...
    async fn start(
        ...
        if let Some(command) = &binary.command {
            let mut command = inazuma_util::command::new_std_command(&command);
...
impl StdioTransport {
    // #[allow(dead_code, reason = "This is used in non test builds of Carrot")]
    async fn start(
        binary: &DebugAdapterBinary,
        log_handlers: LogHandlers,
        cx: &mut AsyncApp,
        ...
        let Some(binary_command) = &binary.command else {
            bail!(
                "When using the `stdio` transport, the path to a debug adapter binary must be set by Carrot."
            );
        };
        let mut command = inazuma_util::command::new_std_command(&binary_command);
