...
use carrot_language::{Buffer, BufferEvent, LanguageName, Toolchain, ToolchainScope};
use carrot_project::{
    Project, ProjectPath, Toolchains, WorktreeId, toolchain_store::ToolchainStoreEvent,
};
        ...
        cx.spawn(async move |cx| {
            ...
            if let Some(toolchain) = selected_toolchain {
                ...
                if let Some(toolchain) = &default_choice {
                    ...
                    let db = cx
                        .update(|_, cx| carrot_workspace::WorkspaceDb::global(cx))
                        .ok()?;
                    db.set_toolchain(
                        workspace_id,
                        worktree_root_path,
