...
use carrot_language::{Language, LanguageName, Toolchain, ToolchainScope};
use carrot_open_path_prompt::OpenPathDelegate;
use carrot_project::{DirectoryLister, Project, ProjectPath, Toolchains, WorktreeId};
use carrot_ui::{
...
use inazuma_util::{ResultExt, maybe, paths::PathStyle, rel_path::RelPath};
use std::{
    borrow::Cow,
    path::{Path, PathBuf},
    sync::Arc,
...
enum AddState {
    Path {
        picker: Entity<Picker<carrot_open_path_prompt::OpenPathDelegate>>,
        ...
        v_flex()
            ...
            .when_some(weak, |this, weak| {
                this.on_action(window.listener_for(
                    &weak,
                    |this: &mut ToolchainSelector, _: &inazuma_menu::Cancel, window, cx| {
                        this.state = State::Search((this.create_search_state)(window, cx));
                                        ...
                                        scope_options.iter().enumerate().map(|(i, scope)| {
                                            ...
                                            div()
                                                ...
                                                .on_action(cx.listener(
                                                    move |this, _: &inazuma_menu::Confirm, _, cx| {
                                                        this.select_scope(
    ...
    fn confirm(&mut self, _: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        if let Some(string_match) = self.matches.get(self.selected_index) {
                ...
                .flatten()
            {
                ...
                let db = carrot_workspace::WorkspaceDb::global(cx);
