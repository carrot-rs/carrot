...
use carrot_net::async_net::UnixListener;
use inazuma_util::fs::make_file_executable;
use smol::lock::Mutex;
...
use inazuma::{AsyncApp, BackgroundExecutor, Task};
use inazuma_util::{ResultExt as _, debug_panic, maybe, paths::PathExt, shell::ShellKind};
use smol::fs;

/// Path to the program used for askpass
///
/// On Unix and remote servers, this defaults to the current executable
/// On Windows, this is set to the CLI variant of carrot
...
impl PasswordProxy {
    pub async fn new(
        ...
        let temp_dir = tempfile::Builder::new()
            .prefix("carrot-askpass")
            .tempdir()?;
        let askpass_socket = temp_dir.path().join("askpass.sock");
        let askpass_script_path = temp_dir.path().join(ASKPASS_SCRIPT_NAME);
        let current_exec = std::env::current_exe()
            .context("Failed to determine current carrot executable path.")?;
...
/// The main function for when Carrot is running in netcat mode for use in askpass.
/// Called from both the remote server binary and the carrot binary in their respective main functions.
pub fn main(socket: &str) {
    use carrot_net::UnixStream;
