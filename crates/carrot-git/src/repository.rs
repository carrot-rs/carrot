...
use inazuma::{AppContext as _, AsyncApp, BackgroundExecutor, SharedString, Task};
...
use inazuma_sum_tree::MapSeekTarget;
...
pub use carrot_askpass::{AskPassDelegate, AskPassResult, AskPassSession};
...
impl GitExcludeOverride {
    ...
    fn remove_auto_generated_block(content: &str) -> String {
        ...
        // Older versions of Carrot didn't have end-of-block markers,
...
pub async fn get_git_committer(cx: &AsyncApp) -> GitCommitter {
    ...
    let git = GitBinary::new(
        ...
        carrot_paths::home_dir().clone(),
        carrot_paths::home_dir().join(".git"),
...
impl GitBinary {
    ...
    pub async fn with_temp_index<R>(
        ...
        let delete_temp_index = inazuma_util::defer({
    ...
    #[allow(clippy::disallowed_methods)]
    pub(crate) fn build_command<S>(&self, args: &[S]) -> inazuma_util::command::Command
    where
        S: AsRef<OsStr>,
    {
...
async fn run_git_command(
    env: Arc<HashMap<String, String>>,
    ask_pass: AskPassDelegate,
    mut command: inazuma_util::command::Command,
    executor: BackgroundExecutor,
...
async fn run_askpass_command(
    ...
    git_process: inazuma_util::command::Child,
...
fn checkpoint_author_envs() -> HashMap<String, String> {
    HashMap::from_iter([
        ("GIT_AUTHOR_NAME".to_string(), "Carrot".to_string()),
        ("GIT_AUTHOR_EMAIL".to_string(), "hi@carrot.dev".to_string()),
        ("GIT_COMMITTER_NAME".to_string(), "Carrot".to_string()),
        (
            "GIT_COMMITTER_EMAIL".to_string(),
            "hi@carrot.dev".to_string(),
        ),
    ])
}
...
mod tests {
    ...
    fn test_branches_parsing() {
        ...
        let input = "*\0060964da10574cd9bf06463a53bf6e0769c5c45e\0\0refs/heads/carrot-patches\0refs/remotes/origin/carrot-patches\0\01733187470\0John Doe\0generated protobuf\n";
        assert_eq!(
            parse_branch_input(input).unwrap(),
            vec![Branch {
                is_head: true,
                ref_name: "refs/heads/carrot-patches".into(),
                upstream: Some(Upstream {
                    ref_name: "refs/remotes/origin/carrot-patches".into(),
            ...
            vec![
                Branch {
                    ...
                    most_recent_commit: Some(CommitSummary {
                        ...
                        author_name: SharedString::new_static("Carrot"),
                        has_parent: true,
                    })
                },
                Branch {
