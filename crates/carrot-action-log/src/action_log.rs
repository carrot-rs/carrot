...
use carrot_fs::MTime;
use carrot_language::{Anchor, Buffer, BufferEvent, Point, ToOffset, ToPoint};
use carrot_project::{Project, ProjectItem, lsp_store::OpenLspBufferHandle};
...
use inazuma_text::{Edit, Patch, Rope};
use inazuma_util::{RangeExt, ResultExt as _};
use std::{
    cmp,
    ops::Range,
    path::{Path, PathBuf},
...
impl ActionLog {
    ...
    async fn maintain_diff(
        ...
        buffer: Entity<Buffer>,
        mut buffer_updates: mpsc::UnboundedReceiver<(ChangeAuthor, inazuma_text::BufferSnapshot)>,
        cx: &mut AsyncApp,
    ) -> Result<()> {
        ...
        let (mut git_diff_updates_tx, mut git_diff_updates_rx) = carrot_watch::channel(());
            ...
            if let Some((git_diff, (buffer_repo, _))) = git_diff.as_ref().zip(buffer_repo) {
                cx.update(|cx| {
                    ...
                    Some(cx.subscribe(git_diff, move |_, event, cx| {
                        if let carrot_buffer_diff::BufferDiffEvent::DiffChanged { .. } = event {
        ...
        let (new_base_text, new_diff_base) = this
            .read_with(cx, |this, cx| {
                ...
                anyhow::Ok(cx.background_spawn(async move {
                    ...
                    let committed_edits = carrot_language::line_diff(
    ...
    pub fn keep_edits_in_range(
        &mut self,
        buffer: Entity<Buffer>,
        buffer_range: Range<impl carrot_language::ToPoint>,
    ...
    pub fn reject_edits_in_ranges(
        &mut self,
        buffer: Entity<Buffer>,
        buffer_ranges: Vec<Range<impl carrot_language::ToPoint>>,
        telemetry: Option<ActionLogTelemetry>,
        cx: &mut Context<Self>,
...
pub struct TrackedBuffer {
    ...
    diff_update: mpsc::UnboundedSender<(ChangeAuthor, inazuma_text::BufferSnapshot)>,
...
mod tests {
    ...
    use carrot_project::{FakeFs, Fs, Project, RemoveOptions};
    ...
    #[inazuma::test]
    async fn test_keep_edits_on_commit(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
