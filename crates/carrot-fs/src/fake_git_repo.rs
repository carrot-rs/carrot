...
use inazuma::{AsyncApp, BackgroundExecutor, SharedString, Task};
...
impl GitRepository for FakeGitRepository {
    ...
    fn load_blob_content(&self, oid: carrot_git::Oid) -> BoxFuture<'_, Result<String>> {
    ...
    fn load_commit(
        ...
        _commit: String,
        _cx: AsyncApp,
    ) -> BoxFuture<'_, Result<carrot_git::repository::CommitDiff>> {
        unimplemented!()
    }
    ...
    fn stash_entries(&self) -> BoxFuture<'_, Result<carrot_git::stash::GitStash>> {
        async { Ok(carrot_git::stash::GitStash::default()) }.boxed()
    }
    ...
    ) -> BoxFuture<'_, Result<carrot_git::blame::Blame>> {
        self.with_state_async(false, move |state| {
            state
                .blames
                .get(&path)
    ...
    fn file_history(
        &self,
        path: RepoPath,
    ) -> BoxFuture<'_, Result<carrot_git::repository::FileHistory>> {
    ...
    fn commit(
        &self,
        _message: inazuma::SharedString,
        _name_and_email: Option<(inazuma::SharedString, inazuma::SharedString)>,
    ...
    fn push(
        ...
        _env: Arc<HashMap<String, String>>,
        _cx: AsyncApp,
    ) -> BoxFuture<'_, Result<carrot_git::repository::RemoteCommandOutput>> {
        unimplemented!()
    }
    ...
    fn check_for_pushed_commit(&self) -> BoxFuture<'_, Result<Vec<inazuma::SharedString>>> {
        future::ready(Ok(Vec::new())).boxed()
    }

    fn diff(&self, _diff: carrot_git::repository::DiffType) -> BoxFuture<'_, Result<String>> {
    ...
    fn diff_stat(
    ...
    ) -> BoxFuture<'_, Result<carrot_git::status::GitDiffStat>> {
        ...
        self.with_state_async(false, move |state| {
            ...
            Ok(carrot_git::status::GitDiffStat {
                entries: entries.into(),
            })
        })
        .boxed()
    ...
    fn checkpoint(&self) -> BoxFuture<'static, Result<GitRepositoryCheckpoint>> {
        ...
        async move {
            executor.simulate_random_delay().await;
            let oid = carrot_git::Oid::random(&mut *executor.rng().lock());
