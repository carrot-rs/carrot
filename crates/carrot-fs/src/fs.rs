...
use carrot_git::repository::{GitRepository, RealGitRepository};
...
#[cfg(feature = "test-support")]
use inazuma_collections::{BTreeMap, btree_map};
#[cfg(feature = "test-support")]
use inazuma_util::normalize_path;
...
impl From<carrot_proto::Timestamp> for MTime {
    fn from(timestamp: carrot_proto::Timestamp) -> Self {
        MTime(timestamp.into())
    }
}

impl From<MTime> for carrot_proto::Timestamp {
...
impl Fs for RealFs {
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            ...
            // See https://github.com/carrot-rs/carrot/pull/8437 for more details.
            let mut tmp_file =
                tempfile::NamedTempFile::new_in(path.parent().unwrap_or(carrot_paths::temp_dir()))?;
    ...
    async fn atomic_write(&self, path: PathBuf, data: String) -> Result<()> {
        smol::unblock(move || {
            ...
            // https://github.com/carrot-rs/carrot/issues/16571
            let temp_dir = TempDir::new_in(path.parent().unwrap_or(carrot_paths::temp_dir()))?;
            let temp_file = {
    ...
    async fn save(&self, path: &Path, text: &Rope, line_ending: LineEnding) -> Result<()> {
        ...
        for chunk in inazuma_text::chunks_with_line_ending(text, line_ending) {
    ...
    async fn watch(
        ...
        use inazuma_util::{ResultExt as _, paths::SanitizedPath};
...
impl FakeFs {
    ...
    pub fn new(executor: inazuma::BackgroundExecutor) -> Arc<Self> {
    ...
    pub fn set_blame_for_repo(
        &self,
        dot_git: &Path,
        blames: Vec<(RepoPath, carrot_git::blame::Blame)>,
    ) {
        self.with_git_state(dot_git, true, |state| {
            state.blames.clear();
    ...
    pub fn paths(&self, include_dot_git: bool) -> Vec<PathBuf> {
        ...
        let mut queue = inazuma_collections::VecDeque::new();
        let state = &*self.state.lock();
        queue.push_back((PathBuf::from(inazuma_util::path!("/")), &state.root));
...
impl Fs for FakeFs {
    ...
    async fn save(&self, path: &Path, text: &Rope, line_ending: LineEnding) -> Result<()> {
        self.simulate_random_delay().await;
        let path = normalize_path(path);
        let content = inazuma_text::chunks_with_line_ending(text, line_ending).collect::<String>();
        if let Some(path) = path.parent() {
            self.create_dir(path).await?;
        }
