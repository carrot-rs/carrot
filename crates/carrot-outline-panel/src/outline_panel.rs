...
use carrot_language::{Anchor, BufferId, BufferSnapshot, OffsetRangeExt, OutlineItem};
use inazuma::{
...
use carrot_project::{File, Fs, GitEntry, GitTraversal, Project, ProjectItem};
use carrot_search::{BufferSearchBar, ProjectSearchView};
...
use carrot_worktree::{Entry, ProjectEntryId, WorktreeId};
use inazuma_settings_framework::{Settings, SettingsStore};
use inazuma_util::{RangeExt, ResultExt, TryFutureExt, debug_panic, rel_path::RelPath};
...
type Outline = OutlineItem<carrot_language::Anchor>;
...
struct SearchState {
    kind: SearchKind,
    query: String,
    matches: Vec<(Range<carrot_editor::Anchor>, Arc<OnceLock<SearchData>>)>,
...
struct HighlightArguments {
    ...
    match_range: Range<carrot_editor::Anchor>,
...
impl SearchState {
    fn new(
        kind: SearchKind,
        query: String,
        previous_matches: HashMap<Range<carrot_editor::Anchor>, Arc<OnceLock<SearchData>>>,
        new_matches: Vec<Range<carrot_editor::Anchor>>,
...
#[derive(Debug)]
struct Excerpt {
    range: ExcerptRange<carrot_language::Anchor>,
...
#[derive(Clone, Debug)]
struct SearchData {
    context_range: Range<carrot_editor::Anchor>,
    context_text: String,
    truncated_left: bool,
...
impl OutlinePanel {
        ...
        cx.new(|cx| {
            ...
            let workspace_subscription = cx.subscribe_in(
                ...
                move |outline_panel, workspace, event, window, cx| {
                    if let carrot_workspace::Event::ActiveItemChanged = event {
    ...
    fn open_excerpts(
        &mut self,
        action: &carrot_editor::actions::OpenExcerpts,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if self.filter_editor.focus_handle(cx).is_focused(window) {
    ...
    fn open_excerpts_split(
        &mut self,
        action: &carrot_editor::actions::OpenExcerptsSplit,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if self.filter_editor.focus_handle(cx).is_focused(window) {
    ...
    fn deploy_context_menu(
        ...
        let context_menu = ContextMenu::build(window, cx, |menu, _, _| {
            menu.context(self.focus_handle.clone())
                .action(
                    carrot_ui::utils::reveal_in_file_manager_label(false),
                ...
                .separator()
                .action("Copy Path", Box::new(carrot_actions::workspace::CopyPath))
                .action(
                    "Copy Relative Path",
                    Box::new(carrot_actions::workspace::CopyRelativePath),
                )
        });
    ...
    fn copy_path(
        &mut self,
        _: &carrot_actions::workspace::CopyPath,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn copy_relative_path(
        &mut self,
        _: &carrot_actions::workspace::CopyRelativePath,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn render_outline(
        ...
        let label_element = carrot_outline::render_item(
            &outline.outline,
            string_match
    ...
    fn entry_element(
        ...
        is_active: bool,
        label_element: inazuma::AnyElement,
        window: &mut Window,
        cx: &mut Context<OutlinePanel>,
    ) -> Stateful<Div> {
        let settings = OutlinePanelSettings::get_global(cx);
        div()
            ...
            .on_click({
                ...
                cx.listener(
                    move |outline_panel, event: &inazuma::ClickEvent, window, cx| {
    ...
    fn location_for_editor_selection(
        ...
        let selection = editor.update(cx, |editor, cx| {
            editor
                .selections
                .newest::<carrot_language::Point>(&editor.display_snapshot(cx))
                .head()
        });
    ...
    fn outline_location(
        ...
        buffer_id: BufferId,
        excerpt_id: ExcerptId,
        multi_buffer_snapshot: carrot_editor::MultiBufferSnapshot,
        editor_snapshot: carrot_editor::EditorSnapshot,
    ...
    fn excerpt_fetch_ranges(
        ...
        (
            BufferSnapshot,
            HashMap<ExcerptId, ExcerptRange<carrot_language::Anchor>>,
        ),
    > {
        self.fs_entries
            .iter()
            ...
            v_flex()
                ...
                .child(h_flex().justify_center().child({
                    let keystroke = match self.position(window, cx) {
                        DockPosition::Left => {
                            window.keystroke_text_for(&carrot_workspace::ToggleLeftDock)
                        }
                        DockPosition::Bottom => {
                            window.keystroke_text_for(&carrot_workspace::ToggleBottomDock)
                        }
                        DockPosition::Right => {
                            window.keystroke_text_for(&carrot_workspace::ToggleRightDock)
                        }
                    };
            ...
            let list_contents = {
                ...
                .when(show_indent_guides, |list| {
                    list.with_decoration(
                        carrot_ui::indent_guides(px(indent_size), IndentGuideColors::panel(cx))
...
mod tests {
    use carrot_db::indoc;
    use carrot_language::{self, FakeLspAdapter, rust_lang};
    use carrot_project::FakeFs;
    use carrot_search::project_search::perform_project_search;
    use carrot_workspace::{OpenOptions, OpenVisible, ToolbarItemView, Workspace};
    use inazuma::{TestAppContext, UpdateGlobal, VisualTestContext, WindowHandle};
    ...
    async fn test_project_search_results_toggling(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx, |workspace, window, cx| {
            ProjectSearchView::deploy_search(
                workspace,
                &carrot_workspace::DeploySearch::default(),
                window,
                cx,
            )
        });
  ...
  crates/
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        outline_panel.update_in(cx, |outline_panel, window, cx| {
            outline_panel.open_excerpts(&carrot_editor::actions::OpenExcerpts, window, cx);
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
        cx.run_until_parked();
      ...
      ErrorBoundary.tsx
        ...
        outline_panel.update_in(cx, |outline_panel, window, cx| {
            outline_panel
                ...
                .update(cx, |editor, cx| {
                    editor.toggle_fold(&carrot_editor::actions::ToggleFold, window, cx)
                });
        });
        cx.executor()
            .advance_clock(UPDATE_DEBOUNCE + Duration::from_millis(100));
  ...
  src/
    ...
    fn init_test(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            carrot_search::project_search::init(cx);
            carrot_search::buffer_search::init(cx);
            super::init(cx);
        });
    }
...
// ... snip
    ...
    fn selected_row_text(editor: &Entity<Editor>, cx: &mut App) -> String {
        editor.update(cx, |editor, cx| {
            let selections = editor.selections.all::<carrot_language::Point>(&editor.display_snapshot(cx));
            ...
            let line_start = carrot_language::Point::new(selection.start.row, 0);
            let line_end = multi_buffer_snapshot.clip_point(carrot_language::Point::new(selection.end.row, u32::MAX), carrot_language::Bias::Right);
        ...
        let mut fake_language_servers = language_registry.register_fake_lsp(
            ...
            FakeLspAdapter {
                capabilities: carrot_lsp::ServerCapabilities {
                    document_symbol_provider: Some(carrot_lsp::OneOf::Left(true)),
                    ..carrot_lsp::ServerCapabilities::default()
                },
                initializer: Some(Box::new(|fake_language_server| {
                    fake_language_server
                        .set_request_handler::<carrot_lsp::request::DocumentSymbolRequest, _, _>(
                            move |_, _| async move {
                                #[allow(deprecated)]
                                Ok(Some(carrot_lsp::DocumentSymbolResponse::Nested(vec![
                                    carrot_lsp::DocumentSymbol {
                                        name: "Foo".to_string(),
                                        detail: None,
                                        kind: carrot_lsp::SymbolKind::STRUCT,
                                        tags: None,
                                        deprecated: None,
                                        ...
                                        selection_range: carrot_lsp::Range::new(
                                        ...
                                        children: Some(vec![
                                            carrot_lsp::DocumentSymbol {
                                                name: "bar".to_string(),
                                                detail: None,
                                                kind: carrot_lsp::SymbolKind::FIELD,
                                                tags: None,
                                                deprecated: None,
                                                range: carrot_lsp::Range::new(
                                            ...
                                            carrot_lsp::DocumentSymbol {
                                                ...
                                                range: carrot_lsp::Range::new(
                                                    ...
                                                    carrot_lsp::Position::new(2, 15),
                                                ),
                                                selection_range: carrot_lsp::Range::new(
                                                    carrot_lsp::Position::new(2, 4),
                                                    carrot_lsp::Position::new(2, 7),
                                                ),
                                                children: None,
                                            },
                                        ]),
...
outline: struct Foo  <==== selected
  ...
  outline: baz"
        ...
        cx.update(|_, cx| {
            inazuma_settings_framework::SettingsStore::update_global(
                cx,
                |store: &mut inazuma_settings_framework::SettingsStore, cx| {
                    store.update_user_settings(cx, |settings| {
                        settings.project.all_languages.defaults.document_symbols =
                            Some(inazuma_settings_framework::DocumentSymbols::On);
                    });
                },
            );
        });
        ...
        cx.update(|_, cx| {
            inazuma_settings_framework::SettingsStore::update_global(
                ...
                |store: &mut inazuma_settings_framework::SettingsStore, cx| {
                    store.update_user_settings(cx, |settings| {
                        settings.project.all_languages.defaults.document_symbols =
                            Some(inazuma_settings_framework::DocumentSymbols::Off);
                    });
                },
            );
        });
