use carrot_agent::{AgentTool, TerminalTool, ToolPermissionDecision};
use carrot_agent_settings::AgentSettings;
use carrot_shell_command_parser::extract_commands;
...
use inazuma_settings_framework::{Settings as _, SettingsStore, ToolPermissionMode};
...
const SETTINGS_DISCLAIMER: &str = "Note: custom tool permissions only apply to the Carrot native agent and don’t extend to external agents connected through the Agent Client Protocol (ACP).";
    ...
    v_flex()
        ...
        .child(
            v_flex()
                ...
                .child(Divider::horizontal().color(carrot_ui::DividerColor::BorderFaded))
                ...
                .when(!rules.invalid_patterns.is_empty(), |this| {
                    this.child(Divider::horizontal().color(carrot_ui::DividerColor::BorderFaded))
...
mod tests {
    ...
    fn test_all_tools_are_in_tool_info_or_excluded() {
        ...
        for tool_name in carrot_agent::ALL_TOOL_NAMES {
        ...
        for tool_id in &tool_info_ids {
            assert!(
                carrot_agent::ALL_TOOL_NAMES.contains(tool_id),
