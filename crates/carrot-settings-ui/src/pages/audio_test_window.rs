use carrot_audio::{AudioSettings, CHANNEL_COUNT, RodioExt, SAMPLE_RATE};
use carrot_platform_title_bar::PlatformTitleBar;
use carrot_release_channel::ReleaseChannel;
use carrot_ui::{Button, ButtonStyle, Label, prelude::*};
use carrot_workspace::client_side_decorations;
use cpal::DeviceId;
use inazuma::{
    App, Context, Entity, FocusHandle, Focusable, Render, Size, Tiling, Window, WindowBounds,
    WindowKind, WindowOptions, px,
};
use inazuma_settings_framework::{AudioInputDeviceName, AudioOutputDeviceName, Settings};
    ...
    thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                ...
                let Ok((_stream, mixer)) = carrot_audio::open_test_output(output_device_id) else {
...
fn open_test_microphone(
    ...
    let stream = carrot_audio::open_input_stream(input_device_id)?;
    let stream = stream
...
impl Render for AudioTestWindow {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let button_style = if is_testing {
            ButtonStyle::tinted(carrot_ui::TintColor::Error)
        } else {
            ButtonStyle::FILLED
        };
    ...
    cx.open_window(
        WindowOptions {
            titlebar: Some(inazuma::TitlebarOptions {
                ...
                traffic_light_position: Some(inazuma::point(px(12.0), px(12.0))),
            }),
            focus: true,
            show: true,
            is_movable: true,
            ...
            window_decorations: Some(inazuma::WindowDecorations::Client),
