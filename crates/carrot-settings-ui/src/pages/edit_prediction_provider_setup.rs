use carrot_codestral::{CODESTRAL_API_URL, codestral_api_key_state, codestral_api_url};
...
use carrot_edit_prediction_ui::{get_available_providers, set_completion_provider};
use carrot_language::language_settings::AllLanguageSettings;
use inazuma::{Entity, ScrollHandle, prelude::*};

use carrot_ui::{
    ButtonLink, ConfiguredApiCard, ContextMenu, DropdownMenu, DropdownStyle, prelude::*,
};
...
fn render_provider_dropdown(window: &mut Window, cx: &mut App) -> AnyElement {
    ...
    let menu = ContextMenu::build(window, cx, move |mut menu, _, cx| {
        ...
        let fs = <dyn carrot_fs::Fs>::global(cx);
                ...
                .child(
                    v_flex()
                        ...
                        .when_some(env_var_name, |this, env_var_name| {
                            this.child({
                                let label = format!(
                                    "Or set the {} env var and restart Carrot.",
                                    env_var_name.as_ref()
                                );
...
fn render_github_copilot_provider(window: &mut Window, cx: &mut App) -> Option<impl IntoElement> {
    let configuration_view = window.use_state(cx, |_, cx| {
        carrot_copilot_ui::ConfigurationView::new(
            ...
            carrot_copilot_ui::ConfigurationMode::EditPrediction,
            cx,
        )
    });
