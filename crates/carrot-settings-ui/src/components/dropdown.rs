...
use inazuma::{App, ElementId, IntoElement, RenderOnce};
    ...
    T: strum::VariantArray + strum::VariantNames + Copy + PartialEq + Send + Sync + 'static,
{
    ...
    on_change: Rc<dyn Fn(T, &mut carrot_ui::Window, &mut App) + 'static>,
}
    ...
    T: strum::VariantArray + strum::VariantNames + Copy + PartialEq + Send + Sync + 'static,
{
    pub fn new(
        ...
        variants: &'static [T],
        labels: &'static [&'static str],
        on_change: impl Fn(T, &mut carrot_ui::Window, &mut App) + 'static,
    ) -> Self {
        Self {
            id: id.into(),
            current_value,
