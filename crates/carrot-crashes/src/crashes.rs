...
use carrot_system_specs::GpuSpecs;
...
async fn connect_and_keepalive(crash_init: InitCrashHandler, handler: CrashHandler) {
    ...
    let carrot_pid = process::id();
    let socket_name = carrot_paths::temp_dir().join(format!("carrot-crash-handler-{carrot_pid}"));
    #[cfg(not(target_os = "windows"))]
...
pub struct CrashServer {
    ...
    active_gpu: Mutex<Option<carrot_system_specs::GpuSpecs>>,
...
pub struct CrashInfo {
    ...
    pub gpus: Vec<carrot_system_specs::GpuInfo>,
    pub active_gpu: Option<carrot_system_specs::GpuSpecs>,
...
impl minidumper::ServerHandler for CrashServer {
    fn create_minidump_file(&self) -> Result<(File, PathBuf), io::Error> {
        let dump_path = carrot_paths::logs_dir()
            .join(
                &self
    ...
    fn on_minidump_created(&self, result: Result<MinidumpBinary, minidumper::Error>) -> LoopAction {
        ...
        let gpus = match carrot_system_specs::read_gpu_info_from_sys_class_drm() {
            Ok(gpus) => gpus,
            Err(err) => {
        ...
        let crash_data_path = carrot_paths::logs_dir()
