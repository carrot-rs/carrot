...
use carrot_agent_settings::{AgentSettings, CompiledRegex, ToolPermissions, ToolRules};
use carrot_shell_command_parser::{
                ...
                if !rules.always_allow.is_empty() {
                    ...
                    return ToolPermissionDecision::Deny(format!(
                        "The {} shell does not support \"always allow\" patterns for the terminal \
                         tool because Carrot cannot parse its command chaining syntax. Please remove \
...
mod tests {
    ...
    use carrot_agent_settings::{AgentProfileId, CompiledRegex, InvalidRegexPattern, ToolRules};
    use inazuma::px;
    use inazuma_settings_framework::{DefaultAgentView, DockPosition, NotifyWhenAgentWaiting};
    use std::sync::Arc;
    ...
    fn normalize_path_collapses_dot_segments() {
        assert_eq!(
            normalize_path("src/../.carrot/settings.json"),
            ".carrot/settings.json"
        );
        assert_eq!(normalize_path("a/b/../c"), "a/c");
        assert_eq!(normalize_path("a/./b/c"), "a/b/c");
        assert_eq!(normalize_path("a/b/./c/../d"), "a/b/d");
        assert_eq!(
            normalize_path(".carrot/settings.json"),
            ".carrot/settings.json"
        );
        assert_eq!(normalize_path("a/b/c"), "a/b/c");
    }
