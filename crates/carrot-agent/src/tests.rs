...
use carrot_agent_settings::AgentProfileId;
use carrot_client::{Client, UserStore};
use carrot_context_server::{ContextServer, ContextServerCommand, ContextServerId};
use carrot_feature_flags::FeatureFlagAppExt as _;
use carrot_fs::{FakeFs, Fs};
use carrot_http_client::FakeHttpClient;
...
use inazuma::{App, AppContext, AsyncApp, Entity, Task, TestAppContext, UpdateGlobal};
use inazuma_collections::IndexMap;
...
fn always_allow_tools(cx: &mut TestAppContext) {
    cx.update(|cx| {
        let mut settings = carrot_agent_settings::AgentSettings::get_global(cx).clone();
        settings.tool_permissions.default = inazuma_settings_framework::ToolPermissionMode::Allow;
        carrot_agent_settings::AgentSettings::override_global(settings, cx);
    });
}
...
async fn test_tool_authorization(cx: &mut TestAppContext) {
    ...
    tool_call_auth_1
        .response
        .send(carrot_acp_thread::SelectedPermissionOutcome::new(
    ...
    assert_eq!(
        message.content,
        vec![
            carrot_language_model::MessageContent::ToolResult(LanguageModelToolResult {
    ...
    assert_eq!(
        message.content,
        vec![carrot_language_model::MessageContent::ToolResult(
...
async fn expect_tool_call_update_fields(
    ...
    let event = events
        ...
        .unwrap();
    match event {
        ThreadEvent::ToolCallUpdate(carrot_acp_thread::ToolCallUpdate::UpdateFields(update)) => {
            update
        }
        event => {
            panic!("Unexpected event {event:?}");
...
async fn test_mcp_tools(cx: &mut TestAppContext) {
    ...
    let mut mcp_tool_calls = setup_context_server(
        "test_server",
        vec![carrot_context_server::types::Tool {
            name: "echo".into(),
            description: None,
    ...
    tool_call_response
        .send(carrot_context_server::types::CallToolResponse {
            content: vec![carrot_context_server::types::ToolResponseContent::Text {
                text: "test".into(),
            }],
            is_error: None,
            meta: None,
...
async fn test_mcp_tool_result_displayed_when_server_disconnected(cx: &mut TestAppContext) {
    ...
    fake_model.send_last_completion_stream_event(LanguageModelCompletionEvent::ToolUse(
        LanguageModelToolUse {
            id: "tool_1".into(),
            name: "issue_read".into(),
            raw_input: json!({"issue_url": "https://github.com/carrot-rs/carrot/issues/47404"})
                .to_string(),
            input: json!({"issue_url": "https://github.com/carrot-rs/carrot/issues/47404"}),
            is_input_complete: true,
            thought_signature: None,
        },
    ));
    ...
    while let Some(event) = replay_events.next().await {
        ...
        match &event {
            ...
            ThreadEvent::ToolCallUpdate(carrot_acp_thread::ToolCallUpdate::UpdateFields(
                update,
...
async fn test_mcp_tool_truncation(cx: &mut TestAppContext) {
    ...
    let _server1_calls = setup_context_server(
        "xxx",
        vec![
            carrot_context_server::types::Tool {
...
async fn test_terminal_tool_cancellation_captures_output(cx: &mut TestAppContext) {
    ...
    thread.update(cx, |thread, _cx| {
        ...
        let result_text = match &tool_result.content {
            carrot_language_model::LanguageModelToolResultContent::Text(text) => text.to_string(),
...
async fn wait_for_terminal_tool_started(
    ...
    for _ in 0..deadline {
        ...
        while let Some(Some(event)) = events.next().now_or_never() {
            if let Ok(ThreadEvent::ToolCallUpdate(
...
async fn test_retry_cancelled_promptly_on_new_send(cx: &mut TestAppContext) {
    ...
    model_a.send_last_completion_stream_error(
        LanguageModelCompletionError::UpstreamProviderError {
            ...
            status: carrot_http_client::StatusCode::INTERNAL_SERVER_ERROR,
            retry_after: None,
        },
    );
...
async fn test_agent_connection(cx: &mut TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        carrot_language_model::init(user_store.clone(), client.clone(), cx);
        carrot_language_models::init(user_store, client.clone(), cx);
        LanguageModelRegistry::test(cx);
    });
    cx.executor().forbid_parking();

    // Create a project for new_thread
    let fake_fs = cx.update(|cx| carrot_fs::FakeFs::new(cx.background_executor().clone()));
    ...
    let result = cx
        .update(|cx| {
            connection.prompt(
                Some(carrot_acp_thread::UserMessageId::new()),
...
async fn test_send_retry_on_error(cx: &mut TestAppContext) {
    ...
    assert!(matches!(
        retry_events[0],
        carrot_acp_thread::RetryStatus { attempt: 1, .. }
    ));
    thread.read_with(cx, |thread, _cx| {
        assert_eq!(
            thread.to_markdown(),
...
async fn test_send_retry_finishes_tool_calls_on_error(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        ...
        vec![
            ...
            LanguageModelRequestMessage {
                role: Role::Assistant,
                content: vec![carrot_language_model::MessageContent::ToolUse(
                    tool_use_1.clone()
                )],
                cache: false,
                reasoning_details: None,
            ...
            LanguageModelRequestMessage {
                role: Role::User,
                content: vec![carrot_language_model::MessageContent::ToolResult(
                    LanguageModelToolResult {
...
fn watch_settings(fs: Arc<dyn Fs>, cx: &mut App) {
    ...
    cx.spawn({
        async move |cx| {
            let (mut new_settings_content_rx, watcher_task) =
                inazuma_settings_framework::watch_config_file(
                    cx.background_executor(),
                    fs,
...
fn setup_context_server(
    name: &'static str,
    tools: Vec<carrot_context_server::types::Tool>,
    context_server_store: &Entity<ContextServerStore>,
    cx: &mut TestAppContext,
) -> mpsc::UnboundedReceiver<(
    carrot_context_server::types::CallToolParams,
    oneshot::Sender<carrot_context_server::types::CallToolResponse>,
)> {
    cx.update(|cx| {
    ...
    let fake_transport = carrot_context_server::test::create_fake_transport(name, cx.executor())
        .on_request::<carrot_context_server::types::requests::Initialize, _>(
            move |_params| async move {
                carrot_context_server::types::InitializeResponse {
                    protocol_version: carrot_context_server::types::ProtocolVersion(
                        carrot_context_server::types::LATEST_PROTOCOL_VERSION.to_string(),
                    ),
                    server_info: carrot_context_server::types::Implementation {
                        name: name.into(),
                        version: "1.0.0".to_string(),
                    },
                    capabilities: carrot_context_server::types::ServerCapabilities {
                        tools: Some(carrot_context_server::types::ToolsCapabilities {
        ...
        .on_request::<carrot_context_server::types::requests::ListTools, _>(move |_params| {
            let tools = tools.clone();
            async move {
                carrot_context_server::types::ListToolsResponse {
                    tools,
                    next_cursor: None,
                    meta: None,
                }
        ...
        .on_request::<carrot_context_server::types::requests::CallTool, _>(move |params| {
    ...
    // Test 1: Deny rule blocks command
    {
        ...
        cx.update(|cx| {
            ...
            settings.tool_permissions.tools.insert(
                ...
                carrot_agent_settings::ToolRules {
                    default: Some(inazuma_settings_framework::ToolPermissionMode::Confirm),
                    always_allow: vec![],
                    always_deny: vec![
                        carrot_agent_settings::CompiledRegex::new(r"rm\s+-rf", false).unwrap(),
                    ],
                    always_confirm: vec![],
        ...
        cx.update(|cx| {
            ...
            settings.tool_permissions.tools.insert(
                ...
                carrot_agent_settings::ToolRules {
                    ...
                    always_allow: vec![
                        carrot_agent_settings::CompiledRegex::new(r"^echo\s", false).unwrap(),
                    ],
                    always_deny: vec![],
                    always_confirm: vec![],
                    invalid_patterns: vec![],
    ...
    // Test 3: global default: allow does NOT override always_confirm patterns
    {
        ...
        cx.update(|cx| {
            ...
            settings.tool_permissions.default =
                inazuma_settings_framework::ToolPermissionMode::Allow;
            settings.tool_permissions.tools.insert(
                TerminalTool::NAME.into(),
                carrot_agent_settings::ToolRules {
                    ...
                    always_deny: vec![],
                    always_confirm: vec![
                        carrot_agent_settings::CompiledRegex::new(r"sudo", false).unwrap(),
                    ],
                    invalid_patterns: vec![],
                },
            );
...
async fn test_edit_file_tool_deny_rule_blocks_edit(cx: &mut TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"sensitive", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    let thread = cx.new(|cx| {
        crate::Thread::new(
            project.clone(),
            cx.new(|_cx| carrot_prompt_store::ProjectContext::default()),
            context_server_registry,
            templates.clone(),
            None,
            cx,
...
async fn test_delete_path_tool_deny_rule_blocks_deletion(cx: &mut TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"important", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    let action_log = cx.new(|_cx| carrot_action_log::ActionLog::new(project.clone()));
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"protected", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"secret", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"confidential", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"readonly", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"internal\.company", false).unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
            },
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                default: Some(inazuma_settings_framework::ToolPermissionMode::Confirm),
                always_allow: vec![
                    carrot_agent_settings::CompiledRegex::new(r"\.md$", false).unwrap(),
                ],
                always_deny: vec![],
                always_confirm: vec![],
                invalid_patterns: vec![],
...
async fn test_edit_file_tool_allow_still_prompts_for_local_settings(cx: &mut TestAppContext) {
    ...
    // Editing a file inside .carrot/ should still prompt even with global default: allow,
    ...
    let _task = cx.update(|cx| {
        tool.run(
            ToolInput::resolved(crate::EditFileToolInput {
                ...
                path: "root/.carrot/settings.json".into(),
                mode: crate::EditFileMode::Edit,
            }),
            event_stream,
            cx,
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                ...
                always_allow: vec![],
                always_deny: vec![
                    carrot_agent_settings::CompiledRegex::new(r"internal\.company\.com", false)
                        .unwrap(),
                ],
                always_confirm: vec![],
                invalid_patterns: vec![],
    ...
    cx.update(|cx| {
        ...
        settings.tool_permissions.tools.insert(
            ...
            carrot_agent_settings::ToolRules {
                default: Some(inazuma_settings_framework::ToolPermissionMode::Confirm),
                always_allow: vec![
                    carrot_agent_settings::CompiledRegex::new(r"docs\.rs", false).unwrap(),
                ],
                always_deny: vec![],
                always_confirm: vec![],
                invalid_patterns: vec![],
    ...
    assert_eq!(
        ...
        vec![
            ...
            LanguageModelRequestMessage {
                role: Role::Assistant,
                content: vec![
                    carrot_language_model::MessageContent::ToolUse(first_tool_use.clone()),
                    carrot_language_model::MessageContent::ToolUse(second_tool_use.clone())
                ],
                cache: false,
                reasoning_details: None,
            },
