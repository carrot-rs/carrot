...
use carrot_language::{Anchor, Buffer, BufferSnapshot, LineIndent, Point, TextBufferSnapshot};
...
use carrot_project::{AgentLocation, Project};
...
use inazuma::{AppContext, AsyncApp, Entity, Task};
...
impl EditAgent {
                ...
                self.project.update(cx, |project, cx| {
                    project.set_agent_location(
                        Some(AgentLocation {
                            ...
                            position: carrot_language::Anchor::min_for_buffer(buffer_id),
                        }),
                        cx,
                    )
                });
            ...
            if self.update_agent_location {
                self.project.update(cx, |project, cx| {
                    project.set_agent_location(
                        Some(AgentLocation {
                            buffer: buffer.downgrade(),
                            position: carrot_language::Anchor::max_for_buffer(buffer_id),
                        }),
                        cx,
                    )
                })
    ...
    fn resolve_old_text<T>(
        ...
        carrot_watch::Receiver<Option<Range<usize>>>,
    )
    where
        T: 'static + Send + Unpin + Stream<Item = Result<EditParserEvent>>,
    {
        let (mut old_range_tx, old_range_rx) = carrot_watch::channel(None);
...
mod tests {
    ...
    async fn test_overwrite_events(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            ...
            Some(AgentLocation {
                buffer: buffer.downgrade(),
                position: carrot_language::Anchor::min_for_buffer(
        ...
        assert_eq!(
            ...
            Some(AgentLocation {
                buffer: buffer.downgrade(),
                position: carrot_language::Anchor::max_for_buffer(
                    cx.update(|cx| buffer.read(cx).remote_id())
                ),
            })
        );
