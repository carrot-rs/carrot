...
pub use carrot_db::*;
pub use carrot_shell_command_parser::extract_commands;
...
use carrot_language_model::{
    IconOrSvg, LanguageModel, LanguageModelProvider, LanguageModelRegistry,
};
use carrot_project::{AgentId, Project, ProjectItem, ProjectPath, Worktree};
use carrot_prompt_store::{
...
pub struct ProjectSnapshot {
    pub worktree_snapshots: Vec<carrot_project::telemetry_snapshot::TelemetryWorktreeSnapshot>,
...
struct ProjectState {
    ...
    project_context_needs_refresh: carrot_watch::Sender<()>,
...
struct Session {
    ...
    acp_thread: Entity<carrot_acp_thread::AcpThread>,
    project_id: EntityId,
...
pub struct LanguageModels {
    ...
    model_list: carrot_acp_thread::AgentModelList,
    refresh_models_rx: carrot_watch::Receiver<()>,
    refresh_models_tx: carrot_watch::Sender<()>,
    _authenticate_all_providers_task: Task<()>,
}

impl LanguageModels {
    fn new(cx: &mut App) -> Self {
        let (refresh_models_tx, refresh_models_rx) = carrot_watch::channel(());

        let mut this = Self {
            models: HashMap::default(),
            model_list: carrot_acp_thread::AgentModelList::Grouped(IndexMap::default()),
            refresh_models_rx,
            refresh_models_tx,
    ...
    fn refresh_list(&mut self, cx: &App) {
        ...
        if !recommended.is_empty() {
            language_model_list.insert(
                carrot_acp_thread::AgentModelGroupName("Recommended".into()),
                recommended,
            );
        }
        ...
        for provider in providers {
            ...
            if !provider_models.is_empty() {
                language_model_list.insert(
                    carrot_acp_thread::AgentModelGroupName(provider.name().0.clone()),
                    provider_models,
                );
            }
        }
        ...
        self.models = models;
        self.model_list = carrot_acp_thread::AgentModelList::Grouped(language_model_list);
        self.refresh_models_tx.send(()).ok();
    }

    fn watch(&self) -> carrot_watch::Receiver<()> {
    ...
    ) -> carrot_acp_thread::AgentModelInfo {
        carrot_acp_thread::AgentModelInfo {
            id: Self::model_id(model),
            name: model.name().0,
            description: None,
            icon: Some(match provider.icon() {
                IconOrSvg::Svg(path) => carrot_acp_thread::AgentModelIcon::Path(path),
                IconOrSvg::Icon(name) => carrot_acp_thread::AgentModelIcon::Named(name),
            }),
        ...
        cx.background_spawn(async move {
            for (provider_id, provider_name, authenticate_task) in authenticate_all_providers {
                if let Err(err) = authenticate_task.await {
                    match err {
                        carrot_language_model::AuthenticateError::CredentialsNotFound => {
                            // Since we're authenticating these providers in the
                            // background for the purposes of populating the
                            // language selector, we don't care about providers
                            // where the credentials are not found.
                        }
                        carrot_language_model::AuthenticateError::ConnectionRefused => {
...
impl NativeAgent {
    ...
    fn register_session(
        ...
        let acp_thread = cx.new(|cx| {
            let mut acp_thread = carrot_acp_thread::AcpThread::new(
                parent_session_id,
                title,
                None,
                connection,
    ...
    async fn maintain_project_context(
        ...
        project_id: EntityId,
        mut needs_refresh: carrot_watch::Receiver<()>,
        cx: &mut AsyncApp,
    ) -> Result<()> {
    ...
    fn handle_project_event(
        ...
        match event {
            carrot_project::Event::WorktreeAdded(_) | carrot_project::Event::WorktreeRemoved(_) => {
                state.project_context_needs_refresh.send(()).ok();
            }
            carrot_project::Event::WorktreeUpdatedEntries(_, items) => {
    ...
    fn handle_prompts_updated_event(
        ...
        _event: &carrot_prompt_store::PromptsUpdatedEvent,
        _cx: &mut Context<Self>,
    ) {
    ...
    fn handle_models_updated_event(
        ...
        event: &carrot_language_model::Event,
        cx: &mut Context<Self>,
    ) {
            ...
            session.thread.update(cx, |thread, cx| {
                ...
                if let Some(model) = summarization_model.clone() {
                    if thread.summarization_model().is_none()
                        || matches!(
                            event,
                            carrot_language_model::Event::ThreadSummaryModelChanged
                        )
                    {
    ...
    fn handle_context_server_store_updated(
        &mut self,
        store: Entity<carrot_project::context_server_store::ContextServerStore>,
        _event: &carrot_project::context_server_store::ServerStatusChangedEvent,
        cx: &mut Context<Self>,
    ) {
    ...
    fn send_mcp_prompt(
        ...
        cx.spawn(async move |this, cx| {
            ...
            for message in prompt.messages {
                let carrot_context_server::types::PromptMessage { role, content } = message;
                let block = mcp_message_content_to_acp_content_block(content);

                match role {
                    carrot_context_server::types::Role::User => {
                        let id = carrot_acp_thread::UserMessageId::new();
                    ...
                    carrot_context_server::types::Role::Assistant => {
                        acp_thread.update(cx, |acp_thread, cx| {
                ...
                last_is_user = role == carrot_context_server::types::Role::User;
            }
                    ...
                    Ok(event) => {
                        ...
                        match event {
                            ...
                            ThreadEvent::ToolCallAuthorization(ToolCallAuthorization {
                                ...
                                cx.background_spawn(async move {
                                    if let carrot_acp_thread::RequestPermissionOutcome::Selected(
                                        outcome,
                                    ) = outcome_task.await
                                    {
                                        response
...
impl carrot_acp_thread::AgentModelSelector for NativeAgentModelSelector {
    fn list_models(&self, cx: &mut App) -> Task<Result<carrot_acp_thread::AgentModelList>> {
    ...
    fn selected_model(&self, cx: &mut App) -> Task<Result<carrot_acp_thread::AgentModelInfo>> {
        let Some(thread) = self
            .connection
            .0
            .read(cx)
    ...
    fn watch(&self, cx: &mut App) -> Option<carrot_watch::Receiver<()>> {
...
pub static ZED_AGENT_ID: LazyLock<AgentId> = LazyLock::new(|| AgentId::new("Carrot Agent"));

impl carrot_acp_thread::AgentConnection for NativeAgentConnection {
    ...
    fn new_session(
        ...
        work_dirs: PathList,
        cx: &mut App,
    ) -> Task<Result<Entity<carrot_acp_thread::AcpThread>>> {
    ...
    fn prompt(
        &self,
        id: Option<carrot_acp_thread::UserMessageId>,
    ...
    fn retry(
        ...
        session_id: &acp::SessionId,
        _cx: &App,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentSessionRetry>> {
    ...
    fn truncate(
        ...
        session_id: &acp::SessionId,
        cx: &App,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentSessionTruncate>> {
    ...
    fn set_title(
        ...
        session_id: &acp::SessionId,
        cx: &App,
    ) -> Option<Rc<dyn carrot_acp_thread::AgentSessionSetTitle>> {
        self.0.read_with(cx, |agent, _cx| {
            agent
                .sessions
                .get(session_id)
    ...
    fn telemetry(&self) -> Option<Rc<dyn carrot_acp_thread::AgentTelemetry>> {
        Some(Rc::new(self.clone()) as Rc<dyn carrot_acp_thread::AgentTelemetry>)
    }
...
impl carrot_acp_thread::AgentTelemetry for NativeAgentConnection {
    fn thread_data(
        &self,
        session_id: &acp::SessionId,
        cx: &mut App,
...
pub struct NativeAgentSessionList {
    thread_store: Entity<ThreadStore>,
    updates_tx: smol::channel::Sender<carrot_acp_thread::SessionListUpdate>,
    updates_rx: smol::channel::Receiver<carrot_acp_thread::SessionListUpdate>,
...
impl NativeAgentSessionList {
    fn new(thread_store: Entity<ThreadStore>, cx: &mut App) -> Self {
        ...
        let subscription = cx.observe(&thread_store, move |_, _| {
            this_tx
                .try_send(carrot_acp_thread::SessionListUpdate::Refresh)
                .ok();
        });
        Self {
            thread_store,
...
impl AgentSessionList for NativeAgentSessionList {
    ...
    fn watch(
        &self,
        _cx: &mut App,
    ) -> Option<smol::channel::Receiver<carrot_acp_thread::SessionListUpdate>> {
...
impl carrot_acp_thread::AgentSessionTruncate for NativeAgentSessionTruncate {
    fn run(&self, message_id: carrot_acp_thread::UserMessageId, cx: &mut App) -> Task<Result<()>> {
...
impl carrot_acp_thread::AgentSessionRetry for NativeAgentSessionRetry {
...
impl carrot_acp_thread::AgentSessionSetTitle for NativeAgentSessionSetTitle {
...
pub struct AcpTerminalHandle {
    terminal: Entity<carrot_acp_thread::Terminal>,
...
mod internal_tests {
    ...
    use super::*;
    use carrot_acp_thread::{AgentConnection, AgentModelGroupName, AgentModelInfo, MentionUri};
    use carrot_fs::FakeFs;
    use carrot_language_model::fake_provider::{FakeLanguageModel, FakeLanguageModelProvider};
    use carrot_language_model::{
    ...
    async fn test_listing_models(cx: &mut TestAppContext) {
        ...
        let carrot_acp_thread::AgentModelList::Grouped(models) = models else {
        ...
        assert_eq!(
            ...
            IndexMap::from_iter([(
                ...
                vec![AgentModelInfo {
                    ...
                    name: "Fake".into(),
                    description: None,
                    icon: Some(carrot_acp_thread::AgentModelIcon::Named(
                        carrot_ui::IconName::CarrotAssistant
                    )),
                    is_latest: false,
                    cost: None,
                }]
    ...
    async fn test_model_selection_persists_to_settings(cx: &mut TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        fs.create_dir(carrot_paths::settings_file().parent().unwrap())
            .await
            .unwrap();
        fs.insert_file(
        ...
        let settings_content = fs.load(carrot_paths::settings_file()).await.unwrap();
    ...
    async fn test_select_model_updates_thinking_enabled(cx: &mut TestAppContext) {
        ...
        fs.create_dir(carrot_paths::settings_file().parent().unwrap())
            .await
            .unwrap();
        fs.insert_file(carrot_paths::settings_file(), b"{}".to_vec())
            .await;
        let project = Project::test(fs.clone(), [], cx).await;
    ...
    async fn test_save_load_thread(cx: &mut TestAppContext) {
        ...
        model.send_last_completion_stream_event(LanguageModelCompletionEvent::UsageUpdate(
            carrot_language_model::TokenUsage {
                input_tokens: 150,
                output_tokens: 75,
                ..Default::default()
            },
        ...
        thread.update(cx, |thread, _cx| {
            thread.set_ui_scroll_position(Some(inazuma::ListOffset {
                item_ix: 5,
                offset_in_item: inazuma::px(12.5),
            }));
        });
        ...
        acp_thread.read_with(cx, |thread, _| {
            ...
            assert_eq!(scroll.offset_in_item, inazuma::px(12.5));
        });
    }
...
fn mcp_message_content_to_acp_content_block(
    content: carrot_context_server::types::MessageContent,
) -> acp::ContentBlock {
    match content {
        carrot_context_server::types::MessageContent::Text {
            text,
            annotations: _,
        } => text.into(),
        carrot_context_server::types::MessageContent::Image {
            data,
            mime_type,
            annotations: _,
        } => acp::ContentBlock::Image(acp::ImageContent::new(data, mime_type)),
        carrot_context_server::types::MessageContent::Audio {
            data,
            mime_type,
            annotations: _,
        } => acp::ContentBlock::Audio(acp::AudioContent::new(data, mime_type)),
        carrot_context_server::types::MessageContent::Resource {
            resource,
            annotations: _,
        } => {
            let mut link =
