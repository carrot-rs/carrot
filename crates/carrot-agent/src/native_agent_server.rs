...
use inazuma_settings_framework::{LanguageModelSelection, Settings as _, update_settings_file};
...
impl AgentServer for NativeAgentServer {
    ...
    fn connect(
        ...
        cx: &mut App,
    ) -> Task<Result<Rc<dyn carrot_acp_thread::AgentConnection>>> {
        ...
        cx.spawn(async move |cx| {
            ...
            Ok(Rc::new(connection) as Rc<dyn carrot_acp_thread::AgentConnection>)
        })
    }
...
mod tests {
    ...
    carrot_agent_servers::e2e_tests::common_e2e_tests!(
        async |fs, cx| {
            let auth = cx.update(|cx| {
                carrot_prompt_store::init(cx);
                let registry = carrot_language_model::LanguageModelRegistry::read_global(cx);
                let auth = registry
                    .provider(&carrot_language_model::ANTHROPIC_PROVIDER_ID)
                    .unwrap()
                    .authenticate(cx);
            ...
            cx.update(|cx| {
                let registry = carrot_language_model::LanguageModelRegistry::global(cx);

                registry.update(cx, |registry, cx| {
                    registry.select_default_model(
                        Some(&carrot_language_model::SelectedModel {
                            provider: carrot_language_model::ANTHROPIC_PROVIDER_ID,
                            model: carrot_language_model::LanguageModelId(
                                "claude-sonnet-4-latest".into(),
                            ),
                        }),
                        cx,
                    );
