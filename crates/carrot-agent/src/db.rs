...
use carrot_acp_thread::UserMessageId;
use carrot_agent_settings::AgentProfileId;
use carrot_env_vars::CARROT_STATELESS;
use carrot_language_model::Speed;
use carrot_sqlez::{
    bindable::{Bind, Column},
    connection::Connection,
    statement::Statement,
};
use carrot_ui::{App, SharedString};
use chrono::{DateTime, Utc};
use futures::{FutureExt, future::Shared};
use inazuma::{BackgroundExecutor, Global, Task};
...
impl From<&DbThreadMetadata> for carrot_acp_thread::AgentSessionInfo {
...
pub struct DbThread {
    ...
    #[serde(default)]
    pub cumulative_token_usage: carrot_language_model::TokenUsage,
    #[serde(default)]
    pub request_token_usage:
        HashMap<carrot_acp_thread::UserMessageId, carrot_language_model::TokenUsage>,
    #[serde(default)]
    ...
    fn upgrade_from_agent_1(thread: crate::legacy_thread::SerializedThread) -> Result<Self> {
        ...
        for (ix, msg) in thread.messages.into_iter().enumerate() {
            let message = match msg.role {
                carrot_language_model::Role::User => {
                ...
                carrot_language_model::Role::Assistant => {
                    let mut content = Vec::new();
                    ...
                    for tool_use in msg.tool_uses {
                        ...
                        content.push(AgentMessageContent::ToolUse(
                            carrot_language_model::LanguageModelToolUse {
                                id: tool_use.id,
                    ...
                    for tool_result in msg.tool_results {
                        ...
                        tool_results.insert(
                            ...
                            carrot_language_model::LanguageModelToolResult {
                ...
                carrot_language_model::Role::System => {
...
impl ThreadsDatabase {
    ...
    pub fn new(executor: BackgroundExecutor) -> Result<Self> {
        let connection = if *CARROT_STATELESS {
            ...
            let threads_dir = carrot_paths::data_dir().join("threads");
