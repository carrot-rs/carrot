...
use carrot_prompt_store::{ProjectContext, WorktreeContext};
use futures::{FutureExt, StreamExt, future::LocalBoxFuture};
use inazuma::{AppContext as _, AsyncApp, Entity, TestAppContext, UpdateGlobal as _};
    ...
    fn assert_diff_any(expected_diffs: Vec<impl Into<String>>) -> Self {
        ...
        Self::new(async move |sample, _judge, _cx| {
            let matches = expected_diffs.iter().any(|possible_diff| {
                carrot_language::apply_diff_patch(&sample.text_before, possible_diff)
                    .map(|expected| {
...
impl StreamingEditToolTest {
    async fn new(cx: &mut TestAppContext) -> Self {
        ...
        cx.update(|cx| {
            ...
            carrot_language_models::init(user_store, client, cx);
        });
    ...
    async fn eval(
        ...
        let sample = EvalSample {
            tool_input,
            diff: carrot_language::unified_diff(
...
fn eval_delete_function() {
    ...
    let possible_diffs = vec![
        carrot_language::unified_diff(input_file_content, output_file_content),
        carrot_language::unified_diff(
            input_file_content,
            &output_file_content
                .replace(
