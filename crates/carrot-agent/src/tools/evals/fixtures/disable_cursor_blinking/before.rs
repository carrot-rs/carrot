...
//! The main point of interest in this crate is [`Editor`] type, which is used in every other Carrot part as a user input element.
...
use carrot_buffer_diff::DiffHunkStatus;
use carrot_client::{Collaborator, ParticipantIndex};
use inazuma_clock::ReplicaId;
use inazuma_collections::{BTreeMap, HashMap, HashSet, VecDeque};
...
use carrot_feature_flags::{DebuggerFeatureFlag, FeatureFlagAppExt};
use futures::{
    FutureExt,
...
use carrot_git::blame::{GitBlame, GlobalBlameRenderer};
use inazuma::{
...
use carrot_language::{BufferRow, CharClassifier, Runnable, RunnableRange, point_to_lsp};
...
pub use carrot_git::blame::BlameRenderer;
pub use proposed_changes_editor::{
    ProposedChangeLocation, ProposedChangesEditor, ProposedChangesEditorToolbar,
};
use smallvec::smallvec;
use std::{cell::OnceCell, iter::Peekable};
use carrot_task::{ResolvedTask, RunnableTag, TaskTemplate, TaskVariables};

pub use carrot_lsp::CompletionContext;
use carrot_lsp::{
...
use carrot_language::BufferSnapshot;
pub use lsp_ext::lsp_tasks;
use movement::TextLayoutDetails;
pub use carrot_multi_buffer::{
...
use rand::prelude::*;
use carrot_rpc::{ErrorExt, proto::*};
use scroll::{Autoscroll, OngoingScroll, ScrollAnchor, ScrollManager, ScrollbarAutoHide};
use selections_collection::{
    MutableSelectionsCollection, SelectionsCollection, resolve_selections,
};
use serde::{Deserialize, Serialize};
use inazuma_settings_framework::{Settings, SettingsLocation, SettingsStore, update_settings_file};
use smallvec::SmallVec;
...
impl inazuma::Global for GlobalDiagnosticRenderer {}
...
pub enum SelectPhase {
    ...
    Update {
        position: DisplayPoint,
        goal_column: u32,
        scroll_delta: inazuma::Point<f32>,
    },
    End,
}
...
/// Carrot's primary implementation of text input, allowing users to edit a [`MultiBuffer`].
///
...
pub struct Editor {
    ...
    pixel_position_of_newest_cursor: Option<inazuma::Point<Pixels>>,
...
impl Editor {
    ...
    pub fn new_file(
        ...
        Self::new_in_workspace(workspace, window, cx).detach_and_prompt_err(
            ...
            |e, _, _| match e.error_code() {
                ...
                "The remote instance of Carrot does not support this yet. It must be upgraded to {}",
    ...
    fn folds_did_change(&mut self, cx: &mut Context<Self>) {
        use inazuma_text::ToOffset as _;
        use inazuma_text::ToPoint as _;
    ...
    fn linked_editing_ranges_for(
        ...
        use inazuma_text::ToOffset as TO;
            ...
            for (buffer, edits) in linked_edits {
                buffer.update(cx, |buffer, cx| {
                    ...
                    let edits = edits
                        .into_iter()
                        .map(|(range, text)| {
                            use inazuma_text::ToPoint as TP;
    ...
    fn trigger_on_type_formatting(
        ...
        // OnTypeFormatting returns a list of edits, no need to pass them between Carrot instances,
    ...
    fn do_completion(
        ...
        use carrot_language::ToOffset as _;
    ...
    fn render_breakpoint(
        ...
        let alt_as_text = inazuma::Keystroke {
    ...
    fn render_edit_prediction_popover(
        ...
        line_height: Pixels,
        scroll_pixel_position: inazuma::Point<Pixels>,
        newest_selection_head: Option<DisplayPoint>,
        editor_width: Pixels,
        style: &EditorStyle,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<(AnyElement, inazuma::Point<Pixels>)> {
    ...
    fn render_edit_prediction_modifier_jump_popover(
        ...
        let scrolled_content_origin =
            content_origin - inazuma::Point::new(scroll_pixel_position.x, Pixels(0.0));
    ...
    fn render_edit_prediction_scroll_popover(
        ...
        line_layouts: &[LineWithInvisibles],
        newest_selection_head: Option<DisplayPoint>,
        scrolled_content_origin: inazuma::Point<Pixels>,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<(AnyElement, inazuma::Point<Pixels>)> {
            ...
            .child(
                h_flex()
                    ...
                    .shadow(vec![inazuma::BoxShadow {
                        color: inazuma::black().opacity(0.05),
        ...
        let result = h_flex()
            ...
            .when(!has_keybind, |el| {
                ...
                el.bg(status_colors.error_background)
                    .border_color(status_colors.error.opacity(0.6))
                    .pl_2()
                    .child(Icon::new(IconName::CarrotPredictError).color(Color::Error))
                    .cursor_default()
                    .hoverable_tooltip(move |_window, cx| {
                        cx.new(|_| MissingEditPredictionKeybindingTooltip).into()
                    })
    ...
    fn render_edit_prediction_cursor_popover(
        ...
        cursor_point: Point,
        style: &EditorStyle,
        accept_keystroke: Option<&inazuma::Keystroke>,
        _window: &Window,
        cx: &mut Context<Editor>,
    ) -> Option<AnyElement> {
                ...
                h_flex()
                    ...
                    .on_click(cx.listener(|this, _event, window, cx| {
                        ...
                        window.dispatch_action(
                            carrot_actions::OpenZedPredictOnboarding.boxed_clone(),
                            cx,
                        );
                    }))
                    .child(
                        h_flex()
                            .flex_1()
                            .gap_2()
                            .child(Icon::new(IconName::CarrotPredict))
                        ...
                        h_flex()
                            ...
                            .child(div().px_1p5().child(match &prediction.completion {
                                InlineCompletion::Move { target, snapshot } => {
                                    ...
                                    {
                                        Icon::new(IconName::CarrotPredictDown)
                                    } else {
                                        Icon::new(IconName::CarrotPredictUp)
                                    }
                                }
                                InlineCompletion::Edit { .. } => Icon::new(IconName::CarrotPredict),
                            }))
                            .child(
                                h_flex()
                                    .gap_1()
    ...
    fn render_edit_prediction_cursor_popover_preview(
        ...
        match &completion.completion {
            ...
            InlineCompletion::Edit {
                ...
                let styled_text = inazuma::StyledText::new(highlighted_edits.text)
                ...
                let left = if first_edit_row != cursor_point.row {
                    ...
                    Icon::new(IconName::CarrotPredict).into_any_element()
                };
    ...
    fn set_breakpoint_context_menu(
        ...
        clicked_point: inazuma::Point<Pixels>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if !cx.has_flag::<DebuggerFeatureFlag>() {
                ...
                self.selection_history.transaction(transaction_id).cloned()
            {
                ...
                log::error!(
                    "No entry in selection_history found for undo. \
                     ...
                     https://github.com/carrot-rs/carrot/issues/22692"
                );
            }
            self.request_autoscroll(Autoscroll::fit(), cx);
    ...
    pub fn expand_excerpt(
        ...
        if should_scroll_up {
            let new_scroll_position =
                current_scroll_position + inazuma::Point::new(0.0, lines_to_expand as f32);
    ...
    pub fn display_to_pixel_point(
        ...
        Some(inazuma::Point::new(source_x, source_y))
    }
    ...
    fn character_size(&self, window: &mut Window) -> inazuma::Size<Pixels> {
        let text_layout_details = self.text_layout_details(window);
        ...
        inazuma::Size::new(em_width, line_height)
    }
...
impl EditorSnapshot {
    ...
    pub fn scroll_position(&self) -> inazuma::Point<f32> {
...
impl EntityInputHandler for Editor {
    ...
    fn bounds_for_range(
        &mut self,
        range_utf16: Range<usize>,
        element_bounds: inazuma::Bounds<Pixels>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Option<inazuma::Bounds<Pixels>> {
        let text_layout_details = self.text_layout_details(window);
        let inazuma::Size {
            width: em_width,
            height: line_height,
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("open-keymap", "Assign Keybinding").size(ButtonSize::Compact).on_click(|_ev, window, cx| {
                            window.dispatch_action(carrot_actions::OpenKeymap.boxed_clone(), cx)
                        }))
                        .child(Button::new("see-docs", "See Docs").size(ButtonSize::Compact).on_click(|_ev, _window, cx| {
                            cx.open_url("https://carrot.dev/docs/completions#edit-predictions-missing-keybinding");
                        })),
                )
        })
    }
...
pub struct LineHighlight {
    ...
    pub border: Option<inazuma::Hsla>,
    pub include_gutter: bool,
