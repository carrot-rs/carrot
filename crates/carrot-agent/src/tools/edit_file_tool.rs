...
use carrot_language::{LanguageRegistry, ToPoint};
use carrot_language_model::{CompletionIntent, LanguageModelToolResultContent};
use carrot_project::lsp_store::{FormatTrigger, LspFormatTarget};
use carrot_project::{Project, ProjectPath};
...
mod tests {
        ...
        let _auth = cx.update(|cx| {
            tool.authorize(
                &EditFileToolInput {
                    ...
                    path: ".carrot/settings.json".into(),
                    mode: EditFileMode::Edit,
                },
                &stream_tx,
                cx,
        ...
        let _auth = cx.update(|cx| {
            tool.authorize(
                &EditFileToolInput {
                    ...
                    path: "root/.carrot/tasks.toml".into(),
                    mode: EditFileMode::Edit,
                },
                &stream_tx,
                cx,
            ...
            let _auth = cx.update(|cx| {
                tool.authorize(
                    &EditFileToolInput {
                        ...
                        path: "project/.carrot/settings.json".into(),
                        mode: mode.clone(),
                    },
                    &stream_tx,
                    cx,
    ...
    async fn test_sensitive_settings_kind_detects_nonexistent_subdirectory(
        ...
        let fs = carrot_project::FakeFs::new(cx.executor());
        let config_dir = carrot_paths::config_dir();
