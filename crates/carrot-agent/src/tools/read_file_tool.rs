...
use carrot_language_model::{LanguageModelImage, LanguageModelToolResultContent};
use carrot_project::{AgentLocation, ImageItem, Project, WorktreeSettings, image_store};
use futures::FutureExt as _;
use inazuma::{App, Entity, SharedString, Task};
use inazuma_settings_framework::Settings;
use inazuma_util::markdown::MarkdownCodeBlock;
use indoc::formatdoc;
...
mod test {
    ...
    async fn test_read_large_file(cx: &mut TestAppContext) {
        ...
        language_registry.add(carrot_language::rust_lang());
        let action_log = cx.new(|_| ActionLog::new(project.clone()));
        let tool = Arc::new(ReadFileTool::new(project, action_log, true));
        let result = cx
            .update(|cx| {
