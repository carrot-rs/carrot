...
use anyhow::Result;
use carrot_context_server::{ContextServerId, client::NotificationSubscription};
use carrot_project::context_server_store::{ContextServerStatus, ContextServerStore};
use futures::FutureExt as _;
use inazuma::{App, AppContext, AsyncApp, Context, Entity, EventEmitter, SharedString, Task};
...
pub struct ContextServerPrompt {
    ...
    pub prompt: carrot_context_server::types::Prompt,
}
...
impl ContextServerRegistry {
    ...
    fn init_registered_server(
        ...
        let tools_updated_subscription = server_store
            ...
            .and_then(|server| {
                ...
                if !client.capable(carrot_context_server::protocol::ServerCapability::Tools) {
                    return None;
                }
    ...
    fn reload_tools_for_server(&mut self, server_id: ContextServerId, cx: &mut Context<Self>) {
        ...
        registered_server.load_tools = cx.spawn(async move |this, cx| {
            let response = client
                .request::<carrot_context_server::types::requests::ListTools>(())
                .await;
    ...
    fn reload_prompts_for_server(&mut self, server_id: ContextServerId, cx: &mut Context<Self>) {
        ...
        if !client.capable(carrot_context_server::protocol::ServerCapability::Prompts) {
            return;
        }
...
struct ContextServerTool {
    ...
    server_id: ContextServerId,
    tool: carrot_context_server::types::Tool,
}
...
impl AnyAgentTool for ContextServerTool {
    ...
    fn input_schema(
        &self,
        format: carrot_language_model::LanguageModelToolSchemaFormat,
    ) -> Result<serde_json::Value> {
        let mut schema = self.tool.input_schema.clone();
        carrot_language_model::tool_schema::adapt_schema_to_format(&mut schema, format)?;
        Ok(match schema {
    ...
    fn run(
        ...
        cx.spawn(async move |_cx| {
            ...
            let request = protocol.request::<carrot_context_server::types::requests::CallTool>(
            ...
            for content in response.content {
                match content {
                    carrot_context_server::types::ToolResponseContent::Text { text } => {
                        result.push_str(&text);
                    }
                    carrot_context_server::types::ToolResponseContent::Image { .. } => {
                        log::warn!("Ignoring image content from tool response");
                    }
                    carrot_context_server::types::ToolResponseContent::Audio { .. } => {
                        log::warn!("Ignoring audio content from tool response");
                    }
                    carrot_context_server::types::ToolResponseContent::Resource { .. } => {
...
pub fn get_prompt(
    ...
    cx: &mut AsyncApp,
) -> Task<Result<carrot_context_server::types::PromptsGetResponse>> {
    ...
    cx.background_spawn(async move {
        let response = protocol
            .request::<carrot_context_server::types::requests::PromptsGet>(
                carrot_context_server::types::PromptsGetParams {
                    name: prompt_name,
