...
use inazuma::{App, Entity, SharedString, Task};
...
mod tests {
    ...
    async fn test_run_rejects_invalid_substitution_before_terminal_creation(
        ...
        let project = carrot_project::Project::test(fs, ["/root".as_ref()], cx).await;
        ...
        assert!(
            !matches!(
                ...
                Ok(Ok(crate::ThreadEvent::ToolCallUpdate(
                    carrot_acp_thread::ToolCallUpdate::UpdateFields(_)
                )))
            ),
    ...
    #[inazuma::test]
    async fn test_run_env_prefixed_allow_pattern_is_used_end_to_end(
        cx: &mut inazuma::TestAppContext,
    ) {
        crate::tests::init_test(cx);
        ...
        cx.update(|cx| {
            let mut settings = carrot_agent_settings::AgentSettings::get_global(cx).clone();
            settings.tool_permissions.default =
                inazuma_settings_framework::ToolPermissionMode::Deny;
            settings.tool_permissions.tools.insert(
                ...
                carrot_agent_settings::ToolRules {
                    ...
                    always_allow: vec![
                        carrot_agent_settings::CompiledRegex::new(
                            r"^PAGER=blah\s+git\s+log(\s|$)",
                            false,
                        )
                        .unwrap(),
                    ],
        ...
        cx.update(|cx| {
            ...
            settings.tool_permissions.tools.insert(
                ...
                carrot_agent_settings::ToolRules {
                    ...
                    always_allow: vec![
                        carrot_agent_settings::CompiledRegex::new(r"^git\b", false).unwrap(),
                    ],
                    always_deny: vec![],
                    always_confirm: vec![],
                    invalid_patterns: vec![],
    ...
    fn test_terminal_tool_input_schema_mentions_forbidden_substitutions() {
        let schema = <TerminalTool as crate::AgentTool>::input_schema(
            carrot_language_model::LanguageModelToolSchemaFormat::JsonSchema,
        );
    ...
    #[inazuma::test]
    async fn test_rejects_variable_expansion(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo ${HOME}", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_positional_parameter(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $1", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_special_parameter_question(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $?", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_special_parameter_dollar(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $$", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_special_parameter_at(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $@", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_command_substitution_dollar_parens(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $(whoami)", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_command_substitution_backticks(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo `whoami`", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_arithmetic_expansion(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("echo $((1 + 1))", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_process_substitution_input(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("cat <(ls)", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_process_substitution_output(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("ls >(cat)", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_env_prefix_with_variable(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("PAGER=$HOME git log", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_env_prefix_with_command_substitution(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("PAGER=$(whoami) git log", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_env_prefix_with_brace_expansion(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
    ...
    #[inazuma::test]
    async fn test_rejects_multiline_with_forbidden_on_second_line(
        cx: &mut inazuma::TestAppContext,
    ) {
        crate::tests::init_test(cx);
    ...
    #[inazuma::test]
    async fn test_rejects_multiline_with_forbidden_mixed(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
        assert_rejected_before_terminal_creation("PAGER=less git log\necho $(whoami)", cx).await;
    }

    #[inazuma::test]
    async fn test_rejects_nested_command_substitution(cx: &mut inazuma::TestAppContext) {
        crate::tests::init_test(cx);
