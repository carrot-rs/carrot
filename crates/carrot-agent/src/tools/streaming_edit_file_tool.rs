...
use carrot_acp_thread::Diff;
use carrot_action_log::ActionLog;
use carrot_language::language_settings::{self, FormatOnSave};
use carrot_language::{Buffer, LanguageRegistry};
use carrot_language_model::LanguageModelToolResultContent;
use carrot_project::lsp_store::{FormatTrigger, LspFormatTarget};
use carrot_project::{AgentLocation, Project, ProjectPath};
use carrot_streaming_diff::{CharOperation, StreamingDiff};
use carrot_ui::SharedString;
use futures::FutureExt as _;
use inazuma::{App, AppContext, AsyncApp, Entity, Task, WeakEntity};
use inazuma_collections::HashSet;
use inazuma_text::ToOffset;
use inazuma_util::rel_path::RelPath;
use inazuma_util::{Deferred, ResultExt};
        ...
        let (new_text, unified_diff) = cx
            .background_spawn({
                ...
                async move {
                    ...
                    let diff = carrot_language::unified_diff(&old_text, &new_text);
                    (new_text, diff)
                }
            })
            .await;
...
mod tests {
    ...
    async fn test_streaming_format_on_save(cx: &mut TestAppContext) {
        ...
        let rust_language = Arc::new(carrot_language::Language::new(
        ...
        let mut fake_language_servers = language_registry.register_fake_lsp(
            ...
            carrot_language::FakeLspAdapter {
                capabilities: carrot_lsp::ServerCapabilities {
                    document_formatting_provider: Some(carrot_lsp::OneOf::Left(true)),
                    ..Default::default()
                },
                ..Default::default()
            },
        ...
        fs.save(
            ...
            carrot_language::LineEnding::Unix,
        )
        .await
        .unwrap();
...
";
        ...
        fake_language_server.set_request_handler::<carrot_lsp::request::Formatting, _, _>({
            |_, _| async move {
                Ok(Some(vec![carrot_lsp::TextEdit {
            ...
            SettingsStore::update_global(cx, |store, cx| {
                store.update_user_settings(cx, |settings| {
                    ...
                    settings.project.all_languages.defaults.formatter =
                        Some(carrot_language::language_settings::FormatterList::default());
                });
            });
        });
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        // Test 1: Path with .carrot component should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            tool.authorize(
                &PathBuf::from(".carrot/settings.json"),
                "test 1",
                &stream_tx,
                cx,
            )
        ...
        // Test 3: Relative path without .carrot should not require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        cx.update(|cx| {
        ...
        // Test 4: Path with .carrot in the middle should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            tool.authorize(
                &PathBuf::from("root/.carrot/tasks.toml"),
                "test 4",
                &stream_tx,
                cx,
            )
        ...
        // 5.1: .carrot/settings.json is a sensitive path — still prompts
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            tool.authorize(
                &PathBuf::from(".carrot/settings.json"),
        ...
        cx.update(|cx| {
            let mut settings = carrot_agent_settings::AgentSettings::get_global(cx).clone();
            settings.tool_permissions.default =
                inazuma_settings_framework::ToolPermissionMode::Confirm;
            carrot_agent_settings::AgentSettings::override_global(settings, cx);
        });
    ...
    async fn test_streaming_needs_confirmation_with_multiple_worktrees(cx: &mut TestAppContext) {
        ...
        let test_cases = vec![
            ...
            (
                "shared/.carrot/settings.json",
                true,
                ".carrot file in third worktree",
            ),
    ...
    async fn test_streaming_needs_confirmation_with_different_modes(cx: &mut TestAppContext) {
        ...
        for _mode in modes {
            // Test .carrot path with different modes
            let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
            let _auth = cx.update(|cx| {
                tool.authorize(
                    &PathBuf::from("project/.carrot/settings.json"),
                    "Edit settings",
                    &stream_tx,
                    cx,
                )
    ...
    async fn setup_test_with_fs(
        cx: &mut TestAppContext,
        fs: Arc<carrot_project::FakeFs>,
        worktree_paths: &[&std::path::Path],
    ) -> (
        Arc<StreamingEditFileTool>,
        Entity<Project>,
        Entity<ActionLog>,
        Arc<carrot_project::FakeFs>,
        Entity<Thread>,
    ) {
