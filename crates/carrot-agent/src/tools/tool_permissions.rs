...
pub async fn canonicalize_worktree_roots<C: inazuma::AppContext>(
...
/// `.carrot/settings.json` where `.carrot` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.carrot`
/// makes `.carrot` an intermediate symlink directory.
...
/// either inside a `.carrot/` local-settings directory or inside the global config dir.
pub async fn sensitive_settings_kind(path: &Path, fs: &dyn Fs) -> Option<SensitiveSettingsKind> {
    let local_settings_folder = carrot_paths::local_settings_folder_name();
    ...
    if let Some(canonical_path) = canonicalize_with_ancestors(path, fs).await {
        let config_dir = fs
            .canonicalize(carrot_paths::config_dir())
            .await
            .unwrap_or_else(|_| carrot_paths::config_dir().to_path_buf());
