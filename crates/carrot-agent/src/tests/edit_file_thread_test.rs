...
async fn test_edit_file_tool_in_thread_context(cx: &mut TestAppContext) {
    ...
    let project = carrot_project::Project::test(fs.clone(), [path!("/project").as_ref()], cx).await;
    ...
    assert!(
        file_content.contains("Hello, Carrot!"),
    ...
    let has_tool_result = last_request.messages.iter().any(|m| {
        m.content
            .iter()
            .any(|c| matches!(c, carrot_language_model::MessageContent::ToolResult(_)))
    });
    assert!(
        has_tool_result,
