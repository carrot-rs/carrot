...
use inazuma_settings_framework::{ActiveSettingsProfileName, SettingsStore};

pub fn init(cx: &mut App) {
    cx.on_action(
        |_: &carrot_actions::settings_profile_selector::Toggle, cx| {
...
mod tests {
    use super::*;
    use carrot_actions::settings_profile_selector;
    use carrot_editor;
    use carrot_project::{FakeFs, Project};
    use carrot_theme_settings::ThemeSettings;
    use carrot_workspace::{self, AppState, Workspace};
    use inazuma::{TestAppContext, UpdateGlobal, VisualTestContext};
    use inazuma_menu::{Cancel, Confirm, SelectNext, SelectPrevious};
