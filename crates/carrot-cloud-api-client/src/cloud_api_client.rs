...
use carrot_cloud_api_types::websocket_protocol::{PROTOCOL_VERSION, PROTOCOL_VERSION_HEADER_NAME};
pub use carrot_cloud_api_types::*;
use carrot_http_client::http::request;
use carrot_http_client::{
...
impl CloudApiClient {
    ...
    pub async fn get_authenticated_user(
        ...
        let request = self.build_request(
            Request::builder().method(Method::GET).uri(
                self.http_client
                    .build_carrot_cloud_url("/client/users/me")?
                    .as_ref(),
            ),
            AsyncBody::default(),
        )?;
    ...
    pub fn connect(&self, cx: &App) -> Result<Task<Result<Connection>>> {
        let mut connect_url = self
            .http_client
            .build_carrot_cloud_url("/client/users/connect")?;
        connect_url
    ...
    pub async fn create_llm_token(
        ...
        let request_builder = Request::builder()
            ...
            .uri(
                self.http_client
                    .build_carrot_cloud_url("/client/llm_tokens")?
                    .as_ref(),
            )
            .when_some(system_id, |builder, system_id| {
                builder.header(CARROT_SYSTEM_ID_HEADER_NAME, system_id)
            });
    ...
    pub async fn submit_agent_feedback(&self, body: SubmitAgentThreadFeedbackBody) -> Result<()> {
        let request = self.build_request(
            Request::builder().method(Method::POST).uri(
                self.http_client
                    .build_carrot_cloud_url("/client/feedback/agent_thread")?
                    .as_ref(),
            ),
    ...
    pub async fn submit_agent_feedback_comments(
        ...
        let request = self.build_request(
            Request::builder().method(Method::POST).uri(
                self.http_client
                    .build_carrot_cloud_url("/client/feedback/agent_thread_comments")?
                    .as_ref(),
            ),
            AsyncBody::from(serde_json::to_string(&body)?),
        )?;
    ...
    pub async fn submit_edit_prediction_feedback(
        ...
        let request = self.build_request(
            Request::builder().method(Method::POST).uri(
                self.http_client
                    .build_carrot_cloud_url("/client/feedback/edit_prediction")?
                    .as_ref(),
            ),
            AsyncBody::from(serde_json::to_string(&body)?),
        )?;
