...
use carrot_project::{Project, Symbol, lsp_store::SymbolLocation};
    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _: &mut Context<Workspace>| {
            workspace.register_action(
                |workspace, _: &carrot_workspace::ToggleProjectSymbols, window, cx| {
...
impl PickerDelegate for ProjectSymbolsDelegate {
    ...
    fn render_match(
        ...
        let highlights = inazuma::combine_highlights(custom_highlights, syntax_runs);
...
mod tests {
    use super::*;
    use carrot_language::{FakeLspAdapter, Language, LanguageConfig, LanguageMatcher};
    use carrot_lsp::OneOf;
    use carrot_project::FakeFs;
    use carrot_workspace::Workspace;
    use futures::StreamExt;
    use inazuma::{TestAppContext, VisualContext};
    ...
    async fn test_project_symbols(cx: &mut TestAppContext) {
        ...
        fake_server.set_request_handler::<carrot_lsp::WorkspaceSymbolRequest, _, _>(
            move |params: carrot_lsp::WorkspaceSymbolParams, cx| {
                ...
                async move {
                    ...
                    Ok(Some(carrot_lsp::WorkspaceSymbolResponse::Flat(
                        matches
                            .into_iter()
        ...
        // https://github.com/carrot-rs/carrot/issues/861
    ...
    fn symbol(name: &str, path: impl AsRef<Path>) -> carrot_lsp::SymbolInformation {
        #[allow(deprecated)]
        carrot_lsp::SymbolInformation {
            ...
            location: carrot_lsp::Location::new(
                carrot_lsp::Uri::from_file_path(path.as_ref()).unwrap(),
