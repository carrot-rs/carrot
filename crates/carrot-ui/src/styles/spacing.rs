use carrot_theme::UiDensity;
use carrot_ui_macros::derive_dynamic_spacing;
use inazuma::{App, Pixels, Rems, px, rems};

// Derives [DynamicSpacing]. See [carrot_ui_macros::derive_dynamic_spacing].
//
...
pub fn ui_density(cx: &mut App) -> UiDensity {
    carrot_theme::theme_settings(cx).ui_density(cx)
}
