use crate::prelude::*;
use inazuma::{FontWeight, Rems, StyleRefinement, UnderlineStyle};
...
impl LabelCommon for LabelLike {
    ...
    fn buffer_font(mut self, cx: &App) -> Self {
        let font = carrot_theme::theme_settings(cx).buffer_font(cx).clone();
    ...
    fn inline_code(mut self, cx: &App) -> Self {
        self.base = self
            .base
            .font(carrot_theme::theme_settings(cx).buffer_font(cx).clone())
...
impl RenderOnce for LabelLike {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        self.base
            ...
            .when(!self.color.is_inherit(), |this| this.text_color(color))
            .font_weight(
                self.weight
                    .unwrap_or(carrot_theme::theme_settings(cx).ui_font(cx).weight),
            )
