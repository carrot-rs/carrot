use crate::prelude::*;
use inazuma::{AnyElement, IntoElement, ParentElement, Styled};
...
/// use carrot_ui::{Banner, Button, Icon, IconName, IconSize, Label, Severity};
///
/// Banner::new()
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (icon, icon_color, bg_color, border_color) = match self.severity {
            Severity::Info => (
                IconName::Info,
                Color::Muted,
                cx.theme().status().info.background.opacity(0.5),
                cx.theme().colors().border.opacity(0.5),
            ),
            Severity::Success => (
                IconName::Check,
                Color::Success,
                cx.theme().status().success.color.opacity(0.1),
                cx.theme().status().success.color.opacity(0.2),
            ),
            Severity::Warning => (
                IconName::Warning,
                Color::Warning,
                cx.theme().status().warning.background.opacity(0.5),
                cx.theme().status().warning.border.opacity(0.4),
            ),
            Severity::Error => (
                IconName::XCircle,
                Color::Error,
                cx.theme().status().error.color.opacity(0.1),
                cx.theme().status().error.color.opacity(0.2),
            ),
        };
