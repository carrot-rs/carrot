...
use inazuma::{AnimationElement, AnyElement, IntoElement, Oklch, Rems, Transformation, img, svg};
...
#[derive(Clone)]
enum IconSource {
    /// An SVG embedded in the Carrot binary.
    ...
    External(Arc<Path>),
    /// An SVG not embedded in the Carrot binary.
...
impl Icon {
    ...
    /// Sets the icon size to `1.25rem` (20px).
    pub fn size_5(mut self) -> Self {
        self.size = rems(1.25);
        self
    }

    /// Sets the icon size to `1.5rem` (24px).
