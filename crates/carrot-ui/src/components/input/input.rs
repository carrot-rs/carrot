...
use inazuma::{
    ...
    IntoElement, IsZero, MouseButton, Oklch, ParentElement as _, Rems, RenderOnce, StyleRefinement,
    ...
    if disabled {
        (
            cx.theme()
                .colors()
                .input
                .mix_oklab(Oklch::transparent_black(), 0.8),
            cx.theme().colors().muted_foreground,
        )
    } else {
        (cx.theme().colors().input, cx.theme().colors().foreground)
    }
}
            ...
            .when(self.appearance, |this| {
                this.bg(bg)
                    ...
                    .when(self.bordered, |this| {
                        this.border_color(cx.theme().colors().input)
                            .border_1()
                ...
                this.pr(self.size.input_px()).child(
                    h_flex()
                        ...
                        .when(state.loading, |this| {
                            this.child(Spinner::new().color(cx.theme().colors().muted_foreground))
                        })
