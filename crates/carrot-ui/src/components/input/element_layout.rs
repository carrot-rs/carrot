...
use inazuma::{
    App, Bounds, Corners, Half, HighlightStyle, HitboxBehavior, InteractiveElement, IntoElement,
    MouseButton, Oklch, Path, Pixels, Point, ShapedLine, SharedString, Styled as _, TextAlign,
    TextRun, TextStyle, Window, point, px, size,
};
use ropey::Rope;
            ...
            let range_styles = if skip {
            ...
            } else {
                {
                    let ht = if cx.theme().is_dark() {
                        carrot_syntax::HighlightTheme::default_dark()
                    } else {
                        carrot_syntax::HighlightTheme::default_light()
                    };
    ...
    fn test_split_runs_by_bg_segments() {
        ...
        for idx in 1..5 {
            ...
            assert!(
                ...
                "segment {} should have black or white text, got {:?}",
                idx,
                c
            );
        }
