...
/// Dark gray popover background for the completion menu.
const POPOVER_BG: inazuma::Oklch = inazuma::Oklch {
    l: 0.248,
    c: 0.0,
    h: 0.0,
    a: 1.0,
...
const POPOVER_BORDER: inazuma::Oklch = inazuma::Oklch {
    l: 0.318,
    c: 0.0,
    h: 0.0,
    a: 1.0,
...
use crate::{
    ActiveTheme, Icon, IconName, IndexPath, InteractiveList, ListDelegate, ListEvent, ListState,
    Selectable, actions, h_flex,
    input::{self, InputState, RopeExt},
};
...
impl RenderOnce for CompletionMenuItem {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        h_flex()
            ...
            .child(Icon::new(icon).text_color(cx.theme().colors().muted_foreground))
            .child(
                div()
                    .flex_shrink_0()
                    .whitespace_nowrap()
    ...
    fn select_item(&mut self, item: &CompletionItem, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn_in(window, async move |_, cx| {
            editor.update_in(cx, |editor, window, cx| {
                ...
                editor.completion_inserted_range =
                            ...
                            div()
                                ...
                                .child(
                                    div()
                                        ...
                                        .child(InteractiveList::new(&self.list)),
                                ),
                        )
                        .children(doc_panel)
