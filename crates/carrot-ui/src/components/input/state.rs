...
//! https://github.com/carrot-rs/carrot/blob/main/crates/gpui/examples/input.rs
use anyhow::Result;
use inazuma::{
    Action, App, AppContext, Bounds, Context, Entity, EntityInputHandler, EventEmitter,
    FocusHandle, Focusable, InteractiveElement as _, IntoElement, KeyDownEvent, MouseMoveEvent,
...
use carrot_syntax::DiagnosticSet;
#[cfg(not(target_family = "wasm"))]
use carrot_syntax::LanguageRegistry;

#[derive(Action, Clone, Default, PartialEq, Eq, Deserialize, schemars::JsonSchema)]
...
pub(crate) fn init(cx: &mut App) {
    // Editing keybindings (Backspace, Delete, Cmd+A, arrows, …) live in
    // the default keymap TOML files under `context = "Input"`. They used
    // to be hard-coded here, which broke whenever `cx.clear_key_bindings`
    // ran during a keymap reload.
    search::init(cx);
...
impl InputState {
    ...
    pub(super) fn update_fold_candidates_incremental(
        &mut self,
            ...
            let result = cx
                ...
                .spawn(async move {
                    ...
                    let injection_layers = if let Some(data) = injection_data {
                        carrot_syntax::SyntaxHighlighter::compute_injection_layers(
                            data, &new_tree, &text,
                        )
                    } else {
                        Default::default()
...
mod tests {
    ...
    impl inazuma::Render for TestRootView {
        fn render(
            &mut self,
    ...
    struct InputView {
        ...
        window_handle: inazuma::WindowHandle<TestRootView>,
    }
    ...
    fn test_highlighting_preserved_after_fold(cx: &mut TestAppContext) {
        ...
        use carrot_syntax::HighlightTheme;
