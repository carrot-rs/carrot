use inazuma::{App, Bounds, Oklch, Pixels, Point, Size, TextAlign, Window, fill, point, px, size};

use carrot_theme::ActiveTheme as _;

use super::element::{LINE_NUMBER_RIGHT_MARGIN, PrepaintState, TextElement};
...
impl TextElement {
    ...
    pub(super) fn paint_selections_and_highlights(
        ...
        for (path, is_active) in prepaint.search_match_paths.iter() {
            ...
            if *is_active {
                window.paint_path(path.clone(), selection_color);
            }
        }
            ...
            .zip(prepaint.last_layout.visible_buffer_lines.iter())
        {
            ...
            if has_ghost_lines && Some(row) == prepaint.current_row {
                ...
                for ghost_line in ghost_lines {
                    ...
                    window.paint_quad(fill(ghost_bounds, cx.theme().colors().editor.background));
    ...
    pub(super) fn paint_inline_completion_suffix(
        ...
        window.paint_quad(fill(bg_bounds, cx.theme().colors().editor.background));
