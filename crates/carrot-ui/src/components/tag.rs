use crate::{ActiveTheme as _, ColorName, Sizable, Size, StyledExt};
use inazuma::{
    AbsoluteLength, AnyElement, App, InteractiveElement as _, IntoElement, Oklch, ParentElement,
...
impl TagVariant {
    ...
    fn border(&self, cx: &App) -> Oklch {
        match self {
            Self::Primary => cx.theme().colors().primary,
            Self::Secondary => cx.theme().colors().border,
            Self::Danger => cx.theme().status().error.color,
            Self::Success => cx.theme().status().success.color,
            Self::Warning => cx.theme().status().warning.color,
            Self::Info => cx.theme().status().info.color,
...
impl RenderOnce for Tag {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let rounded = self.rounded.unwrap_or(
            match self.size {
                Size::XSmall | Size::Small => cx.theme().colors().radius / 2.,
                _ => cx.theme().colors().radius,
            }
            .into(),
        );
