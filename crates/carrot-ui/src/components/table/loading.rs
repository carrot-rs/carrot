use crate::{ActiveTheme, Size, Skeleton, h_flex, v_flex};
use inazuma::{IntoElement, ParentElement as _, RenderOnce, Styled, prelude::FluentBuilder as _};
...
impl RenderOnce for LoadingRow {
    fn render(self, _: &mut inazuma::Window, cx: &mut inazuma::App) -> impl IntoElement {
        ...
        h_flex()
            ...
            .justify_between()
            .overflow_hidden()
            .when(self.header, |this| this.bg(cx.theme().colors().surface))
...
impl RenderOnce for Loading {
    fn render(self, _window: &mut inazuma::Window, _cx: &mut inazuma::App) -> impl IntoElement {
        v_flex()
            .gap_0()
