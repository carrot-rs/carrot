...
use crate::{
    ActiveTheme, ElementExt, Icon, IconName, Scrollbar, StyleSized as _, StyledExt, h_flex,
};
use inazuma::{
    ...
    D: TableDelegate,
{
            ...
            .child(
                div()
                    ...
                    .group_hover(&group_id, |this| {
                        this.bg(cx.theme().colors().border).h_full()
                    })
                    .w(px(1.)),
            )
            .on_drag_move(
    ...
    pub(super) fn render_sort_icon(
        ...
        let (icon, is_on) = match sort {
            ColumnSort::Ascending => (IconName::ArrowUp, true),
            ColumnSort::Descending => (IconName::ArrowDown, true),
        ...
        Some(
            div()
                .id(("icon-sort", col_ix))
                .p(px(2.))
                .rounded(cx.theme().colors().radius / 2.)
                .map(|this| match is_on {
                    true => this,
                    false => this.opacity(0.5),
                })
                .hover(|this| this.bg(cx.theme().colors().secondary).opacity(7.))
                .active(|this| this.bg(cx.theme().colors().element_active).opacity(1.))
                .on_click(
            ...
            tr.h_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .relative()
                        .child(
                            crate::utils::virtual_list::virtual_list(
                                view,
                                row_ix,
                ...
                .when(self.right_clicked_row == Some(row_ix), |this| {
                    this.border_color(inazuma::transparent_white()).child(
                        div()
                            ...
                            .absolute()
                            .border_1()
                            .border_color(cx.theme().colors().element_selection),
                    )
                })
                .on_mouse_down(
                    MouseButton::Right,
