use crate::{
    IconButtonShape, KeyBinding, List, ListItem, ListItemSpacing, ListSeparator, ListSubHeader,
...
impl ContextMenu {
    ...
    /// Like `custom_entry`, but the row has no hover highlight and is not
    /// keyboard-selectable.  Useful for inline widgets (color pickers, etc.)
    /// where individual children handle their own interaction.
    pub fn custom_entry_static(
        mut self,
        entry_render: impl Fn(&mut Window, &mut App) -> AnyElement + 'static,
    ) -> Self {
        self.items.push(ContextMenuItem::CustomEntry {
...
impl Render for ContextMenu {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        // Compact density: scale down to 85% of the UI font for a tight,
        // terminal-friendly context menu appearance.
        let ui_font_size = carrot_theme::theme_settings(cx).ui_font_size(cx) * 0.85;
        ...
        let render_aside = |aside: DocumentationAside, cx: &mut Context<Self>| {
            WithRemSize::new(ui_font_size)
                ...
                .shadow(crate::ElevationIndex::ElevatedSurface.shadow(cx))
                .w_full()
                .p_2()
                .overflow_hidden()
