...
use carrot_i18n::t;
use inazuma::{
    ElementId, InteractiveElement, RenderOnce, StyleRefinement, Window, prelude::FluentBuilder,
};

use crate::prelude::*;
use crate::{Disableable, PopupMenuItem, StyledExt, Tooltip};
...
impl Pagination {
    ...
    fn render_nav_button(&self, is_prev: bool) -> Button {
        ...
        Button::with_id(id)
            ...
            .when(self.compact, |this: Button| this.icon(icon))
            .when(!self.compact, |this: Button| {
                this.child(
                    h_flex()
                        .w_full()
                        .gap_2()
            ...
            .when_some(self.on_click.clone(), |this: Button, handler| {
        ...
        h_flex()
            ...
            .children({
                page_numbers.into_iter().map(|item| match item {
                    ...
                    PageItem::Ellipsis(range) => Button::with_id(SharedString::from(format!(
                        "ellipsis-{}-{}",
