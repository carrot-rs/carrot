...
use crate::{InputEvent, InputState, SliderEvent, SliderState, actions::Confirm};
            ...
            cx.subscribe_in(
                ...
                |this, state, ev: &InputEvent, window, cx| match ev {
                    InputEvent::Change => {
                        ...
                        if let Ok(color) = carrot_theme::parse_color(value.as_str()) {
                    ...
                    InputEvent::PressEnter { .. } => {
                        ...
                        if let Ok(color) = carrot_theme::parse_color(&val) {
                            this.open = false;
