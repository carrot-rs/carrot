...
use crate::{
    ...
    IconName, LabelCommon as _, RegisterComponent, Selectable, Sizable, Size, StyledExt as _, Text,
    ToggleState, example_group_with_title, single_example, v_flex,
};
use inazuma::{
        ...
        .map(|this| {
            if !disabled && show_icon != *anim_state.read(cx) {
                ...
                this.with_animation(
                    ...
                    move |this, delta| this.opacity(if show_icon { delta } else { 1.0 - delta }),
                )
                .into_any_element()
            } else {
...
impl RenderOnce for Checkbox {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let has_label =
            self.label.is_some() || self.label_text.is_some() || !self.children.is_empty();
            ...
            self.base
                ...
                .child(
                    div()
                        ...
                        .border_color(color)
                        .rounded(radius)
                        .when(cx.theme().is_dark() && !self.disabled, |this| {
                            this.shadow_xs()
                        })
                        .map(|this| {
...
impl Component for Checkbox {
    ...
    fn description() -> Option<&'static str> {
        Some(
            "A checkbox with animated check icon, three-state support, and multiple visual styles.",
        )
    }
                        ...
                        vec![
                            ...
                            single_example(
                                ...
                                Checkbox::new("cb_custom")
                                    .toggle_state(ToggleState::Selected)
                                    .style(ToggleStyle::Custom(oklcha(
                                        0.712, 0.1864, 149.8088, 0.7,
                                    )))
                                    .into_any_element(),
                            ),
                        ],
