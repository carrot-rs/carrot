use crate::{ActiveTheme, ElementExt, Icon, IconName, Scrollbar, h_flex, v_flex};

use super::PanelEvent;
use inazuma::{
    AnyElement, App, AppContext, Context, DismissEvent, Div, DragMoveEvent, EntityId, EventEmitter,
    FocusHandle, Focusable, InteractiveElement, IntoElement, MouseButton, MouseDownEvent,
    MouseUpEvent, Oklch, ParentElement, Pixels, Point, Render, Size, StatefulInteractiveElement,
    Styled, Window, div, prelude::FluentBuilder, px, size,
};

use super::Tiles;
use super::tiles::{
    AnyDrag, DRAG_BAR_HEIGHT, DragDrop, DragMoving, DragResizing, HANDLE_SIZE, MINIMUM_SIZE,
    ResizeDrag, ResizeSide, TileChange, round_point_to_nearest_ten,
};
...
impl Tiles {
    ...
    pub(super) fn render_resize_handles(
        ...
        elements.push(
            div()
                .child(
                    div().absolute().right(px(1.)).bottom(px(1.)).child(
                        Icon::new(IconName::Maximize)
                            .size_3()
                            .text_color(cx.theme().colors().muted_foreground.opacity(0.5)),
                    ),
                )
                .child(
                    div()
...
impl Render for Tiles {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let scroll_bounds = self.panels.iter().fold(
            inazuma::Bounds::default(),
            |acc: inazuma::Bounds<Pixels>, item| inazuma::Bounds {
                origin: Point {
