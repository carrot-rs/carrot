use carrot_i18n::t;
use inazuma::{
    AppContext, Context, Corner, DismissEvent, Empty, EventEmitter, Focusable, Render,
    StatefulInteractiveElement, StyleRefinement, Window, div, px, relative, rems,
};

use crate::prelude::*;
use crate::{Placement, Tab, TabBar, Tooltip};

use super::tab_panel::{DragPanel, TabPanel, TabState};
use super::{ClosePanel, DockPlacement, Panel, PanelEvent, PanelStyle, ToggleZoom};
...
impl TabPanel {
    ...
    pub(super) fn render_dock_toggle_button(
        ...
        Some(
            Button::with_id(inazuma::SharedString::from(format!(
                "toggle-dock:{:?}",
                placement
            )))
            .icon(icon)
            ...
            .ghost()
            .tab_stop(false)
            .tooltip(Tooltip::text(if is_open {
                t!("Dock.Collapse")
            } else {
                t!("Dock.Expand")
            }))
