...
use inazuma::{
    ...
    GlobalElementId, Hitbox, HitboxBehavior, InteractiveElement, IntoElement, IsZero, LayoutId,
    ListState, MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Oklch, ParentElement,
...
use inazuma_gpui_util::ResultExt;
...
impl<ScrollHandle: ScrollableHandle> Scrollbars<ScrollHandle> {
    ...
    pub fn with_track_along(mut self, along: ScrollAxes, background_color: Oklch) -> Self {
...
impl<T: ScrollableHandle> ScrollbarState<T> {
    ...
    fn update_track_color(&mut self, track_color: Option<Oklch>) {
...
struct ScrollbarLayout {
    ...
    track_background: Option<(Bounds<Pixels>, Oklch)>,
    axis: ScrollbarAxis,
}
                ...
                } in &prepaint_state.thumbs
                {
                    ...
                    let (thumb_base_color, hovered) = match thumb_state {
                        ThumbState::Dragging(dragged_axis, _) if dragged_axis == axis => {
                            (colors.scrollbar.thumb_active_background, false)
                        }
                        ThumbState::Hover(hovered_axis) if hovered_axis == axis => {
                            (colors.scrollbar.thumb_hover_background, true)
                        }
                        _ => (colors.scrollbar.thumb_background, false),
                    };
                    ...
                    let blending_color = if hovered || reserved_space.needs_scroll_track() {
                        ...
                        let blend_color = colors.surface;
