...
use inazuma::AxisExt;
use inazuma::{
    App, Axis, ElementId, IntoElement, ListState, Oklch, Pixels, Point, ScrollHandle, Size,
...
impl ScrollbarHandle for inazuma::BlockState {
    fn offset(&self) -> Point<Pixels> {
        self.scroll_px_offset_for_scrollbar()
    }
...
impl Scrollbar {
    ...
    pub(super) fn style_for_hovered_thumb(
        cx: &App,
    ) -> (Oklch, Oklch, Oklch, Pixels, Pixels, Pixels) {
        (
            cx.theme().colors().scrollbar.thumb_hover_background,
            cx.theme().colors().scrollbar.track_background,
            cx.theme().colors().border,
