...
//! currently visible content — the same UX pattern as the terminal's
...
use inazuma::{
    ...
    LayoutId, ListState, Pixels, Style, Window, px, size,
};
    ...
    fn find_sticky_item(&self) -> Option<usize> {
        ...
        for ix in 0..self.item_count {
            if let Some(item_bounds) = self.list_state.bounds_for_item(ix) {
                ...
                if item_top < viewport_top && item_bottom > viewport_top + self.header_height {
                    return Some(ix);
                }
            }
        }
...
impl Element for StickyHeader {
    ...
    fn prepaint(
        ...
        let header_element = if let Some(ix) = self.sticky_index {
            ...
            let header_bounds =
                Bounds::new(bounds.origin, size(bounds.size.width, self.header_height));
