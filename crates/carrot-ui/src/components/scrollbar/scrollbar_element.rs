...
use super::scrollbar_core::ScrollbarShow;
use carrot_theme::ActiveTheme;
use inazuma::AxisExt;
use inazuma::{
    App, BorderStyle, Bounds, ContentMask, Corner, CursorStyle, Edges, Element, GlobalElementId,
    HitboxBehavior, InspectorElementId, IsZero, LayoutId, MouseDownEvent, MouseMoveEvent,
    MouseUpEvent, PaintQuad, Pixels, Position, ScrollWheelEvent, Size, Style, Window, fill, point,
    px, size,
};

use super::scrollbar_core::{
    AxisPrepaintState, FADE_OUT_DELAY, FADE_OUT_DURATION, MIN_THUMB_SIZE, PrepaintState, Scrollbar,
                ...
                if state.get().dragged_axis == Some(axis) {
                    ...
                    if let Some(last_time) = state.get().last_scroll_time {
                        ...
                        if is_hovered_on_bar {
                            ...
                            idle_state.0 = cx.theme().colors().scrollbar.thumb_background;
        ...
        window.with_content_mask(
            ...
            |window| {
                for state in prepaint.states.iter() {
                    ...
                    if cx.theme().colors().radius.is_zero() {
                        radius = px(0.);
                    }
