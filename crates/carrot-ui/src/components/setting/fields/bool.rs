...
use crate::{AnySettingField, Checkbox, RenderOptions, Sizable, StyledExt, Switch, ToggleState};
use inazuma::{AnyElement, App, IntoElement, ParentElement as _, StyleRefinement, Window, div};
        ...
        div()
            ...
            .child(if self.use_switch {
                ...
                Checkbox::new("check")
                    .checked(checked)
                    .with_size(options.size)
                    .on_click(move |state: &ToggleState, _window, cx| {
                        set_value(state.selected(), cx);
                    })
                    .into_any_element()
            })
            .into_any_element()
