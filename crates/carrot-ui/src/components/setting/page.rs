...
use crate::{
    ActiveTheme, Button, ButtonVariants, Icon, IconName, Label, LabelCommon as _, RenderOptions,
    ScrollableElement, SettingGroup, Sizable, StyledExt, Tooltip, h_flex, v_flex,
};
            ...
            .child(
                v_flex()
                    ...
                    .child(h_flex().justify_between().child(self.title.clone()).when(
                        ...
                        |this| {
                            this.child(
                                Button::with_id("reset")
                                    .icon(IconName::Undo)
                                    .ghost()
                                    .small()
                                    .tooltip(Tooltip::text(t!("Settings.Reset All")))
                                    .on_click({
                                        let page = self.clone();
                                        move |_, window, cx| {
