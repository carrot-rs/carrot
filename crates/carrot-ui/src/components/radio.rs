...
use crate::{
    ActiveTheme, AxisExt, FocusableExt as _, Sizable, Size, StyledExt, checkbox_check_icon, h_flex,
    text::Text, v_flex,
};
use inazuma::{
...
impl RenderOnce for Radio {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (border_color, bg) = if checked {
            (cx.theme().colors().primary, cx.theme().colors().primary)
        } else {
            (
            ...
            self.base
                ...
                .child(
                    div()
                        ...
                        .when(cx.theme().is_dark() && !disabled, |this| this.shadow_xs())
                        .map(|this| match self.checked {
                            false => this.bg(cx.theme().colors().input),
                            _ => this.bg(bg),
                        })
