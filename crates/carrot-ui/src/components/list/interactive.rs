...
use crate::{
    ActiveTheme, Icon, IconName, IndexPath, ListDelegate, Scrollbar, Selectable, Sizable, Size,
    StyledExt, VirtualListScrollHandle, v_flex, v_virtual_list,
};
use carrot_i18n::t;
use inazuma::{
    App, AvailableSpace, ClickEvent, Context, DefiniteLength, EdgesRefinement, EventEmitter,
    ListSizingBehavior, Pixels, RenderOnce, ScrollStrategy, SharedString,
...
/// Backwards-compatible alias for `InteractiveListState`.
pub type ListState<D> = InteractiveListState<D>;

pub struct InteractiveListState<D: ListDelegate> {
...
impl<D> Focusable for InteractiveListState<D>
where
    D: ListDelegate,
{
...
impl<D> EventEmitter<ListEvent> for InteractiveListState<D> where D: ListDelegate {}
impl<D> Render for InteractiveListState<D>
where
    D: ListDelegate,
{
...
/// The List element.
#[derive(IntoElement)]
pub struct InteractiveList<D: ListDelegate + 'static> {
    state: Entity<InteractiveListState<D>>,
    style: StyleRefinement,
    options: ListOptions,
}

impl<D> InteractiveList<D>
where
    D: ListDelegate + 'static,
{
    /// Create a new InteractiveList element with the given InteractiveListState entity.
    pub fn new(state: &Entity<InteractiveListState<D>>) -> Self {
        Self {
            state: state.clone(),
...
impl<D> Styled for InteractiveList<D>
where
    D: ListDelegate + 'static,
{
...
impl<D> Sizable for InteractiveList<D>
where
    D: ListDelegate + 'static,
{
...
impl<D> RenderOnce for InteractiveList<D>
where
    D: ListDelegate + 'static,
{
