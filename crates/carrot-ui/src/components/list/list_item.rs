...
use inazuma::{
    AnyElement, AnyView, ClickEvent, MouseButton, MouseDownEvent, Pixels, StyleRefinement, Styled,
    px,
};
use inazuma_component_registry::{
    Component, ComponentScope, example_group_with_title, single_example,
};
...
impl RenderOnce for ListItem {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        h_flex()
            ...
            .relative()
            // When an item is inset, indent is on the outer element but
            // horizontal padding moves to the inner child so hover bg
            // fills edge-to-edge.
            .text_color(cx.theme().colors().text)
            .when(self.inset, |this| {
                this.ml(self.indent_level as f32 * self.indent_step_size)
            })
            .when(self.inset && self.selectable && !self.disabled, |this| {
                let hover_bg = cx.theme().colors().ghost_element_hover;
                let contrast_text = if hover_bg.l > 0.5 {
                    inazuma::oklch(0.1, 0.0, 0.0)
                } else {
                    inazuma::oklch(0.95, 0.0, 0.0)
                };
                this.hover(move |style| style.bg(hover_bg).text_color(contrast_text))
            ...
            .when(!self.inset && !self.disabled, |this| {
                ...
                .when(self.selectable, |this| {
                    let hover_bg = cx.theme().colors().ghost_element_hover;
                    let hover_text = if hover_bg.l > 0.5 {
                        inazuma::oklch(0.1, 0.0, 0.0)
                    } else {
                        inazuma::oklch(0.95, 0.0, 0.0)
                    };
                    this.hover(move |style| style.bg(hover_bg).text_color(hover_text))
                        .active(|style| style.bg(cx.theme().colors().ghost_element_active))
