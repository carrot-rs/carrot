...
use crate::{
    Button, ButtonVariant, ButtonVariants, Disableable, Selectable, Sizable, Size, StyledExt,
};
...
mod tests {
    ...
    #[inazuma::test]
    fn test_button_group_builder(_cx: &mut inazuma::TestAppContext) {
        let group = ButtonGroup::new("complex-group")
            .child(Button::new("btn1", "One"))
            .child(Button::new("btn2", "Two"))
            .child(Button::new("btn3", "Three"))
            .primary()
            .large()
            .outline()
            .compact()
