...
use inazuma::{AnyElement, ImageSource, Img, IntoElement, Oklch, Styled, img};
                            ...
                            move || {
                                h_flex()
                                    ...
                                    .map(|el| match (&short_name, &name_for_fallback) {
                                        (Some(initials), Some(_)) => el.child(initials.clone()),
                                        _ => el.child(
                ...
                None => this.child(
                    div()
                        ...
                        .map(|el| match (&self.short_name, &self.name) {
                            ...
                            _ => el.bg(cx.theme().colors().element_disabled).child(
                                Icon::new(IconName::Person)
                                    .color(Color::Muted)
                                    .size(IconSize::Small),
                            ),
