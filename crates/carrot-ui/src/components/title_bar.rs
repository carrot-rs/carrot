...
use inazuma::{
    AnyElement, App, ClickEvent, Context, Decorations, InteractiveElement, IntoElement,
    MouseButton, Oklch, ParentElement, Pixels, Render, RenderOnce, StatefulInteractiveElement as _,
...
impl TitleBar {
    ...
    pub fn title_bar_options() -> TitlebarOptions {
        TitlebarOptions {
            ...
            traffic_light_position: Some(inazuma::point(px(9.0), px(9.0))),
        }
    }
...
impl ControlIcon {
    ...
    fn hover_fg(&self, cx: &App) -> Oklch {
        if self.is_close() {
            ...
            cx.theme().colors().secondary_foreground
        }
    }
    ...
    fn active_bg(&self, cx: &mut App) -> Oklch {
        if self.is_close() {
            cx.theme().status().error.color.opacity(0.7)
        } else {
