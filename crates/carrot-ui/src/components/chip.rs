use crate::prelude::*;
use inazuma::{AnyElement, AnyView, CursorStyle, IntoElement, Oklch, ParentElement, Styled};

/// A compact context chip with optional icon, used in the terminal prompt area.
///
/// Terminal chip design: semi-transparent background, subtle border, colored text.
...
impl RenderOnce for GitBranchChip {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        h_flex()
            ...
            .child(
                div()
                    .text_color(chip.git_branch_text)
                    .child(self.branch.to_string()),
            )
    }
}
...
impl Component for Chip {
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> Option<AnyElement> {
        let chip_examples = vec![
            ...
            single_example("Git Branch", GitBranchChip::new("main").into_any_element()),
            single_example("Git Stats", GitStatsChip::new(5, 42, 13).into_any_element()),
        ];
