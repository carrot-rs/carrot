...
pub struct Link {
    ...
    disabled: bool,
    on_click: Option<Box<dyn Fn(&ClickEvent, &mut inazuma::Window, &mut inazuma::App) + 'static>>,
...
impl Link {
    ...
    pub fn on_click(
        mut self,
        handler: impl Fn(&ClickEvent, &mut inazuma::Window, &mut inazuma::App) + 'static,
    ) -> Self {
...
impl RenderOnce for Link {
    fn render(self, _: &mut inazuma::Window, cx: &mut inazuma::App) -> impl IntoElement {
        ...
        div()
            ...
            .text_decoration_1()
            .text_decoration_color(cx.theme().colors().accent)
            .hover(|this| {
                this.text_color(cx.theme().colors().accent.opacity(0.8))
                    .text_decoration_1()
            })
            .active(|this| {
                this.text_color(cx.theme().colors().accent.opacity(0.6))
                    .text_decoration_1()
            })
            .cursor_pointer()
