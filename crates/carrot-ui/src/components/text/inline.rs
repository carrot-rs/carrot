...
use crate::{ActiveTheme, Selection, TextGlobalState, text::node::LinkMark};
...
impl Inline {
    ...
    fn paint_selected_bounds(&self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        window.paint_quad(inazuma::PaintQuad {
            bounds,
            background: cx.theme().colors().terminal.ansi.blue.alpha(0.01).into(),
            corner_radii: inazuma::Corners::default(),
            border_colors: inazuma::Edges {
                top: inazuma::Oklch::transparent_black(),
                right: inazuma::Oklch::transparent_black(),
                bottom: inazuma::Oklch::transparent_black(),
                left: inazuma::Oklch::transparent_black(),
            },
    ...
    fn layout_selections(
        ...
        let Some(text_view_state) = TextGlobalState::global(cx).text_view_state() else {
    ...
    fn paint_selection(
        ...
        if start_position.y == end_position.y {
            window.paint_quad(quad(
                ...
                px(0.),
                cx.theme().colors().element_selection,
                Edges::default(),
