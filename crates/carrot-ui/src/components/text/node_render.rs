use inazuma::{
    ...
    IntoElement, Length, ObjectFit, ParentElement, SharedString, StatefulInteractiveElement,
    Styled, StyledImage as _, Window, div, img, prelude::FluentBuilder as _, px, relative, rems,
};

use crate::{
    ActiveTheme as _, Icon, IconName, StyledExt, Tooltip,
    components::text::{document::NodeRenderOptions, inline::Inline},
    h_flex, v_flex,
};
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .font_family(
                        carrot_theme::theme_settings(cx)
                            .buffer_font(cx)
                            .family
                            .clone(),
                    )
                    .text_size(carrot_theme::theme_settings(cx).buffer_font_size(cx))
                    .relative()
                                                ...
                                                div()
                                                    ...
                                                    .when(checked, |this| {
                                                        this.bg(cx.theme().colors().primary).child(
                                                            Icon::new(IconName::Check).size_2(),
                                                        )
                                                    }),
                                            )
                                        })
