...
use crate::{TextGlobalState, text::TextViewStyle};
...
impl TextView {
    ...
    /// and use [`inazuma::list`] to render the content in a virtualized way.
    ///
...
impl Element for TextView {
    ...
    fn paint(
        ...
        TextGlobalState::global_mut(cx)
            .text_view_state_stack
            .push(state.clone());
        request_layout.element.paint(window, cx);
        TextGlobalState::global_mut(cx).text_view_state_stack.pop();
