    ...
    fn preview(_window: &mut Window, cx: &mut App) -> Option<AnyElement> {
        let illustration = h_flex()
            ...
            .child(div().absolute().inset_0().size_full().bg(linear_gradient(
                0.,
                linear_color_stop(cx.theme().colors().elevated_surface.opacity(0.1), 0.9),
                linear_color_stop(cx.theme().colors().elevated_surface.opacity(0.), 0.),
            )))
            .child(
                div()
                    .absolute()
