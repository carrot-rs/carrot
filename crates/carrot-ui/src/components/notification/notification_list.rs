...
use crate::{Anchor, TITLE_BAR_HEIGHT, prelude::*};

/// Default notification layout configuration.
struct NotificationLayout {
    placement: Anchor,
    margins: NotificationMargins,
}

struct NotificationMargins {
    top: Pixels,
    bottom: Pixels,
    left: Pixels,
    right: Pixels,
...
impl NotificationType {
    fn icon(&self, cx: &App) -> Icon {
        match self {
            Self::Info => Icon::new(IconName::Info).text_color(cx.theme().status().info.color),
            Self::Success => {
                Icon::new(IconName::Check).text_color(cx.theme().status().success.color)
            }
            Self::Warning => {
                Icon::new(IconName::Warning).text_color(cx.theme().status().warning.color)
            }
            Self::Error => Icon::new(IconName::XCircle).text_color(cx.theme().status().error.color),
        }
    }
}
...
impl Render for Notification {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let layout = NotificationLayout::default();
