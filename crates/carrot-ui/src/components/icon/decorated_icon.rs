use inazuma::{AnyElement, IntoElement, Point};
...
impl Component for DecoratedIcon {
    ...
    fn preview(_window: &mut Window, cx: &mut App) -> Option<AnyElement> {
        let decoration_x =
            IconDecoration::new(IconDecorationKind::X, cx.theme().colors().surface, cx)
        ...
        let decoration_dot =
            IconDecoration::new(IconDecorationKind::Dot, cx.theme().colors().surface, cx)
                .color(cx.theme().status().error.color)
                .position(Point {
                    x: px(-2.),
                    y: px(-2.),
