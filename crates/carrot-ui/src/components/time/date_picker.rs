...
use crate::{
    ActiveTheme, Button, ButtonVariants as _, Disableable, Icon, IconName, Sizable, Size,
    StyleSized as _, StyledExt as _, h_flex,
    input::{Delete, clear_button, input_style},
    utils::actions::{Cancel, Confirm},
    v_flex,
};
...
impl DatePickerState {
    ...
    fn clean(&mut self, _: &inazuma::ClickEvent, window: &mut Window, cx: &mut Context<Self>) {
        cx.stop_propagation();
        match self.date {
            Date::Single(_) => {
    ...
    fn toggle_calendar(&mut self, _: &inazuma::ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        self.open = !self.open;
        cx.notify();
    }
...
impl Render for DatePickerState {
    fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl inazuma::IntoElement {
        Empty
    }
}
                    ...
                    deferred(
                        anchored().snap_to_window_with_margin(px(8.)).child(
                            div()
                                ...
                                .shadow_lg()
                                .rounded((cx.theme().colors().radius * 2.).min(px(8.)))
