...
use carrot_ui_macros::IntoPlot;
use inazuma::{App, Bounds, Oklch, Pixels, SharedString, Window, px};
    ...
    Y: Copy + PartialOrd + Num + ToPrimitive + Sealed + 'static,
{
    fn paint(&mut self, bounds: Bounds<Pixels>, window: &mut Window, cx: &mut App) {
        ...
        // Draw X axis
        let mut axis = PlotAxis::new().stroke(cx.theme().colors().border);
        if self.x_axis {
        ...
        if self.grid {
            Grid::new()
                ...
                .stroke(cx.theme().colors().border)
                .dash_array(&[px(4.), px(2.)])
                .paint(&bounds, window);
        }

        // Draw line
        let stroke = self.stroke.unwrap_or(cx.theme().colors().chart.chart_2);
        let x_fn = x_fn.clone();
        let y_fn = y_fn.clone();
