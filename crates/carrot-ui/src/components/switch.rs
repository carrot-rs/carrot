use crate::{
    ActiveTheme, ButtonCommon as _, ButtonStyle, Clickable as _, Color, Component, ComponentScope,
    Disableable, IconButton, IconName, IconSize, KeyBinding, Label, LabelCommon as _, LabelSize,
    RegisterComponent, Sizable, Size, StyledExt, ToggleState, Tooltip, example_group_with_title,
    h_flex, single_example, v_flex,
};
use inazuma::{
    Animation, AnimationExt as _, AnyElement, AnyView, App, Edges, ElementId, InteractiveElement,
    IntoElement, Oklch, RenderOnce, SharedString, StatefulInteractiveElement, StyleRefinement,
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div().refine_style(&self.style).child(
            h_flex()
                ...
                .when(self.label_position == SwitchLabelPosition::Start, |this| {
                    this.when_some(self.label.clone(), |this, label| {
                        this.child(Label::new(label).size(self.label_size))
                    })
                })
                .child(switch_bar)
                .when(self.label_position == SwitchLabelPosition::End, |this| {
                    this.when_some(self.label.clone(), |this, label| {
                        this.child(Label::new(label).size(self.label_size))
                    })
                })
                ...
                .when_some(
                    on_click.as_ref().cloned().filter(|_| !self.disabled),
                    |this, on_click| {
                        let toggle_state = toggle_state.clone();
            ...
            v_flex()
                ...
                .children(vec![
                    example_group_with_title(
                        "States",
                        vec![
                            single_example("Off", Switch::new("switch_off").into_any_element()),
                            single_example(
                                "On",
                                Switch::new("switch_on").checked(true).into_any_element(),
                            ),
                            single_example(
                                "Disabled Off",
