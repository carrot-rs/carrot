...
/// use carrot_ui::{Button, Callout, IconName, Label, Severity};
///
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let (icon, icon_color, bg_color) = match self.severity {
            Severity::Info => (
                IconName::Info,
                Color::Muted,
                cx.theme().status().info.background.opacity(0.1),
            ),
            Severity::Success => (
                IconName::Check,
                Color::Success,
            ...
            Severity::Warning => (
                IconName::Warning,
                Color::Warning,
                cx.theme().status().warning.background.opacity(0.2),
            ),
            Severity::Error => (
                IconName::XCircle,
                Color::Error,
                cx.theme().status().error.color.opacity(0.08),
            ),
        };
