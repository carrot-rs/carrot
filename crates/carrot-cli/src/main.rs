...
use carrot_cli::{CliRequest, CliResponse, IpcHandshake, ipc::IpcOneShotServer};
use clap::Parser;
...
const URL_PREFIX: [&'static str; 5] = ["carrot://", "http://", "https://", "file://", "ssh://"];

struct Detect;

trait InstalledApp {
    fn carrot_version_string(&self) -> String;
...
#[command(
    ...
    before_help = "The Carrot CLI binary.
This CLI is a separate binary that invokes Carrot.
...
Examples:
    ...
    `carrot path-to-your-project`
          Open your project in Carrot
    `carrot -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | carrot -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/Carrot`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\Carrot`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/carrot`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in Carrot (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print Carrot's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run carrot in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to Carrot.app or the carrot binary
    #[arg(long)]
    carrot: Option<PathBuf>,
    /// Run carrot in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Carrot will attempt to open the paths directly.
    ///
    ...
    wsl: Option<String>,
    /// Not supported in Carrot CLI, only supported on Carrot binary
    ...
    diff: Vec<String>,
    /// Uninstall Carrot from user system
    #[cfg(all(
    ...
    /// by having Carrot act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
...
///
/// This method must return an absolute path, as many carrot
...
mod tests {
    ...
    use inazuma_util::test::TempTree;
    use serde_json::json;
...
fn main() -> Result<()> {
    ...
    if let Some(channel) = std::env::args().nth(1).filter(|arg| arg.starts_with("--")) {
        ...
        if let Ok(channel) = carrot_release_channel::ReleaseChannel::from_str(&channel[2..]) {
            return mac_os::spawn_channel_cli(channel, std::env::args().skip(2).collect());
        }
    }
    let args = Args::parse();

    // `carrot --askpass` Makes carrot operate in nc/netcat mode for use with askpass
    if let Some(socket) = &args.askpass {
        carrot_askpass::main(socket);
        return Ok(());
    }
    ...
    if let Some(dir) = &user_data_dir {
        carrot_paths::set_custom_data_dir(dir);
    }

    #[cfg(target_os = "linux")]
    let args = flatpak::set_bin_if_no_escape(args);

    let app = Detect::detect(args.carrot.as_deref()).context("Bundle detection")?;

    if args.version {
        println!("{}", app.carrot_version_string());
        return Ok(());
    }
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Carrot CLI, only on Carrot binary.",
    ...
    if args.uninstall {
        ...
        let status = std::process::Command::new("sh")
            ...
            .env(
                "CARROT_CHANNEL",
                &*carrot_release_channel::RELEASE_CHANNEL_NAME,
            )
            .status()
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Carrot spawn")?;
    let url = format!("carrot-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `carrot`.
    ...
    // for directory diffs. The CLI process may exit before Carrot has read these
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://carrot.dev/docs/remote-development"
    );
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after Carrot spawn")?;
...
mod linux {
    ...
    use carrot_cli::FORCE_CLI_MODE_ENV_VAR_NAME;
    use fork::Fork;
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/carrot is for Arch (and other non-libexec distros),
                // ./carrot is for the target directory in development builds.
                let possible_locations = [
                    "../libexec/carrot-editor",
                    "../lib/carrot/carrot-editor",
                    "./carrot",
                ];
                possible_locations
                    .iter()
    ...
    impl InstalledApp for App {
        fn carrot_version_string(&self) -> String {
            format!(
                "Carrot {}{}{} – {}",
                if *carrot_release_channel::RELEASE_CHANNEL_NAME == "stable" {
                    "".to_string()
                } else {
                    format!("{} ", *carrot_release_channel::RELEASE_CHANNEL_NAME)
                },
                option_env!("RELEASE_VERSION").unwrap_or_default(),
                match option_env!("CARROT_COMMIT_SHA") {
        ...
        fn launch(&self, ipc_url: String, user_data_dir: Option<&str>) -> anyhow::Result<()> {
            let data_dir = user_data_dir
                .map(PathBuf::from)
                .unwrap_or_else(|| carrot_paths::data_dir().clone());
...
mod flatpak {
    ...
    const EXTRA_LIB_ENV_NAME: &str = "CARROT_FLATPAK_LIB_PATH";
    const NO_ESCAPE_ENV_NAME: &str = "CARROT_FLATPAK_NO_ESCAPE";
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=CARROT_UPDATE_EXPLANATION=Please use flatpak to update carrot".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("carrot").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--carrot";
            }

            if !is_app_location_set {
                args.push("--carrot".into());
                args.push(flatpak_dir.join("libexec").join("carrot-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("dev.carrot.Carrot"))
            && args.carrot.is_none()
        {
            args.carrot = Some("/app/libexec/carrot-editor".into());
            unsafe {
                env::set_var(
                    "CARROT_UPDATE_EXPLANATION",
                    "Please use flatpak to update carrot",
                )
            };
        }
        args
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.carrot.Carrot") {
                return None;
            }
...
mod windows {
    use anyhow::Context;
    use carrot_release_channel::app_identifier;
    use windows::{
        Win32::{
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../Carrot.exe is the standard, lib/carrot is for MSYS2, ./carrot.exe is for the target
                // directory in development builds.
                let possible_locations = [
                    "../Carrot.exe",
                    "../lib/carrot/carrot-editor.exe",
                    "./carrot.exe",
                ];
                possible_locations
                    .iter()
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn carrot_version_string(&self) -> String {
            format!("Carrot {} – {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open carrot-cli:... -a /Applications/Carrot\ Preview.app
                        let urls_to_open =
                ...
                Self::LocalPath { executable, .. } => {
                    ...
                    let subprocess_stdout_file = fs::File::create(
                        executable_parent.join("carrot_dev.log"),
                    )
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/carrot"),
    ...
    pub(super) fn spawn_channel_cli(
        channel: carrot_release_channel::ReleaseChannel,
