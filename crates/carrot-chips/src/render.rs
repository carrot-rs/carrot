use carrot_theme::{ActiveTheme, ChipColors};
use carrot_ui::{Chip, ChipTooltip, Color, Icon, IconSize, h_flex};
use inazuma::{
    AnyElement, App, InteractiveElement, IntoElement, Oklch, ParentElement,
...
pub fn chip_theme_color(colors: &ChipColors, id: ChipId) -> Oklch {
    match id {
        ...
        "dotnet" | "dart" | "lua" | "cobol" | "cmake" | "buf" | "opa" | "solidity" | "rlang"
        | "odin" => BOLD_BLUE,
        "elixir" | "haskell" | "julia" | "fortran" | "fossil_branch" | "hg_branch"
        | "pijul_channel" | "mise" | "nats" => BOLD_PURPLE,
        "elm" | "daml" | "vagrant" | "kubernetes" => BOLD_CYAN,
        "nim" | "ocaml" | "zig" | "openstack" | "pixi" | "vcsh" | "shlvl" | "cmd_duration" => {
            BOLD_YELLOW
        }
        ...
        // DevOps
        "aws" => BOLD_YELLOW,
        "azure" | "docker_context" | "gcloud" | "nix_shell" | "sudo" | "jobs" | "spack" => {
            BOLD_BLUE
        }
...
/// Render a standard chip using the generic `Chip` component from carrot-ui.
///
...
pub fn render_standard_chip(
    ...
    if let Some(ref segments) = output.segments {
        ...
        for seg in segments {
            ...
            container = container.child(div().text_color(color).child(seg.text.clone()));
        }
