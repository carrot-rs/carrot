...
impl ChipProvider for GitStateProvider {
    ...
    fn gather(&self, ctx: &ChipContext) -> ChipOutput {
        let (label, tooltip) =
            detect_git_state(ctx).unwrap_or_else(|| (String::new(), String::new()));
...
fn read_rebase_progress(rebase_dir: &std::path::Path) -> Option<(String, String)> {
    let current =
        read_number_file(rebase_dir, "msgnum").or_else(|| read_number_file(rebase_dir, "next"))?;
    let total =
        read_number_file(rebase_dir, "end").or_else(|| read_number_file(rebase_dir, "last"))?;
    Some((current, total))
}
