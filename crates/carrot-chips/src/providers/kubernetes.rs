...
fn gather_kube_info(ctx: &ChipContext) -> Option<String> {
    ...
    let raw_kubeconfigs =
        env::split_paths(&kube_cfg).map(|file| std::fs::read_to_string(file).ok());
    ...
    let (display_context, display_user) = config
        ...
        .unwrap_or_else(|| {
            ...
            let display_ctx =
                deprecated::get_alias(ctx_name_owned.clone(), &config.context_aliases, "context")
                    .unwrap_or_else(|| ctx_name_owned.clone());

            let display_user = ctx_components
                .user
                .clone()
                .and_then(|user| deprecated::get_alias(user, &config.user_aliases, "user"));
...
"#;
    ...
    fn multi_config_first_context_wins() {
        ...
        let docs =
            parse_kubeconfigs(vec![Some(yaml1.to_string()), Some(yaml2.to_string())].into_iter());
