...
fn get_credentials_file_path(ctx: &ChipContext) -> Option<PathBuf> {
    ctx.get_env("AWS_SHARED_CREDENTIALS_FILE")
...
fn alias_name<V: AsRef<str>, S: std::hash::BuildHasher>(
    name: Option<String>,
    aliases: &std::collections::HashMap<String, V, S>,
) -> Option<String> {
    name.as_ref()
...
fn gather_aws_info(ctx: &ChipContext) -> Option<String> {
    ...
    if !config.force_display
        ...
        && !has_source_profile(ctx, aws_profile.as_ref(), &aws_config_file, &aws_creds_file)
            .unwrap_or(false)
        && !has_defined_credentials(ctx, aws_profile.as_ref(), &aws_creds_file).unwrap_or(false)
    {
        return None;
    }
    ...
    let duration =
        get_credentials_duration(ctx, aws_profile.as_ref(), &aws_config_file, &aws_creds_file).map(
            |d| {
                if d > 0 {
...
mod tests {
    ...
    fn test_alias_name() {
        ...
        assert_eq!(
            alias_name(Some("us-east-1".into()), &aliases),
            Some("use1".to_string())
        );
        assert_eq!(
            alias_name(Some("eu-west-1".into()), &aliases),
