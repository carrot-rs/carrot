...
///
/// The rendering layer (in `carrot-terminal-view`) maps `ChipOutput` to
...
pub struct ChipOutput {
    ...
    /// Primary label text.
    pub label: String,
    /// Optional icon name (from carrot-ui IconName).
...
pub fn parse_version_number(output: &str) -> String {
    output
        .split_whitespace()
        .find(|word| word.starts_with(|c: char| c.is_ascii_digit()) || word.starts_with('v'))
