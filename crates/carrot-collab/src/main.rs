...
use carrot_collab::api::CloudflareIpCountryHeader;
use carrot_collab::db::Database;
use carrot_collab::{
    AppState, Config, Result, api::fetch_extensions_from_blob_store_periodically, db, env,
    executor::Executor,
};
use carrot_collab::{REVISION, ServiceMode, VERSION};
...
async fn main() -> Result<()> {
    ...
    match args.next().as_deref() {
        ...
        Some("seed") => {
            ...
            carrot_collab::seed::seed(&config, &db, false).await?;
        }
        Some("serve") => {
            ...
            if mode.is_collab() || mode.is_api() {
                ...
                if mode.is_collab() {
                    let epoch = state
                        .db
                        .create_server(&state.config.carrot_environment)
                        .await?;
                    let rpc_server = carrot_collab::rpc::Server::new(epoch, state.clone());
                    rpc_server.start().await?;

                    app = app.merge(carrot_collab::rpc::routes(rpc_server.clone()));
                ...
                if mode.is_api() {
                    ...
                    app = app
                        .merge(carrot_collab::api::events::router())
                        .merge(carrot_collab::api::extensions::router())
                }
