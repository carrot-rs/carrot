...
use carrot_rpc::proto::split_repository_update;
pub use connection_pool::{CarrotVersion, ConnectionPool};
            ...
            async move {
                ...
                app_state
                    ...
                    .delete_stale_channel_chat_participants(
                        &app_state.config.carrot_environment,
                        server_id,
                    )
                    .await
                    .trace_err();
                ...
                if let Some((room_ids, channel_ids)) = app_state
                    .db
                    .stale_server_resource_ids(&app_state.config.carrot_environment, server_id)
                    .await
                    .trace_err()
                {
                ...
                app_state
                    .db
                    .delete_stale_servers(&app_state.config.carrot_environment, server_id)
                    .await
                    .trace_err();
            }
            .instrument(span),
        ...
        match &session.principal {
            Principal::User(user) => {
...

                {
                    ...
                    pool.add_connection(connection_id, user.id, user.admin, carrot_version.clone());
                    self.peer.send(
                        connection_id,
                ...
                if should_auto_subscribe_to_channels(&carrot_version) {
...
impl Header for ProtocolVersion {
    fn name() -> &'static HeaderName {
        static CARROT_PROTOCOL_VERSION: OnceLock<HeaderName> = OnceLock::new();
        CARROT_PROTOCOL_VERSION.get_or_init(|| HeaderName::from_static("x-carrot-protocol-version"))
    }
...
impl Header for AppVersionHeader {
    fn name() -> &'static HeaderName {
        static CARROT_APP_VERSION: OnceLock<HeaderName> = OnceLock::new();
        CARROT_APP_VERSION.get_or_init(|| HeaderName::from_static("x-carrot-app-version"))
    }
...
impl Header for ReleaseChannelHeader {
    fn name() -> &'static HeaderName {
        static CARROT_RELEASE_CHANNEL: OnceLock<HeaderName> = OnceLock::new();
        CARROT_RELEASE_CHANNEL.get_or_init(|| HeaderName::from_static("x-carrot-release-channel"))
    }
...
pub async fn handle_websocket_request(
    ...
    if protocol_version != carrot_rpc::PROTOCOL_VERSION {
        return (
    ...
    let Some(version) = app_version_header.map(|header| CarrotVersion(header.0.0)) else {
        return (
            StatusCode::UPGRADE_REQUIRED,
    ...
    if let Some(live_kit) = session.app_state.livekit_client.as_ref() {
        live_kit
            .update_participant(
                ...
                carrot_livekit_api::proto::ParticipantPermission {
                    can_subscribe: true,
                    can_publish,
...
fn should_auto_subscribe_to_channels(version: &CarrotVersion) -> bool {
    version.0.minor < 139
}
...
async fn join_channel_internal(
    ...
    let joined_room = {
        ...
        // If carrot quits without leaving the room, and the user re-opens carrot before the
...
async fn send_channel_message(
    ...
    Err(anyhow!("chat has been removed in the latest version of Carrot").into())
}
