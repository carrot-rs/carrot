...
use carrot_telemetry_events::{Event, EventRequestBody};
use chrono::Duration;
...
pub struct CarrotChecksumHeader(Vec<u8>);

impl Header for CarrotChecksumHeader {
    fn name() -> &'static HeaderName {
        static CARROT_CHECKSUM_HEADER: OnceLock<HeaderName> = OnceLock::new();
        CARROT_CHECKSUM_HEADER.get_or_init(|| HeaderName::from_static("x-carrot-checksum"))
    }
...
pub async fn post_events(
    ...
    TypedHeader(CarrotChecksumHeader(checksum)): TypedHeader<CarrotChecksumHeader>,
    ...
    let request_body: EventRequestBody = serde_json::from_slice(&body).map_err(|err| {
...
pub fn calculate_json_checksum(app: Arc<AppState>, json: &impl AsRef<[u8]>) -> Option<Vec<u8>> {
    let checksum_seed = app.config.carrot_client_checksum_seed.as_ref()?;
...
fn for_snowflake(
    ...
    body.events.into_iter().map(move |event| {
        ...
        // dictionary. See https://app.amplitude.com/data/carrot/Carrot/sources/detail/production/falcon%3A159998
