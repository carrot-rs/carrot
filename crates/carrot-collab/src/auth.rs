...
use carrot_cloud_api_types::GetAuthenticatedUserResponse;
pub use carrot_rpc::auth::random_token;
use std::sync::Arc;
...
pub async fn validate_header<B>(mut req: Request<B>, next: Next<B>) -> impl IntoResponse {
    ...
    if first == "dev-server-token" {
        Err(Error::http(
            StatusCode::UNAUTHORIZED,
            "Dev servers were removed in Carrot 0.157 please upgrade to SSH remoting".to_string(),
        ))?;
    }
