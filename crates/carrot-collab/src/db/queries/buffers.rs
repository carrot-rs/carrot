...
use inazuma_text::{EditOperation, UndoOperation};
use prost::Message;
    ...
    pub async fn join_channel_buffer(
        ...
        self.transaction(|tx| async move {
            ...
            let replica_ids = collaborators
                ...
                .collect::<HashSet<_>>();
            let mut replica_id =
                ReplicaId(inazuma_clock::ReplicaId::FIRST_COLLAB_ID.as_u16() as i32);
...
pub fn operation_from_wire(operation: proto::Operation) -> Option<inazuma_text::Operation> {
    match operation.variant? {
        proto::operation::Variant::Edit(edit) => {
            Some(inazuma_text::Operation::Edit(EditOperation {
                ...
                ranges: edit
                    ...
                    .into_iter()
                    .map(|range| {
                        inazuma_text::FullOffset(range.start as usize)
                            ..inazuma_text::FullOffset(range.end as usize)
                    })
                    .collect(),
        ...
        proto::operation::Variant::Undo(undo) => {
            Some(inazuma_text::Operation::Undo(UndoOperation {
                timestamp: inazuma_clock::Lamport {
