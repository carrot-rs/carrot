...
pub struct Config {
    ...
    pub carrot_environment: Arc<str>,
    pub carrot_client_checksum_seed: Option<String>,
}

impl Config {
    pub fn is_development(&self) -> bool {
        self.carrot_environment == "development".into()
    }

    /// Returns the base `carrot.dev` URL.
    pub fn carrot_dot_dev_url(&self) -> &str {
        match self.carrot_environment.as_ref() {
            "development" => "http://localhost:3000",
            "staging" => "https://staging.carrot.dev",
            _ => "https://carrot.dev",
        }
    }

    /// Returns the base Carrot Cloud URL.
    pub fn carrot_cloud_url(&self) -> &str {
        match self.carrot_environment.as_ref() {
            "development" => "http://localhost:8787",
            _ => "https://cloud.carrot.dev",
        }
    }
...
pub struct AppState {
    ...
    pub livekit_client: Option<Arc<dyn carrot_livekit_api::Client>>,
...
impl AppState {
    pub async fn new(config: Config, executor: Executor) -> Result<Arc<Self>> {
        ...
        {
            Some(Arc::new(carrot_livekit_api::LiveKitClient::new(
                server.clone(),
                key.clone(),
                secret.clone(),
            )) as Arc<dyn carrot_livekit_api::Client>)
        } else {
            None
        };
