...
use carrot_client::RECEIVE_TIMEOUT;
use carrot_git::{
    repository::{RepoPath, Worktree as GitWorktree},
    status::{DiffStat, FileStatus, StatusCode, TrackedStatus},
};
use carrot_git_ui::{git_panel::GitPanel, project_diff::ProjectDiff};
...
fn collect_diff_stats<C: inazuma::AppContext>(
    panel: &inazuma::Entity<GitPanel>,
    cx: &C,
...
async fn test_project_diff(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    cx_b.update(carrot_git_ui::init);
    ...
    cx_b.update(|window, cx| {
        window
            .focused(cx)
            .unwrap()
            .dispatch_action(&carrot_git_ui::project_diff::Diff, window, cx)
    });
