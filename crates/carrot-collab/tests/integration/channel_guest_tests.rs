...
use carrot_collab::db::ChannelId;
...
async fn test_channel_guests(
    ...
    cx_b.update(|cx| {
        carrot_workspace::join_channel(channel_id, client_b.app_state.clone(), None, None, cx)
    })
    .await
    .unwrap();
...
async fn test_channel_guest_promotion(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    let project_a = client_a.build_test_project(cx_a).await;
    cx_a.update(|cx| {
        carrot_workspace::join_channel(channel_id, client_a.app_state.clone(), None, None, cx)
    })
    .await
    .unwrap();
...
#[inazuma::test]
async fn test_channel_requires_carrot_cla(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    let mut server = TestServer::start(cx_a.executor()).await;
    ...
    // Create a parent channel that requires the Carrot CLA
    ...
    server
        .app_state
        .db
        .set_channel_requires_carrot_cla(ChannelId::from_proto(parent_channel_id.0), true)
        .await
        .unwrap();
