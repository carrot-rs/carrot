...
use carrot_collab::db::UserId;
use carrot_editor::Bias;
...
use inazuma::{BackgroundExecutor, Entity, TestAppContext};
...
#[inazuma::test(iterations = 100, on_failure = "crate::save_randomized_test_plan")]
    ...
    async fn on_quiesce(_: &mut TestServer, clients: &mut [(Rc<TestClient>, TestAppContext)]) {
        for (client, client_cx) in clients.iter() {
            for guest_project in client.dev_server_projects().iter() {
                ...
                // https://github.com/carrot-rs/carrot/pull/16696 that wasn't
                // detected until we upgraded the rng crate. This whole crate is
                // going away with DeltaDB soon, so we hold our nose and
                // continue.
                fn null_out_entry_size(entry: &carrot_project::Entry) -> carrot_project::Entry {
                    carrot_project::Entry {
                        size: 0,
                        ..entry.clone()
                    }
                }
...
fn buffer_for_full_path(
    ...
    full_path: &RelPath,
    cx: &TestAppContext,
) -> Option<Entity<carrot_language::Buffer>> {
    client
        .buffers_for_project(project)
        .iter()
        .find(|buffer| {
