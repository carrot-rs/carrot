...
use carrot_channel::ACKNOWLEDGE_DEBOUNCE_INTERVAL;
use carrot_client::{Collaborator, ParticipantIndex, UserId};
use carrot_collab::rpc::{CLEANUP_TIMEOUT, RECONNECT_TIMEOUT};

use carrot_collab_ui::channel_view::ChannelView;
use carrot_editor::{Anchor, Editor, MultiBufferOffset, ToOffset};
use carrot_rpc::{RECEIVE_TIMEOUT, proto::PeerId};
use carrot_workspace::CollaboratorId;
use futures::future;
use inazuma::{BackgroundExecutor, Context, Entity, TestAppContext, Window};
...
async fn test_core_channel_buffers(
    ...
    let channel_id = server
        .make_channel("carrot", None, (&client_a, cx_a), &mut [(&client_b, cx_b)])
        .await;
...
fn buffer_text(
    channel_buffer: &Entity<carrot_language::Buffer>,
