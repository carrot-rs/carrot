...
use inazuma::{BackgroundExecutor, Task, TestAppContext};
        ...
        Some(loop {
            break match self.rng.random_range(0..100) {
                ...
                _ if !clients.is_empty() => {
                    ...
                    let batch_id = inazuma_gpui_util::post_inc(&mut self.next_batch_id);
    ...
    async fn apply_server_operation(
        ...
        match operation {
            ...
            ServerOperation::RestartServer => {
                ...
                let environment = &server.app_state.config.carrot_environment;
...
impl UserTestPlan {
    pub fn next_root_dir_name(&mut self) -> String {
        ...
        let root_id = inazuma_gpui_util::post_inc(&mut self.next_root_id);
