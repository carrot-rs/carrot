...
use carrot_dap::{Capabilities, adapters::DebugTaskDefinition, transport::RequestHandling};
...
async fn test_remote_server_debugger(
    ...
    let session =
        carrot_debugger_ui::tests::start_debug_session(&workspace_window, cx_a, |_| {}).unwrap();
    cx_a.run_until_parked();
    ...
    session.update(
        cx_a,
        |session: &mut carrot_project::debugger::session::Session, _| {
...
async fn test_slow_adapter_startup_retries(
    ...
    let session = carrot_debugger_ui::tests::start_debug_session_with(
        &workspace_window,
        cx_a,
        DebugTaskDefinition {
        ...
        move |client| {
            ...
            client.on_request_ext::<carrot_dap::requests::Initialize, _>(move |_seq, _request| {
...
async fn test_ssh_remote_worktree_trust(cx_a: &mut TestAppContext, server_cx: &mut TestAppContext) {
    ...
    cx_a.read(|cx| {
        assert_eq!(
            ...
            ["...".to_string()],
            "remote .carrot/settings.json must not sync before trust approval"
        )
    });
    ...
    cx_a.read(|cx| {
        assert_eq!(
            LanguageSettings::for_buffer(buffer_before_approval.read(cx), cx).language_servers,
            ["override-rust-analyzer".to_string()],
            "remote .carrot/settings.json should sync after trust approval"
        )
    });
