...
use carrot_collab_ui::{
    channel_view::ChannelView,
    notifications::project_shared_notification::ProjectSharedNotification,
};
use carrot_editor::{Editor, MultiBuffer, MultiBufferOffset, PathKey, SelectionEffects};
...
async fn test_basic_following(
    ...
    executor.advance_clock(carrot_workspace::item::LEADER_UPDATE_THROTTLE);
    executor.run_until_parked();
    ...
    // #[cfg(all(not(target_os = "macos"), not(target_os = "windows")))]
    {
        ...
        use inazuma::TestScreenCaptureSource;
        ...
        // Client B activates Carrot again, which causes the previous editor to become focused again.
        active_call_b
            .update(cx_b, |call, cx| call.set_location(Some(&project_b), cx))
            .await
            .unwrap();
...
async fn test_following_tab_order(
    ...
    let pane_paths = |pane: &Entity<carrot_workspace::Pane>, cx: &mut VisualTestContext| {
...
async fn test_auto_unfollowing(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    editor_b2.update_in(cx_b, |editor, window, cx| {
        editor.move_right(&carrot_editor::actions::MoveRight, window, cx)
    });
    assert_eq!(
        workspace_b.update(cx_b, |workspace, _| workspace.leader_for_pane(&pane_b)),
        None
...
async fn test_following_across_workspaces(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    cx_a.update(|_, cx| {
        let opener =
            carrot_workspace::workspace_opener(cx).expect("workspace opener must be registered");
...
async fn test_following_stops_on_unshare(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    cx_a.executor()
        .advance_clock(carrot_workspace::item::LEADER_UPDATE_THROTTLE);
    cx_a.run_until_parked();
...
async fn test_following_to_channel_notes_without_a_shared_project(
    ...
    cx_a.update(carrot_editor::init);
    cx_b.update(carrot_editor::init);
    cx_c.update(carrot_editor::init);
    cx_a.update(carrot_collab_ui::channel_view::init);
    cx_b.update(carrot_collab_ui::channel_view::init);
    cx_c.update(carrot_collab_ui::channel_view::init);
...
pub(crate) async fn join_channel(
    ...
    cx.update(|cx| {
        carrot_workspace::join_channel(channel_id, client.app_state.clone(), None, None, cx)
    })
    .await
}
