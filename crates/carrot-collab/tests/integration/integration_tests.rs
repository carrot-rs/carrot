...
use carrot_buffer_diff::{DiffHunkSecondaryStatus, DiffHunkStatus, assert_hunks};
use carrot_call::{ActiveCall, Room, room};
use carrot_client::{RECEIVE_TIMEOUT, User};
use carrot_collab::rpc::{CLEANUP_TIMEOUT, RECONNECT_TIMEOUT};
use carrot_fs::{FakeFs, Fs as _, RemoveOptions};
use carrot_git::{
...
use carrot_lsp::{DEFAULT_LSP_REQUEST_TIMEOUT, LanguageServerId, OneOf};
use carrot_project::{
...
use carrot_workspace::{Pane, ParticipantLocation};
...
use inazuma_settings_framework::{
    LanguageServerFormatterSpecifier, PrettierSettingsContent, SettingsStore,
};
    ...
    assert!(
        matches!(
            *client.status().borrow(),
            carrot_client::Status::Connected { .. }
        ),
        "status was {:?}",
        *client.status().borrow()
    );
...
async fn test_basic_calls(
    ...
    let display = inazuma::TestScreenCaptureSource::new();
    ...
    if let carrot_call::room::Event::RemoteVideoTracksChanged { participant_id } = event_b {
    ...
    if let carrot_call::room::Event::RemoteVideoTracksChanged { participant_id } = event_c {
        assert_eq!(participant_id, client_a.peer_id().unwrap());
...
async fn test_project_reconnect(
    ...
    cx_b.update(carrot_editor::init);
...
async fn test_local_settings(
    ...
    client_a
        .fs()
        .insert_file("/dir/.carrot/settings.json", r#"{}"#.into())
        .await;
    executor.run_until_parked();
    cx_b.read(|cx| {
        let store = cx.global::<SettingsStore>();
    ...
    client_a
        .fs()
        .remove_file("/dir/.carrot/settings.json".as_ref(), Default::default())
        .await
        .unwrap();
    client_a
        .fs()
        .create_dir("/dir/b/.carrot".as_ref())
        .await
        .unwrap();
    client_a
        .fs()
        .insert_file("/dir/b/.carrot/settings.json", r#"{"tab_size": 4}"#.into())
        .await;
    executor.run_until_parked();
    cx_b.read(|cx| {
        let store = cx.global::<SettingsStore>();
    ...
    client_a
        .fs()
        .insert_file(
            "/dir/a/.carrot/settings.json",
            r#"{"hard_tabs":true}"#.into(),
        )
        .await;
    client_a
        .fs()
        .remove_file("/dir/b/.carrot/settings.json".as_ref(), Default::default())
        .await
        .unwrap();
    executor.run_until_parked();
...
async fn test_collaborating_with_diagnostics(
    ...
    project_c.update(cx_c, |_, cx| {
        ...
        cx.subscribe(&project_c, {
            move |p, _, event, cx| {
                if let carrot_project::Event::DiskBasedDiagnosticsFinished { .. } = event {
...
async fn test_prettier_formatting_buffer(
    ...
    client_a
        ...
        .await;
    let (project_a, worktree_id) = client_a.build_local_project(&directory, cx_a).await;
    let prettier_format_suffix = carrot_project::TEST_PRETTIER_FORMAT_SUFFIX;
...
async fn test_share_screen_wayland(
    ...
    // User B observes the remote screen sharing track.
    assert_eq!(events_b.borrow().len(), 1);
    if let carrot_call::room::Event::RemoteVideoTracksChanged { participant_id } =
...
async fn test_preview_tabs(cx: &mut TestAppContext) {
    ...
    pane.update_in(cx, |pane, window, cx| {
        pane.close_item_by_id(
            ...
            carrot_workspace::SaveIntent::Skip,
            window,
            cx,
        )
    })
    ...
    pane.update_in(cx, |pane, window, cx| {
        ...
        pane.close_item_by_id(id, carrot_workspace::SaveIntent::Skip, window, cx)
    })
    .await
    .unwrap();
    ...
    pane.update_in(cx, |pane, window, cx| {
        pane.split(
            ...
            carrot_workspace::SplitMode::default(),
            window,
            cx,
        );
    });
    ...
    workspace.update_in(cx, |workspace, window, cx| {
        workspace.activate_pane_in_direction(carrot_workspace::SplitDirection::Left, window, cx)
    });
