...
impl TestDb {
    ...
    pub fn postgres(executor: BackgroundExecutor) -> Self {
        ...
        let url = format!(
            "postgres://postgres@localhost/carrot-test-{}",
            rng.random::<u128>()
        );
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_io()
...
macro_rules! test_both_dbs {
    ($test_name:ident, $postgres_test_name:ident, $sqlite_test_name:ident) => {
        #[inazuma::test]
        async fn $postgres_test_name(cx: &mut inazuma::TestAppContext) {
        ...
        #[inazuma::test]
        async fn $sqlite_test_name(cx: &mut inazuma::TestAppContext) {
