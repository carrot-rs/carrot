...
use carrot_collab::rpc::RECONNECT_TIMEOUT;
use carrot_editor::{
...
use carrot_language::{FakeLspAdapter, language_settings::LanguageSettings, rust_lang};
use carrot_lsp as lsp;
use carrot_lsp::DEFAULT_LSP_REQUEST_TIMEOUT;
use carrot_multi_buffer::{AnchorRangeExt as _, MultiBufferRow};
use carrot_project::{
...
use carrot_recent_projects::disconnected_overlay::DisconnectedOverlay;
use carrot_rpc::RECEIVE_TIMEOUT;
use carrot_workspace::item::Item as _;
use carrot_workspace::{CloseIntent, Workspace};
use futures::{SinkExt, StreamExt, channel::mpsc, lock::Mutex};
use inazuma::{
    App, Entity, Rgba, SharedString, TestAppContext, UpdateGlobal, VisualContext, VisualTestContext,
};
...
async fn test_host_disconnect(
    ...
    cx_b.update(carrot_recent_projects::init);
...
async fn test_newline_above_or_below_does_not_move_guest_cursor(
    ...
    editor_cx_a.update_editor(|editor, window, cx| {
        editor.newline_above(&carrot_editor::actions::NewlineAbove, window, cx)
    });
    ...
    "});
    editor_cx_a.update_editor(|editor, window, cx| {
        editor.newline_below(&carrot_editor::actions::NewlineBelow, window, cx)
    });
    executor.run_until_parked();
    editor_cx_a.assert_editor_state(indoc! {"
...
async fn test_collaborating_with_code_actions(
    ...
    cx_a.background_executor
        .advance_clock(carrot_editor::CODE_ACTIONS_DEBOUNCE_TIMEOUT * 2);
    requests.next().await;
...
async fn test_collaborating_with_renames(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    editor_b.update_in(cx_b, |editor, window, cx| {
        editor.cancel(&carrot_editor::actions::Cancel, window, cx);
    });
    let prepare_rename = editor_b.update_in(cx_b, |editor, window, cx| {
        editor.change_selections(SelectionEffects::no_scroll(), window, cx, |s| {
...
async fn test_share_project(
    ...
    buffer_a.read_with(cx_a, |buffer, _| {
        buffer
            .snapshot()
            .selections_in_range(
                inazuma_text::Anchor::min_max_range_for_buffer(buffer.remote_id()),
                false,
            )
            .count()
            == 1
...
async fn test_mutual_editor_inlay_hint_cache_update(
    ...
    cx_a.update(carrot_editor::init);
    cx_b.update(carrot_editor::init);
...
async fn test_git_blame_is_forwarded(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    let blame = carrot_git::blame::Blame {
        entries: vec![
    ...
    editor_b.update_in(cx_b, |editor_b, window, cx| {
        ...
        editor_b.toggle_git_blame(&carrot_git::Blame {}, window, cx);
    });
...
async fn test_add_breakpoints(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    editor_a.update_in(cx_a, |editor, window, cx| {
        editor.toggle_breakpoint(&carrot_editor::actions::ToggleBreakpoint, window, cx);
    });
    ...
    editor_b.update_in(cx_b, |editor, window, cx| {
        editor.move_down(&carrot_actions::editor::MoveDown, window, cx);
        editor.move_down(&carrot_actions::editor::MoveDown, window, cx);
        editor.toggle_breakpoint(&carrot_editor::actions::ToggleBreakpoint, window, cx);
    });
    ...
    editor_b.update_in(cx_b, |editor, window, cx| {
        editor.move_up(&carrot_actions::editor::MoveUp, window, cx);
        editor.move_up(&carrot_actions::editor::MoveUp, window, cx);
        editor.toggle_breakpoint(&carrot_editor::actions::ToggleBreakpoint, window, cx);
    });
...
fn tab_undo_assert(
    ...
    if a_tabs {
        cx_a.update_editor(|editor, window, cx| {
            editor.tab(&carrot_editor::actions::Tab, window, cx);
        });
    } else {
...
async fn test_remote_project_worktree_trust(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    let has_restricted_worktrees = |project: &inazuma::Entity<carrot_project::Project>,
    ...
    cx_a.update(|cx| {
        carrot_project::trusted_worktrees::init(HashMap::default(), cx);
    });
    cx_b.update(|cx| {
        carrot_project::trusted_worktrees::init(HashMap::default(), cx);
    });
...
fn blame_entry(sha: &str, range: Range<u32>) -> carrot_git::blame::BlameEntry {
    carrot_git::blame::BlameEntry {
