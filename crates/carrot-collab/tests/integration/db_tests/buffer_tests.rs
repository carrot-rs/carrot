...
use carrot_language::proto::{self, serialize_version};
use carrot_rpc::ConnectionId;
use inazuma_text::{Buffer, ReplicaId};
...
async fn test_channel_buffers(db: &Arc<Database>) {
    ...
    let _ = db
        .join_channel_buffer(carrot_id, a_id, connection_id_a)
        .await
        .unwrap();
    ...
    let operations = operations
        .into_iter()
        .map(|op| proto::serialize_operation(&carrot_language::Operation::Buffer(op)))
        .collect::<Vec<_>>();

    db.update_channel_buffer(carrot_id, a_id, &operations)
        .await
        .unwrap();

    let connection_id_b = ConnectionId { owner_id, id: 2 };
    let buffer_response_b = db
        .join_channel_buffer(carrot_id, b_id, connection_id_b)
        .await
        .unwrap();
    ...
    buffer_b.apply_ops(buffer_response_b.operations.into_iter().map(|operation| {
        ...
        if let carrot_language::Operation::Buffer(operation) = operation {
            operation
        } else {
            unreachable!()
        }
    ...
    assert!(
        db.join_channel_buffer(carrot_id, c_id, ConnectionId { owner_id, id: 3 })
            .await
            .is_err()
    );
    ...
    assert_eq!(
        ...
        &[
            carrot_rpc::proto::Collaborator {
                user_id: a_id.to_proto(),
                peer_id: Some(carrot_rpc::proto::PeerId { id: 1, owner_id }),
            ...
            carrot_rpc::proto::Collaborator {
                ...
                peer_id: Some(carrot_rpc::proto::PeerId { id: 2, owner_id }),
    ...
    let carrot_collaborats = db
        .get_channel_buffer_collaborators(carrot_id)
        .await
        .unwrap();
    assert_eq!(carrot_collaborats, &[a_id, b_id]);

    let left_buffer = db
        .leave_channel_buffer(carrot_id, connection_id_b)
        .await
        .unwrap();
...
async fn test_channel_buffers_last_operations(db: &Database) {
    ...
    pretty_assertions::assert_eq!(
        ...
        [
            carrot_rpc::proto::ChannelBufferVersion {
...
async fn update_buffer(
    ...
    user_id: UserId,
    db: &Database,
    operations: Vec<inazuma_text::Operation>,
) {
    let operations = operations
        .into_iter()
        .map(|op| proto::serialize_operation(&carrot_language::Operation::Buffer(op)))
