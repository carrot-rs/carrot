...
use carrot_collab::db::{Channel, ChannelId, ChannelRole, Database, NewUserParams, RoomId, UserId};
use carrot_rpc::{
    ConnectionId,
...
async fn test_channels(db: &Arc<Database>) {
    ...
    let carrot_id = db.create_root_channel("carrot", a_id).await.unwrap();

    // Make sure that people cannot read channels they haven't been invited to
    assert!(db.get_channel(carrot_id, b_id).await.is_err());

    db.invite_channel_member(carrot_id, b_id, a_id, ChannelRole::Member)
        .await
        .unwrap();

    db.respond_to_channel_invite(carrot_id, b_id, true)
        .await
        .unwrap();

    let crdb_id = db
        .create_sub_channel("crdb", carrot_id, a_id)
        .await
        .unwrap();
    let livestreaming_id = db
        .create_sub_channel("livestreaming", carrot_id, a_id)
        .await
        .unwrap();
    let replace_id = db
        .create_sub_channel("replace", carrot_id, a_id)
        .await
        .unwrap();
    ...
    assert_channel_tree_matches(
        ...
        channel_tree(&[
            (carrot_id, &[], "carrot"),
            (crdb_id, &[carrot_id], "crdb"),
            (livestreaming_id, &[carrot_id], "livestreaming"),
            (replace_id, &[carrot_id], "replace"),
            (rust_id, &[], "rust"),
    ...
    let set_channel_admin = db
        .set_channel_member_role(carrot_id, a_id, b_id, ChannelRole::Admin)
        .await;
...
async fn test_channel_renames(db: &Arc<Database>) {
    ...
    let carrot_id = db.create_root_channel("carrot", user_1).await.unwrap();

    db.rename_channel(carrot_id, user_1, "#carrot-archive")
        .await
        .unwrap();

    let channel = db.get_channel(carrot_id, user_1).await.unwrap();
    assert_eq!(channel.name, "carrot-archive");

    let non_permissioned_rename = db.rename_channel(carrot_id, user_2, "hacked-lol").await;
    assert!(non_permissioned_rename.is_err());

    let bad_name_rename = db.rename_channel(carrot_id, user_1, "#").await;
...
async fn test_db_channel_moving(db: &Arc<Database>) {
    ...
    let gpui2_id = db
        .create_sub_channel("gpui2", carrot_id, a_id)
        .await
        .unwrap();
    ...
    // sanity check
    //     /- gpui2
    // carrot -- crdb - livestreaming - livestreaming_sub
    let result = db.get_channels_for_user(a_id).await.unwrap();
    assert_channel_tree(
        result.channels,
        &[
            (carrot_id, &[]),
            (crdb_id, &[carrot_id]),
            (livestreaming_id, &[carrot_id, crdb_id]),
            (
                livestreaming_sub_id,
                &[carrot_id, crdb_id, livestreaming_id],
            ),
            (gpui2_id, &[carrot_id]),
        ],
    );
    ...
    assert_channel_tree(
        ...
        &[
            ...
            (crdb_id, &[carrot_id]),
            (livestreaming_id, &[carrot_id, crdb_id]),
            (livestreaming_sub_id, &[carrot_id, crdb_id]),
            (gpui2_id, &[carrot_id]),
        ],
    );
    ...
    // carrot -- gpui2 -- crdb -- livestreaming
    //                      \- livestreaming_sub
    let result = db.get_channels_for_user(a_id).await.unwrap();
    assert_channel_tree(
        result.channels,
        &[
            (carrot_id, &[]),
            (gpui2_id, &[carrot_id]),
            (crdb_id, &[carrot_id, gpui2_id]),
            (livestreaming_id, &[carrot_id, gpui2_id, crdb_id]),
            (livestreaming_sub_id, &[carrot_id, gpui2_id, crdb_id]),
        ],
    );
}
...
async fn test_db_channel_moving_bugs(db: &Arc<Database>) {
    ...
    let carrot_id = db.create_root_channel("carrot", user_id).await.unwrap();

    let projects_id = db
        .create_sub_channel("projects", carrot_id, user_id)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (carrot_id, &[]),
            (projects_id, &[carrot_id]),
            (livestreaming_id, &[carrot_id, projects_id]),
        ],
    );
    ...
    db.move_channel(carrot_id, livestreaming_id, user_id)
        .await
        .unwrap_err();
    let result = db.get_channels_for_user(user_id).await.unwrap();
    assert_channel_tree(
...
async fn test_user_is_channel_participant(db: &Arc<Database>) {
    ...
    let carrot_channel = db.create_root_channel("carrot", admin).await.unwrap();
    let internal_channel_id = db
        .create_sub_channel("active", carrot_channel, admin)
        .await
        .unwrap();
    let public_channel_id = db
        .create_sub_channel("vim", carrot_channel, admin)
        .await
        .unwrap();

    db.set_channel_visibility(
        carrot_channel,
        carrot_collab::db::ChannelVisibility::Public,
        admin,
    )
    .await
    .unwrap();
    ...
    .await
    .unwrap();
    db.invite_channel_member(carrot_channel, member, admin, ChannelRole::Member)
        .await
        .unwrap();
    db.invite_channel_member(carrot_channel, guest, admin, ChannelRole::Guest)
        .await
        .unwrap();

    db.respond_to_channel_invite(carrot_channel, member, true)
        .await
        .unwrap();
    ...
    db.respond_to_channel_invite(carrot_channel, guest, true)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (carrot_channel, &[]),
            (public_channel_id, &[carrot_channel]),
        ],
    );
    ...
    assert_channel_tree(
        ...
        &[
            (carrot_channel, &[]),
            (internal_channel_id, &[carrot_channel]),
            (public_channel_id, &[carrot_channel]),
        ],
    );

    db.set_channel_member_role(carrot_channel, admin, guest, ChannelRole::Banned)
        .await
        .unwrap();
    assert!(
        db.transaction(|tx| async move {
    ...
    db.remove_channel_member(carrot_channel, guest, admin)
        .await
        .unwrap();
    ...
    db.transaction(|tx| async move {
        db.check_user_is_channel_participant(
            &db.get_channel_internal(carrot_channel, &tx).await.unwrap(),
            guest,
            &tx,
        )
        .await
