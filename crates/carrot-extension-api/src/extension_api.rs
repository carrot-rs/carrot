//! The Carrot Rust Extension API allows you write extensions for Carrot in Rust.
...
pub use wit::{
    ...
    carrot::extension::context_server::ContextServerConfiguration,
    carrot::extension::dap::{
    ...
    carrot::extension::github::{
        GithubRelease, GithubReleaseAsset, GithubReleaseOptions, github_release_by_tag_name,
        latest_github_release,
    },
    carrot::extension::nodejs::{
        node_binary_path, npm_install_package, npm_package_installed_version,
        npm_package_latest_version,
    },
    carrot::extension::platform::{Architecture, Os, current_platform},
    carrot::extension::slash_command::{
...
pub mod lsp {
    pub use crate::wit::carrot::extension::lsp::{
        Completion, CompletionKind, InsertTextFormat, Symbol, SymbolKind,
    };
}

/// A result returned from a Carrot extension.
...
pub trait Extension: Send + Sync {
    ...
    /// Locators are entities that convert a Carrot task into a debug scenario.
    ///
    /// They can be provided even by extensions that don't provide a debug adapter.
    /// For all tasks applicable to a given buffer, Carrot will query all locators to find one that can turn the task into a debug scenario.
    ...
    /// `dap_locator_create_scenario` if you're able to do that. Make sure to not fill out `build` field in that case, as that will prevent Carrot from running second phase of resolution in such case.
...
/// Registers the provided type as a Carrot extension.
///
...
macro_rules! register_extension {
    ($extension_type:ty) => {
        ...
        pub extern "C" fn __init_extension() {
            ...
            carrot_extension_api::register_extension(|| {
                Box::new(<$extension_type as carrot_extension_api::Extension>::new())
            });
        }
    };
}
...
#[unsafe(link_section = "carrot:api-version")]
#[doc(hidden)]
