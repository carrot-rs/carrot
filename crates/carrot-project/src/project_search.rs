...
use anyhow::Context;
use carrot_fs::Fs;
use carrot_language::{Buffer, BufferSnapshot};
use carrot_rpc::{AnyProtoClient, proto};
use futures::FutureExt as _;
use futures::{SinkExt, StreamExt, select_biased, stream::FuturesOrdered};
use inazuma::{App, AppContext, AsyncApp, BackgroundExecutor, Entity, Priority, Task};
use inazuma_collections::HashSet;
use parking_lot::Mutex;
use postage::oneshot;
use smol::channel::{Receiver, Sender, bounded, unbounded};

use carrot_worktree::{Entry, ProjectEntryId, Snapshot, Worktree, WorktreeSettings};
use inazuma_util::{ResultExt, maybe, paths::compare_rel_paths, rel_path::RelPath};
...
impl Search {
            ...
            cx.spawn(async move |cx| {
                ...
                let (candidate_searcher, tasks) = match self.kind {
                    ...
                    SearchKind::Remote {
                        ...
                        let cancel_ongoing_search = inazuma_util::defer({
    ...
    async fn grab_buffer_snapshots(
        ...
        find_all_matches_tx: Sender<(
            Entity<Buffer>,
            BufferSnapshot,
            oneshot::Sender<(Entity<Buffer>, Vec<Range<carrot_language::Anchor>>)>,
        )>,
        results: Sender<oneshot::Receiver<(Entity<Buffer>, Vec<Range<carrot_language::Anchor>>)>>,
        mut cx: AsyncApp,
    ) {
        _ = maybe!(async move {
    ...
    async fn ensure_matched_ranges_are_reported_in_order(
        rx: Receiver<oneshot::Receiver<(Entity<Buffer>, Vec<Range<carrot_language::Anchor>>)>>,
...
impl<T: 'static + Send> AdaptiveBatcher<T> {
    pub fn new(cx: &BackgroundExecutor) -> (Self, Receiver<Vec<T>>) {
        ...
        let _batch_task = cx.spawn_with_priority(inazuma::Priority::High, async move {
