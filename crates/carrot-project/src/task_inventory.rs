...
use anyhow::Result;
use carrot_dap::DapRegistry;
use carrot_language::{
    Buffer, ContextLocation, ContextProvider, File, Language, LanguageToolchainStore, Location,
    language_settings::LanguageSettings,
};
use carrot_lsp::{LanguageServerId, LanguageServerName};
use carrot_paths::{debug_task_file_name, task_file_name};
use carrot_task::{
...
use inazuma::{App, AppContext as _, Context, Entity, SharedString, Task, WeakEntity};
use inazuma_collections::{HashMap, HashSet, VecDeque};
use inazuma_settings_framework::{InvalidSettingsError, parse_json_with_comments};
use inazuma_text::{BufferId, Point, ToPoint};
use inazuma_util::{NumericPrefixWithSuffix, ResultExt as _, post_inc, rel_path::RelPath};
...
pub enum TaskSourceKind {
    ...
    UserInput,
    /// Tasks from the worktree's .carrot/task.json
    Worktree {
        id: WorktreeId,
        directory_in_worktree: Arc<RelPath>,
        id_base: Cow<'static, str>,
    },
    /// ~/.config/carrot/task.json - like global files with task definitions, applicable to any path
    AbsPath {
        id_base: Cow<'static, str>,
        abs_path: PathBuf,
    },
...
pub struct TaskContexts {
    ...
    pub latest_selection: Option<inazuma_text::Anchor>,
}
