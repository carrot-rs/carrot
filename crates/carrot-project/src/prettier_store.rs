...
use carrot_lsp::{LanguageServer, LanguageServerId, LanguageServerName};
use carrot_node_runtime::NodeRuntime;
use carrot_paths::default_prettier_dir;
use carrot_prettier::Prettier;
use futures::{
    FutureExt,
    future::{self, Shared},
    stream::FuturesUnordered,
};
use inazuma::{AppContext as _, AsyncApp, Context, Entity, EventEmitter, Task, WeakEntity};
use inazuma_collections::{HashMap, HashSet};
use inazuma_settings_framework::Settings;
use inazuma_util::{ResultExt, TryFutureExt, rel_path::RelPath};
    ...
    pub fn install_default_prettier(
        ...
        let previous_installation_task = match &mut self.default_prettier.prettier {
            PrettierInstallation::NotInstalled {
                ...
                if installation_attempt > carrot_prettier::FAIL_THRESHOLD {
                    *installation_task = None;
                    log::warn!(
...
pub(super) async fn format_with_prettier(
    ...
    buffer: &Entity<Buffer>,
    cx: &mut AsyncApp,
) -> Option<Result<carrot_language::Diff>> {
...
impl PrettierInstance {
    pub fn prettier_task(
        ...
        if self.attempt > carrot_prettier::FAIL_THRESHOLD {
            match prettier_dir {
...
async fn save_prettier_server_file(fs: &dyn Fs) -> anyhow::Result<()> {
    let prettier_wrapper_path = default_prettier_dir().join(carrot_prettier::PRETTIER_SERVER_FILE);
    fs.save(
        &prettier_wrapper_path,
        &inazuma_text::Rope::from(carrot_prettier::PRETTIER_SERVER_JS),
    ...
    .with_context(|| {
        format!(
            ...
            carrot_prettier::PRETTIER_SERVER_FILE
        )
    })?;
    Ok(())
}
...
async fn should_write_prettier_server_file(fs: &dyn Fs) -> bool {
    ...
    prettier_server_file_contents != carrot_prettier::PRETTIER_SERVER_JS
}
