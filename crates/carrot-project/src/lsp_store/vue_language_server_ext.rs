...
use inazuma::{AppContext, WeakEntity};
...
impl carrot_lsp::notification::Notification for VueServerRequest {
    type Params = Vec<(u64, String, serde_json::Value)>;

    const METHOD: &'static str = "tsserver/request";
}

impl carrot_lsp::notification::Notification for TypescriptServerResponse {
                ...
                for (request_id, command, payload) in requests.into_iter() {
                    ...
                    cx.background_spawn(async move {
                        ...
                        let response_body = match response {
                            inazuma_util::ConnectionResult::Result(Ok(result)) => match result {
                            ...
                            inazuma_util::ConnectionResult::Result(Err(error)) => {
                                log::warn!(
