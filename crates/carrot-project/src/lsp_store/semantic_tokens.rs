...
use carrot_language::{Buffer, LanguageName, language_settings::all_language_settings};
use carrot_lsp::{AdapterServerCapabilities, LanguageServerId};
use carrot_rpc::{TypedEnvelope, proto};
use futures::{
    FutureExt as _,
    future::{Shared, join_all},
};
use inazuma::{App, AppContext, AsyncApp, Context, Entity, ReadGlobal as _, SharedString, Task};
...
use inazuma_text::{Anchor, Bias, OffsetUtf16, PointUtf16, Unclipped};
...
pub(super) struct SemanticTokenConfig {
    ...
    global_mode: inazuma_settings_framework::SemanticTokens,
}
...
impl SemanticTokenConfig {
    ...
    pub(super) fn update_global_mode(
        &mut self,
        new_mode: inazuma_settings_framework::SemanticTokens,
    ) -> bool {
            ...
            .entry((server_id, language.cloned()))
        {
            ...
            hash_map::Entry::Vacant(v) => {
                ...
                let legend = match tokens_provider {
                    carrot_lsp::SemanticTokensServerCapabilities::SemanticTokensOptions(opts) => {
                        &opts.legend
                    }
...
pub(super) struct RawSemanticTokens {
    pub servers: HashMap<carrot_lsp::LanguageServerId, Arc<ServerSemanticTokens>>,
}
...
mod tests {
    ...
    use carrot_lsp::SEMANTIC_TOKEN_MODIFIERS;
