...
use carrot_worktree::File;
use futures::{
    FutureExt as _,
    future::{Shared, join_all},
};
use inazuma::{AppContext as _, AsyncApp, Context, Entity, SharedString, Task};
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.background_spawn(async move {
                let resolve_task = lang_server
                    .request::<carrot_lsp::request::ColorPresentationRequest>(
                        carrot_lsp::ColorPresentationParams {
