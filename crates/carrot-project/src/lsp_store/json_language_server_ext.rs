...
use carrot_lsp::LanguageServer;
use inazuma::{App, AsyncApp, Entity, Global, Task, WeakEntity};

use crate::LspStore;

const LOGGER: carrot_log::Logger = carrot_log::scoped!("json-schema");
...
/// In our case, we provide the uris for actions on server startup under the `carrot://schemas/action/{normalize_action_name}` scheme.
...
impl carrot_lsp::request::Request for SchemaContentRequest {
...
impl carrot_lsp::notification::Notification for SchemaContentsChanged {
    const METHOD: &'static str = "json/schemaContent";
    type Params = String;
}

pub fn notify_schemas_changed(lsp_store: Entity<LspStore>, uris: &[String], cx: &App) {
    carrot_log::trace!(LOGGER => "Notifying schema changes for URIs: {:?}", uris);
        ...
        for states in local.language_servers.values() {
            let json_server = match states {
                super::LanguageServerState::Running {
                    adapter, server, ..
                } if adapter.adapter.is_primary_carrot_json_schema_adapter() => server.clone(),
                _ => continue,
            };
    ...
    for server in servers {
        for uri in uris {
            carrot_log::trace!(LOGGER => "Notifying server {NAME} (id {ID:?}) of schema change for URI: {uri:?}",
                NAME = server.name(),
                ID = server.server_id()
            );
...
pub fn register_requests(lsp_store: WeakEntity<LspStore>, language_server: &LanguageServer) {
    language_server
        .on_request::<SchemaContentRequest, _, _>(move |params, cx| {
            ...
            async move {
                carrot_log::trace!(LOGGER => "Handling schema request for {:?}", &params);
                let result = resolution.await;
                match &result {
                    Ok(content) => {carrot_log::trace!(LOGGER => "Schema request resolved with {}B schema", content.len());},
                    Err(err) => {carrot_log::warn!(LOGGER => "Schema request failed: {}", err);},
                }
                result
            }
        })
