...
use inazuma::{App, AppContext as _, AsyncApp, Context, Entity, Task};
use inazuma_collections::HashMap;
use inazuma_settings_framework::Settings as _;
use inazuma_text::{BufferId, Point};
...
pub enum InvalidationStrategy {
    ...
    /// in addition to LSP capabilities, Carrot allows omitting certain hint kinds (defined by the corresponding LSP part: type/parameter/other) and toggling hints.
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.spawn(async move |_, cx| {
                let resolve_task = lang_server
                    .request::<carrot_lsp::request::InlayHintResolveRequest>(
