...
mod environment;
use carrot_buffer_diff::BufferDiff;
use carrot_git::repository::get_git_committer;
...
use carrot_dap::inline_value::{InlineValueLocation, VariableLookupKind, VariableScope};
...
use inazuma_collections::{BTreeSet, HashMap, HashSet, IndexSet};
...
use ::carrot_git::{blame::Blame, status::FileStatus};
use carrot_language::{
...
use carrot_remote::wsl_path_to_windows_path;
use carrot_remote::{RemoteClient, RemoteConnectionOptions};
use carrot_rpc::{
    AnyProtoClient, ErrorCode,
    proto::{LanguageServerPromptResponse, REMOTE_SERVER_PROJECT_ID},
};
use carrot_snippet::Snippet;
pub use carrot_snippet_provider;
use carrot_snippet_provider::SnippetProvider;
use inazuma::{
    App, AppContext, AsyncApp, BorrowAppContext, Context, Entity, EventEmitter, Oklch,
    SharedString, Task, WeakEntity, Window,
};
use inazuma_settings_framework::{
    InvalidSettingsError, RegisterSetting, Settings, SettingsLocation, SettingsStore,
};
use lsp_command::*;
...
use carrot_worktree::{CreatedEntry, Snapshot, Traversal};
pub use carrot_worktree::{
    Entry, EntryKind, FS_WATCH_LATENCY, File, LocalWorktree, PathChange, ProjectEntryId,
    UpdatedEntriesSet, UpdatedGitRepositoriesSet, Worktree, WorktreeId, WorktreeSettings,
};
use inazuma_text::{Anchor, BufferId, OffsetRangeExt, Point, Rope};
use inazuma_util::{
...
pub struct Project {
    ...
    collab_client: Arc<carrot_client::Client>,
    ...
    client_subscriptions: Vec<carrot_client::Subscription>,
...
pub enum Event {
    ...
    // [`carrot_lsp::notification::DidOpenTextDocument`] was sent to this server using the buffer data.
    // Carrot's buffer-related data is updated accordingly.
    ...
    SnippetEdit(BufferId, Vec<(carrot_lsp::Range, Snippet)>),
...
impl ProjectPath {
    pub fn from_file(value: &dyn carrot_language::File, cx: &App) -> Self {
        ProjectPath {
...
pub struct InlayHint {
    pub position: carrot_language::Anchor,
...
pub enum CompletionSource {
    Lsp {
        ...
        lsp_completion: Box<carrot_lsp::CompletionItem>,
        /// A set of defaults for this completion item.
        lsp_defaults: Option<Arc<carrot_lsp::CompletionListItemDefaults>>,
...
impl CompletionSource {
    ...
    pub fn lsp_completion(
        &self,
        apply_defaults: bool,
    ) -> Option<Cow<'_, carrot_lsp::CompletionItem>> {
        if let Self::Lsp {
            lsp_completion,
            lsp_defaults,
            ..
                    ...
                    || default_insert_text_mode.is_some()
                {
                    ...
                    if completion_with_defaults.text_edit.is_none() {
                        match default_edit_range {
                            Some(carrot_lsp::CompletionListItemDefaultsEditRange::Range(range)) => {
                                completion_with_defaults.text_edit = Some(
                                    carrot_lsp::CompletionTextEdit::Edit(carrot_lsp::TextEdit {
                                        range: *range,
                            ...
                            Some(
                                carrot_lsp::CompletionListItemDefaultsEditRange::InsertAndReplace {
                                    insert,
                                    replace,
                                },
                            ) => {
...
pub enum LspAction {
    ...
    Action(Box<carrot_lsp::CodeAction>),
    /// A command data to run as an action.
    Command(carrot_lsp::Command),
    /// A code lens data to run as an action.
    CodeLens(carrot_lsp::CodeLens),
}
...
impl LspAction {
    ...
    pub fn action_kind(&self) -> Option<carrot_lsp::CodeActionKind> {
        match self {
            Self::Action(action) => action.kind.clone(),
            Self::Command(_) => Some(carrot_lsp::CodeActionKind::new("command")),
            Self::CodeLens(_) => Some(carrot_lsp::CodeActionKind::new("code lens")),
        }
    }

    fn edit(&self) -> Option<&carrot_lsp::WorkspaceEdit> {
        match self {
    ...
    fn command(&self) -> Option<&carrot_lsp::Command> {
        match self {
...
pub enum ResolveState {
    Resolved,
    CanResolve(LanguageServerId, Option<carrot_lsp::LSPAny>),
    Resolving,
}
impl InlayHint {
...
pub struct InlayHintLabelPart {
    ...
    pub location: Option<(LanguageServerId, carrot_lsp::Location)>,
}
...
pub struct DocumentHighlight {
    pub range: Range<carrot_language::Anchor>,
...
pub struct Hover {
    ...
    pub range: Option<Range<carrot_language::Anchor>>,
...
pub struct DocumentColor {
    pub lsp_range: carrot_lsp::Range,
    pub color: carrot_lsp::Color,
    pub resolved: bool,
...
pub struct ColorPresentation {
    ...
    pub text_edit: Option<carrot_lsp::TextEdit>,
    pub additional_text_edits: Vec<carrot_lsp::TextEdit>,
}
...
pub const DEFAULT_COMPLETION_CONTEXT: CompletionContext = CompletionContext {
    trigger_kind: carrot_lsp::CompletionTriggerKind::INVOKED,
...
/// Whether to disable all AI features in Carrot.
///
/// Default: false
...
impl inazuma_settings_framework::Settings for DisableAiSettings {
...
impl DisableAiSettings {
    ...
    /// allowing `disable_ai` to be configured per-project in `.carrot/settings.json`.
    ...
    pub fn is_ai_disabled_for_file(
        file: Option<&Arc<dyn carrot_language::File>>,
        cx: &App,
    ) -> bool {
        let location = file.map(|f| inazuma_settings_framework::SettingsLocation {
...
impl Project {
        ...
        cx.new(|cx: &mut Context<Self>| {
            ...
            let context_server_store = cx.new(|cx| {
                ContextServerStore::remote(
                    carrot_rpc::proto::REMOTE_SERVER_PROJECT_ID,
                    remote.clone(),
                    worktree_store.clone(),
                    Some(weak_self.clone()),
                    cx,
    ...
    #[cfg(feature = "test-support")]
    pub fn client_subscriptions(&self) -> &Vec<carrot_client::Subscription> {
    ...
    pub async fn example(
        ...
        let client = cx.update(|cx| carrot_client::Client::new(clock, http_client.clone(), cx));
    ...
    #[inline]
    pub fn remote_connection_state(&self, cx: &App) -> Option<carrot_remote::ConnectionState> {
        self.remote_client
            .as_ref()
    ...
    pub fn default_path_list(&self, cx: &App) -> PathList {
        ...
        if worktree_roots.is_empty() {
            PathList::new(&[carrot_paths::home_dir().as_path()])
        } else {
                        ...
                        self.lsp_store.update(cx, |lsp_store, _| {
                                ...
                                .get_mut(language_server_id)
                            {
                                ...
                                language_server_status.workspace_folders = update
                                    .workspace_folders
                                    .iter()
                                    .filter_map(|uri_str| carrot_lsp::Uri::from_str(uri_str).ok())
                                    .collect();
                            }
                        });
                    }
    ...
    fn on_remote_client_event(
        ...
        event: &carrot_remote::RemoteClientEvent,
        cx: &mut Context<Self>,
    ) {
        match event {
            &carrot_remote::RemoteClientEvent::Disconnected { server_not_running } => {
                self.worktree_store.update(cx, |store, cx| {
                    store.disconnected_from_host(cx);
                });
                self.buffer_store.update(cx, |buffer_store, cx| {
                ...
                Err(InvalidSettingsError::Tasks { message, path }) => {
                    ...
                    cx.emit(Event::Toast {
                        ...
                        link: Some(ToastLink {
                            ...
                            url: "https://carrot.dev/docs/tasks",
                        }),
                        message,
                    });
                }
    ...
    fn on_buffer_event(
        ...
        match event {
            ...
            BufferEvent::Operation {
                ...
                let operation = carrot_language::proto::serialize_operation(operation);
    ...
    pub fn resolve_path_in_buffer(
        ...
        if inazuma_util::paths::is_absolute(path, self.path_style(cx)) || path.starts_with("~") {
            ...
            let send_updates_for_buffers = this.update(cx, |this, cx| {
                response
                    ...
                    .map(|buffer| {
                        ...
                        let remote_version =
                            carrot_language::proto::deserialize_version(&buffer.version);
            ...
            location
                ...
                .update(cx, |buffer, cx| {
                    buffer.set_agent_selections(
                        Arc::from([carrot_language::Selection {
                            id: 0,
                            start: location.position,
                            end: location.position,
                            reversed: false,
                            goal: carrot_language::SelectionGoal::None,
                        }]),
                        false,
                        CursorShape::Hollow,
                        cx,
...
impl Completion {
    pub fn kind(&self) -> Option<CompletionItemKind> {
        self.source
            // `carrot_lsp::CompletionListItemDefaults` has no `kind` field
    ...
    pub fn sort_key(&self) -> (usize, &str) {
        ...
        let kind_key = self
            ...
            .and_then(|lsp_completion_kind| match lsp_completion_kind {
                carrot_lsp::CompletionItemKind::KEYWORD => Some(0),
                carrot_lsp::CompletionItemKind::VARIABLE => Some(1),
                carrot_lsp::CompletionItemKind::CONSTANT => Some(2),
                carrot_lsp::CompletionItemKind::PROPERTY => Some(3),
                _ => None,
            })
    ...
    pub fn is_snippet(&self) -> bool {
        self.source
            // `carrot_lsp::CompletionListItemDefaults` has `insert_text_format` field
            .lsp_completion(true)
            .is_some_and(|lsp_completion| {
                lsp_completion.insert_text_format == Some(carrot_lsp::InsertTextFormat::SNIPPET)
            })
    }
...
fn proto_to_prompt(level: proto::language_server_prompt_request::Level) -> inazuma::PromptLevel {
    match level {
        proto::language_server_prompt_request::Level::Info(_) => inazuma::PromptLevel::Info,
        proto::language_server_prompt_request::Level::Warning(_) => inazuma::PromptLevel::Warning,
        proto::language_server_prompt_request::Level::Critical(_) => inazuma::PromptLevel::Critical,
    }
}

fn provide_inline_values(
    captures: impl Iterator<Item = (Range<usize>, carrot_language::DebuggerTextObject)>,
    snapshot: &carrot_language::BufferSnapshot,
    max_row: usize,
    ...
    for (capture_range, capture_kind) in captures {
        match capture_kind {
            carrot_language::DebuggerTextObject::Variable => {
            ...
            carrot_language::DebuggerTextObject::Scope => {
