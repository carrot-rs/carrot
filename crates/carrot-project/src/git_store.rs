...
use carrot_askpass::{AskPassDelegate, EncryptedPassword, IKnowWhatIAmDoingAndIHaveReadTheDocs};
use carrot_buffer_diff::{BufferDiff, BufferDiffEvent};
use carrot_client::ProjectId;
use carrot_fs::Fs;
use carrot_git::{
...
use inazuma_sum_tree::{Edit, SumTree, TreeMap};
...
struct BufferGitState {
    ...
    oid_diffs: HashMap<Option<carrot_git::Oid>, WeakEntity<BufferDiff>>,
    ...
    oid_texts: HashMap<carrot_git::Oid, Arc<str>>,
    head_changed: bool,
    index_changed: bool,
...
enum DiffKind {
    Unstaged,
    Uncommitted,
    SinceOid(Option<carrot_git::Oid>),
}
...
impl GitStore {
    ...
    pub fn file_history(
        ...
        path: RepoPath,
        cx: &mut App,
    ) -> Task<Result<carrot_git::repository::FileHistory>> {
        ...
        for update in updated_git_repositories.iter() {
            ...
            } = update
            {
                let original_repo_abs_path: Arc<Path> =
                    carrot_git::repository::original_repo_path_from_common_dir(common_dir_abs_path)
                        .into();
    ...
    fn on_buffer_diff_event(
        &mut self,
        diff: Entity<carrot_buffer_diff::BufferDiff>,
        ...
        let options = envelope
            ...
            .map(|_| match envelope.payload.options() {
                proto::push::PushOptions::SetUpstream => {
                    carrot_git::repository::PushOptions::SetUpstream
                }
                proto::push::PushOptions::Force => carrot_git::repository::PushOptions::Force,
            });
    ...
    async fn handle_get_blob_content(
        ...
        let oid = carrot_git::Oid::from_str(&request.payload.oid)?;
        let repository_id = RepositoryId(request.payload.repository_id);
        let content = this
            .update(&mut cx, |this, cx| {
                let repository = this.repositories().get(&repository_id)?;
...
impl BufferGitState {
    ...
    fn oid_diff(&self, oid: Option<carrot_git::Oid>) -> Option<Entity<BufferDiff>> {
...
impl RepositorySnapshot {
    ...
    /// created from.
    ///
    /// For example, if you had both `~/code/carrot` and `~/code/worktrees/carrot-2`,
    /// then `~/code/carrot` is the main worktree and `~/code/worktrees/carrot-2` is a linked worktree.
...
impl Repository {
    ...
    pub fn file_history(
        &mut self,
        path: RepoPath,
    ) -> oneshot::Receiver<Result<carrot_git::repository::FileHistory>> {
            ...
            match git_repo {
                ...
                RepositoryState::Remote(RemoteRepositoryState { client, project_id }) => {
                    let response = client
                        ...
                        .await?;
                    Ok(carrot_git::repository::FileHistory {
                        entries: response
                            .entries
                            .into_iter()
                            .map(|entry| carrot_git::repository::FileHistoryEntry {
                                sha: entry.sha.into(),
    ...
    pub fn commit(
        ...
        let askpass_id = inazuma_util::post_inc(&mut self.latest_askpass_id);
            ...
            .chain(
                update
                    ...
                    .filter_map(|updated_status| {
                        Some(inazuma_sum_tree::Edit::Insert(
                            updated_status.try_into().log_err()?,
                        ))
                    }),
            )
            .collect::<Vec<_>>();
    ...
    fn spawn_local_git_worker(
        ...
        cx.spawn(async move |_, cx| {
            ...
            {
                carrot_git_hosting_providers::register_additional_providers(
...
pub async fn resolve_git_worktree_to_main_repo(fs: &dyn Fs, path: &Path) -> Option<PathBuf> {
    ...
    let common_dir = fs
        ...
        .await
        .ok()?;
    Some(carrot_git::repository::original_repo_path_from_common_dir(
        &common_dir,
    ))
}
...
pub fn worktrees_directory_for_repo(
    ...
    let resolved = inazuma_util::normalize_path(&joined);
...
fn serialize_blame_buffer_response(
    blame: Option<carrot_git::blame::Blame>,
...
fn deserialize_blame_buffer_response(
...
) -> Option<carrot_git::blame::Blame> {
    let response = response.blame_response?;
    let entries = response
        .entries
        .into_iter()
        .filter_map(|entry| {
            Some(carrot_git::blame::BlameEntry {
                sha: carrot_git::Oid::from_bytes(&entry.sha).ok()?,
    ...
    let messages = response
        ...
        .filter_map(|message| {
            Some((
                carrot_git::Oid::from_bytes(&message.oid).ok()?,
                message.message,
            ))
        })
        .collect::<HashMap<_, _>>();
...
fn branch_to_proto(branch: &carrot_git::repository::Branch) -> proto::Branch {
    proto::Branch {
...
fn worktree_to_proto(worktree: &carrot_git::repository::Worktree) -> proto::Worktree {
    proto::Worktree {
...
fn proto_to_worktree(proto: &proto::Worktree) -> carrot_git::repository::Worktree {
    carrot_git::repository::Worktree {
...
fn proto_to_branch(proto: &proto::Branch) -> carrot_git::repository::Branch {
    carrot_git::repository::Branch {
        is_head: proto.is_head,
        ref_name: proto.ref_name.clone().into(),
        upstream: proto
            .upstream
            .as_ref()
            .map(|upstream| carrot_git::repository::Upstream {
                ref_name: upstream.ref_name.to_string().into(),
                tracking: upstream
                    .tracking
                    .as_ref()
                    .map(|tracking| {
                        carrot_git::repository::UpstreamTracking::Tracked(UpstreamTrackingStatus {
                            ahead: tracking.ahead as u32,
                            behind: tracking.behind as u32,
                        })
                    })
                    .unwrap_or(carrot_git::repository::UpstreamTracking::Gone),
            }),
        most_recent_commit: proto.most_recent_commit.as_ref().map(|commit| {
            carrot_git::repository::CommitSummary {
