...
use carrot_dap::{DapLocator, DebugRequest, adapters::DebugAdapterName};
use inazuma::{BackgroundExecutor, SharedString};

use carrot_task::{DebugScenario, SpawnInTerminal, TaskTemplate, VariableName};
...
impl DapLocator for PythonLocator {
    ...
    async fn create_scenario(
        ...
        let program_position = mod_name
            .is_none()
            .then(|| {
                let carrot_file = VariableName::File.template_value_with_whitespace();
                build_config.args.iter().position(|arg| *arg == carrot_file)
            })
            .flatten();
