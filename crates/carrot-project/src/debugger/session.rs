...
use base64::Engine;
use carrot_dap::adapters::{DebugAdapterBinary, DebugAdapterName};
use carrot_dap::messages::Response;
use carrot_dap::requests::{Request, RunInTerminal, StartDebugging};
use carrot_dap::transport::TcpTransport;
use carrot_dap::{
...
use carrot_remote::RemoteClient;
use carrot_task::SharedTaskContext;
use carrot_worktree::Worktree;
...
use inazuma_text::{PointUtf16, ToPointUtf16};
use inazuma_util::command::Stdio;
use inazuma_util::command::new_command;
use inazuma_util::{ResultExt, debug_panic, maybe};
...
pub struct StackFrame {
    pub dap: carrot_dap::StackFrame,
    pub scopes: Vec<carrot_dap::Scope>,
}

impl From<carrot_dap::StackFrame> for StackFrame {
    fn from(stack_frame: carrot_dap::StackFrame) -> Self {
        Self {
            scopes: vec![],
            dap: stack_frame,
        }
...
#[derive(Debug, Clone)]
pub struct Thread {
    dap: carrot_dap::Thread,
    stack_frames: Vec<StackFrame>,
    stack_frames_error: Option<SharedString>,
    _has_stopped: bool,
}

impl From<carrot_dap::Thread> for Thread {
    fn from(dap: carrot_dap::Thread) -> Self {
        Self {
            dap,
...
pub struct DataBreakpointState {
    pub dap: carrot_dap::DataBreakpoint,
    pub is_enabled: bool,
...
fn client_source(abs_path: &Path) -> carrot_dap::Source {
    carrot_dap::Source {
        name: abs_path
            .file_name()
...
impl RunningMode {
    ...
    ) -> Task<Result<Vec<carrot_dap::Breakpoint>>> {
    ...
    fn initialize_sequence(
        ...
        let launch = match raw.request {
            carrot_dap::StartDebuggingRequestArgumentsRequest::Launch => self.request(Launch {
                raw: raw.configuration,
            }),
            carrot_dap::StartDebuggingRequestArgumentsRequest::Attach => self.request(Attach {
                raw: raw.configuration,
            }),
        };
    ...
    where
        <R::DapRequest as carrot_dap::requests::Request>::Response: 'static,
        <R::DapRequest as carrot_dap::requests::Request>::Arguments: 'static + Send,
    {
...
pub struct SessionSnapshot {
    ...
    variables: HashMap<VariableReference, Vec<carrot_dap::Variable>>,
    stack_frames: IndexMap<StackFrameId, StackFrame>,
    locations: HashMap<u64, carrot_dap::LocationsResponse>,
    modules: Vec<carrot_dap::Module>,
    loaded_sources: Vec<carrot_dap::Source>,
}
...
pub struct Session {
    ...
    output:
        Box<circular_buffer::CircularBuffer<MAX_TRACKED_OUTPUT_EVENTS, carrot_dap::OutputEvent>>,
...
impl CompletionsQuery {
    pub fn new(
        buffer: &carrot_language::Buffer,
        cursor_position: carrot_language::Anchor,
        frame_id: Option<u64>,
    ) -> Self {
...
impl Session {
    ...
    pub fn on_app_quit(&mut self, cx: &mut Context<Self>) -> Task<()> {
        ...
        cx.background_spawn(async move {
            if supports_terminate {
                client
                    .request::<carrot_dap::requests::Terminate>(carrot_dap::TerminateArguments {
                        restart: Some(false),
                    })
                    .await
                    .ok();
                ...
                client
                    .request::<carrot_dap::requests::Disconnect>(carrot_dap::DisconnectArguments {
                        restart: Some(false),
        ...
        cx.spawn(async move |this, cx| {
            while let Some(output) = rx.next().await {
                this.update(cx, |this, _| {
                    let event = carrot_dap::OutputEvent {
                        category: None,
                        output,
                        group: None,
    ...
    fn handle_start_debugging_request(
        &mut self,
        request: carrot_dap::messages::Request,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
    ...
    fn handle_run_in_terminal_request(
        ...
        let request_args = match serde_json::from_value::<RunInTerminalRequestArguments>(
            ...
            Err(error) => {
                return cx.spawn(async move |session, cx| {
                    let error = serde_json::to_value(carrot_dap::ErrorResponse {
                        error: Some(carrot_dap::Message {
                            id: request.seq,
        ...
        cx.spawn(async move |session, cx| {
            let result = inazuma_util::maybe!(async move {
            ...
            let (success, body) = match result {
                Ok(pid) => (
                    true,
                    serde_json::to_value(carrot_dap::RunInTerminalResponse {
                        process_id: None,
                ...
                Err(error) => (
                    false,
                    serde_json::to_value(carrot_dap::ErrorResponse {
                        error: Some(carrot_dap::Message {
                            id: seq,
                            format: error.to_string(),
                            variables: None,
    ...
    pub fn output(
        &self,
        since: OutputToken,
    ) -> (impl Iterator<Item = &carrot_dap::OutputEvent>, OutputToken) {
    ...
    pub(crate) fn handle_dap_event(&mut self, event: Box<Events>, cx: &mut Context<Self>) {
        match *event {
            ...
            Events::Thread(event) => {
                ...
                match event.reason {
                    carrot_dap::ThreadEventReason::Started => {
                        self.active_snapshot
                            .thread_states
                            .continue_thread(thread_id);
                    }
                    carrot_dap::ThreadEventReason::Exited => {
            ...
            Events::Module(event) => {
                match event.reason {
                    carrot_dap::ModuleEventReason::New => {
                        self.active_snapshot.modules.push(event.module);
                    }
                    carrot_dap::ModuleEventReason::Changed => {
                    ...
                    carrot_dap::ModuleEventReason::Removed => {
                        self.active_snapshot
                            .modules
    ...
    pub fn threads(&mut self, cx: &mut Context<Self>) -> Vec<(carrot_dap::Thread, ThreadStatus)> {
        self.fetch(
    ...
    pub fn completions(
        ...
        cx: &mut Context<Self>,
    ) -> Task<Result<Vec<carrot_dap::CompletionItem>>> {
    ...
    pub fn scopes(&mut self, stack_frame_id: u64, cx: &mut Context<Self>) -> &[carrot_dap::Scope] {
    ...
    pub fn data_breakpoint_info(
        ...
        mode: Option<String>,
        cx: &mut Context<Self>,
    ) -> Task<Option<carrot_dap::DataBreakpointInfoResponse>> {
    ...
    pub fn location(
        ...
        reference: u64,
        cx: &mut Context<Self>,
    ) -> Option<carrot_dap::LocationsResponse> {
        self.fetch(
...
async fn spawn_companion(
    ...
    cx: &mut AsyncApp,
) -> Result<(u16, inazuma_util::command::Child)> {
...
async fn get_or_install_companion(node: NodeRuntime, cx: &mut AsyncApp) -> Result<PathBuf> {
    const PACKAGE_NAME: &str = "@carrot-rs/js-debug-companion-cli";
    ...
    let dir = carrot_paths::debug_adapters_dir().join("js-debug-companion");
