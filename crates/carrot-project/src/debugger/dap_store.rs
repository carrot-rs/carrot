...
use carrot_language::{Buffer, LanguageToolchainStore};
...
use inazuma_settings_framework::InlayHintKind;
...
use carrot_task::{DebugScenario, SharedTaskContext, SpawnInTerminal, TaskTemplate};
use carrot_worktree::Worktree;
use inazuma_settings_framework::{Settings, SettingsLocation, WorktreeId};
...
impl DapStore {
                ...
                cx.spawn(async move |_, cx| {
                    ...
                    if let Some(c) = binary.connection {
                        ...
                        if remote.read_with(cx, |remote, _cx| remote.shares_network_interface()) {
                            ...
                            port = carrot_dap::transport::TcpTransport::unused_port(host).await?;
    ...
    pub fn new_session(
        ...
        let session_id = SessionId(inazuma_util::post_inc(&mut self.next_session_id));
    ...
    pub fn resolve_inline_value_locations(
        ...
        inline_value_locations: Vec<carrot_dap::inline_value::InlineValueLocation>,
        cx: &mut Context<Self>,
        ...
        cx.spawn(async move |_, cx| {
            ...
            for inline_value_location in inline_value_locations.iter() {
                let point = snapshot.point_to_point_utf16(carrot_language::Point::new(
                    ...
                    VariableLookupKind::Variable => {
                        let variable_search =
                            if inline_value_location.scope
                                == carrot_dap::inline_value::VariableScope::Local
                            {
...
#[async_trait]
impl carrot_dap::adapters::DapDelegate for DapAdapterDelegate {
