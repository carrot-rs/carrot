...
pub struct WorktreeStore {
    ...
    initial_scan_complete: (carrot_watch::Sender<bool>, carrot_watch::Receiver<bool>),
...
impl WorktreeStore {
    ...
    pub fn local(
        ...
        Self {
            ...
            retain_worktrees,
            initial_scan_complete: carrot_watch::channel(true),
    ...
    pub fn next_worktree_id(&self) -> impl Future<Output = Result<WorktreeId>> + use<> {
        let strategy = match (&self.state, &self.downstream_client) {
            ...
            // we are just a local carrot project, we can assign ids
                    ...
                    async move |fs: &dyn Fs, old_path: &Path, new_path: &Path, overwrite| {
                        fs.rename(
                            ...
                            carrot_fs::RenameOptions {
                                overwrite,
                                ..carrot_fs::RenameOptions::default()
                            },
                        )
                        .await
    ...
    pub fn add(&mut self, worktree: &Entity<Worktree>, cx: &mut Context<Self>) {
        ...
        cx.subscribe(worktree, |_, worktree, event, cx| {
            ...
            match event {
                ...
                carrot_worktree::Event::UpdatedGitRepositories(set) => {
                    cx.emit(WorktreeStoreEvent::WorktreeUpdatedGitRepositories(
                        worktree_id,
                        set.clone(),
                    ));
                }
                carrot_worktree::Event::DeletedEntry(id) => {
