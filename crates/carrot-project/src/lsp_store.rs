...
use carrot_client::{TypedEnvelope, proto};
use carrot_http_client::HttpClient;
use carrot_language::{
...
use carrot_node_runtime::read_package_installed_version;
use carrot_rpc::{
...
use inazuma_collections::{BTreeMap, BTreeSet, HashMap, HashSet, btree_map};
use inazuma_settings_framework::{Settings, SettingsLocation, SettingsStore};
use inazuma_sum_tree::Dimensions;
use inazuma_text::{Anchor, BufferId, LineEnding, OffsetRangeExt, ToPoint as _};
...
pub use carrot_fs::*;
pub use carrot_language::Location;
#[cfg(any(test, feature = "test-support"))]
pub use carrot_prettier::FORMAT_SUFFIX as TEST_PRETTIER_FORMAT_SUFFIX;
...
impl ProgressToken {
    fn from_lsp(value: carrot_lsp::NumberOrString) -> Self {
        match value {
            carrot_lsp::NumberOrString::Number(number) => Self::Number(number),
            carrot_lsp::NumberOrString::String(string) => Self::String(SharedString::new(string)),
        }
    }

    fn to_lsp(&self) -> carrot_lsp::NumberOrString {
        match self {
            Self::Number(number) => carrot_lsp::NumberOrString::Number(*number),
            Self::String(string) => carrot_lsp::NumberOrString::String(string.to_string()),
        }
    }
...
pub struct LocalLspStore {
    ...
    _subscription: inazuma::Subscription,
    ...
    workspace_pull_diagnostics_result_ids: HashMap<
        ...
        HashMap<Option<SharedString>, HashMap<PathBuf, Option<SharedString>>>,
    >,
    restricted_worktrees_tasks: HashMap<WorktreeId, (Subscription, carrot_watch::Receiver<bool>)>,
...
impl LocalLspStore {
                ...
                if can_trust {
                    ...
                    match self.restricted_worktrees_tasks.entry(worktree_id) {
                        ...
                        hash_map::Entry::Vacant(v) => {
                            let (mut tx, rx) = carrot_watch::channel::<bool>(false);
            ...
            cx.spawn(async move |cx| {
                let result = async {
                    ...
                    let did_change_configuration_params =
                        carrot_lsp::DidChangeConfigurationParams {
                    ...
                    language_server.notify::<carrot_lsp::notification::DidChangeConfiguration>(
    ...
    fn get_language_server_binary(
        ...
        wait_until_worktree_trust: Option<carrot_watch::Receiver<bool>>,
        cx: &mut App,
    ...
    fn setup_lsp_messages(
        ...
        language_server
            .on_notification::<carrot_lsp::notification::PublishDiagnostics, _>({
                let adapter = adapter.clone();
            ...
            .detach();
        language_server
            .on_request::<carrot_lsp::request::WorkspaceConfiguration, _, _>({
        ...
        language_server
            .on_request::<carrot_lsp::request::WorkspaceFoldersRequest, _, _>({
                let this = lsp_store.clone();
                move |_, cx| {
                    let this = this.clone();
                    let cx = cx.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::WorkDoneProgressCreate, _, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::RegisterCapability, _, _>({
        ...
        language_server
            .on_request::<carrot_lsp::request::UnregisterCapability, _, _>({
                let lsp_store = lsp_store.clone();
                move |params, cx| {
                    let lsp_store = lsp_store.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::ApplyWorkspaceEdit, _, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let mut cx = cx.clone();
                    let this = this.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::InlayHintRefreshRequest, _, _>({
                let lsp_store = lsp_store.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::CodeLensRefresh, _, _>({
                let this = lsp_store.clone();
                move |(), cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::SemanticTokensRefresh, _, _>({
                let lsp_store = lsp_store.clone();
                let request_id = Arc::new(AtomicUsize::new(0));
                move |(), cx| {
                    let lsp_store = lsp_store.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::WorkspaceDiagnosticRefresh, _, _>({
                let this = lsp_store.clone();
                move |(), cx| {
                    let this = this.clone();
                    let mut cx = cx.clone();
        ...
        language_server
            .on_request::<carrot_lsp::request::ShowMessageRequest, _, _>({
                let this = lsp_store.clone();
                ...
                move |params, cx| {
                    ...
                    async move {
                        ...
                        let level = match params.typ {
                            carrot_lsp::MessageType::ERROR => PromptLevel::Critical,
                            carrot_lsp::MessageType::WARNING => PromptLevel::Warning,
                            _ => PromptLevel::Info,
                        };
                        ...
                        if did_update {
                            ...
                            if let Some(ref selected_action) = response {
                                let context = carrot_language::PromptResponseContext {
                                    message,
            ...
            .detach();
        language_server
            .on_notification::<carrot_lsp::notification::ShowMessage, _>({
                let this = lsp_store.clone();
                let name = name.to_string();
                move |params, cx| {
                    let this = this.clone();
        ...
        language_server
            .on_notification::<carrot_lsp::notification::Progress, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    if let Some(this) = this.upgrade() {
                        this.update(cx, |this, cx| {
        ...
        language_server
            .on_notification::<carrot_lsp::notification::LogMessage, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    if let Some(this) = this.upgrade() {
                        this.update(cx, |_, cx| {
        ...
        language_server
            .on_notification::<carrot_lsp::notification::LogTrace, _>({
                let this = lsp_store.clone();
                move |params, cx| {
                    let mut cx = cx.clone();
                    if let Some(this) = this.upgrade() {
        ...
        for buffer in &buffers {
            ...
            buffer.handle.update(cx, |buffer, cx| {
                let Some(formatting_transaction) =
                    ...
                    carrot_log::warn!(logger => "no formatting transaction");
                    return;
                };
                if formatting_transaction.edit_ids.is_empty() {
                    carrot_log::debug!(logger => "no changes made while formatting");
                    buffer.forget_transaction(formatting_transaction_id);
                    return;
                }
                if !push_to_history {
                    carrot_log::trace!(logger => "forgetting format transaction");
    ...
    async fn format_buffer_locally(
        ...
        if settings.remove_trailing_whitespace_on_save {
            carrot_log::trace!(logger => "removing trailing whitespace");
            let diff = buffer
                .handle
        ...
        if settings.ensure_final_newline_on_save {
            carrot_log::trace!(logger => "ensuring final newline");
            extend_formatting_transaction(buffer, formatting_transaction_id, cx, |buffer, cx| {
        ...
        if should_run_code_actions_on_format {
            ...
            if have_code_actions_to_run_on_format {
                carrot_log::trace!(logger => "going to run code actions on format");
        ...
        for formatter in formatters {
            let formatter = if formatter == &Formatter::Auto {
                if settings.prettier.allowed {
                    carrot_log::trace!(logger => "Formatter set to auto: defaulting to prettier");
                    &Formatter::Prettier
                } else {
                    carrot_log::trace!(logger => "Formatter set to auto: defaulting to primary language server");
                    &Formatter::LanguageServer(
                        inazuma_settings_framework::LanguageServerFormatterSpecifier::Current,
                    )
                }
            } else {
                formatter
            ...
            .await
            {
                carrot_log::error!(logger => "Formatter failed, skipping: {err:#}");
            }
        }
    ...
    async fn apply_formatter(
        ...
        match formatter {
            Formatter::None => {
                carrot_log::trace!(logger => "skipping formatter 'none'");
                return Ok(());
            }
            Formatter::Auto => {
            ...
            Formatter::Prettier => {
                let logger = carrot_log::scoped!(logger => "prettier");
                carrot_log::trace!(logger => "formatting");
                let _timer = carrot_log::time!(logger => "Formatting buffer via prettier");
                ...
                let Some(diff) = diff else {
                    carrot_log::trace!(logger => "No changes");
                    return Ok(());
                };
            ...
            Formatter::External { command, arguments } => {
                let logger = carrot_log::scoped!(logger => "command");
                carrot_log::trace!(logger => "formatting");
                let _timer = carrot_log::time!(logger => "Formatting buffer via external command");
            ...
            Formatter::LanguageServer(specifier) => {
                let logger = carrot_log::scoped!(logger => "language-server");
                carrot_log::trace!(logger => "formatting");
                let _timer = carrot_log::time!(logger => "Formatting buffer using language server");

                let Some(buffer_path_abs) = buffer.abs_path.as_ref() else {
                    carrot_log::warn!(logger => "Cannot format buffer that is not backed by a file on disk using language servers. Skipping");
                    return Ok(());
                };

                let language_server = match specifier {
                    inazuma_settings_framework::LanguageServerFormatterSpecifier::Specific {
                        name,
                ...
                let edits = if let Some(ranges) = buffer.ranges.as_ref() {
                    carrot_log::trace!(logger => "formatting ranges");
                    ...
                    carrot_log::trace!(logger => "formatting full");
                    Self::format_via_lsp(
                        &lsp_store,
                        &buffer.handle,
                        buffer_path_abs,
            ...
            Formatter::CodeAction(code_action_name) => {
                let logger = carrot_log::scoped!(logger => "code-actions");
                carrot_log::trace!(logger => "formatting");
                let _timer = carrot_log::time!(logger => "Formatting buffer using code actions");

                let Some(buffer_path_abs) = buffer.abs_path.as_ref() else {
                    carrot_log::warn!(logger => "Cannot format buffer that is not backed by a file on disk using code actions. Skipping");
                    return Ok(());
                };

                let code_action_kind: CodeActionKind = code_action_name.clone().into();
                carrot_log::trace!(logger => "Attempting to resolve code actions {:?}", &code_action_kind);
                ...
                if actions_and_servers.is_empty() {
                    carrot_log::warn!(logger => "No code actions were resolved, continuing");
                    return Ok(());
                }
                ...
                'actions: for (mut action, server_index) in actions_and_servers {
                    ...
                    carrot_log::trace!(logger => "Executing {}", describe_code_action(&action));
                    ...
                    if let Some(edit) = action.lsp_action.edit().cloned() {
                        ...
                        if let Some(document_changes) = edit.document_changes {
                            match document_changes {
                                carrot_lsp::DocumentChanges::Edits(edits) => operations.extend(
                                    edits
                                        .into_iter()
                                        .map(carrot_lsp::DocumentChangeOperation::Edit),
                                ),
                                carrot_lsp::DocumentChanges::Operations(ops) => operations = ops,
                            }
                        } else if let Some(changes) = edit.changes {
                            operations.extend(changes.into_iter().map(|(uri, edits)| {
                                carrot_lsp::DocumentChangeOperation::Edit(
                                    carrot_lsp::TextDocumentEdit {
                                        text_document:
                                            carrot_lsp::OptionalVersionedTextDocumentIdentifier {
                                                uri,
                                                version: None,
                                            },
                        ...
                        for operation in operations {
                            let op = match operation {
                                carrot_lsp::DocumentChangeOperation::Edit(op) => op,
                                carrot_lsp::DocumentChangeOperation::Op(_) => {
                                    carrot_log::warn!(
                                        logger =>
                        ...
                        if edits.is_empty() {
                            carrot_log::warn!(logger => "No edits resolved from LSP");
                            continue;
                        }
                    ...
                    carrot_log::info!(logger => "Executing command {}", &command.command);
                    ...
                    let execute_command_result = server
                        .request::<carrot_lsp::request::ExecuteCommand>(
                            carrot_lsp::ExecuteCommandParams {
    ...
    pub async fn format_ranges_via_lsp(
        ...
        let text_document = carrot_lsp::TextDocumentIdentifier::new(uri);
        ...
        let lsp_edits = {
            ...
            for range in lsp_ranges {
                if let Some(mut edit) = language_server
                    .request::<carrot_lsp::request::RangeFormatting>(
                        carrot_lsp::DocumentRangeFormattingParams {
    ...
    async fn format_via_lsp(
        ...
        let logger = carrot_log::scoped!("lsp_format");
        carrot_log::debug!(logger => "Formatting via LSP");
        ...
        let lsp_edits = if matches!(formatting_provider, Some(p) if *p != OneOf::Left(false)) {
            let _timer = carrot_log::time!(logger => "format-full");
            language_server
                .request::<carrot_lsp::request::Formatting>(
                    carrot_lsp::DocumentFormattingParams {
                        text_document,
                        options: lsp_command::lsp_formatting_options(settings),
                        work_done_progress_params: Default::default(),
                    },
            ...
            let _timer = carrot_log::time!(logger => "format-range");
            let buffer_start = carrot_lsp::Position::new(0, 0);
            let buffer_end = buffer.read_with(cx, |b, _| point_to_lsp(b.max_point_utf16()));
            language_server
                .request::<carrot_lsp::request::RangeFormatting>(
                    carrot_lsp::DocumentRangeFormattingParams {
                        text_document: text_document.clone(),
                        range: carrot_lsp::Range::new(buffer_start, buffer_end),
                        options: lsp_command::lsp_formatting_options(settings),
                        work_done_progress_params: Default::default(),
                    },
                    request_timeout,
    ...
    async fn format_via_external_command(
        ...
        use inazuma_util::command::Stdio;
        let mut child = inazuma_util::command::new_command(command);
        ...
        match &mut action.lsp_action {
            LspAction::Action(lsp_action) => {
                    ...
                    && (lsp_action.command.is_none() || lsp_action.edit.is_none())
                {
                    **lsp_action = lang_server
                        .request::<carrot_lsp::request::CodeActionResolveRequest>(
                            *lsp_action.clone(),
                            request_timeout,
                        )
                        .await
            ...
            LspAction::CodeLens(lens) => {
                if !action.resolved && GetCodeLens::can_resolve_lens(&lang_server.capabilities()) {
                    *lens = lang_server
                        .request::<carrot_lsp::request::CodeLensResolve>(
                            lens.clone(),
                            request_timeout,
                        )
                        .await
    ...
    pub(crate) fn unregister_old_buffer_from_language_servers(
        ...
        let Ok(file_url) = carrot_lsp::Uri::from_file_path(old_path.as_path()) else {
    ...
    async fn get_server_code_actions_from_action_kinds(
        lsp_store: &WeakEntity<LspStore>,
        language_server_id: LanguageServerId,
        code_action_kinds: Vec<carrot_lsp::CodeActionKind>,
        buffer: &Entity<Buffer>,
        cx: &mut AsyncApp,
        ...
        let actions = lsp_store
            .update(cx, move |this, cx| {
                let request = GetCodeActions {
                    range: inazuma_text::Anchor::min_max_range_for_buffer(
    ...
    pub fn edits_from_lsp(
        &mut self,
        buffer: &Entity<Buffer>,
        lsp_edits: impl 'static + Send + IntoIterator<Item = carrot_lsp::TextEdit>,
        server_id: LanguageServerId,
        version: Option<i32>,
        cx: &mut Context<LspStore>,
        ...
        cx.background_spawn(async move {
            ...
            while let Some((range, mut new_text)) = lsp_edits.next() {
                ...
                if range.end.row > range.start.row {
                    ...
                    let range_edits = carrot_language::text_diff(old_text.as_str(), &new_text);
    ...
    pub(crate) async fn deserialize_workspace_edit(
        ...
        if let Some(document_changes) = edit.document_changes {
            ...
            operations.extend(changes.into_iter().map(|(uri, edits)| {
                carrot_lsp::DocumentChangeOperation::Edit(carrot_lsp::TextDocumentEdit {
        ...
        for operation in operations {
            match operation {
                carrot_lsp::DocumentChangeOperation::Op(carrot_lsp::ResourceOp::Create(op)) => {
                    let abs_path = op
                        .uri
                        .to_file_path()
                    ...
                    if abs_path.ends_with("/") {
                        ...
                        fs.create_file(
                            &abs_path,
                            op.options
                                .map(|options| carrot_fs::CreateOptions {
                ...
                carrot_lsp::DocumentChangeOperation::Op(carrot_lsp::ResourceOp::Rename(op)) => {
                    ...
                    let options = carrot_fs::RenameOptions {
                        overwrite: op
                            .options
                            .as_ref()
                ...
                carrot_lsp::DocumentChangeOperation::Op(carrot_lsp::ResourceOp::Delete(op)) => {
                    let abs_path = op
                        .uri
                        .to_file_path()
                        .map_err(|()| anyhow!("can't convert URI to path"))?;
                    let options = op
                        .options
                        .map(|options| carrot_fs::RemoveOptions {
                ...
                carrot_lsp::DocumentChangeOperation::Edit(op) => {
    ...
    async fn on_lsp_workspace_edit(
        this: WeakEntity<LspStore>,
        params: carrot_lsp::ApplyWorkspaceEditParams,
        server_id: LanguageServerId,
        cx: &mut AsyncApp,
    ) -> Result<carrot_lsp::ApplyWorkspaceEditResponse> {
        ...
        Ok(carrot_lsp::ApplyWorkspaceEditResponse {
            applied: true,
            failed_change: None,
            failure_reason: None,
        })
        ...
        for watcher in watchers {
                ...
                Self::worktree_and_path_for_file_watcher(&worktrees, watcher, cx)
            {
                ...
                let (path, pattern) = match &watcher.glob_pattern {
                    carrot_lsp::GlobPattern::String(s) => {
                    ...
                    carrot_lsp::GlobPattern::Relative(rp) => {
                        let Ok(mut base_uri) = match &rp.base_uri {
                            carrot_lsp::OneOf::Left(workspace_folder) => &workspace_folder.uri,
                            carrot_lsp::OneOf::Right(base_uri) => base_uri,
                        }
                        .to_file_path() else {
                            continue;
                        };
...
pub struct LspStore {
    ...
    _maintain_workspace_config: (Task<Result<()>>, carrot_watch::Sender<()>),
    _maintain_buffer_languages: Task<()>,
    diagnostic_summaries:
        HashMap<WorktreeId, HashMap<Arc<RelPath>, HashMap<LanguageServerId, DiagnosticSummary>>>,
    pub lsp_server_capabilities: HashMap<LanguageServerId, carrot_lsp::ServerCapabilities>,
...
pub enum LspStoreEvent {
    ...
    SnippetEdit {
        buffer_id: BufferId,
        edits: Vec<(carrot_lsp::Range, Snippet)>,
...
struct CoreSymbol {
    ...
    pub name: String,
    pub kind: carrot_lsp::SymbolKind,
...
impl LspStore {
    ...
    pub fn new_local(
        ...
        let _maintain_workspace_config = {
            let (sender, receiver) = carrot_watch::channel(());
        ...
        match event {
            WorktreeStoreEvent::WorktreeAdded(worktree) => {
                ...
                cx.subscribe(worktree, |this, worktree, event, cx| match event {
                    carrot_worktree::Event::UpdatedEntries(changes) => {
                        this.update_local_worktree_language_servers(&worktree, changes, cx);
                    }
                    carrot_worktree::Event::UpdatedGitRepositories(_)
                    | carrot_worktree::Event::DeletedEntry(_) => {}
                })
                .detach()
            }
    ...
    fn on_buffer_event(
        &mut self,
        buffer: Entity<Buffer>,
        event: &carrot_language::BufferEvent,
        cx: &mut Context<Self>,
    ) {
        match event {
            carrot_language::BufferEvent::Edited { .. } => {
                self.on_buffer_edited(buffer, cx);
            }

            carrot_language::BufferEvent::Saved => {
                self.on_buffer_saved(buffer, cx);
            }

            carrot_language::BufferEvent::Reloaded => {
                    ...
                    this.update(cx, |this, cx| {
                        ...
                        for handle in this.buffer_store.read(cx).buffers() {
                            ...
                            if buffer.language().is_none()
                                || buffer.language() == Some(&*carrot_language::PLAIN_TEXT)
                            {
    ...
    fn detect_language_for_buffer(
        ...
        buffer_handle: &Entity<Buffer>,
        cx: &mut Context<Self>,
    ) -> Option<carrot_language::AvailableLanguage> {
    ...
    where
        F: FnMut(&carrot_lsp::ServerCapabilities) -> bool,
    {
    ...
    fn all_capable_for_proto_request<F>(
        ...
        mut check: F,
        cx: &App,
    ) -> Vec<(carrot_lsp::LanguageServerId, carrot_lsp::LanguageServerName)>
    where
        F: FnMut(&carrot_lsp::LanguageServerName, &carrot_lsp::ServerCapabilities) -> bool,
    {
        let Some(language) = buffer.read(cx).language().cloned() else {
            return Vec::default();
        };
    ...
    where
        R: LspCommand,
        <R::LspRequest as carrot_lsp::request::Request>::Result: Send,
        <R::LspRequest as carrot_lsp::request::Request>::Params: Send,
    {
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client() {
            ...
            cx.spawn(async move |this, cx| {
                ...
                let _result = lang_server
                    .request::<carrot_lsp::request::ExecuteCommand>(carrot_lsp::ExecuteCommandParams {
                        command: command.command.clone(),
                        arguments: command.arguments.clone().unwrap_or_default(),
                        ..carrot_lsp::ExecuteCommandParams::default()
                    }, request_timeout)
                    .await.into_response()
                    .context("execute command")?;
        ...
        if let Some((client, project_id)) = self.upstream_client() {
            ...
            cx.background_spawn(async move {
                client
                    ...
                    .await?
                    .transaction
                    .map(carrot_language::proto::deserialize_transaction)
                    .transpose()
            })
    ...
    async fn resolve_completion_local(
        server: Arc<carrot_lsp::LanguageServer>,
        completions: Rc<RefCell<Box<[Completion]>>>,
        ...
        let request = {
            ...
            match &completion.source {
                CompletionSource::Lsp {
                    ...
                    server.request::<carrot_lsp::request::ResolveCompletionItem>(
    ...
    async fn regenerate_completion_labels(
        ...
        let mut new_label = match completion_item {
            Some(completion_item) => {
                ...
                // the resolvable properties Carrot advertises. Regenerate labels here to handle this.
    ...
    pub fn apply_additional_edits_for_completion(
        ...
        completion_index: usize,
        push_to_history: bool,
        all_commit_ranges: Vec<Range<carrot_language::Anchor>>,
        cx: &mut Context<Self>,
    ) -> Task<Result<Option<Transaction>>> {
        if let Some((client, project_id)) = self.upstream_client() {
            let buffer = buffer_handle.read(cx);
            ...
            cx.spawn(async move |_, cx| {
                let request = {
                    ...
                    proto::ApplyCompletionAdditionalEdits {
                        ...
                        all_commit_ranges: all_commit_ranges
                            .iter()
                            .cloned()
                            .map(carrot_language::proto::serialize_anchor_range)
                            .collect(),
                    }
                };
                ...
                let transaction = carrot_language::proto::deserialize_transaction(transaction)?;
                buffer_handle
                    .update(cx, |buffer, _| {
                ...
                if let Some(edits) = additional_text_edits {
                    ...
                    buffer_handle.update(cx, |buffer, cx| {
                        ...
                        for (range, text) in edits {
                            ...
                            // https://github.com/carrot-rs/carrot/issues/26136
                            ...
                            //https://github.com/carrot-rs/carrot/pull/1871
                            if !has_overlap {
                                buffer.edit([(range, text)], None, cx);
                            }
                        }
                        ...
                        |mut acc, (server_id, uri, diagnostics, new_registration_id)| {
                            ...
                            acc.entry(server_id)
                                ...
                                .push(DocumentDiagnosticsUpdate {
                                    server_id,
                                    diagnostics: carrot_lsp::PublishDiagnosticsParams {
                                        uri,
                                        diagnostics,
                                        version: None,
                                    },
    ...
    pub fn symbols(&self, query: &str, cx: &mut Context<Self>) -> Task<Result<Vec<Symbol>>> {
        ...
        if let Some((upstream_client, project_id)) = self.upstream_client().as_ref() {
            ...
            struct WorkspaceSymbolsResult {
                ...
                lsp_symbols: Vec<(String, SymbolKind, carrot_lsp::Location, Option<String>)>,
            }
            ...
            for (seed, state) in local.language_server_ids.iter() {
                ...
                requests.push(
                    server
                        .request::<carrot_lsp::request::WorkspaceSymbolRequest>(
                            carrot_lsp::WorkspaceSymbolParams {
                                query: query.to_string(),
                                ..Default::default()
                            },
                            request_timeout,
                        ...
                        .map(move |response| {
                            let lsp_symbols = response
                                ...
                                .map(|symbol_response| match symbol_response {
                                    carrot_lsp::WorkspaceSymbolResponse::Flat(flat_responses) => {
                                        flat_responses
                                            .into_iter()
                                            .map(|lsp_symbol| {
                                                (
                                    ...
                                    carrot_lsp::WorkspaceSymbolResponse::Nested(
                                        nested_responses,
                                    ) => nested_responses
                                        .into_iter()
    ...
    pub fn on_buffer_edited(
        ...
        let uri = carrot_lsp::Uri::from_file_path(&abs_path)
            .ok()
        ...
        for language_server in language_servers {
                ...
                buffer
                    ...
                    .map(|edit| {
                        ...
                        let new_text = next_snapshot
                            ...
                            .collect();
                        carrot_lsp::TextDocumentContentChangeEvent {
                            range: Some(carrot_lsp::Range::new(
            ...
            let document_sync_kind = language_server
                ...
                .and_then(|sync| match sync {
                    carrot_lsp::TextDocumentSyncCapability::Kind(kind) => Some(*kind),
                    carrot_lsp::TextDocumentSyncCapability::Options(options) => options.change,
                });

            let content_changes: Vec<_> = match document_sync_kind {
                Some(carrot_lsp::TextDocumentSyncKind::FULL) => {
                    vec![carrot_lsp::TextDocumentContentChangeEvent {
                        range: None,
                        range_length: None,
                        text: next_snapshot.text(),
                    }]
                }
                Some(carrot_lsp::TextDocumentSyncKind::INCREMENTAL) => build_incremental_change(),
                _ => {
                    #[cfg(any(test, feature = "test-support"))]
                    {
            ...
            language_server
                .notify::<carrot_lsp::notification::DidChangeTextDocument>(
                    carrot_lsp::DidChangeTextDocumentParams {
                        text_document: carrot_lsp::VersionedTextDocumentIdentifier::new(
                            uri.clone(),
                            next_version,
                        ),
                        content_changes,
    ...
    pub fn on_buffer_saved(
        ...
        let worktree_id = file.worktree_id(cx);
        let abs_path = file.as_local()?.abs_path(cx);
        let text_document = carrot_lsp::TextDocumentIdentifier {
        ...
        for server in local.language_servers_for_worktree(worktree_id) {
            if let Some(include_text) = include_text(server.as_ref()) {
                ...
                server
                    .notify::<carrot_lsp::notification::DidSaveTextDocument>(
                        carrot_lsp::DidSaveTextDocumentParams {
                            text_document: text_document.clone(),
                            text,
                        },
                    )
                                ...
                                LanguageServerState::Running {
                                    ...
                                    Some(cx.spawn(async move |_, cx| {
                                        let settings =
                                            ...
                                            .ok()?;
                                        server
                                            .notify::<carrot_lsp::notification::DidChangeConfiguration>(
                                                carrot_lsp::DidChangeConfigurationParams { settings },
                                            )
                                            .ok()?;
                                        Some(())
                                    }))
    ...
    fn maintain_workspace_config(
        external_refresh_requests: carrot_watch::Receiver<()>,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
        let (mut settings_changed_tx, mut settings_changed_rx) = carrot_watch::channel(());
    ...
    pub fn merge_diagnostic_entries<'a>(
        ...
        merge: impl Fn(&carrot_lsp::Uri, &Diagnostic, &App) -> bool + Clone,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        ...
        for mut update in diagnostic_updates {
            ...
            let document_uri = carrot_lsp::Uri::from_file_path(abs_path)
    ...
    pub fn open_buffer_for_symbol(
        ...
        if let Some((client, project_id)) = self.upstream_client() {
            ...
            let symbol_uri = if let Ok(uri) = carrot_lsp::Uri::from_file_path(symbol_abs_path) {
                uri
            } else {
    ...
    where
        <T::LspRequest as carrot_lsp::request::Request>::Params: Send,
        <T::LspRequest as carrot_lsp::request::Request>::Result: Send,
    {
        let sender_id = envelope.original_sender_id().unwrap_or_default();
    ...
    async fn handle_start_language_server(
        ...
        let server_capabilities =
            serde_json::from_str::<carrot_lsp::ServerCapabilities>(&envelope.payload.capabilities)
                .with_context(|| {
                    format!(
    ...
    pub(super) fn did_rename_entry(
        ...
        maybe!({
            ...
            let old_uri = carrot_lsp::Uri::from_file_path(old_path)
                .ok()
                .map(|uri| uri.to_string())?;
            let new_uri = carrot_lsp::Uri::from_file_path(new_path)
                .ok()
                .map(|uri| uri.to_string())?;
    ...
    fn lsp_notify_abs_paths_changed(
        ...
        maybe!({
            ...
            let changes = changes
                ...
                .filter_map(|event| {
                    let typ = match event.kind? {
                        PathEventKind::Created => carrot_lsp::FileChangeType::CREATED,
                        PathEventKind::Removed => carrot_lsp::FileChangeType::DELETED,
                        PathEventKind::Changed | PathEventKind::Rescan => {
                            carrot_lsp::FileChangeType::CHANGED
                        }
                    };
                    Some(carrot_lsp::FileEvent {
            ...
            if !changes.is_empty() {
                server
                    .notify::<carrot_lsp::notification::DidChangeWatchedFiles>(
                        carrot_lsp::DidChangeWatchedFilesParams { changes },
                    )
                    .ok();
            }
            Some(())
    ...
    fn on_lsp_progress(
        &mut self,
        progress_params: carrot_lsp::ProgressParams,
        language_server_id: LanguageServerId,
        disk_based_diagnostics_progress_token: Option<String>,
        cx: &mut Context<Self>,
    ) {
        match progress_params.value {
            carrot_lsp::ProgressParamsValue::WorkDone(progress) => {
            ...
            carrot_lsp::ProgressParamsValue::WorkspaceDiagnostic(report) => {
                let registration_id = match progress_params.token {
                    carrot_lsp::NumberOrString::Number(_) => None,
                    carrot_lsp::NumberOrString::String(token) => token
    ...
    fn handle_work_done_progress(
        &mut self,
        progress: carrot_lsp::WorkDoneProgress,
        language_server_id: LanguageServerId,
        disk_based_diagnostics_progress_token: Option<String>,
        token: ProgressToken,
        cx: &mut Context<Self>,
        ...
        match progress {
            carrot_lsp::WorkDoneProgress::Begin(report) => {
            ...
            carrot_lsp::WorkDoneProgress::Report(report) => self.on_lsp_work_progress(
                language_server_id,
                token,
                LanguageServerProgress {
                    title: None,
            ...
            carrot_lsp::WorkDoneProgress::End(_) => {
    ...
    pub async fn handle_resolve_completion_documentation(
        ...
        let completion = this
            .read_with(&cx, |this, cx| {
                ...
                anyhow::Ok(cx.background_spawn(async move {
                    ...
                    if can_resolve {
                        server
                            .request::<carrot_lsp::request::ResolveCompletionItem>(
                                lsp_completion,
                                request_timeout,
                            )
                            .await
        ...
        let documentation = match completion.documentation {
            Some(carrot_lsp::Documentation::String(text)) => text,

            Some(carrot_lsp::Documentation::MarkupContent(carrot_lsp::MarkupContent {
                kind,
                value,
            })) => {
                documentation_is_markdown = kind == carrot_lsp::MarkupKind::Markdown;
                value
            }
    ...
    async fn handle_on_type_formatting(
        ...
        let transaction = on_type_formatting
            .await?
            .as_ref()
            .map(carrot_language::proto::serialize_transaction);
    ...
    async fn handle_open_buffer_for_symbol(
        ...
        this.update(&mut cx, |this, cx| {
            let is_private = buffer
                ...
                .unwrap_or_default();
            if is_private {
                Err(anyhow!(carrot_rpc::ErrorCode::UnsharedItem))
            } else {
                this.buffer_store
                    .update(cx, |buffer_store, cx| {
    ...
    async fn handle_apply_additional_edits_for_completion(
        ...
        let (buffer, completion, all_commit_ranges) = this.update(&mut cx, |this, cx| {
            ...
            let all_commit_ranges = envelope
                ...
                .all_commit_ranges
                .into_iter()
                .map(carrot_language::proto::deserialize_anchor_range)
    ...
    pub fn format(
        ...
        let logger = carrot_log::scoped!("format");
        if self.as_local().is_some() {
            carrot_log::trace!(logger => "Formatting locally");
            let logger = carrot_log::scoped!(logger => "local");
            let buffers = buffers
                .into_iter()
                .map(|buffer_handle| {
                    let buffer = buffer_handle.read(cx);
            ...
            cx.spawn(async move |lsp_store, cx| {
                ...
                carrot_log::trace!(logger => "Formatting {:?} buffers", formattable_buffers.len());

                let format_timer = carrot_log::time!(logger => "Formatting buffers");
                ...
                carrot_log::trace!(logger => "Formatting completed with result {:?}", result.as_ref().map(|_| "<project-transaction>"));
            ...
            carrot_log::trace!(logger => "Formatting remotely");
            let logger = carrot_log::scoped!(logger => "remote");
            ...
            let buffer_store = self.buffer_store();
            cx.spawn(async move |lsp_store, cx| {
                carrot_log::trace!(logger => "Sending remote format request");
                let request_timer = carrot_log::time!(logger => "remote format request");
                let result = client
                ...
                carrot_log::trace!(logger => "Remote format request resolved to {:?}", result.as_ref().map(|_| "<project_transaction>"));
                ...
                let transaction_response = result?;
                let _timer = carrot_log::time!(logger => "deserializing project transaction");
                buffer_store
                    .update(cx, |buffer_store, cx| {
                        buffer_store.deserialize_project_transaction(
                            transaction_response,
            ...
            carrot_log::trace!(logger => "Not formatting");
            Task::ready(Ok(ProjectTransaction::default()))
        }
    }
    ...
    pub fn merge_lsp_diagnostics(
        &mut self,
        source_kind: DiagnosticSourceKind,
        lsp_diagnostics: Vec<DocumentDiagnosticsUpdate<carrot_lsp::PublishDiagnosticsParams>>,
        merge: impl Fn(&carrot_lsp::Uri, &Diagnostic, &App) -> bool + Clone,
        cx: &mut Context<Self>,
    ) -> Result<()> {
    ...
    fn lsp_to_document_diagnostics(
        ...
        source_kind: DiagnosticSourceKind,
        server_id: LanguageServerId,
        mut lsp_diagnostics: carrot_lsp::PublishDiagnosticsParams,
        disk_based_sources: &[String],
        registration_id: Option<SharedString>,
    ) -> DocumentDiagnostics {
    ...
    fn insert_newly_running_language_server(
        ...
        self.buffer_store.clone().update(cx, |buffer_store, cx| {
            ...
            for buffer_handle in buffer_store.buffers() {
                ...
                if local.registered_buffers.contains_key(&buffer_id) {
                    let abs_path = file.abs_path(cx);
                    let uri = match carrot_lsp::Uri::from_file_path(&abs_path) {
                        Ok(uri) => uri,
                        Err(()) => {
            ...
            if let Some((LanguageServerState::Running { server, .. }, status)) = server.zip(status)
            {
                for (token, progress) in &status.pending_work {
                    ...
                    if progress.is_cancellable {
                        server
                            .notify::<carrot_lsp::notification::WorkDoneProgressCancel>(
    ...
    pub(super) fn update_local_worktree_language_servers(
        ...
        for server_id in &language_server_ids {
            ...
            {
                let params = carrot_lsp::DidChangeWatchedFilesParams {
                    changes: changes
                        .iter()
                        .filter_map(|(path, _, change)| {
                            ...
                            let typ = match change {
                                ...
                                PathChange::Added => carrot_lsp::FileChangeType::CREATED,
                                PathChange::Removed => carrot_lsp::FileChangeType::DELETED,
                                PathChange::Updated => carrot_lsp::FileChangeType::CHANGED,
                                PathChange::AddedOrUpdated => carrot_lsp::FileChangeType::CHANGED,
                            };
                            let uri = carrot_lsp::Uri::from_file_path(
                                worktree_handle.read(cx).absolutize(&path),
                            )
                            .ok()?;
                            Some(carrot_lsp::FileEvent { uri, typ })
                        })
                        .collect(),
                };
                if !params.changes.is_empty() {
                    server
                        .notify::<carrot_lsp::notification::DidChangeWatchedFiles>(params)
                        .ok();
                }
            }
        }
    ...
    fn serialize_symbol(symbol: &Symbol) -> proto::Symbol {
        let mut result = proto::Symbol {
            ...
            kind: unsafe { mem::transmute::<carrot_lsp::SymbolKind, i32>(symbol.kind) },
    ...
    fn deserialize_symbol(serialized_symbol: proto::Symbol) -> Result<CoreSymbol> {
        ...
        let kind = unsafe { mem::transmute::<i32, carrot_lsp::SymbolKind>(serialized_symbol.kind) };
    ...
    fn apply_workspace_diagnostic_report(
        &mut self,
        server_id: LanguageServerId,
        report: carrot_lsp::WorkspaceDiagnosticReportResult,
        registration_id: Option<SharedString>,
        cx: &mut Context<Self>,
    ) {
    ...
    fn register_server_capabilities(
        &mut self,
        server_id: LanguageServerId,
        params: carrot_lsp::RegistrationParams,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        let server = self
        ...
        for reg in params.registrations {
            match reg.method.as_str() {
                ...
                "workspace/didChangeWorkspaceFolders" => {
                    ...
                    let caps = carrot_lsp::WorkspaceFoldersServerCapabilities {
                        supported: Some(true),
                ...
                "textDocument/codeAction" => {
                    ...
                    let provider = match options {
                        OneOf::Left(value) => {
                            carrot_lsp::CodeActionProviderCapability::Simple(value)
                        }
                ...
                "textDocument/hover" => {
                    let options = parse_register_capabilities(reg)?;
                    let provider = match options {
                        OneOf::Left(value) => carrot_lsp::HoverProviderCapability::Simple(value),
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
                ...
                "textDocument/didChange" => {
                    if let Some(sync_kind) = reg
                        ...
                        .map(serde_json::from_value::<carrot_lsp::TextDocumentSyncKind>)
                        .transpose()?
                    {
                        server.update_capabilities(|capabilities| {
                            ...
                            capabilities.text_document_sync = Some(
                                carrot_lsp::TextDocumentSyncCapability::Options(sync_options),
                            );
                        });
                        notify_server_capabilities_updated(&server, cx);
                    }
                ...
                "textDocument/didSave" => {
                        ...
                        .transpose()?
                    {
                        server.update_capabilities(|capabilities| {
                            let mut sync_options =
                                Self::take_text_document_sync_options(capabilities);
                            sync_options.save = Some(TextDocumentSyncSaveOptions::SaveOptions(
                                carrot_lsp::SaveOptions { include_text },
                            ));
                            capabilities.text_document_sync = Some(
                                carrot_lsp::TextDocumentSyncCapability::Options(sync_options),
                            );
                ...
                "textDocument/documentColor" => {
                    let options = parse_register_capabilities(reg)?;
                    let provider = match options {
                        OneOf::Left(value) => carrot_lsp::ColorProviderCapability::Simple(value),
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
                ...
                "textDocument/foldingRange" => {
                    ...
                    let provider = match options {
                        OneOf::Left(value) => {
                            carrot_lsp::FoldingRangeProviderCapability::Simple(value)
                        }
                        OneOf::Right(caps) => caps,
                    };
                    server.update_capabilities(|capabilities| {
    ...
    fn unregister_server_capabilities(
        &mut self,
        server_id: LanguageServerId,
        params: carrot_lsp::UnregistrationParams,
        cx: &mut Context<Self>,
    ) -> anyhow::Result<()> {
        let server = self
            .language_server_for_id(server_id)
            ...
            match unreg.method.as_str() {
                ...
                "workspace/didChangeWorkspaceFolders" => {
                    server.update_capabilities(|capabilities| {
                        capabilities
                            .workspace
                            .get_or_insert_with(|| carrot_lsp::WorkspaceServerCapabilities {
    ...
    fn take_text_document_sync_options(
        capabilities: &mut carrot_lsp::ServerCapabilities,
    ) -> carrot_lsp::TextDocumentSyncOptions {
        match capabilities.text_document_sync.take() {
            Some(carrot_lsp::TextDocumentSyncCapability::Options(sync_options)) => sync_options,
            Some(carrot_lsp::TextDocumentSyncCapability::Kind(sync_kind)) => {
                let mut sync_options = carrot_lsp::TextDocumentSyncOptions::default();
                sync_options.change = Some(sync_kind);
                sync_options
            }
            None => carrot_lsp::TextDocumentSyncOptions::default(),
        }
    }
        ...
        loop {
            ...
            'request: loop {
                ...
                let Ok(previous_result_ids) = lsp_store.update(cx, |lsp_store, _| {
                    lsp_store
                        ...
                        .filter_map(|(abs_path, result_id)| {
                            ...
                            Some(carrot_lsp::PreviousResultId {
                                uri,
                ...
                let response_result = server
                    .request_with_timer::<carrot_lsp::WorkspaceDiagnosticRequest, _>(
                        carrot_lsp::WorkspaceDiagnosticParams {
                            previous_result_ids,
                            identifier: identifier.clone(),
                            work_done_progress_params: Default::default(),
                            partial_result_params: carrot_lsp::PartialResultParams {
                                partial_result_token: Some(carrot_lsp::ProgressToken::String(token)),
                            },
                        },
...
fn buffer_diagnostic_identifier(options: &DiagnosticServerCapabilities) -> Option<SharedString> {
    match &options {
        carrot_lsp::DiagnosticServerCapabilities::Options(diagnostic_options) => diagnostic_options
            .identifier
            .as_deref()
            .map(SharedString::new),
        carrot_lsp::DiagnosticServerCapabilities::RegistrationOptions(registration_options) => {
...
fn workspace_diagnostic_identifier(
    ...
    match &options {
        carrot_lsp::DiagnosticServerCapabilities::Options(diagnostic_options) => {
...
impl From<carrot_lsp::Documentation> for CompletionDocumentation {
    fn from(docs: carrot_lsp::Documentation) -> Self {
        match docs {
            carrot_lsp::Documentation::String(text) => {
            ...
            carrot_lsp::Documentation::MarkupContent(carrot_lsp::MarkupContent { kind, value }) => {
                match kind {
                    carrot_lsp::MarkupKind::PlainText => {
                    ...
                    carrot_lsp::MarkupKind::Markdown => {
...
pub struct LocalLspAdapterDelegate {
    lsp_store: WeakEntity<LspStore>,
    worktree: carrot_worktree::Snapshot,
    fs: Arc<dyn Fs>,
    http_client: Arc<dyn HttpClient>,
...
impl LspAdapterDelegate for LocalLspAdapterDelegate {
    ...
    async fn npm_package_installed_version(
        ...
        let output = inazuma_util::command::new_command(&npm)
            .args(["root", "-g"])
            .envs(env)
    ...
    async fn try_exec(&self, command: LanguageServerBinary) -> Result<()> {
        ...
        let output = inazuma_util::command::new_command(&command.path)
    ...
    fn update_status(
        ...
        status: carrot_language::BinaryStatus,
    ) {
...
async fn populate_labels_for_symbols(
    ...
    for (language, mut symbols) in symbols_by_language {
        label_params.clear();
        label_params.extend(symbols.iter_mut().map(|symbol| carrot_language::Symbol {
...
fn include_text(server: &carrot_lsp::LanguageServer) -> Option<bool> {
    match server.capabilities().text_document_sync.as_ref()? {
        carrot_lsp::TextDocumentSyncCapability::Options(opts) => match opts.save.as_ref()? {
            // Server wants didSave but didn't specify includeText.
            carrot_lsp::TextDocumentSyncSaveOptions::Supported(true) => Some(false),
            // Server doesn't want didSave at all.
            carrot_lsp::TextDocumentSyncSaveOptions::Supported(false) => None,
            // Server provided SaveOptions.
            carrot_lsp::TextDocumentSyncSaveOptions::SaveOptions(save_options) => {
                Some(save_options.include_text.unwrap_or(false))
            }
        },
        // We do not have any save info. Kind affects didChange only.
        carrot_lsp::TextDocumentSyncCapability::Kind(_) => None,
    }
}
