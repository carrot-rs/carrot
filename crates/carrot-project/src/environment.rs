...
use carrot_rpc::proto::{self, REMOTE_SERVER_PROJECT_ID};
use carrot_task::{Shell, shell_to_proto};
...
use inazuma::{App, AppContext as _, Context, Entity, EventEmitter, Task, WeakEntity};
...
impl ProjectEnvironment {
    ...
    /// Returns the inherited CLI environment, if this project was opened from the Carrot CLI.
        ...
        match remote_client {
            ...
            None => Some({
                let shell = TerminalSettings::get(
                    Some(inazuma_settings_framework::SettingsLocation {
    ...
    pub fn directory_environment(
        ...
        match remote_client {
            ...
            None => self
                ...
                .ok()
                .map(|worktree| {
                    let shell = carrot_terminal::terminal_settings::TerminalSettings::get(
                        worktree.as_ref().map(|(worktree, path)| {
                            inazuma_settings_framework::SettingsLocation {
...
fn set_origin_marker(env: &mut HashMap<String, String>, origin: EnvironmentOrigin) {
    env.insert(CARROT_ENVIRONMENT_ORIGIN_MARKER.to_string(), origin.into());
}

const CARROT_ENVIRONMENT_ORIGIN_MARKER: &str = "CARROT_ENVIRONMENT";
...
async fn load_directory_shell_environment(
    ...
    let mut envs = inazuma_util::shell_env::capture(shell.clone(), args, abs_path)
        .await
        .with_context(|| {
