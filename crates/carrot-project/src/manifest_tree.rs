...
use carrot_language::{ManifestDelegate, ManifestName, ManifestQuery};
use carrot_worktree::{Event as WorktreeEvent, Snapshot, Worktree};
use inazuma::{App, AppContext as _, Context, Entity, Subscription};
                ...
                match event {
                    WorktreeEvent::UpdatedEntries(changes) => {
                        for (path, _, kind) in changes.iter() {
                            if kind == &carrot_worktree::PathChange::Removed {
