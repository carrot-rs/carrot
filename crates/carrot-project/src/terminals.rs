use anyhow::Result;
use inazuma::{App, AppContext as _, Context, Entity, Task, WeakEntity};
use inazuma_collections::HashMap;

use carrot_language::LanguageName;
use carrot_remote::RemoteClient;
use carrot_task::{Shell, ShellBuilder, ShellKind, SpawnInTerminal};
use carrot_terminal::{
    TaskState, TaskStatus, Terminal, TerminalBuilder, insert_carrot_terminal_env,
    terminal_settings::TerminalSettings,
};
use futures::{FutureExt, future::Shared};
use inazuma_settings_framework::{Settings, SettingsLocation};
use inazuma_util::{command::new_std_command, get_default_system_shell, maybe, rel_path::RelPath};
...
pub struct Terminals {
    pub(crate) local_handles: Vec<WeakEntity<carrot_terminal::Terminal>>,
}
...
impl Project {
    ...
    /// In remote projects: opens in Carrot's launch directory (bypasses SSH).
    ...
    pub fn create_local_terminal(
        ...
        let working_directory = if self.remote_client.is_some() {
            // Remote project: don't use remote paths, let shell use Carrot's cwd
            None
        } else {
    ...
    pub fn exec_in_shell(
        ...
        cx.spawn(async move |project, cx| {
            ...
            project.update(cx, move |_, cx| {
                ...
                .map(|mut process| {
                    inazuma_util::set_pre_exec_to_start_new_session(&mut process);
    ...
    pub fn local_terminal_handles(&self) -> &Vec<WeakEntity<carrot_terminal::Terminal>> {
...
fn create_remote_shell(
    ...
    insert_carrot_terminal_env(&mut env, &carrot_release_channel::AppVersion::global(cx));
