...
use carrot_context_server::oauth::{self, McpOAuthTokenProvider, OAuthDiscovery, OAuthSession};
use carrot_context_server::transport::{HttpTransport, TransportError};
...
use carrot_rpc::{AnyProtoClient, TypedEnvelope, proto};
use futures::future::Either;
use futures::{FutureExt as _, StreamExt as _, future::join_all};
use inazuma::{
    App, AsyncApp, Context, Entity, EventEmitter, Subscription, Task, WeakEntity, actions,
};
use inazuma_collections::{HashMap, HashSet};
use inazuma_settings_framework::{Settings as _, SettingsStore};
...
impl ContextServerStore {
    ...
    fn resolve_project_settings<'a>(
        ...
        let location = worktree_store
            ...
            .next()
            .map(|worktree| inazuma_settings_framework::SettingsLocation {
...
async fn resolve_start_failure(
    ...
    match carrot_context_server::oauth::discover(&http_client, &server_url, www_authenticate).await
    {
        Ok(discovery) => {
            log::info!(
