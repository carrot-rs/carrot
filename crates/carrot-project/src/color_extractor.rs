...
use carrot_lsp::{CompletionItem, Documentation};
use inazuma::{Oklch, Rgba, hsla};
...
pub fn extract_color(item: &CompletionItem) -> Option<Oklch> {
...
fn parse(str: &str, mode: ParseMode) -> Option<Oklch> {
...
fn from_hex(hex: &str) -> Option<Oklch> {
    Rgba::try_from(hex).map(Oklch::from).ok()
}

fn from_rgb(r: &str, g: &str, b: &str, a: Option<&str>) -> Option<Oklch> {
...
fn from_hsl(h: &str, s: &str, l: &str, a: Option<&str>) -> Option<Oklch> {
