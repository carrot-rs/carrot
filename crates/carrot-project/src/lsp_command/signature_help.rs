...
use carrot_rpc::proto::{self, documentation};
...
pub struct SignatureHelp {
    ...
    pub(super) original_data: carrot_lsp::SignatureHelp,
}
            ...
            if let Some(parameters) = &signature.parameters {
                for (index, parameter) in parameters.iter().enumerate() {
                    let label_range = match &parameter.label {
                        &carrot_lsp::ParameterLabel::LabelOffsets([offset1, offset2]) => {
                            maybe!({
                        ...
                        carrot_lsp::ParameterLabel::Simple(parameter_label) => {
...
fn documentation_to_markdown(
    documentation: &carrot_lsp::Documentation,
    language_registry: Option<Arc<LanguageRegistry>>,
    cx: &mut App,
) -> Entity<Markdown> {
    match documentation {
        carrot_lsp::Documentation::String(string) => {
            cx.new(|cx| Markdown::new_text(SharedString::from(string), cx))
        }
        carrot_lsp::Documentation::MarkupContent(markup) => match markup.kind {
            carrot_lsp::MarkupKind::PlainText => {
                cx.new(|cx| Markdown::new_text(SharedString::from(&markup.value), cx))
            }
            carrot_lsp::MarkupKind::Markdown => cx.new(|cx| {
                Markdown::new(
...
pub fn lsp_to_proto_signature(lsp_help: carrot_lsp::SignatureHelp) -> proto::SignatureHelp {
    proto::SignatureHelp {
        signatures: lsp_help
            .signatures
            .into_iter()
                ...
                parameters: signature
                    ...
                    .map(|parameter_info| proto::ParameterInformation {
                        label: Some(match parameter_info.label {
                            carrot_lsp::ParameterLabel::Simple(label) => {
                                proto::parameter_information::Label::Simple(label)
                            }
                            carrot_lsp::ParameterLabel::LabelOffsets(offsets) => {
...
fn lsp_to_proto_documentation(documentation: carrot_lsp::Documentation) -> proto::Documentation {
    proto::Documentation {
        content: Some(match documentation {
            carrot_lsp::Documentation::String(string) => {
                proto::documentation::Content::Value(string)
            }
            carrot_lsp::Documentation::MarkupContent(content) => {
                proto::documentation::Content::MarkupContent(proto::MarkupContent {
                    is_markdown: matches!(content.kind, carrot_lsp::MarkupKind::Markdown),
                    value: content.value,
                })
            }
        }),
...
pub fn proto_to_lsp_signature(proto_help: proto::SignatureHelp) -> carrot_lsp::SignatureHelp {
    carrot_lsp::SignatureHelp {
        signatures: proto_help
            .signatures
            .into_iter()
            .map(|signature| carrot_lsp::SignatureInformation {
                label: signature.label,
...
) -> Option<carrot_lsp::Documentation> {
    {
        Some(match documentation.content? {
            documentation::Content::Value(string) => carrot_lsp::Documentation::String(string),
            documentation::Content::MarkupContent(markup) => {
                carrot_lsp::Documentation::MarkupContent(if markup.is_markdown {
                    ...
                    carrot_lsp::MarkupContent {
                        kind: carrot_lsp::MarkupKind::PlainText,
                        value: markup.value,
                    }
                })
            }
