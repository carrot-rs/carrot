use carrot_git::{repository::RepoPath, status::GitSummary};
use carrot_worktree::{Entry, PathProgress, PathTarget, Traversal};
use inazuma_collections::HashMap;
use inazuma_sum_tree::Cursor;
...
impl<'a> ChildEntriesGitIter<'a> {
    pub fn new(
        repo_snapshots: &'a HashMap<RepositoryId, RepositorySnapshot>,
        worktree_snapshot: &'a carrot_worktree::Snapshot,
        parent_path: &'a RelPath,
    ) -> Self {
