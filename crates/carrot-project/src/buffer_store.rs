...
use carrot_worktree::{File, PathChange, ProjectEntryId, Worktree, WorktreeId, WorktreeSettings};
use inazuma_settings_framework::Settings;
use inazuma_text::{BufferId, ReplicaId};
use inazuma_util::{ResultExt as _, TryFutureExt, debug_panic, maybe, rel_path::RelPath};
...
pub enum BufferStoreEvent {
    ...
    BufferChangedFilePath {
        buffer: Entity<Buffer>,
        old_file: Option<Arc<dyn carrot_language::File>>,
    },
}

#[derive(Default, Debug, Clone)]
pub struct ProjectTransaction(pub HashMap<Entity<Buffer>, carrot_language::Transaction>);
            ...
            proto::create_buffer_for_peer::Variant::State(mut state) => {
                ...
                let buffer_result = maybe!({
                    ...
                    if let Some(file) = state.file.take() {
                        let worktree_id = carrot_worktree::WorktreeId::from_proto(file.worktree_id);
                        let worktree = self
                            .worktree_store
                            .read(cx)
            ...
            proto::create_buffer_for_peer::Variant::Chunk(chunk) => {
                ...
                let result = maybe!({
                    let operations = chunk
                        .operations
                        .into_iter()
                        .map(carrot_language::proto::deserialize_operation)
...
impl LocalBufferStore {
                    ...
                    if has_changed_file {
                        downstream_client
                            .send(proto::UpdateBufferFile {
                                ...
                                file: Some(carrot_language::File::to_proto(&*new_file, cx)),
                            })
                            .log_err();
                    }
                    downstream_client
    ...
    fn subscribe_to_worktree(
        ...
        cx.subscribe(worktree, |this, worktree, event, cx| {
            if worktree.read(cx).is_local()
                && let carrot_worktree::Event::UpdatedEntries(changes) = event
            {
    ...
    fn local_worktree_entry_changed(
        ...
        path: &Arc<RelPath>,
        worktree: &Entity<carrot_worktree::Worktree>,
        snapshot: &carrot_worktree::Snapshot,
        cx: &mut Context<BufferStore>,
    ) -> Option<()> {
    ...
    fn create_buffer(
        ...
        cx.spawn(async move |buffer_store, cx| {
            let buffer = cx.new(|cx| {
                Buffer::local("", cx).with_language(
                    language.unwrap_or_else(|| carrot_language::PLAIN_TEXT.clone()),
                    cx,
                )
            });
...
impl BufferStore {
        ...
        let buffers = self
            ...
            .map(|buffer| {
                ...
                proto::BufferVersion {
                    ...
                    version: carrot_language::proto::serialize_version(&buffer.version),
                }
            })
            .collect();
    ...
    pub fn handle_synchronize_buffers(
        ...
        for buffer in envelope.payload.buffers {
            ...
            let remote_version = carrot_language::proto::deserialize_version(&buffer.version);
            if let Some(buffer) = self.get(buffer_id) {
                ...
                client
                    .send(proto::BufferReloaded {
                        project_id,
                        buffer_id: buffer_id.into(),
                        version: carrot_language::proto::serialize_version(buffer.saved_version()),
                        mtime: buffer.saved_mtime().map(|time| time.into()),
                        line_ending: carrot_language::proto::serialize_line_ending(
                            buffer.line_ending(),
                        ) as i32,
                    })
                    .log_err();
    ...
    pub fn create_local_buffer(
        ...
        let buffer = cx.new(|cx| {
            Buffer::local(text, cx).with_language(
                language.unwrap_or_else(|| carrot_language::PLAIN_TEXT.clone()),
                cx,
            )
        });
    ...
    pub fn serialize_project_transaction_for_peer(
        ...
        for (buffer, transaction) in project_transaction.0 {
            ...
            serialized_transaction
                .transactions
                .push(carrot_language::proto::serialize_transaction(&transaction));
        }
        serialized_transaction
    }
    ...
    pub(crate) fn register_project_search_result_handle(
        ...
        let handle = inazuma_util::post_inc(&mut self.project_search.next_id);
