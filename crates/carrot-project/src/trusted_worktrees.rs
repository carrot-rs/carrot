//! A module, responsible for managing the trust logic in Carrot.
//!
...
//! This will set up a [`inazuma::Global`] with [`TrustedWorktrees`] entity that will persist, restore and allow querying for worktree trust.
...
//!
//! Carrot does not consider invisible, `worktree.is_visible() == false` worktrees in Carrot, as those are programmatically created inside Carrot for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! Carrot has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty Carrot it's possible to open just a file, same as after opening a directory in Carrot it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening Carrot's settings.json file via `carrot: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and Carrot needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in Carrot, it's a full worktree which may spawn multiple language servers associated with it.
...
//! * "path override"
//!
//! To ease trusting multiple directory worktrees at once, it's possible to trust a parent directory of a certain directory worktree opened in Carrot.
...
use carrot_rpc::{AnyProtoClient, proto};
use inazuma::{
...
use inazuma_settings_framework::{Settings as _, WorktreeId};
...
impl TrustedWorktreesStore {
        ...
        for trusted_path in trusted_paths.iter().chain(
            ...
            match trusted_path {
                ...
                PathTrust::AbsPath(abs_path) => {
                    debug_assert!(
                        inazuma_util::paths::is_absolute(
    ...
    /// Requires Carrot's restart to take proper effect.
    ...
    pub fn can_trust(
        ...
        // Carrot opened an "internal" directory: e.g. a tmp dir for `keymap_editor.rs` needs.
