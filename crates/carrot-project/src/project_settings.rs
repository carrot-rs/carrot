...
use carrot_context_server::ContextServerCommand;
use carrot_dap::adapters::DebugAdapterName;
use carrot_fs::Fs;
use carrot_git::repository::DEFAULT_WORKTREE_DIRECTORY;
use carrot_lsp::{DEFAULT_LSP_REQUEST_TIMEOUT_SECS, LanguageServerName};
use carrot_paths::{
...
use inazuma::{AsyncApp, BorrowAppContext, Context, Entity, EventEmitter, Subscription, Task};
use inazuma_collections::HashMap;
pub use inazuma_settings_framework::BinarySettings;
pub use inazuma_settings_framework::DirenvSettings;
pub use inazuma_settings_framework::LspSettings;
...
use carrot_worktree::{PathChange, UpdatedEntriesSet, Worktree, WorktreeId};
...
pub struct ProjectSettings {
    ...
    // We should change to use a non content type (inazuma_settings_framework::LspSettings is a content type)
    ...
    pub lsp: HashMap<LanguageServerName, inazuma_settings_framework::LspSettings>,
...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary Carrot should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, Carrot will download its own copy of Node.
    pub ignore_system_version: bool,
}

impl From<inazuma_settings_framework::NodeBinarySettings> for NodeBinarySettings {
    fn from(settings: inazuma_settings_framework::NodeBinarySettings) -> Self {
        Self {
            path: settings.path,
...
impl From<inazuma_settings_framework::ContextServerSettingsContent> for ContextServerSettings {
    fn from(value: inazuma_settings_framework::ContextServerSettingsContent) -> Self {
        match value {
            inazuma_settings_framework::ContextServerSettingsContent::Stdio {
                enabled,
                remote,
                command,
            ...
            inazuma_settings_framework::ContextServerSettingsContent::Http {
                enabled,
                url,
                headers,
                timeout,
...
impl Into<inazuma_settings_framework::ContextServerSettingsContent> for ContextServerSettings {
    fn into(self) -> inazuma_settings_framework::ContextServerSettingsContent {
        match self {
            ContextServerSettings::Stdio {
                enabled,
                remote,
...
impl DiagnosticSeverity {
    pub fn into_lsp(self) -> Option<carrot_lsp::DiagnosticSeverity> {
        match self {
            DiagnosticSeverity::Off => None,
            DiagnosticSeverity::Error => Some(carrot_lsp::DiagnosticSeverity::ERROR),
            DiagnosticSeverity::Warning => Some(carrot_lsp::DiagnosticSeverity::WARNING),
            DiagnosticSeverity::Info => Some(carrot_lsp::DiagnosticSeverity::INFORMATION),
            DiagnosticSeverity::Hint => Some(carrot_lsp::DiagnosticSeverity::HINT),
        }
    }
}

impl From<inazuma_settings_framework::DiagnosticSeverityContent> for DiagnosticSeverity {
    fn from(severity: inazuma_settings_framework::DiagnosticSeverityContent) -> Self {
        match severity {
            inazuma_settings_framework::DiagnosticSeverityContent::Off => DiagnosticSeverity::Off,
            inazuma_settings_framework::DiagnosticSeverityContent::Error => {
                DiagnosticSeverity::Error
            }
            inazuma_settings_framework::DiagnosticSeverityContent::Warning => {
                DiagnosticSeverity::Warning
            }
            inazuma_settings_framework::DiagnosticSeverityContent::Info => DiagnosticSeverity::Info,
            inazuma_settings_framework::DiagnosticSeverityContent::Hint => DiagnosticSeverity::Hint,
            inazuma_settings_framework::DiagnosticSeverityContent::All => DiagnosticSeverity::Hint,
        }
    }
}
...
impl From<carrot_lsp::DiagnosticSeverity> for GoToDiagnosticSeverity {
    fn from(severity: carrot_lsp::DiagnosticSeverity) -> Self {
        match severity {
            carrot_lsp::DiagnosticSeverity::ERROR => Self::Error,
            carrot_lsp::DiagnosticSeverity::WARNING => Self::Warning,
            carrot_lsp::DiagnosticSeverity::INFORMATION => Self::Information,
            carrot_lsp::DiagnosticSeverity::HINT => Self::Hint,
            _ => Self::Error,
        }
    }
}
...
impl GoToDiagnosticSeverityFilter {
    pub fn matches(&self, severity: carrot_lsp::DiagnosticSeverity) -> bool {
...
pub struct GitSettings {
    ...
    pub git_gutter: inazuma_settings_framework::GitGutterSetting,
    ...
    pub hunk_style: inazuma_settings_framework::GitHunkStyleSetting,
...
impl From<inazuma_settings_framework::GitPathStyle> for GitPathStyle {
    fn from(style: inazuma_settings_framework::GitPathStyle) -> Self {
        match style {
            inazuma_settings_framework::GitPathStyle::FileNameFirst => GitPathStyle::FileNameFirst,
            inazuma_settings_framework::GitPathStyle::FilePathFirst => GitPathStyle::FilePathFirst,
        }
    }
}
...
pub struct InlineBlameSettings {
    ...
    ///
    /// Default: 0
    pub delay_ms: inazuma_settings_framework::DelayMs,
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in Carrot.
...
/// SettingsObserver observers changes to .carrot/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/carrot/{settings, task}.json and sends the content
/// upstream.
        ...
        Self {
            ...
            _global_debug_config_watcher: if watch_global_configs {
                Self::subscribe_to_global_debug_scenarios_changes(
                    fs.clone(),
                    carrot_paths::debug_scenarios_file().clone(),
                    cx,
                )
            } else {
                Task::ready(())
        ...
        match event {
            WorktreeStoreEvent::WorktreeAdded(worktree) => cx
                .subscribe(worktree, |this, worktree, event, cx| {
                    if let carrot_worktree::Event::UpdatedEntries(changes) = event {
