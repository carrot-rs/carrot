...
use carrot_lsp::{DiagnosticSeverity, DiagnosticTag};
use carrot_project::lsp_command::*;
use carrot_rpc::proto::{self};
use serde_json::json;

#[test]
fn test_serialize_lsp_diagnostic() {
    let lsp_diagnostic = carrot_lsp::Diagnostic {
        range: carrot_lsp::Range {
            start: carrot_lsp::Position::new(0, 1),
            end: carrot_lsp::Position::new(2, 3),
        },
        severity: Some(DiagnosticSeverity::ERROR),
        code: Some(carrot_lsp::NumberOrString::String("E001".to_string())),
...
fn test_deserialize_lsp_diagnostic() {
    ...
    assert_eq!(
        lsp_diagnostic.code,
        Some(carrot_lsp::NumberOrString::String("ERR".to_string()))
    );
...
fn test_related_information() {
    let related_info = carrot_lsp::DiagnosticRelatedInformation {
        location: carrot_lsp::Location {
            uri: carrot_lsp::Uri::from_str("file:///test.rs").unwrap(),
            range: carrot_lsp::Range {
                start: carrot_lsp::Position::new(1, 1),
                end: carrot_lsp::Position::new(1, 5),
            },
        },
    ...
    let lsp_diagnostic = carrot_lsp::Diagnostic {
        range: carrot_lsp::Range {
            start: carrot_lsp::Position::new(0, 0),
            end: carrot_lsp::Position::new(0, 1),
        },
