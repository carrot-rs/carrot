...
use carrot_language::{CodeLabel, HighlightId};

use carrot_project::lsp_store::*;
...
fn test_trailing_newline_in_completion_documentation() {
    let doc = carrot_lsp::Documentation::String(
    ...
    let doc = carrot_lsp::Documentation::String("  some value  \n".to_string());
    let completion_doc: CompletionDocumentation = doc.into();
    assert!(matches!(
        completion_doc,
