use anyhow::Result;
use carrot_context_server::test::create_fake_transport;
use carrot_context_server::{ContextServer, ContextServerId};
use carrot_http_client::{FakeHttpClient, Response};
use carrot_project::context_server_store::registry::ContextServerDescriptorRegistry;
use carrot_project::context_server_store::*;
use carrot_project::project_settings::ContextServerSettings;
use carrot_project::worktree_store::WorktreeStore;
use carrot_project::{
...
use inazuma_settings_framework::settings_content::SaturatingBool;
use inazuma_settings_framework::{ContextServerCommand, Settings, SettingsStore};
...
fn set_context_server_configuration(
    context_servers: Vec<(
        Arc<str>,
        inazuma_settings_framework::ContextServerSettingsContent,
    )>,
...
async fn test_remote_context_server(cx: &mut TestAppContext) {
    ...
    let client = FakeHttpClient::create(|_| async move {
        use carrot_http_client::AsyncBody;
...
impl ContextServerDescriptor for FakeContextServerDescriptor {
    ...
    fn configuration(
        ...
        _worktree_store: Entity<WorktreeStore>,
        _cx: &AsyncApp,
    ) -> Task<Result<Option<::carrot_extension::ContextServerConfiguration>>> {
        Task::ready(Ok(None))
    }
}
