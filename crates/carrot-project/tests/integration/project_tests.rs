...
use carrot_paths::{config_dir, global_gitignore_path, tasks_file};
use encoding_rs;
...
use carrot_task::{ResolvedTask, ShellKind, TaskContext};
use carrot_worktree::WorktreeModelHandle as _;
use inazuma_settings_framework::SettingsStore;
use inazuma_sum_tree::SumTree;
use inazuma_text::{Anchor, PointUtf16, ReplicaId, ToOffset, Unclipped};
use inazuma_util::{
...
#[inazuma::test]
async fn test_block_via_channel(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_block_via_smol(cx: &mut inazuma::TestAppContext) {
    cx.executor().allow_parking();
...
async fn test_default_session_work_dirs_falls_back_to_home_for_empty_project(
    ...
    assert_eq!(ordered_paths, vec![carrot_paths::home_dir().to_path_buf()]);
}
...
#[cfg(not(windows))]
#[inazuma::test]
async fn test_symlinks(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
#[inazuma::test]
async fn test_editorconfig_support(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    // .editorconfig overrides .carrot/settings
    ...
    // When max_line_length is "off", default to .carrot/settings.json
...
#[inazuma::test]
async fn test_external_editorconfig_support(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_internal_editorconfig_root_stops_traversal(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_external_editorconfig_root_stops_traversal(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_external_editorconfig_root_in_parent_stops_traversal(
    cx: &mut inazuma::TestAppContext,
) {
    init_test(cx);
...
#[inazuma::test]
async fn test_external_editorconfig_shared_across_worktrees(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_external_editorconfig_modification_triggers_refresh(
    cx: &mut inazuma::TestAppContext,
) {
    init_test(cx);
...
#[inazuma::test]
async fn test_adding_worktree_discovers_external_editorconfigs(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_removing_worktree_cleans_up_external_editorconfig(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_git_provider_project_setting(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.update(|cx| {
        GitHostingProviderRegistry::default_global(cx);
        carrot_git_hosting_providers::init(cx);
    });
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.carrot/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
...
#[inazuma::test]
async fn test_managing_project_specific_settings(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    TaskStore::init(None);
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: rel_path(".carrot").into(),
        id_base: "local worktree tasks from directory \".carrot\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: rel_path("b/.carrot").into(),
                    id_base: "local worktree tasks from directory \"b/.carrot\"".into()
                },
        ...
        vec![
            ...
            (
                TaskSourceKind::AbsPath {
                    abs_path: carrot_paths::tasks_file().clone(),
...
#[inazuma::test]
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    TaskStore::init(None);

    // We need to start with a valid `.carrot/tasks.toml` file as otherwise the
    ...
    // Update the `.carrot/tasks.toml` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.carrot/tasks.toml").as_ref(),
        &"[[tasks]]\nlabel = \"test $ZED_FOO\"\ncommand = \"echo\"\n".into(),
        Default::default(),
    )
    .await
    ...
    project.update(cx, |_, cx| {
        ...
        cx.subscribe(&project, move |_, _, event: &Event, _| match event {
            Event::Toast {
                ...
                assert_eq!(*url, "https://carrot.dev/docs/tasks");
...
#[inazuma::test]
async fn test_fallback_to_single_worktree_tasks(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    TaskStore::init(None);
            ...
            get_all_tasks(
                ...
                Arc::new(TaskContexts {
                    ...
                    active_worktree_context: Some((worktree_id, {
                        ...
                        worktree_context
                            .task_variables
                            .insert(carrot_task::VariableName::WorktreeRoot, "/dir".to_string());
                        worktree_context
                    })),
                    other_worktree_contexts: Vec::new(),
                    lsp_task_sources: HashMap::default(),
...
async fn test_running_multiple_instances_of_a_single_server_in_one_worktree(
    ...
    let _fake_python_server = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            name: "ty",
            capabilities: carrot_lsp::ServerCapabilities {
                ..Default::default()
            },
            ..Default::default()
        },
...
#[inazuma::test]
async fn test_managing_language_servers(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut fake_rust_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            ...
            capabilities: carrot_lsp::ServerCapabilities {
                completion_provider: Some(carrot_lsp::CompletionOptions {
                    trigger_characters: Some(vec![".".to_string(), "::".to_string()]),
                    ..Default::default()
                }),
                text_document_sync: Some(carrot_lsp::TextDocumentSyncCapability::Options(
                    carrot_lsp::TextDocumentSyncOptions {
                        save: Some(carrot_lsp::TextDocumentSyncSaveOptions::Supported(true)),
                        ..Default::default()
                    },
                )),
                ..Default::default()
    ...
    assert_eq!(
        fake_rust_server
            .receive_notification::<carrot_lsp::notification::DidOpenTextDocument>()
            .await
            .text_document,
        carrot_lsp::TextDocumentItem {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/test.rs")).unwrap(),
            version: 0,
    ...
    assert_eq!(
        fake_rust_server
            .receive_notification::<carrot_lsp::notification::DidChangeTextDocument>()
            .await
            .text_document,
        carrot_lsp::VersionedTextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/test.rs")).unwrap(),
            1
        )
    );
    ...
    assert_eq!(
        fake_json_server
            ...
            .text_document,
        carrot_lsp::TextDocumentItem {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/package.json")).unwrap(),
            version: 0,
            text: "{\"a\": 1}".to_string(),
    ...
    assert_eq!(
        fake_rust_server
            ...
            .text_document,
        carrot_lsp::VersionedTextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/test2.rs")).unwrap(),
            1
        )
    );
    ...
    assert_eq!(
        fake_rust_server
            .receive_notification::<carrot_lsp::notification::DidSaveTextDocument>()
            .await
            .text_document,
        carrot_lsp::TextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/Cargo.toml")).unwrap()
        )
    );
    assert_eq!(
        fake_json_server
    ...
    assert_eq!(
        fake_rust_server
            .receive_notification::<carrot_lsp::notification::DidCloseTextDocument>()
            .await
            .text_document,
        carrot_lsp::TextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/test2.rs")).unwrap()
    ...
    assert_eq!(
        fake_rust_server
            ...
            .text_document,
        carrot_lsp::TextDocumentItem {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/test3.rs")).unwrap(),
            version: 0,
    ...
    assert_eq!(
        fake_rust_server
            ...
            .text_document,
        carrot_lsp::TextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/test3.rs")).unwrap()
        ),
    );
    assert_eq!(
        fake_json_server
            ...
            .text_document,
        carrot_lsp::TextDocumentItem {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/test3.json")).unwrap(),
            version: 0,
            text: rust_buffer2.update(cx, |buffer, _| buffer.text()),
            language_id: "json".to_string(),
        },
    ...
    assert_eq!(
        fake_json_server
            ...
            .text_document,
        carrot_lsp::VersionedTextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/test3.json")).unwrap(),
            1
        )
    );
    ...
    let mut rust_shutdown_requests = fake_rust_server
        .set_request_handler::<carrot_lsp::request::Shutdown, _, _>(|_, _| future::ready(Ok(())));
    ...
    let close_message = carrot_lsp::DidCloseTextDocumentParams {
        text_document: carrot_lsp::TextDocumentIdentifier::new(
            carrot_lsp::Uri::from_file_path(path!("/dir/package.json")).unwrap(),
        ),
    };
    assert_eq!(
        fake_json_server
...
#[inazuma::test]
async fn test_language_server_relative_path(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_language_server_tilde_path(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let expected_path = carrot_paths::home_dir().join(".local/bin/rust-analyzer");
    assert_eq!(
...
async fn test_rescan_fs_change_is_reported_to_language_servers_as_changed(
    ...
    fake_server
        .request::<carrot_lsp::request::RegisterCapability>(
            carrot_lsp::RegistrationParams {
                registrations: vec![carrot_lsp::Registration {
                    id: Default::default(),
                    method: "workspace/didChangeWatchedFiles".to_string(),
                    register_options: serde_json::to_value(
                        carrot_lsp::DidChangeWatchedFilesRegistrationOptions {
                            watchers: vec![carrot_lsp::FileSystemWatcher {
                                glob_pattern: carrot_lsp::GlobPattern::String(
        ...
        .into_response()
        .unwrap();
    fake_server.handle_notification::<carrot_lsp::notification::DidChangeWatchedFiles, _>({
    ...
    assert_eq!(
        &*file_changes.lock(),
        &[carrot_lsp::FileEvent {
            uri: carrot_lsp::Uri::from_file_path(path!("/the-root/Cargo.lock")).unwrap(),
            typ: carrot_lsp::FileChangeType::CHANGED,
        }]
    );
}

#[inazuma::test]
async fn test_reporting_fs_changes_to_language_servers(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let _out_of_worktree_buffer = project
        .update(cx, |project, cx| {
            project.open_local_buffer_via_lsp(
                carrot_lsp::Uri::from_file_path(path!("/the-registry/dep1/src/dep1.rs")).unwrap(),
                server_id,
                cx,
            )
        })
    ...
    assert_eq!(
        ...
        &[
            ...
            carrot_lsp::FileEvent {
                uri: carrot_lsp::Uri::from_file_path(path!("/the-root/src/b.rs")).unwrap(),
                typ: carrot_lsp::FileChangeType::DELETED,
            },
            carrot_lsp::FileEvent {
                uri: carrot_lsp::Uri::from_file_path(path!("/the-root/src/c.rs")).unwrap(),
                typ: carrot_lsp::FileChangeType::CREATED,
            },
            carrot_lsp::FileEvent {
                uri: carrot_lsp::Uri::from_file_path(path!("/the-root/target/y/out/y2.rs"))
                    .unwrap(),
                typ: carrot_lsp::FileChangeType::CREATED,
            },
            carrot_lsp::FileEvent {
                uri: carrot_lsp::Uri::from_file_path(path!("/the/stdlib/src/string.rs")).unwrap(),
                typ: carrot_lsp::FileChangeType::CHANGED,
            },
        ]
    );
...
#[inazuma::test]
async fn test_single_file_worktrees_diagnostics(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    lsp_store.update(cx, |lsp_store, cx| {
        lsp_store
            .update_diagnostics(
                LanguageServerId(0),
                carrot_lsp::PublishDiagnosticsParams {
                    uri: Uri::from_file_path(path!("/dir/a.rs")).unwrap(),
                    version: None,
                    diagnostics: vec![carrot_lsp::Diagnostic {
                        range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(0, 4),
                            carrot_lsp::Position::new(0, 5),
                        ),
                        severity: Some(carrot_lsp::DiagnosticSeverity::ERROR),
...
#[inazuma::test]
async fn test_omitted_diagnostics(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
        ...
        lsp_store
            .update_diagnostics(
                ...
                carrot_lsp::PublishDiagnosticsParams {
                    ...
                    diagnostics: vec![carrot_lsp::Diagnostic {
                        range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(0, 8),
                            carrot_lsp::Position::new(0, 9),
                        ),
                        severity: Some(carrot_lsp::DiagnosticSeverity::ERROR),
...
#[inazuma::test]
async fn test_disk_based_diagnostics_progress(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server.notify::<carrot_lsp::notification::PublishDiagnostics>(
        carrot_lsp::PublishDiagnosticsParams {
            uri: Uri::from_file_path(path!("/dir/a.rs")).unwrap(),
            version: None,
            diagnostics: vec![carrot_lsp::Diagnostic {
        ...
        assert_eq!(
            ...
            &[DiagnosticEntryRef {
                ...
                diagnostic: &Diagnostic {
                    severity: carrot_lsp::DiagnosticSeverity::ERROR,
                    message: "undefined variable 'A'".to_string(),
                    group_id: 0,
                    is_primary: true,
...
#[inazuma::test]
async fn test_restarting_server_with_diagnostics_running(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    project.update(cx, |project, cx| {
        assert_eq!(
            project
                .language_servers_running_disk_based_diagnostics(cx)
                .collect::<Vec<_>>(),
            [] as [carrot_language::LanguageServerId; 0]
        );
    });
}

#[inazuma::test]
async fn test_restarting_server_with_diagnostics_published(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_restarted_server_reporting_invalid_buffer_version(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server.notify::<carrot_lsp::notification::PublishDiagnostics>(
        carrot_lsp::PublishDiagnosticsParams {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/a.rs")).unwrap(),
            version: Some(10000),
            diagnostics: Vec::new(),
        },
    );
...
#[inazuma::test]
async fn test_cancel_language_server_work(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server
        .start_progress_with(
            "another-token",
            carrot_lsp::WorkDoneProgressBegin {
    ...
    let cancel_notification = fake_server
        .receive_notification::<carrot_lsp::notification::WorkDoneProgressCancel>()
        .await;
    assert_eq!(
...
#[inazuma::test]
async fn test_toggling_enable_language_server(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_rust_server_1
        .receive_notification::<carrot_lsp::notification::Exit>()
        .await;
...
#[inazuma::test(iterations = 3)]
async fn test_transforming_diagnostics(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server.notify::<carrot_lsp::notification::PublishDiagnostics>(
        carrot_lsp::PublishDiagnosticsParams {
            ...
            diagnostics: vec![
                ...
                carrot_lsp::Diagnostic {
                    range: carrot_lsp::Range::new(
                        carrot_lsp::Position::new(1, 9),
                        carrot_lsp::Position::new(1, 11),
                    ),
                    severity: Some(DiagnosticSeverity::ERROR),
                ...
                carrot_lsp::Diagnostic {
                    range: carrot_lsp::Range::new(
                        carrot_lsp::Position::new(2, 9),
                        carrot_lsp::Position::new(2, 12),
                    ),
                    severity: Some(DiagnosticSeverity::ERROR),
                    source: Some("disk".to_string()),
...
#[inazuma::test]
async fn test_empty_diagnostic_ranges(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_diagnostics_from_multiple_language_servers(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_diagnostic_summaries_cleared_on_server_restart(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_diagnostic_summaries_cleared_on_buffer_reload(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut fake_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            capabilities: carrot_lsp::ServerCapabilities {
                diagnostic_provider: Some(carrot_lsp::DiagnosticServerCapabilities::Options(
                    carrot_lsp::DiagnosticOptions {
                ...
                ..carrot_lsp::ServerCapabilities::default()
            },
            initializer: Some(Box::new(move |fake_server| {
                let pull_count = closure_pull_count.clone();
                fake_server
                    .set_request_handler::<carrot_lsp::request::DocumentDiagnosticRequest, _, _>(
                        move |_, _| {
                            let pull_count = pull_count.clone();
                            async move {
                                pull_count.fetch_add(1, atomic::Ordering::SeqCst);
                                Ok(carrot_lsp::DocumentDiagnosticReportResult::Report(
                                    carrot_lsp::DocumentDiagnosticReport::Full(
                                        carrot_lsp::RelatedFullDocumentDiagnosticReport {
                                            related_documents: None,
                                            full_document_diagnostic_report:
                                                carrot_lsp::FullDocumentDiagnosticReport {
                                                    result_id: None,
                                                    items: Vec::new(),
                                                },
                                        },
...
#[inazuma::test]
async fn test_edits_from_lsp2_with_past_version(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_edits_from_lsp2_with_edits_on_adjacent_lines(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
        ...
        .update(cx, |lsp_store, cx| {
            lsp_store.as_local_mut().unwrap().edits_from_lsp(
                ...
                [
                    ...
                    carrot_lsp::TextEdit {
                        range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(1, 0),
                            carrot_lsp::Position::new(7, 0),
                        ),
                        new_text: "".into(),
                    },
                ],
...
#[inazuma::test]
async fn test_invalid_edits_from_lsp2(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
async fn test_definition(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server.set_request_handler::<carrot_lsp::request::GotoDefinition, _, _>(
        |params, _| async move {
            ...
            assert_eq!(params.position, carrot_lsp::Position::new(0, 22));

            Ok(Some(carrot_lsp::GotoDefinitionResponse::Scalar(
                carrot_lsp::Location::new(
                    carrot_lsp::Uri::from_file_path(path!("/dir/a.rs")).unwrap(),
                    carrot_lsp::Range::new(
                        carrot_lsp::Position::new(0, 9),
                        carrot_lsp::Position::new(0, 10),
                    ),
                ),
...
#[inazuma::test]
async fn test_completions_with_text_edit(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::Completion, _, _>(|_, _| async {
            Ok(Some(carrot_lsp::CompletionResponse::Array(vec![
                carrot_lsp::CompletionItem {
                    label: "labelText".into(),
                    insert_text: Some("insertText".into()),
                    text_edit: Some(carrot_lsp::CompletionTextEdit::Edit(carrot_lsp::TextEdit {
                        range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(0, text.len() as u32 - 3),
                            carrot_lsp::Position::new(0, text.len() as u32),
                        ),
...
#[inazuma::test]
async fn test_completions_with_edit_ranges(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    // Test 1: When text_edit is None but text_edit_text exists with default edit_range
    {
        ...
        fake_server
            .set_request_handler::<carrot_lsp::request::Completion, _, _>(|_, _| async {
                Ok(Some(carrot_lsp::CompletionResponse::List(
                    carrot_lsp::CompletionList {
                        is_incomplete: false,
                        item_defaults: Some(carrot_lsp::CompletionListItemDefaults {
                            edit_range: Some(
                                carrot_lsp::CompletionListItemDefaultsEditRange::Range(
                                    carrot_lsp::Range::new(
                                        carrot_lsp::Position::new(0, text.len() as u32 - 3),
                                        carrot_lsp::Position::new(0, text.len() as u32),
                                    ),
                        ...
                        items: vec![carrot_lsp::CompletionItem {
                            label: "labelText".into(),
...
#[inazuma::test]
async fn test_completions_without_edit_ranges(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::Completion, _, _>(|_, _| async move {
            Ok(Some(carrot_lsp::CompletionResponse::Array(vec![
                carrot_lsp::CompletionItem {
...
#[inazuma::test]
async fn test_completions_with_carriage_returns(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test(iterations = 10)]
async fn test_apply_code_actions_with_commands(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut fake_language_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            capabilities: carrot_lsp::ServerCapabilities {
                code_action_provider: Some(carrot_lsp::CodeActionProviderCapability::Options(
                    carrot_lsp::CodeActionOptions {
                        resolve_provider: Some(true),
                        ..carrot_lsp::CodeActionOptions::default()
                    },
                )),
                execute_command_provider: Some(carrot_lsp::ExecuteCommandOptions {
                    commands: vec!["_the/command".to_string()],
                    ..carrot_lsp::ExecuteCommandOptions::default()
                }),
                ..carrot_lsp::ServerCapabilities::default()
            },
            ..FakeLspAdapter::default()
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::CodeActionRequest, _, _>(|_, _| async move {
            Ok(Some(vec![
                carrot_lsp::CodeActionOrCommand::CodeAction(carrot_lsp::CodeAction {
                    title: "The code action".into(),
                    data: Some(serde_json::json!({
                        "command": "_the/command",
                    })),
                    ..carrot_lsp::CodeAction::default()
                }),
                carrot_lsp::CodeActionOrCommand::CodeAction(carrot_lsp::CodeAction {
                    title: "two".into(),
                    ..carrot_lsp::CodeAction::default()
    ...
    fake_server.set_request_handler::<carrot_lsp::request::CodeActionResolveRequest, _, _>(
        |mut action, _| async move {
            if action.data.is_some() {
                action.command = Some(carrot_lsp::Command {
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::ExecuteCommand, _, _>({
            ...
            move |params, _| {
                ...
                async move {
                    fake.server
                        .request::<carrot_lsp::request::ApplyWorkspaceEdit>(
                            carrot_lsp::ApplyWorkspaceEditParams {
                                label: None,
                                edit: carrot_lsp::WorkspaceEdit {
                                    changes: Some(
                                        [(
                                            carrot_lsp::Uri::from_file_path(path!("/dir/a.ts"))
                                                .unwrap(),
                                            vec![carrot_lsp::TextEdit {
                                                range: carrot_lsp::Range::new(
                                                    carrot_lsp::Position::new(0, 0),
                                                    carrot_lsp::Position::new(0, 0),
                                                ),
...
#[inazuma::test]
async fn test_rename_file_to_new_directory(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test(iterations = 10)]
async fn test_save_file(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test(iterations = 10)]
async fn test_save_file_spawns_language_server(cx: &mut inazuma::TestAppContext) {
    // Issue: #24349
    init_test(cx);
    ...
    assert_eq!(
        fake_rust_server
            ...
            .text_document,
        carrot_lsp::TextDocumentItem {
            uri: carrot_lsp::Uri::from_file_path(path!("/dir/file.rs")).unwrap(),
            version: 0,
            text: "".to_string(),
            language_id: "rust".to_string(),
        }
...
#[inazuma::test(iterations = 30)]
async fn test_file_changes_multiple_times_on_disk(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test(iterations = 30)]
async fn test_edit_buffer_while_it_reloads(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_save_in_single_file_worktree(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_save_as(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_save_as_existing_file(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test(retries = 5)]
async fn test_rescan_and_remote_updates(cx: &mut inazuma::TestAppContext) {
    use carrot_worktree::WorktreeModelHandle as _;
    ...
    let buffer_for_path = |path: &'static str, cx: &mut inazuma::TestAppContext| {
        let buffer = project.update(cx, |p, cx| p.open_local_buffer(dir.path().join(path), cx));
        async move { buffer.await.unwrap() }
    };
    let id_for_path = |path: &'static str, cx: &mut inazuma::TestAppContext| {
        project.update(cx, |project, cx| {
...
#[cfg(target_os = "linux")]
#[inazuma::test(retries = 5)]
async fn test_recreated_directory_receives_child_events(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
#[inazuma::test(iterations = 10)]
async fn test_buffer_identity_across_renames(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_buffer_deduping(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_buffer_is_dirty(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    buffer1.update(cx, |buffer, cx| {
        ...
        assert_eq!(
            *events.lock(),
            &[
                carrot_language::BufferEvent::Edited { is_local: true },
                carrot_language::BufferEvent::DirtyChanged
            ]
        );
        events.lock().clear();
        buffer.did_save(
    ...
    buffer1.update(cx, |buffer, cx| {
        assert!(!buffer.is_dirty());
        assert_eq!(*events.lock(), &[carrot_language::BufferEvent::Saved]);
        events.lock().clear();
    ...
    assert_eq!(
        ...
        &[carrot_language::BufferEvent::FileHandleChanged]
    );
...
#[inazuma::test]
async fn test_dirty_buffer_reloads_after_undo(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_buffer_file_changes_on_disk(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_buffer_line_endings(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_grouped_diagnostics(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let message = carrot_lsp::PublishDiagnosticsParams {
        uri: buffer_uri.clone(),
        diagnostics: vec![
            carrot_lsp::Diagnostic {
                range: carrot_lsp::Range::new(
                    carrot_lsp::Position::new(1, 8),
                    carrot_lsp::Position::new(1, 9),
                ),
                severity: Some(DiagnosticSeverity::WARNING),
                message: "error 1".to_string(),
                related_information: Some(vec![carrot_lsp::DiagnosticRelatedInformation {
                    location: carrot_lsp::Location {
                        uri: buffer_uri.clone(),
                        range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(1, 8),
                            carrot_lsp::Position::new(1, 9),
            ...
            carrot_lsp::Diagnostic {
                range: carrot_lsp::Range::new(
                    carrot_lsp::Position::new(2, 8),
                    carrot_lsp::Position::new(2, 17),
                ),
                severity: Some(DiagnosticSeverity::ERROR),
                message: "error 2".to_string(),
                related_information: Some(vec![
                    carrot_lsp::DiagnosticRelatedInformation {
                        location: carrot_lsp::Location {
                            uri: buffer_uri.clone(),
                            range: carrot_lsp::Range::new(
                                carrot_lsp::Position::new(1, 13),
                                carrot_lsp::Position::new(1, 15),
                            ),
                        },
...
#[inazuma::test]
async fn test_lsp_rename_notifications(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let language_registry = project.read_with(cx, |project, _| project.languages().clone());
    language_registry.add(rust_lang());
    let watched_paths = carrot_lsp::FileOperationRegistrationOptions {
        filters: vec![
            FileOperationFilter {
                scheme: Some("file".to_owned()),
                pattern: carrot_lsp::FileOperationPattern {
                    glob: "**/*.rs".to_owned(),
                    matches: Some(carrot_lsp::FileOperationPatternKind::File),
                    options: None,
                },
            },
            FileOperationFilter {
                ...
                pattern: carrot_lsp::FileOperationPattern {
                    glob: "**/**".to_owned(),
                    matches: Some(carrot_lsp::FileOperationPatternKind::Folder),
                    options: None,
                },
            },
        ],
    ...
    let mut fake_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            capabilities: carrot_lsp::ServerCapabilities {
                workspace: Some(carrot_lsp::WorkspaceServerCapabilities {
                    workspace_folders: None,
                    file_operations: Some(carrot_lsp::WorkspaceFileOperationsServerCapabilities {
    ...
    let expected_edit = carrot_lsp::WorkspaceEdit {
        changes: None,
        document_changes: Some(DocumentChanges::Edits({
            vec![TextDocumentEdit {
                edits: vec![carrot_lsp::Edit::Plain(carrot_lsp::TextEdit {
                    range: carrot_lsp::Range {
                        start: carrot_lsp::Position {
                            line: 0,
                            character: 1,
                        },
                        end: carrot_lsp::Position {
                            line: 0,
                            character: 3,
                        },
                    },
                ...
                text_document: carrot_lsp::OptionalVersionedTextDocumentIdentifier {
...
#[inazuma::test]
async fn test_rename(cx: &mut inazuma::TestAppContext) {
    // hi
    init_test(cx);
    ...
    let mut fake_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            capabilities: carrot_lsp::ServerCapabilities {
                rename_provider: Some(carrot_lsp::OneOf::Right(carrot_lsp::RenameOptions {
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::PrepareRenameRequest, _, _>(
            |params, _| async move {
                assert_eq!(
                    params.text_document.uri.as_str(),
                    uri!("file:///dir/one.rs")
                );
                assert_eq!(params.position, carrot_lsp::Position::new(0, 7));
                Ok(Some(carrot_lsp::PrepareRenameResponse::Range(
                    carrot_lsp::Range::new(
                        carrot_lsp::Position::new(0, 6),
                        carrot_lsp::Position::new(0, 9),
                    ),
                )))
            },
    ...
    fake_server
        .set_request_handler::<carrot_lsp::request::Rename, _, _>(|params, _| async move {
            assert_eq!(
            ...
            assert_eq!(
                ...
                carrot_lsp::Position::new(0, 7)
            );
            assert_eq!(params.new_name, "THREE");
            Ok(Some(carrot_lsp::WorkspaceEdit {
                changes: Some(
                    [
                        (
                            carrot_lsp::Uri::from_file_path(path!("/dir/one.rs")).unwrap(),
                            vec![carrot_lsp::TextEdit::new(
                                carrot_lsp::Range::new(
                                    carrot_lsp::Position::new(0, 6),
                                    carrot_lsp::Position::new(0, 9),
                                ),
                        ...
                        (
                            carrot_lsp::Uri::from_file_path(path!("/dir/two.rs")).unwrap(),
                            vec![
                                carrot_lsp::TextEdit::new(
                                    carrot_lsp::Range::new(
                                        carrot_lsp::Position::new(0, 24),
                                        carrot_lsp::Position::new(0, 27),
                                    ),
                                    "THREE".to_string(),
                                ),
                                carrot_lsp::TextEdit::new(
                                    carrot_lsp::Range::new(
                                        carrot_lsp::Position::new(0, 35),
                                        carrot_lsp::Position::new(0, 38),
                                    ),
                                    "THREE".to_string(),
                                ),
                            ],
...
#[inazuma::test]
async fn test_search(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_with_inclusions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_with_exclusions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_with_buffer_exclusions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_with_exclusions_and_inclusions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_multiple_worktrees_with_inclusions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_in_gitignored_dirs(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_search_with_unicode(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_create_entry(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_multiple_language_server_hovers(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut language_servers = [
        language_registry.register_fake_lsp(
            ...
            FakeLspAdapter {
                ...
                capabilities: carrot_lsp::ServerCapabilities {
                    hover_provider: Some(carrot_lsp::HoverProviderCapability::Simple(true)),
                    ..carrot_lsp::ServerCapabilities::default()
                },
                ..FakeLspAdapter::default()
            },
        ...
        match new_server_name.as_ref() {
            "TailwindServer" | "TypeScriptServer" => {
                servers_with_hover_requests.insert(
                    ...
                    new_server.set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(
                        move |_, _| {
                            let name = new_server_name.clone();
                            async move {
                                Ok(Some(carrot_lsp::Hover {
                                    contents: carrot_lsp::HoverContents::Scalar(
                                        carrot_lsp::MarkedString::String(format!("{name} hover")),
                                    ),
                                    range: None,
                                }))
                            }
            ...
            "NoHoverCapabilitiesServer" => {
                let _never_handled = new_server
                    .set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(|_, _| async move {
                        panic!(
...
#[inazuma::test]
async fn test_hovers_with_empty_parts(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut request_handled = fake_server
        .set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(move |_, _| async move {
            Ok(Some(carrot_lsp::Hover {
                contents: carrot_lsp::HoverContents::Array(vec![
                    carrot_lsp::MarkedString::String("".to_string()),
                    carrot_lsp::MarkedString::String("      ".to_string()),
                    carrot_lsp::MarkedString::String("\n\n\n".to_string()),
                ]),
                range: None,
            }))
        });
...
#[inazuma::test]
async fn test_code_actions_only_kinds(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut fake_language_servers = language_registry.register_fake_lsp(
        ...
        FakeLspAdapter {
            capabilities: carrot_lsp::ServerCapabilities {
                code_action_provider: Some(carrot_lsp::CodeActionProviderCapability::Simple(true)),
                ..carrot_lsp::ServerCapabilities::default()
            },
            ..FakeLspAdapter::default()
        },
    ...
    let mut request_handled = fake_server
        .set_request_handler::<carrot_lsp::request::CodeActionRequest, _, _>(
            move |_, _| async move {
                Ok(Some(vec![
                    carrot_lsp::CodeActionOrCommand::CodeAction(carrot_lsp::CodeAction {
...
async fn test_code_actions_without_requested_kinds_do_not_send_only_filter(
    ...
    let mut request_handled = fake_server.set_request_handler::<
        carrot_lsp::request::CodeActionRequest,
        _,
        _,
    >(move |params, _| async move {
        ...
        Ok(Some(vec![carrot_lsp::CodeActionOrCommand::CodeAction(
            carrot_lsp::CodeAction {
...
#[inazuma::test]
async fn test_multiple_language_server_actions(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    ...
    let mut language_server_rxs = [
        language_registry.register_fake_lsp(
            ...
            FakeLspAdapter {
                name: language_server_names[0],
                capabilities: carrot_lsp::ServerCapabilities {
                    code_action_provider: Some(carrot_lsp::CodeActionProviderCapability::Simple(
                        true,
                    )),
                    ..carrot_lsp::ServerCapabilities::default()
                },
        ...
        match new_server_name.0.as_ref() {
            "TailwindServer" | "TypeScriptServer" => {
                servers_with_actions_requests.insert(
                    new_server_name.clone(),
                    new_server.set_request_handler::<carrot_lsp::request::CodeActionRequest, _, _>(
                        move |_, _| {
                            let name = new_server_name.clone();
                            async move {
                                Ok(Some(vec![carrot_lsp::CodeActionOrCommand::CodeAction(
...
#[inazuma::test]
async fn test_reordering_worktrees(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_unstaged_diff_for_buffer(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_uncommitted_diff_for_buffer(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_staging_hunks(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test(seeds(340, 472))]
async fn test_staging_hunks_with_delayed_fs_event(cx: &mut inazuma::TestAppContext) {
    use DiffHunkSecondaryStatus::*;
    init_test(cx);
...
#[inazuma::test]
async fn test_single_file_diffs(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
#[cfg(not(windows))]
async fn test_staging_hunk_preserve_executable_permission(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_home_dir_as_git_repository(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    let fs = FakeFs::new(cx.background_executor.clone());
    let home = carrot_paths::home_dir();
    fs.insert_tree(
        home,
        json!({
            ".git": {},
...
#[inazuma::test]
async fn test_git_repository_status(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
#[inazuma::test]
#[ignore]
async fn test_git_status_postprocessing(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
async fn test_repository_pending_ops_staging(
    executor: inazuma::BackgroundExecutor,
    cx: &mut inazuma::TestAppContext,
) {
    init_test(cx);
    ...
    assert_eq!(
        pending_ops_all
            .lock()
            .get(
                &carrot_worktree::PathKey(repo_path("a.txt").as_ref().clone()),
                ()
            )
            .unwrap()
            .ops,
...
async fn test_repository_pending_ops_stage_all(
    ...
    assert_eq!(
        pending_ops_all
            .lock()
            .get(
                &carrot_worktree::PathKey(repo_path("b.txt").as_ref().clone()),
                ()
            )
            .unwrap()
            .ops,
...
#[cfg(any())]
#[inazuma::test]
async fn test_conflicted_cherry_pick(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
    ...
    pretty_assertions::assert_eq!(
        git_status(&repo),
        inazuma_collections::HashMap::from_iter([("a.txt".to_owned(), git2::Status::CONFLICTED)])
    );
    tree.flush_fs_events(cx).await;
    project
        .update(cx, |project, cx| project.git_scans_complete(cx))
    ...
    pretty_assertions::assert_eq!(git_status(&repo), inazuma_collections::HashMap::default());
    tree.flush_fs_events(cx).await;
    let conflicts = repository.update(cx, |repository, _| {
        repository
            .merge_conflicts
...
#[inazuma::test]
async fn test_update_gitignore(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    let fs = FakeFs::new(cx.background_executor.clone());
    fs.insert_tree(
        path!("/root"),
...
async fn test_rename_work_directory(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
    let root = TempTree::new(json!({
        "projects": {
...
#[inazuma::test]
#[cfg_attr(target_os = "windows", ignore)]
async fn test_file_status(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
#[inazuma::test]
#[ignore]
async fn test_ignored_dirs_events(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.executor().allow_parking();
...
#[inazuma::test(iterations = 10)]
async fn test_rescan_with_gitignore(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    cx.update(|cx| {
...
#[inazuma::test]
async fn test_git_worktrees_and_submodules(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_repository_deduplication(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
    let fs = FakeFs::new(cx.background_executor.clone());
    fs.insert_tree(
        path!("/root"),
...
#[inazuma::test]
async fn test_buffer_changed_file_path_updates_git_diff(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_undo_encoding_change(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_initial_scan_complete(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
pub fn init_test(cx: &mut inazuma::TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        carrot_release_channel::init(semver::Version::new(0, 0, 0), cx);
    });
}
...
fn git_commit(msg: &'static str, repo: &git2::Repository) {
    ...
    let signature = Signature::now("test", "test@carrot.dev").unwrap();
...
#[cfg(any())]
#[track_caller]
fn git_status(repo: &git2::Repository) -> inazuma_collections::HashMap<String, git2::Status> {
    repo.statuses(None)
        .unwrap()
        .iter()
...
#[inazuma::test]
async fn test_git_worktree_remove(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_optimistic_hunks_in_staged_files(cx: &mut inazuma::TestAppContext) {
    use DiffHunkSecondaryStatus::*;
    init_test(cx);
...
#[inazuma::test]
async fn test_read_only_files_setting(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_read_only_files_empty_setting(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
#[inazuma::test]
async fn test_read_only_files_with_lock_files(cx: &mut inazuma::TestAppContext) {
    init_test(cx);
...
mod disable_ai_settings_tests {
    ...
    async fn test_disable_ai_project_level_settings(cx: &mut TestAppContext) {
        use carrot_worktree::WorktreeId;
        use inazuma_settings_framework::{
            LocalSettingsKind, LocalSettingsPath, SettingsLocation, SettingsStore,
        };
        ...
        let rel_path = |path: &str| -> std::sync::Arc<inazuma_util::rel_path::RelPath> {
            std::sync::Arc::from(inazuma_util::rel_path::RelPath::unix(path).unwrap())
        };
