...
use carrot_project::{agent_server_store::*, worktree_store::WorktreeIdCounter};
use inazuma::{AppContext, AsyncApp, SharedString, Task, TestAppContext};
...
fn archive_launcher_constructs_with_all_fields() {
    use carrot_extension::AgentServerManifestEntry;
...
async fn archive_agent_uses_extension_and_agent_id_for_cache_key(cx: &mut TestAppContext) {
    ...
    let agent = LocalExtensionArchiveAgent {
        fs,
        http_client,
        node_runtime: carrot_node_runtime::NodeRuntime::unavailable(),
        project_environment,
...
fn test_tilde_expansion_in_settings() {
    let settings = inazuma_settings_framework::CustomAgentServerSettings::Custom {
