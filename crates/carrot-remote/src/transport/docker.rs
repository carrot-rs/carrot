...
impl DockerExecConnection {
    ...
    async fn ensure_server_binary(
        ...
        let tmp_path_gz = carrot_paths::remote_server_dir_relative().join(
            RelPath::unix(&format!(
                "{}-download-{}.gz",
                binary_name,
                std::process::id()
    ...
    async fn upload_and_chown(
        ...
        let mut command = inazuma_util::command::new_command(&docker_cli);
        command.arg("cp");
        command.arg("-a");
        ...
        let mut chown_command = inazuma_util::command::new_command(&docker_cli);
        ...
        anyhow::bail!(
            "failed to change ownership for carrot_remote_server via chown: {}",
            stderr,
        );
    }
    ...
    async fn run_docker_command(
        ...
        let mut command = inazuma_util::command::new_command(self.docker_cli());
    ...
    fn kill_inner(&self) -> Result<()> {
        if let Some(pid) = self.proxy_process.lock().take() {
            if let Ok(_) = inazuma_util::command::new_command("kill")
                .arg(pid.to_string())
                .spawn()
            {
                Ok(())
