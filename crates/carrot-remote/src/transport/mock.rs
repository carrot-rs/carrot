...
use carrot_rpc::{AnyProtoClient, proto::Envelope};
use futures::{
...
use inazuma::{App, AppContext as _, AsyncApp, Global, Task, TestAppContext};
use inazuma_collections::HashMap;
use inazuma_util::paths::{PathStyle, RemotePathBuf};
use std::{
    path::PathBuf,
    sync::{
        Arc,
...
impl RemoteClientDelegate for MockDelegate {
    fn ask_password(
        &self,
        _prompt: String,
        _sender: futures::channel::oneshot::Sender<carrot_askpass::EncryptedPassword>,
        _cx: &mut AsyncApp,
    ) {
    ...
    fn download_server_binary_locally(
        ...
        _release_channel: carrot_release_channel::ReleaseChannel,
