...
use carrot_paths::remote_server_dir_relative;
use carrot_release_channel::{AppVersion, ReleaseChannel};
use carrot_rpc::proto::Envelope;
use futures::{
    AsyncReadExt as _, FutureExt as _,
    channel::mpsc::{Sender, UnboundedReceiver, UnboundedSender},
    select_biased,
};
use inazuma::{App, AppContext as _, AsyncApp, Task};
use inazuma_collections::HashMap;
pub use inazuma_settings_framework::SshPortForwardOption;
use inazuma_util::command::{Child, Stdio};
use inazuma_util::{
...
impl From<inazuma_settings_framework::SshConnection> for SshConnectionOptions {
    fn from(val: inazuma_settings_framework::SshConnection) -> Self {
...
struct SshSocket {
    ...
    #[cfg(windows)]
    _proxy: carrot_askpass::PasswordProxy,
}
...
impl MasterProcess {
    pub fn new(
        ...
        let mut master_process = inazuma_util::command::new_command("ssh");
        master_process
            .kill_on_drop(true)
            .stdin(Stdio::null())
            .stdout(Stdio::piped())
...
impl SshRemoteConnection {
    pub(crate) async fn new(
        ...
        use carrot_askpass::AskPassResult;

        let destination = connection_options.ssh_destination();

        let temp_dir = tempfile::Builder::new()
            .prefix("carrot-ssh-session")
            .tempdir()?;
        let askpass_delegate = carrot_askpass::AskPassDelegate::new(cx, {
            let delegate = delegate.clone();
            move |prompt, tx, cx| delegate.ask_password(prompt, tx, cx)
        });

        let mut askpass =
            carrot_askpass::AskPassSession::new(cx.background_executor().clone(), askpass_delegate)
                .await?;
        ...
        let socket = SshSocket::new(
            ...
            askpass
                .get_password()
                .or_else(|| carrot_askpass::EncryptedPassword::try_from("").ok())
    ...
    async fn ensure_server_binary(
        ...
        let dst_path =
            carrot_paths::remote_server_dir_relative().join(RelPath::unix(&binary_name).unwrap());
        ...
        .await?
        {
            let tmp_path = carrot_paths::remote_server_dir_relative().join(
                RelPath::unix(&format!(
                    "download-{}-{}",
                    std::process::id(),
    ...
    fn build_scp_command(
        ...
        dest_path_str: &str,
        args: Option<&[&str]>,
    ) -> inazuma_util::command::Command {
        let mut command = inazuma_util::command::new_command("scp");
    ...
    fn build_sftp_command(&self) -> inazuma_util::command::Command {
        let mut command = inazuma_util::command::new_command("sftp");
        self.socket.ssh_options(&mut command, false).args(
            self.socket
                .connection_options
                .port
...
impl SshSocket {
    ...
    async fn new(
        ...
        password: carrot_askpass::EncryptedPassword,
        ...
        let _proxy = carrot_askpass::PasswordProxy::new(Box::new(get_password), executor).await?;
    ...
    fn ssh_command(
        ...
        let mut command = inazuma_util::command::new_command("ssh");
    ...
    fn ssh_options<'a>(
        &self,
        command: &'a mut inazuma_util::command::Command,
        include_port_forwards: bool,
    ) -> &'a mut inazuma_util::command::Command {
    ...
    async fn platform_windows(&self, shell: ShellKind) -> Result<RemotePlatform> {
        ...
        Ok(RemotePlatform {
            ...
            arch: match output.trim() {
                ...
                arch => anyhow::bail!(
                    "Prebuilt remote servers are not yet available for windows-{arch}. See https://carrot.dev/docs/remote-development"
                ),
            },
        })
    }
...
mod tests {
    ...
    fn test_build_command_quotes_env_assignment() -> Result<()> {
        let mut input_env = HashMap::default();
        input_env.insert("CARROT$(echo foo)".to_string(), "value".to_string());
        ...
        assert!(
            remote_command.contains("exec env 'CARROT$(echo foo)=value' remote_program"),
