...
use inazuma_util::command::Child;
...
fn parse_platform(output: &str) -> Result<RemotePlatform> {
    ...
    let os = match os {
        ...
        _ => anyhow::bail!(
            "Prebuilt remote servers are not yet available for {os:?}. See https://carrot.dev/docs/remote-development"
        ),
    };
        ...
        || arch.starts_with("aarch64")
    {
        ...
        anyhow::bail!(
            "Prebuilt remote servers are not yet available for {arch:?}. See https://carrot.dev/docs/remote-development"
        )
    };
...
async fn build_remote_server_from_source(
    ...
    use inazuma_util::command::{Command, Stdio, new_command};
