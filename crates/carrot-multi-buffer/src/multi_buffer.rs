...
use inazuma::{App, Context, Entity, EntityId, EventEmitter};
use inazuma_clock::ReplicaId;
use inazuma_collections::{BTreeMap, Bound, HashMap, HashSet};
use itertools::Itertools;

#[cfg(any(test, feature = "test-support"))]
use inazuma::AppContext as _;

use carrot_theme::SyntaxTheme;
use carrot_tracing::instrument;
use inazuma_rope::DimensionPair;
use inazuma_settings_framework::Settings;
use inazuma_sum_tree::{Bias, Cursor, Dimension, Dimensions, SumTree, TreeMap};
use inazuma_text::{
...
///
/// See <https://carrot.dev/features#multi-buffers>
...
pub enum Event {
    ExcerptsAdded {
        ...
        excerpts: Vec<(ExcerptId, ExcerptRange<carrot_language::Anchor>)>,
    },
    ExcerptsRemoved {
        ids: Vec<ExcerptId>,
...
pub struct MultiBufferDiffHunk {
    ...
    pub buffer_range: Range<inazuma_text::Anchor>,
...
impl TextDimension for BufferOffset {
    ...
    fn from_chunk(chunk: inazuma_rope::ChunkSlice) -> Self {
...
impl<'a> inazuma_sum_tree::Dimension<'a, inazuma_rope::ChunkSummary> for BufferOffset {
    fn zero(cx: ()) -> Self {
        BufferOffset(<usize as inazuma_sum_tree::Dimension<
            'a,
            inazuma_rope::ChunkSummary,
        >>::zero(cx))
    }

    fn add_summary(&mut self, summary: &'a inazuma_rope::ChunkSummary, cx: ()) {
...
impl carrot_language::ToPoint for BufferOffset {
    fn to_point(&self, snapshot: &inazuma_text::BufferSnapshot) -> Point {
        self.0.to_point(snapshot)
    }
}

impl carrot_language::ToPointUtf16 for BufferOffset {
    fn to_point_utf16(&self, snapshot: &inazuma_text::BufferSnapshot) -> PointUtf16 {
        self.0.to_point_utf16(snapshot)
    }
}

impl carrot_language::ToOffset for BufferOffset {
    fn to_offset(&self, snapshot: &inazuma_text::BufferSnapshot) -> usize {
        self.0.to_offset(snapshot)
    }
}

impl carrot_language::ToOffsetUtf16 for BufferOffset {
    fn to_offset_utf16(&self, snapshot: &inazuma_text::BufferSnapshot) -> OffsetUtf16 {
...
struct BufferState {
    buffer: Entity<Buffer>,
    last_version: RefCell<inazuma_clock::Global>,
    ...
    _subscriptions: [inazuma::Subscription; 2],
}

struct DiffState {
    diff: Entity<BufferDiff>,
    main_buffer: Option<Entity<carrot_language::Buffer>>,
...
struct DiffStateSnapshot {
    ...
    main_buffer: Option<carrot_language::BufferSnapshot>,
}
...
impl DiffState {
    ...
    fn new_inverted(
        diff: Entity<BufferDiff>,
        main_buffer: Entity<carrot_language::Buffer>,
...
struct DiffTransformHunkInfo {
    ...
    hunk_start_anchor: inazuma_text::Anchor,
...
pub struct ExcerptInfo {
    ...
    pub buffer_id: BufferId,
    pub range: ExcerptRange<inazuma_text::Anchor>,
...
pub struct RowInfo {
    ...
    pub diff_status: Option<carrot_buffer_diff::DiffHunkStatus>,
...
struct Excerpt {
    ...
    range: ExcerptRange<inazuma_text::Anchor>,
...
pub struct MultiBufferExcerpt<'a> {
    excerpt: &'a Excerpt,
    diff_transforms:
        inazuma_sum_tree::Cursor<'a, 'static, DiffTransform, DiffTransforms<MultiBufferOffset>>,
...
pub struct ReversedMultiBufferChunks<'a> {
    ...
    current_chunks: Option<inazuma_rope::Chunks<'a>>,
...
pub struct MultiBufferBytes<'a> {
    range: Range<MultiBufferOffset>,
    cursor: MultiBufferCursor<'a, MultiBufferOffset, BufferOffset>,
    excerpt_bytes: Option<inazuma_text::Bytes<'a>>,
    has_trailing_newline: bool,
    chunk: &'a [u8],
}
...
impl<'a, MBD: MultiBufferDimension> Dimension<'a, DiffTransformSummary> for DiffTransforms<MBD> {
    fn zero(cx: <DiffTransformSummary as inazuma_sum_tree::Summary>::Context<'_>) -> Self {
        Self {
    ...
    fn add_summary(
        ...
        cx: <DiffTransformSummary as inazuma_sum_tree::Summary>::Context<'_>,
    ) {
...
impl MultiBuffer {
    ...
    #[carrot_tracing::instrument(skip_all)]
    ...
    pub fn set_active_selections(
        ...
        let mut selections_by_buffer: HashMap<BufferId, Vec<Selection<inazuma_text::Anchor>>> =
            Default::default();
    ...
    pub fn excerpts_for_buffer(
        ...
        ExcerptId,
        Arc<BufferSnapshot>,
        ExcerptRange<inazuma_text::Anchor>,
    )> {
        let mut excerpts = Vec::new();
        let snapshot = self.read(cx);
        let mut cursor = snapshot.excerpts.cursor::<Option<&Locator>>(());
    ...
    pub fn excerpt_containing(
        ...
        cx: &App,
    ) -> Option<(ExcerptId, Entity<Buffer>, Range<inazuma_text::Anchor>)> {
        let snapshot = self.read(cx);
    ...
    pub fn text_anchor_for_position<T: ToOffset>(
        ...
        position: T,
        cx: &App,
    ) -> Option<(Entity<Buffer>, carrot_language::Anchor)> {
        let snapshot = self.read(cx);
    ...
    fn on_buffer_event(
        ...
        use carrot_language::BufferEvent;
    ...
    pub fn add_diff(&mut self, diff: Entity<BufferDiff>, cx: &mut Context<Self>) {
        ...
        self.buffer_diff_changed(
            diff.clone(),
            inazuma_text::Anchor::min_max_range_for_buffer(buffer_id),
            cx,
        );
    ...
    pub fn expand_or_collapse_diff_hunks_inner(
        ...
        for (range, end_excerpt_id) in ranges {
            for diff_hunk in snapshot.diff_hunks_in_range(range) {
                ...
                excerpt_edits.push(inazuma_text::Edit {
                    old: start..end,
                    new: start..end,
                });
            }
    ...
    fn sync_diff_transforms(
        snapshot: &mut MultiBufferSnapshot,
        excerpt_edits: Vec<inazuma_text::Edit<ExcerptOffset>>,
...
#[cfg(any(test, feature = "test-support"))]
impl MultiBuffer {
    pub fn build_simple(text: &str, cx: &mut inazuma::App) -> Entity<Self> {
    ...
    pub fn build_from_buffer(buffer: Entity<Buffer>, cx: &mut inazuma::App) -> Entity<Self> {
        cx.new(|cx| Self::singleton(buffer, cx))
    }

    pub fn build_random(rng: &mut impl rand::Rng, cx: &mut inazuma::App) -> Entity<Self> {
        cx.new(|cx| {
    ...
    pub fn randomly_edit(
        ...
        use inazuma_util::RandomCharIter;
...
impl MultiBufferSnapshot {
    ...
    /// Resolves the given [`inazuma_text::Anchor`]s to [`crate::Anchor`]s if the anchor is within a visible excerpt.
    ///
    /// The passed in anchors must be ordered.
    pub fn text_anchors_to_visible_anchors(
        &self,
        anchors: impl IntoIterator<Item = inazuma_text::Anchor>,
    ) -> Vec<Option<Anchor>> {
    ...
    pub fn clip_offset(&self, offset: MultiBufferOffset, bias: Bias) -> MultiBufferOffset {
        self.clip_dimension(offset, bias, inazuma_text::BufferSnapshot::clip_offset)
    }

    pub fn clip_point(&self, point: Point, bias: Bias) -> Point {
        self.clip_dimension(point, bias, inazuma_text::BufferSnapshot::clip_point)
    }
    ...
    pub fn clip_offset_utf16(
        ...
        self.clip_dimension(
            offset,
            bias,
            inazuma_text::BufferSnapshot::clip_offset_utf16,
        )
    }
    ...
    pub fn offset_to_point(&self, offset: MultiBufferOffset) -> Point {
        self.convert_dimension(offset, inazuma_text::BufferSnapshot::offset_to_point)
    }

    pub fn offset_to_point_utf16(&self, offset: MultiBufferOffset) -> PointUtf16 {
        self.convert_dimension(offset, inazuma_text::BufferSnapshot::offset_to_point_utf16)
    }

    pub fn point_to_point_utf16(&self, point: Point) -> PointUtf16 {
        self.convert_dimension(point, inazuma_text::BufferSnapshot::point_to_point_utf16)
    }

    pub fn point_utf16_to_point(&self, point: PointUtf16) -> Point {
        self.convert_dimension(point, inazuma_text::BufferSnapshot::point_utf16_to_point)
    }

    #[instrument(skip_all)]
    pub fn point_to_offset(&self, point: Point) -> MultiBufferOffset {
        self.convert_dimension(point, inazuma_text::BufferSnapshot::point_to_offset)
    }

    pub fn point_to_offset_utf16(&self, point: Point) -> MultiBufferOffsetUtf16 {
        self.convert_dimension(point, inazuma_text::BufferSnapshot::point_to_offset_utf16)
    }

    pub fn offset_utf16_to_offset(&self, offset: MultiBufferOffsetUtf16) -> MultiBufferOffset {
        self.convert_dimension(offset, inazuma_text::BufferSnapshot::offset_utf16_to_offset)
    }

    pub fn offset_to_offset_utf16(&self, offset: MultiBufferOffset) -> MultiBufferOffsetUtf16 {
        self.convert_dimension(offset, inazuma_text::BufferSnapshot::offset_to_offset_utf16)
    }

    pub fn point_utf16_to_offset(&self, point: PointUtf16) -> MultiBufferOffset {
        self.convert_dimension(point, inazuma_text::BufferSnapshot::point_utf16_to_offset)
    }

    pub fn point_utf16_to_offset_utf16(&self, point: PointUtf16) -> MultiBufferOffsetUtf16 {
        self.convert_dimension(
            point,
            inazuma_text::BufferSnapshot::point_utf16_to_offset_utf16,
        )
    }
    ...
    fn clip_dimension<MBD, BD>(
        ...
        position: MBD,
        bias: Bias,
        clip_buffer_position: fn(&inazuma_text::BufferSnapshot, BD, Bias) -> BD,
    ) -> MBD
    where
    ...
    fn convert_dimension<MBR1, MBR2, BR1, BR2>(
        &self,
        key: MBR1,
        convert_buffer_dimension: fn(&inazuma_text::BufferSnapshot, BR1) -> BR2,
    ) -> MBR2
    where
    ...
    fn resolve_summary_for_anchor<MBD>(
        ...
        >,
        excerpt_buffer: &inazuma_text::BufferSnapshot,
    ) -> MBD
    where
    ...
    /// Wraps the [`inazuma_text::Anchor`] in a [`crate::Anchor`] if this multi-buffer is a singleton.
    pub fn as_singleton_anchor(&self, text_anchor: inazuma_text::Anchor) -> Option<Anchor> {
    ...
    pub fn anchor_range_in_excerpt(
        &self,
        excerpt_id: ExcerptId,
        text_anchor: Range<inazuma_text::Anchor>,
    ) -> Option<Range<Anchor>> {
    ...
    pub fn anchor_in_excerpt(
        &self,
        excerpt_id: ExcerptId,
        text_anchor: inazuma_text::Anchor,
    ) -> Option<Anchor> {
        let excerpt = self.excerpt(self.latest_excerpt_id(excerpt_id))?;
    ...
    pub fn anchors_in_excerpt(
        &self,
        excerpt_id: ExcerptId,
        text_anchors: impl IntoIterator<Item = inazuma_text::Anchor>,
    ...
    fn anchor_in_excerpt_(excerpt: &Excerpt, text_anchor: inazuma_text::Anchor) -> Option<Anchor> {
    ...
    pub fn context_range_for_excerpt(
        &self,
        excerpt_id: ExcerptId,
    ) -> Option<Range<inazuma_text::Anchor>> {
    ...
    pub fn excerpt_range_for_excerpt(
        &self,
        excerpt_id: ExcerptId,
    ) -> Option<ExcerptRange<inazuma_text::Anchor>> {
    ...
    pub fn runnable_ranges(
        &self,
        range: Range<Anchor>,
    ) -> impl Iterator<Item = (Range<Anchor>, carrot_language::RunnableRange)> + '_ {
        let range = range.start.to_offset(self)..range.end.to_offset(self);
        self.lift_buffer_metadata(range, move |buffer, range| {
            Some(
                buffer
    ...
    where
        MBD::TextDimension: 'a
            + inazuma_text::ToOffset
            + inazuma_text::FromAnchor
            + Sub<Output = MBD::TextDimension>
            + fmt::Debug
            + ops::Add<Output = MBD::TextDimension>
            + ops::AddAssign
        ...
        V: std::fmt::Debug,
    {
        let text_ranges = ranges
            ...
            .collect();
        inazuma_text::debug::GlobalDebugRanges::with_locked(|debug_ranges| {
...
impl Excerpt {
    ...
    fn clip_anchor(&self, text_anchor: inazuma_text::Anchor) -> inazuma_text::Anchor {
        if text_anchor
...
impl inazuma_sum_tree::Item for Excerpt {
...
impl inazuma_sum_tree::Item for ExcerptIdMapping {
...
impl inazuma_sum_tree::KeyedItem for ExcerptIdMapping {
    type Key = ExcerptId;
...
impl inazuma_sum_tree::Item for DiffTransform {
    type Summary = DiffTransformSummary;

    fn summary(
        &self,
        _: <Self::Summary as inazuma_sum_tree::Summary>::Context<'_>,
    ) -> Self::Summary {
        match self {
...
impl inazuma_sum_tree::ContextLessSummary for DiffTransformSummary {
    fn zero() -> Self {
...
impl inazuma_sum_tree::ContextLessSummary for ExcerptId {
    fn zero() -> Self {
        Self(0)
    }
...
impl inazuma_sum_tree::ContextLessSummary for ExcerptSummary {
    fn zero() -> Self {
        Self::default()
    }
...
impl<'a> inazuma_sum_tree::SeekTarget<'a, ExcerptSummary, Option<&'a Locator>> for Locator {
    fn cmp(&self, cursor_location: &Option<&'a Locator>, _: ()) -> cmp::Ordering {
        Ord::cmp(&Some(self), cursor_location)
    }
}

impl inazuma_sum_tree::SeekTarget<'_, ExcerptSummary, ExcerptSummary> for Locator {
    fn cmp(&self, cursor_location: &ExcerptSummary, _: ()) -> cmp::Ordering {
        Ord::cmp(self, &cursor_location.excerpt_locator)
    }
}

impl<'a, MBD> inazuma_sum_tree::Dimension<'a, ExcerptSummary> for ExcerptDimension<MBD>
where
...
impl<'a> inazuma_sum_tree::Dimension<'a, ExcerptSummary> for Option<&'a Locator> {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, ExcerptSummary> for Option<ExcerptId> {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, DiffTransformSummary> for MultiBufferOffset {
    fn zero(_: ()) -> Self {
...
impl<MBD> inazuma_sum_tree::SeekTarget<'_, DiffTransformSummary, DiffTransformSummary>
...
impl<'a, MBD> inazuma_sum_tree::SeekTarget<'a, DiffTransformSummary, DiffTransforms<MBD>>
    for ExcerptDimension<MBD>
where
    MBD: MultiBufferDimension + Ord,
{
...
impl<'a, MBD: MultiBufferDimension> inazuma_sum_tree::Dimension<'a, DiffTransformSummary>
    for ExcerptDimension<MBD>
{
    fn zero(_: ()) -> Self {
        ExcerptDimension(MBD::default())
