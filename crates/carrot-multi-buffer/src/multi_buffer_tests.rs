...
use carrot_language::{Buffer, Rope};
use inazuma::{App, TestAppContext};
...
struct ReferenceMultibuffer {
    ...
    inverted_diffs: HashMap<BufferId, (Entity<BufferDiff>, Entity<carrot_language::Buffer>)>,
}
...
struct ReferenceExcerpt {
    ...
    expanded_diff_hunks: Vec<inazuma_text::Anchor>,
}
...
impl ReferenceMultibuffer {
    ...
    fn insert_excerpt_after(
        ...
        (buffer_handle, anchor_range): (Entity<Buffer>, Range<inazuma_text::Anchor>),
    ) {
        ...
        match rng.random_range(0..100) {
            ...
            _ => {
                ...
                let (excerpt_buffer, diff, inverted_main_buffer) = if create_new_buffer {
                    ...
                    if create_inverted {
                        let mut main_buffer_text = inazuma_util::RandomCharIter::new(&mut rng)
                            .take(256)
                            .collect::<String>();
                        let main_buffer = cx.new(|cx| Buffer::local(main_buffer_text.clone(), cx));
                        inazuma_text::LineEnding::normalize(&mut main_buffer_text);
                        let main_buffer_id =
                        ...
                        let mut base_text = inazuma_util::RandomCharIter::new(&mut rng)
                            .take(256)
                            .collect::<String>();
...
async fn test_summaries_for_anchors(cx: &mut TestAppContext) {
    ...
    let anchor_1 = Anchor::in_buffer(ids[0], inazuma_text::Anchor::MIN);
    let point_1 = snapshot.summaries_for_anchors::<Point, _>([&anchor_1])[0];
    assert_eq!(point_1, Point::new(0, 0));

    let anchor_2 = Anchor::in_buffer(ids[1], inazuma_text::Anchor::MIN);
...
fn assert_line_indents(snapshot: &MultiBufferSnapshot) {
    ...
    let text = inazuma_text::Buffer::new(ReplicaId::LOCAL, buffer_id, snapshot.text());
