...
use carrot_workspace::notifications::{NotificationId, show_app_notification};
use inazuma::{App, AppContext as _, DismissEvent, Global, actions};
use inazuma_util::{ResultExt as _, defer};
...
struct EtwSessionHandle {
    writer: carrot_net::OwnedWriteHalf,
    _listener: carrot_net::UnixListener,
    socket_path: PathBuf,
}
...
fn show_etw_notification(cx: &mut App, message: impl Into<inazuma::SharedString>) {
...
fn show_etw_notification_with_action(
    cx: &mut App,
    message: impl Into<inazuma::SharedString>,
    button_label: impl Into<inazuma::SharedString>,
    on_click: impl Fn(&mut inazuma::Window, &mut inazuma::Context<MessageNotification>)
    + Send
    + Sync
    + 'static,
) {
...
fn start_etw_recording(cx: &mut App, heap_pid: Option<u32>) {
    ...
    let save_dialog = cx.prompt_for_new_path(&PathBuf::default(), Some("carrot-trace.etl"));
...
const INSTANCE_NAME: &str = "Carrot";
...
fn heap_tracing_profile(heap_pid: Option<u32>) -> String {
    let (heap_provider, heap_collector) = match heap_pid {
        Some(pid) => (
            format!(
                r#"
    <HeapEventProvider Id="CarrotHeapProvider">
      ...
      <Collectors Operation="Add">
        <HeapEventCollectorId Value="HeapCollector_WPRHeapCollector">
          <HeapEventProviders Operation="Set">
            <HeapEventProviderId Value="CarrotHeapProvider"/>
          </HeapEventProviders>
...
<WindowsPerformanceRecorder Version="1.0" Author="Carrot HQ">
  <Profiles>
    {heap_provider}

    <Profile Id="CarrotHeap.Verbose.Memory" Base="Heap.Verbose.Memory" Name="CarrotHeap" DetailLevel="Verbose" LoggingMode="Memory" Description="Heap tracing">
      {heap_collector}
    </Profile>
  </Profiles>
...
fn build_profile_collection(heap_pid: Option<u32>) -> Result<IProfileCollection> {
    ...
    unsafe {
        ...
        collection
            ...
            .wpr_context(&collection)
            .context("Add CarrotHeap profile to collection")?;
    }
...
pub fn record_etw_trace(
    ...
    let mut stream =
        carrot_net::UnixStream::connect(socket_path).context("Connect to parent socket")?;
...
fn record_etw_trace_inner(
    heap_pid: Option<u32>,
    output_path: &Path,
    stream: &mut carrot_net::UnixStream,
) -> Result<()> {
...
fn receive_command(stream: &mut carrot_net::UnixStream) -> Result<(Command, bool)> {
...
pub struct EtwSession {
    output_path: PathBuf,
    stream: BufReader<carrot_net::UnixStream>,
    listener: carrot_net::UnixListener,
    socket_path: PathBuf,
}

pub fn launch_etw_recording(heap_pid: Option<u32>, output_path: &Path) -> Result<EtwSession> {
    let sock_path = std::env::temp_dir().join(format!("carrot-etw-{}.sock", std::process::id()));

    _ = std::fs::remove_file(&sock_path);
    let listener =
        carrot_net::UnixListener::bind(&sock_path).context("Bind Unix socket for ETW IPC")?;

    let exe_path = std::env::current_exe().context("Failed to get current exe path")?;
    let pid_arg = heap_pid.map_or(-1i64, |pid| pid as i64);
    let args = format!(
        "--record-etw-trace --etw-carrot-pid {} --etw-output \"{}\" --etw-socket \"{}\"",
        pid_arg,
        output_path.display(),
        sock_path.display(),
    );
