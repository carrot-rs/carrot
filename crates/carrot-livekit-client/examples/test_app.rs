...
use carrot_livekit_api::token::{self, VideoGrant};
use log::LevelFilter;
use simplelog::SimpleLogger;

actions!(carrot_livekit_client, [Quit]);
...
fn main() {
    ...
    inazuma_platform::application().run(|cx| {
        ...
        cx.set_menus([Menu::new("Carrot").items([MenuItem::action("Quit", Quit)])]);
...
fn quit(_: &Quit, cx: &mut inazuma::App) {
    cx.quit();
}

struct LivekitWindow {
    room: Arc<carrot_livekit_client::Room>,
    microphone_track: Option<LocalTrackPublication>,
    screen_share_track: Option<LocalTrackPublication>,
    microphone_stream: Option<carrot_livekit_client::AudioStream>,
    ...
    fn handle_room_event(&mut self, event: RoomEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match event {
            ...
            RoomEvent::TrackSubscribed {
                ...
                let output = self.remote_participant(participant);
                match track {
                    carrot_livekit_client::RemoteTrack::Audio(track) => {
                        output.audio_output_stream = Some((
                            publication,
                            room.play_remote_audio_track(&track, cx).unwrap(),
                        ));
                    }
                    carrot_livekit_client::RemoteTrack::Video(track) => {
