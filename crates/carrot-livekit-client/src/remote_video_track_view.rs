...
impl Render for RemoteVideoTrackView {
    fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(latest_frame) = &self.latest_frame {
            use inazuma::Styled as _;
            return inazuma::surface(latest_frame.clone())
                .size_full()
                .into_any_element();
        }
        ...
        if let Some(latest_frame) = &self.latest_frame {
            ...
            use inazuma::ParentElement;
            return ui::h_flex()
                .size_full()
                .child(inazuma::img(latest_frame.clone()).size_full())
                .into_any_element();
        }
