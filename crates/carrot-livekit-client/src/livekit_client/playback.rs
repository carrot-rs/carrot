...
use carrot_audio::{AudioSettings, CHANNEL_COUNT, SAMPLE_RATE};
use cpal::DeviceId;
...
use inazuma_util::{ResultExt as _, maybe};
use livekit::webrtc::{
                ...
                .spawn_with_priority(Priority::RealtimeAudio, async move {
                    maybe!({
                        ...
                        let mut rodio_effects = RodioEffectsAdaptor::new(buf.len())
                            .automatic_gain_control(0.50, 1.0, 0.0, 5.0);
                                    ...
                                    while data.len() > 0 {
                                        ...
                                        if buf.capacity() == buf.len() {
                                                ...
                                                .load(Ordering::Relaxed)
                                            {
                                                ...
                                                sampled.clear();
                                                sampled.extend(rodio_effects.by_ref().map(|s| {
                                                    use cpal::Sample;
                                                    s.to_sample::<i16>()
                                                }));
                                            }
...
/// is not owned by carrot-rs while the code would be surrounded by
/// carrot-rs owned code.
///
...
#[cfg(target_os = "macos")]
fn cv_pixel_buffer_to_retained(old: &core_video::pixel_buffer::CVPixelBuffer) -> RemoteVideoFrame {
    use core_foundation::base::TCFType;
    let raw = old.as_concrete_TypeRef() as *mut objc2_core_video::CVBuffer;
    unsafe { objc2_core_foundation::CFRetained::retain(std::ptr::NonNull::new_unchecked(raw)) }
}

#[cfg(not(target_os = "macos"))]
pub type RemoteVideoFrame = Arc<inazuma::RenderImage>;

#[cfg(not(target_os = "macos"))]
fn video_frame_buffer_from_webrtc(buffer: Box<dyn VideoBuffer>) -> Option<RemoteVideoFrame> {
    use image::{Frame, RgbaImage};
    use inazuma::RenderImage;
...
fn video_frame_buffer_to_webrtc(frame: ScreenCaptureFrame) -> Option<impl AsRef<dyn VideoBuffer>> {
    ...
    unsafe { Some(webrtc::video_frame::native::NativeBuffer::from_cv_pixel_buffer(pixel_buffer)) }
}
...
mod macos {
    ...
    pub struct PreventAppNapGuard {
        activity: Retained<objc2::runtime::AnyObject>,
    }
    ...
    impl Drop for PreventAppNapGuard {
        fn drop(&mut self) {
            unsafe {
                let process_info = NSProcessInfo::processInfo();
                let _: () = objc2::msg_send![&process_info, endActivity: &*self.activity];
            }
        }
    }

    /// Implementation from: https://github.com/carrot-rs/cpal/blob/fd8bc2fd39f1f5fdee5a0690656caff9a26d9d50/src/host/coreaudio/macos/property_listener.rs#L15
