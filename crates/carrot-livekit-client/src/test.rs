...
use carrot_livekit_api::{proto, token};
use inazuma::{App, AsyncApp, BackgroundExecutor};
use inazuma_collections::{
    BTreeMap, HashMap, HashSet, btree_map::Entry as BTreeEntry, hash_map::Entry,
};
...
impl TestServer {
    ...
    async fn join_room(&self, token: String, client_room: Room) -> Result<ParticipantIdentity> {
        ...
        let claims = carrot_livekit_api::token::validate(&token, &self.secret_key)?;
    ...
    pub(crate) fn is_track_muted(&self, token: &str, track_sid: &TrackSid) -> Option<bool> {
        let claims = carrot_livekit_api::token::validate(token, &self.secret_key).ok()?;
        let room_name = claims.video.room.unwrap();
...
#[async_trait]
impl carrot_livekit_api::Client for TestApiClient {
    fn url(&self) -> &str {
        &self.url
    }
    ...
    async fn update_participant(
        ...
        room: String,
        identity: String,
        permission: carrot_livekit_api::proto::ParticipantPermission,
    ) -> Result<()> {
        let server = TestServer::get(&self.url)?;
        server
...
impl Room {
    ...
    pub fn stats_task(
        &self,
        _cx: &impl inazuma::AppContext,
    ) -> inazuma::Task<Result<SessionStats>> {
        inazuma::Task::ready(Ok(SessionStats::default()))
    }
}
