...
use carrot_worktree::Snapshot;
use inazuma_settings_framework::DevContainerConnection;
use inazuma_util::command::Command;
...
impl DevContainerCli {
    fn command(&self, use_podman: bool) -> Command {
        let mut command = if let Some(node_runtime_path) = &self.node_runtime_path {
            let mut command = inazuma_util::command::new_command(
...
pub(crate) async fn ensure_devcontainer_cli(
    ...
    if let Err(e) = command.output().await {
        log::error!(
            "Unable to find devcontainer CLI in $PATH. Checking for a carrot installed version. Error: {:?}",
            e
        );
        ...
        let datadir_cli_path = carrot_paths::devcontainer_dir()
            .join("node_modules")
            .join("@devcontainers")
            .join("cli")
        ...
        if let Err(e) = fs::create_dir_all(carrot_paths::devcontainer_dir()).await {
...
mod tests {
    ...
    fn should_parse_from_devcontainer_json() {
        let json = r#"{"outcome":"success","containerId":"826abcac45afd412abff083ab30793daff2f3c8ce2c831df728baf39933cb37a","remoteUser":"vscode","remoteWorkspaceFolder":"/workspaces/carrot"}"#;
        ...
        assert_eq!(up.remote_workspace_folder, "/workspaces/carrot");

        let json_in_plaintext = r#"[2026-01-22T16:19:08.802Z] @devcontainers/cli 0.80.1. Node.js v22.21.1. darwin 24.6.0 arm64.
            {"outcome":"success","containerId":"826abcac45afd412abff083ab30793daff2f3c8ce2c831df728baf39933cb37a","remoteUser":"vscode","remoteWorkspaceFolder":"/workspaces/carrot"}"#;
