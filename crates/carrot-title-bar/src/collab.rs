...
use carrot_actions::ShowCallStats;
use carrot_call::{ActiveCall, Room};
use carrot_channel::ChannelStore;
use carrot_client::{User, proto::PeerId};
use carrot_livekit_client::ConnectionQuality;
use carrot_project::WorktreeSettings;
use carrot_remote_connection::RemoteConnectionModal;
...
use carrot_workspace::{ParticipantLocation, notifications::DetachAndPromptErr};
use inazuma::{
    AnyElement, IntoElement, MouseButton, Oklch, Path, ScreenCaptureSource, Styled, WeakEntity,
    canvas, point,
};
use inazuma::{App, Task, Window};
use inazuma_icons::IconName;
use inazuma_settings_framework::{Settings as _, SettingsLocation};
...
impl TitleBar {
                        ...
                        .child(
                            Avatar::new(user.avatar_uri.clone())
                                ...
                                .when(is_muted, |avatar| {
                                    avatar.indicator(
                                        AvatarAudioStatusIndicator::new(
                                            carrot_ui::AudioStatus::Muted,
                                        )
                                        .tooltip({
    ...
    fn render_screen_list(&self) -> impl IntoElement {
        PopoverMenu::new("screen-share-screen-list")
            ...
            .trigger(
                carrot_ui::ButtonLike::new_rounded_right("screen-share-screen-list-trigger")
                    .child(
                        h_flex()
                            .mx_neg_0p5()
                            .h_full()
