...
use carrot_cloud_api_types::ExtensionProvides;
use carrot_extension::extension_builder::{CompileExtensionOptions, ExtensionBuilder};
use carrot_extension::{ExtensionManifest, ExtensionSnippets};
use carrot_fs::{CopyOptions, Fs, RealFs, copy_recursive};
use carrot_language::LanguageConfig;
use carrot_reqwest_client::ReqwestClient;
use carrot_snippet_provider::file_to_snippets;
use carrot_snippet_provider::format::VsSnippetsFile;
...
async fn main() -> Result<()> {
    ...
    let manifest_json = serde_json::to_string(&carrot_cloud_api_types::ExtensionApiManifest {
        name: manifest.name,
...
async fn test_themes(
    ...
    for relative_theme_path in &manifest.themes {
        ...
        let theme_family =
            carrot_theme_settings::deserialize_user_theme(&fs.load_bytes(&theme_path).await?)?;
