...
use carrot_ui::{Icon, IconName, Label, h_flex, prelude::*};
use carrot_workspace::{
    ItemNavHistory, PaneRole, Workspace,
...
use inazuma_text::{BufferId, OffsetRangeExt};
...
pub fn init(cx: &mut App) {
    carrot_editor::set_diagnostic_renderer(diagnostic_renderer::DiagnosticRenderer {}, cx);
...
pub(crate) struct ProjectDiagnosticsEditor {
    ...
    diagnostics: HashMap<BufferId, Vec<DiagnosticEntry<inazuma_text::Anchor>>>,
...
impl ProjectDiagnosticsEditor {
    ...
    fn new(
        ...
        let project_event_subscription = cx.subscribe_in(
            ...
            |this, _project, event, window, cx| match event {
                carrot_project::Event::DiskBasedDiagnosticsStarted { .. } => {
                    cx.notify();
                }
                carrot_project::Event::DiskBasedDiagnosticsFinished { language_server_id } => {
    ...
    fn diagnostics_are_unchanged(
        &self,
        existing: &[DiagnosticEntry<inazuma_text::Anchor>],
        new: &[DiagnosticEntryRef<'_, inazuma_text::Anchor>],
    ...
    fn update_excerpts(
        ...
        cx.spawn_in(window, async move |this, cx| {
            let diagnostics = buffer_snapshot
                .diagnostics_in_range::<_, inazuma_text::Anchor>(
            ...
            let cmp_excerpts =
                |buffer_snapshot: &BufferSnapshot,
                 a: &ExcerptRange<inazuma_text::Anchor>,
                 b: &ExcerptRange<inazuma_text::Anchor>| {
            ...
            this.update_in(cx, |this, window, cx| {
                ...
                #[cfg(test)]
                {
                        ...
                        block_ids.iter().zip(cloned_blocks.into_iter().flatten())
                    {
                        ...
                        carrot_editor::test::set_block_content_for_tests(
                            &this.editor,
                            *block_id,
                            cx,
                            move |cx| {
                                carrot_markdown::MarkdownElement::rendered_text(
                                    markdown.clone(),
                                    cx,
                                    carrot_editor::hover_popover::diagnostics_markdown_style,
                                )
                            },
                        );
                    }
...
impl DiagnosticsToolbarEditor for WeakEntity<ProjectDiagnosticsEditor> {
    ...
    fn get_diagnostics_for_buffer(
        ...
        cx: &App,
    ) -> Vec<carrot_language::DiagnosticEntry<inazuma_text::Anchor>> {
        self.read_with(cx, |project_diagnostics_editor, _cx| {
            project_diagnostics_editor
                .diagnostics
                .get(&buffer_id)
