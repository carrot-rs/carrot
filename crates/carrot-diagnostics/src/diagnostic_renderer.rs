...
use carrot_language::{BufferId, Diagnostic, DiagnosticEntryRef, LanguageRegistry};
use carrot_lsp::DiagnosticSeverity;
use carrot_markdown::{Markdown, MarkdownElement};
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{CopyButton, prelude::*};
use inazuma::{AppContext, Entity, Focusable, WeakEntity};
use inazuma_settings_framework::Settings;
use inazuma_text::{AnchorRangeExt, Point};
...
impl carrot_editor::DiagnosticRenderer for DiagnosticRenderer {
    fn render_group(
        &self,
        diagnostic_group: Vec<DiagnosticEntryRef<'_, Point>>,
        buffer_id: BufferId,
...
impl DiagnosticBlock {
    pub fn render_block(&self, editor: WeakEntity<Editor>, bcx: &BlockContext) -> AnyElement {
        ...
        let (background_color, border_color) = match self.severity {
            DiagnosticSeverity::ERROR => {
                (status_colors.error.background, status_colors.error.color)
            }
    ...
    pub fn open_link(
        ...
        let Some(diagnostic_link) = link.strip_prefix("file://#diagnostic-") else {
            carrot_editor::hover_popover::open_markdown_url(link, window, cx);
            return;
        };
