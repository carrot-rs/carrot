...
use carrot_language::{Buffer, Capability, DiagnosticEntry, DiagnosticEntryRef, Point};
use carrot_project::{
    DiagnosticSummary, Event, Project, ProjectItem, ProjectPath,
    project_settings::{DiagnosticSeverity, ProjectSettings},
};
use carrot_ui::{Button, ButtonStyle, Icon, IconName, Label, Tooltip, h_flex, prelude::*};
use carrot_workspace::{
    ItemHandle, ItemNavHistory, PaneRole, Workspace,
...
use inazuma_text::{Anchor, BufferSnapshot, OffsetRangeExt};
use std::{
    any::{Any, TypeId},
    cmp::{self, Ordering},
    sync::Arc,
...
impl BufferDiagnosticsEditor {
    ...
    fn update_excerpts(
        ...
        let max_severity = Self::max_diagnostics_severity(self.include_warnings)
            .into_lsp()
            .unwrap_or(carrot_lsp::DiagnosticSeverity::WARNING);
        ...
        cx.spawn_in(window, async move |buffer_diagnostics_editor, mut cx| {
            ...
            for diagnostic_block in blocks.iter() {
                ...
                let bin_search = |probe: &ExcerptRange<inazuma_text::Anchor>| {
                    let context_start = || {
                        probe
                            .context
                            .start
            ...
            buffer_diagnostics_editor.update_in(cx, |buffer_diagnostics_editor, window, cx| {
                ...
                // `carrot_editor::test::editor_content_with_blocks` function can then
...
impl Item for BufferDiagnosticsEditor {
    ...
    fn save(
        &mut self,
        options: carrot_workspace::item::SaveOptions,
        project: Entity<Project>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
...
impl DiagnosticsToolbarEditor for WeakEntity<BufferDiagnosticsEditor> {
    ...
    fn get_diagnostics_for_buffer(
        &self,
        _buffer_id: inazuma_text::BufferId,
        cx: &App,
