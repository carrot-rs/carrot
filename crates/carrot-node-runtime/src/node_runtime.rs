...
struct NodeRuntimeState {
    ...
    options: carrot_watch::Receiver<Option<NodeBinaryOptions>>,
...
impl NodeRuntime {
    ...
    pub fn unavailable() -> Self {
        NodeRuntime(Arc::new(Mutex::new(NodeRuntimeState {
            http: Arc::new(carrot_http_client::BlockedHttpClient),
            instance: None,
            last_options: None,
            options: carrot_watch::channel(Some(NodeBinaryOptions::default())).1,
        ...
        let instance = if options.allow_binary_download {
            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                Ok(instance) => {
                    log::log!(
                        log_level,
                        "using Carrot managed Node.js at {} since {}",
                ...
                Err(err) => {
                    ...
                    Box::new(UnavailableNodeRuntime {
                        error_message: format!(
                            "failure while downloading and/or installing Carrot managed Node.js, \
                            restart Carrot to retry: {}",
                            err
                        )
                        .into(),
                    }) as Box<dyn NodeRuntimeTrait>
...
impl ManagedNodeRuntime {
    ...
    async fn install_if_needed(http: &Arc<dyn HttpClient>) -> Result<Self> {
        ...
        let node_containing_dir = carrot_paths::data_dir().join("node");
        ...
        let valid = if fs::metadata(&node_binary).await.is_ok() {
            let result = inazuma_util::command::new_command(&node_binary)
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "Carrot managed Node.js binary at {} failed check with output: {:?}",
                            node_binary.display(),
                            output
                        );
                        false
                ...
                Err(err) => {
                    log::warn!(
                        "Carrot managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        node_binary.display(),
                        err
                    );
...
impl NodeRuntimeTrait for ManagedNodeRuntime {
    ...
    async fn run_npm_subcommand(
        ...
        let attempt = || async {
            ...
            let mut command = inazuma_util::command::new_command(npm_command.path);
...
impl SystemNodeRuntime {
    ...
    async fn new(node: PathBuf, npm: PathBuf) -> Result<Self> {
        let output = inazuma_util::command::new_command(&node)
            .arg("--version")
            .output()
            .await
        ...
        let scratch_dir = carrot_paths::data_dir().join("node");
...
fn configure_npm_command(command: &mut inazuma_util::command::Command, directory: Option<&Path>) {
