...
use inazuma_gpui_util::defer;

use anyhow::anyhow;
use bytes::{BufMut, Bytes, BytesMut};
use carrot_http_client::{RedirectPolicy, Url, http};
...
impl ReqwestClient {
    ...
    pub fn proxy_and_user_agent(proxy: Option<Url>, user_agent: &str) -> anyhow::Result<Self> {
        ...
        let client = client
            .use_preconfigured_tls(carrot_http_client_tls::tls_config())
            .build()?;
...
impl carrot_http_client::HttpClient for ReqwestClient {
    ...
    fn send(
        &self,
        req: http::Request<carrot_http_client::AsyncBody>,
    ) -> futures::future::BoxFuture<
        'static,
        anyhow::Result<carrot_http_client::Response<carrot_http_client::AsyncBody>>,
    > {
        ...
        let request = request.body(match body.0 {
            carrot_http_client::Inner::Empty => reqwest::Body::default(),
            carrot_http_client::Inner::Bytes(cursor) => cursor.into_inner().into(),
            carrot_http_client::Inner::AsyncReader(stream) => {
        ...
        async move {
            ...
            *builder.headers_mut().unwrap() = headers;
            ...
            let bytes = response
                ...
                .into_async_read();
            let body = carrot_http_client::AsyncBody::from_reader(bytes);
...
#[cfg(test)]
mod tests {
    use carrot_http_client::{HttpClient, Url};
