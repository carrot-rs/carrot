...
use carrot_buffer_diff::{DiffHunkStatus, DiffHunkStatusKind};
use carrot_feature_flags::{DiffReviewFeatureFlag, FeatureFlagAppExt as _};
use carrot_file_icons::FileIcons;
use carrot_git::{Oid, blame::BlameEntry, commit::ParsedCommitMessage, status::FileStatus};
use carrot_language::{
    HighlightedText, IndentGuideSettings, language_settings::ShowWhitespaceSetting,
};
use carrot_markdown::Markdown;
use carrot_multi_buffer::{
...
use inazuma::{
    ...
    FontWeight, GlobalElementId, Hitbox, HitboxBehavior, InteractiveElement, IntoElement, IsZero,
    Length, Modifiers, ModifiersChangedEvent, MouseButton, MouseClickEvent, MouseDownEvent,
    MouseMoveEvent, MousePressureEvent, MouseUpEvent, Oklch, PaintQuad, ParentElement, Pixels,
...
use carrot_theme::{ActiveTheme, Appearance, PlayerColor};
use carrot_theme_settings::BufferLineHeight;
use carrot_ui::{
    ButtonLike, ContextMenu, Indicator, KeyBinding, POPOVER_Y_PADDING, ShowScrollbar, Tooltip,
...
use inazuma_util::{RangeExt, ResultExt, debug_panic};
...
impl EditorElement {
    ...
    fn mouse_dragged(
        ...
        let scroll_delta = {
            ...
            let mut scroll_delta = inazuma::Point::<f32>::default();
    ...
    pub(crate) fn mouse_moved(
            ...
            && split_side != Some(SplitSide::Left)
        {
            ...
            {
                let as_point =
                    inazuma_text::ToPoint::to_point(&buffer_anchor.text_anchor, buffer_snapshot);
    ...
    fn layout_visible_cursors(
        ...
        text_hitbox: &Hitbox,
        content_origin: inazuma::Point<Pixels>,
        scroll_position: inazuma::Point<ScrollOffset>,
        scroll_pixel_position: inazuma::Point<ScrollPixelOffset>,
        line_height: Pixels,
        em_width: Pixels,
        em_advance: Pixels,
    ...
    fn layout_scrollbars(
        ...
        content_offset: inazuma::Point<Pixels>,
        scroll_position: inazuma::Point<ScrollOffset>,
    ...
    fn layout_minimap(
        ...
        scroll_position: inazuma::Point<f64>,
        scrollbar_layout_information: &ScrollbarLayoutInformation,
    ...
    fn prepaint_expand_toggles(
        &self,
        expand_toggles: &mut [Option<(AnyElement, inazuma::Point<Pixels>)>],
        window: &mut Window,
        cx: &mut App,
    ) {
    ...
    fn layout_inline_diagnostics(
        ...
        scroll_position: inazuma::Point<ScrollOffset>,
        scroll_pixel_position: inazuma::Point<ScrollPixelOffset>,
        edit_prediction_popover_origin: Option<inazuma::Point<Pixels>>,
        start_row: DisplayRow,
        end_row: DisplayRow,
        line_height: Pixels,
        em_width: Pixels,
        ...
        let severity_to_color = |sev: &carrot_lsp::DiagnosticSeverity| match sev {
            &carrot_lsp::DiagnosticSeverity::ERROR => Color::Error,
            &carrot_lsp::DiagnosticSeverity::WARNING => Color::Warning,
            &carrot_lsp::DiagnosticSeverity::INFORMATION => Color::Info,
            &carrot_lsp::DiagnosticSeverity::HINT => Color::Hint,
            _ => Color::Error,
        };
    ...
    fn layout_inline_code_actions(
        ...
        let icon_size = carrot_ui::IconSize::XSmall;
        ...
        let absolute_offset = inazuma::point(start_x, start_y);
        button.layout_as_root(inazuma::AvailableSpace::min_size(), window, cx);
        button.prepaint_as_root(
            absolute_offset,
            inazuma::AvailableSpace::min_size(),
            window,
            cx,
        );
        Some(button)
    ...
    fn layout_blame_popover(
        ...
        let cursor_point = self
            ...
            .read(cx)
            .selections
            .newest::<carrot_language::Point>(&editor_snapshot.display_snapshot)
            .head();
    ...
    fn layout_blame_entries(
        ...
        let mut last_used_color: Option<(Oklch, Oid)> = None;
    ...
    fn layout_indent_guides(
        &self,
        content_origin: inazuma::Point<Pixels>,
        text_origin: inazuma::Point<Pixels>,
    ...
    fn calculate_indent_guide_bounds(
        ...
        line_height: Pixels,
        snapshot: &DisplaySnapshot,
    ) -> (f64, inazuma::Pixels, Range<DisplayRow>) {
    ...
    fn layout_expand_toggles(
        ...
        window: &mut Window,
        cx: &mut App,
    ) -> Vec<Option<(AnyElement, inazuma::Point<Pixels>)>> {
    ...
    fn layout_line_numbers(
        ...
        let segments = buffer_rows.iter().enumerate().flat_map(|(ix, row_info)| {
            ...
            if !(snapshot.number_deleted_lines
                || relative_line_numbers_enabled && relative_number.is_some())
                && row_info
                    .diff_status
                    .is_some_and(|status| status.is_deleted())
            {
    ...
    fn bg_segments_per_row(
        rows: Range<DisplayRow>,
        selections: &[(PlayerColor, Vec<SelectionLayout>)],
        highlight_ranges: &[(Range<DisplayPoint>, Oklch)],
        base_background: Oklch,
    ) -> Vec<Vec<(Range<DisplayPoint>, Oklch)>> {
    ...
    fn merge_overlapping_ranges(
        ranges: Vec<(Range<DisplayPoint>, Oklch)>,
        base_background: Oklch,
    ) -> Vec<(Range<DisplayPoint>, Oklch)> {
        struct Boundary {
            pos: DisplayPoint,
            is_start: bool,
            index: usize,
        ...
        let mut processed_ranges: Vec<(Range<DisplayPoint>, Oklch)> = Vec::new();
        let mut active_ranges: SmallVec<[(usize, Oklch); 8]> = SmallVec::new();
    ...
    fn layout_lines(
        ...
        bg_segments_per_row: &[Vec<(Range<DisplayPoint>, Oklch)>],
        window: &mut Window,
        cx: &mut App,
    ...
    fn layout_popovers_above_or_below_line(
        &self,
        target_position: inazuma::Point<Pixels>,
        line_height: Pixels,
        min_height: Pixels,
        max_height: Pixels,
        ...
        window.with_text_style(Some(text_style), |window| {
            ...
            let mut current_position = inazuma::Point {
    ...
    fn layout_hover_popovers(
        ...
        fn place_popovers_above(
            hovered_point: inazuma::Point<Pixels>,
    ...
    fn layout_word_diff_highlights(
        ...
        start_row: DisplayRow,
        snapshot: &EditorSnapshot,
        highlighted_ranges: &mut Vec<(Range<DisplayPoint>, Oklch)>,
        cx: &mut App,
    ) {
        ...
        for (hunk, _) in display_hunks {
            ...
            } = &hunk
            {
                    ...
                    .any(|row| row.is_some_and(|row| display_row_range.contains(&row)))
                {
                    ...
                    let bounds = Bounds::new(inazuma::Point::new(x, y), size);
                    control_bounds.push((display_row_range.start, bounds));

                    window.with_absolute_element_offset(inazuma::Point::new(x, y), |window| {
    ...
    fn paint_indent_guides(
        ...
        let faded_color = |color: Oklch, alpha: f32| {
            let mut faded = color;
            faded.a = alpha;
            faded
        };
    ...
    fn paint_redactions(&mut self, layout: &EditorLayout, window: &mut Window) {
        ...
        let redaction_color = inazuma::rgb(0x0e1111);
                    ...
                    let scrollbar_markers = cx
                        .background_spawn(async move {
                                ...
                                background_highlights.iter()
                            {
                                ...
                                {
                                    let mut color = theme.status().info.color;
                            ...
                            if scrollbar_settings.diagnostics != ScrollbarDiagnostics::None {
                                    ...
                                    .filter(|diagnostic| {
                                        match (
                                            ...
                                            (
                                                ...
                                                carrot_lsp::DiagnosticSeverity::ERROR,
                                            ) => true,
                                            (
                                                ScrollbarDiagnostics::Warning,
                                                carrot_lsp::DiagnosticSeverity::ERROR
                                                | carrot_lsp::DiagnosticSeverity::WARNING,
                                            ) => true,
                                            (
                                ...
                                let marker_row_ranges = diagnostics.into_iter().map(|diagnostic| {
                                    ...
                                    let color = match diagnostic.diagnostic.severity {
                                        ...
                                        carrot_lsp::DiagnosticSeverity::INFORMATION => {
                                            theme.status().info.color
                                        }
                                        _ => theme.status().hint.color,
                                    };
                                    ColoredRange {
                ...
                if phase == DispatchPhase::Bubble && hitbox.should_handle_scroll(window) {
                    ...
                    editor.update(cx, |editor, cx| {
                        ...
                        let (delta, axis) = match delta {
                            inazuma::ScrollDelta::Pixels(mut pixels) => {
                                //Trackpad
                                let axis = position_map.snapshot.ongoing_scroll.filter(&mut pixels);
                                (pixels, axis)
                            }

                            inazuma::ScrollDelta::Lines(lines) => {
                                //Not trackpad
...
pub fn render_breadcrumb_text(
    ...
    prefix: Option<inazuma::AnyElement>,
    ...
    let highlighted_segments = segments.into_iter().enumerate().map(|(index, segment)| {
        ...
        if index == 0
            && !carrot_workspace::TabBarSettings::get_global(cx).show
        ...
        Some(editor) => element
            ...
            .child(
                ButtonLike::new("toggle outline view")
                    .child(breadcrumbs)
                    .when(multibuffer_header, |this| this.style(ButtonStyle::Ghost))
                    .when(!multibuffer_header, |this| {
                            ...
                            v_flex()
                                ...
                                .child(
                                    h_flex()
                                        ...
                                        .child(carrot_ui::KeyBinding::for_action_in(
                        ...
                        .on_click({
                            ...
                            move |_, window, cx| {
                                if let Some((editor, callback)) = editor
                                    .upgrade()
                                    .zip(carrot_actions::outline::TOGGLE_OUTLINE.get())
                                {
...
fn apply_dirty_filename_style(
    ...
    text_style: &inazuma::TextStyle,
    cx: &App,
) -> Option<inazuma::AnyElement> {
    ...
    let highlight_style = inazuma::HighlightStyle {
...
struct JumpTargetInExcerptInput<'a> {
    id: ExcerptId,
    buffer: &'a carrot_language::BufferSnapshot,
    excerpt_start_anchor: inazuma_text::Anchor,
    jump_anchor: inazuma_text::Anchor,
}
...
fn header_jump_data_inner(
    ...
    let jump_position = carrot_language::ToPoint::to_point(&for_excerpt.jump_anchor, buffer);
    let excerpt_start = for_excerpt.excerpt_start_anchor;
    let rows_from_excerpt_start = if for_excerpt.jump_anchor == excerpt_start {
        0
    } else {
        let excerpt_start_point = carrot_language::ToPoint::to_point(&excerpt_start, buffer);
                            ...
                            |path_header| {
                                ...
                                path_header
                                    ...
                                    .when_some(breadcrumbs, |then, breadcrumbs| {
                                        let font =
                                            carrot_theme_settings::ThemeSettings::get_global(cx)
                                                .buffer_font
                                                .clone();
                        ...
                        project.read(cx).worktree_for_id(file.worktree_id(cx), cx)
                {
                    ...
                    menu = menu
                        .when_some(abs_path, |menu, abs_path| {
                            menu.entry(
                                "Copy Path",
                                Some(Box::new(carrot_actions::workspace::CopyPath)),
                        ...
                        .when_some(relative_path, |menu, relative_path| {
                            menu.entry(
                                "Copy Relative Path",
                                Some(Box::new(carrot_actions::workspace::CopyRelativePath)),
                                window.handler_for(&editor, move |_, _, cx| {
                                    cx.write_to_clipboard(ClipboardItem::new_string(
                                ...
                                window.handler_for(&editor, move |editor, _, cx| {
                                    if let Some(project) = &mut editor.project {
                                        project.update(cx, |_, cx| {
                                            cx.emit(carrot_project::Event::RevealInProjectPanel(
                                                entry_id,
                                            ))
                                        });
                                    }
...
fn render_blame_entry(
    ...
    blame_entry: BlameEntry,
    style: &EditorStyle,
    last_used_color: &mut Option<(Oklch, Oid)>,
    editor: Entity<Editor>,
...
impl LineWithInvisibles {
    ...
    fn split_runs_by_bg_segments(
        text_runs: &[TextRun],
        bg_segments: &[(Range<DisplayPoint>, Oklch)],
        min_contrast: f32,
    ...
    fn prepaint_with_custom_offset(
        ...
        let mut fragment_origin =
            content_origin + inazuma::point(Pixels::from(-scroll_pixel_position.x), line_y);
                ...
                match buffer_font_size {
                    AbsoluteLength::Pixels(pixels) => {
                        let rem_size_scale = {
                            ...
                            let default_font_size_scale = 14. / carrot_ui::BASE_REM_SIZE_IN_PX;
                    ...
                    AbsoluteLength::Rems(rems) => {
                        Some(rems.to_pixels(carrot_ui::BASE_REM_SIZE_IN_PX.into()))
                    }
                }
            }
...
impl Element for EditorElement {
    ...
    fn request_layout(
        ...
        _inspector_id: Option<&inazuma::InspectorElementId>,
        window: &mut Window,
        cx: &mut App,
        ...
        window.with_rem_size(rem_size, |window| {
            window.with_text_style(Some(text_style), |window| {
                window.with_content_mask(Some(ContentMask { bounds }), |window| {
                    ...
                    let scroll_max: inazuma::Point<ScrollPixelOffset> = point(
                        ScrollPixelOffset::from(
...
pub struct EditorLayout {
    ...
    spacer_blocks: Vec<BlockLayout>,
    highlighted_ranges: Vec<(Range<DisplayPoint>, Oklch)>,
    highlighted_gutter_ranges: Vec<(Range<DisplayPoint>, Oklch)>,
    redacted_ranges: Vec<Range<DisplayPoint>>,
    cursors: Vec<(DisplayPoint, Oklch)>,
    ...
    expand_toggles: Vec<Option<(AnyElement, inazuma::Point<Pixels>)>>,
    ...
    document_colors: Option<(DocumentColorsRenderMode, Vec<(Range<DisplayPoint>, Oklch)>)>,
...
struct StickyHeaders {
    ...
    gutter_background: Oklch,
    content_background: Oklch,
...
impl ScrollbarLayout {
    ...
    fn thumb_hovered(&self, position: &inazuma::Point<Pixels>) -> bool {
        self.thumb_bounds
...
pub(crate) struct PositionMap {
    ...
    pub scroll_position: inazuma::Point<ScrollOffset>,
    pub scroll_pixel_position: inazuma::Point<ScrollPixelOffset>,
    pub scroll_max: inazuma::Point<ScrollOffset>,
    pub em_width: Pixels,
...
impl PositionMap {
    pub(crate) fn point_for_position(&self, position: inazuma::Point<Pixels>) -> PointForPosition {
...
impl CursorLayout {
    ...
    pub fn bounding_rect(&self, origin: inazuma::Point<Pixels>) -> Bounds<Pixels> {
        Bounds {
    ...
    fn bounds(&self, origin: inazuma::Point<Pixels>) -> Bounds<Pixels> {
        match self.shape {
            ...
            CursorShape::Underline => Bounds {
                origin: self.origin
                    + origin
                    + inazuma::Point::new(Pixels::ZERO, self.line_height - px(2.0)),
    ...
    pub fn paint(&mut self, origin: inazuma::Point<Pixels>, window: &mut Window, cx: &mut App) {
        let bounds = self.bounds(origin);
...
impl HighlightedRange {
    ...
    fn paint_lines(
        ...
        let mut builder = if fill {
            inazuma::PathBuilder::fill()
        } else {
            inazuma::PathBuilder::stroke(px(1.))
        };
...
mod tests {
    ...
    use carrot_language::{Buffer, language_settings, tree_sitter_python};
    use inazuma::{TestAppContext, VisualTestContext, oklch, oklcha};
    use inazuma_util::test::sample_text;
    use log::info;
                        ...
                        .map(|row| RowInfo {
                            ...
                            diff_status: (row == DELETED_LINE).then(|| {
                                DiffHunkStatus::deleted(
                                    carrot_buffer_diff::DiffHunkSecondaryStatus::NoSecondaryHunk,
                                )
                            }),
                            ..Default::default()
                        })
    ...
    async fn test_layout_line_numbers_with_folded_lines(cx: &mut TestAppContext) {
        ...
        let python_lang = carrot_languages::language("python", tree_sitter_python::LANGUAGE.into());
    ...
    fn test_merge_overlapping_ranges() {
        ...
        let blended = Oklch::blend(Oklch::blend(base_bg, color1), color2);
    ...
    #[cfg(test)]
    fn generate_test_run(len: usize, color: Oklch) -> TextRun {
        TextRun {
            len,
            color,
            ..Default::default()
    ...
    #[inazuma::test]
    fn test_split_runs_by_bg_segments(cx: &mut inazuma::TestAppContext) {
        init_test(cx, |_| {});
