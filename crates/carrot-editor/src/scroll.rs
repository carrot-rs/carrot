...
use carrot_language::language_settings::{AllLanguageSettings, SoftWrap};
use carrot_language::{Bias, Point};
use carrot_ui::ScrollbarAutoHide;
use carrot_workspace::{ItemId, WorkspaceId};
use core::fmt::Debug;
use inazuma::{
...
pub struct ScrollAnchor {
    pub offset: inazuma::Point<ScrollOffset>,
    pub anchor: Anchor,
}
...
impl ScrollAnchor {
    pub(super) fn new() -> Self {
        Self {
            offset: inazuma::Point::default(),
            anchor: Anchor::min(),
        }
    }

    pub fn scroll_position(&self, snapshot: &DisplaySnapshot) -> inazuma::Point<ScrollOffset> {
...
impl OngoingScroll {
    ...
    pub fn filter(&self, delta: &mut inazuma::Point<Pixels>) -> Option<Axis> {
...
impl ScrollManager {
    ...
    pub fn offset(&self, cx: &App) -> inazuma::Point<f64> {
    ...
    fn set_anchor(
        ...
        let adjusted_anchor = if self.forbid_vertical_scroll {
            ...
            ScrollAnchor {
                offset: inazuma::Point::new(anchor.offset.x, current.scroll_anchor.offset.y),
...
impl Editor {
    ...
    fn set_scroll_position_taking_display_map(
        ...
        let adjusted_position = if self.scroll_manager.forbid_vertical_scroll {
            ...
            inazuma::Point::new(scroll_position.x, current_position.y)
        } else {
            scroll_position
        };
    ...
    pub fn scroll_position(&self, cx: &mut Context<Self>) -> inazuma::Point<ScrollOffset> {
        let display_map = self.display_map.update(cx, |map, cx| map.snapshot(cx));
