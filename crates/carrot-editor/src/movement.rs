...
use carrot_language::{CharClassifier, Point};
use carrot_multi_buffer::{MultiBufferOffset, MultiBufferRow, MultiBufferSnapshot};
use carrot_workspace::searchable::Direction;
use inazuma::{Pixels, WindowTextSystem};
...
mod tests {
    ...
    use inazuma::{AppContext as _, font, px};
    use inazuma_settings_framework::SettingsStore;
    use inazuma_util::post_inc;

    #[inazuma::test]
    fn test_previous_word_start(cx: &mut inazuma::App) {
        init_test(cx);

        fn assert(marked_text: &str, cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_previous_subword_start(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_find_preceding_boundary(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_find_preceding_boundary_with_inlays(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_next_word_end(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_next_subword_end(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_find_boundary(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_move_up_and_down_with_excerpts(cx: &mut inazuma::TestAppContext) {
        cx.update(|cx| {
            init_test(cx);
        });
