...
//! [`TextSummary`]: carrot_multi_buffer::MBTextSummary
//! [`WrapRow`]: wrap_map::WrapRow
//! [`InlayBufferRows`]: inlay_map::InlayBufferRows
//! [`InlayChunks`]: inlay_map::InlayChunks
//! [`Edit<T>`]: inazuma_text::Edit
//! [`Edit<S>`]: inazuma_text::Edit
//! [`Chunk`]: carrot_language::Chunk
...
use inazuma_collections::{HashMap, HashSet, IndexSet};
...
use carrot_project::project_settings::DiagnosticSeverity;
use carrot_project::{InlayId, lsp_store::LspFoldingRange, lsp_store::TokenType};
use carrot_tracing::instrument;
use carrot_ui::{SharedString, px};
use inazuma_settings_framework::Settings;
use inazuma_sum_tree::{Bias, TreeMap};
use inazuma_text::{BufferId, LineIndent, Patch};
...
pub enum HighlightKey {
    ...
    /// Carrot cmdline — AST-driven command-line token role. The
    /// `usize` is the role tier (Command, Subcommand, Flag,
    /// Positional, Separator, Path, GitRef, …) so each role picks
    /// its own `HighlightStyle`.
    CommandAst(usize),
...
pub struct SemanticTokenHighlight {
    ...
    pub server_id: carrot_lsp::LanguageServerId,
}
...
impl DisplayMap {
    ...
    pub(crate) fn set_companion(
        ...
        let Some((companion_display_map, companion)) = companion else {
            ...
            let edits = Patch::new(vec![inazuma_text::Edit {
                old: WrapRow(0)
        ...
        companion_display_map.update(cx, |companion_display_map, cx| {
            ...
            let companion_edits = Patch::new(vec![inazuma_text::Edit {
                old: WrapRow(0)
                    ..companion_display_map
                        .block_map
                        .wrap_snapshot
    ...
    pub fn fold_buffers(
        &mut self,
        buffer_ids: impl IntoIterator<Item = carrot_language::BufferId>,
        cx: &mut App,
    ) {
    ...
    #[instrument(skip_all)]
    pub(crate) fn is_buffer_folded(&self, buffer_id: carrot_language::BufferId) -> bool {
            ...
            let placeholder = if let Some(collapsed_text) = folding_range.collapsed_text {
                FoldPlaceholder {
                    render: Arc::new({
                        ...
                        move |fold_id, _fold_range, cx: &mut inazuma::App| {
                            use inazuma::{Element as _, ParentElement as _};
    ...
    #[cfg(test)]
    pub fn is_rewrapping(&self, cx: &inazuma::App) -> bool {
...
impl DisplaySnapshot {
                ...
                .map(|severity| HighlightStyle {
                    ...
                    underline: (chunk.underline
                        ...
                        && !(chunk.is_unnecessary
                            && severity > carrot_lsp::DiagnosticSeverity::WARNING))
                        .then(|| {
    ...
    pub(super) fn combined_highlights(
        ...
        syntax_theme: &carrot_theme::SyntaxTheme,
...
pub mod tests {
    ...
    use inazuma::{
        App, AppContext as _, BorrowAppContext, Element, Oklch, Rgba, div, font, observe, px,
    };

    use carrot_theme::{LoadThemes, SyntaxTheme};
    use inazuma_settings_framework::{SettingsContent, SettingsStore};
    use inazuma_text::PointUtf16;
    use inazuma_util::test::{marked_text_ranges, sample_text};
    ...
    async fn test_random_display_map(cx: &mut inazuma::TestAppContext, mut rng: StdRng) {
        ...
        let buffer = cx.update(|cx| {
            if rng.random() {
                ...
                let text = inazuma_util::RandomCharIter::new(&mut rng)
                    .take(len)
                    .collect::<String>();
    ...
    async fn test_soft_wraps(cx: &mut inazuma::TestAppContext) {
        ...
        _ = cx.update_window(window, |_, window, cx| {
            ...
            assert_eq!(
                movement::up(
                    ...
                    carrot_language::SelectionGoal::None,
                    false,
                    &text_layout_details,
                ),
                (
                    DisplayPoint::new(DisplayRow(0), 7),
                    carrot_language::SelectionGoal::HorizontalPosition(f64::from(x))
                )
            );
            assert_eq!(
                movement::down(
    ...
    #[inazuma::test]
    fn test_text_chunks(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});
    ...
    #[inazuma::test]
    fn test_inlays_with_newlines_after_blocks(cx: &mut inazuma::TestAppContext) {
    ...
    #[inazuma::test]
    async fn test_chunks(cx: &mut inazuma::TestAppContext) {
        let text = r#"
            fn outer() {}
        ...
        let theme = SyntaxTheme::new_test(vec![
            ("mod.body", inazuma::red()),
            ("fn.name", inazuma::blue()),
        ]);
        ...
        assert_eq!(
            ...
            vec![
                ("fn ".to_string(), None),
                ("outer".to_string(), Some(inazuma::blue())),
                ("() {}\n\nmod module ".to_string(), None),
                ("{\n    fn ".to_string(), Some(inazuma::red())),
                ("inner".to_string(), Some(inazuma::blue())),
                ("() {}\n}".to_string(), Some(inazuma::red())),
            ]
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            vec![
                ("fn ".to_string(), None),
                ("out".to_string(), Some(inazuma::blue())),
                ("⋯".to_string(), None),
                ("  fn ".to_string(), Some(inazuma::red())),
                ("inner".to_string(), Some(inazuma::blue())),
                ("() {}\n}".to_string(), Some(inazuma::red())),
    ...
    #[inazuma::test]
    async fn test_chunks_with_syntax_highlighting_across_blocks(cx: &mut inazuma::TestAppContext) {
        cx.background_executor
            .set_block_on_ticks(usize::MAX..=usize::MAX);
        ...
        let theme = SyntaxTheme::new_test(vec![
            ("string", inazuma::red()),
            ("punctuation", inazuma::blue()),
            ("keyword", inazuma::green()),
        ]);
        let language = Arc::new(
            Language::new(
                LanguageConfig {
        ...
        pretty_assertions::assert_eq!(
            ...
            [
                ("const".into(), Some(inazuma::green())),
                (" A".into(), None),
                (":".into(), Some(inazuma::blue())),
                (" &str = ".into(), None),
                ("\"\n    one\n".into(), Some(inazuma::red())),
                ("\n".into(), None),
                ("    two\n    three\n\"".into(), Some(inazuma::red())),
                (";".into(), Some(inazuma::blue())),
                ("\n".into(), None),
    ...
    #[inazuma::test]
    async fn test_chunks_with_diagnostics_across_blocks(cx: &mut inazuma::TestAppContext) {
        cx.background_executor
            .set_block_on_ticks(usize::MAX..=usize::MAX);
        ...
        let black = inazuma::black().to_rgb();
        let red = inazuma::red().to_rgb();
        ...
        let snapshot = map.update(cx, |map, cx| map.snapshot(cx));
        let mut chunks = Vec::<(String, Option<carrot_lsp::DiagnosticSeverity>, Rgba)>::new();
    ...
    #[inazuma::test]
    async fn test_point_translation_with_replace_blocks(cx: &mut inazuma::TestAppContext) {
        cx.background_executor
            .set_block_on_ticks(usize::MAX..=usize::MAX);
    ...
    #[inazuma::test]
    async fn test_chunks_with_soft_wrapping(cx: &mut inazuma::TestAppContext) {
        cx.background_executor
            .set_block_on_ticks(usize::MAX..=usize::MAX);
        ...
        assert_eq!(
            ...
            [
                ("fn \n".to_string(), None),
                ("oute".to_string(), Some(inazuma::blue())),
                ("\n".to_string(), None),
                ("r".to_string(), Some(inazuma::blue())),
                ("() \n{}\n\n".to_string(), None),
            ]
        );
        assert_eq!(
    ...
    #[inazuma::test]
    async fn test_chunks_with_text_highlights(cx: &mut inazuma::TestAppContext) {
        cx.update(|cx| init_test(cx, &|_| {}));

        let theme = SyntaxTheme::new_test(vec![
            ("operator", inazuma::red()),
            ("string", inazuma::green()),
        ]);
        let language = Arc::new(
            Language::new(
                LanguageConfig {
        ...
        assert_eq!(
            ...
            [
                ...
                ("a".to_string(), None, Some(inazuma::blue())),
                (":".to_string(), Some(inazuma::red()), Some(inazuma::blue())),
                (" B = ".to_string(), None, None),
                ("\"c ".to_string(), Some(inazuma::green()), None),
                (
                    "d".to_string(),
                    Some(inazuma::green()),
                    Some(inazuma::blue())
                ),
                ("\"".to_string(), Some(inazuma::green()), None),
            ]
        );
    }

    #[inazuma::test]
    fn test_clip_point(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});

        fn assert(text: &str, shift_right: bool, bias: Bias, cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_clip_at_line_ends(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});

        fn assert(text: &str, cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_creases(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});
    ...
    #[inazuma::test]
    fn test_tabs_with_multibyte_chars(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});
    ...
    #[inazuma::test]
    fn test_max_point(cx: &mut inazuma::App) {
        init_test(cx, &|_| {});
    ...
    fn chunks(
        ...
        theme: &SyntaxTheme,
        cx: &mut App,
    ) -> Vec<(String, Option<Oklch>, Option<Oklch>)> {
        let snapshot = map.update(cx, |map, cx| map.snapshot(cx));
        let mut chunks: Vec<(String, Option<Oklch>, Option<Oklch>)> = Vec::new();
    ...
    #[inazuma::test]
    fn test_isomorphic_display_point_ranges_for_buffer_range(cx: &mut inazuma::TestAppContext) {
        cx.update(|cx| init_test(cx, &|_| {}));
