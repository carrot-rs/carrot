...
use carrot_language::{Chunk, Edit, Point, TextSummary};
...
use carrot_ui::{ActiveTheme, IntoElement as _, ParentElement as _, Styled as _, div};
use inazuma_collections::BTreeSet;
use inazuma_sum_tree::{Bias, Cursor, Dimensions, SumTree};
use inazuma_text::{ChunkBitmaps, Patch};
use std::{
    cmp, iter,
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for MultiBufferOffset {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for Point {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
pub struct InlayChunks<'a> {
    ...
    inlay_chunks: Option<inazuma_text::ChunkWithBitmaps<'a>>,
        ...
        let chunk = match self.transforms.item()? {
            ...
            Transform::Inlay(inlay) => {
                                            ...
                                            div()
                                                ...
                                                .border_color(
                                                    if cx.theme().appearance().is_light() {
                                                        inazuma::black().opacity(0.5)
                                                    } else {
                                                        inazuma::white().opacity(0.5)
                                                    },
                                                )
                                                .bg(color),
                                        )
                ...
                let inlay_chunks = self.inlay_chunks.get_or_insert_with(|| {
                    ...
                    inazuma_text::ChunkWithBitmaps(chunks)
                });
                let ChunkBitmaps {
                    text: inlay_chunk,
                    chars,
...
impl InlayMap {
    ...
    pub fn sync(
        ...
        mut buffer_edits: Vec<inazuma_text::Edit<MultiBufferOffset>>,
...
impl InlaySnapshot {
    ...
    #[carrot_tracing::instrument(skip_all, fields(point))]
...
mod tests {
    ...
    use carrot_multi_buffer::Anchor;
    use carrot_project::{InlayHint, InlayHintLabel, ResolveState};
    use inazuma::{App, HighlightStyle};
    use inazuma_collections::HashMap;
    use inazuma_settings_framework::SettingsStore;
    use inazuma_sum_tree::TreeMap;
    use inazuma_text::{Patch, Rope};
        ...
        assert_eq!(
            Inlay::hint(
                ...
                &InlayHint {
                    ...
                    position: inazuma_text::Anchor::MIN,
                    padding_left: false,
                    padding_right: false,
                    tooltip: None,
                    kind: None,
    ...
    fn test_inlay_utf8_boundary_panic_fix(cx: &mut App) {
        ...
        //
        // See https://github.com/carrot-rs/carrot/issues/33641
        ...
        let inlay = Inlay {
            id: InlayId::Hint(0),
            position,
            content: InlayContent::Text(inazuma_text::Rope::from(inlay_text)),
        };
    ...
    fn test_inlay_utf8_boundaries(cx: &mut App) {
        ...
        for test_case in test_cases {
            ...
            let inlay = Inlay {
                id: InlayId::Hint(0),
                position,
                content: InlayContent::Text(inazuma_text::Rope::from(test_case.inlay_text)),
            };
