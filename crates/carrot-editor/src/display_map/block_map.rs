...
use carrot_language::{Patch, Point};
...
use carrot_ui::{ElementId, IntoElement};
use inazuma::{AnyElement, App, EntityId, Pixels, Window};
use inazuma_collections::{Bound, HashMap, HashSet};
use inazuma_sum_tree::{Bias, ContextLessSummary, Dimensions, SumTree, TreeMap};
use inazuma_text::{BufferId, Edit};
...
const NEWLINES: &[u8; inazuma_rope::Chunk::MASK_BITS] = &[b'\n'; _];
const BULLETS: &[u8; inazuma_rope::Chunk::MASK_BITS] = &[b'*'; _];
...
#[derive(inazuma::AppContext, inazuma::VisualContext)]
...
pub struct BlockChunks<'a> {
    transforms: inazuma_sum_tree::Cursor<'a, 'static, Transform, Dimensions<BlockRow, WrapRow>>,
...
impl BlockMap {
    ...
    #[carrot_tracing::instrument(skip_all, fields(edits = ?edits))]
    fn sync(
        &self,
        wrap_snapshot: &WrapSnapshot,
        mut edits: WrapPatch,
        ...
        while let Some(edit) = edits.next() {
            let span = carrot_tracing::debug_span!("while edits", edit = ?edit);
            ...
            loop {
                let span = carrot_tracing::debug_span!("decide where edit ends loop");
                let _enter = span.enter();
            ...
            for (block_placement, block) in blocks_in_edit.drain(..) {
                let span = carrot_tracing::debug_span!(
                    "for block in edits",
...
#[carrot_tracing::instrument(skip(tree, wrap_snapshot))]
...
impl inazuma_sum_tree::Item for Transform {
...
impl inazuma_sum_tree::ContextLessSummary for TransformSummary {
    fn zero() -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for WrapRow {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for BlockRow {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
mod tests {
    ...
    use carrot_language::{Buffer, Capability, Point};
    use carrot_multi_buffer::{MultiBuffer, PathKey};
    use inazuma::{App, AppContext as _, Element, div, font, px};
    ...
    #[inazuma::test]
    fn test_basic_blocks(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_replace_with_heights(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_blocks_on_wrapped_lines(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_replace_lines(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_custom_blocks_inside_buffer_folds(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_basic_buffer_fold(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test(iterations = 60)]
    fn test_random_blocks(cx: &mut inazuma::TestAppContext, mut rng: StdRng) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_remove_intersecting_replace_blocks_edge_case(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_folded_buffer_with_near_blocks(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_folded_buffer_with_near_blocks_on_last_line(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
    ...
    #[inazuma::test]
    fn test_companion_spacer_blocks(cx: &mut inazuma::TestAppContext) {
        cx.update(init_test);
        ...
        let rhs_edits = Patch::new(vec![inazuma_text::Edit {
            old: WrapRow(0)..rhs_wrap_snapshot.max_point().row(),
            new: WrapRow(0)..rhs_wrap_snapshot.max_point().row(),
        }]);
        let lhs_edits = Patch::new(vec![inazuma_text::Edit {
