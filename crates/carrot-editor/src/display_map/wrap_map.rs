...
use carrot_multi_buffer::{MultiBufferSnapshot, RowInfo};
use inazuma::{App, AppContext as _, Context, Entity, Font, LineWrapper, Pixels, Task};
...
pub type WrapEdit = inazuma_text::Edit<WrapRow>;
pub type WrapPatch = inazuma_text::Patch<WrapRow>;
...
impl WrapSnapshot {
                ...
                for (i, _) in (edit.new_rows.start..edit.new_rows.end).enumerate() {
                    while let Some(chunk) = remaining.take().or_else(|| chunks.next()) {
                        if let Some(ix) = chunk.text.find('\n') {
                            ...
                            line_fragments.push(inazuma::LineFragment::text(prefix));
                            ...
                            {
                                line_fragments
                                    .push(inazuma::LineFragment::element(width, chunk.text.len()));
                            } else {
                                line_fragments.push(inazuma::LineFragment::text(chunk.text));
                            }
    ...
    #[carrot_tracing::instrument(skip_all, fields(point=?point))]
    ...
    fn check_invariants(&self) {
        #[cfg(test)]
        {
            ...
            let text = carrot_language::Rope::from(self.text().as_str());
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for TabPoint {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl inazuma_sum_tree::SeekTarget<'_, TransformSummary, TransformSummary> for TabPoint {
    #[carrot_tracing::instrument(skip_all)]
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for WrapPoint {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
mod tests {
    ...
    use inazuma::{LineFragment, px, test::observe};
    use inazuma_settings_framework::SettingsStore;
    use inazuma_text::Rope;
    use rand::prelude::*;
    use smol::stream::StreamExt;
    use std::{cmp, env, num::NonZeroU32};

    #[inazuma::test]
    async fn test_prev_row_boundary(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        ...
        fn test_wrap_snapshot(
            ...
            let buffer = cx.new(|cx| carrot_language::Buffer::local(text, cx));
    ...
    async fn test_random_wraps(cx: &mut inazuma::TestAppContext, mut rng: StdRng) {
