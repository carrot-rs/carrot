use carrot_language::Chunk;
use carrot_multi_buffer::{
    MultiBufferChunks, MultiBufferOffset, MultiBufferSnapshot, ToOffset as _,
};
...
mod tests {
    ...
    fn test_random_chunk_bitmaps(cx: &mut App, mut rng: StdRng) {
        ...
        let mut highlights = inazuma_sum_tree::TreeMap::default();
        let highlight_count = rng.random_range(1..10);

        for _i in 0..highlight_count {
            let style = HighlightStyle {
                color: Some(inazuma::Oklch {
                    l: rng.random::<f32>(),
                    c: rng.random::<f32>(),
                    h: rng.random::<f32>(),
                    a: 1.0,
