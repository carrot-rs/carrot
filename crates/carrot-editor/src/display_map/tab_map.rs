...
use carrot_multi_buffer::MultiBufferSnapshot;
...
const SPACES: &[u8; inazuma_rope::Chunk::MASK_BITS] = &[b' '; _];
...
impl TabSnapshot {
    ...
    #[carrot_tracing::instrument(skip_all, fields(rows))]
...
pub type TabEdit = inazuma_text::Edit<TabPoint>;
...
impl<'a> From<&'a str> for TextSummary {
    ...
    fn from(text: &'a str) -> Self {
        let sum = inazuma_text::TextSummary::from(text);
...
mod tests {
    ...
    #[inazuma::test]
    fn test_expand_tabs(cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_collapse_tabs(cx: &mut inazuma::App) {
        let input = "A\tBC\tDEF\tG\tHI\tJ\tK\tL\tM";
    ...
    #[inazuma::test]
    fn test_to_fold_point_panic_reproduction(cx: &mut inazuma::App) {
    ...
    fn test_collapse_tabs_random(cx: &mut inazuma::App, mut rng: StdRng) {
    ...
    #[inazuma::test]
    fn test_long_lines(cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_long_lines_with_character_spanning_max_expansion_column(cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_marking_tabs(cx: &mut inazuma::App) {
        let input = "\t \thello";
    ...
    #[inazuma::test]
    fn test_empty_chunk_after_leading_tab_trim(cx: &mut inazuma::App) {
    ...
    #[inazuma::test(iterations = 100)]
    fn test_random_tabs(cx: &mut inazuma::App, mut rng: StdRng) {
        let tab_size = NonZeroU32::new(rng.random_range(1..=4)).unwrap();
        ...
        let text = inazuma_text::Rope::from(tabs_snapshot.text().as_str());
        log::info!(
    ...
    #[inazuma::test(iterations = 100)]
    fn test_to_tab_point_random(cx: &mut inazuma::App, mut rng: StdRng) {
    ...
    #[inazuma::test]
    fn test_tab_stop_cursor_utf8(cx: &mut inazuma::App) {
    ...
    #[inazuma::test]
    fn test_tab_stop_with_end_range_utf8(cx: &mut inazuma::App) {
        let input = "A\tBC\t"; // DEF\tG\tHI\tJ\tK\tL\tM
    ...
    #[inazuma::test(iterations = 100)]
    fn test_tab_stop_cursor_random_utf8(cx: &mut inazuma::App, mut rng: StdRng) {
    ...
    #[inazuma::test]
    fn test_tab_stop_cursor_utf16(cx: &mut inazuma::App) {
    ...
    #[inazuma::test(iterations = 100)]
    fn test_tab_stop_cursor_random_utf16(cx: &mut inazuma::App, mut rng: StdRng) {
        // Generate random input string with up to 512 characters including tabs
        let len = rng.random_range(0..=2048);
        let input = inazuma_util::RandomCharIter::new(&mut rng)
            .take(len)
            .collect::<String>();
