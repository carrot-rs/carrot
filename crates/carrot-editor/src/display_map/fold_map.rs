...
use carrot_language::{Edit, HighlightId, Point};
...
use carrot_ui::IntoElement as _;
use inazuma::{AnyElement, App, ElementId, HighlightStyle, Pixels, SharedString, Stateful, Window};
use inazuma_sum_tree::{Bias, Cursor, Dimensions, FilterCursor, SumTree, Summary, TreeMap};
...
impl Default for FoldPlaceholder {
    fn default() -> Self {
        Self {
            render: Arc::new(|_, _, _| inazuma::Empty.into_any_element()),
...
impl FoldPlaceholder {
    ...
    pub fn fold_element(fold_id: FoldId, cx: &App) -> Stateful<inazuma::Div> {
    ...
    pub fn test() -> Self {
        Self {
            render: Arc::new(|_id, _range, _cx| inazuma::Empty.into_any_element()),
            constrain_width: true,
            merge_adjacent: true,
            type_tag: None,
            collapsed_text: None,
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for FoldPoint {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl FoldMapWriter<'_> {
    ...
    pub(crate) fn fold<T: ToOffset>(
        ...
        let buffer = &snapshot.buffer;
        folds
            .sort_unstable_by(|a, b| inazuma_sum_tree::SeekTarget::cmp(&a.range, &b.range, buffer));
...
impl inazuma_sum_tree::Item for Fold {
...
impl inazuma_sum_tree::Summary for FoldSummary {
...
impl<'a> inazuma_sum_tree::Dimension<'a, FoldSummary> for FoldRange {
    fn zero(_cx: &MultiBufferSnapshot) -> Self {
        Default::default()
    }
...
impl inazuma_sum_tree::SeekTarget<'_, FoldSummary, FoldRange> for FoldRange {
    fn cmp(&self, other: &Self, buffer: &MultiBufferSnapshot) -> Ordering {
        AnchorRangeExt::cmp(&self.0, &other.0, buffer)
    }
}

impl<'a> inazuma_sum_tree::Dimension<'a, FoldSummary> for MultiBufferOffset {
    fn zero(_cx: &MultiBufferSnapshot) -> Self {
        Default::default()
    }
...
pub struct Chunk<'a> {
    ...
    pub diagnostic_severity: Option<carrot_lsp::DiagnosticSeverity>,
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for FoldOffset {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for InlayPoint {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, TransformSummary> for InlayOffset {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
mod tests {
    ...
    #[inazuma::test]
    fn test_basic_folds(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_adjacent_folds(cx: &mut inazuma::App) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_overlapping_folds(cx: &mut inazuma::App) {
        let buffer = MultiBuffer::build_simple(&sample_text(5, 6, 'a'), cx);
        let buffer_snapshot = buffer.read(cx).snapshot(cx);
    ...
    #[inazuma::test]
    fn test_merging_folds_via_edit(cx: &mut inazuma::App) {
        init_test(cx);
        let buffer = MultiBuffer::build_simple(&sample_text(5, 6, 'a'), cx);
        let subscription = buffer.update(cx, |buffer, _| buffer.subscribe());
        let buffer_snapshot = buffer.read(cx).snapshot(cx);
    ...
    #[inazuma::test]
    fn test_folds_in_range(cx: &mut inazuma::App) {
        let buffer = MultiBuffer::build_simple(&sample_text(5, 6, 'a'), cx);
        let buffer_snapshot = buffer.read(cx).snapshot(cx);
    ...
    fn test_random_folds(cx: &mut inazuma::App, mut rng: StdRng) {
        init_test(cx);
    ...
    #[inazuma::test]
    fn test_buffer_rows(cx: &mut inazuma::App) {
    ...
    #[inazuma::test(iterations = 100)]
    fn test_random_chunk_bitmaps(cx: &mut inazuma::App, mut rng: StdRng) {
        init_test(cx);
