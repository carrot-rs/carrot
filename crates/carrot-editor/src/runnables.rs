...
use carrot_language::{Buffer, BufferRow, Runnable};
...
use carrot_task::{
    ResolvedTask, RunnableTag, TaskContext, TaskTemplate, TaskVariables, VariableName,
};
use carrot_ui::{Clickable as _, Color, IconButton, IconSize, Toggleable as _};
use inazuma::{
...
use inazuma_text::{BufferId, OffsetRangeExt as _, ToOffset as _, ToPoint as _};
...
pub struct RunnableTasks {
    ...
    pub offset: carrot_multi_buffer::Anchor,
...
impl Editor {
    ...
    pub fn render_run_indicator(
        ...
        IconButton::new(
            ...
            carrot_ui::IconName::PlayOutlined,
        )
    ...
    fn runnable_rows(
        ...
        prefer_lsp: bool,
        runnable_ranges: Vec<(Range<Anchor>, carrot_language::RunnableRange)>,
...
mod tests {
    ...
    use carrot_language::{ContextProvider, FakeLspAdapter};
    ...
    use carrot_task::{TaskTemplate, TaskTemplates};
    use futures::StreamExt as _;
    use inazuma::{AppContext as _, Entity, Task, TestAppContext};
    ...
    impl ContextProvider for TestRustContextProvider {
        fn associated_tasks(
            &self,
            _: Option<Entity<carrot_language::Buffer>>,
            _: &inazuma::App,
    ...
    fn rust_lang_with_task_context() -> Arc<carrot_language::Language> {
        Arc::new(
    ...
    fn rust_lang_with_lsp_task_context() -> Arc<carrot_language::Language> {
        Arc::new(
            Arc::try_unwrap(rust_lang())
                .unwrap()
    ...
    fn collect_runnable_labels(
        ...
        inazuma_text::BufferId,
        carrot_language::BufferRow,
        Vec<String>,
    )> {
    ...
    async fn test_multi_buffer_runnables_on_scroll(cx: &mut TestAppContext) {
        ...
        let multi_buffer = cx.new(|cx| {
            let mut multi_buffer = MultiBuffer::new(carrot_language::Capability::ReadWrite);
    ...
    async fn test_lsp_runnables_removed_after_edit(cx: &mut TestAppContext) {
        ...
        use carrot_project::lsp_store::lsp_ext_command::Runnables;
        fake_server.set_request_handler::<Runnables, _, _>(move |params, _| async move {
            ...
            if text.contains("main.rs") {
                let uri =
                    carrot_lsp::Uri::from_file_path(path!("/project/main.rs")).expect("valid uri");
                Ok(vec![Runnable {
                    label: "LSP test_one".into(),
                    location: Some(carrot_lsp::LocationLink {
                        origin_selection_range: None,
                        target_uri: uri,
                        target_range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(0, 0),
                            carrot_lsp::Position::new(3, 1),
                        ),
                        target_selection_range: carrot_lsp::Range::new(
                            carrot_lsp::Position::new(0, 0),
                            carrot_lsp::Position::new(3, 1),
                        ),
                    }),
