...
use carrot_language::{Buffer, OutlineItem};
...
use inazuma::{App, Context, HighlightStyle, Task};
...
impl Editor {
    ...
    pub fn buffer_outline_items(
        ...
        buffer_id: BufferId,
        cx: &mut Context<Self>,
    ) -> Task<Vec<OutlineItem<inazuma_text::Anchor>>> {
...
fn highlights_from_buffer(
    ...
    item: &OutlineItem<inazuma_text::Anchor>,
...
mod tests {
    ...
    use inazuma_settings_framework::DocumentSymbols;
    ...
    fn lsp_range(
        ...
        carrot_lsp::Range {
            start: carrot_lsp::Position::new(start_line, start_char),
            end: carrot_lsp::Position::new(end_line, end_char),
        }
    }

    fn nested_symbol(
        name: &str,
        kind: carrot_lsp::SymbolKind,
        range: carrot_lsp::Range,
        selection_range: carrot_lsp::Range,
        children: Vec<carrot_lsp::DocumentSymbol>,
            ...
            .set_request_handler::<carrot_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    Ok(Some(carrot_lsp::DocumentSymbolResponse::Nested(vec![
                        nested_symbol(
                            "main",
                            carrot_lsp::SymbolKind::FUNCTION,
                            lsp_range(0, 0, 2, 1),
                            lsp_range(0, 3, 0, 7),
                            Vec::new(),
                        ),
    ...
    async fn test_lsp_document_symbols_flat_response(cx: &mut TestAppContext) {
        ...
        let mut symbol_request = cx
            .set_request_handler::<carrot_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    #[allow(deprecated)]
                    Ok(Some(carrot_lsp::DocumentSymbolResponse::Flat(vec![
                        carrot_lsp::SymbolInformation {
                            ...
                            location: carrot_lsp::Location {
                                uri: carrot_lsp::Uri::from_file_path(path!("/a/main.rs")).unwrap(),
    ...
    async fn test_lsp_document_symbols_empty_response(cx: &mut TestAppContext) {
        ...
        let mut symbol_request = cx
            .set_request_handler::<carrot_lsp::request::DocumentSymbolRequest, _, _>(
                move |_, _, _| async move {
                    Ok(Some(carrot_lsp::DocumentSymbolResponse::Nested(Vec::new())))
                },
            );
