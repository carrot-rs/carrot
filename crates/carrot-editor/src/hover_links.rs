...
use carrot_language::{Bias, ToOffset};
use carrot_lsp::LanguageServerId;
use carrot_project::{InlayId, LocationLink, Project, ResolvedPath};
use carrot_theme::ActiveTheme as _;
use inazuma::{App, AsyncWindowContext, Context, Entity, Modifiers, Pixels, Task, Window, px};
use inazuma_settings_framework::Settings;
use inazuma_text::OffsetRangeExt;
use inazuma_util::{ResultExt, TryFutureExt as _, maybe};
...
pub enum HoverLink {
    ...
    File(ResolvedPath),
    Text(LocationLink),
    InlayHint(carrot_lsp::Location, LanguageServerId),
}
...
pub enum TriggerPoint {
    Text(Anchor),
    InlayHint(InlayHighlight, carrot_lsp::Location, LanguageServerId),
}
...
pub fn exclude_link_to_position(
    ...
    current_position: &inazuma_text::Anchor,
            ...
            this.update(cx, |editor, cx| {
                ...
                if let Some((symbol_range, definitions)) = result {
                    ...
                    if underline_hovered_link {
                        let style = inazuma::HighlightStyle {
...
) -> Option<(Range<inazuma_text::Anchor>, String)> {
...
) -> Option<(Range<inazuma_text::Anchor>, ResolvedPath)> {
    let project = project?;
    let snapshot = buffer.read_with(cx, |buffer, _| buffer.snapshot());
...
mod tests {
    ...
    use carrot_lsp::request::{GotoDefinition, GotoTypeDefinition};
    use carrot_multi_buffer::MultiBufferOffset;
    use carrot_workspace::item::Item;
    use futures::StreamExt;
    use inazuma::{Modifiers, MousePressureEvent, PressureStage};
    use inazuma_settings_framework::InlayHintSettingsContent;
    use inazuma_util::{assert_set_eq, path};
    use indoc::indoc;
    ...
    async fn test_hover_type_links(cx: &mut inazuma::TestAppContext) {
        ...
        let mut cx = EditorLspTestContext::new_rust(
            carrot_lsp::ServerCapabilities {
                ...
                type_definition_provider: Some(
                    carrot_lsp::TypeDefinitionProviderCapability::Simple(true),
                ),
                ..Default::default()
            },
            cx,
        ...
        let mut requests =
            cx.set_request_handler::<GotoTypeDefinition, _, _>(move |url, _, _| async move {
                Ok(Some(carrot_lsp::GotoTypeDefinitionResponse::Link(vec![
                    carrot_lsp::LocationLink {
    ...
    async fn test_hover_links(cx: &mut inazuma::TestAppContext) {
        ...
        let mut cx = EditorLspTestContext::new_rust(
            carrot_lsp::ServerCapabilities {
                hover_provider: Some(carrot_lsp::HoverProviderCapability::Simple(true)),
                definition_provider: Some(carrot_lsp::OneOf::Left(true)),
                ..Default::default()
            },
            cx,
        )
        ...
        let mut requests =
            cx.set_request_handler::<GotoDefinition, _, _>(move |url, _, _| async move {
                Ok(Some(carrot_lsp::GotoDefinitionResponse::Link(vec![
                    carrot_lsp::LocationLink {
                        origin_selection_range: Some(symbol_range),
                        target_uri: url.clone(),
                        target_range,
    ...
    #[inazuma::test]
    async fn test_inlay_hover_links(cx: &mut inazuma::TestAppContext) {
    ...
    async fn test_urls(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_state(indoc! {"
            Let's test a [complex](https://carrot.dev/channel/had-(oops)) caseˇ.
        "});

        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://carrot.dev/channel/had-(ˇoops)) case.
            "});
        ...
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"
            Let's test a [complex](«https://carrot.dev/channel/had-(oops)ˇ») case.
        "},
        );
        ...
        assert_eq!(
            cx.opened_url(),
            Some("https://carrot.dev/channel/had-(oops)".into())
        );
    }
    ...
    async fn test_hover_preconditions(cx: &mut inazuma::TestAppContext) {
        ...
        // No link
        cx.set_state(indoc! {"
            Let's test a [complex](https://carrot.dev/channel/) caseˇ.
        "});
        assert_no_highlight!(cx);

        // No modifier
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://carrot.dev/channel/ˇ) case.
            "});
        ...
        // Modifier active
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://carrot.dev/channeˇl/) case.
            "});
        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
        ...
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://carrot.dev/ˇchannel/) case.
            "});
        cx.simulate_mouse_move(screen_coord, None, Modifiers::none());
        ...
        // Cursor active again
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://ˇcarrot.dev/channel/) case.
            "});
        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
    ...
    async fn test_urls_at_beginning_of_buffer(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_state(indoc! {"https://carrot.dev/releases is a cool ˇwebpage."});

        let screen_coord =
            cx.pixel_position(indoc! {"https://carrot.dev/relˇeases is a cool webpage."});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"«https://carrot.dev/releasesˇ» is a cool webpage."},
        );

        cx.simulate_click(screen_coord, Modifiers::secondary_key());
        assert_eq!(cx.opened_url(), Some("https://carrot.dev/releases".into()));
    }
    ...
    async fn test_urls_at_end_of_buffer(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_state(indoc! {"A cool ˇwebpage is https://carrot.dev/releases"});

        let screen_coord =
            cx.pixel_position(indoc! {"A cool webpage is https://carrot.dev/releˇases"});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"A cool webpage is «https://carrot.dev/releasesˇ»"},
        );
