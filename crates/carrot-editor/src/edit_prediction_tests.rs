use carrot_edit_prediction_types::{
    EditPredictionDelegate, EditPredictionIconSet, PredictedCursorPosition,
};
use carrot_language::EditPredictionsMode;
use carrot_language::{Buffer, CodeLabel};
use carrot_multi_buffer::{Anchor, ExcerptId, MultiBufferSnapshot, ToPoint};
use carrot_project::{Completion, CompletionResponse, CompletionSource};
use carrot_ui::prelude::*;
use inazuma::{Entity, KeyBinding, KeybindingKeystroke, Keystroke, Modifiers, NoAction, Task};
use inazuma_text::{Point, ToOffset};
use indoc::indoc;
use std::{
    ops::Range,
    rc::Rc,
...
use carrot_rpc::proto::PeerId;
use carrot_workspace::CollaboratorId;
...
#[inazuma::test]
async fn test_edit_prediction_cursor_position_inside_insertion(cx: &mut inazuma::TestAppContext) {
    init_test(cx, |_| {
        eprintln!("");
    });
...
#[inazuma::test]
async fn test_edit_prediction_jump_disabled_for_non_carrot_providers(
    cx: &mut inazuma::TestAppContext,
) {
    init_test(cx, |_| {});

    let mut cx = EditorTestContext::new(cx).await;
    let provider = cx.new(|_| FakeNonCarrotEditPredictionDelegate::default());
    ...
    // For non-Carrot providers, there should be no move completion (jump functionality disabled)
    cx.editor(|editor, _, _| {
        if let Some(completion_state) = &editor.active_edit_prediction {
            ...
            match &completion_state.completion {
                EditPrediction::Edit { .. } => {
                    // This is expected for non-Carrot providers
                }
                EditPrediction::MoveWithin { .. } | EditPrediction::MoveOutside { .. } => {
                    panic!(
                        "Non-Carrot providers should not show Move predictions (jump functionality)"
                    );
                }
            }
        }
...
#[inazuma::test]
async fn test_edit_prediction_preview_cleanup_on_toggle_off(cx: &mut inazuma::TestAppContext) {
    init_test(cx, |_| {});
...
fn load_default_keymap(cx: &mut inazuma::TestAppContext) {
    cx.update(|cx| {
        cx.bind_keys(
            inazuma_settings_framework::KeymapFile::load_asset_allow_partial_failure(
                inazuma_settings_framework::DEFAULT_KEYMAP_PATH,
                cx,
            )
...
#[inazuma::test]
async fn test_inline_edit_prediction_keybind_selection_cases(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_tab_accepts_edit_prediction_over_completion(cx: &mut inazuma::TestAppContext) {
    init_test(cx, |_| {});
    load_default_keymap(cx);
...
#[inazuma::test]
async fn test_cursor_popover_edit_prediction_keybind_cases(cx: &mut inazuma::TestAppContext) {
...
#[inazuma::test]
async fn test_cancel_clears_stale_edit_prediction_in_menu(cx: &mut inazuma::TestAppContext) {
    init_test(cx, |_| {});
    load_default_keymap(cx);
...
async fn propose_edits_with_preview<T: ToOffset + Clone>(
    ...
    cx.update(|_, cx| {
        provider.update(cx, |provider, _| {
            provider.set_edit_prediction(Some(
                carrot_edit_prediction_types::EditPrediction::Local {
                    id: None,
...
fn propose_edits_non_zed<T: ToOffset>(
    provider: &Entity<FakeNonCarrotEditPredictionDelegate>,
    edits: Vec<(Range<T>, &str)>,
    cx: &mut EditorTestContext,
) {
    let snapshot = cx.buffer_snapshot();
...
impl CompletionProvider for FakeCompletionMenuProvider {
    fn completions(
        ...
        _buffer_position: inazuma_text::Anchor,
        _trigger: CompletionContext,
        _window: &mut Window,
        _cx: &mut Context<crate::Editor>,
    ) -> Task<anyhow::Result<Vec<CompletionResponse>>> {
        let completion = Completion {
            replace_range: inazuma_text::Anchor::MIN..inazuma_text::Anchor::MAX,
...
pub struct FakeEditPredictionDelegate {
    pub completion: Option<carrot_edit_prediction_types::EditPrediction>,
...
impl FakeEditPredictionDelegate {
    pub fn set_edit_prediction(
        &mut self,
        completion: Option<carrot_edit_prediction_types::EditPrediction>,
    ) {
...
impl EditPredictionDelegate for FakeEditPredictionDelegate {
    ...
    fn icons(&self, _cx: &inazuma::App) -> EditPredictionIconSet {
        EditPredictionIconSet::new(IconName::CarrotPredict)
    }

    fn is_enabled(
        &self,
        _buffer: &inazuma::Entity<carrot_language::Buffer>,
        _cursor_position: carrot_language::Anchor,
        _cx: &inazuma::App,
    ) -> bool {
        true
    }

    fn is_refreshing(&self, _cx: &inazuma::App) -> bool {
        false
    }
    ...
    fn refresh(
        ...
        _cursor_position: carrot_language::Anchor,
        _debounce: bool,
        _cx: &mut inazuma::Context<Self>,
    ) {
        self.refresh_count.fetch_add(1, atomic::Ordering::SeqCst);
    }

    fn accept(&mut self, _cx: &mut inazuma::Context<Self>) {}

    fn discard(
        &mut self,
        _reason: carrot_edit_prediction_types::EditPredictionDiscardReason,
        _cx: &mut inazuma::Context<Self>,
    ) {
    }
    ...
    fn suggest<'a>(
        ...
        _cursor_position: carrot_language::Anchor,
        _cx: &mut inazuma::Context<Self>,
    ) -> Option<carrot_edit_prediction_types::EditPrediction> {
        self.completion.clone()
    }
}

#[derive(Default, Clone)]
pub struct FakeNonCarrotEditPredictionDelegate {
    pub completion: Option<carrot_edit_prediction_types::EditPrediction>,
}

impl FakeNonCarrotEditPredictionDelegate {
    pub fn set_edit_prediction(
        &mut self,
        completion: Option<carrot_edit_prediction_types::EditPrediction>,
    ) {
...
impl EditPredictionDelegate for FakeNonCarrotEditPredictionDelegate {
    fn name() -> &'static str {
        "fake-non-carrot-provider"
    }
