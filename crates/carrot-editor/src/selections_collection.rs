...
use carrot_language::{Bias, Point, PointUtf16, Selection, SelectionGoal};
use carrot_multi_buffer::{MultiBufferDimension, MultiBufferOffset};
use inazuma::Pixels;
...
impl SelectionsCollection {
    ...
    fn build_columnar_selection_from_utf16_columns(
        ...
        let is_empty = positions.start == positions.end;
        let line_len_utf16 =
            snapshot.line_len_utf16(carrot_multi_buffer::MultiBufferRow(buffer_row));
...
impl<'snap, 'a> MutableSelectionsCollection<'snap, 'a> {
    ...
    pub fn remove_selections_from_buffer(&mut self, buffer_id: carrot_language::BufferId) {
        let mut changed = false;
...
/// but don't share any positions remain separate, see: https://github.com/carrot-rs/carrot/issues/24748
