...
use carrot_language::{Bias, BufferSnapshot, Edit};
use carrot_markdown::Markdown;
use carrot_multi_buffer::{MultiBuffer, RowInfo};
use carrot_project::{
...
use inazuma::{
    AnyElement, App, AppContext as _, Context, Entity, Oklch, ScrollHandle, Subscription, Task,
...
impl inazuma_sum_tree::Item for GitBlameEntry {
...
impl inazuma_sum_tree::ContextLessSummary for GitBlameEntrySummary {
    fn zero() -> Self {
        Default::default()
    }
...
impl<'a> inazuma_sum_tree::Dimension<'a, GitBlameEntrySummary> for u32 {
    fn zero(_cx: ()) -> Self {
        Default::default()
    }
...
struct GitBlameBuffer {
    ...
    buffer_edits: inazuma_text::Subscription<usize>,
...
impl inazuma::Global for GlobalBlameRenderer {}
    ...
    pub fn new(
        ...
        let multi_buffer_subscription = cx.subscribe(
            ...
            |git_blame, multi_buffer, event, cx| match event {
                carrot_multi_buffer::Event::DirtyChanged => {
                    if !multi_buffer.read(cx).is_dirty(cx) {
                        git_blame.generate(cx);
                    }
                }
                carrot_multi_buffer::Event::ExcerptsAdded { .. }
                | carrot_multi_buffer::Event::ExcerptsEdited { .. } => {
        ...
        let project_subscription = cx.subscribe(&project, {
            ...
            move |git_blame, _, event, cx| {
                if let carrot_project::Event::WorktreeUpdatedEntries(_, updated) = event {
    ...
    fn generate(&mut self, cx: &mut Context<Self>) {
        ...
        self.task = cx.spawn(async move |this, cx| {
            ...
            for buffers in buffers_to_blame.chunks(4) {
                let span = carrot_tracing::debug_span!("for each chunk of buffers");
...
mod tests {
    ...
    #[inazuma::test]
    async fn test_blame_error_notifications(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_blame_for_rows(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    #[inazuma::test]
    async fn test_blame_for_rows_with_edits(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
    ...
    async fn test_blame_random(mut rng: StdRng, cx: &mut inazuma::TestAppContext) {
