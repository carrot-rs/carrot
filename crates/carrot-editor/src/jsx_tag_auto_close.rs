...
use carrot_multi_buffer::{BufferOffset, MultiBuffer, ToOffset};
use inazuma::{Context, Entity, Window};
use inazuma_collections::HashMap;
use inazuma_util::ResultExt as _;
use std::ops::Range;

use carrot_language::{
    BufferSnapshot, JsxTagAutoCloseConfig, Node, language_settings::LanguageSettings,
};
use inazuma_text::{Anchor, OffsetRangeExt as _};
...
pub(crate) type InitialBufferVersionsMap =
    HashMap<carrot_language::BufferId, inazuma_clock::Global>;
...
pub(crate) fn handle_from(
    ...
    struct JsxAutoCloseEditContext {
        ...
        config: carrot_language::JsxTagAutoCloseConfig,
        edits: Vec<Range<Anchor>>,
    }

    let mut edit_contexts = HashMap::<
        (carrot_language::BufferId, carrot_language::LanguageId),
        JsxAutoCloseEditContext,
    >::default();
    ...
    for ((buffer_id, _), auto_close_context) in edit_contexts {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if buffer_parse_status == carrot_language::ParseStatus::Parsing {
                let Some(carrot_language::ParseStatus::Idle) =
            ...
            for edit in &edits {
                ...
                if let Some(selection) = buffer_selection_map.get_mut(&(
                    ...
                    if selection.0.head().bias() != inazuma_text::Bias::Right
                        || selection.0.tail().bias() != inazuma_text::Bias::Right
                    {
                        continue;
                    }
...
mod jsx_tag_autoclose_tests {
    ...
    use super::*;
    use carrot_languages::language;
    use carrot_multi_buffer::{MultiBufferOffset, PathKey};
    use inazuma::{AppContext as _, TestAppContext};
    use inazuma_text::Selection;
    ...
    async fn test_multibuffer(cx: &mut TestAppContext) {
        ...
        let buffer_a = cx.new(|cx| {
            let mut buf = carrot_language::Buffer::local("<div", cx);
            buf.set_language(
        ...
        let buffer_b = cx.new(|cx| {
            let mut buf = carrot_language::Buffer::local("<pre", cx);
            buf.set_language(
                Some(language("tsx", tree_sitter_typescript::LANGUAGE_TSX.into())),
                cx,
            );
        ...
        let buffer_c = cx.new(|cx| carrot_language::Buffer::local("<span", cx));
        let buffer = cx.new(|cx| {
            let mut buf = MultiBuffer::new(carrot_language::Capability::ReadWrite);
            let range_a = carrot_language::Point::zero()..buffer_a.read(cx).max_point();
            let range_b = carrot_language::Point::zero()..buffer_b.read(cx).max_point();
            let range_c = carrot_language::Point::zero()..buffer_c.read(cx).max_point();
