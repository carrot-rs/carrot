...
pub struct EditorDb(carrot_db::sqlez::thread_safe_connection::ThreadSafeConnection);
...
carrot_db::static_connection!(EditorDb, [WorkspaceDb]);
...
mod tests {
    ...
    #[inazuma::test]
    async fn test_save_and_get_serialized_editor(cx: &mut inazuma::TestAppContext) {
        let db = cx.update(|cx| carrot_workspace::WorkspaceDb::global(cx));
    ...
    // 1. Open a file, fold some sections, close Carrot
    // 2. Add text at the START of the file externally (shifts all offsets)
    // 3. Reopen Carrot - folds should be restored at their NEW correct positions
    // The search uses contains_str_at() to find fingerprints in the buffer.

    #[inazuma::test]
    async fn test_save_and_get_file_folds(cx: &mut inazuma::TestAppContext) {
        let db = cx.update(|cx| carrot_workspace::WorkspaceDb::global(cx));
        let workspace_id = db.next_id().await.unwrap();
        let editor_db = cx.update(|cx| EditorDb::global(cx));
