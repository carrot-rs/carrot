...
use carrot_ui::ActiveTheme as _;
...
pub(super) struct SemanticTokenState {
    ...
    enabled: bool,
    update_task: Task<()>,
    fetched_for_buffers: HashMap<BufferId, inazuma_clock::Global>,
}
...
fn buffer_into_editor_highlights<'a, 'b>(
    ...
    multi_buffer_snapshot: &'a carrot_multi_buffer::MultiBufferSnapshot,
...
mod tests {
    ...
    use carrot_languages::FakeLspAdapter;
    ...
    use carrot_workspace::{Workspace, WorkspaceHandle as _};
    use futures::StreamExt as _;
    use inazuma::{AppContext as _, Entity, Focusable as _, HighlightStyle, TestAppContext};
    use inazuma_rope::Point;
    use inazuma_settings_framework::{
        LanguageSettingsContent, SemanticTokenRules, SemanticTokens, SettingsStore,
    };
    use serde_json::json;
    ...
    async fn lsp_semantic_tokens_full_capability(cx: &mut TestAppContext) {
        ...
        let mut cx = EditorLspTestContext::new_rust(
            carrot_lsp::ServerCapabilities {
                semantic_tokens_provider: Some(
                    carrot_lsp::SemanticTokensServerCapabilities::SemanticTokensOptions(
                        carrot_lsp::SemanticTokensOptions {
                            legend: carrot_lsp::SemanticTokensLegend {
                                token_types: vec!["function".into()],
                                token_modifiers: Vec::new(),
                            },
                            full: Some(carrot_lsp::SemanticTokensFullOptions::Delta {
                                delta: None,
                            }),
                            ..carrot_lsp::SemanticTokensOptions::default()
                        },
                    ),
                ),
        ...
        let mut full_request = cx
            .set_request_handler::<carrot_lsp::request::SemanticTokensFullRequest, _, _>(
                move |_, _, _| {
                    full_counter_clone.fetch_add(1, atomic::Ordering::Release);
                    async move {
                        Ok(Some(carrot_lsp::SemanticTokensResult::Tokens(
                            carrot_lsp::SemanticTokens {
                                data: vec![
                                    0, // delta_line
                                    3, // delta_start
                                    4, // length
    ...
    async fn lsp_semantic_tokens_delta(cx: &mut TestAppContext) {
        ...
        let mut delta_request = cx
            .set_request_handler::<carrot_lsp::request::SemanticTokensFullDeltaRequest, _, _>(
                move |_, params, _| {
                    delta_counter_clone.fetch_add(1, atomic::Ordering::Release);
                    assert_eq!(params.previous_result_id, "a");
                    async move {
                        Ok(Some(
                            carrot_lsp::SemanticTokensFullDeltaResult::TokensDelta(
                                carrot_lsp::SemanticTokensDelta {
                                    edits: Vec::new(),
    ...
    async fn lsp_semantic_tokens_multiserver_full(cx: &mut TestAppContext) {
        ...
        let toml_legend_1 = carrot_lsp::SemanticTokensLegend {
            token_types: vec!["property".into()],
            token_modifiers: Vec::new(),
        };
        let toml_legend_2 = carrot_lsp::SemanticTokensLegend {
            token_types: vec!["number".into()],
            token_modifiers: Vec::new(),
        };

        let app_state = cx.update(carrot_workspace::AppState::test);
                    ...
                    semantic_tokens_provider: Some(
                        carrot_lsp::SemanticTokensServerCapabilities::SemanticTokensOptions(
                            carrot_lsp::SemanticTokensOptions {
                                ...
                                full: Some(carrot_lsp::SemanticTokensFullOptions::Delta { delta: None }),
                                ..carrot_lsp::SemanticTokensOptions::default()
                            },
                        ),
                    ),
    ...
    async fn lsp_semantic_tokens_multibuffer_part(cx: &mut TestAppContext) {
        ...
        let toml_legend = carrot_lsp::SemanticTokensLegend {
            token_types: vec!["property".into()],
            token_modifiers: Vec::new(),
        };
        let rust_legend = carrot_lsp::SemanticTokensLegend {
    ...
    async fn test_semantic_tokens_rules_changes_restyle_tokens(cx: &mut TestAppContext) {
        use inazuma::{Oklch, Rgba, UpdateGlobal as _};
        use inazuma_settings_framework::{GlobalLspSettingsContent, SemanticTokenRule};
    ...
    async fn test_theme_override_changes_restyle_semantic_tokens(cx: &mut TestAppContext) {
        use carrot_theme_settings::{HighlightStyleContent, ThemeStyleContent};
        use inazuma::{Oklch, Rgba, UpdateGlobal as _};
    ...
    async fn test_per_theme_overrides_restyle_semantic_tokens(cx: &mut TestAppContext) {
        ...
        cx.update(|_, cx| {
            SettingsStore::update_global(cx, |store, cx| {
                store.update_user_settings(cx, |settings| {
                    settings.theme.theme_overrides = inazuma_collections::HashMap::from_iter([(
                        theme_name.clone(),
                        ThemeStyleContent {
                            syntax: IndexMap::from_iter([(
                                "function".to_string(),
