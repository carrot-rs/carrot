...
use carrot_language::{DiagnosticEntry, Language, LanguageRegistry};
use carrot_lsp::DiagnosticSeverity;
use carrot_markdown::{Markdown, MarkdownElement, MarkdownStyle};
use carrot_multi_buffer::{MultiBufferOffset, ToOffset, ToPoint};
use carrot_project::{HoverBlock, HoverBlockKind, InlayHintLabelPart};
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{
    CopyButton, ScrollAxes, Scrollbars, WithScrollbar, prelude::*, theme_is_transparent,
};
use carrot_workspace::{OpenOptions, OpenVisible, Workspace};
use inazuma::{
    AnyElement, App, AsyncWindowContext, Bounds, Context, Entity, Focusable as _, FontWeight,
    InteractiveElement, IntoElement, MouseButton, Oklch, ParentElement, Pixels, ScrollHandle, Size,
...
async fn parse_blocks(
    ...
    let combined_text = blocks
        ...
        .map(|block| match &block.kind {
            carrot_project::HoverBlockKind::PlainText
            | carrot_project::HoverBlockKind::Markdown => Cow::Borrowed(block.text.trim()),
            carrot_project::HoverBlockKind::Code { language } => {
            ...
            cx.spawn_in(window, async move |_, cx| {
                ...
                editor.update_in(cx, |editor, window, cx| {
                    editor.change_selections(Default::default(), window, cx, |selections| {
                        selections.select_ranges([inazuma_text::Point::new(accum - 1, 0)
                            ..inazuma_text::Point::new(accum - 1, 0)]);
                    });
                })
            })
...
impl HoverState {
    ...
    pub fn is_mouse_getting_closer(&mut self, mouse_position: inazuma::Point<Pixels>) -> bool {
        if !self.visible() {
            return false;
        }
    ...
    pub(crate) fn render(
        ...
        if visible_rows.end <= point.row() {
            point = crate::movement::up_by_rows(
                ...
                inazuma_text::SelectionGoal::None,
                true,
                text_layout_details,
            )
            .0;
                    ...
                    div()
                        ...
                        .child(
                            MarkdownElement::new(markdown, hover_markdown_style(window, cx))
                                .code_block_renderer(carrot_markdown::CodeBlockRenderer::Default {
                                    copy_button: false,
    ...
    pub fn render(
        ...
        div()
            ...
            .when(theme_is_transparent(cx), |this| {
                this.bg(inazuma::transparent_black())
            })
...
mod tests {
    ...
    use carrot_markdown::parser::MarkdownEvent;
    use carrot_project::InlayId;
    use inazuma::App;
    use inazuma_collections::BTreeSet;
    use inazuma_settings_framework::InlayHintSettingsContent;
    ...
    fn get_hover_popover_delay(cx: &inazuma::TestAppContext) -> u64 {
        cx.read(|cx: &App| -> u64 { EditorSettings::get_global(cx).hover_popover_delay.0 })
    }

    impl InfoPopover {
        fn get_rendered_text(&self, cx: &inazuma::App) -> String {
    ...
    async fn test_mouse_hover_info_popover_with_autocomplete_popover(
        ...
        let symbol_range = cx.lsp_range(indoc! {"
                ...
                fn test() { «println!»(); }
            "});
        let mut requests = cx.set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(
    ...
    async fn test_empty_hovers_filtered(cx: &mut inazuma::TestAppContext) {
        ...
        let symbol_range = cx.lsp_range(indoc! {"
            «fn» test() { println!(); }
        "});
        cx.set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(
            move |_, _, _| async move {
                Ok(Some(carrot_lsp::Hover {
    ...
    async fn test_line_ends_trimmed(cx: &mut inazuma::TestAppContext) {
        ...
        cx.set_request_handler::<carrot_lsp::request::HoverRequest, _, _>(move |_, _, _| {
    ...
    #[inazuma::test]
    // https://github.com/carrot-rs/carrot/issues/15498
    async fn test_info_hover_with_hrs(cx: &mut inazuma::TestAppContext) {
        ...
        cx.lsp
            .set_request_handler::<carrot_lsp::request::HoverRequest, _, _>({
                |_, _| async move {
                    Ok(Some(carrot_lsp::Hover {
                        contents: carrot_lsp::HoverContents::Markup(carrot_lsp::MarkupContent {
                            kind: carrot_lsp::MarkupKind::Markdown,
    ...
    #[inazuma::test]
    async fn test_hover_inlay_label_parts(cx: &mut inazuma::TestAppContext) {
        ...
        let mut cx = EditorLspTestContext::new_rust(
            carrot_lsp::ServerCapabilities {
                inlay_hint_provider: Some(carrot_lsp::OneOf::Right(
                    carrot_lsp::InlayHintServerCapabilities::Options(
                        carrot_lsp::InlayHintOptions {
                            resolve_provider: Some(true),
                            ..Default::default()
                        },
                    ),
            ...
            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(move |params, _| {
                ...
                async move {
                    ...
                    Ok(Some(vec![carrot_lsp::InlayHint {
                        ...
                        label: carrot_lsp::InlayHintLabel::LabelParts(vec![
                            carrot_lsp::InlayHintLabelPart {
        ...
        cx.lsp
            .set_request_handler::<carrot_lsp::request::InlayHintResolveRequest, _, _>(
                move |mut hint_to_resolve, _| {
                    ...
                    async move {
                        ...
                        hint_to_resolve.label = carrot_lsp::InlayHintLabel::LabelParts(vec![
                            carrot_lsp::InlayHintLabelPart {
                                value: ": ".to_string(),
                                ..Default::default()
                            },
                            carrot_lsp::InlayHintLabelPart {
                                ...
                                location: Some(carrot_lsp::Location {
                                    uri: task_uri.clone(),
                                    range: new_type_target_range,
                                }),
                                tooltip: Some(carrot_lsp::InlayHintLabelPartTooltip::String(
    ...
    fn test_find_hovered_hint_part_with_multibyte_characters() {
        ...
        use carrot_project::InlayHintLabelPart;
