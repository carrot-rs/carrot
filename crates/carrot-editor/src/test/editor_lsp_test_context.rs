...
use anyhow::Result;
use carrot_language::{markdown_lang, rust_lang};
...
use carrot_lsp::{notification, request};
use carrot_project::Project;
use carrot_workspace::{AppState, Workspace, WorkspaceHandle};
use futures::Future;
use inazuma::{Context, Entity, Focusable as _, VisualTestContext, Window};
...
pub struct EditorLspTestContext {
    ...
    pub lsp: carrot_lsp::FakeLanguageServer,
    pub workspace: Entity<Workspace>,
    pub buffer_lsp_url: carrot_lsp::Uri,
}
...
impl EditorLspTestContext {
    pub async fn new(
        ...
        Self {
            ...
            lsp,
            workspace,
            buffer_lsp_url: carrot_lsp::Uri::from_file_path(root.join("dir").join(file_name))
                .unwrap(),
        }
    }
    ...
    pub async fn new_typescript(
        ...
        let language = Language::new(
            LanguageConfig {
                ...
                brackets: carrot_language::BracketPairConfig {
                    pairs: vec![carrot_language::BracketPair {
                        start: "{".to_string(),
                        end: "}".to_string(),
                        close: true,
                        surround: true,
    ...
    pub async fn new_html(cx: &mut inazuma::TestAppContext) -> Self {
        let language = Language::new(
            LanguageConfig {
                name: "HTML".into(),
                matcher: LanguageMatcher {
    ...
    pub async fn new_markdown_with_rust(cx: &mut inazuma::TestAppContext) -> Self {
    ...
    #[track_caller]
    pub fn lsp_range(&mut self, marked_text: &str) -> carrot_lsp::Range {
        let ranges = self.ranges(marked_text);
        self.to_lsp_range(MultiBufferOffset(ranges[0].start)..MultiBufferOffset(ranges[0].end))
    }

    pub fn to_lsp_range(&mut self, range: Range<MultiBufferOffset>) -> carrot_lsp::Range {
        use carrot_language::ToPointUtf16;
    ...
    pub fn to_lsp(&mut self, offset: MultiBufferOffset) -> carrot_lsp::Position {
        use carrot_language::ToPointUtf16;
    ...
    where
        ...
        F: 'static + Send + FnMut(carrot_lsp::Uri, T::Params, inazuma::AsyncApp) -> Fut,
