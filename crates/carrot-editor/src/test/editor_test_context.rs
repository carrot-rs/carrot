...
use carrot_git::repository::RepoPath;
use carrot_language::{Buffer, BufferSnapshot, LanguageRegistry};
use carrot_multi_buffer::{Anchor, ExcerptRange, MultiBufferOffset, MultiBufferRow, PathKey};
...
pub struct EditorTestContext {
    pub cx: inazuma::VisualTestContext,
...
impl EditorTestContext {
    pub async fn new(cx: &mut inazuma::TestAppContext) -> EditorTestContext {
    ...
    pub fn text_anchor_range(&mut self, marked_text: &str) -> Range<carrot_language::Anchor> {
        let ranges = self.ranges(marked_text);
    ...
    ///
    /// See the `inazuma_util::test::marked_text_ranges` function for more information.
    #[track_caller]
    ...
    pub fn assert_excerpts_with_selections(&mut self, marked_text: &str) {
        ...
        for (ix, (excerpt_id, snapshot, range)) in excerpts.into_iter().enumerate() {
            ...
            let selections = selections
                ...
                .map(|s| {
                    let head = inazuma_text::ToOffset::to_offset(&s.head().text_anchor, &snapshot)
                        - inazuma_text::ToOffset::to_offset(&range.context.start, &snapshot);
                    let tail = inazuma_text::ToOffset::to_offset(&s.head().text_anchor, &snapshot)
                        - inazuma_text::ToOffset::to_offset(&range.context.start, &snapshot);
                    tail..head
                })
                .collect::<Vec<_>>();
...
impl Deref for EditorTestContext {
    type Target = inazuma::VisualTestContext;
