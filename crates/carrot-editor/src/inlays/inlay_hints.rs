...
use carrot_lsp::LanguageServerId;
use carrot_multi_buffer::{Anchor, ExcerptId, MultiBufferSnapshot};
use carrot_project::{
...
use carrot_ui::{Context, Window};
use futures::future::join_all;
use inazuma::{App, Entity, Pixels, Task};
use inazuma_clock::Global;
use inazuma_collections::{HashMap, HashSet};
use inazuma_text::{Bias, BufferId};
...
impl Editor {
    ...
    pub fn update_inlay_link_and_hover_points(
        ...
        mouse_position: Option<inazuma::Point<Pixels>>,
        secondary_held: bool,
        shift_held: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
...
pub mod tests {
    ...
    use carrot_language::language_settings::InlayHintKind;
    use carrot_language::{Capability, FakeLspAdapter};
    use carrot_language::{Language, LanguageConfig, LanguageMatcher};
    use carrot_languages::rust_lang;
    use carrot_lsp::{DEFAULT_LSP_REQUEST_TIMEOUT, FakeLanguageServer};
    use carrot_multi_buffer::{MultiBuffer, MultiBufferOffset, PathKey};
    use carrot_project::{FakeFs, Project};
    use carrot_ui::App;
    use futures::{StreamExt, future};
    use inazuma::{AppContext as _, Context, TestAppContext, WindowHandle};
    use inazuma_collections::HashSet;
    use inazuma_settings_framework::{
        AllLanguageSettingsContent, InlayHintSettingsContent, SettingsStore,
    };
    use inazuma_text::{OffsetRangeExt, Point};
    use inazuma_util::path;
    use inazuma_util::paths::natural_sort;
    ...
    #[inazuma::test]
    async fn test_basic_cache_update_with_duplicate_hints(cx: &mut inazuma::TestAppContext) {
        ...
        let (_, editor, fake_server) = prepare_test_objects(cx, |fake_server, file_with_hints| {
            ...
            fake_server.set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                move |params, _| {
                    ...
                    async move {
                        ...
                        assert_eq!(
                            ...
                            carrot_lsp::Uri::from_file_path(file_with_hints).unwrap(),
                        );
                        Ok(Some(vec![carrot_lsp::InlayHint {
                            position: carrot_lsp::Position::new(0, i),
                            label: carrot_lsp::InlayHintLabel::String(i.to_string()),
                            kind: None,
                            text_edits: None,
                            tooltip: None,
                            padding_left: None,
        ...
        fake_server
            .request::<carrot_lsp::request::InlayHintRefreshRequest>(
                (),
    ...
    #[inazuma::test]
    async fn test_racy_cache_updates(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                enabled: Some(true),
                ...
                move |params, _| {
                    ...
                    async move {
                        ...
                        Ok(Some(vec![carrot_lsp::InlayHint {
                            position: carrot_lsp::Position::new(0, i),
                            label: carrot_lsp::InlayHintLabel::String(i.to_string()),
                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
                            text_edits: None,
                            tooltip: None,
                            padding_left: None,
                            padding_right: None,
    ...
    #[inazuma::test]
    async fn test_cache_update_on_lsp_completion_tasks(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
            ...
            fake_server.set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                move |params, _| {
                    ...
                    async move {
                        ...
                        Ok(Some(vec![carrot_lsp::InlayHint {
                            position: carrot_lsp::Position::new(0, current_call_id),
                            label: carrot_lsp::InlayHintLabel::String(current_call_id.to_string()),
                            kind: None,
                            text_edits: None,
                            tooltip: None,
                            padding_left: None,
        ...
        fake_server
            .request::<carrot_lsp::request::WorkDoneProgressCreate>(
                carrot_lsp::WorkDoneProgressCreateParams {
                    token: carrot_lsp::ProgressToken::Number(progress_token),
                },
                DEFAULT_LSP_REQUEST_TIMEOUT,
            )
            .await
        ...
        cx.executor().run_until_parked();
        fake_server.notify::<carrot_lsp::notification::Progress>(carrot_lsp::ProgressParams {
            token: carrot_lsp::ProgressToken::Number(progress_token),
            value: carrot_lsp::ProgressParamsValue::WorkDone(carrot_lsp::WorkDoneProgress::Begin(
                carrot_lsp::WorkDoneProgressBegin::default(),
            )),
        });
        cx.executor().run_until_parked();
        ...
        fake_server.notify::<carrot_lsp::notification::Progress>(carrot_lsp::ProgressParams {
            token: carrot_lsp::ProgressToken::Number(progress_token),
            value: carrot_lsp::ProgressParamsValue::WorkDone(carrot_lsp::WorkDoneProgress::End(
                carrot_lsp::WorkDoneProgressEnd::default(),
            )),
        });
        cx.executor().run_until_parked();
    ...
    #[inazuma::test]
    async fn test_no_hint_updates_for_unrelated_language_files(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
            ...
            let fake_servers = language_registry.register_fake_lsp(
                ...
                FakeLspAdapter {
                    ...
                    capabilities: carrot_lsp::ServerCapabilities {
                        inlay_hint_provider: Some(carrot_lsp::OneOf::Left(true)),
                        ..Default::default()
                    },
                    initializer: Some(Box::new({
                        move |fake_server| {
                            ...
                            fake_server
                                .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                                    move |params, _| {
                                        let i = match name {
                                            "Rust" => {
                                                assert_eq!(
                                                    params.text_document.uri,
                                                    carrot_lsp::Uri::from_file_path(path!(
                                                        "/a/main.rs"
                                                    ))
                                                    .unwrap(),
                                                );
                                        ...
                                        async move {
                                            ...
                                            Ok(Some(vec![carrot_lsp::InlayHint {
                                                ...
                                                label: carrot_lsp::InlayHintLabel::String(
                                                    i.to_string(),
                                                ),
                                                kind: None,
                                                text_edits: None,
    ...
    #[inazuma::test]
    async fn test_hint_setting_changes(cx: &mut inazuma::TestAppContext) {
        let allowed_hint_kinds = HashSet::from_iter([None, Some(InlayHintKind::Type)]);
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                        ...
                        async move {
                            ...
                            Ok(Some(vec![
                                carrot_lsp::InlayHint {
                                    position: carrot_lsp::Position::new(0, 1),
                                    label: carrot_lsp::InlayHintLabel::String(
                                        "type hint".to_string(),
                                    ),
                                    kind: Some(carrot_lsp::InlayHintKind::TYPE),
                                ...
                                carrot_lsp::InlayHint {
                                    position: carrot_lsp::Position::new(0, 2),
                                    label: carrot_lsp::InlayHintLabel::String(
                                        "parameter hint".to_string(),
                                    ),
                                    kind: Some(carrot_lsp::InlayHintKind::PARAMETER),
                                    text_edits: None,
                                    tooltip: None,
                                    padding_left: None,
                                    padding_right: None,
                                ...
                                carrot_lsp::InlayHint {
                                    position: carrot_lsp::Position::new(0, 3),
                                    label: carrot_lsp::InlayHintLabel::String(
    ...
    #[inazuma::test]
    async fn test_hint_request_cancellation(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
    ...
    #[inazuma::test(iterations = 4)]
    async fn test_large_buffer_inlay_requests_split(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                enabled: Some(true),
                ..InlayHintSettingsContent::default()
                        ...
                        fake_server
                            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                                move |params, _| {
                                    ...
                                    let task_lsp_request_count =
                                        Arc::clone(&closure_lsp_request_count);
                                    async move {
                                        assert_eq!(
                                            params.text_document.uri,
                                            carrot_lsp::Uri::from_file_path(path!("/a/main.rs"))
                                                .unwrap(),
                                        );
    ...
    #[inazuma::test]
    async fn test_multiple_excerpts_large_multibuffer(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
        ...
        fake_server
            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(move |params, _| {
                let task_editor_edited = Arc::clone(&closure_editor_edited);
                async move {
                    let hint_text = if params.text_document.uri
                        == carrot_lsp::Uri::from_file_path(path!("/a/main.rs")).unwrap()
                    {
                        "main hint"
                    } else if params.text_document.uri
                        == carrot_lsp::Uri::from_file_path(path!("/a/other.rs")).unwrap()
                    {
                        "other hint"
                    } else {
                    ...
                    let positions = [
                        carrot_lsp::Position::new(0, 2),
                        carrot_lsp::Position::new(4, 2),
                        carrot_lsp::Position::new(22, 2),
                        carrot_lsp::Position::new(44, 2),
                        carrot_lsp::Position::new(56, 2),
                        carrot_lsp::Position::new(67, 2),
                    ];
                    let out_of_range_hint = carrot_lsp::InlayHint {
                        position: carrot_lsp::Position::new(
                    ...
                    Ok(Some(
                        std::iter::once(out_of_range_hint)
                            .chain(positions.into_iter().enumerate().map(|(i, position)| {
                                carrot_lsp::InlayHint {
                                    position,
                                    label: carrot_lsp::InlayHintLabel::String(format!(
                                        "{hint_text}{E} #{i}",
    ...
    #[inazuma::test]
    async fn test_editing_in_multi_buffer(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                enabled: Some(true),
                ..InlayHintSettingsContent::default()
...
let c = 3;"#
                        ...
                        move |params, _| {
                            ...
                            async move {
                                    ...
                                    == carrot_lsp::Uri::from_file_path(path!("/a/main.rs")).unwrap()
                                {
                                    ...
                                    == carrot_lsp::Uri::from_file_path(path!("/a/lib.rs")).unwrap()
                                {
                                ...
                                Ok(Some(
                                    (params.range.start.line..params.range.end.line)
                                        .map(|row| carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(row, 0),
                                            label: carrot_lsp::InlayHintLabel::String(format!(
    ...
    #[inazuma::test]
    async fn test_excerpts_removed(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
    ...
    #[inazuma::test]
    async fn test_inside_char_boundary_range_hints(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
    ...
    #[inazuma::test]
    async fn test_toggle_inlay_hints(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(false),
    ...
    #[inazuma::test]
    async fn test_modifiers_change(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
    ...
    #[inazuma::test]
    async fn test_inlays_at_the_same_place(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
    ...
    #[inazuma::test]
    async fn test_invalidation_and_addition_race(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                enabled: Some(true),
                ..InlayHintSettingsContent::default()
...
"#
                        ...
                        move |params, _| {
                            ...
                            async move {
                                    ...
                                    == carrot_lsp::Uri::from_file_path(path!("/a/main.rs")).unwrap()
                                {
                                    Ok(Some(vec![
                                        carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(1, 9),
                                            label: carrot_lsp::InlayHintLabel::String(
                                                ": i32".to_owned(),
                                            ),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
                                        ...
                                        carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(19, 9),
                                            label: carrot_lsp::InlayHintLabel::String(
                                                ": i33".to_owned(),
                                            ),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
                                    ...
                                    == carrot_lsp::Uri::from_file_path(path!("/a/lib.rs")).unwrap()
                                {
                                    Ok(Some(vec![
                                        carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(1, 10),
                                            label: carrot_lsp::InlayHintLabel::String(
                                                ": i34".to_owned(),
                                            ),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
                                        ...
                                        carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(29, 10),
                                            label: carrot_lsp::InlayHintLabel::String(
                                                ": i35".to_owned(),
                                            ),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
    ...
    #[inazuma::test]
    async fn test_edit_then_scroll_race(cx: &mut inazuma::TestAppContext) {
                            ...
                            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                                move |params, _| {
                                    ...
                                    async move {
                                        ...
                                        Ok(Some(vec![carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(start_line + 1, 9),
                                            label: carrot_lsp::InlayHintLabel::String(format!(
                                                "chunk_{start_line}"
                                            )),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
    ...
    #[inazuma::test]
    async fn test_refresh_requested_multi_server(cx: &mut inazuma::TestAppContext) {
                        ...
                        fake_server
                            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                                move |_params, _| {
                                    ...
                                    async move {
                                        Ok(Some(vec![carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(0, 9),
                                            label: carrot_lsp::InlayHintLabel::String(format!(
                                                "server_a_{count}"
                                            )),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
                        ...
                        fake_server
                            .set_request_handler::<carrot_lsp::request::InlayHintRequest, _, _>(
                                move |_params, _| {
                                    ...
                                    async move {
                                        Ok(Some(vec![carrot_lsp::InlayHint {
                                            position: carrot_lsp::Position::new(0, 22),
                                            label: carrot_lsp::InlayHintLabel::String(format!(
                                                "server_b_{count}"
                                            )),
                                            kind: Some(carrot_lsp::InlayHintKind::TYPE),
    ...
    #[inazuma::test]
    async fn test_multi_language_multibuffer_no_duplicate_hints(cx: &mut inazuma::TestAppContext) {
        init_test(cx, &|settings| {
            settings.defaults.inlay_hints = Some(InlayHintSettingsContent {
                show_value_hints: Some(true),
                enabled: Some(true),
