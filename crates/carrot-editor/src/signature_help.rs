...
use carrot_multi_buffer::{Anchor, MultiBufferOffset, ToOffset};
...
impl Editor {
    ...
    pub(super) fn should_open_signature_help_automatically(
        ...
        let bracket_range = |position: MultiBufferOffset| {
            ...
            let start = buffer_snapshot.clip_offset(range.start, inazuma_text::Bias::Left);
            let end = buffer_snapshot.clip_offset(range.end, inazuma_text::Bias::Right);
            start..end
        };
    ...
    pub fn render(
                        ...
                        |this, param_doc| {
                            this.child(div().h_px().bg(cx.theme().colors().border_variant).my_1())
                                .child(
                                    ...
                                    .code_block_renderer(
                                        carrot_markdown::CodeBlockRenderer::Default {
                                            copy_button: false,
                                            border: false,
        ...
        let controls = if self.signatures.len() > 1 {
            ...
            let next_button = IconButton::new("signature_help_next", IconName::ChevronDown)
                ...
                .icon_size(IconSize::Small)
                .tooltip(move |_window, cx| {
                    carrot_ui::Tooltip::for_action("Next Signature", &crate::SignatureHelpNext, cx)
                })
                .on_click(cx.listener(|editor, _, window, cx| {
