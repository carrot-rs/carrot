...
use carrot_buffer_diff::{BufferDiff, BufferDiffSnapshot};
use inazuma_collections::HashMap;

use carrot_language::{Buffer, Capability, HighlightedText};
...
use inazuma_settings_framework::DiffViewStyle;
use inazuma_text::{Bias, BufferId, OffsetRangeExt as _, Patch, ToPoint as _};
...
use carrot_workspace::{
    ActivatePaneLeft, ActivatePaneRight, Item, PaneRole, ToolbarItemLocation, Workspace,
...
use carrot_actions::assistant::InlineAssist;
...
where
    F: Fn(
        &BufferDiffSnapshot,
        RangeInclusive<Point>,
        &inazuma_text::BufferSnapshot,
    ) -> Patch<Point>,
{
    struct PendingExcerpt<'a> {
        source_excerpt_id: ExcerptId,
        target_excerpt_id: ExcerptId,
        source_buffer: &'a inazuma_text::BufferSnapshot,
        target_buffer: &'a inazuma_text::BufferSnapshot,
...
fn patch_for_excerpt(
    ...
    source_excerpt_id: ExcerptId,
    target_excerpt_id: ExcerptId,
    target_buffer: &inazuma_text::BufferSnapshot,
    source_context_range: Range<Point>,
    patch: &Patch<Point>,
    ...
    let mut merged_edits: Vec<inazuma_text::Edit<Point>> = Vec::new();
    for edit in edits {
...
impl SplittableEditor {
    ...
    fn debug_print(&self, cx: &mut App) {
        ...
        use crate::display_map::Block;
        use carrot_buffer_diff::DiffHunkStatusKind;
        ...
        fn format_row(
            ...
            row_infos: &[carrot_multi_buffer::RowInfo],
    ...
    fn check_excerpt_mapping_invariants(&self, cx: &inazuma::App) {
        use carrot_multi_buffer::{ExcerptId, PathKey};
...
impl Item for SplittableEditor {
    ...
    fn tab_content_text(&self, detail: usize, cx: &App) -> carrot_ui::SharedString {
    ...
    fn tab_tooltip_text(&self, cx: &App) -> Option<carrot_ui::SharedString> {
        self.rhs_editor.read(cx).tab_tooltip_text(cx)
    }

    fn tab_icon(&self, window: &Window, cx: &App) -> Option<carrot_ui::Icon> {
    ...
    fn for_each_project_item(
        &self,
        cx: &App,
        f: &mut dyn FnMut(inazuma::EntityId, &dyn carrot_project::ProjectItem),
    ) {
    ...
    fn capability(&self, cx: &App) -> carrot_language::Capability {
    ...
    fn save_as(
        &mut self,
        project: Entity<Project>,
        path: carrot_project::ProjectPath,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> inazuma::Task<anyhow::Result<()>> {
        self.rhs_editor
    ...
    fn pixel_position_of_cursor(&self, cx: &App) -> Option<inazuma::Point<inazuma::Pixels>> {
...
impl SearchableItem for SplittableEditor {
    ...
    fn replace(
        ...
        query: &carrot_project::search::SearchQuery,
        token: SearchToken,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn find_matches_with_token(
        ...
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> inazuma::Task<(Vec<Self::Match>, SearchToken)> {
    ...
    fn active_match_index(
        &mut self,
        direction: carrot_workspace::searchable::Direction,
        matches: &[Self::Match],
        token: SearchToken,
        window: &mut Window,
        cx: &mut Context<Self>,
...
//     fn as_searchable(&self, _this: &Entity<Self>, cx: &App) -> Option<Box<dyn carrot_workspace::searchable::SearchableItemHandle>> {
...
mod tests {
    ...
    use carrot_language::language_settings::SoftWrap;
    use carrot_language::{Buffer, Capability};
    use carrot_multi_buffer::{MultiBuffer, PathKey};
    use carrot_project::Project;
    use carrot_ui::{VisualContext as _, div, px};
    use carrot_workspace::{Item, Workspace};
    use inazuma::Element as _;
    use inazuma::{AppContext as _, Entity, Pixels, VisualTestContext};
    ...
    async fn test_random_split_editor(mut rng: StdRng, cx: &mut inazuma::TestAppContext) {
        use carrot_multi_buffer::ExpandExcerptDirection;
    ...
    #[inazuma::test]
    async fn test_expand_excerpt_with_hunk_before_excerpt_start(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        ...
        editor.update(cx, |editor, cx| {
            editor.expand_excerpts(
                ...
                2,
                carrot_multi_buffer::ExpandExcerptDirection::UpAndDown,
                cx,
            );
        });
    }
    ...
    #[inazuma::test]
    async fn test_basic_alignment(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
    ...
    #[inazuma::test]
    async fn test_deleting_unmodified_lines(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_deleting_added_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_inserting_consecutive_blank_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_reverting_deletion_hunk(cx: &mut inazuma::TestAppContext) {
        use carrot_git::Restore;
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_deleting_added_lines(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_soft_wrap_at_end_of_excerpt(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_soft_wrap_before_modification_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_soft_wrap_before_deletion_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_soft_wrap_spacer_after_editing_second_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_no_base_text(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_deleting_char_in_added_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_soft_wrap_spacer_before_added_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    #[ignore]
    async fn test_joining_added_line_with_unmodified_line(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_added_file_at_end(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_adding_line_to_addition_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_scrolling(cx: &mut inazuma::TestAppContext) {
        ...
        rhs_editor.update_in(cx, |e, window, cx| {
            e.set_scroll_position(inazuma::Point::new(0., 10.), window, cx);
        });
    ...
    #[inazuma::test]
    async fn test_edit_line_before_soft_wrapped_line_preceding_hunk(
        cx: &mut inazuma::TestAppContext,
    ) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_sync_between_split_views(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_deletion_and_resplit_sync(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_sync_with_unsplit_start(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_buffer_folding_sync(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_in_middle_of_added_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_below_in_middle_of_added_hunk(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_custom_block_resize_syncs_balancing_block(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_edit_spanning_excerpt_boundaries_then_resplit(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_range_folds_removed_on_split(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_multiline_inlays_create_spacers(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_split_after_removing_folded_buffer(cx: &mut inazuma::TestAppContext) {
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_two_path_keys_for_one_buffer(cx: &mut inazuma::TestAppContext) {
        use carrot_multi_buffer::PathKey;
        use inazuma_rope::Point;
        use unindent::Unindent as _;
    ...
    #[inazuma::test]
    async fn test_act_as_type(cx: &mut inazuma::TestAppContext) {
