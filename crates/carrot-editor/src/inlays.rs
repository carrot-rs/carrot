...
use carrot_project::{InlayHint, InlayId};
use inazuma::{Context, HighlightStyle, Oklch, Rgba, Task};
...
impl Inlay {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn get_color(&self) -> Option<Oklch> {
        match self.content {
...
impl Editor {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn inline_value_inlays(&self, cx: &inazuma::App) -> Vec<Inlay> {
        self.display_map
            .read(cx)
            .current_inlays()
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn all_inlays(&self, cx: &inazuma::App) -> Vec<Inlay> {
        self.display_map
            .read(cx)
            .current_inlays()
            .cloned()
