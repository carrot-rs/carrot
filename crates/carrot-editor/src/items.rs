...
use carrot_rpc::proto::{self, update_view};
use carrot_ui::{IconDecorationKind, prelude::*};
use carrot_workspace::item::{Dedup, ItemSettings, SerializableItem, TabContentParams};
use carrot_workspace::{
    CollaboratorId, ItemId, ItemNavHistory, PaneRole, ToolbarItemLocation, ViewId, Workspace,
    WorkspaceId,
...
use inazuma_text::{BufferId, BufferSnapshot, Selection};
use inazuma_util::{ResultExt, TryFutureExt, paths::PathExt};
use std::{
    any::{Any, TypeId},
    borrow::Cow,
    cmp::{self, Ordering},
...
impl FollowableItem for Editor {
    ...
    fn to_follow_event(event: &EditorEvent) -> Option<carrot_workspace::item::FollowEvent> {
        match event {
...
) -> Option<(ExcerptId, ExcerptRange<carrot_language::Anchor>)> {
    let context = {
        let start = carrot_language::proto::deserialize_anchor(excerpt.context_start?)?;
        let end = carrot_language::proto::deserialize_anchor(excerpt.context_end?)?;
        start..end
    };
    let primary = excerpt
        .primary_start
        ...
        .and_then(|(start, end)| {
            let start = carrot_language::proto::deserialize_anchor(start)?;
            let end = carrot_language::proto::deserialize_anchor(end)?;
            Some(start..end)
        })
...
fn deserialize_anchor(anchor: proto::EditorAnchor) -> Option<Anchor> {
    ...
    Some(Anchor::in_buffer(
        excerpt_id,
        carrot_language::proto::deserialize_anchor(anchor.anchor?)?,
    ))
}
...
impl Item for Editor {
    ...
    fn tab_content(&self, params: TabContentParams, _: &Window, cx: &App) -> AnyElement {
        ...
        let description = params.detail.and_then(|detail| {
            ...
            Some(inazuma_util::truncate_and_trailoff(
                description,
                MAX_TAB_TITLE_LEN,
            ))
        });
        ...
        h_flex()
            .gap_2()
            .child(
                Label::new(inazuma_util::truncate_and_trailoff(
                    &self.title(cx),
                    MAX_TAB_TITLE_LEN,
                ))
                .color(label_color)
    ...
    fn pixel_position_of_cursor(&self, _: &App) -> Option<inazuma::Point<Pixels>> {
    ...
    fn breadcrumbs(&self, cx: &App) -> Option<(Vec<HighlightedText>, Option<Font>)> {
        if self.buffer.read(cx).is_singleton() {
            let font = carrot_theme_settings::ThemeSettings::get_global(cx)
                .buffer_font
                .clone();
    ...
    fn added_to_workspace(
        ...
        if let Some(workspace_entity) = &workspace.weak_handle().upgrade() {
            cx.subscribe(
                workspace_entity,
                |editor, _, event: &carrot_workspace::Event, _cx| {
                    if let carrot_workspace::Event::ModalOpened = event {
        ...
        if !has_folds {
            if let Some(workspace_id) = workspace.database_id()
                && let Some(file_path) = self.buffer().read(cx).as_singleton().and_then(|buffer| {
                    carrot_project::File::from_dyn(buffer.read(cx).file())
    ...
    fn tab_extra_context_menu_actions(
        ...
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Vec<(SharedString, Box<dyn inazuma::Action>)> {
        ...
        if is_markdown {
            actions.push((
                ...
                Box::new(OpenMarkdownPreview) as Box<dyn inazuma::Action>,
            ));
        }
        ...
        if is_svg {
            actions.push((
                ...
                Box::new(OpenSvgPreview) as Box<dyn inazuma::Action>,
            ));
        }
...
pub struct RestorationData {
    pub scroll_position: (BufferRow, inazuma::Point<ScrollOffset>),
    ...
    fn for_project_item(
        ...
        if let Some((excerpt_id, _, snapshot)) =
            ...
            && let Some(restoration_data) = Self::project_item_kind()
                ...
                .and_then(|data| {
                    let file = carrot_project::File::from_dyn(buffer.read(cx).file())?;
    ...
    pub fn update_restoration_data(
        ...
        cx.defer(move |cx| {
            editor.update(cx, |editor, cx| {
                ...
                let file_abs_path =
                    carrot_project::File::from_dyn(buffer.read(cx).file())?.abs_path(cx);
...
fn path_for_file<'a>(
    file: &'a Arc<dyn carrot_language::File>,
    mut height: usize,
    include_filename: bool,
    cx: &'a App,
) -> Option<Cow<'a, str>> {
    if carrot_project::File::from_dyn(Some(file)).is_none() {
        return None;
    }
...
mod tests {
    ...
    use super::*;
    use carrot_fs::MTime;
    use carrot_language::TestFile;
    use carrot_project::FakeFs;
    use inazuma::{App, VisualTestContext};
    use inazuma_util::{path, rel_path::RelPath};
    use serde_json::json;
    use std::path::{Path, PathBuf};

    #[inazuma::test]
    fn test_path_for_file(cx: &mut App) {
        let file: Arc<dyn carrot_language::File> = Arc::new(TestFile {
    ...
    #[inazuma::test]
    async fn test_deserialize(cx: &mut inazuma::TestAppContext) {
        init_test(cx, |_| {});
        ...
        // Test case 1: Deserialize with path and contents
        {
            ...
            let db = cx.update(|_, cx| carrot_workspace::WorkspaceDb::global(cx));
        ...
        // Test case 3: Deserialize with no path (untitled buffer, with content and language)
        {
            ...
            project.read_with(cx, |project, _| {
                project.languages().add(carrot_languages::rust_lang())
            });
    ...
    // Regression test for https://github.com/carrot-rs/carrot/issues/35947
