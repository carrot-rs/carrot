...
use inazuma::{App, HighlightStyle, Oklch, SharedString, UnderlineStyle, px};
...
impl DiagnosticSeverity {
    pub fn bg(&self, cx: &App) -> Oklch {
        let status = cx.theme().status();
        match self {
            Self::Error => status.error.background,
            Self::Warning => status.warning.background,
            Self::Info => status.info.background,
            Self::Hint => status.hint.background,
        }
    }

    pub fn fg(&self, cx: &App) -> Oklch {
        let status = cx.theme().status();
        match self {
            Self::Error => status.error.color,
            Self::Warning => status.warning.color,
            Self::Info => status.info.color,
            Self::Hint => status.hint.color,
        }
    }

    pub fn border(&self, cx: &App) -> Oklch {
        let status = cx.theme().status();
        match self {
            Self::Error => status.error.border,
            Self::Warning => status.warning.border,
            Self::Info => status.info.border,
            Self::Hint => status.hint.border,
        }
    }

    pub fn highlight_style(&self, cx: &App) -> HighlightStyle {
        let status = cx.theme().status();
        let color = match self {
            Self::Error => Some(status.error.color),
            Self::Warning => Some(status.warning.color),
            Self::Info => Some(status.info.color),
            Self::Hint => Some(status.hint.color),
        };
