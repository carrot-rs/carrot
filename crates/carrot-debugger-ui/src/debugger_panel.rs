...
use carrot_dap::{DapRegistry, StartDebuggingRequestArguments};
use carrot_dap::{client::SessionId, debugger_settings::DebuggerSettings};
use carrot_editor::{Editor, MultiBufferOffset, ToPoint};
...
use carrot_actions::debug_panel::ToggleFocus;
use carrot_language::Buffer;
use carrot_project::debugger::session::{Session, SessionQuirks, SessionState, SessionStateEvent};
use carrot_project::{DebugScenarioContext, Fs, ProjectPath, TaskSourceKind, WorktreeId};
use carrot_project::{Project, debugger::session::ThreadStatus};
use carrot_rpc::proto::{self};
use carrot_task::{DebugScenario, SharedTaskContext};
use carrot_ui::{
    ContextMenu, Divider, PopoverMenu, PopoverMenuHandle, SplitButton, Tab, Tooltip, prelude::*,
};
use carrot_workspace::SplitDirection;
use carrot_workspace::item::SaveOptions;
use carrot_workspace::{
    Item, Pane, Workspace,
    dock::{DockPosition, Panel, PanelEvent},
};
use inazuma_settings_framework::Settings;
use inazuma_util::redact::redact_command;
...
impl DebugPanel {
    ...
    pub(crate) fn top_controls_strip(
        ...
        let is_side = self.position(window, cx).axis() == inazuma::Axis::Horizontal;
        ...
        let edit_debug_json_button = || {
            IconButton::new("debug-edit-debug-json", IconName::Code)
                ...
                .on_click(|_, window, cx| {
                    window.dispatch_action(carrot_actions::OpenProjectDebugTasks.boxed_clone(), cx);
                })
        ...
        let documentation_button = || {
            IconButton::new("debug-open-documentation", IconName::CircleHelp)
                .icon_size(IconSize::Small)
                .on_click(move |_, _, cx| cx.open_url("https://carrot.dev/docs/debugger"))
            ...
            IconButton::new("debug-open-logs", IconName::Notepad)
                ...
                .on_click(move |_, window, cx| {
                    window.dispatch_action(
                        carrot_debugger_tools::OpenDebugAdapterLogs.boxed_clone(),
                        cx,
                    )
                })
        ...
        let close_bottom_panel_button = {
            h_flex().pl_0p5().gap_1().child(Divider::vertical()).child(
                IconButton::new("debug-close-panel", IconName::Close)
                    .icon_size(IconSize::Small)
                    .on_click(move |_, window, cx| {
                        window.dispatch_action(carrot_workspace::ToggleBottomDock.boxed_clone(), cx)
                    })
        ...
        let thread_status = active_session
            ...
            .unwrap_or(carrot_project::debugger::session::ThreadStatus::Exited);
                                    ...
                                    .when(
                                        ...
                                        |this| {
                                            this.child(Divider::vertical()).child(
                                                ...
                                                .style(carrot_ui::SplitButtonStyle::Outlined),
                                            )
                                        },
                                    )
                                },
        ...
        self.workspace
            .update(cx, |workspace, cx| {
                ...
                cx.spawn_in(window, async move |workspace, cx| {
                    ...
                    path.push(carrot_paths::local_settings_folder_name());
                    if !fs.is_dir(path.as_path()).await {
                        fs.create_dir(path.as_path()).await?;
                    }
                    path.pop();

                    path.push(carrot_paths::local_debug_file_relative_path().as_std_path());
                    ...
                    let project_path = workspace.update(cx, |workspace, cx| {
                        workspace
                            ...
                            .context(
                                "Couldn't get project path for .carrot/debug.json in active worktree",
                            )
                    })??;
    ...
    fn render_history_toggle_button(
        ...
        PopoverMenu::new("debug-back-in-history-menu")
            .trigger(
                carrot_ui::ButtonLike::new_rounded_right("debug-back-in-history-menu-trigger")
...
async fn register_session_inner(
    ...
    let serialized_layout = this
        .update(cx, |_, cx| {
            persistence::get_serialized_layout(
                &adapter_name,
                &carrot_db::kvp::KeyValueStore::global(cx),
            )
        })
        .ok()
        .flatten();
...
impl Render for DebugPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .on_action({
                let this = this.clone();
                move |_: &carrot_workspace::ActivatePaneLeft, window, cx| {
                    this.update(cx, |this, cx| {
            ...
            .on_action({
                let this = this.clone();
                move |_: &carrot_workspace::ActivatePaneRight, window, cx| {
                    this.update(cx, |this, cx| {
            ...
            .on_action({
                let this = this.clone();
                move |_: &carrot_workspace::ActivatePaneUp, window, cx| {
                    this.update(cx, |this, cx| {
            ...
            .on_action({
                let this = this.clone();
                move |_: &carrot_workspace::ActivatePaneDown, window, cx| {
                    this.update(cx, |this, cx| {
                    ...
                    let welcome_experience = v_flex()
                        ...
                        .child(
                            Button::new("edit-debug-settings", "Edit debug.json")
                                ...
                                .on_click(|_, window, cx| {
                                    window.dispatch_action(
                                        carrot_actions::OpenProjectDebugTasks.boxed_clone(),
                                        cx,
                                    );
                                }),
                        )
                        .child(
                            Button::new("open-debugger-docs", "Debugger Docs")
                                ...
                                .on_click(|_, _, cx| {
                                    cx.open_url("https://carrot.dev/docs/debugger")
                                }),
                        )
                        .child(
                            Button::new(
                            ...
                            .on_click(|_, window, cx| {
                                window.dispatch_action(
                                    carrot_actions::Extensions {
                                        category_filter: Some(
                                            carrot_actions::ExtensionCategoryFilter::DebugAdapters,
                                        ),
                                        id: None,
                                    }
                                    .boxed_clone(),
...
impl carrot_workspace::DebuggerProvider for DebuggerProvider {
    fn start_session(
        &self,
