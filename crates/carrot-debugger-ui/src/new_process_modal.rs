...
use carrot_language::{LanguageName, LanguageRegistry};
use carrot_tasks_ui::{TaskOverrides, TasksModal};
use inazuma_collections::{FxHashMap, HashMap, HashSet};
use std::{
    borrow::Cow,
    path::{Path, PathBuf},
    sync::Arc,
...
use carrot_project::{
    DebugScenarioContext, Project, TaskContexts, TaskSourceKind, task_store::TaskStore,
};
use carrot_task::{
    CarrotDebugConfig, DebugScenario, RevealTarget, SharedTaskContext, VariableName,
};
use carrot_ui::{
...
use carrot_workspace::{ModalView, Workspace, notifications::DetachAndPromptErr, pane};
use inazuma::{
...
use inazuma_util::{ResultExt, debug_panic, rel_path::RelPath, shell::ShellKind};
...
impl NewProcessModal {
    pub(super) fn show(
        ...
        cx.spawn_in(window, async move |workspace, cx| {
            let task_contexts = workspace.update_in(cx, |workspace, window, cx| {
                ...
                carrot_tasks_ui::task_contexts(workspace, window, cx)
            })?;
                        ...
                        async move |this, cx| {
                            ...
                            let (lsp_tasks, prefer_lsp) =
                                workspace.update(cx, |workspace, cx| {
                                    let lsp_tasks = carrot_editor::lsp_tasks(
    ...
    fn debug_scenario(&self, debugger: &str, cx: &App) -> Task<Option<DebugScenario>> {
        ...
        let session_scenario = CarrotDebugConfig {
        ...
        cx.spawn(async move |_| {
            adapter?
                .config_from_carrot_format(session_scenario)
                .await
                .ok()
        })
    }
    ...
    fn adapter_drop_down_menu(
        ...
        .style(carrot_ui::DropdownStyle::Outlined)
        .tab_index(0)
        ...
        v_flex()
            ...
            .child(
                h_flex().p_2().pb_0p5().w_full().child(
                    ...
                    .style(carrot_ui::ToggleButtonGroupStyle::Outlined)
...
impl Focusable for NewProcessModal {
    fn focus_handle(&self, cx: &carrot_ui::App) -> inazuma::FocusHandle {
...
impl ConfigureMode {
    pub(super) fn new(window: &mut Window, cx: &mut App) -> Entity<Self> {
        let program = cx.new(|cx| {
            InputField::new(window, cx, "ENV=Carrot ~/bin/program --option")
                .label("Program")
                .tab_stop(true)
                .tab_index(1)
        });
    ...
    pub(super) fn debug_request(&self, cx: &App) -> carrot_task::LaunchRequest {
        ...
        if cfg!(windows) {
            return carrot_task::LaunchRequest {
    ...
    fn render(
        ...
        v_flex()
            ...
            .child(
                Switch::new("debugger-stop-on-entry")
                    .toggle_state(self.stop_on_entry)
                    .tab_index(3_isize)
                    .label("Stop on Entry")
...
pub(super) struct AttachMode {
    pub(super) definition: CarrotDebugConfig,
...
impl AttachMode {
    pub(super) fn new(
        ...
        let definition = CarrotDebugConfig {
            adapter: debugger.unwrap_or(DebugAdapterName("".into())).0,
            label: "Attach New Session Setup".into(),
            request: carrot_dap::DebugRequest::Attach(carrot_task::AttachRequest {
                process_id: None,
            }),
    ...
    pub(super) fn debug_request(&self) -> carrot_task::AttachRequest {
        carrot_task::AttachRequest { process_id: None }
    }
}
...
impl DebugDelegate {
    ...
    pub fn tasks_loaded(
        ...
        languages: Arc<LanguageRegistry>,
        lsp_tasks: Vec<(TaskSourceKind, carrot_task::ResolvedTask)>,
        current_resolved_tasks: Vec<(TaskSourceKind, carrot_task::ResolvedTask)>,
            ...
            this.update(cx, |this, cx| {
                ...
                let hide_vscode = scenarios.iter().any(|(kind, _)| match kind {
                    TaskSourceKind::Worktree {
                        ...
                        id_base: _,
                    } => dir.ends_with(RelPath::unix(".carrot").unwrap()),
                    _ => false,
                });
...
impl PickerDelegate for DebugDelegate {
    type ListItem = carrot_ui::ListItem;
    ...
    fn confirm_input(&mut self, _: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        let args = args.collect::<Vec<_>>();
        let task = carrot_task::TaskTemplate {
        ...
        let Some(adapter): Option<DebugAdapterName> =
            carrot_language::language_settings::LanguageSettings::for_buffer(buffer, cx)
                .debuggers
                .first()
    ...
    fn render_match(
        ...
        let language_icon = language_name.as_ref().and_then(|lang| {
            carrot_file_icons::FileIcons::get(cx)
...
pub(crate) fn resolve_path(path: &mut String) {
    if path.starts_with('~') {
        let home = carrot_paths::home_dir().to_string_lossy().into_owned();
...
impl NewProcessModal {
    pub(crate) fn set_configure(
        ...
        self.debugger = Some(carrot_dap::adapters::DebugAdapterName(
            "fake-adapter".into(),
        ));
