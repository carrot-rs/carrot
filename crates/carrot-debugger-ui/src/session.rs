...
use carrot_dap::client::SessionId;
use carrot_project::debugger::session::Session;
use inazuma::{App, Axis, Entity, EventEmitter, FocusHandle, Focusable, Task, WeakEntity};

use carrot_project::{Project, debugger::session::SessionQuirks};
use carrot_rpc::proto;
use carrot_ui::prelude::*;
use carrot_workspace::{
    CollaboratorId, FollowableItem, PaneRole, ViewId, Workspace,
    item::{self, Item},
};
...
impl FollowableItem for DebugSession {
    ...
    fn to_follow_event(_event: &Self::Event) -> Option<carrot_workspace::item::FollowEvent> {
        None
    }
    ...
    fn dedup(
        ...
        _window: &Window,
        cx: &App,
    ) -> Option<carrot_workspace::item::Dedup> {
