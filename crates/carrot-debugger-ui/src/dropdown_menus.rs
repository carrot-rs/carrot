...
use carrot_project::debugger::session::{ThreadId, ThreadStatus};
use carrot_ui::{
    CommonAnimationExt, ContextMenu, DropdownMenu, DropdownStyle, Indicator, prelude::*,
};
use inazuma::{Corner, Entity, WeakEntity};
use inazuma_collections::HashMap;
use inazuma_util::{maybe, truncate_and_trailoff};
        ...
        let icon = {
            if is_terminated {
                ...
                match self
                    ...
                    .unwrap_or_default()
                {
                    carrot_project::debugger::session::ThreadStatus::Stopped => {
...
impl DebugPanel {
    ...
    pub(crate) fn render_thread_dropdown(
        ...
        threads: Vec<(carrot_dap::Thread, ThreadStatus)>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Option<DropdownMenu> {
        const MAX_LABEL_CHARS: usize = 150;
