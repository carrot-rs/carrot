...
use carrot_workspace::pane_group::SplitDirection;
use carrot_workspace::{Item, dock::Panel, move_active_item};
...
async fn test_basic_show_debug_panel(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    client.on_request::<Threads, _>(move |_, _| {
        Ok(carrot_dap::ThreadsResponse {
            threads: vec![carrot_dap::Thread {
                id: 1,
                name: "Thread 1".into(),
            }],
        })
    ...
    client.on_request::<StackTrace, _>(move |_, _| {
        Ok(carrot_dap::StackTraceResponse {
    ...
    client
        .fake_event(carrot_dap::messages::Events::Stopped(
            carrot_dap::StoppedEvent {
                reason: carrot_dap::StoppedEventReason::Pause,
                description: None,
                thread_id: Some(1),
...
async fn test_handle_start_debugging_request(
    ...
    let _subscription = carrot_project::debugger::test::intercept_debug_sessions(cx, {
        let launched_with = launched_with.clone();
        move |client| {
            let launched_with = launched_with.clone();
            client.on_request::<carrot_dap::requests::Launch, _>(move |_, args| {
                launched_with.lock().replace(args.raw);
                Ok(())
            });
            client.on_request::<carrot_dap::requests::Attach, _>(move |_, _| {
...
async fn test_handle_start_debugging_reverse_request(
    ...
    client.on_request::<carrot_dap::requests::Threads, _>(move |_, _| {
        Ok(carrot_dap::ThreadsResponse {
            threads: vec![carrot_dap::Thread {
                id: 1,
                name: "Thread 1".into(),
    ...
    let _reverse_request_subscription =
        carrot_project::debugger::test::intercept_debug_sessions(cx, |_| {});
    client
        .fake_reverse_request::<StartDebugging>(StartDebuggingRequestArguments {
            configuration: json!({}),
            request: StartDebuggingRequestArgumentsRequest::Launch,
    ...
    child_client.on_request::<carrot_dap::requests::Threads, _>(move |_, _| {
        Ok(carrot_dap::ThreadsResponse {
            threads: vec![carrot_dap::Thread {
                id: 1,
                name: "Thread 1".into(),
...
async fn test_debug_panel_item_thread_status_reset_on_failure(
    ...
    let session = start_debug_session(&workspace, cx, |client| {
        client.on_request::<carrot_dap::requests::Initialize, _>(move |_, _| {
            Ok(carrot_dap::Capabilities {
...
async fn test_send_breakpoints_when_editor_has_been_saved(
    ...
    client.on_request::<SetBreakpoints, _>({
        ...
        move |_, args| {
            ...
            Ok(carrot_dap::SetBreakpointsResponse {
...
async fn test_debug_session_is_shutdown_when_attach_and_launch_request_fails(
    ...
    start_debug_session(&workspace, cx, |client| {
        client.on_request::<carrot_dap::requests::Initialize, _>(|_, _| {
            Err(ErrorResponse {
                error: Some(Message {
...
async fn test_active_debug_line_setting(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    client.on_request::<carrot_dap::requests::Scopes, _>(move |_, _| {
        Ok(carrot_dap::ScopesResponse {
            scopes: Vec::default(),
        })
    });
    ...
    client.on_request::<StackTrace, _>(move |_, args| {
        ...
        Ok(carrot_dap::StackTraceResponse {
            stack_frames: vec![carrot_dap::StackFrame {
                id: 1,
                name: "frame 1".into(),
                source: Some(carrot_dap::Source {
    ...
    client
        .fake_event(carrot_dap::messages::Events::Stopped(
            carrot_dap::StoppedEvent {
                reason: carrot_dap::StoppedEventReason::Breakpoint,
                description: None,
                thread_id: Some(1),
                preserve_focus_hint: None,
                text: None,
    ...
    main_editor.update_in(cx, |editor, window, cx| {
        ...
        let point = editor
            .snapshot(window, cx)
            .buffer_snapshot()
            .summary_for_anchor::<carrot_language::Point>(
                &active_debug_lines.first().unwrap().0.start,
            );
    ...
    client
        .fake_event(carrot_dap::messages::Events::Continued(
            carrot_dap::ContinuedEvent {
                thread_id: 0,
...
async fn test_adapter_shutdown_with_child_sessions_on_app_quit(
    ...
    parent_client
        ...
        .await;
    let _subscription = carrot_project::debugger::test::intercept_debug_sessions(cx, |_| {});
...
async fn test_restart_request_is_not_sent_more_than_once_until_response(
    ...
    client.on_request::<carrot_dap::requests::Restart, _>({
