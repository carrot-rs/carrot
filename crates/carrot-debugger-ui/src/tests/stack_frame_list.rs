...
use carrot_editor::{Editor, ToPoint as _};
...
async fn test_fetch_initial_stack_frames_and_go_to_stack_frame(
    ...
    client.on_request::<Scopes, _>(move |_, _| Ok(carrot_dap::ScopesResponse { scopes: vec![] }));
...
async fn test_select_stack_frame(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let _ = workspace.update(cx, |workspace, window, cx| {
        workspace.toggle_dock(carrot_workspace::dock::DockPosition::Bottom, window, cx);
    });
        ...
        .update(cx, |workspace, window, cx| {
            ...
            assert_eq!(
                ...
                editors[0].update(cx, |editor, cx| {
                    ...
                    editor
                        .highlighted_rows::<carrot_editor::ActiveDebugLine>()
                        .map(|(range, _)| {
...
async fn test_collapsed_entries(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let stack_frames = vec![
        ...
        StackFrame {
            ...
            instruction_pointer_reference: None,
            module_id: None,
            presentation_hint: Some(carrot_dap::StackFramePresentationHint::Deemphasize),
        },
        StackFrame {
            id: 3,
...
async fn test_stack_frame_filter(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    stack_frame_list.update(cx, |stack_frame_list, cx| {
        ...
        stack_frame_list.toggle_frame_filter(
            Some(carrot_project::debugger::session::ThreadStatus::Stopped),
            cx,
        );
        assert_eq!(
...
async fn test_stack_frame_filter_persistence(
    ...
    let threads_response = carrot_dap::ThreadsResponse {
        threads: vec![carrot_dap::Thread {
            id: 1,
            name: "Thread 1".into(),
        }],
    ...
    let stack_trace_response = carrot_dap::StackTraceResponse {
    ...
    let stopped_event = carrot_dap::StoppedEvent {
        reason: carrot_dap::StoppedEventReason::Pause,
        description: None,
        thread_id: Some(1),
        preserve_focus_hint: None,
    ...
    client
        .fake_event(carrot_dap::messages::Events::Stopped(stopped_event.clone()))
        .await;
    ...
    let workspace_id = workspace
        .update(cx, |workspace, _window, _| workspace.database_id())
        .ok()
        .flatten()
    ...
    client
        .fake_event(carrot_dap::messages::Events::Terminated(None))
        .await;
    cx.run_until_parked();
    ...
    client2.on_request::<Scopes, _>(move |_, _| Ok(carrot_dap::ScopesResponse { scopes: vec![] }));
