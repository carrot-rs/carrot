...
use carrot_dap::{FakeAdapter, adapters::DebugTaskDefinition};
use carrot_project::{FakeFs, Project};
use carrot_task::{AttachRequest, SharedTaskContext};
...
async fn test_direct_attach_to_process(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let _session = start_debug_session_with(
        ...
        |client| {
            client.on_request::<carrot_dap::requests::Attach, _>(move |_, args| {
                let raw = &args.raw;
...
async fn test_show_attach_modal_and_select_process(
    ...
    let _initialize_subscription =
        carrot_project::debugger::test::intercept_debug_sessions(cx, |client| {
            client.on_request::<carrot_dap::requests::Attach, _>(move |_, args| {
                let raw = &args.raw;
                assert_eq!(raw["request"], "attach");
        ...
        .update(cx, |multi, window, cx| {
            ...
            multi.toggle_modal(window, cx, |window, cx| {
                AttachModal::with_processes(
                    ...
                    .collect(),
                    true,
                    ModalIntent::AttachToProcess(carrot_task::CarrotDebugConfig {
                        adapter: FakeAdapter::ADAPTER_NAME.into(),
                        request: carrot_dap::DebugRequest::Attach(AttachRequest::default()),
...
async fn test_attach_with_pick_pid_variable(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let pick_pid_placeholder = carrot_task::VariableName::PickProcessId.template_value();
    workspace
