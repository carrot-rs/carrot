...
use carrot_project::{FakeFs, Fs as _, Project};
use carrot_task::{
    CarrotDebugConfig, DebugRequest, DebugScenario, LaunchRequest, SharedTaskContext, TaskContext,
    VariableName,
};
...
async fn test_debug_session_substitutes_variables_and_relativizes_paths(
    ...
    let home_dir = carrot_paths::home_dir();
    ...
    for (input_path, expected_path) in test_cases {
        let _subscription = carrot_project::debugger::test::intercept_debug_sessions(cx, {
            ...
            move |client| {
                client.on_request::<carrot_dap::requests::Launch, _>({
                    let called_launch = called_launch.clone();
...
async fn test_save_debug_scenario_to_file(executor: BackgroundExecutor, cx: &mut TestAppContext) {
    ...
    let debug_json_content = fs
        .load(path!("/project/.carrot/debug.json").as_ref())
        .await
...
async fn test_debug_modal_subtitles_with_multiple_worktrees(
    ...
    assert_eq!(
        subtitles.as_slice(),
        [path!(".carrot/debug.json"), path!(".carrot/debug.json")]
    );
}
...
async fn test_dap_adapter_config_conversion_and_validation(cx: &mut TestAppContext) {
    ...
    for adapter_name in adapter_names {
        ...
        let mut adapter_specific_config = carrot_config.clone();
        adapter_specific_config.adapter = adapter_name.to_string().into();

        let debug_scenario = adapter
            .config_from_carrot_format(adapter_specific_config)
            .await
            .unwrap_or_else(|_| {
                panic!(
                    "Adapter {} should successfully convert from Carrot format",
                    adapter_name
                )
            });
        ...
        match request_type {
            carrot_dap::StartDebuggingRequestArgumentsRequest::Launch => {}
            carrot_dap::StartDebuggingRequestArgumentsRequest::Attach => {
                panic!(
