...
use carrot_editor::{Editor, MultiBufferOffsetUtf16};
use carrot_project::debugger::{self, breakpoint_store::SourceBreakpoint, session::ThreadStatus};
use debugger_panel::DebugPanel;
use inazuma::{Action, App, DispatchPhase, EntityInputHandler, actions};
use new_process_modal::{NewProcessModal, NewProcessMode};
use schemars::JsonSchema;
use serde::Deserialize;

use carrot_actions::debug_panel::{Toggle, ToggleFocus};
use carrot_tasks_ui::{Spawn, TaskOverrides};
use carrot_ui::{FluentBuilder, InteractiveElement};
use carrot_workspace::{ShutdownDebugAdapters, Workspace};
...
pub struct ToggleDataBreakpoint {
    ...
    /// Write
    #[serde(default)]
    pub access_type: Option<carrot_dap::DataBreakpointAccessType>,
}
...
pub fn init(cx: &mut App) {
    carrot_workspace::FollowableViewRegistry::register::<DebugSession>(cx);
            ...
            editor
                .register_action_renderer(move |editor, window, cx| {
                    ...
                    window.on_action_when(
                        ...
                        TypeId::of::<carrot_editor::actions::RunToCursor>(),
                        {
                            ...
                            move |_, phase, _, cx| {
                                ...
                                maybe!({
                                    let (buffer, position, _) = editor
                                        .update(cx, |editor, cx| {
                                            let cursor_point: carrot_language::Point = editor
                                                .selections
                    ...
                    window.on_action(
                        TypeId::of::<carrot_editor::actions::EvaluateSelectedText>(),
                        move |_, _, window, cx| {
                            let status = maybe!({
                                let text = editor
                                    .update(cx, |editor, cx| {
                                        ...
                                        state.session().update(cx, |session, cx| {
                                            session
                                                .evaluate(
                                                    text,
                                                    Some(
                                                        carrot_dap::EvaluateArgumentsContext::Repl,
                                                    ),
                                                    stack_id,
                                                    None,
                                                    cx,
...
fn spawn_task_or_modal(
    ...
    action: &Spawn,
    window: &mut carrot_ui::Window,
    cx: &mut carrot_ui::Context<Workspace>,
) {
    match action {
        Spawn::ByName {
            task_name,
            ...
            carrot_tasks_ui::spawn_tasks_filtered(
