...
use anyhow::Result;
use carrot_dap::{CompletionItem, CompletionItemType, OutputEvent};
use carrot_editor::{
...
use carrot_language::{Anchor, Buffer, CharScopeContext, CodeLabel, TextBufferSnapshot, ToOffset};
use carrot_project::{
...
use carrot_term::vte::ansi;
use carrot_theme::ActiveTheme as _;
use carrot_theme::Theme;
use carrot_theme_settings::ThemeSettings;
use carrot_ui::{ContextMenu, Divider, PopoverMenu, SplitButton, Tooltip, prelude::*};
use inazuma::{
    Action as _, AppContext, Context, Corner, Entity, FocusHandle, Focusable, HighlightStyle,
    Oklch, Render, Subscription, Task, TextStyle, WeakEntity, actions,
};
...
impl Console {
    pub fn new(
        ...
        let console = cx.new(|cx| {
            ...
            editor.move_to_end(&carrot_editor::actions::MoveToEnd, window, cx);
        ...
        self.console.update(cx, |_, cx| {
            cx.spawn_in(window, async move |console, cx| {
                ...
                console.update_in(cx, |console, window, cx| {
                    ...
                    console.move_to_end(&carrot_editor::actions::MoveToEnd, window, cx);
    ...
    pub fn watch_expression(
        ...
        self.session.update(cx, |session, cx| {
            session
                .evaluate(
                    expression.clone(),
                    Some(carrot_dap::EvaluateArgumentsContext::Repl),
    ...
    fn render_submit_menu(
        ...
        PopoverMenu::new(id.into())
            .trigger(
                carrot_ui::ButtonLike::new_rounded_right("console-confirm-split-button-right")
                ...
                this.child(Divider::horizontal()).child(
                    h_flex()
                        ...
                        .child(SplitButton::new(
                            carrot_ui::ButtonLike::new_rounded_all(ElementId::Name(
        ...
        cx.spawn(async move |_, cx| {
            ...
            let completions = matches
                ...
                .filter_map(|string_match| {
                    ...
                    Some(carrot_project::Completion {
            ...
            Ok(vec![carrot_project::CompletionResponse {
            ...
            let completions = completions
                ...
                .map(|completion| {
                    ...
                    carrot_project::Completion {
                        ...
                        snippet_deduplication_key: None,
                        confirm: None,
                        source: carrot_project::CompletionSource::Dap { sort_text },
                        insert_text_mode: None,
                    }
                })
                .collect();
...
mod tests {
    ...
    use carrot_editor::{MultiBufferOffset, test::editor_test_context::EditorTestContext};
