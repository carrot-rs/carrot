...
use carrot_actions::{ToggleEnableBreakpoint, UnsetBreakpoint};
use carrot_dap::{Capabilities, ExceptionBreakpointsFilter, adapters::DebugAdapterName};
...
impl BreakpointList {
        ...
        self.selected_ix.and_then(|ix| {
            self.breakpoints.get(ix).map(|bp| match &bp.kind {
                BreakpointEntryKind::LineBreakpoint(bp) => (
                    ...
                    bp.breakpoint.state
                        == carrot_project::debugger::breakpoint_store::BreakpointState::Enabled,
                ),
    ...
    fn render_list(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        .with_horizontal_sizing_behavior(inazuma::ListHorizontalSizingBehavior::Unconstrained)
...
#[derive(Clone, Debug)]
struct DataBreakpoint(carrot_project::debugger::session::DataBreakpointState);
    ...
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let style_for_toggle = |mode, is_enabled| {
            if is_enabled && self.strip_mode == Some(mode) && self.is_selected {
                carrot_ui::ButtonStyle::FILLED
            } else {
                carrot_ui::ButtonStyle::SUBTLE
            }
        };
