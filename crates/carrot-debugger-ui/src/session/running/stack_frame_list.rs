...
use carrot_language::PointUtf16;
use carrot_project::debugger::breakpoint_store::ActiveStackFrame;
use carrot_project::debugger::session::{Session, SessionEvent, StackFrame, ThreadStatus};
use carrot_project::{ProjectItem, ProjectPath};
use carrot_ui::{Tooltip, WithScrollbar, prelude::*};
use carrot_workspace::{Workspace, WorkspaceId};
...
pub enum StackFrameEntry {
    Normal(carrot_dap::StackFrame),
    /// Used to indicate that the frame is artificial and is a visual label or separator
    Label(carrot_dap::StackFrame),
    Collapsed(Vec<carrot_dap::StackFrame>),
}
...
impl StackFrameList {
    pub fn new(
        ...
        let list_state = ListState::new(0, inazuma::ListAlignment::Top, px(1000.));
    ...
    #[cfg(test)]
    pub(crate) fn dap_stack_frames(&self, cx: &mut App) -> Vec<carrot_dap::StackFrame> {
    ...
    pub fn build_entries(
        ...
        for stack_frame in stack_frames.iter() {
            ...
            match stack_frame.dap.presentation_hint {
                Some(carrot_dap::StackFramePresentationHint::Deemphasize)
                | Some(carrot_dap::StackFramePresentationHint::Subtle) => {
                    collapsed_entries.push(stack_frame.dap.clone());
                }
                Some(carrot_dap::StackFramePresentationHint::Label) => {
    ...
    fn go_to_stack_frame_inner(
        &mut self,
        stack_frame: carrot_dap::StackFrame,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
    ...
    pub(crate) fn abs_path_from_stack_frame(
        stack_frame: &carrot_dap::StackFrame,
    ) -> Option<Arc<Path>> {
    ...
    fn render_normal_entry(
        ...
        let should_deemphasize = matches!(
            ...
            Some(
                carrot_dap::StackFramePresentationHint::Subtle
                    | carrot_dap::StackFramePresentationHint::Deemphasize
            )
        );
        h_flex()
            .rounded_md()
...
impl Focusable for StackFrameList {
    fn focus_handle(&self, _: &inazuma::App) -> inazuma::FocusHandle {
