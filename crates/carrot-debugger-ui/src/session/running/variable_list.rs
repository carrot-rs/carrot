...
use carrot_project::debugger::{
    dap_command::DataBreakpointContext,
    session::{Session, SessionEvent, Watcher},
};
use carrot_ui::{
    ContextMenu, ListItem, ScrollAxes, ScrollableHandle, Tooltip, WithScrollbar, prelude::*,
};
use inazuma::{
    Action, AnyElement, ClickEvent, ClipboardItem, Context, DismissEvent, Empty, Entity,
    FocusHandle, Focusable, MouseDownEvent, Oklch, Point, Subscription, TextStyleRefinement,
    UniformListScrollHandle, WeakEntity, actions, anchored, deferred, uniform_list,
};
use inazuma_menu::{SelectFirst, SelectLast, SelectNext, SelectPrevious};
use inazuma_util::{debug_panic, maybe};
...
enum DapEntry {
    Watcher(Watcher),
    Variable(carrot_dap::Variable),
    Scope(carrot_dap::Scope),
}
...
impl DapEntry {
    ...
    fn as_variable(&self) -> Option<&carrot_dap::Variable> {
        match self {
    ...
    fn as_scope(&self) -> Option<&carrot_dap::Scope> {
        match self {
...
impl VariableList {
    ...
    fn build_entries(&mut self, cx: &mut Context<Self>) {
        ...
        let text_pixels = carrot_ui::TextSize::Default.pixels(cx).to_f64() as f32;
    ...
    pub fn completion_variables(&self, _cx: &mut Context<Self>) -> Vec<carrot_dap::Variable> {
        self.entries
            .iter()
                                    ...
                                    for access in access_types {
                                        menu = menu.action(
                                            format!(
                                                ...
                                                match access {
                                                    carrot_dap::DataBreakpointAccessType::Read => "Read",
                                                    carrot_dap::DataBreakpointAccessType::Write => "Write",
                                                    carrot_dap::DataBreakpointAccessType::ReadWrite =>
                                                        "Read/Write",
                                                }
                                            ),
    ...
    #[track_caller]
    #[cfg(test)]
    pub(crate) fn scopes(&self) -> Vec<carrot_dap::Scope> {
        self.entries
            .iter()
            .filter_map(|entry| match &entry.entry {
    ...
    #[track_caller]
    #[cfg(test)]
    pub(crate) fn variables_per_scope(
        &self,
    ) -> Vec<(carrot_dap::Scope, Vec<carrot_dap::Variable>)> {
        let mut scopes: Vec<(carrot_dap::Scope, Vec<_>)> = Vec::new();
        let mut idx = 0;
    ...
    #[track_caller]
    #[cfg(test)]
    pub(crate) fn variables(&self) -> Vec<carrot_dap::Variable> {
        self.entries
            .iter()
            .filter_map(|entry| match &entry.entry {
    ...
    fn create_variable_editor(default: &str, window: &mut Window, cx: &mut App) -> Entity<Editor> {
        let editor = cx.new(|cx| {
            ...
            editor.select_all(&carrot_editor::actions::SelectAll, window, cx);
            editor
        });
        ...
        div()
            ...
            .child(
                ...
                .end_slot(
                    ...
                    .icon_size(carrot_ui::IconSize::Indicator),
                ),
            )
            .into_any()
    }
        ...
        v_flex()
            ...
            .child(
                ...
                .with_horizontal_sizing_behavior(
                    inazuma::ListHorizontalSizingBehavior::Unconstrained,
                )
                .gap_1_5()
                .size_full()
                .flex_grow(),
            ...
            .custom_scrollbars(
                carrot_ui::Scrollbars::new(ScrollAxes::Both)
