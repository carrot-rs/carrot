...
use carrot_rpc::proto::ViewId;
use carrot_task::{
    BuildTaskDefinition, CarrotDebugConfig, DebugScenario, SharedTaskContext, Shell, ShellBuilder,
    SpawnInTerminal, TaskContext, substitute_variables_in_str,
};
use carrot_terminal_view::TerminalView;
use carrot_ui::{
...
use carrot_workspace::{
    ActivePaneDecorator, DraggedTab, Item, ItemHandle, Member, Pane, PaneGroup, PaneRole,
...
static PROCESS_ID_PLACEHOLDER: LazyLock<String> =
    LazyLock::new(|| carrot_task::VariableName::PickProcessId.template_value());
...
impl Item for SubView {
    ...
    fn tab_content(
        &self,
        params: carrot_workspace::item::TabContentParams,
        _: &Window,
        cx: &App,
    ) -> AnyElement {
        let label = Label::new(self.kind.to_shared_string())
            .size(carrot_ui::LabelSize::Small)
            .color(params.text_color())
            .line_height_style(carrot_ui::LineHeightStyle::UiLabel);

        if !params.selected && self.show_indicator.as_ref()(cx) {
            return h_flex()
                .justify_between()
                .child(carrot_ui::Indicator::dot())
                .gap_2()
                .child(label)
                .into_any_element();
        }
...
impl inazuma::Render for DebugTerminal {
...
impl RunningState {
    ...
    pub(crate) fn substitute_variables_in_config(
        ...
        match config {
            ...
            serde_json::Value::String(s) => {
                // Some built-in carrot tasks wrap their arguments in quotes as they might contain spaces.
    ...
    pub(crate) fn new(
        ...
        let panes = if let Some(root) = serialized_pane_layout.and_then(|serialized_layout| {
            ...
            carrot_workspace::PaneGroup::with_root(root)
        } else {
    ...
    pub(crate) fn resolve_scenario(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if config_is_valid {
                ...
                let carrot_config = CarrotDebugConfig {
                    label: label.clone(),
                ...
                let scenario = dap_registry
                    .adapter(&adapter)
                    .with_context(|| anyhow!("{}: is not a valid adapter name", &adapter))?.config_from_carrot_format(carrot_config)
                    .await?;
                ...
                anyhow::bail!("Carrot cannot determine how to run this debug scenario. `build` field was not provided and Debug Adapter won't accept provided configuration because: {e}");
            };
    ...
    fn handle_run_in_terminal(
        ...
        let kind = carrot_task::SpawnInTerminal {
            id: carrot_task::TaskId("debug".to_string()),
            ...
            reveal: carrot_task::RevealStrategy::NoFocus,
            reveal_target: carrot_task::RevealTarget::Dock,
            hide: carrot_task::HideStrategy::Never,
            shell,
            show_summary: false,
            show_command: false,
            show_rerun: false,
            save: carrot_task::SaveStrategy::default(),
        };
        ...
        let terminal_task = cx.spawn_in(window, async move |_, cx| {
            ...
            // carrot-terminal doesn't expose PIDs yet; return 0 as placeholder
            Ok(0u32)
        });
    ...
    pub(crate) fn serialize_layout(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self._schedule_serialize.is_none() {
            self._schedule_serialize = Some(cx.spawn_in(window, async move |this, cx| {
                ...
                let kvp = this
                    .read_with(cx, |_, cx| carrot_db::kvp::KeyValueStore::global(cx))
                    .ok();
                if let Some(kvp) = kvp {
    ...
    pub(crate) fn selected_stack_frame_id(&self, cx: &App) -> Option<carrot_dap::StackFrameId> {
    ...
    fn default_pane_layout(
        ...
        let group_root = carrot_workspace::PaneAxis::new(
            dock_axis.invert(),
            [leftmost_pane, center_pane, rightmost_pane]
                .into_iter()
                .map(carrot_workspace::Member::Pane)
                .collect(),
        );
