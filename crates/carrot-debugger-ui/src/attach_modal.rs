use carrot_dap::{DapRegistry, DebugRequest};
use carrot_project::Project;
use carrot_rpc::proto;
use carrot_task::CarrotDebugConfig;
use futures::channel::oneshot;
use inazuma::{AppContext, DismissEvent, Entity, EventEmitter, Focusable, Render, Task};
use inazuma::{Subscription, WeakEntity};
...
use carrot_ui::{Context, Tooltip, prelude::*};
use carrot_ui::{ListItem, ListItemSpacing};
...
pub(crate) enum ModalIntent {
    ...
    AttachToProcess(CarrotDebugConfig),
}
    ...
    fn confirm(&mut self, _secondary: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        match &mut self.intent {
            ...
            ModalIntent::AttachToProcess(definition) => {
                ...
                cx.spawn_in(window, async move |this, cx| {
                    let Ok(scenario) = adapter.config_from_carrot_format(definition).await else {
                        return;
                    };
