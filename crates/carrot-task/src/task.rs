//! Baseline interface of Tasks in Carrot: all tasks in Carrot are intended to use those for implementing their own logic.
...
pub use carrot_actions::RevealTarget;
pub use debug_format::{
    AttachRequest, BuildTaskDefinition, CarrotDebugConfig, DebugRequest, DebugScenario,
    DebugTaskFile, LaunchRequest, Request, TcpArgumentsTemplate,
};
pub use inazuma_util::shell::{Shell, ShellKind};
pub use inazuma_util::shell_builder::ShellBuilder;
...
/// Contains all information needed by Carrot to spawn a new terminal tab for the given task.
...
impl SpawnInTerminal {
    pub fn to_proto(&self) -> carrot_proto::SpawnInTerminal {
        carrot_proto::SpawnInTerminal {
    ...
    pub fn from_proto(proto: carrot_proto::SpawnInTerminal) -> Self {
        Self {
...
/// Variables, available for use in [`TaskContext`] when a Carrot's [`TaskTemplate`] gets resolved into a [`ResolvedTask`].
...
impl FromStr for VariableName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let without_prefix = s.strip_prefix(CARROT_VARIABLE_NAME_PREFIX).ok_or(())?;
        let value = match without_prefix {
            ...
            _ => {
                if let Some(custom_name) =
                    without_prefix.strip_prefix(CARROT_CUSTOM_VARIABLE_NAME_PREFIX)
                {
...
pub const CARROT_VARIABLE_NAME_PREFIX: &str = "CARROT_";
const CARROT_CUSTOM_VARIABLE_NAME_PREFIX: &str = "CUSTOM_";

impl std::fmt::Display for VariableName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            Self::File => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}FILE"),
            Self::Filename => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}FILENAME"),
            Self::RelativeFile => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}RELATIVE_FILE"),
            Self::RelativeDir => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}RELATIVE_DIR"),
            Self::Dirname => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}DIRNAME"),
            Self::Stem => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}STEM"),
            Self::WorktreeRoot => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}WORKTREE_ROOT"),
            Self::Symbol => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}SYMBOL"),
            Self::Row => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}ROW"),
            Self::Column => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}COLUMN"),
            Self::SelectedText => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}SELECTED_TEXT"),
            Self::Language => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}LANGUAGE"),
            Self::RunnableSymbol => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}RUNNABLE_SYMBOL"),
            Self::PickProcessId => write!(f, "{CARROT_VARIABLE_NAME_PREFIX}PICK_PID"),
            Self::Custom(s) => write!(
                f,
                "{CARROT_VARIABLE_NAME_PREFIX}{CARROT_CUSTOM_VARIABLE_NAME_PREFIX}{s}"
            ),
        }
    }
}

/// Container for predefined environment variables that describe state of Carrot at the time the task was spawned.
...
/// Keeps all Carrot-related state inside, used to produce a resolved task out of its template.
...
pub struct TaskContext {
    ...
    /// Environment variables obtained when loading the project into Carrot.
...
pub fn shell_from_proto(proto: carrot_proto::Shell) -> anyhow::Result<Shell> {
    let shell_type = proto.shell_type.context("invalid shell type")?;
    let shell = match shell_type {
        carrot_proto::shell::ShellType::System(_) => Shell::System,
        carrot_proto::shell::ShellType::Program(program) => Shell::Program(program),
        carrot_proto::shell::ShellType::WithArguments(program) => Shell::WithArguments {
...
pub fn shell_to_proto(shell: Shell) -> carrot_proto::Shell {
    let shell_type = match shell {
        Shell::System => carrot_proto::shell::ShellType::System(carrot_proto::System {}),
        Shell::Program(program) => carrot_proto::shell::ShellType::Program(program),
        Shell::WithArguments {
            program,
            args,
            title_override: _,
        } => carrot_proto::shell::ShellType::WithArguments(carrot_proto::shell::WithArguments {
            program,
            args,
        }),
    };
