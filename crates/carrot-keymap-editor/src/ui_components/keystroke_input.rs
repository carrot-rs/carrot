...
impl KeystrokeInput {
    ...
    fn determine_stop_recording_binding(window: &mut Window) -> Option<inazuma::KeyBinding> {
        if cfg!(test) {
            Some(inazuma::KeyBinding::new(
    ...
    fn on_inner_focus_in(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        if self.intercept_subscription.is_none() {
            let listener = cx.listener(|this, event: &inazuma::KeystrokeEvent, window, cx| {
    ...
    fn render_keystrokes(&self, is_recording: bool) -> impl Iterator<Item = Div> {
        ...
        keystrokes.iter().map(move |keystroke| {
            h_flex().children(carrot_ui::render_keybinding_keystroke(
                keystroke,
                Some(Color::Default),
                Some(rems(0.875).into()),
                carrot_ui::PlatformStyle::platform(),
                false,
            ))
        })
    }
            ...
            Icon::new(IconName::Circle)
                ...
                .with_animation(
                    ...
                    Animation::new(std::time::Duration::from_secs(2))
                        .repeat()
                        .with_easing(inazuma::pulsating_between(0.4, 0.8)),
                    {
    ...
    impl KeystrokeInputTestHelper {
        ...
        pub fn send_modifiers(&mut self, modifiers: &str) -> &mut Self {
            ...
            let event = ModifiersChangedEvent {
                ...
                capslock: inazuma::Capslock::default(),
            };
