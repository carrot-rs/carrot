...
use carrot_editor::{CompletionProvider, Editor, EditorEvent, EditorMode, SizingBehavior};
use carrot_fs::Fs;
use carrot_language::{Language, LanguageConfig, ToOffset as _};
use inazuma::{
...
use carrot_json_compat::infer_json_indent_size;
use carrot_notifications::status_toast::{StatusToast, ToastIcon};
use carrot_project::{CompletionDisplayOptions, Project};
use carrot_ui::{
    ActiveTheme as _, App, Banner, BorrowAppContext, ContextMenu, DataTable, IconButtonShape,
    IconPosition, Indicator, Modal, ModalFooter, ModalHeader, ParentElement as _, PopoverMenu,
    Render, Section, SharedString, Styled as _, TableColumnWidths, TableInteractionState,
    TableResizeBehavior, Tooltip, Window, prelude::*,
};
use carrot_ui_input::InputField;
use carrot_workspace::{
    Item, ModalView, PaneRole, SerializableItem, Workspace, notifications::NotifyTaskExt as _,
...
use carrot_actions::{ChangeKeybinding, OpenKeymap};
...
type ConflictKeybindMapping = HashMap<
    ...
    Vec<(
        Option<inazuma::KeyBindingContextPredicate>,
        Vec<ConflictOrigin>,
    )>,
>;
    ...
    fn new(key_bindings: &[ProcessedBinding]) -> Self {
            ...
            .flat_map(|(index, binding)| Some(index).zip(binding.keybind_information()))
        {
            ...
            let predicate = mapping
                .context
                .and_then(|ctx| inazuma::KeyBindingContextPredicate::parse(&ctx).ok());
...
struct KeymapEditor {
    ...
    action_args_temp_dir_worktree: Option<Entity<carrot_project::Worktree>>,
...
fn disabled_binding_matches_context(
    disabled_binding: &inazuma::KeyBinding,
    binding: &inazuma::KeyBinding,
) -> bool {
    match (
...
impl KeymapEditor {
    fn new(workspace: WeakEntity<Workspace>, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let table_interaction_state = cx.new(|cx| {
            TableInteractionState::new(cx).with_custom_scrollbar(
                carrot_ui::Scrollbars::for_settings::<carrot_editor::EditorSettings>(),
            )
        });
        ...
        cx.spawn({
            ...
            async move |this, cx| {
                let temp_dir = tempfile::tempdir_in(carrot_paths::temp_dir())?;
    ...
    async fn update_matches(
        ...
        let action_query = carrot_command_palette::normalize_action_query(&action_query);
    ...
    fn process_bindings(
        ...
        carrot_keybind_context_language: Arc<Language>,
        humanized_action_names: &HumanizedActionNameCache,
        cx: &mut App,
    ) -> (
        ...
        for (binding_index, &key_binding) in key_bindings.iter().enumerate() {
            if inazuma::is_unbind(key_binding.action()) {
                continue;
            }
            ...
            let keystroke_text =
                carrot_ui::text_for_keybinding_keystrokes(key_binding.keystrokes(), cx);
            ...
            let context = key_binding
                ...
                .map(|predicate| {
                    KeybindContextString::Local(
                        ...
                        carrot_keybind_context_language.clone(),
                    )
                })
    ...
    fn on_keymap_changed(&mut self, window: &mut Window, cx: &mut Context<KeymapEditor>) {
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let carrot_keybind_context_language =
            ...
            let (action_query, keystroke_query) = this.update(cx, |this, cx| {
                let (key_bindings, string_match_candidates, actions_with_schemas) =
                    Self::process_bindings(
                        json_language,
                        carrot_keybind_context_language,
    ...
    fn copy_context_to_clipboard(
        ...
        carrot_telemetry::event!("Keybinding Context Copied", context = context);
        cx.write_to_clipboard(inazuma::ClipboardItem::new_string(context));
    }
    ...
    fn copy_action_to_clipboard(
        ...
        carrot_telemetry::event!("Keybinding Action Copied", action = action);
        cx.write_to_clipboard(inazuma::ClipboardItem::new_string(action));
    }
    ...
    fn toggle_carrot_defaults_filter(&mut self, cx: &mut Context<Self>) {
        self.source_filters.carrot_defaults = !self.source_filters.carrot_defaults;
        self.on_query_changed(cx);
    }
    ...
    fn render_filter_dropdown(
        ...
        return PopoverMenu::new("keymap-editor-filter-menu")
                ...
                Some(ContextMenu::build_persistent(window, cx, {
                    ...
                    move |mut menu, _window, cx| {
                        ...
                        menu = menu
                            ...
                            .map(add_filter(
                                "Default",
                                source_filters.carrot_defaults,
                                None,
                                &focus_handle,
                                &keymap_editor,
                                Some(|editor, cx| {
                                    editor.toggle_carrot_defaults_filter(cx);
                                }),
                            ))
                            .map(add_filter(
                                "Vim",
            ...
            .anchor(inazuma::Corner::TopRight)
...
impl HumanizedActionNameCache {
    fn new(cx: &App) -> Self {
        let cache = HashMap::from_iter(cx.all_action_names().iter().map(|&action_name| {
            (
                action_name,
                carrot_command_palette::humanize_action_name(action_name).into(),
            )
        }));
        Self { cache }
    }
...
impl KeyBinding {
    fn new(binding: &inazuma::KeyBinding, source: KeybindSource) -> Self {
        Self {
...
fn muted_styled_text(text: SharedString, cx: &App) -> StyledText {
    ...
    StyledText::new(text).with_highlights([(
        0..len,
        inazuma::HighlightStyle::color(cx.theme().colors().text_muted),
    )])
}
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "Edit in JSON")
                                            ...
                                            .key_binding(
                                                carrot_ui::KeyBinding::for_action_in(&carrot_actions::OpenKeymapFile, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10.))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                                                    carrot_actions::OpenKeymapFile.boxed_clone(),
                                                    cx,
                                                );
                                            })
                                    )
                                    .child(
                                        Button::new("create", "Create Keybinding")
                                            ...
                                            .key_binding(
                                                carrot_ui::KeyBinding::for_action_in(&OpenCreateKeybindingModal, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10.))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                            ...
                            range
                                .filter_map(|index| {
                                    ...
                                    let action = div()
                                        ...
                                        .child({
                                            if action_name != inazuma::NoAction.name() {
                                                binding
                                                    .action()
                                                    .humanized_name
                                                    .clone()
                                    ...
                                    let keystrokes = binding.key_binding().map_or(
                                        binding
                                            ...
                                            .unwrap_or_default()
                                            .into_any_element(),
                                        |binding| carrot_ui::KeyBinding::from_keystrokes(binding.keystrokes.clone(), binding.source).into_any_element()
                                    );
                                    ...
                                    row.id(row_id.clone())
                                        .when(!is_unbound_by_unbind, |row| {
                                            row.on_any_mouse_down(cx.listener(
                                                move |this,
                                                      mouse_down_event: &inazuma::MouseDownEvent,
                                                      window,
                                                      cx| {
...
impl KeybindingEditorModal {
    pub fn new(
        ...
        let has_action_editor = create && editing_keybind.action().name == inazuma::NoAction.name();
        ...
        let (action_editor, action_name_to_static) = if has_action_editor {
            ...
            let humanized_names: HashMap<&'static str, SharedString> = actions
                ...
                .map(|&name| {
                    (
                        name,
                        carrot_command_palette::humanize_action_name(name).into(),
                    )
                })
                .collect();
    ...
    fn validate_context(&self, cx: &App) -> anyhow::Result<Option<String>> {
        ...
        inazuma::KeyBindingContextPredicate::parse(&context)
...
impl ActionArgumentsEditor {
    fn new(
        ...
        cx.spawn_in(window, async move |this, cx| {
            let result = async {
                ...
                let file_name = carrot_json_schema_store::normalized_action_file_name(action_name);
                ...
                let editor = cx.new_window_entity(|window, cx| {
                    let multi_buffer =
                        cx.new(|cx| carrot_editor::MultiBuffer::singleton(buffer, cx));
    ...
    async fn create_temp_buffer(
        ...
        fs: Arc<dyn Fs>,
        cx: &mut AsyncApp,
    ) -> anyhow::Result<(Entity<carrot_language::Buffer>, Option<tempfile::TempDir>)> {
        let (temp_file_path, temp_dir) = {
            ...
            async move {
                let temp_dir_backup = match temp_dir.as_ref() {
                    Some(_) => None,
                    None => {
                        let temp_dir = carrot_paths::temp_dir();
...
impl Render for ActionArgumentsEditor {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let settings = carrot_theme_settings::ThemeSettings::get_global(cx);
...
impl CompletionProvider for KeyContextCompletionProvider {
    fn completions(
        &self,
        _excerpt_id: carrot_editor::ExcerptId,
        buffer: &Entity<carrot_language::Buffer>,
        buffer_position: carrot_language::Anchor,
        _trigger: carrot_editor::CompletionContext,
        _window: &mut Window,
        cx: &mut Context<Editor>,
    ) -> inazuma::Task<anyhow::Result<Vec<carrot_project::CompletionResponse>>> {
        ...
        inazuma::Task::ready(Ok(vec![carrot_project::CompletionResponse {
            completions: self
                .contexts
                .iter()
                .map(|context| carrot_project::Completion {
                    replace_range: replace_range.clone(),
                    label: carrot_language::CodeLabel::plain(context.to_string(), None),
                    new_text: context.to_string(),
                    documentation: None,
                    source: carrot_project::CompletionSource::Custom,
                    icon_path: None,
                    match_start: None,
    ...
    fn is_completion_trigger(
        &self,
        _buffer: &Entity<carrot_language::Buffer>,
        _position: carrot_language::Anchor,
        text: &str,
...
async fn load_keybind_context_language(
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("Carrot Keybind Context")
        })
        .context("Failed to load Carrot Keybind Context language")
        .log_err();
    ...
    language.unwrap_or_else(|| {
        Arc::new(Language::new(
            LanguageConfig {
                name: "Carrot Keybind Context".into(),
                ..Default::default()
            },
...
async fn save_keybinding_update(
    ...
    let keymap_contents = inazuma_settings_framework::KeymapFile::load_keymap_file(fs)
        .await
    ...
    let target = inazuma_settings_framework::KeybindUpdateTarget {
    ...
    let source = inazuma_settings_framework::KeybindUpdateTarget {
    ...
    let operation = if !create {
        inazuma_settings_framework::KeybindUpdateOperation::Replace {
            target,
            target_keybind_source: existing.keybind_source().unwrap_or(KeybindSource::User),
            source,
        }
    } else {
        inazuma_settings_framework::KeybindUpdateOperation::Add {
            source,
            from: Some(target),
        }
    };
    ...
    let updated_keymap_contents = inazuma_settings_framework::KeymapFile::update_keybinding(
        operation,
        keymap_contents,
        tab_size,
        keyboard_mapper,
    ...
    fs.write(
        carrot_paths::keymap_file().as_path(),
...
async fn remove_keybinding(
    ...
    let operation = inazuma_settings_framework::KeybindUpdateOperation::Remove {
        target: inazuma_settings_framework::KeybindUpdateTarget {
...
fn collect_contexts_from_assets() -> Vec<SharedString> {
    let mut keymap_assets = vec![
        inazuma_util::asset_str::<SettingsAssets>(inazuma_settings_framework::DEFAULT_KEYMAP_PATH),
        inazuma_util::asset_str::<SettingsAssets>(inazuma_settings_framework::VIM_KEYMAP_PATH),
    ];
    keymap_assets.extend(
        BaseKeymap::OPTIONS
            .iter()
            .filter_map(|(_, base_keymap)| base_keymap.asset_path())
            .map(inazuma_util::asset_str::<SettingsAssets>),
    );
    ...
    for keymap_asset in keymap_assets {
        ...
        for section in keymap.sections() {
            ...
            let Ok(root_context) = inazuma::KeyBindingContextPredicate::parse(context_expr) else {
                continue;
            };
...
fn normalized_ctx_eq(
    a: &inazuma::KeyBindingContextPredicate,
    b: &inazuma::KeyBindingContextPredicate,
) -> bool {
    use inazuma::KeyBindingContextPredicate::*;
    return match (a, b) {
    ...
    fn flatten_and<'a>(
        pred: &'a inazuma::KeyBindingContextPredicate,
        operands: &mut Vec<&'a inazuma::KeyBindingContextPredicate>,
    ) {
        use inazuma::KeyBindingContextPredicate::*;
        match pred {
            And(left, right) => {
...
mod persistence {
    use carrot_db::{query, sqlez::domain::Domain, sqlez_macros::sql};
    use carrot_workspace::WorkspaceDb;

    pub struct KeybindingEditorDb(carrot_db::sqlez::thread_safe_connection::ThreadSafeConnection);
    ...
    carrot_db::static_connection!(KeybindingEditorDb, [WorkspaceDb]);
...
mod tests {
    ...
    fn normalized_ctx_cmp() {
        ...
        fn cmp(a: &str, b: &str) -> bool {
            let a = inazuma::KeyBindingContextPredicate::parse(a)
                .expect("Failed to parse keybinding context a");
            let b = inazuma::KeyBindingContextPredicate::parse(b)
    ...
    fn binding_is_unbound_by_unbind_respects_precedence() {
        let binding = inazuma::KeyBinding::new("tab", carrot_actions::OpenKeymap, None);
        let unbind =
