use carrot_acp_thread::{MentionUri, selection_name};
use carrot_agent::{ThreadStore, outline};
use agent_client_protocol as acp;
use carrot_agent_servers::{AgentServer, AgentServerDelegate};
use anyhow::{Context as _, Result, anyhow};
use carrot_assistant_slash_commands::{codeblock_fence_for_path, collect_diagnostics_output};
...
use carrot_http_client::{AsyncBody, HttpClientWithUrl};
use itertools::Either;
use carrot_language::Buffer;
use carrot_language_model::LanguageModelImage;
use carrot_multi_buffer::MultiBufferRow;
use postage::stream::Stream as _;
use carrot_project::{Project, ProjectItem, ProjectPath, Worktree};
...
use carrot_ui::{Disclosure, Toggleable, prelude::*};
use inazuma_util::{ResultExt, debug_panic, rel_path::RelPath};
use carrot_workspace::{Workspace, notifications::NotifyResultExt as _};
...
impl MentionSet {
    ...
    fn confirm_mention_for_thread(
        ...
        let server = Rc::new(carrot_agent::NativeAgentServer::new(
        ...
        cx.spawn(async move |_, cx| {
            ...
            let agent = agent.downcast::<carrot_agent::NativeAgentConnection>().unwrap();
            let summary = agent
                .0
                .update(cx, |agent, cx| {
    ...
    fn confirm_mention_for_diagnostics(
        ...
        let diagnostics_task = collect_diagnostics_output(
            project,
            carrot_assistant_slash_commands::Options {
                include_errors,
                include_warnings,
                path_matcher: None,
            },
    ...
    pub fn confirm_mention_for_git_diff(
        ...
        let diff_receiver = repo.update(cx, |repo, cx| {
            repo.diff(
                carrot_git::repository::DiffType::MergeBase { base_ref: base_ref },
                cx,
            )
        });
...
pub(crate) async fn insert_images_as_context(
    images: Vec<(inazuma::Image, SharedString)>,
    editor: Entity<Editor>,
...
pub(crate) fn paste_images_as_context(
    ...
    Some(window.spawn(cx, async move |mut cx| {
        ...
        let (mut images, paths): (Vec<(inazuma::Image, SharedString)>, Vec<_>) = clipboard
            .into_entries()
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        if let Some(image) = self.image.clone() {
            div()
                .p_1p5()
                .elevation_2(cx)
                .child(inazuma::img(image).h_auto().max_w_96().rounded_sm())
                .into_any_element()
        } else {
...
async fn fetch_url_content(http_client: Arc<HttpClientWithUrl>, url: String) -> Result<String> {
    ...
    use carrot_html_to_markdown::{TagHandler, convert_html_to_markdown, markdown};
    ...
    match content_type {
        ContentType::Html => {
            let mut handlers: Vec<TagHandler> = vec![
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::WebpageChromeRemover)),
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::ParagraphHandler)),
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::HeadingHandler)),
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::ListHandler)),
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::TableHandler::new())),
                Rc::new(RefCell::new(carrot_html_to_markdown::markdown::StyledTextHandler)),
            ];
            if url.contains("wikipedia.org") {
                use carrot_html_to_markdown::structure::wikipedia;
                ...
                handlers.push(Rc::new(RefCell::new(carrot_html_to_markdown::markdown::CodeHandler)));
            }
