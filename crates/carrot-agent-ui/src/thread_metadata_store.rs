...
use carrot_acp_thread::AgentSessionInfo;
use carrot_agent::{ThreadStore, ZED_AGENT_ID};
...
use carrot_feature_flags::{AgentV2FeatureFlag, FeatureFlagAppExt};
use futures::{FutureExt as _, future::Shared};
use inazuma::{AppContext as _, Entity, Global, Subscription, Task};
use carrot_project::AgentId;
use carrot_ui::{App, Context, SharedString};
...
pub struct ThreadMetadata {
    ...
    /// `None` for native Carrot threads, `Some("claude-code")` etc. for ACP agents.
...
impl ThreadMetadata {
    ...
    pub fn from_thread(thread: &Entity<carrot_acp_thread::AcpThread>, cx: &App) -> Self {
...
impl SidebarThreadMetadataStore {
    ...
    pub fn init_global(cx: &mut App) {
        ...
        let db = smol::block_on(carrot_db::open_test_db::<ThreadMetadataDb>(&db_name));
    ...
    fn new(db: ThreadMetadataDb, cx: &mut Context<Self>) -> Self {
        ...
        cx.observe_new::<carrot_acp_thread::AcpThread>(move |thread, _window, cx| {
    ...
    fn handle_thread_update(
        &mut self,
        thread: Entity<carrot_acp_thread::AcpThread>,
        event: &carrot_acp_thread::AcpThreadEvent,
        cx: &mut Context<Self>,
    ) {
        // Don't track subagent threads in the sidebar.
        ...
        match event {
            carrot_acp_thread::AcpThreadEvent::NewEntry
            | carrot_acp_thread::AcpThreadEvent::TitleUpdated
            | carrot_acp_thread::AcpThreadEvent::EntryUpdated(_)
            | carrot_acp_thread::AcpThreadEvent::EntriesRemoved(_)
            | carrot_acp_thread::AcpThreadEvent::ToolAuthorizationRequested(_)
            | carrot_acp_thread::AcpThreadEvent::ToolAuthorizationReceived(_)
            | carrot_acp_thread::AcpThreadEvent::Retry(_)
            | carrot_acp_thread::AcpThreadEvent::Stopped(_)
            | carrot_acp_thread::AcpThreadEvent::Error
            | carrot_acp_thread::AcpThreadEvent::LoadError(_)
            | carrot_acp_thread::AcpThreadEvent::Refusal => {
...
carrot_db::static_connection!(ThreadMetadataDb, []);
...
mod tests {
    use super::*;
    use carrot_acp_thread::{AgentConnection, StubAgentConnection};
    use carrot_action_log::ActionLog;
    use carrot_agent::DbThread;
    use agent_client_protocol as acp;
    ...
    async fn test_store_initializes_cache_from_database(cx: &mut TestAppContext) {
        ...
        let test_name = thread.name().unwrap_or("unknown_test");
        let db_name = format!("THREAD_METADATA_DB_{}", test_name);
        let db = ThreadMetadataDb(smol::block_on(carrot_db::open_test_db::<ThreadMetadataDb>(
            &db_name,
        )));
    ...
    async fn test_subagent_threads_excluded_from_sidebar_metadata(cx: &mut TestAppContext) {
        ...
        let subagent_thread = cx.update(|cx| {
            ...
            cx.new(|cx| {
                carrot_acp_thread::AcpThread::new(
