...
use carrot_editor::{Anchor, AnchorRangeExt, MultiBuffer, MultiBufferSnapshot, ToOffset as _, ToPoint};
use futures::{
...
use inazuma::{App, AppContext as _, AsyncApp, Context, Entity, EventEmitter, Subscription, Task};
use carrot_language::{Buffer, IndentKind, LanguageName, Point, TransactionId, line_diff};
...
use carrot_streaming_diff::{CharOperation, LineDiff, LineOperation, StreamingDiff};
use uuid::Uuid;
...
impl CodegenAlternative {
    ...
    fn handle_buffer_event(
        ...
        event: &carrot_multi_buffer::Event,
        cx: &mut Context<Self>,
    ) {
        if let carrot_multi_buffer::Event::TransactionUndone { transaction_id } = event
    ...
    fn build_request_tools(
        ...
        let language_name = if let Some(language) = language.as_ref() {
            if Arc::ptr_eq(language, &carrot_language::PLAIN_TEXT) {
                None
            } else {
                Some(language.name())
            }
        ...
        Ok(cx.spawn(async move |_cx| {
            ...
            let tools = vec![
                LanguageModelRequestTool {
                    ...
                    input_schema: carrot_language_model::tool_schema::root_schema_for::<RewriteSectionInput>(tool_input_format).to_value(),
                ...
                LanguageModelRequestTool {
                    ...
                    input_schema: carrot_language_model::tool_schema::root_schema_for::<FailureMessageInput>(tool_input_format).to_value(),
                    use_input_streaming: false,
                },
            ];
    ...
    pub fn handle_stream(
        ...
        let anthropic_reporter = carrot_language_model::AnthropicEventReporter::new(&model, cx);
        ...
        // as in https://github.com/carrot-rs/carrot/issues/39088
            ...
            let generate = async {
                ...
                let line_based_stream_diff: Task<anyhow::Result<()>> = cx.background_spawn({
                    ...
                    async move {
                        ...
                        anthropic_reporter.report(carrot_language_model::AnthropicEventData {
                            completion_type: carrot_language_model::AnthropicCompletionType::Editor,
                            event: carrot_language_model::AnthropicEventType::Response,
...
mod tests {
    ...
    async fn test_allows_model_to_output_backticks(cx: &mut TestAppContext) {
        init_test(cx);
        let text = "- Improved; `cmd+click` behavior. Now requires `cmd` to be pressed before the click starts or it doesn't run. ([#44579](https://github.com/carrot-rs/carrot/pull/44579); thanks [Zachiah](https://github.com/Zachiah))";
