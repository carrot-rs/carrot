...
use carrot_agent::ContextServerRegistry;
use anyhow::Result;
use carrot_cloud_api_types::Plan;
use inazuma_collections::HashMap;
use carrot_context_server::ContextServerId;
use carrot_editor::{Editor, MultiBufferOffset, SelectionEffects, scroll::Autoscroll};
...
use carrot_language_models::AllLanguageModelSettings;
...
use carrot_workspace::{Workspace, create_and_open_local_file};
use carrot_actions::{ExtensionCategoryFilter, OpenBrowser};
...
impl AgentConfiguration {
    pub fn new(
        ...
        let subscriptions = vec![
            cx.subscribe_in(
                ...
                window,
                |this, _, event: &carrot_language_model::Event, window, cx| match event {
                    carrot_language_model::Event::AddedProvider(provider_id) => {
                        let provider = LanguageModelRegistry::read_global(cx).provider(provider_id);
                        if let Some(provider) = provider {
                            this.add_provider_configuration_view(&provider, window, cx);
                        }
                    }
                    carrot_language_model::Event::RemovedProvider(provider_id) => {
    ...
    fn add_provider_configuration_view(
        ...
        let configuration_view = provider.configuration_view(
            carrot_language_model::ConfigurationViewTargetAgent::CarrotAgent,
            window,
            cx,
        );
...
impl AgentConfiguration {
    ...
    fn render_provider_configuration_block(
        ...
        let is_carrot_provider = provider.id() == ZED_CLOUD_PROVIDER_ID;
        let current_plan = if is_carrot_provider {
            self.workspace
                .upgrade()
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        h_flex()
                                            ...
                                            .map(|this| {
                                                if is_carrot_provider && is_signed_in {
                                                    this.child(
                                                        self.render_carrot_plan_info(current_plan, cx),
                                                    )
                                                } else {
                                                    this.when(
    ...
    fn render_provider_configuration_section(
        ...
        v_flex()
            ...
            .child(self.render_section_title(
                "LLM Providers",
                "Add at least one provider to use AI-powered features with Carrot's native agent.",
    ...
    fn render_carrot_plan_info(&self, plan: Option<Plan>, cx: &mut Context<Self>) -> impl IntoElement {
        if let Some(plan) = plan {
            ...
            let (plan_name, label_color, bg_color) = match plan {
                Plan::CarrotFree => ("Free", Color::Default, free_chip_bg),
                Plan::CarrotProTrial => ("Pro Trial", Color::Accent, pro_chip_bg),
                Plan::CarrotPro => ("Pro", Color::Accent, pro_chip_bg),
                Plan::CarrotBusiness => ("Business", Color::Accent, pro_chip_bg),
                Plan::CarrotStudent => ("Student", Color::Accent, pro_chip_bg),
            };
    ...
    fn render_context_servers_section(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(self.render_section_title(
                ...
                "All MCP servers connected directly or via a Carrot extension.",
        ...
        .then(|| {
            ...
            .on_click(cx.listener({
                ...
                move |this, _, _window, cx| {
                    let server: Rc<dyn carrot_agent_servers::AgentServer> =
                        Rc::new(carrot_agent_servers::CustomAgentServer::new(agent.id()));
