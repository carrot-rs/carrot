...
use carrot_acp_thread::{AcpThread, MentionUri, ThreadStatus};
use carrot_agent::{ContextServerRegistry, SharedThread, ThreadStore};
use agent_client_protocol as acp;
use carrot_agent_servers::AgentServer;
use inazuma_collections::HashSet;
use carrot_db::kvp::{Dismissable, KeyValueStore};
use itertools::Itertools;
use carrot_project::AgentId;
use serde::{Deserialize, Serialize};
use inazuma_settings_framework::{LanguageModelProviderSetting, LanguageModelSelection};

use carrot_feature_flags::{AgentV2FeatureFlag, FeatureFlagAppExt as _};
use carrot_actions::agent::{
...
use carrot_agent_settings::AgentSettings;
use carrot_ai_onboarding::AgentPanelOnboarding;
use anyhow::{Context as _, Result, anyhow};
use carrot_assistant_slash_command::SlashCommandWorkingSet;
use carrot_assistant_text_thread::{TextThread, TextThreadEvent, TextThreadSummary};
use carrot_client::UserStore;
use carrot_cloud_api_types::Plan;
use inazuma_collections::HashMap;
use carrot_editor::{Anchor, AnchorRangeExt as _, Editor, EditorEvent, MultiBuffer};
use carrot_extension::ExtensionEvents;
...
use carrot_language_model::{ConfigurationError, LanguageModelRegistry};
use carrot_project::project_settings::ProjectSettings;
use carrot_project::{Project, ProjectPath, Worktree};
use carrot_prompt_store::{PromptBuilder, PromptStore, UserPromptId};
use carrot_rules_library::{RulesLibrary, open_rules_library};
use carrot_search::{BufferSearchBar, buffer_search};
        ...
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace
                ...
                .register_action(|_workspace, _: &ResetOnboarding, window, cx| {
                    window.dispatch_action(carrot_workspace::RestoreBanner.boxed_clone(), cx);
                    window.refresh();
                })
...
impl AgentType {
    ...
    fn label(&self) -> SharedString {
        match self {
            Self::NativeAgent | Self::TextThread => "Carrot Agent".into(),
...
pub struct AgentPanel {
    ...
    text_thread_store: Entity<carrot_assistant_text_thread::TextThreadStore>,
...
impl AgentPanel {
        ...
        cx.spawn(async move |cx| {
            ...
            let text_thread_store = workspace
                .update(cx, |workspace, cx| {
                    ...
                    carrot_assistant_text_thread::TextThreadStore::new(
                        project,
                        prompt_builder,
                        slash_commands,
                        cx,
        ...
        let extension_subscription = if let Some(extension_events) = ExtensionEvents::try_global(cx)
        {
            Some(
                cx.subscribe(&extension_events, |this, _source, event, cx| match event {
                    carrot_extension::Event::ExtensionInstalled(_)
                    | carrot_extension::Event::ExtensionUninstalled(_)
                    | carrot_extension::Event::ExtensionsInstalledChanged => {
    ...
    fn new_text_thread(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        carrot_telemetry::event!("Agent Thread Started", agent = "carrot-text");
    ...
    pub fn go_back(&mut self, _: &carrot_workspace::GoBack, window: &mut Window, cx: &mut Context<Self>) {
                ...
                if let Some(workspace) = workspace.upgrade() {
                    workspace.update(cx, |workspace, cx| {
                        ...
                        workspace.show_toast(
                            carrot_workspace::Toast::new(
                                carrot_workspace::notifications::NotificationId::unique::<ThreadCopiedToast>(),
    ...
    fn show_deferred_toast(
        workspace: &WeakEntity<carrot_workspace::Workspace>,
        message: &'static str,
        cx: &mut App,
    ) {
        ...
        cx.defer(move |cx| {
            if let Some(workspace) = workspace.upgrade() {
                workspace.update(cx, |workspace, cx| {
                    ...
                    workspace.show_toast(
                        carrot_workspace::Toast::new(
                            carrot_workspace::notifications::NotificationId::unique::<ClipboardToast>(),
                            message,
                        )
                        .autohide(),
                        cx,
            ...
            this.update_in(cx, |_, _window, cx| {
                if let Some(workspace) = workspace.upgrade() {
                    workspace.update(cx, |workspace, cx| {
                        ...
                        workspace.show_toast(
                            carrot_workspace::Toast::new(
                                carrot_workspace::notifications::NotificationId::unique::<ThreadLoadedToast>(),
    ...
    pub(crate) fn active_native_agent_thread(&self, cx: &App) -> Option<Entity<carrot_agent::Thread>> {
        match &self.active_view {
            ActiveView::AgentThread {
                conversation_view, ..
    ...
    fn reset_start_thread_in_to_default(&mut self, cx: &mut Context<Self>) {
        use inazuma_settings_framework::{NewThreadLocation, Settings};
        ...
        if let Some(extension_store) = ExtensionStore::try_global(cx) {
            let (manifests, extensions_dir) = {
                ...
                let manifests: Vec<_> = installed
                    ...
                    .collect();
                let extensions_dir = carrot_paths::extensions_dir().join("installed");
    ...
    pub(crate) fn create_agent_thread(
        ...
        let thread_store = server
            .clone()
            .downcast::<carrot_agent::NativeAgentServer>()
            .is_some()
    ...
    fn classify_worktrees(
        &self,
        cx: &App,
    ) -> (Vec<Entity<carrot_project::git_store::Repository>>, Vec<PathBuf>) {
        let project = &self.project;
        let repositories = project.read(cx).repositories(cx).clone();
        let mut git_repos: Vec<Entity<carrot_project::git_store::Repository>> = Vec::new();
    ...
    fn start_worktree_creations(
        git_repos: &[Entity<carrot_project::git_store::Repository>],
        branch_name: &str,
        worktree_directory_setting: &str,
        cx: &mut Context<Self>,
    ) -> Result<(
        Vec<(
            Entity<carrot_project::git_store::Repository>,
            PathBuf,
    ...
    async fn await_and_rollback_on_failure(
        ...
        let mut repos_and_paths: Vec<(Entity<carrot_project::git_store::Repository>, PathBuf)> =
            Vec::new();
    ...
    async fn setup_new_workspace(
        ...
        app_state: Arc<carrot_workspace::AppState>,
        window_handle: Option<inazuma::WindowHandle<carrot_shell::AppShell>>,
        ...
        new_window_handle.update(cx, |_app_shell, window, cx| {
            new_workspace.update(cx, |workspace, cx| {
                if has_non_git {
                    let toast_id = carrot_workspace::notifications::NotificationId::unique::<AgentPanel>();
                    workspace.show_toast(
                        carrot_workspace::Toast::new(
                            toast_id,
                ...
                if let Some(path) = remapped_active_path {
                    let open_task = workspace.open_paths(
                        vec![path],
                        carrot_workspace::OpenOptions::default(),
                        None,
                        window,
                        cx,
                    );
...
impl Panel for AgentPanel {
    ...
    fn remote_id() -> Option<carrot_proto::PanelId> {
        Some(carrot_proto::PanelId::AssistantPanel)
    }

    fn icon(&self, _window: &Window, cx: &App) -> Option<IconName> {
        (self.enabled(cx) && AgentSettings::get_global(cx).button).then_some(IconName::CarrotAssistant)
    }
...
impl AgentPanel {
                    ...
                    .map(|r| r.read(cx).title_editor.clone())
                {
                    if is_generating_title {
                        ...
                        div()
                            ...
                            .on_action({
                                let conversation_view = conversation_view.downgrade();
                                move |_: &carrot_editor::actions::Cancel, window, cx| {
                                    if let Some(conversation_view) = conversation_view.upgrade() {
                                        conversation_view.focus_handle(cx).focus(window, cx);
                                    }
                                }
    ...
    fn render_panel_options_menu(
        ...
        let text_thread_with_messages = match &self.active_view {
            ActiveView::TextThread {
                ...
                .read(cx)
                .messages(cx)
                .any(|message| message.role == carrot_language_model::Role::Assistant),
            _ => false,
        };
                    ...
                    Some(ContextMenu::build(window, cx, |mut menu, _window, _| {
                        ...
                        menu = menu
                            ...
                            .action(
                                ...
                                Box::new(carrot_actions::Extensions {
                                    category_filter: Some(
                                        carrot_actions::ExtensionCategoryFilter::ContextServers,
                                    ),
                                    id: None,
                                }),
                            )
    ...
    fn render_recent_entries_menu(
        ...
        PopoverMenu::new("agent-nav-menu")
            ...
            .menu({
                ...
                move |window, cx| {
                    carrot_telemetry::event!("View Thread History Clicked");
    ...
    fn render_toolbar_back_button(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        IconButton::new("go-back", IconName::ArrowLeft)
            ...
            .on_click(cx.listener(|this, _, window, cx| {
                this.go_back(&carrot_workspace::GoBack, window, cx);
            }))
            .tooltip({
                move |_window, cx| {
                    Tooltip::for_action_in("Go Back", &carrot_workspace::GoBack, &focus_handle, cx)
                }
            })
    }
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        > = {
            ...
            Rc::new(move |window, cx| {
                carrot_telemetry::event!("New Thread Clicked");
                ...
                Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                    menu.context(focus_handle.clone())
                        ...
                        .item(
                            ContextMenuEntry::new("Carrot Agent")
                                .when(
                        ...
                        .map(|mut menu| {
                            ...
                            let registry_store = carrot_project::AgentRegistryStore::try_global(cx);
                        ...
                        .item(
                            ContextMenuEntry::new("Add More Agents")
                                ...
                                .handler({
                                    move |window, cx| {
                                        window
                                            .dispatch_action(Box::new(carrot_actions::AcpRegistry), cx)
                                    }
                                }),
                        )
                }))
        ...
        if use_v2_empty_toolbar {
            ...
            let agent_icon = if let Some(icon_path) = selected_agent_custom_icon_for_button {
                ...
                let icon_name = selected_agent_builtin_icon.unwrap_or(IconName::CarrotAgent);
    ...
    fn should_render_trial_end_upsell(&self, cx: &mut Context<Self>) -> bool {
        ...
        match &self.active_view {
            ActiveView::TextThread { .. } => {
                if LanguageModelRegistry::global(cx)
                    ...
                    .default_model()
                    .is_some_and(|model| {
                        model.provider.id() != carrot_language_model::ZED_CLOUD_PROVIDER_ID
                    })
                {
                    return false;
                }
        ...
        plan.is_some_and(|plan| plan == Plan::CarrotFree) && has_previous_trial
    }
    ...
    fn should_render_onboarding(&self, cx: &mut Context<Self>) -> bool {
        ...
        if user_store.plan().is_some_and(|plan| plan == Plan::CarrotPro)
            && user_store
                .subscription_period()
        ...
        let has_configured_non_carrot_providers = LanguageModelRegistry::read_global(cx)
            .visible_providers()
            .iter()
            .any(|provider| {
                provider.is_authenticated(cx)
                    && provider.id() != carrot_language_model::ZED_CLOUD_PROVIDER_ID
            });
        ...
        match &self.active_view {
            ...
            ActiveView::AgentThread { conversation_view } => {
                ...
                history_is_empty || !has_configured_non_carrot_providers
            }
            ActiveView::TextThread { .. } => {
    ...
    fn emit_configuration_error_telemetry_if_needed(
        ...
        if let Some(kind) = error_kind {
            ...
            carrot_telemetry::event!("Agent Panel Error Shown", kind = kind, message = message,);
        }
    }
    ...
    fn render_configuration_error(
        ...
        let carrot_provider_configured = AgentSettings::get_global(cx)
            .default_model
            .as_ref()
            .is_some_and(|selection| selection.provider.0.as_str() == "carrot.dev");

        let callout = if carrot_provider_configured {
            Callout::new()
                .icon(IconName::Warning)
                .severity(Severity::Warning)
                .when(border_bottom, |this| {
                    this.border_position(carrot_ui::BorderPosition::Bottom)
                })
                .title("Sign in to continue using Carrot as your LLM provider.")
                .actions_slot(
    ...
    fn render_workspace_trust_message(&self, cx: &Context<Self>) -> Option<impl IntoElement> {
        ...
        Some(
            Callout::new()
                .icon(IconName::Warning)
                .severity(Severity::Warning)
                .border_position(carrot_ui::BorderPosition::Bottom)
...
impl carrot_rules_library::InlineAssistDelegate for PromptLibraryInlineAssist {
    fn assist(
        &self,
...
impl AgentPanelDelegate for ConcreteAssistantPanelDelegate {
    ...
    fn open_remote_text_thread(
        ...
        _text_thread_id: carrot_assistant_text_thread::TextThreadId,
        _window: &mut Window,
...
mod tests {
    ...
    use carrot_acp_thread::{StubAgentConnection, ThreadStatus};
    use carrot_agent_servers::CODEX_ID;
    use carrot_assistant_text_thread::TextThreadStore;
    use carrot_feature_flags::FeatureFlagAppExt;
    ...
    async fn test_active_thread_serialize_and_load_round_trip(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            carrot_agent::ThreadStore::init_global(cx);
            carrot_language_model::LanguageModelRegistry::test(cx);
        });
        ...
        let prompt_builder = Arc::new(carrot_prompt_store::PromptBuilder::new(None).unwrap());
    ...
    async fn test_new_text_thread_action_handler(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            let slash_command_registry =
                carrot_assistant_slash_command::SlashCommandRegistry::default_global(cx);
            slash_command_registry
                .register_command(carrot_assistant_slash_commands::DefaultSlashCommand, false);
    ...
    fn test_conflict_resource_block_structure() {
        ...
        assert!(
            uri.starts_with("carrot:///agent/merge-conflict"),
            "URI should use the carrot merge-conflict scheme, got: {uri}"
        );
