...
use carrot_acp_thread::{AcpThread, AgentThreadEntry};
use carrot_agent::ThreadStore;
use agent_client_protocol::ToolCallId;
use inazuma_collections::HashMap;
use carrot_editor::{Editor, EditorEvent, EditorMode, MinimapVisibility, SizingBehavior};
use inazuma::{
...
use carrot_ui::{Context, TextSize};
        ...
        match thread_entry {
            ...
            AgentThreadEntry::ToolCall(tool_call) => {
                ...
                let is_tool_call_completed =
                    matches!(tool_call.status, carrot_acp_thread::ToolCallStatus::Completed);
...
impl AssistantMessageEntry {
    ...
    pub fn sync(&mut self, message: &carrot_acp_thread::AssistantMessage) {
        if let Some(carrot_acp_thread::AssistantMessageChunk::Thought { .. }) = message.chunks.last() {
...
impl Entry {
    ...
    pub fn editor_for_diff(&self, diff: &Entity<carrot_acp_thread::Diff>) -> Option<Entity<Editor>> {
        self.content_map()?
            .get(&diff.entity_id())
            .cloned()
...
mod tests {
    ...
    async fn test_diff_sync(cx: &mut TestAppContext) {
        ...
        let history: Option<inazuma::WeakEntity<crate::ThreadHistory>> = None;
