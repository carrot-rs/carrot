...
use carrot_context_server::{ContextServerCommand, ContextServerId};
...
use inazuma_settings_framework::{Settings as _, update_settings_file};
use std::sync::Arc;
...
enum ConfigurationTarget {
    ...
    Extension {
        ...
        installation: Option<carrot_extension::ContextServerConfiguration>,
    },
}
...
enum ConfigurationSource {
    ...
    Extension {
        ...
        repository_url: Option<SharedString>,
        installation_instructions: Option<Entity<carrot_markdown::Markdown>>,
    ...
    cx.spawn(async move |cx| {
        ...
        match result {
            ...
            futures::future::Either::Right(_) => Err(Arc::from(format!(
                "Timed out waiting for context server `{}` to start. Check the Carrot log for details.",
