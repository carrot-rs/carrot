...
use carrot_agent_settings::{AgentProfile, AgentProfileId, AgentSettings, builtin_profiles};
...
impl ManageProfilesModal {
                    ...
                    move |cx| {
                        ...
                        settings
                            ...
                            .and_then(|selection| {
                                ...
                                let provider_id = carrot_language_model::LanguageModelProviderId(
                                ...
                                let model = provider
                                    ...
                                    .clone();
                                Some(carrot_language_model::ConfiguredModel { provider, model })
                            })
                    }
                },
                {
    ...
    fn configure_builtin_tools(
        ...
        let tool_names: Vec<Arc<str>> = carrot_agent::ALL_TOOL_NAMES
            .iter()
            .copied()
            .filter(|name| {
                let supported_by_provider = provider.as_ref().map_or(true, |provider| {
                    carrot_agent::tool_supports_provider(name, provider)
                });
