...
// TODO(phase20): terminal_inline_assistant depends on carrot-terminal-view which will be built in Phase 20
...
use carrot_assistant_slash_command::SlashCommandRegistry;
...
use carrot_fs::Fs;
use inazuma::{Action, App, Context, Entity, SharedString, UpdateGlobal, Window, actions};
use carrot_language::{
    LanguageRegistry,
...
use carrot_project::{AgentId, DisableAiSettings};
use carrot_prompt_store::PromptBuilder;
use schemars::JsonSchema;
use serde::{Deserialize, Serialize};
use inazuma_settings_framework::{DockPosition, DockSide, LanguageModelSelection, Settings as _, SettingsStore};
use std::any::TypeId;
...
impl Agent {
    pub fn id(&self) -> AgentId {
        match self {
            Self::NativeAgent => carrot_agent::ZED_AGENT_ID.clone(),
    ...
    pub fn server(
        &self,
        fs: Arc<dyn carrot_fs::Fs>,
        thread_store: Entity<carrot_agent::ThreadStore>,
    ) -> Rc<dyn carrot_agent_servers::AgentServer> {
        match self {
            Self::NativeAgent => Rc::new(carrot_agent::NativeAgentServer::new(fs, thread_store)),
            Self::Custom { id: name } => {
                Rc::new(carrot_agent_servers::CustomAgentServer::new(name.clone()))
            }
        }
    }
}
...
pub fn init(
    ...
    carrot_assistant_text_thread::init(client, cx);
    carrot_rules_library::init(cx);
    if !is_eval {
        // Initializing the language model from the user settings messes with the eval, so we only initialize them when
        // we're not running inside of the eval.
        init_language_model_settings(cx);
    }
    carrot_assistant_slash_command::init(cx);
    agent_panel::init(cx);
    ...
    // TODO(phase20): terminal_inline_assistant::init(fs.clone(), prompt_builder, cx);
            ...
            move |workspace: &mut Workspace,
                ...
                if let Some(existing) = existing {
                    existing.update(cx, |_, cx| {
                        carrot_project::AgentRegistryStore::global(cx)
...
fn update_command_palette_filter(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _| {
        ...
        if disable_ai {
            ...
            filter.hide_namespace("carrot_predict_onboarding");
            filter.hide_namespace("edit_prediction");

            filter.hide_action_types(&edit_prediction_actions);
            filter.hide_action_types(&[TypeId::of::<carrot_actions::OpenCarrotPredictOnboarding>()]);
        } else {
            if agent_enabled {
...
fn init_language_model_settings(cx: &mut App) {
    ...
    cx.subscribe(
        ...
        |_, event: &carrot_language_model::Event, cx| match event {
            carrot_language_model::Event::ProviderStateChanged(_)
            | carrot_language_model::Event::AddedProvider(_)
            | carrot_language_model::Event::RemovedProvider(_)
            | carrot_language_model::Event::ProvidersChanged => {
                update_active_language_model_from_settings(cx);
            }
            _ => {}
        },
...
fn update_active_language_model_from_settings(cx: &mut App) {
    ...
    fn to_selected_model(selection: &LanguageModelSelection) -> carrot_language_model::SelectedModel {
        carrot_language_model::SelectedModel {
...
fn register_slash_commands(cx: &mut App) {
    ...
    slash_command_registry.register_command(carrot_assistant_slash_commands::FileSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::DeltaSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::OutlineSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::TabSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::PromptSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::SelectionCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::DefaultSlashCommand, false);
    slash_command_registry.register_command(carrot_assistant_slash_commands::NowSlashCommand, false);
    slash_command_registry
        .register_command(carrot_assistant_slash_commands::DiagnosticsSlashCommand, true);
    slash_command_registry.register_command(carrot_assistant_slash_commands::FetchSlashCommand, true);

    cx.observe_flag::<carrot_assistant_slash_commands::StreamingExampleSlashCommandFeatureFlag, _>({
        move |is_enabled, _cx| {
            if is_enabled {
                slash_command_registry.register_command(
                    carrot_assistant_slash_commands::StreamingExampleSlashCommand,
                    false,
                );
            }
        }
...
mod tests {
    ...
    use carrot_agent_settings::{AgentProfileId, AgentSettings};
    use carrot_command_palette_hooks::CommandPaletteFilter;
    use carrot_editor::actions::AcceptEditPrediction;
    use inazuma::{BorrowAppContext, TestAppContext, px};
    use carrot_project::DisableAiSettings;
