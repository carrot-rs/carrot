...
use carrot_acp_thread::{AcpThread, AcpThreadEvent};
use carrot_action_log::{ActionLogTelemetry, LastRejectUndo};
...
use carrot_language::{Buffer, Capability, OffsetRangeExt, Point};
use carrot_multi_buffer::PathKey;
use carrot_project::{Project, ProjectItem, ProjectPath};
...
use carrot_ui::{CommonAnimationExt, IconButtonShape, KeyBinding, Tooltip, prelude::*, vertical_divider};
...
use carrot_actions::assistant::ToggleFocus;
        ...
        for (buffer, diff_handle) in sorted_buffers {
            ...
            let diff_hunk_ranges = diff_handle
                ...
                .hunks_intersecting_range(
                    carrot_language::Anchor::min_max_range_for_buffer(snapshot.remote_id()),
                    &snapshot,
                )
    ...
    pub fn move_to_path(&self, path_key: PathKey, window: &mut Window, cx: &mut App) {
        if let Some(position) = self.multibuffer.read(cx).location_for_path(&path_key, cx) {
            self.editor.update(cx, |editor, cx| {
                let first_hunk = editor
                    .diff_hunks_in_ranges(
                        &[position..carrot_editor::Anchor::max()],
                        &self.multibuffer.read(cx).read(cx),
                    )
                    .next();
    ...
    fn reject_all(&mut self, _: &RejectAll, window: &mut Window, cx: &mut Context<Self>) {
        self.editor.update(cx, |editor, cx| {
            ...
            reject_edits_in_ranges(
                ...
                &snapshot,
                &self.thread,
                vec![carrot_editor::Anchor::min()..carrot_editor::Anchor::max()],
                self.workspace.clone(),
                window,
                cx,
            );
...
fn keep_edits_in_ranges(
    ...
    buffer_snapshot: &MultiBufferSnapshot,
    thread: &Entity<AcpThread>,
    ranges: Vec<Range<carrot_editor::Anchor>>,
    window: &mut Window,
    cx: &mut Context<Editor>,
) {
...
fn update_editor_selection(
    editor: &mut Editor,
    buffer_snapshot: &MultiBufferSnapshot,
    diff_hunks: &[carrot_multi_buffer::MultiBufferDiffHunk],
    window: &mut Window,
    cx: &mut Context<Editor>,
) {
    ...
    if !diff_hunks.iter().any(|hunk| {
        hunk.row_range
            .contains(&carrot_multi_buffer::MultiBufferRow(newest_cursor.row))
    }) {
        return;
    }
    ...
    let target_hunk = {
        diff_hunks
            ...
            .and_then(|last_kept_hunk| {
                ...
                editor
                    .diff_hunks_in_ranges(
                        &[last_kept_hunk_end..carrot_editor::Anchor::max()],
                        buffer_snapshot,
                    )
                    .nth(1)
            })
            .or_else(|| {
                ...
                editor
                    .diff_hunks_in_ranges(
                        &[carrot_editor::Anchor::min()..first_kept_hunk_start],
                        buffer_snapshot,
                    )
                    .next()
            })
...
impl Item for AgentDiffPane {
    ...
    fn tab_icon(&self, _window: &Window, _cx: &App) -> Option<Icon> {
        Some(Icon::new(IconName::CarrotAssistant).color(Color::Muted))
    }
...
fn diff_hunk_controls(
    thread: &Entity<AcpThread>,
    workspace: WeakEntity<Workspace>,
) -> carrot_editor::RenderDiffHunkControlsFn {
...
fn render_diff_hunk_controls(
    ...
    hunk_range: Range<carrot_editor::Anchor>,
...
impl carrot_editor::Addon for AgentDiffAddon {
    fn to_any(&self) -> &dyn std::any::Any {
        self
    }

    fn extend_key_context(&self, key_context: &mut inazuma::KeyContext, _: &App) {
...
impl AgentDiff {
    ...
    fn handle_workspace_event(
        ...
        if let carrot_workspace::Event::ItemAdded { item } = event
...
impl carrot_editor::Addon for EditorAgentDiffAddon {
    fn to_any(&self) -> &dyn std::any::Any {
        self
    }
...
mod tests {
    use super::*;
    use crate::Keep;
    use carrot_acp_thread::AgentConnection as _;
    use carrot_agent_settings::AgentSettings;
    use carrot_editor::EditorSettings;
    ...
    use carrot_workspace::{PathList, Workspace};
    ...
    async fn test_multibuffer_agent_diff(cx: &mut TestAppContext) {
        ...
        let connection = Rc::new(carrot_acp_thread::StubAgentConnection::new());
        let thread = cx
            .update(|cx| {
    ...
    async fn test_single_file_review_diff(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            carrot_theme_settings::init(carrot_theme::LoadThemes::JustBase, cx);
            carrot_language_model::init_settings(cx);
            carrot_workspace::register_project_item::<Editor>(cx);
        });
