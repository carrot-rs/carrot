...
use carrot_acp_thread::MentionUri;
...
use carrot_ui::{ContextMenu, Disclosure, ElevationIndex, prelude::*};
use inazuma_util::paths::PathStyle;
use inazuma_util::{ResultExt, debug_panic};
use carrot_workspace::{CollaboratorId, Workspace};
use carrot_actions::agent::{Chat, PasteRaw};
    ...
    let text_anchor = match position {
        ...
        MentionInsertPosition::EndOfBuffer => {
            ...
            editor.update(cx, |editor, cx| {
                editor.edit(
                    [(
                        carrot_multi_buffer::Anchor::max()..carrot_multi_buffer::Anchor::max(),
                        new_text,
                    )],
                    cx,
                );
...
enum ResolvedPastedContextItem {
    Image(inazuma::Image, inazuma::SharedString),
...
async fn resolve_pasted_context_items(
    ...
    cx: &mut inazuma::AsyncWindowContext,
...
impl MessageEditor {
        ...
        let editor = cx.new(|cx| {
            ...
            editor.set_custom_context_menu(|editor, _point, window, cx| {
                ...
                Some(ContextMenu::build(window, cx, |menu, _, _| {
                    menu.action("Cut", Box::new(carrot_editor::actions::Cut))
                        .action_disabled_when(
                            !has_selection,
                            "Copy",
                            Box::new(carrot_editor::actions::Copy),
                        )
                        .action("Paste", Box::new(carrot_editor::actions::Paste))
    ...
    fn command_hint(&self, snapshot: &MultiBufferSnapshot) -> Option<Inlay> {
        ...
        Some(Inlay::hint(
            ...
            &InlayHint {
                ...
                padding_right: false,
                tooltip: None,
                resolve_state: carrot_project::ResolveState::Resolved,
            },
        ))
    }
        ...
        cx.spawn_in(window, async move |_, cx| {
            editor
                .update_in(cx, |editor, window, cx| {
                    ...
                    editor.show_completions(&carrot_editor::actions::ShowCompletions, window, cx);
                })
                .log_err();
        })
        .detach();
    ...
    fn cancel(&mut self, _: &carrot_editor::actions::Cancel, _: &mut Window, cx: &mut Context<Self>) {
    ...
    fn paste(&mut self, _: &Paste, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if should_insert_creases && let Some(selections) = editor_clipboard_selections {
            ...
            for selection in selections {
                ...
                {
                    let crease_text =
                        carrot_acp_thread::selection_name(Some(file_path.as_ref()), &line_range);
    ...
    fn insert_crease_impl(
        ...
        use carrot_editor::display_map::{Crease, FoldPlaceholder};
        ...
        self.editor.update(cx, |editor, cx| {
            ...
            let crease = Crease::inline(
                ...
                |_, _, _, _| inazuma::Empty.into_any(),
            );
    ...
    pub fn add_images_from_picker(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let paths_receiver = cx.prompt_for_paths(inazuma::PathPromptOptions {
            files: true,
            directories: false,
            multiple: true,
            ...
            .child({
                ...
                EditorElement::new(
                    ...
                    EditorStyle {
                        ...
                        inlay_hints_style: carrot_editor::make_inlay_hints_style(cx),
                        ..Default::default()
                    },
                )
            })
...
mod tests {
    ...
    use carrot_acp_thread::MentionUri;
    use carrot_agent::{ThreadStore, outline};
    use agent_client_protocol as acp;
    ...
    use carrot_lsp::{CompletionContext, CompletionTriggerKind};
    use parking_lot::RwLock;
    use carrot_project::{CompletionIntent, Project, ProjectPath};
    use serde_json::{Value, json};

    use inazuma_text::Point;
    use carrot_ui::{App, Context, IntoElement, Render, SharedString, Window};
    use inazuma_util::{path, inazuma_util::paths::PathStyle, rel_path::rel_path};
    use carrot_workspace::{AppState, Item, Workspace};
    ...
    fn test_parse_mention_links() {
        // Single file mention
        let text = "[@bundle-mac](file:///Users/test/carrot/script/bundle-mac)";
    ...
    #[inazuma::test]
    async fn test_slash_command_validation(cx: &mut inazuma::TestAppContext) {
        init_test(cx);
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            "/test",
    ...
    async fn test_completion_provider_commands(cx: &mut TestAppContext) {
        ...
        editor.update_in(&mut cx, |editor, window, cx| {
            assert!(editor.has_visible_completions_menu());
            editor.confirm_completion(&carrot_editor::actions::ConfirmCompletion::default(), window, cx);
        });
        ...
        editor.update_in(&mut cx, |editor, window, cx| {
            ...
            // Delete argument
            for _ in 0..5 {
                editor.backspace(&carrot_editor::actions::Backspace, window, cx);
            }
        });
    ...
    async fn test_context_completion_provider_mentions(cx: &mut TestAppContext) {
        ...
        editor.update_in(&mut cx, |editor, window, cx| {
            assert!(editor.has_visible_completions_menu());
            editor.context_menu_next(&carrot_editor::actions::ContextMenuNext, window, cx);
            editor.context_menu_next(&carrot_editor::actions::ContextMenuNext, window, cx);
            editor.context_menu_next(&carrot_editor::actions::ContextMenuNext, window, cx);
            editor.context_menu_next(&carrot_editor::actions::ContextMenuNext, window, cx);
            editor.confirm_completion(&carrot_editor::actions::ConfirmCompletion::default(), window, cx);
        ...
        let plain_text_language = Arc::new(carrot_language::Language::new(
            carrot_language::LanguageConfig {
                name: "Plain Text".into(),
                matcher: carrot_language::LanguageMatcher {
        ...
        let mut fake_language_servers = language_registry.register_fake_lsp(
            "Plain Text",
            carrot_language::FakeLspAdapter {
                capabilities: carrot_lsp::ServerCapabilities {
                    workspace_symbol_provider: Some(carrot_lsp::OneOf::Left(true)),
                    ..Default::default()
                },
                ..Default::default()
            },
        ...
        fake_language_server.set_request_handler::<carrot_lsp::WorkspaceSymbolRequest, _, _>(
            move |_, _| async move {
                Ok(Some(carrot_lsp::WorkspaceSymbolResponse::Flat(vec![
                    #[allow(deprecated)]
                    carrot_lsp::SymbolInformation {
                        name: "MySymbol".into(),
                        location: carrot_lsp::Location {
                            uri: carrot_lsp::Uri::from_file_path(path!("/dir/a/one.txt")).unwrap(),
                            range: carrot_lsp::Range::new(
                                carrot_lsp::Position::new(0, 0),
                                carrot_lsp::Position::new(0, 1),
                            ),
                        },
                        kind: carrot_lsp::SymbolKind::CONSTANT,
                        tags: None,
                        container_name: None,
                        deprecated: None,
                    },
    ...
    async fn test_autoscroll_after_insert_selections(cx: &mut TestAppContext) {
        ...
        message_editor.update_in(&mut cx, |message_editor, window, cx| {
            message_editor.editor.update(cx, |editor, cx| {
                ...
                editor.set_scroll_position(inazuma::Point::new(0., 0.), window, cx);
            });
        });
    ...
    fn write_test_png_file(extension: Option<&str>) -> PathBuf {
        ...
        let file_name = match extension {
            Some(extension) => format!("carrot-agent-ui-test-{}.{}", uuid::Uuid::new_v4(), extension),
            None => format!("carrot-agent-ui-test-{}", uuid::Uuid::new_v4()),
        };
