...
mod tests {
    ...
    use inazuma::{AsyncApp, TestAppContext, http_client};
    use carrot_language_model::{
        ...
        model_specs
            ...
            .map(|(provider, name)| {
                ...
                ModelInfo {
                    ...
                    icon: IconOrSvg::Icon(IconName::CarrotAgent),
                    is_favorite,
                }
            })
            .collect()
    ...
    fn test_exact_match(cx: &mut TestAppContext) {
        let models = create_models(vec![
            ("carrot","Claude 3.7 Sonnet"),
            ("carrot","Claude 3.7 Sonnet Thinking"),
            ("carrot","gpt-5"),
            ("carrot","gpt-5-mini"),
    ...
    fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        // In the case below, `carrot/gpt-5-mini` and `openai/gpt-5-mini` have identical
        // similarity scores, but `carrot/gpt-5-mini` was higher in the models list,
        // so it should appear first in the results.
        let results = matcher.fuzzy_search("mini");
        assert_models_eq(results, vec!["carrot/gpt-5-mini", "openai/gpt-5-mini"]);
        ...
        assert_models_eq(results, vec!["carrot/Claude 3.7 Sonnet Thinking"]);
    }

    #[inazuma::test]
    fn test_recommended_models_also_appear_in_other(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("carrot","claude")]);
        let all_models = create_models(vec![
            ("carrot","claude"), // Should also appear in "other"
            ("carrot","gemini"),
            ("copilot", "o3"),
        ]);
        ...
        assert_models_eq(
            actual_all_models,
            vec!["carrot/claude", "carrot/gemini", "copilot/o3"],
        );
    }
    ...
    fn test_models_from_different_providers(_cx: &mut TestAppContext) {
        ...
        assert_models_eq(
            actual_all_models,
            vec!["carrot/claude", "carrot/gemini", "copilot/claude"],
        );
    }
    ...
    fn test_favorites_section_appears_when_favorites_exist(_cx: &mut TestAppContext) {
        ...
        let all_models = create_models_with_favorites(
            vec![("carrot","claude"), ("carrot","gemini"), ("openai", "gpt-4")],
            vec![("carrot","gemini")],
        );
        ...
        assert_models_eq(grouped_models.favorites, vec!["carrot/gemini"]);
    }

    #[inazuma::test]
    fn test_no_favorites_section_when_no_favorites(_cx: &mut TestAppContext) {
        let recommended_models = create_models(vec![("carrot","claude")]);
        let all_models = create_models(vec![("carrot","claude"), ("carrot","gemini")]);
    ...
    fn test_models_have_correct_actions(_cx: &mut TestAppContext) {
        let recommended_models =
            create_models_with_favorites(vec![("carrot","claude")], vec![("carrot","claude")]);
        let all_models = create_models_with_favorites(
            vec![("carrot","claude"), ("carrot","gemini"), ("openai", "gpt-4")],
            vec![("carrot","claude")],
        );
        ...
        for entry in &entries {
            if let LanguageModelPickerEntry::Model(info) = entry {
                if info.model.telemetry_id() == "carrot/claude" {
                    assert!(info.is_favorite, "carrot/claude should be a favorite");
                } else {
                    assert!(
                        !info.is_favorite,
    ...
    fn test_favorites_appear_in_other_sections(_cx: &mut TestAppContext) {
        let favorites = vec![("carrot","gemini"), ("openai", "gpt-4")];

        let recommended_models =
            create_models_with_favorites(vec![("carrot","claude")], favorites.clone());
        ...
        assert_models_eq(grouped_models.favorites, vec!["carrot/gemini", "openai/gpt-4"]);
        assert_models_eq(grouped_models.recommended, vec!["carrot/claude"]);
        assert_models_eq(
            grouped_models.all.values().flatten().cloned().collect(),
            vec!["carrot/claude", "carrot/gemini", "openai/gpt-4", "openai/gpt-3.5"],
        );
    }
}
