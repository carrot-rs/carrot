...
use anyhow::Result;
use carrot_assistant_slash_command::{SlashCommand, SlashCommandOutputSection, SlashCommandWorkingSet};
use carrot_assistant_slash_commands::{DefaultSlashCommand, FileSlashCommand, selections_creases};
use carrot_client::{proto, carrot_urls};
...
use carrot_editor::{FoldPlaceholder, display_map::CreaseId};
use carrot_fs::Fs;
...
use inazuma_picker::{Picker, popover_menu::PickerPopoverMenu};
use carrot_project::{Project, Worktree};
use carrot_project::{ProjectPath, lsp_store::LocalLspAdapterDelegate};
...
use inazuma_text::SelectionGoal;
use carrot_ui::{
...
use carrot_actions::agent::{AddSelectionToThread, PasteRaw, ToggleModelSelector};

use crate::CycleFavoriteModels;

use crate::{slash_command::SlashCommandCompletionProvider, slash_command_picker};
use carrot_assistant_text_thread::{
...
struct ScrollPosition {
    offset_before_cursor: inazuma::Point<ScrollOffset>,
    cursor: Anchor,
}
...
pub struct TextThreadEditor {
    ...
    pending_thought_process: Option<(CreaseId, carrot_language::Anchor)>,
    blocks: HashMap<MessageId, (MessageHeader, CustomBlockId)>,
    image_blocks: HashSet<CustomBlockId>,
    scroll_position: Option<ScrollPosition>,
    remote_id: Option<carrot_workspace::ViewId>,
    pending_slash_command_creases: HashMap<Range<carrot_language::Anchor>, CreaseId>,
...
impl TextThreadEditor {
    pub fn init(cx: &mut App) {
        carrot_workspace::FollowableViewRegistry::register::<TextThreadEditor>(cx);
    ...
    fn assist(&mut self, _: &Assist, window: &mut Window, cx: &mut Context<Self>) {
        ...
        carrot_telemetry::event!("Agent Message Sent", agent = "carrot-text");
    ...
    fn cancel(
        &mut self,
        _: &carrot_editor::actions::Cancel,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.last_error = None;
    ...
    pub fn run_command(
        &mut self,
        command_range: Range<carrot_language::Anchor>,
        name: &str,
        arguments: &[String],
            ...
            for (section, status) in sections {
                ...
                creases.push(
                    ...
                    .with_metadata(CreaseMetadata {
                        icon_path: SharedString::from(IconName::CarrotAgent.path()),
    ...
    fn insert_slash_command_output_sections(
        &mut self,
        sections: impl IntoIterator<Item = SlashCommandOutputSection<carrot_language::Anchor>>,
        expand_result: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
                ...
                editor
                    ...
                    .map(|s| {
                        let (start, end) = if s.is_empty() {
                            let row = carrot_multi_buffer::MultiBufferRow(s.start.row);
    ...
    fn copy(&mut self, _: &carrot_editor::actions::Copy, _window: &mut Window, cx: &mut Context<Self>) {
    ...
    fn cut(&mut self, _: &carrot_editor::actions::Cut, window: &mut Window, cx: &mut Context<Self>) {
        if self.editor.read(cx).selections.count() == 1 {
    ...
    fn paste(
        &mut self,
        action: &carrot_editor::actions::Paste,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        let editor_clipboard_selections = cx.read_from_clipboard().and_then(|item| {
            item.entries().iter().find_map(|entry| match entry {
                ClipboardEntry::String(text) => {
                    text.metadata_json::<Vec<carrot_editor::ClipboardSelection>>()
                }
                _ => None,
            })
        });
                    ...
                    self.editor.update(cx, |editor, cx| {
                        ...
                        for selection in selections {
                                ...
                                (selection.file_path, selection.line_range)
                            {
                                ...
                                let fence = carrot_assistant_slash_commands::codeblock_fence_for_path(
                                    file_path.to_str(),
                                ...
                                let crease_text = carrot_acp_thread::selection_name(
        ...
        let mut images: Vec<inazuma::Image> = Vec::new();
    ...
    fn paste_raw(&mut self, _: &PasteRaw, window: &mut Window, cx: &mut Context<Self>) {
        self.editor.update(cx, |editor, cx| {
            editor.paste(&carrot_editor::actions::Paste, window, cx);
        });
    }
                    ...
                    anchor.is_valid(&buffer).then(|| BlockProperties {
                        ...
                        render: Arc::new(move |cx| {
                            ...
                            h_flex()
                                ...
                                .child(
                                    img(image.clone())
                                        .object_fit(inazuma::ObjectFit::ScaleDown)
                                        .w(image_size.width)
                                        .h(image_size.height),
                                )
                                .into_any_element()
    ...
    fn render_language_model_selector(
        ...
        let provider_icon = active_provider
            .as_ref()
            .map(|p| p.icon())
            .unwrap_or(IconOrSvg::Icon(IconName::CarrotAgent));
    ...
    fn render_payment_required_error(&self, cx: &mut Context<Self>) -> AnyElement {
        const ERROR_MESSAGE: &str = "Free tier exceeded. Subscribe and add payment to continue using Carrot LLMs. You'll be billed at cost for tokens used.";
...
fn render_pending_slash_command_gutter_decoration(
    ...
    let mut icon = IconButton::new(
        ...
        carrot_ui::IconName::TriangleRight,
    )
    .on_click(move |_e, window, cx| confirm_command(window, cx))
    .icon_size(carrot_ui::IconSize::Small)
    .size(carrot_ui::ButtonSize::None);
...
impl Item for TextThreadEditor {
    type Event = carrot_editor::EditorEvent;
...
impl FollowableItem for TextThreadEditor {
    fn remote_id(&self) -> Option<carrot_workspace::ViewId> {
        self.remote_id
    }

    fn to_state_proto(&self, window: &mut Window, cx: &mut App) -> Option<carrot_proto::view::Variant> {
        ...
        Some(carrot_proto::view::Variant::ContextEditor(
            carrot_proto::view::ContextEditor {
                context_id,
                editor: if let Some(carrot_proto::view::Variant::Editor(proto)) = editor_proto {
                    Some(proto)
                } else {
                    None
                },
    ...
    fn from_state_proto(
        ...
        id: carrot_workspace::ViewId,
        state: &mut Option<carrot_proto::view::Variant>,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<Task<Result<Entity<Self>>>> {
        let carrot_proto::view::Variant::ContextEditor(_) = state.as_ref()? else {
            return None;
        };
        let Some(carrot_proto::view::Variant::ContextEditor(state)) = state.take() else {
            unreachable!()
        };
                ...
                .update_in(cx, |text_thread_editor, window, cx| {
                    ...
                    text_thread_editor.editor.update(cx, |editor, cx| {
                        editor.apply_update_proto(
                            &project,
                            carrot_proto::update_view::Variant::Editor(carrot_proto::update_view::Editor {
    ...
    fn add_event_to_update_proto(
        &self,
        event: &Self::Event,
        update: &mut Option<carrot_proto::update_view::Variant>,
        window: &mut Window,
        cx: &mut App,
    ) -> bool {
        self.editor.update(cx, |editor, cx| {
    ...
    fn apply_update_proto(
        &mut self,
        project: &Entity<Project>,
        message: carrot_proto::update_view::Variant,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Task<Result<()>> {
        self.editor.update(cx, |editor, cx| {
...
mod tests {
    use super::*;
    use carrot_editor::{MultiBufferOffset, SelectionEffects};
    use carrot_fs::FakeFs;
    use inazuma::{App, TestAppContext, VisualTestContext};
    use indoc::indoc;
    use carrot_language::{Buffer, LanguageRegistry};
    ...
    async fn test_copy_paste_whole_message(cx: &mut TestAppContext) {
        let (context, text_thread_editor, mut cx) = setup_text_thread_editor_text(vec![
            (Role::User, "What is the Carrot editor?"),
            (
                Role::Assistant,
                "Carrot is a modern, high-performance code editor designed from the ground up for speed and collaboration.",
            ),
            (Role::User, ""),
        ],cx).await;
    ...
    fn assert_copy_paste_text_thread_editor<T: carrot_editor::ToOffset>(
