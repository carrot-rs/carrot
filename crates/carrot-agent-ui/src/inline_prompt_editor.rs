...
use carrot_editor::actions::Paste;
use carrot_editor::code_context_menus::CodeContextMenu;
use carrot_editor::display_map::{CreaseId, EditorMargins};
use carrot_editor::{AnchorRangeExt as _, MultiBufferOffset, ToOffset as _};
use carrot_editor::{
...
use carrot_language_model::{LanguageModel, LanguageModelRegistry};
use carrot_markdown::{HeadingLevelStyles, Markdown, MarkdownElement, MarkdownStyle};
...
use carrot_ui::utils::WithRemSize;
use carrot_ui::{IconButtonShape, KeyBinding, PopoverMenuHandle, Tooltip, prelude::*};
use uuid::Uuid;
use carrot_workspace::notifications::NotificationId;
use carrot_workspace::{Toast, Workspace};
use carrot_actions::{
...
impl<T: 'static> PromptEditor<T> {
    ...
    pub fn unlink(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.editor = cx.new(|cx| {
            ...
            editor.set_soft_wrap_mode(carrot_language::language_settings::SoftWrap::EditorWidth, cx);
    ...
    pub fn placeholder_text(mode: &PromptEditorMode, window: &mut Window, cx: &mut App) -> String {
        ...
        let agent_panel_keybinding =
            carrot_ui::text_for_action(&carrot_actions::assistant::ToggleFocus, window, cx)
