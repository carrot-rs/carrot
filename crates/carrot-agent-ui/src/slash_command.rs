...
use anyhow::Result;
pub use carrot_assistant_slash_command::SlashCommand;
use carrot_assistant_slash_command::{AfterCompletion, SlashCommandLine, SlashCommandWorkingSet};
use carrot_editor::{CompletionProvider, Editor, ExcerptId};
use inazuma_fuzzy::{StringMatchCandidate, match_strings};
use inazuma::{App, AppContext as _, Context, Entity, Task, WeakEntity, Window};
use carrot_language::{Anchor, Buffer, ToPoint};
...
impl SlashCommandCompletionProvider {
    ...
    fn complete_command_name(
        ...
        window: &mut Window,
        cx: &mut App,
    ) -> Task<Result<Vec<carrot_project::CompletionResponse>>> {
        ...
        window.spawn(cx, async move |cx| {
            ...
            cx.update(|_, cx| {
                ...
                vec![carrot_project::CompletionResponse {
                    completions,
    ...
    fn complete_command_argument(
        ...
        *flag = new_cancel_flag.clone();
        if let Some(command) = self.slash_commands.command(command_name, cx) {
            ...
            Task::ready(Ok(vec![carrot_project::CompletionResponse {
                completions: Vec::new(),
                display_options: CompletionDisplayOptions::default(),
                is_incomplete: true,
            }]))
...
impl CompletionProvider for SlashCommandCompletionProvider {
    fn completions(
        ...
        _: carrot_editor::CompletionContext,
        window: &mut Window,
        ...
        let Some((name, arguments, command_range, last_argument_range)) =
            ...
            return Task::ready(Ok(vec![carrot_project::CompletionResponse {
                completions: Vec::new(),
                display_options: CompletionDisplayOptions::default(),
                is_incomplete: false,
            }]));
