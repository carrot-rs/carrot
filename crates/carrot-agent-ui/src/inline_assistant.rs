use carrot_language_model::AnthropicEventData;
use carrot_language_model::report_anthropic_event;
use std::cmp;
use std::mem;
use std::ops::Range;
use std::rc::Rc;
...
use carrot_editor::EditorSnapshot;
use carrot_editor::MultiBufferOffset;
use carrot_editor::RowExt;
use carrot_editor::SelectionEffects;
use carrot_editor::scroll::ScrollOffset;
use carrot_editor::{
...
use carrot_language::{Buffer, Point, Selection, TransactionId};
use carrot_language_model::{ConfigurationError, ConfiguredModel, LanguageModelRegistry};
use carrot_multi_buffer::MultiBufferRow;
use parking_lot::Mutex;
use carrot_project::{CodeAction, DisableAiSettings, LspAction, Project, ProjectTransaction};
use carrot_prompt_store::{PromptBuilder, PromptStore};
use inazuma_settings_framework::{Settings, SettingsStore};

use carrot_terminal_view::{TerminalView, terminal_panel::TerminalPanel};
use inazuma_text::{OffsetRangeExt, ToPoint as _};
use carrot_ui::prelude::*;
use inazuma_util::{RangeExt, ResultExt, maybe};
use carrot_workspace::{ItemHandle, Toast, Workspace, dock::Panel, notifications::NotificationId};
use carrot_actions::agent::OpenSettings;
...
impl InlineAssistant {
    ...
    fn handle_workspace_event(
        &mut self,
        workspace: Entity<Workspace>,
        event: &carrot_workspace::Event,
        window: &mut Window,
        cx: &mut App,
    ) {
        match event {
            carrot_workspace::Event::UserSavedItem { item, .. } => {
            ...
            carrot_workspace::Event::ItemAdded { item } => {
    ...
    pub fn inline_assist(
        ...
        action: &carrot_actions::assistant::InlineAssist,
        window: &mut Window,
        ...
        let handle_assist =
            |window: &mut Window, cx: &mut Context<Workspace>| match inline_assist_target {
                ...
                InlineAssistTarget::Terminal(_active_terminal) => {
                    // TODO(phase20): TerminalInlineAssistant — depends on carrot-terminal-view
                }
            };
            ...
            }))
        {
            ...
            if let Some(model) = LanguageModelRegistry::read_global(cx).inline_assistant_model() {
                ...
                report_anthropic_event(
                    &model.model,
                    AnthropicEventData {
                        completion_type: carrot_language_model::AnthropicCompletionType::Editor,
                        event: carrot_language_model::AnthropicEventType::Invoked,
                        language_name: buffer.language().map(|language| language.name().to_proto()),
                        message_id: None,
                    },
                    cx,
            ...
            if let Some(model) = LanguageModelRegistry::read_global(cx).inline_assistant_model() {
                ...
                let (phase, event_type, anthropic_event_type) = if undo {
                    (
                        ...
                        carrot_language_model::AnthropicEventType::Reject,
                    )
                } else {
                    (
                        "accepted",
                        "Assistant Response Accepted",
                        carrot_language_model::AnthropicEventType::Accept,
                    )
                };
                ...
                report_anthropic_event(
                    &model.model,
                    carrot_language_model::AnthropicEventData {
                        completion_type: carrot_language_model::AnthropicCompletionType::Editor,
        ...
        editor.update(cx, |editor, cx| {
            ...
            for (new_row, old_row_range) in deleted_row_ranges {
                ...
                let deleted_lines_editor = cx.new(|cx| {
                    let multi_buffer =
                        cx.new(|_| MultiBuffer::without_headers(carrot_language::Capability::ReadOnly));
                    ...
                    editor.set_soft_wrap_mode(carrot_language::language_settings::SoftWrap::None, cx);
...
struct EditorInlineAssists {
    ...
    highlight_updates: carrot_watch::Sender<()>,
...
impl EditorInlineAssists {
    fn new(editor: &Entity<Editor>, window: &mut Window, cx: &mut App) -> Self {
        let (highlight_updates_tx, mut highlight_updates_rx) = carrot_watch::channel(());
        Self {
            assist_ids: Vec::new(),
            scroll_lock: None,
            ...
            _subscriptions: vec![
                ...
                editor.update(cx, |editor, cx| {
                    ...
                    editor.register_action(move |_: &carrot_editor::actions::Newline, window, cx| {
                        InlineAssistant::update_global(cx, |this, cx| {
                            if let Some(editor) = editor_handle.upgrade() {
                ...
                editor.update(cx, |editor, cx| {
                    let editor_handle = cx.entity().downgrade();
                    editor.register_action(move |_: &carrot_editor::actions::Cancel, window, cx| {
                        InlineAssistant::update_global(cx, |this, cx| {
                            if let Some(editor) = editor_handle.upgrade() {
    ...
    fn code_actions(
        ...
        if has_diagnostics {
            ...
            Task::ready(Ok(vec![CodeAction {
                server_id: carrot_language::LanguageServerId(0),
                range: snapshot.anchor_before(range.start)..snapshot.anchor_after(range.end),
                lsp_action: LspAction::Action(Box::new(carrot_lsp::CodeAction {
...
pub mod evals {
    ...
    use carrot_editor::{Editor, MultiBuffer, MultiBufferOffset};
    use eval_utils::{EvalOutput, NoProcessor};
    use carrot_fs::FakeFs;
    use futures::channel::mpsc;
    use inazuma::{AppContext, TestAppContext, UpdateGlobal as _};
    use carrot_language::Buffer;
    use carrot_language_model::{LanguageModelRegistry, SelectedModel};
    ...
    where
        SetupF: FnOnce(&mut inazuma::VisualTestContext),
        TestF: FnOnce(&mut inazuma::VisualTestContext),
    {
        let fs = FakeFs::new(cx.executor());
        let app_state = cx.update(|cx| carrot_workspace::AppState::test(cx));
    ...
    fn run_eval(
        ...
        eval_utils::eval(iterations, expected_pass_ratio, NoProcessor, move || {
            let dispatcher = inazuma::TestDispatcher::new(rand::random());
