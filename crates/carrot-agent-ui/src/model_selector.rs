...
use carrot_acp_thread::{AgentModelIcon, AgentModelInfo, AgentModelList, AgentModelSelector};
...
use carrot_ui::{DocumentationAside, DocumentationSide, IntoElement, prelude::*};
...
impl PickerDelegate for ModelPickerDelegate {
    ...
    fn documentation_aside(
        ...
        _window: &mut Window,
        cx: &mut Context<Picker<Self>>,
    ) -> Option<carrot_ui::DocumentationAside> {
...
mod tests {
    ...
    fn create_model_list(grouped_models: Vec<(&str, Vec<&str>)>) -> AgentModelList {
        AgentModelList::Grouped(IndexMap::from_iter(grouped_models.into_iter().map(
            |(group, models)| {
                (
                    carrot_acp_thread::AgentModelGroupName(group.to_string().into()),
                    models
                        .into_iter()
                        .map(|model| carrot_acp_thread::AgentModelInfo {
    ...
    async fn test_fuzzy_match(cx: &mut TestAppContext) {
        ...
        assert_models_eq(
            results,
            vec![("carrot", vec!["gpt-5-mini"]), ("openai", vec!["gpt-5-mini"])],
        );
    ...
    fn test_favorites_section_appears_when_favorites_exist(_cx: &mut TestAppContext) {
        let models = create_model_list(vec![
            ("carrot", vec!["carrot/claude", "carrot/gemini"]),
            ("openai", vec!["openai/gpt-5"]),
        ]);
        let favorites = create_favorites(vec!["carrot/gemini"]);
        ...
        assert_eq!(model_ids[0], "carrot/gemini");
    }

    #[inazuma::test]
    fn test_no_favorites_section_when_no_favorites(_cx: &mut TestAppContext) {
        let models = create_model_list(vec![("carrot", vec!["carrot/claude", "carrot/gemini"])]);
        ...
        assert!(matches!(
            entries.first(),
            Some(ModelPickerEntry::Separator(s)) if s == "carrot"
        ));
    }
    ...
    fn test_models_have_correct_actions(_cx: &mut TestAppContext) {
        ...
        let favorites = create_favorites(vec!["carrot/claude"]);

        let entries = info_list_to_picker_entries(models, &favorites);

        for entry in &entries {
            if let ModelPickerEntry::Model(info, is_favorite) = entry {
                if info.id.0.as_ref() == "carrot/claude" {
                    assert!(is_favorite, "carrot/claude should be a favorite");
                } else {
    ...
    fn test_favorites_appear_in_both_sections(_cx: &mut TestAppContext) {
        ...
        let favorites = create_favorites(vec!["carrot/gemini", "openai/gpt-5"]);
        ...
        assert!(model_ids[2..].contains(&"carrot/gemini"));
    ...
    fn test_favorites_are_not_duplicated_when_repeated_in_other_sections(_cx: &mut TestAppContext) {
        let models = create_model_list(vec![
            ("Recommended", vec!["carrot/claude", "anthropic/claude"]),
            ("Carrot", vec!["carrot/claude", "carrot/gpt-5"]),
    ...
    fn test_flat_model_list_with_favorites(_cx: &mut TestAppContext) {
        let models = AgentModelList::Flat(vec![
            carrot_acp_thread::AgentModelInfo {
                id: acp::ModelId::new("carrot/claude".to_string()),
                name: "Claude".into(),
                description: None,
                icon: None,
                is_latest: false,
            ...
            carrot_acp_thread::AgentModelInfo {
                id: acp::ModelId::new("carrot/gemini".to_string()),
                name: "Gemini".into(),
                description: None,
                icon: None,
                is_latest: false,
