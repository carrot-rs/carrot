use carrot_agent_servers::GEMINI_ID;
use inazuma::{
    ClickEvent, DismissEvent, Entity, EventEmitter, FocusHandle, Focusable, MouseDownEvent, Render,
    linear_color_stop, linear_gradient,
};
use carrot_ui::{TintColor, Vector, VectorName, prelude::*};
...
macro_rules! acp_onboarding_event {
    ($name:expr) => {
        carrot_telemetry::event!($name, source = "ACP Onboarding");
    };
    ($name:expr, $($key:ident $(= $value:expr)?),+ $(,)?) => {
        carrot_telemetry::event!($name, source = "ACP Onboarding", $($key $(= $value)?),+);
    };
}
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let illustration = h_flex()
            ...
            .child(
                div().absolute().inset_0().w(px(515.)).h(px(126.)).child(
                    Vector::new(VectorName::AcpGrid, rems_from_px(515.), rems_from_px(126.))
                        .color(carrot_ui::Color::Custom(cx.theme().colors().text.opacity(0.02))),
                ),
            )
            ...
            .child(
                ...
                .color(carrot_ui::Color::Custom(cx.theme().colors().text.opacity(0.8))),
            )
            .child(
                v_flex()
                    .gap_1p5()
        ...
        let heading = v_flex()
            ...
            .child(Headline::new("Bring Your Own Agent to Carrot").size(HeadlineSize::Large));

        let copy = "Bring the agent of your choice to Carrot via our new Agent Client Protocol (ACP), starting with Google's Gemini CLI integration.";
