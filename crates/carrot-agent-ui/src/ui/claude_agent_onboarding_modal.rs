use carrot_agent_servers::CLAUDE_AGENT_ID;
use inazuma::{
...
macro_rules! claude_agent_onboarding_event {
    ($name:expr) => {
        carrot_telemetry::event!($name, source = "ACP Claude Code Onboarding");
    };
    ($name:expr, $($key:ident $(= $value:expr)?),+ $(,)?) => {
        carrot_telemetry::event!($name, source = "ACP Claude Code Onboarding", $($key $(= $value)?),+);
    };
}
...
impl Render for ClaudeCodeOnboardingModal {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let heading = v_flex()
            ...
            .child(Headline::new("Claude Agent: Natively in Carrot").size(HeadlineSize::Large));

        let copy = "Powered by the Agent Client Protocol, you can now run Claude Agent as\na first-class citizen in Carrot's agent panel.";
