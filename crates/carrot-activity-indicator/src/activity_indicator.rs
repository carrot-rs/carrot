use carrot_auto_update::DismissMessage;
use carrot_editor::Editor;
use carrot_extension_host::{ExtensionOperation, ExtensionStore};
use carrot_language::{
...
use carrot_workspace::{StatusItemView, Workspace, item::ItemHandle};
use futures::StreamExt;
use inazuma::{
    App, Context, CursorStyle, Entity, EventEmitter, Render, SharedString,
    StatefulInteractiveElement, Styled, Window, actions,
};
use inazuma_util::truncate_and_trailoff;
...
struct Content {
    icon: Option<inazuma::AnyElement>,
    message: String,
    on_click:
        ...
        let this = cx.new(|cx| {
            ...
            cx.spawn(async move |this, cx| {
                while let Some(job_event) = job_events.next().await {
                    this.update(cx, |this: &mut ActivityIndicator, cx| {
                        match job_event {
                            carrot_fs::JobEvent::Started { info } => {
                                this.fs_jobs.retain(|j| j.id != info.id);
                                this.fs_jobs.push(info);
                            }
                            carrot_fs::JobEvent::Completed { id } => {
            ...
            cx.subscribe(
                ...
                |activity_indicator, _, event, cx| {
                    if let LspStoreEvent::LanguageServerUpdate { name, message, .. } = event {
                        if let carrot_proto::update_language_server::Variant::StatusUpdate(
                            status_update,
                        ) = message
                        {
                            ...
                            let status = match &status_update.status {
                                Some(carrot_proto::status_update::Status::Binary(
                                    binary_status,
                                )) => {
                                    if let Some(binary_status) =
                                        carrot_proto::ServerBinaryStatus::from_i32(*binary_status)
                                    {
                                        let binary_status = match binary_status {
                                            carrot_proto::ServerBinaryStatus::None => {
                                                BinaryStatus::None
                                            }
                                            carrot_proto::ServerBinaryStatus::CheckingForUpdate => {
                                                BinaryStatus::CheckingForUpdate
                                            }
                                            carrot_proto::ServerBinaryStatus::Downloading => {
                                                BinaryStatus::Downloading
                                            }
                                            carrot_proto::ServerBinaryStatus::Starting => {
                                                BinaryStatus::Starting
                                            }
                                            carrot_proto::ServerBinaryStatus::Stopping => {
                                                BinaryStatus::Stopping
                                            }
                                            carrot_proto::ServerBinaryStatus::Stopped => {
                                                BinaryStatus::Stopped
                                            }
                                            carrot_proto::ServerBinaryStatus::Failed => {
                                ...
                                Some(carrot_proto::status_update::Status::Health(
                                    health_status,
                                )) => {
                                    if let Some(health) =
                                        carrot_proto::ServerHealth::from_i32(*health_status)
                                    {
                                        let health = match health {
                                            carrot_proto::ServerHealth::Ok => ServerHealth::Ok,
                                            carrot_proto::ServerHealth::Warning => {
                                                ServerHealth::Warning
                                            }
                                            carrot_proto::ServerHealth::Error => {
                                                ServerHealth::Error
                                            }
                                        };
            ...
            cx.subscribe(
                ...
                |_, _, event: &GitStoreEvent, cx| {
                    if let carrot_project::git_store::GitStoreEvent::JobsUpdated = event {
                        cx.notify()
                    }
                },
            )
        ...
        if let Some(message) = self.pending_environment_error(cx) {
            return Some(Content {
                ...
                on_click: Some(Arc::new(move |this, window, cx| {
                    ...
                    window.dispatch_action(Box::new(carrot_workspace::OpenLog), cx);
                })),
