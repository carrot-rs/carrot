...
use carrot_auto_update::AutoUpdater;
use carrot_markdown::{Markdown, MarkdownElement, MarkdownStyle};
use carrot_release_channel::ReleaseChannel;
use carrot_remote::{ConnectionIdentifier, RemoteClient, RemoteConnectionOptions, RemotePlatform};
...
use carrot_ui_input::{ERASED_EDITOR_FACTORY, ErasedEditor};
...
use inazuma::{
    ...
    Render, SharedString, Task, TextStyleRefinement, WeakEntity,
};
...
impl carrot_remote::RemoteClientDelegate for RemoteClientDelegate {
    fn ask_password(
        &self,
        prompt: String,
        tx: oneshot::Sender<EncryptedPassword>,
...
pub fn connect(
    ...
    cx.spawn(async move |cx| {
        let connection = carrot_remote::connect(connection_options, delegate.clone(), cx);
        ...
        cx.update(|cx| {
            carrot_remote::RemoteClient::new(unique_identifier, connection, rx, delegate, cx)
        })
        .await
    })
}
