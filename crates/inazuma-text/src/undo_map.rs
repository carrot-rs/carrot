...
impl inazuma_sum_tree::Item for UndoMapEntry {
...
impl inazuma_sum_tree::KeyedItem for UndoMapEntry {
    type Key = UndoMapKey;
...
struct UndoMapKey {
    edit_id: inazuma_clock::Lamport,
    undo_id: inazuma_clock::Lamport,
}

impl inazuma_sum_tree::ContextLessSummary for UndoMapKey {
    fn zero() -> Self {
        UndoMapKey {
            edit_id: Lamport::MIN,
            undo_id: Lamport::MIN,
...
impl UndoMap {
    pub fn insert(&mut self, undo: &UndoOperation) {
        let edits = undo
            ...
            .iter()
            .map(|(edit_id, count)| {
                inazuma_sum_tree::Edit::Insert(UndoMapEntry {
                    key: UndoMapKey {
                        edit_id: *edit_id,
    ...
    pub fn is_undone(&self, edit_id: inazuma_clock::Lamport) -> bool {
    ...
    pub fn undo_count(&self, edit_id: inazuma_clock::Lamport) -> u32 {
        let mut cursor = self.0.cursor::<UndoMapKey>(());
        cursor.seek(
            &UndoMapKey {
                edit_id,
