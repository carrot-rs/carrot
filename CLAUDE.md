...
### Font Architecture (see ARCHITECTURE.md)
- **Two slots, three roles, one resolver.** User-facing config has exactly two pickers (`theme.fonts.ui` / `theme.fonts.mono`). Internally, code distinguishes three roles — `FontRole::{Body, Code, Terminal}` — that all funnel through a single resolver in `carrot-theme`. The mapping (`Body → ui`, `Code|Terminal → mono`) lives in one place; everything else asks for a role.
- **Niemals direkt auf `theme.fonts.{ui,mono}` aus Call-Sites zugreifen** — immer durch `body_font(cx)` / `code_font(cx)` / `terminal_font(cx)` Convenience-Functions in `carrot_theme`. Plus `body_font_size(cx)` / `code_font_size(cx)` / `terminal_font_size(cx)` und `symbol_map_for(role, cx)`. Damit bleibt der Resolver der einzige Touchpoint, und spätere Override-Slots (z.B. `[theme.fonts.overrides.terminal]`) brauchen keine Call-Site-Updates.
- **Surface-spezifische Font-Settings sind verboten.** Density-Decisions (Agent-Panel kompakter, Tooltip kleiner, Notification kleiner) leben als Multiplikator im Component-Layer (`body_font_size(cx) * 0.85`) oder als späterer `density.{compact,comfortable}` Token. Niemals als neue Font-Setting-Section unter `[theme.fonts]`. Das alte `theme.agent_*_font_size` ist genau diese Anti-Pattern, deshalb gestrichen.
- **Font-Achsen sind in Inazuma's `Font`-Struct vollständig auszudrücken.** Wenn ein OpenType-Feature (Width / Weight / Style / optical size) in einer Font existiert und gebundled ist, muss es über das `Font`-Struct adressierbar sein — nicht über Family-Name-Tricks (`family = "CRT-02 Condensed"` als Hack) oder Backend-Hardcodes. Plattform-Patches an allen 4 Backends simultan (deckt sich mit existierender Hard Rule „For platform-layer changes, update all platforms simultaneously").
- **Default-Font-Switches passieren in `font_name_with_fallbacks` (`crates/inazuma/src/text_system/font_types.rs`).** Two-Liner: `.CarrotSans → "<new family>"`, `.CarrotMono → "<new family>"`. `default.toml` bleibt bei den Aliases — keine User-Config-Migration je wieder nötig.
